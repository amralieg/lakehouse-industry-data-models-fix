...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`member` (
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: member.current_tier is currently a DECIMAL(18,2) — a denormalized scalar value that should be a proper FK reference to loyalty.tier. Adding current_tier_id (BIGINT) as a FK to loyalty.tier.tier_id nor',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`points_ledger` (
    ...
    `accrual_rule_id` DECIMAL(18,2) COMMENT 'Foreign key linking to loyalty.accrual_rule. Business justification: points_ledger is the financial-grade immutable record of every points movement. For accrual entries, the specific accrual_rule that governed the points calculation (rate, multiplier, eligibility) shou',
    ...
    `offer_id` BIGINT COMMENT 'Foreign key linking to loyalty.offer. Business justification: points_ledger records every points movement including bonus points triggered by offer redemptions (e.g., double-points offers, bonus point campaigns). Without an offer_id FK, there is no way to trace ',
    ...
    `pos_terminal_id` BIGINT COMMENT 'Foreign key linking to restaurant.pos_terminal. Business justification: Points are earned at a specific POS terminal during a transaction. POS-level points accrual auditing, fraud detection, and terminal reconciliation reports require knowing which terminal processed each',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`reward` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Rewards (free items, discounts) are brand-specific — a reward valid at Brand A cannot be redeemed at Brand B. Brand-level reward catalog management and redemption cost accounting require this FK. The ',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Reward catalog management requires tracking which employee created each reward for audit, accountability, and content governance. The existing created_by plain-text field is denormalized; this FK enab',
    `combo_meal_id` BIGINT COMMENT 'Foreign key linking to menu.combo_meal. Business justification: Combo Meal Reward Fulfillment: rewards frequently grant free combo meals (a distinct product from menu_item). POS and OLO systems must resolve which combo_meal_id to fulfill. Role-prefix free_combo_m',
    ...
    `modifier_group_id` BIGINT COMMENT 'Foreign key linking to menu.modifier_group. Business justification: Modifier Upgrade Reward: a common restaurant loyalty reward type grants a free modifier upgrade (e.g., free size upgrade, free add-on). The POS must resolve which modifier_group the reward applies to.',
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Rewards in the catalog belong to a specific loyalty program — points costs, redemption rules, and availability are program-specific. Without program_id on reward, the reward catalog cannot be scoped t',
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: When a loyalty reward is a free menu item, the kitchen and inventory system must know which stock_item to pull and decrement. This FK enables COGS tracking for free-item rewards and inventory deductio',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`redemption` (
    ...
    `combo_meal_id` BIGINT COMMENT 'Foreign key linking to menu.combo_meal. Business justification: Combo Meal Redemption Tracking: when a loyalty reward for a combo meal is redeemed, operations and finance need to track which combo_meal was fulfilled for PMIX reporting and food cost reconciliation.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`accrual_rule` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Loyalty accrual rule approval is a compliance and change-management process requiring an auditable link to the approving employee. The existing approved_by plain-text field is denormalized; this FK en',
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Accrual rules are scoped to specific brands in multi-brand enterprises. Brand managers need to query which earning rules apply to their brand for loyalty cost reporting and P&L attribution. No existin',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`offer` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Offers are created and governed by specific restaurant brands in a multi-brand portfolio. Brand marketing teams manage offer catalogs per brand for campaign planning and promotional ROI reporting. No ',
    ...
    `menu_item_id` BIGINT COMMENT 'Foreign key linking to menu.menu_item. Business justification: Free Item Offer Fulfillment: when a loyalty offer grants a free specific menu item, the POS/OLO system must resolve the exact menu_item_id to apply the discount. free_item_sku is a denormalized refere',
    `menu_id` BIGINT COMMENT 'Foreign key linking to menu.menu. Business justification: Menu-Scoped Offer Configuration: loyalty offers are scoped to specific menus (e.g., breakfast menu, digital menu, franchise menu). Marketing and operations teams configure and report on which menu an ',
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Offers are scoped to a specific loyalty program — a Bronze/Silver/Gold offer structure for Program A is distinct from Program Bs offers. Without program_id on offer, there is no way to filter or asso',
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: Promotional offers with a free item (e.g., free fries with purchase) require inventory planning — operations must know which stock_item to pre-position and how much to order. This FK links the offer',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Supplier-funded promotional offers (e.g., bonus points on a suppliers featured product) are governed by a supplier contract specifying funding terms, redemption caps, and reimbursement. This FK enabl',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`program` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: A loyalty program is owned and operated by a specific brand. Brand governance, program P&L reporting, and franchise agreement compliance all require knowing which brand owns each program. The existing',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` (
    `offer_distribution_id` BIGINT COMMENT 'Primary key for the offer_distribution association',
    `member_id` BIGINT COMMENT 'Foreign key linking to the loyalty member who received the offer distribution.',
    `offer_id` BIGINT COMMENT 'Foreign key linking to the loyalty offer that was distributed to the member.',
    `channel` STRING COMMENT 'The specific channel through which this offer was distributed to this member. May differ from the offer-level distribution_channel when personalized channel routing is applied. Sourced from detection phase distribution_channel and offer description assignment_channel.',
    `distributed_at` TIMESTAMP COMMENT 'Timestamp when the offer was distributed (sent) to the member. Marks the start of the distribution lifecycle. Sourced from detection phase distribution_date.',
    `distribution_channel` STRING COMMENT 'Primary channel through which the offer is communicated to members: push_notification, email, in_app (mobile app banner), sms, pos_display (printed receipt or screen), direct_mail. [Moved from offer: The offer-level distribution_channel captures the primary/default channel for the offer campaign. However, the per-member assignment channel (which may be personalized) belongs to the offer_distribution association. The offer master should retain distribution_channel as the campaign-level default; the association captures the actual per-member delivery channel.]. Valid values are `push_notification|email|in_app|sms|pos_display|direct_mail`',
    `is_viewed` BOOLEAN COMMENT 'Flag indicating whether the member has viewed (opened) this offer in their offer wallet or via the delivery channel. Sourced directly from detection phase is_viewed_flag.',
    `member_expiry_date` DATE COMMENT 'Member-specific expiration date for this offer assignment. May differ from the offer-level end_date due to personalized expiry windows. Sourced from detection phase expiration_date and offer description member-level expiry.',
    `personalization_score` DECIMAL(18,2) COMMENT 'Numeric score representing the degree of personalization applied to this specific offer-member assignment, used for targeting model evaluation and A/B testing. Sourced directly from detection phase personalization_score.',
    `personalized_flag` BOOLEAN COMMENT 'Indicates whether this offer is personalized to individual members based on purchase history and preferences (True) or is a mass offer distributed to all eligible members (False). [Moved from offer: This boolean on the offer master indicates whether the offer is personalization-eligible at the campaign level. The actual per-member personalization score and personalization state belong to the offer_distribution association record, not the offer master. The offer master flag can be retained as a campaign-level indicator, but the per-member personalization detail moves to the association.]',
    `redeemed_at` TIMESTAMP COMMENT 'Timestamp when the member redeemed this specific offer distribution. Null if not yet redeemed. Belongs to the distribution record, not the offer master.',
    `redemption_status` STRING COMMENT 'Current lifecycle state of this specific offer-member distribution record. Tracks the full funnel from distribution through redemption or expiry. Sourced directly from detection phase redemption_status and offer description delivery_status.',
    CONSTRAINT pk_offer_distribution PRIMARY KEY(`offer_distribution_id`)
) COMMENT 'This association product represents the Distribution Event between a loyalty offer and a loyalty member. It captures the full lifecycle of a single offer being assigned and delivered to a specific member — from initial distribution through delivery, engagement, and final redemption or expiry. Each record links one offer to one member and carries attributes that exist only in the context of that specific offer-member assignment: when it was sent, through which channel, whether the member viewed it, its member-specific expiry, and its current redemption status. This is the operational record that powers the member offer wallet, delivery tracking, and redemption lift analytics. Sourced from Salesforce CRM campaign execution.. Existence Justification: In restaurant loyalty operations, the marketing team actively distributes specific offers to specific members as a first-class business process — this is not derivable from redemption data alone. An offer can be distributed to thousands of members, and a member can hold multiple active offers in their offer wallet simultaneously. The distribution event (offer sent → delivered → viewed → redeemed/expired) has its own lifecycle, attributes, and business name (offer distribution or member offer assignment) that neither the offer master nor the member master can capture.';
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ADD CONSTRAINT `fk_loyalty_offer_distribution_member_id` FOREIGN KEY (`member_id`) REFERENCES `vibe_restaurants_v1`.`loyalty`.`member`(`member_id`);
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ADD CONSTRAINT `fk_loyalty_offer_distribution_offer_id` FOREIGN KEY (`offer_id`) REFERENCES `vibe_restaurants_v1`.`loyalty`.`offer`(`offer_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`points_ledger` ALTER COLUMN `pos_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Pos Terminal Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`reward` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Employee Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`reward` ALTER COLUMN `combo_meal_id` SET TAGS ('dbx_business_glossary_term' = 'Free Combo Meal Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`reward` ALTER COLUMN `modifier_group_id` SET TAGS ('dbx_business_glossary_term' = 'Modifier Group Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`reward` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`reward` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Item Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`redemption` ALTER COLUMN `combo_meal_id` SET TAGS ('dbx_business_glossary_term' = 'Redeemed Combo Meal Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer` SET TAGS ('dbx_subdomain' = 'offer_distribution');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer` ALTER COLUMN `menu_id` SET TAGS ('dbx_business_glossary_term' = 'Menu Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Item Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_redemption` SET TAGS ('dbx_subdomain' = 'offer_distribution');
...
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` SET TAGS ('dbx_subdomain' = 'offer_distribution');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` SET TAGS ('dbx_association_edges' = 'loyalty.offer,loyalty.member');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `offer_distribution_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Distribution - Offer Distribution Id');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Distribution - Member Id');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `offer_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Distribution - Offer Id');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `channel` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `distributed_at` SET TAGS ('dbx_business_glossary_term' = 'Distribution Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `distribution_channel` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `distribution_channel` SET TAGS ('dbx_value_regex' = 'push_notification|email|in_app|sms|pos_display|direct_mail');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `is_viewed` SET TAGS ('dbx_business_glossary_term' = 'Offer Viewed Flag');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `member_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Member Offer Expiry Date');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `personalization_score` SET TAGS ('dbx_business_glossary_term' = 'Personalization Score');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `personalized_flag` SET TAGS ('dbx_business_glossary_term' = 'Personalized Flag');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `redeemed_at` SET TAGS ('dbx_business_glossary_term' = 'Redemption Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`loyalty`.`offer_distribution` ALTER COLUMN `redemption_status` SET TAGS ('dbx_business_glossary_term' = 'Offer Redemption Status');
