...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` (
    `promotion_accrual_id` DECIMAL(18,2) COMMENT 'Unique identifier for the promotion accrual record. Primary key.',
    `employee_id` BIGINT COMMENT 'Name or identifier of the user who approved the accrual. Nullable for unapproved accruals.',
    `finance_accrual_id` DECIMAL(18,2) COMMENT 'Cross-reference to canonical finance accrual record',
    ...
    `accrual_context` STRING COMMENT 'Promotion-specific accrual context',
    ...
    `accrual_rate` DECIMAL(18,2) COMMENT '',
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'Cost center responsible for the promotional spend, used for internal cost allocation and P&L reporting.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` (
    `promotion_deduction_id` DECIMAL(18,2) COMMENT 'Unique identifier for the promotion deduction record. Primary key for the promotion deduction entity representing the complete lifecycle from initial short-payment through final settlement.',
    ...
    `deduction_reason_code` STRING COMMENT '',
    `deduction_source` DECIMAL(18,2) COMMENT 'Source system or channel through which the deduction was received. Retailer portal indicates web-based submission, EDI 846 indicates electronic data interchange, manual entry indicates internal creation, bank reconciliation indicates discovery during payment matching, and automated matching indicates system-generated from remittance data.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`deduction_settlement` (
    ...
    `promotion_deduction_id` DECIMAL(18,2) COMMENT 'Reference to the originating retailer deduction that this settlement resolves. Links to the deduction record in the trade promotion management system.',
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'The cost center responsible for this settlement expense. Used for internal cost allocation and profitability analysis by organizational unit.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_rebate_agreement` (
    `promotion_rebate_agreement_id` DECIMAL(18,2) COMMENT 'Unique identifier for the promotion rebate agreement. Primary key for this entity.',
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Rebate expenses are allocated to a cost center for budgeting and expense control, a standard finance practice.',
    ...
    `profit_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.profit_center. Business justification: Tracks rebate impact on profit centers, enabling profit‑center performance reporting and rebate ROI analysis.',
    ...
    `payment_frequency` DECIMAL(18,2) COMMENT 'How often rebate payments are made to the retailer. May differ from measurement period (e.g., measured monthly but paid quarterly).',
    `payment_method` DECIMAL(18,2) COMMENT 'Method by which rebate payments are remitted. Check is physical check, ACH is Automated Clearing House electronic transfer, wire transfer is same-day bank transfer, credit memo is applied to customer account balance, and offset invoice reduces future invoices.',
    `payment_terms_days` DECIMAL(18,2) COMMENT 'Number of days after settlement calculation when payment is due to the retailer. Standard terms are 30, 45, or 60 days.',
    ...
    `sap_rebate_agreement_number` DECIMAL(18,2) COMMENT 'SAP SD rebate agreement number for financial integration and payment processing. Links to SAP FI/CO for accrual and settlement posting.',
    `source_system_code` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`rebate_settlement` (
    `rebate_settlement_id` DECIMAL(18,2) COMMENT 'Unique identifier for the rebate settlement transaction. Primary key for the rebate settlement record.',
    ...
    `promotion_rebate_agreement_id` DECIMAL(18,2) COMMENT 'Reference to the parent rebate agreement under which this settlement is calculated. Links to the master rebate agreement that defines terms, tiers, and conditions.',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'Method by which the rebate payment is issued to the trade partner. Defines the financial instrument used for settlement.',
    `payment_reference_number` DECIMAL(18,2) COMMENT 'External reference number from the payment system or financial institution. Links this settlement to the actual payment transaction in SAP FI or banking system.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`post_event_analysis` (
    ...
    `incremental_revenue_amount` DECIMAL(18,2) COMMENT '',
    `incremental_volume_units` DECIMAL(18,2) COMMENT '',
    ...
    `lift_percentage` DECIMAL(18,2) COMMENT '',
    ...
    `source_system_code` STRING COMMENT '',
    `total_spend_amount` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`promoted_price` (
    ...
    `regular_price` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`lift_measurement` (
    ...
    `promotion_event_id` BIGINT COMMENT '',
    ...
    `actual_units` DECIMAL(18,2) COMMENT '',
    ...
    `baseline_units` DECIMAL(18,2) COMMENT '',
    ...
    `lift_units` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` (
    ...
    `tpo_baseline_scenario_id` BIGINT COMMENT 'Reference to the baseline or control TPO scenario against which this scenario is compared for incremental analysis.',
    ...
    `predicted_roi_percentage` DECIMAL(18,2) COMMENT '',
    `predicted_spend_amount` DECIMAL(18,2) COMMENT '',
    `predicted_volume_units` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`pos_redemption` (
    ...
    `redeemed_units` DECIMAL(18,2) COMMENT '',
    `redemption_amount` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` (
    ...
    `offer_value_amount` DECIMAL(18,2) COMMENT '',
    `offer_value_percentage` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` (
    ...
    `trade_promotion_id` BIGINT COMMENT '',
    `actual_spend_amount` DECIMAL(18,2) COMMENT 'Actual spend against this allocation',
    ...
    `allocated_budget` DECIMAL(18,2) COMMENT 'Allocated budget amount',
    `allocated_budget_amount` DECIMAL(18,2) COMMENT '',
    `allocated_spend_amount` DECIMAL(18,2) COMMENT 'Spend allocated to this promotion_event within the specific campaign_flight',
    `allocated_volume_units` DECIMAL(18,2) COMMENT '',
    `allocation_amount` DECIMAL(18,2) COMMENT '',
    ...
    `allocation_type` STRING COMMENT 'Type of allocation (fixed, percentage, volume-based)',
    `approval_date` DATE COMMENT 'Date the allocation was approved',
    `approval_status` STRING COMMENT 'Approval status of allocation',
    `approved_by` STRING COMMENT 'Person who approved the allocation',
    `approved_date` DATE COMMENT 'Date the allocation was approved',
    `budget_amount` DECIMAL(18,2) COMMENT 'Budget amount allocated to this flight',
    ...
    `flight_end_date` DATE COMMENT '',
    `flight_start_date` DATE COMMENT '',
    `incremental_volume_estimate` DECIMAL(18,2) COMMENT 'Estimated incremental volume from this allocation',
    `is_active` BOOLEAN COMMENT '',
    `is_approved` BOOLEAN COMMENT 'Whether the allocation has been approved',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the record was last updated',
    `lift_percentage` DECIMAL(18,2) COMMENT 'Expected or actual volume lift percentage',
    ...
    `roi_estimate` DECIMAL(18,2) COMMENT 'Estimated return on investment',
    `roi_ratio` DECIMAL(18,2) COMMENT 'Return on investment ratio for the allocation',
    `source_system_code` STRING COMMENT '',
    `start_date` DATE COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `variance_amount` DECIMAL(18,2) COMMENT 'Variance between planned and actual',
    `variance_percent` DECIMAL(18,2) COMMENT 'Variance percentage',
    `flight_allocation_status` STRING COMMENT 'Lifecycle status of the flight_allocation record',
    `expiration_date` DATE COMMENT 'Date the flight_allocation expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the flight_allocation',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the flight_allocation',
    `flight_allocation_description` STRING COMMENT 'Free-text description of the flight_allocation',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the flight_allocation was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the flight_allocation was last updated',
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` ADD CONSTRAINT `fk_promotion_tpo_scenario_tpo_baseline_scenario_id` FOREIGN KEY (`tpo_baseline_scenario_id`) REFERENCES `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario`(`tpo_scenario_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`trade_promotion` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Owner User ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` SET TAGS ('dbx_ssot_owner' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` SET TAGS ('dbx_ssot_role' = 'canonical');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` SET TAGS ('dbx_ssot_reference' = 'marketing.marketing_event');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_ssot_superseded_by' = 'marketing.marketing_event');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_ssot_status' = 'duplicate_resolved');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` ALTER COLUMN `event_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_event` ALTER COLUMN `event_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`event_sku` ALTER COLUMN `label_version_id` SET TAGS ('dbx_business_glossary_term' = 'Label Version Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`event_sku` ALTER COLUMN `lot_batch_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Batch Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`event_sku` ALTER COLUMN `regulatory_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Registration Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`event_sku` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`trade_spend_allocation` SET TAGS ('dbx_subdomain' = 'trade_planning');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`trade_spend_allocation` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`funding_agreement` SET TAGS ('dbx_subdomain' = 'trade_planning');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`funding_agreement` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` SET TAGS ('dbx_subdomain' = 'financial_settlement');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` SET TAGS ('dbx_ssot_owner' = 'promotion');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` SET TAGS ('dbx_accrual_scope' = 'trade_promotion');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` SET TAGS ('dbx_ssot_role' = 'promotion_accrual_view');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` SET TAGS ('dbx_ssot_reference' = 'finance.finance_accrual');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `promotion_accrual_id` SET TAGS ('dbx_ssot_superseded_by' = 'finance.finance_accrual');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `promotion_accrual_id` SET TAGS ('dbx_ssot_status' = 'duplicate_resolved');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `finance_accrual_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `accrual_number` SET TAGS ('dbx_business_glossary_term' = 'Accrual Number');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Accrual Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `baseline_volume` SET TAGS ('dbx_business_glossary_term' = 'Baseline Volume');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `deduction_claim_number` SET TAGS ('dbx_business_glossary_term' = 'Deduction Claim Number');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `dispute_reason` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `gmroi` SET TAGS ('dbx_business_glossary_term' = 'Gross Margin Return on Investment (GMROI)');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `incremental_volume` SET TAGS ('dbx_business_glossary_term' = 'Incremental Volume');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `is_disputed` SET TAGS ('dbx_business_glossary_term' = 'Is Disputed');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `payment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Reference Number');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Accrual Period End Date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Accrual Period Start Date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `profit_center_code` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Code');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `profit_center_code` SET TAGS ('dbx_value_regex' = '^PC[0-9]{6}$');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `quantity_sold` SET TAGS ('dbx_business_glossary_term' = 'Quantity Sold');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `rate` SET TAGS ('dbx_business_glossary_term' = 'Accrual Rate');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `recognition_date` SET TAGS ('dbx_business_glossary_term' = 'Recognition Date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `reversal_date` SET TAGS ('dbx_business_glossary_term' = 'Reversal Date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `roi_percentage` SET TAGS ('dbx_business_glossary_term' = 'Return on Investment (ROI) Percentage');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `sap_co_document_number` SET TAGS ('dbx_business_glossary_term' = 'SAP Controlling (CO) Document Number');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `sap_co_document_number` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `sap_fi_document_number` SET TAGS ('dbx_business_glossary_term' = 'SAP Financial Accounting (FI) Document Number');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `sap_fi_document_number` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_accrual` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_value_regex' = 'each|case|pallet|kilogram|liter');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` ALTER COLUMN `retailer_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` ALTER COLUMN `retailer_contact_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` ALTER COLUMN `retailer_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_deduction` ALTER COLUMN `retailer_contact_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`deduction_settlement` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_rebate_agreement` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_rebate_agreement` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_rebate_agreement` ALTER COLUMN `agreement_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promotion_rebate_agreement` ALTER COLUMN `agreement_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`rebate_settlement` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`rebate_settlement` ALTER COLUMN `tertiary_rebate_created_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`rebate_settlement` ALTER COLUMN `tertiary_rebate_created_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`rebate_settlement` ALTER COLUMN `tertiary_rebate_created_by_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`post_event_analysis` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`promoted_price` SET TAGS ('dbx_subdomain' = 'consumer_incentives');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`lift_measurement` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` ALTER COLUMN `tpo_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Promotion Optimization (TPO) Scenario ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` ALTER COLUMN `tpo_baseline_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Baseline Scenario ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` ALTER COLUMN `scenario_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`tpo_scenario` ALTER COLUMN `scenario_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`retailer_compliance` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`retailer_compliance` ALTER COLUMN `auditor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`retailer_compliance` ALTER COLUMN `auditor_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`baseline_volume` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`baseline_volume` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`pos_redemption` SET TAGS ('dbx_subdomain' = 'consumer_incentives');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`pos_redemption` ALTER COLUMN `clearinghouse_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`pos_redemption` ALTER COLUMN `clearinghouse_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` SET TAGS ('dbx_subdomain' = 'consumer_incentives');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `tertiary_consumer_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `tertiary_consumer_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `tertiary_consumer_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `offer_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`consumer_offer` ALTER COLUMN `offer_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` SET TAGS ('dbx_subdomain' = 'consumer_incentives');
...
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_status` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_status` SET TAGS ('dbx_classification' = 'status');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_status` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `flight_allocation_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`promotion`.`flight_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
