...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`store_planogram` (
    ...
    `merchandising_planogram_id` BIGINT COMMENT 'FK to the authoritative merchandising planogram design record (SSOT owner)',
...
) COMMENT '[References SSOT: merchandising.merchandising_planogram] Authoritative source for in-store planogram definitions. Defines shelf layout, fixture assignments, and SKU placement for store execution and compliance tracking.';
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`pl` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`remodel` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`audit` (
    ...
    `department_id` BIGINT COMMENT 'Foreign key linking to store.department. Business justification: Operational audits are frequently department-scoped (e.g., food safety audit for deli department, pharmacy compliance audit, licensed department inspection). audit already has location_id for store-le',
    ...
    `scope` STRING COMMENT 'Description of the areas, departments, or processes covered by this audit (e.g., Grocery Department - Refrigeration Units, Full Store - All Departments, Pharmacy - Controlled Substances).',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`dsd_receiving` (
    ...
    `department_id` BIGINT COMMENT 'Foreign key linking to store.department. Business justification: Direct Store Delivery (DSD) receipts are directed to specific store departments (e.g., beverages to the drinks department, snacks to the grocery department). dsd_receiving has location_id and receivin',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`license` (
    ...
    `department_id` BIGINT COMMENT 'Foreign key linking to store.department. Business justification: Licenses and permits are often department-specific within a store (e.g., pharmacy license, liquor license for the wine department, deli/food handler permits). The department table already has licensed',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` (
    `format_offer_eligibility_id` BIGINT COMMENT 'Unique identifier for the format-offer eligibility record',
    `category_id` BIGINT COMMENT 'Reference to the merchandise category this eligibility is scoped to',
    `associate_id` BIGINT COMMENT 'Associate who approved or last reviewed this eligibility rule.',
    `format_associate_id` BIGINT COMMENT 'Reference to the associate who configured this eligibility rule',
    `format_id` BIGINT COMMENT 'Reference to the store format this eligibility applies to',
    `price_zone_id` BIGINT COMMENT 'Price zone that determines localized offer terms for this format.',
    `promo_campaign_id` BIGINT COMMENT 'Reference to the parent promotional campaign',
    `promo_offer_id` BIGINT COMMENT 'Reference to the promotional offer being evaluated for eligibility',
    `region_id` BIGINT COMMENT 'Foreign key linking to store.region. Business justification: format_offer_eligibility has geo_region_code and region_code as STRING attributes for geographic restriction of offer eligibility. Normalizing region_code to region_id -> store.region.region_id establ',
    `sku_id` BIGINT COMMENT 'Reference to a specific SKU if eligibility is item-level',
    `approval_status` STRING COMMENT 'Approval workflow status for the eligibility rule (pending, approved, rejected).',
    `approved_by` STRING COMMENT 'Name or identifier of the person who approved this eligibility',
    `approved_timestamp` TIMESTAMP COMMENT 'Timestamp when the eligibility was approved',
    `budget_cap_amount` DECIMAL(18,2) COMMENT 'Maximum budget allocated for this offer in this format',
    `budget_consumed_amount` DECIMAL(18,2) COMMENT 'Total promotional budget consumed to date for this format-offer combination.',
    `channel_applicability` STRING COMMENT 'Channels where this eligibility applies (e.g. in_store, online, omni)',
    `channel_restriction` STRING COMMENT 'Channel restriction for the offer within this format (in_store_only, online_only, omni).',
    `compliance_approved_flag` BOOLEAN COMMENT 'Indicates whether the offer has passed compliance review for this format.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this eligibility record was created',
    `cumulative_redemption_count` STRING COMMENT 'Running count of total redemptions under this rule',
    `currency_code` STRING COMMENT 'ISO currency code for monetary values',
    `customer_segment_restriction` STRING COMMENT 'Customer segment codes that restrict eligibility',
    `day_of_week_restriction` STRING COMMENT 'Days of the week when the offer is valid (e.g. mon,tue,wed)',
    `display_priority` STRING COMMENT 'Display priority order for the offer within the store format POS or signage system.',
    `effective_end_date` DATE COMMENT 'Date when this eligibility rule expires',
    `effective_start_date` DATE COMMENT 'Date when this eligibility rule becomes active',
    `eligibility_flag` BOOLEAN COMMENT 'Indicates whether the offer is eligible for this format',
    `eligibility_status` STRING COMMENT 'Current status of the eligibility rule (e.g. active, suspended, expired)',
    `exclusion_reason` STRING COMMENT 'Reason why an offer was excluded from a format if not eligible',
    `exclusion_reason_code` STRING COMMENT 'Reason code if the format is excluded from the offer (e.g., regulatory, capacity, brand_fit).',
    `geo_region_code` STRING COMMENT 'Geographic region code for regional eligibility restrictions',
    `last_evaluation_date` DATE COMMENT 'Date when eligibility was last evaluated or refreshed.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp of the last modification to any field',
    `localized_discount_value` DECIMAL(18,2) COMMENT 'Discount value adjusted for the specific format or market',
    `localized_minimum_purchase` DECIMAL(18,2) COMMENT 'Minimum purchase threshold adjusted for the format',
    `max_discount_cap_amount` DECIMAL(18,2) COMMENT 'Maximum discount amount that can be applied per transaction',
    `max_redemption_per_store` STRING COMMENT 'Maximum number of redemptions allowed per store in this format',
    `minimum_basket_amount` DECIMAL(18,2) COMMENT 'Minimum basket value required to activate the offer',
    `notes` STRING COMMENT 'Free-text notes about this eligibility configuration',
    `offer_type` STRING COMMENT 'Type of promotional offer such as percentage_off, bogo, fixed_amount, or bundle.',
    `override_flag` BOOLEAN COMMENT 'Indicates if standard eligibility rules were overridden',
    `priority_rank` STRING COMMENT 'Priority ranking when multiple eligibility rules conflict',
    `redemption_count` STRING COMMENT 'Current count of redemptions for this offer within this format.',
    `requires_loyalty_membership_flag` BOOLEAN COMMENT 'Indicates if loyalty membership is required for eligibility',
    `seasonal_flag` BOOLEAN COMMENT 'Indicates whether this eligibility is tied to a specific seasonal period.',
    `stacking_allowed_flag` BOOLEAN COMMENT 'Indicates if this offer can be stacked with other promotions',
    `target_audience` STRING COMMENT 'Target audience for the offer such as all_customers, loyalty_members, or new_customers.',
    `tier_requirement` STRING COMMENT 'Loyalty tier required for eligibility (e.g. gold, platinum)',
    `time_window_end` STRING COMMENT 'End time of the daily window when the offer is valid',
    `time_window_start` STRING COMMENT 'Start time of the daily window when the offer is valid',
    `total_budget_allocated_amount` DECIMAL(18,2) COMMENT 'Total budget allocated across all stores in this format',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to this record',
    `validation_rule_expression` STRING COMMENT 'Business rule expression for complex eligibility validation',
    CONSTRAINT pk_format_offer_eligibility PRIMARY KEY(`format_offer_eligibility_id`)
) COMMENT 'Defines which promotional offers are eligible for specific store formats, controlling offer availability by format type with localized discount values, redemption limits, and temporal restrictions.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`carrier_agreement` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`sales_territory` (
    ...
    `associate_id` BIGINT COMMENT 'Reference to the employee who manages this sales territory. Responsible for territory performance and team oversight.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`store`.`region` (
    ...
    `price_zone_code` STRING COMMENT 'Code identifying the pricing zone assigned to this region. Price zones group stores with similar competitive and cost structures to enable zone-based pricing strategies and promotional pricing rules.',
...
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ADD CONSTRAINT `fk_store_store_planogram_department_id` FOREIGN KEY (`department_id`) REFERENCES `vibe_retail_v1`.`store`.`department`(`department_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ADD CONSTRAINT `fk_store_store_planogram_format_id` FOREIGN KEY (`format_id`) REFERENCES `vibe_retail_v1`.`store`.`format`(`format_id`);
...
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ADD CONSTRAINT `fk_store_pl_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
...
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ADD CONSTRAINT `fk_store_remodel_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ADD CONSTRAINT `fk_store_remodel_format_id` FOREIGN KEY (`format_id`) REFERENCES `vibe_retail_v1`.`store`.`format`(`format_id`);
...
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ADD CONSTRAINT `fk_store_audit_department_id` FOREIGN KEY (`department_id`) REFERENCES `vibe_retail_v1`.`store`.`department`(`department_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ADD CONSTRAINT `fk_store_audit_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ADD CONSTRAINT `fk_store_audit_previous_audit_id` FOREIGN KEY (`previous_audit_id`) REFERENCES `vibe_retail_v1`.`store`.`audit`(`audit_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ADD CONSTRAINT `fk_store_dsd_receiving_department_id` FOREIGN KEY (`department_id`) REFERENCES `vibe_retail_v1`.`store`.`department`(`department_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ADD CONSTRAINT `fk_store_dsd_receiving_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`license` ADD CONSTRAINT `fk_store_license_department_id` FOREIGN KEY (`department_id`) REFERENCES `vibe_retail_v1`.`store`.`department`(`department_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`license` ADD CONSTRAINT `fk_store_license_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`license` ADD CONSTRAINT `fk_store_license_renewed_from_license_id` FOREIGN KEY (`renewed_from_license_id`) REFERENCES `vibe_retail_v1`.`store`.`license`(`license_id`);
...
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ADD CONSTRAINT `fk_store_format_offer_eligibility_format_id` FOREIGN KEY (`format_id`) REFERENCES `vibe_retail_v1`.`store`.`format`(`format_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ADD CONSTRAINT `fk_store_format_offer_eligibility_region_id` FOREIGN KEY (`region_id`) REFERENCES `vibe_retail_v1`.`store`.`region`(`region_id`);
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ADD CONSTRAINT `fk_store_carrier_agreement_location_id` FOREIGN KEY (`location_id`) REFERENCES `vibe_retail_v1`.`store`.`location`(`location_id`);
...
ALTER TABLE `vibe_retail_v1`.`store`.`location` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`location` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Dc Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`format` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`pos_terminal` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` SET TAGS ('dbx_ssot_role' = 'authoritative');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` SET TAGS ('dbx_ssot_owner' = 'store.store_planogram');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` SET TAGS ('dbx_ssot_concept' = 'planogram');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `store_planogram_id` SET TAGS ('dbx_business_glossary_term' = 'Store Planogram (POG) ID');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Store Department Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `format_id` SET TAGS ('dbx_business_glossary_term' = 'Store Format Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `item_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Item Hierarchy Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `product_brand_id` SET TAGS ('dbx_business_glossary_term' = 'Product Brand Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `approved_by` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `compliance_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Required Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `compliance_tolerance_percent` SET TAGS ('dbx_business_glossary_term' = 'Compliance Tolerance Percent');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `compliant_store_count` SET TAGS ('dbx_business_glossary_term' = 'Compliant Store Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `deployed_store_count` SET TAGS ('dbx_business_glossary_term' = 'Deployed Store Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `facing_count` SET TAGS ('dbx_business_glossary_term' = 'Facing Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `fixture_count` SET TAGS ('dbx_business_glossary_term' = 'Fixture Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `fixture_type` SET TAGS ('dbx_business_glossary_term' = 'Fixture Type');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `max_on_shelf_quantity` SET TAGS ('dbx_business_glossary_term' = 'Maximum On-Shelf Quantity');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `merchandising_strategy` SET TAGS ('dbx_business_glossary_term' = 'Merchandising Strategy');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `merchandising_strategy` SET TAGS ('dbx_value_regex' = 'category_dominance|brand_blocking|price_point_segmentation|cross_merchandising|impulse');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `min_on_shelf_quantity` SET TAGS ('dbx_business_glossary_term' = 'Minimum On-Shelf Quantity');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_code` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) Code');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_code` SET TAGS ('dbx_value_regex' = '^POG-[A-Z0-9]{6,12}$');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_name` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) Name');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_status` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) Status');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_status` SET TAGS ('dbx_value_regex' = 'draft|approved|active|retired|suspended');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_type` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) Type');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `planogram_type` SET TAGS ('dbx_value_regex' = 'standard|seasonal|promotional|reset|test');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `primary_sku_count` SET TAGS ('dbx_business_glossary_term' = 'Primary Stock Keeping Unit (SKU) Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `promotional_sku_count` SET TAGS ('dbx_business_glossary_term' = 'Promotional Stock Keeping Unit (SKU) Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `replenishment_frequency` SET TAGS ('dbx_business_glossary_term' = 'Replenishment Frequency');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `replenishment_frequency` SET TAGS ('dbx_value_regex' = 'daily|twice_daily|weekly|as_needed|continuous');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `shelf_count` SET TAGS ('dbx_business_glossary_term' = 'Shelf Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `sku_count` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU) Count');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `target_gmroi` SET TAGS ('dbx_business_glossary_term' = 'Target Gross Margin Return on Investment (GMROI)');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `target_sales_per_linear_foot` SET TAGS ('dbx_business_glossary_term' = 'Target Sales Per Linear Foot');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `total_linear_feet` SET TAGS ('dbx_business_glossary_term' = 'Total Linear Feet');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `version` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) Version');
ALTER TABLE `vibe_retail_v1`.`store`.`store_planogram` ALTER COLUMN `version` SET TAGS ('dbx_value_regex' = '^v?[0-9]+.[0-9]+(.[0-9]+)?$');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `fixture_id` SET TAGS ('dbx_business_glossary_term' = 'Fixture ID');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `creative_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Creative Asset Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `merchandising_planogram_id` SET TAGS ('dbx_business_glossary_term' = 'Planogram (POG) ID');
ALTER TABLE `vibe_retail_v1`.`store`.`fixture` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `calibration_date` SET TAGS ('dbx_business_glossary_term' = 'Calibration Date');
...
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `counting_zone_code` SET TAGS ('dbx_business_glossary_term' = 'Counting Zone Code');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `counting_zone_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,20}$');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `counting_zone_name` SET TAGS ('dbx_business_glossary_term' = 'Counting Zone Name');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `counting_zone_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `data_quality_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `data_source_system` SET TAGS ('dbx_business_glossary_term' = 'Data Source System');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `day_of_week` SET TAGS ('dbx_business_glossary_term' = 'Day of Week');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `hour_of_day` SET TAGS ('dbx_business_glossary_term' = 'Hour of Day');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `inbound_count` SET TAGS ('dbx_business_glossary_term' = 'Inbound Count');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `is_holiday` SET TAGS ('dbx_business_glossary_term' = 'Is Holiday');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `is_promotional_event` SET TAGS ('dbx_business_glossary_term' = 'Is Promotional Event');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `is_store_open` SET TAGS ('dbx_business_glossary_term' = 'Is Store Open');
...
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `measurement_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Measurement Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `net_occupancy_estimate` SET TAGS ('dbx_business_glossary_term' = 'Net Occupancy Estimate');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `outbound_count` SET TAGS ('dbx_business_glossary_term' = 'Outbound Count');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `peak_occupancy` SET TAGS ('dbx_business_glossary_term' = 'Peak Occupancy');
...
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `sensor_type` SET TAGS ('dbx_business_glossary_term' = 'Sensor Type');
...
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `transaction_count` SET TAGS ('dbx_business_glossary_term' = 'Transaction Count');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `weather_condition_code` SET TAGS ('dbx_business_glossary_term' = 'Weather Condition Code');
ALTER TABLE `vibe_retail_v1`.`store`.`traffic_count` ALTER COLUMN `zone_type` SET TAGS ('dbx_business_glossary_term' = 'Zone Type');
...
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `closure_days` SET TAGS ('dbx_business_glossary_term' = 'Closure Days');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `closure_flag` SET TAGS ('dbx_business_glossary_term' = 'Closure Flag');
...
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `comp_store_qualification_status` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store (Comp Store) Qualification Status');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `comp_store_qualification_status` SET TAGS ('dbx_value_regex' = 'qualified|not_qualified|pending_review');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_atv` SET TAGS ('dbx_business_glossary_term' = 'Current Period Average Transaction Value (ATV)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_conversion_rate` SET TAGS ('dbx_business_glossary_term' = 'Current Period Conversion Rate (CR)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_footfall` SET TAGS ('dbx_business_glossary_term' = 'Current Period Footfall');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_net_sales` SET TAGS ('dbx_business_glossary_term' = 'Current Period Net Sales');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_transaction_count` SET TAGS ('dbx_business_glossary_term' = 'Current Period Transaction Count');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_units_sold` SET TAGS ('dbx_business_glossary_term' = 'Current Period Units Sold');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `current_period_upt` SET TAGS ('dbx_business_glossary_term' = 'Current Period Units Per Transaction (UPT)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `data_source_system` SET TAGS ('dbx_business_glossary_term' = 'Data Source System');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `months_in_operation` SET TAGS ('dbx_business_glossary_term' = 'Months in Operation');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_atv` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Average Transaction Value (ATV)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_conversion_rate` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Conversion Rate (CR)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_footfall` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Footfall');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_net_sales` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Net Sales');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_transaction_count` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Transaction Count');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_units_sold` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Units Sold');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `prior_period_upt` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Units Per Transaction (UPT)');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `remodel_flag` SET TAGS ('dbx_business_glossary_term' = 'Remodel Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_district` SET TAGS ('dbx_business_glossary_term' = 'Reporting District');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_period_type` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Type');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_period_type` SET TAGS ('dbx_value_regex' = 'week|month|quarter|year');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `reporting_region` SET TAGS ('dbx_business_glossary_term' = 'Reporting Region');
...
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `store_number` SET TAGS ('dbx_business_glossary_term' = 'Store Number');
ALTER TABLE `vibe_retail_v1`.`store`.`comparable_sales` ALTER COLUMN `store_open_date` SET TAGS ('dbx_business_glossary_term' = 'Store Open Date');
...
ALTER TABLE `vibe_retail_v1`.`store`.`pl` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `pl_id` SET TAGS ('dbx_business_glossary_term' = 'Store Profit & Loss (P&L) ID');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `financial_period_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Period ID');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `atv_amount` SET TAGS ('dbx_business_glossary_term' = 'Average Transaction Value (ATV) Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `cogs_amount` SET TAGS ('dbx_business_glossary_term' = 'Cost of Goods Sold (COGS) Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `comp_sales_flag` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store Sales (Comp Sales) Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `comp_sales_growth_percent` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store Sales (Comp Sales) Growth Percent');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `depreciation_amount` SET TAGS ('dbx_business_glossary_term' = 'Depreciation Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `discounts_amount` SET TAGS ('dbx_business_glossary_term' = 'Discounts Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `ebitda_amount` SET TAGS ('dbx_business_glossary_term' = 'Earnings Before Interest, Taxes, Depreciation, and Amortization (EBITDA) Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `ebitda_percent` SET TAGS ('dbx_business_glossary_term' = 'Earnings Before Interest, Taxes, Depreciation, and Amortization (EBITDA) Percent');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `finalized_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Finalized Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `gross_margin_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Margin Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `gross_margin_percent` SET TAGS ('dbx_business_glossary_term' = 'Gross Margin Percent');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `gross_sales_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Sales Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `labor_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `marketing_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Marketing Expense Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `net_sales_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Sales Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Profit & Loss (P&L) Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `occupancy_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Occupancy Cost Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `other_operating_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Other Operating Expense Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Period End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Period Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `pl_status` SET TAGS ('dbx_business_glossary_term' = 'Profit & Loss (P&L) Status');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `pl_status` SET TAGS ('dbx_value_regex' = 'preliminary|final|restated');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `reporting_entity` SET TAGS ('dbx_business_glossary_term' = 'Reporting Entity');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `returns_amount` SET TAGS ('dbx_business_glossary_term' = 'Returns Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `shrinkage_amount` SET TAGS ('dbx_business_glossary_term' = 'Shrinkage Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `supply_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Supply Expense Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `total_operating_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Operating Expense Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `transaction_count` SET TAGS ('dbx_business_glossary_term' = 'Transaction Count');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `units_sold` SET TAGS ('dbx_business_glossary_term' = 'Units Sold');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`pl` ALTER COLUMN `upt` SET TAGS ('dbx_business_glossary_term' = 'Units Per Transaction (UPT)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `shrinkage_event_id` SET TAGS ('dbx_business_glossary_term' = 'Shrinkage Event ID');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `cost_value_lost` SET TAGS ('dbx_business_glossary_term' = 'Cost Value Lost');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `detection_method` SET TAGS ('dbx_business_glossary_term' = 'Detection Method');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `police_report_number` SET TAGS ('dbx_business_glossary_term' = 'Police Report Number');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `product_description` SET TAGS ('dbx_business_glossary_term' = 'Product Description');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `quantity_lost` SET TAGS ('dbx_business_glossary_term' = 'Quantity Lost');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `recovery_amount` SET TAGS ('dbx_business_glossary_term' = 'Recovery Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `recovery_method` SET TAGS ('dbx_business_glossary_term' = 'Recovery Method');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Resolution Date');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `resolution_status` SET TAGS ('dbx_business_glossary_term' = 'Resolution Status');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `responsible_party_type` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Type');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `shrinkage_type` SET TAGS ('dbx_business_glossary_term' = 'Shrinkage Type');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `total_retail_value_lost` SET TAGS ('dbx_business_glossary_term' = 'Total Retail Value Lost');
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `unit_retail_value` SET TAGS ('dbx_business_glossary_term' = 'Unit Retail Value');
...
ALTER TABLE `vibe_retail_v1`.`store`.`shrinkage_event` ALTER COLUMN `zone_location` SET TAGS ('dbx_business_glossary_term' = 'Zone Location');
ALTER TABLE `vibe_retail_v1`.`store`.`department` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`department` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`department` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Department Manager ID');
...
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` SET TAGS ('dbx_subdomain' = 'store_operations');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `remodel_id` SET TAGS ('dbx_business_glossary_term' = 'Store Remodel ID');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Contractor Vendor Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Launch Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `merchandising_planogram_id` SET TAGS ('dbx_business_glossary_term' = 'Target Planogram Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `format_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Store Format Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Remodel Completion Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `actual_spend_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Spend Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Remodel Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Project Approval Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `approved_by` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `capital_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Capital Budget Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `closure_end_date` SET TAGS ('dbx_business_glossary_term' = 'Temporary Closure End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `closure_start_date` SET TAGS ('dbx_business_glossary_term' = 'Temporary Closure Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `comp_sales_exclusion_flag` SET TAGS ('dbx_business_glossary_term' = 'Comparable Sales (Comp Sales) Exclusion Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `contractor_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Contractor Reference Number');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `format_change_flag` SET TAGS ('dbx_business_glossary_term' = 'Store Format Change Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `impacted_departments` SET TAGS ('dbx_business_glossary_term' = 'Impacted Departments');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `inspection_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Final Inspection Completion Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `new_fixture_count` SET TAGS ('dbx_business_glossary_term' = 'New Fixture Count');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `new_format` SET TAGS ('dbx_business_glossary_term' = 'New Store Format');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Remodel Project Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `permit_number` SET TAGS ('dbx_business_glossary_term' = 'Building Permit Number');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `planned_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Remodel Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_manager_name` SET TAGS ('dbx_business_glossary_term' = 'Project Manager Name');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_manager_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_number` SET TAGS ('dbx_business_glossary_term' = 'Remodel Project Number');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,20}$');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_status` SET TAGS ('dbx_business_glossary_term' = 'Remodel Project Status');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `project_status` SET TAGS ('dbx_value_regex' = 'planned|approved|in_progress|completed|cancelled|on_hold');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `projected_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Projected Remodel Completion Date');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `remodel_type` SET TAGS ('dbx_business_glossary_term' = 'Remodel Type');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `remodel_type` SET TAGS ('dbx_value_regex' = 'full_remodel|department_refresh|fixture_replacement|technology_upgrade|accessibility_improvement|format_conversion');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `roi_projection_percent` SET TAGS ('dbx_business_glossary_term' = 'Return on Investment (ROI) Projection Percentage');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `square_footage_change` SET TAGS ('dbx_business_glossary_term' = 'Square Footage Change');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `sustainability_features` SET TAGS ('dbx_business_glossary_term' = 'Sustainability Features');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `technology_upgrades` SET TAGS ('dbx_business_glossary_term' = 'Technology Upgrades');
ALTER TABLE `vibe_retail_v1`.`store`.`remodel` ALTER COLUMN `temporary_closure_flag` SET TAGS ('dbx_business_glossary_term' = 'Temporary Closure Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`cluster` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`cluster` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`cluster` ALTER COLUMN `semantic_layer_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Semantic Layer Entity Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`cluster` ALTER COLUMN `target_customer_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Customer Segment');
...
ALTER TABLE `vibe_retail_v1`.`store`.`sfs_fulfillment_node` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`sfs_fulfillment_node` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`sfs_fulfillment_node` ALTER COLUMN `inventory_node_id` SET TAGS ('dbx_business_glossary_term' = 'Inventory Node Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`sfs_fulfillment_node` ALTER COLUMN `promo_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Offer Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`sfs_fulfillment_node` ALTER COLUMN `secondary_carrier_code` SET TAGS ('dbx_business_glossary_term' = 'Secondary Carrier Code');
...
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `previous_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Audit ID');
...
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `audit_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Date');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `audit_number` SET TAGS ('dbx_business_glossary_term' = 'Audit Number');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `audit_number` SET TAGS ('dbx_value_regex' = '^AUD-[0-9]{8}-[A-Z0-9]{6}$');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `auditor_name` SET TAGS ('dbx_business_glossary_term' = 'Auditor Name');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `auditor_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `auditor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `auditor_type` SET TAGS ('dbx_business_glossary_term' = 'Auditor Type');
...
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `citation_issued_flag` SET TAGS ('dbx_business_glossary_term' = 'Citation Issued Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `closed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Closed Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `corrective_action_due_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Due Date');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `corrective_action_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Required Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `critical_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Findings Count');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `district_manager_notified_flag` SET TAGS ('dbx_business_glossary_term' = 'District Manager Notified Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `end_time` SET TAGS ('dbx_business_glossary_term' = 'Audit End Time');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `fine_amount` SET TAGS ('dbx_business_glossary_term' = 'Fine Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `fine_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Quarter');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_value_regex' = 'Q1|Q2|Q3|Q4');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
...
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `major_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Major Findings Count');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `minor_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Minor Findings Count');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `overall_score_percent` SET TAGS ('dbx_business_glossary_term' = 'Overall Audit Score Percentage');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `pass_fail_status` SET TAGS ('dbx_business_glossary_term' = 'Pass/Fail Status');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `pass_fail_status` SET TAGS ('dbx_value_regex' = 'pass|fail|conditional_pass|pending_review');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `photographic_evidence_count` SET TAGS ('dbx_business_glossary_term' = 'Photographic Evidence Count');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `previous_audit_score_percent` SET TAGS ('dbx_business_glossary_term' = 'Previous Audit Score Percentage');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `regulatory_agency` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `regulatory_case_number` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Case Number');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `report_url` SET TAGS ('dbx_business_glossary_term' = 'Audit Report URL');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `resolution_status` SET TAGS ('dbx_business_glossary_term' = 'Resolution Status');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `resolution_status` SET TAGS ('dbx_value_regex' = 'open|in_progress|pending_verification|closed|overdue');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `scope` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `score_variance_percent` SET TAGS ('dbx_business_glossary_term' = 'Score Variance Percentage');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `start_time` SET TAGS ('dbx_business_glossary_term' = 'Audit Start Time');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `store_manager_notified_flag` SET TAGS ('dbx_business_glossary_term' = 'Store Manager Notified Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `summary` SET TAGS ('dbx_business_glossary_term' = 'Audit Summary');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `total_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Total Findings Count');
ALTER TABLE `vibe_retail_v1`.`store`.`audit` ALTER COLUMN `total_items_audited` SET TAGS ('dbx_business_glossary_term' = 'Total Items Audited');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `dsd_receiving_id` SET TAGS ('dbx_business_glossary_term' = 'Direct Store Delivery (DSD) Receiving ID');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `inbound_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Inbound Shipment Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `inbound_appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Appointment Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order (PO) ID');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Receiving Associate ID');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `inventory_node_id` SET TAGS ('dbx_business_glossary_term' = 'Receiving Inventory Node Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier ID');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `chargeback_amount` SET TAGS ('dbx_business_glossary_term' = 'Chargeback Amount');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `chargeback_reason` SET TAGS ('dbx_business_glossary_term' = 'Chargeback Reason');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `cost_variance` SET TAGS ('dbx_business_glossary_term' = 'Cost Variance');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `delivery_manifest_number` SET TAGS ('dbx_business_glossary_term' = 'Delivery Manifest Number');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `delivery_window_end` SET TAGS ('dbx_business_glossary_term' = 'Delivery Window End Time');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `delivery_window_start` SET TAGS ('dbx_business_glossary_term' = 'Delivery Window Start Time');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `driver_name` SET TAGS ('dbx_business_glossary_term' = 'Driver Name');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `driver_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `driver_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `driver_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `driver_signature_captured` SET TAGS ('dbx_business_glossary_term' = 'Driver Signature Captured Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `invoice_match_status` SET TAGS ('dbx_business_glossary_term' = 'Invoice Match Status');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `invoice_match_status` SET TAGS ('dbx_value_regex' = 'matched|unmatched|variance_within_tolerance|variance_exceeds_tolerance|pending_reconciliation');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Receiving Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `on_time_delivery_flag` SET TAGS ('dbx_business_glossary_term' = 'On-Time Delivery Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `putaway_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Putaway Completed Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `putaway_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Putaway Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `quantity_variance` SET TAGS ('dbx_business_glossary_term' = 'Quantity Variance');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receipt_date` SET TAGS ('dbx_business_glossary_term' = 'Receipt Date');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receipt_number` SET TAGS ('dbx_business_glossary_term' = 'Receipt Number');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receipt_status` SET TAGS ('dbx_business_glossary_term' = 'Receipt Status');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receipt_status` SET TAGS ('dbx_value_regex' = 'accepted|partially_accepted|rejected|pending_review|disputed');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receipt_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Receipt Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receiving_location` SET TAGS ('dbx_business_glossary_term' = 'Receiving Location');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `receiving_location` SET TAGS ('dbx_value_regex' = 'receiving_dock|back_room|sales_floor|cooler|freezer');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `reconciliation_date` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Date');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Status');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_value_regex' = 'pending|reconciled|disputed|closed');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `rejection_notes` SET TAGS ('dbx_business_glossary_term' = 'Rejection Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `scan_based_trading_flag` SET TAGS ('dbx_business_glossary_term' = 'Scan-Based Trading (SBT) Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `temperature_check_required` SET TAGS ('dbx_business_glossary_term' = 'Temperature Check Required Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `temperature_check_result` SET TAGS ('dbx_business_glossary_term' = 'Temperature Check Result');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `temperature_check_result` SET TAGS ('dbx_value_regex' = 'passed|failed|not_applicable|not_performed');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `temperature_reading` SET TAGS ('dbx_business_glossary_term' = 'Temperature Reading (Fahrenheit)');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `total_cost_invoiced` SET TAGS ('dbx_business_glossary_term' = 'Total Cost Invoiced');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `total_cost_received` SET TAGS ('dbx_business_glossary_term' = 'Total Cost Received');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `total_quantity_invoiced` SET TAGS ('dbx_business_glossary_term' = 'Total Quantity Invoiced');
ALTER TABLE `vibe_retail_v1`.`store`.`dsd_receiving` ALTER COLUMN `total_quantity_received` SET TAGS ('dbx_business_glossary_term' = 'Total Quantity Received');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_repository_license' = 'Apache-2.0');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_provenance' = 'LLM-generated');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_last_human_reviewed_YYYY_MM_DD_by' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_last_human_reviewed' = 'YYYY-MM-DD');
ALTER TABLE `vibe_retail_v1`.`store`.`license` SET TAGS ('dbx_license' = 'Apache-2.0');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `license_id` SET TAGS ('dbx_business_glossary_term' = 'Store License ID');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `renewed_from_license_id` SET TAGS ('dbx_business_glossary_term' = 'Renewed From License Id');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `renewed_from_license_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Associate ID');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `audit_trail_reference` SET TAGS ('dbx_business_glossary_term' = 'Audit Trail Reference');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Compliance Notes');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `inspection_frequency_months` SET TAGS ('dbx_business_glossary_term' = 'Inspection Frequency Months');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `inspection_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Inspection Required Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `last_violation_date` SET TAGS ('dbx_business_glossary_term' = 'Last Violation Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `next_inspection_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Inspection Due Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `reinstatement_conditions` SET TAGS ('dbx_business_glossary_term' = 'Reinstatement Conditions');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `reinstatement_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Reinstatement Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `renewal_lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Renewal Lead Time Days');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `revocation_date` SET TAGS ('dbx_business_glossary_term' = 'Revocation Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `revocation_reason` SET TAGS ('dbx_business_glossary_term' = 'Revocation Reason');
...
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `supporting_document_reference` SET TAGS ('dbx_business_glossary_term' = 'Supporting Document Reference');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `suspension_end_date` SET TAGS ('dbx_business_glossary_term' = 'Suspension End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `suspension_reason` SET TAGS ('dbx_business_glossary_term' = 'Suspension Reason');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `suspension_start_date` SET TAGS ('dbx_business_glossary_term' = 'Suspension Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`license` ALTER COLUMN `violation_count` SET TAGS ('dbx_business_glossary_term' = 'Violation Count');
ALTER TABLE `vibe_retail_v1`.`store`.`cluster_membership` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`store`.`cluster_membership` SET TAGS ('dbx_subdomain' = 'store_operations');
ALTER TABLE `vibe_retail_v1`.`store`.`cluster_membership` SET TAGS ('dbx_association_edges' = 'store.store_cluster,store.store_location');
...
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `format_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Creator User Identifier');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `format_associate_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `format_associate_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `region_id` SET TAGS ('dbx_business_glossary_term' = 'Region Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `approved_by` SET TAGS ('dbx_pii_name' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Format Eligibility End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Format Eligibility Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `eligibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Flag');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `localized_discount_value` SET TAGS ('dbx_business_glossary_term' = 'Localized Discount Value');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `localized_minimum_purchase` SET TAGS ('dbx_business_glossary_term' = 'Localized Minimum Purchase Threshold');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Format-Specific Priority Rank');
ALTER TABLE `vibe_retail_v1`.`store`.`format_offer_eligibility` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` SET TAGS ('dbx_subdomain' = 'retail_performance');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` SET TAGS ('dbx_association_edges' = 'store.store_location,fulfillment.carrier');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `carrier_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Store Carrier Agreement Identifier');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Store Carrier Agreement - Fulfillment Carrier Id');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store Carrier Agreement - Location Id');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `active_status` SET TAGS ('dbx_business_glossary_term' = 'Agreement Active Status');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `contract_end_date` SET TAGS ('dbx_business_glossary_term' = 'Contract End Date');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `contract_start_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Start Date');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `cutoff_time_local` SET TAGS ('dbx_business_glossary_term' = 'Store-Specific Cutoff Time');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `negotiated_rate` SET TAGS ('dbx_business_glossary_term' = 'Negotiated Rate');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `pickup_frequency` SET TAGS ('dbx_business_glossary_term' = 'Carrier Pickup Frequency');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Carrier Priority Rank');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `service_level_agreement` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement');
ALTER TABLE `vibe_retail_v1`.`store`.`carrier_agreement` ALTER COLUMN `volume_commitment_monthly` SET TAGS ('dbx_business_glossary_term' = 'Monthly Volume Commitment');
ALTER TABLE `vibe_retail_v1`.`store`.`sales_territory` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`store`.`sales_territory` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `vibe_retail_v1`.`store`.`sales_territory` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Manager Id');
...
ALTER TABLE `vibe_retail_v1`.`store`.`region` SET TAGS ('dbx_subdomain' = 'store_operations');
