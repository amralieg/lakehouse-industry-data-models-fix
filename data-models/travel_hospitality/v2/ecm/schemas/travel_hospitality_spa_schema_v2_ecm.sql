...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` (
    ...
    `revenue_center` DECIMAL(18,2) COMMENT 'Revenue center to which this treatment is assigned for financial reporting and GOP (Gross Operating Profit) analysis per USALI standards.',
    `room_type_required` BOOLEAN COMMENT 'Type of treatment room or facility required to deliver this service (e.g., single room, couples suite, wet room, outdoor cabana). Used for facility scheduling and capacity planning.',
    ...
    `skill_level_required` BOOLEAN COMMENT 'Minimum skill level classification required for a therapist to perform this treatment. Used for therapist assignment and training planning.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the user (spa manager, revenue manager, or property director) who approved this treatment menu for publication.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the employee master record in the workforce domain. Links therapist to their employment record for payroll, benefits, and HR management.',
    ...
    `procurement_org_unit_id` BIGINT COMMENT 'Foreign key linking to procurement.org_unit. Business justification: Therapists belong to organizational units (spa department/team) for cost center allocation, labor reporting, scheduling pool management, and management hierarchy. Required for USALI labor cost reporti',
    `procurement_position_id` BIGINT COMMENT 'Foreign key linking to procurement.position. Business justification: Therapists occupy formal positions with job codes, pay grades, FLSA classification, and USALI department codes. Critical for labor budgeting, headcount planning, compensation administration, union eli',
    ...
    `certification_level` BIGINT COMMENT 'Professional certification or skill level of the therapist within the spas internal competency framework. Determines service eligibility, pricing tier, and training pathway.. Valid values are `entry|intermediate|advanced|master|director`',
    ...
    `phone_number` BIGINT COMMENT 'Primary contact phone number for the therapist. Used for emergency contact, shift change notifications, and operational communication.',
    ...
    `primary_license_number` BIGINT COMMENT 'State or national professional license number for massage therapy, esthetics, or other regulated spa services. Required for regulatory compliance and insurance coverage.',
    ...
    `secondary_license_number` BIGINT COMMENT 'Additional professional license number for therapists certified in multiple disciplines (e.g., both massage therapy and esthetics) or holding licenses in multiple states.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` (
    ...
    `certification_level` BIGINT COMMENT 'The proficiency level or tier of the certification. Used to match therapist expertise to guest service expectations and premium treatment offerings.. Valid values are `entry|intermediate|advanced|master|instructor`',
    ...
    `certification_number` BIGINT COMMENT 'The unique certificate or license number issued by the certifying body. This is the externally-known identifier for the certification.',
    ...
    `continuing_education_hours_required` BOOLEAN COMMENT 'The number of continuing education hours required for renewal of this certification. Used to track therapist compliance with professional development requirements.',
    ...
    `cost_currency_code` DECIMAL(18,2) COMMENT 'The three-letter ISO 4217 currency code for the certification cost. Supports multi-currency operations for global property portfolios.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Foreign key linking to procurement.employee. Business justification: Spa facilities require operational manager assignment for P&L accountability, labor scheduling oversight, vendor management, and guest escalation handling. Current facility_manager_name is denormalize',
    ...
    `procurement_org_unit_id` BIGINT COMMENT 'Foreign key linking to procurement.org_unit. Business justification: Spa facilities are organizational units with cost centers, budgeted headcount, labor productivity standards, and USALI department classification. Required for financial consolidation, labor cost alloc',
    ...
    `operating_hours_friday` BIGINT COMMENT 'Facility operating hours on Friday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_monday` BIGINT COMMENT 'Facility operating hours on Monday in 24-hour format (HH:MM-HH:MM) or closed if not operational. Example: 06:00-22:00.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_saturday` BIGINT COMMENT 'Facility operating hours on Saturday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_sunday` BIGINT COMMENT 'Facility operating hours on Sunday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_thursday` BIGINT COMMENT 'Facility operating hours on Thursday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_tuesday` BIGINT COMMENT 'Facility operating hours on Tuesday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    `operating_hours_wednesday` BIGINT COMMENT 'Facility operating hours on Wednesday in 24-hour format (HH:MM-HH:MM) or closed if not operational.. Valid values are `^([0-1][0-9]|2[0-3]):[0-5][0-9]-([0-1][0-9]|2[0-3]):[0-5][0-9]$|^closed$`',
    ...
    `peak_season_months` BIGINT COMMENT 'Comma-separated list of month names or numbers representing peak demand periods for the facility (e.g., June,July,August or 6,7,8). Supports revenue management and staffing planning.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` (
    ...
    `max_occupancy` DECIMAL(18,2) COMMENT 'Maximum number of guests that can be accommodated simultaneously in the treatment room. Critical for scheduling and safety compliance.',
    ...
    `room_number` BIGINT COMMENT 'Alphanumeric room number or code used for operational identification and wayfinding within the spa facility.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Identifier of the system user or staff member who created the appointment booking. Used for accountability and performance tracking. May be guest self-service identifier for online bookings.',
    ...
    `confirmation_number` BIGINT COMMENT 'Externally-facing unique confirmation code provided to the guest for appointment reference and verification. Generated at booking time and used for guest communication.. Valid values are `^[A-Z0-9]{8,12}$`',
    ...
    `prepayment_currency_code` DECIMAL(18,2) COMMENT 'Three-letter ISO 4217 currency code for the prepayment amount. Required when prepayment_amount is populated. Supports multi-currency operations.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the staff member who created this spa package booking on behalf of the guest. Null if guest self-booked.',
    ...
    `confirmation_number` BIGINT COMMENT 'Guest-facing confirmation number for the spa package booking.. Valid values are `^[A-Z0-9]{6,20}$`',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'Payment instrument used to pay for the spa package. [ENUM-REF-CANDIDATE: credit card|debit card|room charge|loyalty points|gift certificate|cash|bank transfer — 7 candidates stripped; promote to reference product]',
    `payment_status` DECIMAL(18,2) COMMENT 'Current payment status of the spa package booking.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`package` (
    ...
    `requires_deposit` DECIMAL(18,2) COMMENT 'Indicates whether a deposit is required at time of booking to secure the package reservation.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` (
    ...
    `form_number` BIGINT COMMENT 'Human-readable business identifier for the intake form, typically formatted as IF- followed by numeric sequence. Used for guest communication and record retrieval.. Valid values are `^IF-[0-9]{8,12}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'System user ID of the staff member who posted the charge transaction. Used for audit trail and accountability.',
    ...
    `charge_number` DECIMAL(18,2) COMMENT 'Externally-known unique reference number for the spa charge transaction, used for guest communication and reconciliation.',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'Method of payment for the spa charge: room_charge (posted to folio), credit_card (direct payment), cash, gift_card, loyalty_points (redemption), or comp (complimentary).',
    ...
    `revenue_center_code` DECIMAL(18,2) COMMENT 'USALI revenue center code for the spa department or facility where the charge was incurred. Used for departmental P&L reporting.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the employee who processed the retail transaction.',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'Primary payment instrument used for the transaction (e.g., credit card, cash, room charge, gift card, loyalty points redemption). [ENUM-REF-CANDIDATE: cash|credit_card|debit_card|room_charge|gift_card|loyalty_points|mobile_payment|bank_transfer — 8 candidates stripped; promote to reference product]',
    `payment_reference` DECIMAL(18,2) COMMENT 'Reference number or authorization code from the payment processor for credit card or electronic payments. Masked or tokenized for security.',
    ...
    `receipt_number` BIGINT COMMENT 'Printed receipt number provided to the guest as proof of purchase.',
    ...
    `transaction_number` BIGINT COMMENT 'Business identifier for the retail transaction, typically generated by the Point of Sale (POS) system. Externally visible transaction reference.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` (
    ...
    `batch_number` BIGINT COMMENT 'The manufacturer batch or lot number for product traceability. Used for quality control, recalls, and regulatory compliance.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` (
    ...
    `membership_number` BIGINT COMMENT 'Externally visible unique membership number issued to the guest. Used for identification at spa facilities and on membership cards.. Valid values are `^[A-Z0-9]{8,20}$`',
    ...
    `payment_method_token` DECIMAL(18,2) COMMENT 'Tokenized reference to the payment method on file for recurring membership charges. Token is stored securely in PCI-compliant payment gateway and used for auto-renewal billing. Does not contain actual card numbers.',
    `payment_method_type` DECIMAL(18,2) COMMENT 'Type of payment method used for membership billing. Credit card and debit card are most common for individual memberships, bank account for direct debit, corporate billing for employer-sponsored memberships, room charge for in-house guests who want membership fees added to their folio.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the spa reception or front desk staff member who processed the check-in and credit redemption for this visit.',
    ...
    `feedback_comments` DECIMAL(18,2) COMMENT 'Free-text guest feedback or comments about the visit experience. Captured for service quality improvement and issue resolution.',
    ...
    `visit_number` BIGINT COMMENT 'Business identifier for the visit transaction. Externally visible reference number used for guest communication and service tracking.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` (
    ...
    `confirmation_number` BIGINT COMMENT 'Booking confirmation number provided to the guest. Used for guest communication and reservation lookup.. Valid values are `^[A-Z0-9]{6,15}$`',
    ...
    `pass_number` BIGINT COMMENT 'Externally visible unique alphanumeric identifier for the day pass. Used for guest reference, check-in, and operational tracking.. Valid values are `^[A-Z0-9]{8,20}$`',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'The payment instrument used to purchase the day pass. Used for financial reconciliation and payment analytics. [ENUM-REF-CANDIDATE: credit_card|debit_card|cash|mobile_payment|gift_certificate|loyalty_points|bank_transfer — 7 candidates stripped; promote to reference product]',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the primary instructor or therapist who leads this fitness class.',
    ...
    `fitness_class_code` STRING COMMENT '',
    ...
    `fitness_class_description` STRING COMMENT '',
    ...
    `equipment_required` BOOLEAN COMMENT 'List of equipment or props needed for the class (e.g., yoga mat, dumbbells, resistance bands, spin bike, pool access).',
    `fitness_class_status` STRING COMMENT '',
    `fitness_level` BIGINT COMMENT 'The recommended fitness or experience level for participants (beginner, intermediate, advanced, all levels).. Valid values are `beginner|intermediate|advanced|all_levels`',
    `instructor_certification_required` BOOLEAN COMMENT 'The professional certifications or qualifications required for an instructor to teach this class (e.g., RYT-200, ACE Certified, CPR/AED).',
    `intensity_level` BIGINT COMMENT 'The physical intensity or exertion level of the class (low, moderate, high, variable).. Valid values are `low|moderate|high|variable`',
    ...
    `fitness_class_name` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the fitness instructor or therapist leading this session. Enables instructor utilization tracking, performance analysis, and guest preference matching.',
    ...
    `fitness_class_session_code` STRING COMMENT '',
    ...
    `equipment_required` BOOLEAN COMMENT 'List or description of equipment needed for the session (e.g., yoga mat, blocks, strap or spin bike, towel, water bottle). Informs guests what to bring or expect to be provided.',
    `fitness_class_session_status` STRING COMMENT '',
    ...
    `fitness_class_session_name` STRING COMMENT '',
    ...
    `revenue_center` DECIMAL(18,2) COMMENT 'The revenue center or department code to which session revenue is allocated. Aligns with property accounting structure and USALI reporting requirements.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the system user who processed the cancellation. Null for guest self-service cancellations or non-cancelled enrollments.',
    ...
    `spa_class_enrollment_code` STRING COMMENT '',
    ...
    `spa_class_enrollment_description` STRING COMMENT '',
    ...
    `enrollment_number` BIGINT COMMENT 'Unique business identifier for the enrollment transaction. Used for guest communication and service desk reference.. Valid values are `^ENR-[0-9]{8,12}$`',
    ...
    `folio_reference_number` BIGINT COMMENT 'Reference number of the guest folio where charges were posted. Null for non-room-charge payment methods. Links to OPERA PMS folio.',
    `guest_feedback_comments` DECIMAL(18,2) COMMENT 'Free-text comments provided by the guest about their class experience. Null if no feedback provided. Used for service improvement.',
    ...
    `spa_class_enrollment_name` STRING COMMENT '',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'Method used to pay for the class enrollment. Room charge posts to guest folio, membership indicates included in membership benefits. [ENUM-REF-CANDIDATE: room_charge|credit_card|cash|membership|complimentary|loyalty_points|gift_certificate — 7 candidates stripped; promote to reference product]',
    ...
    `spa_class_enrollment_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the system user or staff member who created the tee time booking. Links to user master data.',
    ...
    `golf_tee_time_code` STRING COMMENT '',
    ...
    `confirmation_number` BIGINT COMMENT 'Externally-facing unique confirmation number provided to the guest for the tee time booking. Used for guest communication and check-in.. Valid values are `^[A-Z0-9]{8,12}$`',
    ...
    `golf_tee_time_description` STRING COMMENT '',
    `golf_tee_time_status` STRING COMMENT '',
    ...
    `loyalty_member_number` BIGINT COMMENT 'The guests loyalty program member number if applicable. Used for points accrual and member benefits.',
    `loyalty_points_earned` STRING COMMENT 'Number of loyalty program points earned by the guest for this tee time booking.',
    `golf_tee_time_name` STRING COMMENT '',
    ...
    `payment_method` DECIMAL(18,2) COMMENT 'The payment method used for the tee time booking. Values: room_charge (charged to hotel room), credit_card, cash, comp (complimentary), voucher (gift certificate or prepaid voucher), member_account (club member account).',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` (
    `amenity_pricing_id` BIGINT COMMENT 'Unique identifier for the amenity pricing rule record.',
    `procurement_employee_id` BIGINT COMMENT 'Reference to the user who approved this pricing rule for operational use.',
    ...
    `amenity_pricing_status` STRING COMMENT '',
    ...
    `amenity_pricing_code` STRING COMMENT '',
    ...
    `day_of_week_applicability` BIGINT COMMENT 'Days of the week when this pricing rule applies (e.g., MON-FRI, SAT-SUN, ALL). Enables weekday vs weekend pricing differentiation.',
    `amenity_pricing_description` STRING COMMENT '',
    ...
    `amenity_pricing_name` STRING COMMENT '',
    ...
    `time_of_day_end` BIGINT COMMENT 'End time for time-based pricing rules. Format HH:MM in 24-hour notation.. Valid values are `^([01]?[0-9]|2[0-3]):[0-5][0-9]$`',
    `time_of_day_start` BIGINT COMMENT 'Start time for time-based pricing rules (e.g., morning rates, evening rates). Format HH:MM in 24-hour notation.. Valid values are `^([01]?[0-9]|2[0-3]):[0-5][0-9]$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the staff user who processed the cancellation if cancelled by staff. Null for guest-initiated or system cancellations.',
    ...
    `cancellation_fee_currency_code` DECIMAL(18,2) COMMENT 'Three-letter ISO 4217 currency code for the cancellation fee amount.',
    ...
    `cancellation_log_status` STRING COMMENT '',
    ...
    `cancellation_number` BIGINT COMMENT 'Business-facing unique cancellation reference number used for tracking and guest communication.',
    ...
    `cancellation_log_code` STRING COMMENT '',
    ...
    `cancellation_log_name` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` (
    ...
    `capacity_rating` BIGINT COMMENT 'Maximum capacity or load rating of the equipment (e.g., weight limit, occupancy, volume).',
    ...
    `condition_rating` BIGINT COMMENT 'Assessment of the physical condition and functionality of the equipment (excellent, good, fair, poor, critical).. Valid values are `excellent|good|fair|poor|critical`',
    ...
    `equipment_description` STRING COMMENT '',
    ...
    `equipment_status` STRING COMMENT '',
    ...
    `model_number` BIGINT COMMENT 'Manufacturers model number or designation for this equipment type.',
    ...
    `serial_number` BIGINT COMMENT 'Unique serial number assigned by the manufacturer to this specific equipment unit.',
    `service_contract_number` BIGINT COMMENT 'Reference number for the maintenance service contract covering this equipment.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Identifier of the user (spa director, revenue manager, or general manager) who approved this wellness program for operational deployment and guest booking.',
    ...
    `wellness_program_code` STRING COMMENT '',
    ...
    `wellness_program_description` STRING COMMENT '',
    ...
    `wellness_program_name` STRING COMMENT '',
    ...
    `revenue_center_code` DECIMAL(18,2) COMMENT 'Code identifying the revenue center or department to which wellness program revenue is allocated. Aligns with Uniform System of Accounts for the Lodging Industry (USALI) chart of accounts.',
    ...
    `wellness_program_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` (
    ...
    `program_treatment_code` STRING COMMENT '',
    ...
    `program_treatment_description` STRING COMMENT '',
    ...
    `program_treatment_name` STRING COMMENT '',
    ...
    `program_treatment_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` (
    ...
    `package_treatment_code` STRING COMMENT '',
    ...
    `package_treatment_description` STRING COMMENT '',
    ...
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `package_treatment_name` STRING COMMENT '',
    ...
    `package_treatment_status` STRING COMMENT '',
    ...
    `time_of_day_preference` BIGINT COMMENT 'Recommended or required time of day for this treatment within the package (morning, afternoon, evening, flexible). Used for scheduling optimization and guest experience design. Explicitly identified in detection phase.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`product` (
    ...
    `discontinued_date` DATE COMMENT '',
    ...
    `equipment_required` BOOLEAN COMMENT 'Specialized equipment or apparatus required to deliver the treatment (e.g., vichy shower, infrared sauna, cryotherapy chamber).',
    ...
    `is_active` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_synced_at` TIMESTAMP COMMENT '',
    ...
    `room_type_required` BOOLEAN COMMENT 'Type of treatment room or facility required to deliver the service (e.g., massage room, facial suite, hydrotherapy room, couples suite).',
    ...
    `structure_verified` BOOLEAN COMMENT 'Marker confirming required model structure of 17 domains and 356 products is preserved; additions allowed, no removals/renames/merges.',
    `taxable_flag` BOOLEAN COMMENT 'Indicates whether the product is subject to sales tax or value-added tax (VAT).',
    `therapist_certification_required` BOOLEAN COMMENT 'Specific professional certification or training required for therapists to perform this treatment (e.g., licensed massage therapist, esthetician, aromatherapist).',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` (
    `group_booking_id` BIGINT COMMENT 'Primary key for group_booking',
    `procurement_employee_id` BIGINT COMMENT 'Reference to the sales manager or group coordinator responsible for managing this booking.',
    ...
    `booking_number` BIGINT COMMENT 'Externally-visible unique booking reference number assigned to the group reservation for tracking and communication purposes.',
    ...
    `group_booking_code` STRING COMMENT '',
    ...
    `group_booking_description` STRING COMMENT '',
    ...
    `group_booking_status` STRING COMMENT '',
    `is_active` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    ...
    `group_booking_name` STRING COMMENT '',
    ...
    `payment_terms` DECIMAL(18,2) COMMENT 'Description of payment terms and schedule agreed upon for the group booking.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` (
    ...
    `retail_product_code` STRING COMMENT '',
    ...
    `retail_product_name` STRING COMMENT '',
    ...
    `package_size` BIGINT COMMENT 'Size or volume of the product package (e.g., 50ml, 8oz, 100g) including the unit.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` (
    ...
    `workforce_employee_id` BIGINT COMMENT 'Identifier of the user or system process that last modified this product line record. Used for audit and accountability purposes.',
    ...
    `product_line_code` STRING COMMENT '',
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'Accounting cost center code to which expenses and revenues for this product line are allocated. Used for departmental P&L reporting.',
    ...
    `product_line_description` STRING COMMENT '',
    ...
    `facility_type_required` BOOLEAN COMMENT 'Type of facility or treatment room required to deliver services in this product line. Examples: private treatment room, couples suite, hydrotherapy room, fitness studio, outdoor pavilion.',
    ...
    `product_line_name` STRING COMMENT '',
    ...
    `revenue_category` DECIMAL(18,2) COMMENT 'Financial reporting category for revenue generated by this product line. Aligns with property-level and corporate financial reporting structures.',
    ...
    `therapist_certification_required` BOOLEAN COMMENT 'Specific professional certifications or licenses required for staff to deliver services in this product line. Examples: licensed massage therapist, certified yoga instructor, personal trainer certification.',
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ADD CONSTRAINT `fk_spa_membership_visit_therapist_id` FOREIGN KEY (`therapist_id`) REFERENCES `vibe_travel_hospitality_v1`.`spa`.`therapist`(`therapist_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ADD CONSTRAINT `fk_spa_membership_visit_treatment_id` FOREIGN KEY (`treatment_id`) REFERENCES `vibe_travel_hospitality_v1`.`spa`.`treatment`(`treatment_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ADD CONSTRAINT `fk_spa_day_pass_renewed_day_pass_id` FOREIGN KEY (`renewed_day_pass_id`) REFERENCES `vibe_travel_hospitality_v1`.`spa`.`day_pass`(`day_pass_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `gender_preference` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `treatment_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `treatment_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment` ALTER COLUMN `treatment_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,15}$');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `menu_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `menu_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `menu_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_menu` ALTER COLUMN `menu_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` SET TAGS ('dbx_subdomain' = 'therapist_management');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `procurement_org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `procurement_position_id` SET TAGS ('dbx_business_glossary_term' = 'Position Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `certification_level` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `email_address` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `email_address` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `first_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `first_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `first_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `first_name` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `gender` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `last_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `last_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `last_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `last_name` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_type' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_class' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `phone_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `preferred_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `preferred_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `preferred_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `preferred_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `primary_license_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist` ALTER COLUMN `secondary_license_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` SET TAGS ('dbx_subdomain' = 'therapist_management');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_level` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `certification_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `instructor_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `instructor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `instructor_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_therapist_certification` ALTER COLUMN `instructor_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Manager Employee Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `procurement_org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_phone` SET TAGS ('dbx_pii_type' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_phone` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_phone` SET TAGS ('dbx_pii_class' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `contact_phone` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `gender_designation` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_friday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_monday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_saturday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_sunday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_thursday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_tuesday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `operating_hours_wednesday` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_facility` ALTER COLUMN `peak_season_months` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `gender_designation` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `room_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `room_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `room_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `room_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`treatment_room` ALTER COLUMN `room_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `attribution_event_id` SET TAGS ('dbx_business_glossary_term' = 'Attribution Event Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Booked By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `guest_gender_preference` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment` ALTER COLUMN `therapist_gender_preference` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Booked By Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Room Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `addon_services_amount` SET TAGS ('dbx_business_glossary_term' = 'Add-On Services Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_channel` SET TAGS ('dbx_business_glossary_term' = 'Booking Channel');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_channel` SET TAGS ('dbx_value_regex' = 'web|mobile app|phone|front desk|concierge|in-room tablet');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_date` SET TAGS ('dbx_business_glossary_term' = 'Booking Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_status` SET TAGS ('dbx_business_glossary_term' = 'Booking Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_status` SET TAGS ('dbx_value_regex' = 'pending|confirmed|in-progress|completed|cancelled|no-show');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `booking_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Booking Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `cancellation_deadline` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Deadline');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `cancelled_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancelled Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completion Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `complimentary_amenities` SET TAGS ('dbx_business_glossary_term' = 'Complimentary Amenities');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Confirmation Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Discount Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `guest_arrival_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Guest Arrival Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `loyalty_points_redeemed` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Redeemed');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `package_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Package Duration Minutes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `package_notes` SET TAGS ('dbx_business_glossary_term' = 'Package Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `package_price` SET TAGS ('dbx_business_glossary_term' = 'Package Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `payment_status` SET TAGS ('dbx_business_glossary_term' = 'Payment Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `redemption_status` SET TAGS ('dbx_business_glossary_term' = 'Redemption Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `redemption_status` SET TAGS ('dbx_value_regex' = 'not started|partially redeemed|fully redeemed|expired');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `scheduled_end_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `scheduled_start_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Start Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`appointment_package` ALTER COLUMN `special_requests` SET TAGS ('dbx_business_glossary_term' = 'Special Requests');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,10}$');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_class' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `package_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `package_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `package_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package` ALTER COLUMN `package_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`therapist_schedule` SET TAGS ('dbx_subdomain' = 'therapist_management');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` ALTER COLUMN `cardiovascular_condition_details` SET TAGS ('dbx_pii' = 'card');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` ALTER COLUMN `cardiovascular_condition_details` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` ALTER COLUMN `form_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` ALTER COLUMN `has_cardiovascular_condition` SET TAGS ('dbx_pii' = 'card');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`intake_form` ALTER COLUMN `has_cardiovascular_condition` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Posted By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `charge_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `discount_amount` SET TAGS ('dbx_monetary' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `payment_method` SET TAGS ('dbx_monetary' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `service_charge_amount` SET TAGS ('dbx_monetary' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `tax_amount` SET TAGS ('dbx_monetary' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `total_charge_amount` SET TAGS ('dbx_monetary' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Unit Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`charge` ALTER COLUMN `unit_price` SET TAGS ('dbx_monetary' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `campaign_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Cashier ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Discount Amount');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_phone` SET TAGS ('dbx_pii_type' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_phone` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_phone` SET TAGS ('dbx_pii_class' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `guest_phone` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `receipt_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `sales_channel` SET TAGS ('dbx_business_glossary_term' = 'Sales Channel');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `sales_channel` SET TAGS ('dbx_value_regex' = 'in_person|online|mobile_app|phone|concierge');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `service_charge_amount` SET TAGS ('dbx_business_glossary_term' = 'Service Charge Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `subtotal_amount` SET TAGS ('dbx_business_glossary_term' = 'Subtotal Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `total_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `total_quantity` SET TAGS ('dbx_business_glossary_term' = 'Total Quantity');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `transaction_date` SET TAGS ('dbx_business_glossary_term' = 'Transaction Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `transaction_number` SET TAGS ('dbx_business_glossary_term' = 'Transaction Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `transaction_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_transaction` ALTER COLUMN `transaction_status` SET TAGS ('dbx_business_glossary_term' = 'Transaction Status');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `health_safety_incident_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `health_safety_incident_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `batch_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `current_stock_quantity` SET TAGS ('dbx_business_glossary_term' = 'Current Stock Quantity');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `last_replenishment_quantity` SET TAGS ('dbx_business_glossary_term' = 'Last Replenishment Quantity');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `last_sale_date` SET TAGS ('dbx_business_glossary_term' = 'Last Sale Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Lead Time Days');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `maximum_stock_level` SET TAGS ('dbx_business_glossary_term' = 'Maximum Stock Level');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `reorder_point` SET TAGS ('dbx_business_glossary_term' = 'Reorder Point');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `reorder_quantity` SET TAGS ('dbx_business_glossary_term' = 'Reorder Quantity');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_inventory` ALTER COLUMN `reorder_triggered_flag` SET TAGS ('dbx_business_glossary_term' = 'Reorder Triggered Flag');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` SET TAGS ('dbx_subdomain' = 'membership_access');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` ALTER COLUMN `service_case_id` SET TAGS ('dbx_business_glossary_term' = 'Service Case Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` ALTER COLUMN `membership_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` ALTER COLUMN `promotional_code` SET TAGS ('dbx_business_glossary_term' = 'Promotional Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership` ALTER COLUMN `promotional_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,20}$');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` SET TAGS ('dbx_subdomain' = 'membership_access');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Processed By Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Room Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `check_in_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Check-In Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `check_out_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Check-Out Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `complimentary_flag` SET TAGS ('dbx_business_glossary_term' = 'Complimentary Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `credits_consumed` SET TAGS ('dbx_business_glossary_term' = 'Credits Consumed');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `feedback_comments` SET TAGS ('dbx_business_glossary_term' = 'Feedback Comments');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `gratuity_amount` SET TAGS ('dbx_business_glossary_term' = 'Gratuity Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `guest_count` SET TAGS ('dbx_business_glossary_term' = 'Guest Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `guest_satisfaction_score` SET TAGS ('dbx_business_glossary_term' = 'Guest Satisfaction Score (GSS)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `no_show_flag` SET TAGS ('dbx_business_glossary_term' = 'No-Show Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `remaining_credit_balance` SET TAGS ('dbx_business_glossary_term' = 'Remaining Credit Balance');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `retail_purchase_amount` SET TAGS ('dbx_business_glossary_term' = 'Retail Purchase Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `scheduled_end_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled End Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `scheduled_start_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Start Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `special_request` SET TAGS ('dbx_business_glossary_term' = 'Special Request');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `visit_date` SET TAGS ('dbx_business_glossary_term' = 'Visit Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `visit_number` SET TAGS ('dbx_business_glossary_term' = 'Visit Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `visit_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`membership_visit` ALTER COLUMN `visit_status` SET TAGS ('dbx_business_glossary_term' = 'Visit Status');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` SET TAGS ('dbx_subdomain' = 'membership_access');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Booking Channel ID');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `adult_count` SET TAGS ('dbx_business_glossary_term' = 'Adult Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `booking_source` SET TAGS ('dbx_business_glossary_term' = 'Booking Source');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `booking_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Booking Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `cancellation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `check_in_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Check-In Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `check_out_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Check-Out Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `child_count` SET TAGS ('dbx_business_glossary_term' = 'Child Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Confirmation Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,15}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Discount Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `group_booking_flag` SET TAGS ('dbx_business_glossary_term' = 'Group Booking Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `guest_type` SET TAGS ('dbx_business_glossary_term' = 'Guest Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `guest_type` SET TAGS ('dbx_value_regex' = 'local_resident|tourist|business_traveler|group_member|loyalty_member|non_member');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `loyalty_member_flag` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Member Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `package_flag` SET TAGS ('dbx_business_glossary_term' = 'Package Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_date` SET TAGS ('dbx_business_glossary_term' = 'Day Pass Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_number` SET TAGS ('dbx_business_glossary_term' = 'Day Pass Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,20}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_status` SET TAGS ('dbx_business_glossary_term' = 'Day Pass Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_status` SET TAGS ('dbx_value_regex' = 'sold|active|expired|refunded|cancelled|no_show');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_type` SET TAGS ('dbx_business_glossary_term' = 'Day Pass Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `pass_type` SET TAGS ('dbx_value_regex' = 'spa_day_pass|pool_day_pass|fitness_day_pass|golf_day_pass|resort_amenity_pass|wellness_day_pass');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `refund_amount` SET TAGS ('dbx_business_glossary_term' = 'Refund Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `refund_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Refund Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `service_charge_amount` SET TAGS ('dbx_business_glossary_term' = 'Service Charge Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `special_requests` SET TAGS ('dbx_business_glossary_term' = 'Special Requests');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `subtotal_amount` SET TAGS ('dbx_business_glossary_term' = 'Subtotal Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `total_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`day_pass` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Unit Price');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` SET TAGS ('dbx_subdomain' = 'facility_operations');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Instructor ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `age_restriction_maximum` SET TAGS ('dbx_business_glossary_term' = 'Maximum Age Restriction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `age_restriction_minimum` SET TAGS ('dbx_business_glossary_term' = 'Minimum Age Restriction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `cancellation_policy_hours` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Hours');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_benefits` SET TAGS ('dbx_business_glossary_term' = 'Class Benefits');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_code` SET TAGS ('dbx_business_glossary_term' = 'Class Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,12}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_description` SET TAGS ('dbx_business_glossary_term' = 'Class Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_image_url` SET TAGS ('dbx_business_glossary_term' = 'Class Image URL');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_name` SET TAGS ('dbx_business_glossary_term' = 'Class Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_status` SET TAGS ('dbx_business_glossary_term' = 'Class Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_status` SET TAGS ('dbx_value_regex' = 'active|inactive|seasonal|suspended|discontinued');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_subcategory` SET TAGS ('dbx_business_glossary_term' = 'Class Subcategory');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `class_type` SET TAGS ('dbx_business_glossary_term' = 'Class Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `complimentary_flag` SET TAGS ('dbx_business_glossary_term' = 'Complimentary Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `contraindications` SET TAGS ('dbx_business_glossary_term' = 'Contraindications');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Duration in Minutes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `equipment_provided_flag` SET TAGS ('dbx_business_glossary_term' = 'Equipment Provided Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `equipment_required` SET TAGS ('dbx_business_glossary_term' = 'Equipment Required');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `fitness_level` SET TAGS ('dbx_business_glossary_term' = 'Fitness Level');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `fitness_level` SET TAGS ('dbx_value_regex' = 'beginner|intermediate|advanced|all_levels');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `fitness_level` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `instructor_certification_required` SET TAGS ('dbx_business_glossary_term' = 'Instructor Certification Required');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `intensity_level` SET TAGS ('dbx_business_glossary_term' = 'Intensity Level');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `intensity_level` SET TAGS ('dbx_value_regex' = 'low|moderate|high|variable');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `intensity_level` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `loyalty_points_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `maximum_participants` SET TAGS ('dbx_business_glossary_term' = 'Maximum Participants');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `member_price` SET TAGS ('dbx_business_glossary_term' = 'Member Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `minimum_participants` SET TAGS ('dbx_business_glossary_term' = 'Minimum Participants');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_business_glossary_term' = 'Online Booking Enabled Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `package_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Package Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `pregnancy_safe_flag` SET TAGS ('dbx_business_glossary_term' = 'Pregnancy Safe Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `seasonal_availability` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Availability');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class` ALTER COLUMN `seasonal_availability` SET TAGS ('dbx_value_regex' = 'year_round|summer_only|winter_only|spring_fall|peak_season|off_season');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` SET TAGS ('dbx_subdomain' = 'facility_operations');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Instructor ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `booking_status` SET TAGS ('dbx_business_glossary_term' = 'Booking Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `booking_status` SET TAGS ('dbx_value_regex' = 'not_open|open|limited|closed|waitlist_only');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `cancellation_notes` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `cancellation_policy_hours` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Hours');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `class_format` SET TAGS ('dbx_business_glossary_term' = 'Class Format');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `class_format` SET TAGS ('dbx_value_regex' = 'in_person|virtual|hybrid|outdoor|poolside');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `difficulty_level` SET TAGS ('dbx_business_glossary_term' = 'Difficulty Level');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `difficulty_level` SET TAGS ('dbx_value_regex' = 'beginner|intermediate|advanced|all_levels|adaptive');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `difficulty_level` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Session Duration Minutes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `end_time` SET TAGS ('dbx_business_glossary_term' = 'Session End Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `enrolled_count` SET TAGS ('dbx_business_glossary_term' = 'Enrolled Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `equipment_required` SET TAGS ('dbx_business_glossary_term' = 'Equipment Required');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `guest_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Guest Segment');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `guest_segment` SET TAGS ('dbx_value_regex' = 'resort_guest|spa_member|day_pass|local_resident|loyalty_member|corporate_group');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `loyalty_points_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `loyalty_points_value` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Value');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `maximum_capacity` SET TAGS ('dbx_business_glossary_term' = 'Maximum Capacity');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `minimum_age` SET TAGS ('dbx_business_glossary_term' = 'Minimum Age');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Session Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_business_glossary_term' = 'Online Booking Enabled Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `published_flag` SET TAGS ('dbx_business_glossary_term' = 'Published Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `published_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Published Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `revenue_center` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_code` SET TAGS ('dbx_business_glossary_term' = 'Session Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_date` SET TAGS ('dbx_business_glossary_term' = 'Session Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_name` SET TAGS ('dbx_business_glossary_term' = 'Session Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_price` SET TAGS ('dbx_business_glossary_term' = 'Session Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_status` SET TAGS ('dbx_business_glossary_term' = 'Session Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_type` SET TAGS ('dbx_business_glossary_term' = 'Session Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`fitness_class_session` ALTER COLUMN `session_type` SET TAGS ('dbx_value_regex' = 'group|private|semi_private|complimentary|premium|special_event');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_subdomain' = 'booking_services');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_ssot_owner' = 'spa.spa_class_enrollment');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_ssot_family' = 'class_enrollment');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_ssot_role' = 'derived');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_ssot_source' = 'event.event_class_enrollment');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` SET TAGS ('dbx_ssot' = 'owner');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Cancelled By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` ALTER COLUMN `enrollment_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`spa_class_enrollment` ALTER COLUMN `folio_reference_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Booked By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `booking_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Booking Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `caddie_fee_per_caddie` SET TAGS ('dbx_business_glossary_term' = 'Caddie Fee Per Caddie');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `caddie_requested_flag` SET TAGS ('dbx_business_glossary_term' = 'Caddie Requested Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_value_regex' = 'guest_request|weather|course_maintenance|overbooking|no_show|other');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `cancellation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `cart_fee_per_cart` SET TAGS ('dbx_business_glossary_term' = 'Cart Fee Per Cart');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `cart_rental_flag` SET TAGS ('dbx_business_glossary_term' = 'Cart Rental Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `check_in_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Check-In Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `club_rental_fee_per_set` SET TAGS ('dbx_business_glossary_term' = 'Club Rental Fee Per Set');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `club_rental_flag` SET TAGS ('dbx_business_glossary_term' = 'Club Rental Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completion Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Tee Time Confirmation Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,12}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `confirmation_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `loyalty_member_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `number_of_carts` SET TAGS ('dbx_business_glossary_term' = 'Number of Carts');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `number_of_club_sets` SET TAGS ('dbx_business_glossary_term' = 'Number of Club Sets');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `number_of_holes` SET TAGS ('dbx_business_glossary_term' = 'Number of Holes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `number_of_players` SET TAGS ('dbx_business_glossary_term' = 'Number of Players');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `rate_type` SET TAGS ('dbx_business_glossary_term' = 'Rate Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `rate_type` SET TAGS ('dbx_value_regex' = 'rack|guest|member|twilight|promotional|tournament');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `special_requests` SET TAGS ('dbx_business_glossary_term' = 'Special Requests');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `tee_time` SET TAGS ('dbx_business_glossary_term' = 'Tee Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `tee_time_date` SET TAGS ('dbx_business_glossary_term' = 'Tee Time Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `tee_time_status` SET TAGS ('dbx_business_glossary_term' = 'Tee Time Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`golf_tee_time` ALTER COLUMN `tee_time_status` SET TAGS ('dbx_value_regex' = 'booked|confirmed|checked_in|completed|cancelled|no_show');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'draft|pending_approval|approved|rejected');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,15}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `commission_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Commission Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `commission_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Commission Rate Percent');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `day_of_week_applicability` SET TAGS ('dbx_business_glossary_term' = 'Day of Week Applicability');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `day_of_week_applicability` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `discount_percentage` SET TAGS ('dbx_business_glossary_term' = 'Discount Percentage');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `gift_certificate_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Gift Certificate Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `gratuity_included_flag` SET TAGS ('dbx_business_glossary_term' = 'Gratuity Included Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `guest_segment` SET TAGS ('dbx_business_glossary_term' = 'Guest Segment');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `loyalty_tier_code` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Tier Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `loyalty_tier_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,15}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `maximum_advance_booking_days` SET TAGS ('dbx_business_glossary_term' = 'Maximum Advance Booking Days');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `maximum_charge` SET TAGS ('dbx_business_glossary_term' = 'Maximum Charge');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `minimum_advance_booking_hours` SET TAGS ('dbx_business_glossary_term' = 'Minimum Advance Booking Hours');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `minimum_charge` SET TAGS ('dbx_business_glossary_term' = 'Minimum Charge');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_business_glossary_term' = 'Online Booking Enabled Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `package_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Package Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `price_type` SET TAGS ('dbx_business_glossary_term' = 'Price Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_code` SET TAGS ('dbx_business_glossary_term' = 'Pricing Rule Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_-]{3,20}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_name` SET TAGS ('dbx_business_glossary_term' = 'Pricing Rule Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_rule_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `pricing_unit` SET TAGS ('dbx_business_glossary_term' = 'Pricing Unit');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Priority Rank');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `season_code` SET TAGS ('dbx_business_glossary_term' = 'Season Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `season_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,10}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `tax_inclusive_flag` SET TAGS ('dbx_business_glossary_term' = 'Tax Inclusive Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `time_of_day_end` SET TAGS ('dbx_business_glossary_term' = 'Time of Day End');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `time_of_day_end` SET TAGS ('dbx_value_regex' = '^([01]?[0-9]|2[0-3]):[0-5][0-9]$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `time_of_day_end` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `time_of_day_start` SET TAGS ('dbx_business_glossary_term' = 'Time of Day Start');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`amenity_pricing` ALTER COLUMN `time_of_day_start` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Cancelled By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Fee Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_fee_applicable_flag` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Fee Applicable Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_fee_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Fee Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_fee_waived_flag` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Fee Waived Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_notes` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_number` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_type` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancellation_type` SET TAGS ('dbx_value_regex' = 'CANCELLATION|NO_SHOW|LATE_CANCELLATION');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancelled_by_party` SET TAGS ('dbx_business_glossary_term' = 'Cancelled By Party');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `cancelled_by_party` SET TAGS ('dbx_value_regex' = 'GUEST|STAFF|SYSTEM|PROPERTY');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `fee_waiver_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Fee Waiver Reason Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `fee_waiver_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Fee Waiver Reason Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `guest_notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Guest Notification Sent Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `guest_notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Guest Notification Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `late_cancellation_flag` SET TAGS ('dbx_business_glossary_term' = 'Late Cancellation Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `no_show_flag` SET TAGS ('dbx_business_glossary_term' = 'No-Show Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `original_appointment_value` SET TAGS ('dbx_business_glossary_term' = 'Original Appointment Value');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `rebooking_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Rebooking Completed Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `rebooking_offer_extended_flag` SET TAGS ('dbx_business_glossary_term' = 'Rebooking Offer Extended Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`cancellation_log` ALTER COLUMN `revenue_recovery_amount` SET TAGS ('dbx_business_glossary_term' = 'Revenue Recovery Amount');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` SET TAGS ('dbx_subdomain' = 'facility_operations');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `pip_item_id` SET TAGS ('dbx_business_glossary_term' = 'Pip Item Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `capacity_rating` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `condition_rating` SET TAGS ('dbx_business_glossary_term' = 'Condition Rating');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `condition_rating` SET TAGS ('dbx_value_regex' = 'excellent|good|fair|poor|critical');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `condition_rating` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `depreciation_method` SET TAGS ('dbx_business_glossary_term' = 'Depreciation Method');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `depreciation_method` SET TAGS ('dbx_value_regex' = 'straight line|declining balance|units of production|not applicable');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `dimensions` SET TAGS ('dbx_business_glossary_term' = 'Dimensions');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `disposal_date` SET TAGS ('dbx_business_glossary_term' = 'Disposal Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `disposal_method` SET TAGS ('dbx_business_glossary_term' = 'Disposal Method');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `disposal_method` SET TAGS ('dbx_value_regex' = 'sold|donated|recycled|scrapped|transferred');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `equipment_type` SET TAGS ('dbx_business_glossary_term' = 'Equipment Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `expected_useful_life_years` SET TAGS ('dbx_business_glossary_term' = 'Expected Useful Life Years');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `installation_date` SET TAGS ('dbx_business_glossary_term' = 'Installation Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `last_safety_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Safety Inspection Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `last_service_date` SET TAGS ('dbx_business_glossary_term' = 'Last Service Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `manufacturer` SET TAGS ('dbx_business_glossary_term' = 'Manufacturer Name');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `model_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `equipment_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `equipment_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `equipment_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `equipment_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `operational_status` SET TAGS ('dbx_business_glossary_term' = 'Operational Status');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `serial_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`equipment` ALTER COLUMN `service_contract_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` SET TAGS ('dbx_subdomain' = 'treatment_catalog');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `menu_id` SET TAGS ('dbx_business_glossary_term' = 'Dietary Menu Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `guest_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Guest Segment Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `booking_channel_availability` SET TAGS ('dbx_business_glossary_term' = 'Booking Channel Availability');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `cancellation_policy_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `dietary_component_flag` SET TAGS ('dbx_business_glossary_term' = 'Dietary Component Included Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `duration_days` SET TAGS ('dbx_business_glossary_term' = 'Program Duration in Days');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Program Effective End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Program Effective Start Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_value_regex' = '^[0-9]{4,10}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `included_fitness_sessions` SET TAGS ('dbx_business_glossary_term' = 'Included Fitness Sessions Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `loyalty_points_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Eligible Flag');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `medical_supervision_required_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `medical_supervision_required_flag` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `medical_supervision_required_flag` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Program Operational Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_business_glossary_term' = 'Online Booking Enabled Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `package_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Package Bundling Eligible Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_brochure_url` SET TAGS ('dbx_business_glossary_term' = 'Program Brochure Document URL');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,12}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_description` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_image_url` SET TAGS ('dbx_business_glossary_term' = 'Program Marketing Image URL');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_name` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_price` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `program_type` SET TAGS ('dbx_value_regex' = 'detox|weight_management|stress_relief|sleep_optimization|fitness_bootcamp|mindfulness_retreat');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `revenue_center_code` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`wellness_program` ALTER COLUMN `season_type` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Availability Type');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` SET TAGS ('dbx_subdomain' = 'treatment_catalog');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_code` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `frequency_per_week` SET TAGS ('dbx_business_glossary_term' = 'Weekly Treatment Frequency');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_status` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`program_treatment` ALTER COLUMN `program_treatment_status` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` SET TAGS ('dbx_subdomain' = 'treatment_catalog');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_code` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `included_treatments` SET TAGS ('dbx_business_glossary_term' = 'Included Treatments List');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Active Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `optional_flag` SET TAGS ('dbx_business_glossary_term' = 'Optional Treatment Indicator');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_status` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `package_treatment_status` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `quantity` SET TAGS ('dbx_business_glossary_term' = 'Treatment Quantity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`package_treatment` ALTER COLUMN `time_of_day_preference` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` SET TAGS ('dbx_subdomain' = 'retail_revenue');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` SET TAGS ('dbx_ssot_role' = 'owner');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` SET TAGS ('dbx_ssot_family' = 'spa_product');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` SET TAGS ('dbx_ssot_domain_anchor' = 'spa');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `base_price` SET TAGS ('dbx_monetary' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_category` SET TAGS ('dbx_business_glossary_term' = 'Product Category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_code` SET TAGS ('dbx_business_glossary_term' = 'Product Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `commission_rate` SET TAGS ('dbx_business_glossary_term' = 'Commission Rate');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `commission_rate` SET TAGS ('dbx_monetary' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_temporal_normalized' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Duration Minutes');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_temporal_normalized' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_temporal_normalized' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_sensitivity' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_category' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_class' = 'special_category');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gift_certificate_eligible_flag` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gift_certificate_eligible_flag` SET TAGS ('dbx_typed' = 'boolean');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gratuity_eligible_flag` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `gratuity_eligible_flag` SET TAGS ('dbx_typed' = 'boolean');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `inventory_tracked_flag` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `inventory_tracked_flag` SET TAGS ('dbx_typed' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `is_active` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `is_active` SET TAGS ('dbx_typed' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_temporal_normalized' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `member_price` SET TAGS ('dbx_business_glossary_term' = 'Member Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `member_price` SET TAGS ('dbx_monetary' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `minimum_age_requirement` SET TAGS ('dbx_business_glossary_term' = 'Minimum Age Requirement');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_temporal_normalized' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `product_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `online_booking_enabled_flag` SET TAGS ('dbx_typed' = 'boolean');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `structure_verified` SET TAGS ('dbx_model_meta' = 'structure_check_17_domains_356_products');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `taxable_flag` SET TAGS ('dbx_semantic' = 'boolean');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product` ALTER COLUMN `taxable_flag` SET TAGS ('dbx_typed' = 'boolean');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` SET TAGS ('dbx_subdomain' = 'booking_services');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Id');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `billing_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `billing_address` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `billing_address` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `billing_address` SET TAGS ('dbx_pii_class' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `billing_address` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_number` SET TAGS ('dbx_business_glossary_term' = 'Booking Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_source` SET TAGS ('dbx_business_glossary_term' = 'Booking Source');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_status` SET TAGS ('dbx_business_glossary_term' = 'Booking Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `booking_type` SET TAGS ('dbx_business_glossary_term' = 'Booking Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `cancellation_policy` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `cancelled_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancelled Timestamp');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_name` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_phone` SET TAGS ('dbx_pii_type' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_phone` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_phone` SET TAGS ('dbx_pii_class' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `lead_guest_phone` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `number_of_guests` SET TAGS ('dbx_business_glossary_term' = 'Number Of Guests');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `number_of_rooms` SET TAGS ('dbx_business_glossary_term' = 'Number Of Rooms');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `organization_name` SET TAGS ('dbx_business_glossary_term' = 'Organization Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `organization_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `organization_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `organization_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `organization_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`group_booking` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `brand_name` SET TAGS ('dbx_business_glossary_term' = 'Brand Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `brand_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `brand_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `brand_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `retail_product_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `discontinuation_date` SET TAGS ('dbx_business_glossary_term' = 'Discontinuation Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `height_cm` SET TAGS ('dbx_business_glossary_term' = 'Height Cm');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `image_url` SET TAGS ('dbx_business_glossary_term' = 'Image Url');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `ingredients` SET TAGS ('dbx_business_glossary_term' = 'Ingredients');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `launch_date` SET TAGS ('dbx_business_glossary_term' = 'Launch Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `length_cm` SET TAGS ('dbx_business_glossary_term' = 'Length Cm');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `loyalty_points` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `manufacturer_name` SET TAGS ('dbx_business_glossary_term' = 'Manufacturer Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `manufacturer_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `manufacturer_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `manufacturer_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `manufacturer_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `minimum_stock_level` SET TAGS ('dbx_business_glossary_term' = 'Minimum Stock Level');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `organic_certified` SET TAGS ('dbx_business_glossary_term' = 'Organic Certified');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `package_size` SET TAGS ('dbx_business_glossary_term' = 'Package Size');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `package_size` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `product_code` SET TAGS ('dbx_business_glossary_term' = 'Product Code');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `product_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `product_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `product_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `product_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit Of Measure');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Unit Price');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`retail_product` ALTER COLUMN `vegan` SET TAGS ('dbx_business_glossary_term' = 'Vegan');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` SET TAGS ('dbx_subdomain' = 'retail_revenue');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `workforce_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Id');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `workforce_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `workforce_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `brand_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `brand_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `brand_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `commission_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Commission Rate Percent');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_sensitivity' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_category' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `gender_restriction` SET TAGS ('dbx_pii_class' = 'special_category');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_code` SET TAGS ('dbx_business_glossary_term' = 'Line Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_description` SET TAGS ('dbx_business_glossary_term' = 'Line Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_name` SET TAGS ('dbx_business_glossary_term' = 'Line Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `line_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `loyalty_points_multiplier` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Multiplier');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `minimum_age_requirement` SET TAGS ('dbx_business_glossary_term' = 'Minimum Age Requirement');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `requires_medical_clearance_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `requires_medical_clearance_flag` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_travel_hospitality_v1`.`spa`.`product_line` ALTER COLUMN `requires_medical_clearance_flag` SET TAGS ('dbx_mask_non_prod' = 'true');
