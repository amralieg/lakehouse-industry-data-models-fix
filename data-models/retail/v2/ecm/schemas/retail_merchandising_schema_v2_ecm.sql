...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` (
    ...
    `category_id` BIGINT COMMENT 'Category targeted',
    ...
    `merch_plan_id` BIGINT COMMENT 'Parent merch plan',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`category` (
    `category_id` BIGINT COMMENT 'Surrogate primary key uniquely identifying each merchandise category.',
    `buyer_id` BIGINT COMMENT 'Foreign key to the buyer responsible for this categorys assortment and vendor negotiations.',
    `cost_center_id` BIGINT COMMENT 'Foreign key to the finance cost center for category expense allocation.',
    `item_hierarchy_id` BIGINT COMMENT 'Linked product hierarchy node',
    `parent_category_id` BIGINT COMMENT 'Self-referencing foreign key to the parent category, enabling hierarchical tree traversal.',
    `associate_id` BIGINT COMMENT 'Foreign key to the associate supporting category operations.',
    `compliance_program_id` BIGINT COMMENT 'Foreign key to the primary compliance program governing this category (e.g. food safety for grocery).',
    `profit_center_id` BIGINT COMMENT 'Foreign key to the finance profit center for category revenue attribution.',
    `actual_gmroi` DECIMAL(18,2) COMMENT 'Actual achieved GMROI for the current or most recent completed period.',
    `actual_sell_through_rate` DECIMAL(18,2) COMMENT 'Actual sell-through rate achieved in the current or most recent period.',
    `assortment_breadth_target` STRING COMMENT 'Target number of distinct SKUs to carry in this category.',
    `assortment_depth_target` STRING COMMENT 'Target average units per SKU to maintain in this category.',
    `assortment_gap_findings` STRING COMMENT 'Notes from the most recent assortment gap analysis identifying missing product opportunities.',
    `category_role` STRING COMMENT 'Strategic role assigned to this category within the overall assortment strategy. Common values: destination, routine, seasonal, convenience, occasional.. Valid values are `destination|routine|convenience|seasonal|occasional`',
    `category_status` STRING COMMENT 'Lifecycle status of the category: active, inactive, pending_review, archived.. Valid values are `active|inactive|pending|discontinued`',
    `category_code` STRING COMMENT 'Short alphanumeric code uniquely identifying the category within the hierarchy.. Valid values are `^[A-Z0-9]{2,10}$`',
    `created_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this category record was first created.',
    `category_description` STRING COMMENT 'Detailed text description of the category scope and included merchandise.',
    `division` STRING COMMENT 'Top-level merchandise division this category belongs to.',
    `effective_end_date` DATE COMMENT 'Date until which this category definition is effective; NULL for current records.',
    `effective_start_date` DATE COMMENT 'Date from which this category definition is effective (SCD Type 2).',
    `hierarchy_level` STRING COMMENT 'Numeric depth in the category tree (1=division, 2=department, 3=class, 4=subclass, etc.).',
    `hierarchy_path` STRING COMMENT 'Materialized path string representing the full ancestry, e.g. /grocery/dairy/yogurt.',
    `is_leaf_node` BOOLEAN COMMENT 'Whether this category is a leaf node with no children; only leaf categories hold SKU assignments.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'UTC timestamp of the most recent modification to this record.',
    `last_review_date` DATE COMMENT 'Date of the most recent formal category review.',
    `merchandise_type` STRING COMMENT 'Classification of merchandise: hardlines, softlines, consumables, fresh, general_merchandise.',
    `modified_by_user` STRING COMMENT 'Username or system identifier of the last user to modify this record.',
    `category_name` STRING COMMENT 'Human-readable name of the category as displayed in planning tools and reports.',
    `next_review_date` DATE COMMENT 'Scheduled date for the next formal category review.',
    `otb_budget_amount` DECIMAL(18,2) COMMENT 'Open-to-buy budget allocation for this category in the current planning period.',
    `otb_currency_code` STRING COMMENT 'ISO 4217 currency code for the OTB budget amount.. Valid values are `^[A-Z]{3}$`',
    `peak_season` STRING COMMENT 'Primary peak selling season for this category: spring, summer, fall, winter, holiday, back_to_school.',
    `performance_variance_notes` STRING COMMENT 'Commentary on significant variances between target and actual category performance.',
    `private_label_penetration_target` DECIMAL(18,2) COMMENT 'Target percentage of sales from private/own-brand products within this category.',
    `recommended_actions` STRING COMMENT 'Recommended corrective or strategic actions from the latest category review.',
    `review_frequency` STRING COMMENT 'How often this category undergoes formal review: weekly, monthly, quarterly, annually.. Valid values are `weekly|monthly|quarterly|semi_annual|annual`',
    `seasonality_flag` BOOLEAN COMMENT 'Whether this category exhibits significant seasonal demand patterns.',
    `strategic_objective` STRING COMMENT 'Text description of the strategic objective for this category (e.g. grow_share, defend_margin, traffic_driver, profit_generator).',
    `target_gmroi` DECIMAL(18,2) COMMENT 'Target Gross Margin Return on Inventory Investment for category performance measurement.',
    `target_inventory_turns` DECIMAL(18,2) COMMENT 'Target annual inventory turnover rate for this category.',
    `target_margin_percent` DECIMAL(18,2) COMMENT 'Target gross margin percentage for the category, used in assortment and pricing decisions.',
    `target_sell_through_rate` DECIMAL(18,2) COMMENT 'Target sell-through rate as a percentage of units sold vs. units received.',
    CONSTRAINT pk_category PRIMARY KEY(`category_id`)
) COMMENT 'Hierarchical merchandise category structure used for assortment planning, financial reporting, and buyer accountability. Supports multiple hierarchy levels with parent-child relationships. Each category has performance targets, strategic objectives, and assigned ownership.';
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`buyer` (
    ...
    `associate_id` BIGINT COMMENT 'Workforce associate record',
    ...
    `primary_buyer_associate_id` BIGINT COMMENT 'Human resources employee identifier linking the buyer to the workforce management system. Used for payroll, performance tracking, and organizational hierarchy.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` (
    ...
    `merch_plan_id` BIGINT COMMENT 'Merch plan this OTB supports',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` (
    ...
    `sku_id` BIGINT COMMENT 'SKU ordered',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` (
    ...
    `location_id` BIGINT COMMENT 'Store location',
    ...
    `store_planogram_id` BIGINT COMMENT 'Foreign key reference to the SSOT owner store.store_planogram.',
...
) COMMENT '[SSOT Owner] Merchandising planning view of planograms for category management and space planning. References store.store_planogram as the authoritative execution planogram. Adds planning context like seasonal flags and buyer assignments.';
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` (
    ...
    `markdown_id` BIGINT COMMENT 'Linked pricing markdown',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`category_campaign_placement` (
    ...
    `promo_campaign_id` BIGINT COMMENT 'Campaign context',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` (
    `buyer_profit_center_assignment_id` BIGINT COMMENT 'Unique identifier for the buyer-profit center assignment',
    `associate_id` BIGINT COMMENT 'Reference to the associate who approved this assignment',
    `category_id` BIGINT COMMENT 'Reference to the primary merchandise category for this assignment',
    `cost_center_id` BIGINT COMMENT 'Reference to the associated cost center for expense tracking',
    `buyer_id` BIGINT COMMENT 'Reference to the merchandising buyer being assigned',
    `profit_center_id` BIGINT COMMENT 'Reference to the finance profit center being assigned to the buyer',
    `season_id` BIGINT COMMENT 'Reference to the merchandising season this assignment applies to',
    `actual_inventory_turns` DECIMAL(18,2) COMMENT 'Actual achieved inventory turnover rate for the current period.',
    ...
    `assigned_category_list` STRING COMMENT 'Comma-separated list of category codes assigned within this profit center',
    `assignment_notes` STRING COMMENT 'Notes about the scope or special conditions of this assignment.',
    `assignment_reason_code` STRING COMMENT 'Reason for the assignment (e.g., new_hire, reorganization, promotion, temporary_coverage).',
    `assignment_status` STRING COMMENT 'Current status (e.g. active, pending_approval, suspended, terminated)',
    `assignment_type` STRING COMMENT 'Type of assignment (e.g. primary, backup, interim, shared)',
    `budget_authority_limit_amount` DECIMAL(18,2) COMMENT 'Maximum purchase order amount the buyer can authorize without additional approval for this profit center.',
    ...
    `delegation_flag` BOOLEAN COMMENT 'Indicates if buying authority has been delegated to another buyer',
    `effective_end_date` DATE COMMENT 'Date when this assignment ends',
    `effective_start_date` DATE COMMENT 'Date when this assignment becomes effective',
    `escalation_threshold_amount` DECIMAL(18,2) COMMENT 'Monetary threshold above which purchases require escalation',
    `fiscal_year` STRING COMMENT 'Fiscal year to which this assignment and its targets apply.',
    `gross_margin_actual_pct` DECIMAL(18,2) COMMENT 'Actual gross margin percentage achieved for the current period.',
    `last_review_date` DATE COMMENT 'Date of the most recent performance review for this assignment',
    `margin_target_pct` DECIMAL(18,2) COMMENT 'Target gross margin percentage for this assignment',
    `margin_target_percentage` DECIMAL(18,2) COMMENT 'Target gross margin percentage for the buyer within this profit center.',
    `markdown_budget_amount` DECIMAL(18,2) COMMENT 'Allocated markdown budget for the buyer within this profit center.',
    `markdown_spent_amount` DECIMAL(18,2) COMMENT 'Markdown budget consumed to date.',
    `next_review_date` DATE COMMENT 'Date of the next scheduled review',
    `notes` STRING COMMENT 'Free-text notes about this assignment',
    `otb_allocation_amount` DECIMAL(18,2) COMMENT 'Open-to-buy budget amount allocated to this buyer for this profit center',
    `otb_consumed_amount` DECIMAL(18,2) COMMENT 'Open-to-buy budget consumed to date for this assignment.',
    `performance_bonus_pct` DECIMAL(18,2) COMMENT 'Performance bonus percentage tied to target achievement',
    `primary_flag` BOOLEAN COMMENT 'Indicates if this is the buyers primary profit center assignment',
    `revenue_target_amount` DECIMAL(18,2) COMMENT 'Revenue target for this buyer within this profit center',
    `review_frequency` STRING COMMENT 'How often this assignment is reviewed (e.g. monthly, quarterly, annually)',
    `shrinkage_target_pct` DECIMAL(18,2) COMMENT 'Target shrinkage percentage for this profit center assignment',
    `sku_count` STRING COMMENT 'Number of SKUs managed under this assignment',
    `store_count` STRING COMMENT 'Number of stores covered by this buyer-profit center assignment.',
    `target_inventory_turns` DECIMAL(18,2) COMMENT 'Target inventory turnover rate for the buyer within this profit center.',
    `territory_code` STRING COMMENT 'Geographic territory code associated with this profit center assignment.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to this record',
    `vendor_count` STRING COMMENT 'Number of vendors managed under this assignment',
    CONSTRAINT pk_buyer_profit_center_assignment PRIMARY KEY(`buyer_profit_center_assignment_id`)
) COMMENT 'Assigns buyers to profit centers with associated financial targets including OTB budgets, revenue goals, margin targets, and performance metrics for category management accountability.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`merchandising`.`category_accrual_rule` (
    `category_accrual_rule_id` BIGINT COMMENT 'Unique identifier for the category accrual rule',
    `accrual_rule_id` BIGINT COMMENT 'Reference to the parent loyalty accrual rule',
    `associate_id` BIGINT COMMENT 'Associate who approved this category accrual rule.',
    `category_created_by_associate_id` BIGINT COMMENT 'Reference to the associate who created this rule',
    `category_id` BIGINT COMMENT 'Reference to the merchandise category this rule applies to',
    `gl_account_id` BIGINT COMMENT 'Reference to the GL account for accrual liability posting',
    `promo_campaign_id` BIGINT COMMENT 'Reference to the promotional campaign if rule is campaign-specific',
    `season_id` BIGINT COMMENT 'Season during which this accrual rule is applicable.',
    `tier_id` BIGINT COMMENT 'Loyalty tier to which this accrual rule applies, if tier-specific.',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor funding this accrual if vendor-funded',
    `accrual_basis` STRING COMMENT 'Basis for accrual calculation such as net_sales, gross_sales, or units_sold.',
    `accrual_cap_amount` DECIMAL(18,2) COMMENT 'Maximum accrual amount that can be earned under this rule.',
    `accrual_frequency` STRING COMMENT 'How often accruals are calculated (e.g. per_transaction, daily_batch, weekly)',
    `accrual_rate_pct` DECIMAL(18,2) COMMENT 'Percentage rate applied for cashback-style accruals',
    `accrual_type` STRING COMMENT 'Type of accrual (e.g. points, cashback, miles, tier_credit)',
    `approval_date` DATE COMMENT 'Date when the accrual rule was approved.',
    `approval_status` STRING COMMENT 'Approval workflow status (e.g. pending, approved, rejected)',
    `auto_reverse_flag` BOOLEAN COMMENT 'Indicates whether accruals under this rule are automatically reversed at period close.',
    `cap_amount` DECIMAL(18,2) COMMENT 'Maximum accrual amount per period or transaction',
    `cap_period` STRING COMMENT 'Period over which the cap applies (e.g. daily, weekly, monthly, lifetime)',
    `category_accrual_rule_status` STRING COMMENT 'Current status (e.g. active, draft, suspended, expired)',
    `channel_applicability` STRING COMMENT 'Channels where this rule applies (e.g. in_store, online, all)',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the rule was created',
    `currency_code` STRING COMMENT 'ISO currency code for monetary thresholds and caps',
    `effective_end_date` DATE COMMENT 'Date when this accrual rule expires',
    `effective_start_date` DATE COMMENT 'Date when this accrual rule becomes active',
    `eligible_channel` STRING COMMENT 'Channel eligibility for this rule (in_store, online, omni).',
    `exclusion_sku_list` STRING COMMENT 'Comma-separated list of SKUs excluded from this accrual rule',
    `last_calculation_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent accrual calculation run',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp of the last modification to this rule',
    `last_posted_date` DATE COMMENT 'Date of the most recent accrual posting to the GL.',
    `maximum_accrual_amount` DECIMAL(18,2) COMMENT 'Maximum accrual amount or points cap per transaction or period.',
    `maximum_accrual_cap_amount` DECIMAL(18,2) COMMENT 'Maximum accrual amount per transaction or period.',
    `minimum_spend_threshold` DECIMAL(18,2) COMMENT 'Minimum transaction amount required to trigger accrual',
    `notes` STRING COMMENT 'Free-text notes about this accrual rule',
    `points_multiplier` DECIMAL(18,2) COMMENT 'Multiplier applied to base points earning for this category',
    `posting_frequency` STRING COMMENT 'Frequency at which accruals are posted to the general ledger such as daily, weekly, or monthly.',
    `qualifying_sales_threshold` DECIMAL(18,2) COMMENT 'Minimum qualifying sales amount before accrual begins.',
    `qualifying_transaction_type` STRING COMMENT 'Transaction types that qualify (e.g. purchase, exchange, all)',
    `retroactive_flag` BOOLEAN COMMENT 'Indicates if the rule applies retroactively to past transactions',
    `reversal_allowed_flag` BOOLEAN COMMENT 'Indicates if accrued points can be reversed on return',
    `rule_description` STRING COMMENT 'Detailed description of the accrual rule logic and business purpose.',
    `rule_name` STRING COMMENT 'Human-readable name identifying this accrual rule for reporting and management.',
    `rule_priority` STRING COMMENT 'Priority ranking when multiple rules apply to the same transaction',
    `settlement_frequency` STRING COMMENT 'How often accruals are settled (e.g. monthly, quarterly, annually).',
    `settlement_method` STRING COMMENT 'How vendor-funded accruals are settled (e.g. invoice, deduction, credit)',
    `stacking_allowed_flag` BOOLEAN COMMENT 'Whether this rule can stack with other accrual rules on the same transaction.',
    `tier_requirement` STRING COMMENT 'Minimum loyalty tier required to benefit from this rule',
    `total_accrued_amount` DECIMAL(18,2) COMMENT 'Running total of amounts accrued under this rule',
    `total_posted_amount` DECIMAL(18,2) COMMENT 'Cumulative amount posted under this accrual rule.',
    `transaction_type_filter` STRING COMMENT 'Transaction types eligible for this rule (e.g., sale, return_credit).',
    `variance_threshold_pct` DECIMAL(18,2) COMMENT 'Percentage threshold above which accrual variances trigger alerts or reviews.',
    CONSTRAINT pk_category_accrual_rule PRIMARY KEY(`category_accrual_rule_id`)
) COMMENT 'Defines loyalty points accrual rules at the merchandise category level, specifying multipliers, caps, channel applicability, and vendor-funded accrual configurations for category-specific earning programs.';
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ADD CONSTRAINT `fk_merchandising_assortment_plan_primary_assortment_category_id` FOREIGN KEY (`primary_assortment_category_id`) REFERENCES `vibe_retail_v1`.`merchandising`.`category`(`category_id`);
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` ADD CONSTRAINT `fk_merchandising_buyer_profit_center_assignment_category_id` FOREIGN KEY (`category_id`) REFERENCES `vibe_retail_v1`.`merchandising`.`category`(`category_id`);
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` ADD CONSTRAINT `fk_merchandising_buyer_profit_center_assignment_season_id` FOREIGN KEY (`season_id`) REFERENCES `vibe_retail_v1`.`merchandising`.`season`(`season_id`);
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category_accrual_rule` ADD CONSTRAINT `fk_merchandising_category_accrual_rule_season_id` FOREIGN KEY (`season_id`) REFERENCES `vibe_retail_v1`.`merchandising`.`season`(`season_id`);
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `season_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `inventory_turn_target` SET TAGS ('dbx_business_glossary_term' = 'Inventory Turn Target');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `plan_end_date` SET TAGS ('dbx_business_glossary_term' = 'Plan End Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `plan_notes` SET TAGS ('dbx_business_glossary_term' = 'Plan Notes');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `plan_start_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Start Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_beginning_inventory_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Beginning Inventory Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Cost Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_ending_inventory_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Ending Inventory Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_margin_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Margin Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_margin_percent` SET TAGS ('dbx_business_glossary_term' = 'Planned Margin Percent');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_markdown_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Markdown Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_markdown_percent` SET TAGS ('dbx_business_glossary_term' = 'Planned Markdown Percent');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_receipt_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Receipt Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_sales_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Sales Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `planned_units` SET TAGS ('dbx_business_glossary_term' = 'Planned Units');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `prior_year_margin_percent` SET TAGS ('dbx_business_glossary_term' = 'Prior Year Margin Percent');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `prior_year_sales_amount` SET TAGS ('dbx_business_glossary_term' = 'Prior Year Sales Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `prior_year_units` SET TAGS ('dbx_business_glossary_term' = 'Prior Year Units');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merch_plan` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `assortment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Assortment Plan ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `primary_assortment_category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `assortment_breadth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Breadth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `assortment_depth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Depth Target');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `external_plan_reference` SET TAGS ('dbx_business_glossary_term' = 'External Plan Reference');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `otb_budget_amount` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_plan` ALTER COLUMN `target_inventory_turn_rate` SET TAGS ('dbx_business_glossary_term' = 'Target Inventory Turn Rate');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` SET TAGS ('dbx_domain' = 'merchandising');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` SET TAGS ('dbx_subdomain' = 'category_management');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` SET TAGS ('dbx_layer' = 'ecm');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `parent_category_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `assortment_breadth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Breadth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `assortment_depth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Depth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `assortment_gap_findings` SET TAGS ('dbx_business_glossary_term' = 'Assortment Gap Findings');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_role` SET TAGS ('dbx_business_glossary_term' = 'Category Role');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_status` SET TAGS ('dbx_business_glossary_term' = 'Category Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_code` SET TAGS ('dbx_business_glossary_term' = 'Category Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_description` SET TAGS ('dbx_business_glossary_term' = 'Category Description');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `division` SET TAGS ('dbx_business_glossary_term' = 'Division');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `hierarchy_level` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Level');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `hierarchy_path` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Path');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `is_leaf_node` SET TAGS ('dbx_business_glossary_term' = 'Is Leaf Node');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `merchandise_type` SET TAGS ('dbx_business_glossary_term' = 'Merchandise Type');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_name` SET TAGS ('dbx_business_glossary_term' = 'Category Name');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `category_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `otb_budget_amount` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `peak_season` SET TAGS ('dbx_business_glossary_term' = 'Peak Season');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `performance_variance_notes` SET TAGS ('dbx_business_glossary_term' = 'Performance Variance Notes');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `private_label_penetration_target` SET TAGS ('dbx_business_glossary_term' = 'Private Label Penetration Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `recommended_actions` SET TAGS ('dbx_business_glossary_term' = 'Recommended Actions');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `seasonality_flag` SET TAGS ('dbx_business_glossary_term' = 'Seasonality Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `strategic_objective` SET TAGS ('dbx_business_glossary_term' = 'Strategic Objective');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `target_inventory_turns` SET TAGS ('dbx_business_glossary_term' = 'Target Inventory Turns');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category` ALTER COLUMN `target_margin_percent` SET TAGS ('dbx_business_glossary_term' = 'Target Margin Percent');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` SET TAGS ('dbx_subdomain' = 'buying_operations');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `primary_buyer_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `semantic_layer_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Semantic Layer Entity Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `assigned_category_codes` SET TAGS ('dbx_business_glossary_term' = 'Assigned Category Codes');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `assigned_department_codes` SET TAGS ('dbx_business_glossary_term' = 'Assigned Department Codes');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_status` SET TAGS ('dbx_business_glossary_term' = 'Buyer Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_type` SET TAGS ('dbx_business_glossary_term' = 'Buyer Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buying_authority_limit` SET TAGS ('dbx_business_glossary_term' = 'Buying Authority Limit');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buying_authority_limit` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_code` SET TAGS ('dbx_business_glossary_term' = 'Buyer Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `data_source_system` SET TAGS ('dbx_business_glossary_term' = 'Data Source System');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `division_code` SET TAGS ('dbx_business_glossary_term' = 'Division Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `international_sourcing_flag` SET TAGS ('dbx_business_glossary_term' = 'International Sourcing Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `inventory_turn_target` SET TAGS ('dbx_business_glossary_term' = 'Inventory Turn Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `language_proficiency` SET TAGS ('dbx_business_glossary_term' = 'Language Proficiency');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `last_modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `last_performance_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Performance Review Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `markdown_percentage_limit` SET TAGS ('dbx_business_glossary_term' = 'Markdown Percentage Limit');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `buyer_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `office_location_code` SET TAGS ('dbx_business_glossary_term' = 'Office Location Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `otb_budget_limit` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `preferred_vendor_count` SET TAGS ('dbx_business_glossary_term' = 'Preferred Vendor Count');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `private_label_focus_flag` SET TAGS ('dbx_business_glossary_term' = 'Private Label Focus Flag');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `vendor_negotiation_rating` SET TAGS ('dbx_business_glossary_term' = 'Vendor Negotiation Rating');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer` ALTER COLUMN `years_of_buying_experience` SET TAGS ('dbx_business_glossary_term' = 'Years of Buying Experience');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `assortment_breadth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Breadth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `assortment_depth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Depth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `buy_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Buy Deadline Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `clearance_exit_date` SET TAGS ('dbx_business_glossary_term' = 'Clearance Exit Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_code` SET TAGS ('dbx_business_glossary_term' = 'Season Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}[0-9]{2}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `first_receipt_date` SET TAGS ('dbx_business_glossary_term' = 'First Receipt Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `line_review_date` SET TAGS ('dbx_business_glossary_term' = 'Line Review Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `markdown_entry_date` SET TAGS ('dbx_business_glossary_term' = 'Markdown Entry Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_name` SET TAGS ('dbx_business_glossary_term' = 'Season Name');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `otb_budget_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `planning_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planning Start Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_status` SET TAGS ('dbx_business_glossary_term' = 'Season Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `season_type` SET TAGS ('dbx_business_glossary_term' = 'Season Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `target_gmroi` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `target_inventory_turns` SET TAGS ('dbx_business_glossary_term' = 'Target Inventory Turns');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `target_inventory_turns` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `target_sell_through_rate` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`season` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `assortment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Assortment Plan ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `actual_receipts_at_cost` SET TAGS ('dbx_business_glossary_term' = 'Actual Receipts at Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_adjustment_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Adjustment Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_decrease_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Decrease Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_end_date` SET TAGS ('dbx_business_glossary_term' = 'Budget End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_increase_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Increase Amount');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_start_date` SET TAGS ('dbx_business_glossary_term' = 'Budget Start Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_transfer_in_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Transfer In Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `budget_transfer_out_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Transfer Out Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `committed_amount` SET TAGS ('dbx_business_glossary_term' = 'Committed Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `inventory_turn_target` SET TAGS ('dbx_business_glossary_term' = 'Inventory Turn Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `last_reconciliation_date` SET TAGS ('dbx_business_glossary_term' = 'Last Reconciliation Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`otb_budget` ALTER COLUMN `planned_receipts_at_cost` SET TAGS ('dbx_business_glossary_term' = 'Planned Receipts at Cost');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` SET TAGS ('dbx_subdomain' = 'buying_operations');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `buying_order_id` SET TAGS ('dbx_business_glossary_term' = 'Buying Order ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Destination Location ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `cancellation_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `destination_type` SET TAGS ('dbx_business_glossary_term' = 'Destination Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `duty_cost` SET TAGS ('dbx_business_glossary_term' = 'Duty Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `exchange_rate` SET TAGS ('dbx_business_glossary_term' = 'Exchange Rate');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `freight_cost` SET TAGS ('dbx_business_glossary_term' = 'Freight Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `landed_cost` SET TAGS ('dbx_business_glossary_term' = 'Landed Cost');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `planned_receipt_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Receipt Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `ship_window_end_date` SET TAGS ('dbx_business_glossary_term' = 'Ship Window End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `ship_window_start_date` SET TAGS ('dbx_business_glossary_term' = 'Ship Window Start Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `total_order_cost` SET TAGS ('dbx_business_glossary_term' = 'Total Order Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `total_order_quantity` SET TAGS ('dbx_business_glossary_term' = 'Total Order Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order` ALTER COLUMN `vendor_order_number` SET TAGS ('dbx_business_glossary_term' = 'Vendor Order Number');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` SET TAGS ('dbx_subdomain' = 'buying_operations');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `buying_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Buying Order Line ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `buying_order_id` SET TAGS ('dbx_business_glossary_term' = 'Buying Order ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `allocation_quantity` SET TAGS ('dbx_business_glossary_term' = 'Allocation Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `cancel_date` SET TAGS ('dbx_business_glossary_term' = 'Cancel Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `cancelled_quantity` SET TAGS ('dbx_business_glossary_term' = 'Cancelled Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `delivery_date` SET TAGS ('dbx_business_glossary_term' = 'Delivery Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `drop_ship_flag` SET TAGS ('dbx_business_glossary_term' = 'Drop Ship Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `extended_cost` SET TAGS ('dbx_business_glossary_term' = 'Extended Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `extended_cost` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Lead Time Days');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `line_number` SET TAGS ('dbx_business_glossary_term' = 'Line Number');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `line_status` SET TAGS ('dbx_business_glossary_term' = 'Line Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `ordered_quantity` SET TAGS ('dbx_business_glossary_term' = 'Ordered Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `pack_size` SET TAGS ('dbx_business_glossary_term' = 'Pack Size');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `planned_margin_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Margin Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `planned_margin_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `planned_margin_percent` SET TAGS ('dbx_business_glossary_term' = 'Planned Margin Percent');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `planned_margin_percent` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `private_label_flag` SET TAGS ('dbx_business_glossary_term' = 'Private Label Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `product_description` SET TAGS ('dbx_business_glossary_term' = 'Product Description');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `received_quantity` SET TAGS ('dbx_business_glossary_term' = 'Received Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `retail_price` SET TAGS ('dbx_business_glossary_term' = 'Retail Price');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `store_cluster_code` SET TAGS ('dbx_business_glossary_term' = 'Store Cluster Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `unit_cost` SET TAGS ('dbx_business_glossary_term' = 'Unit Cost');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `unit_cost` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buying_order_line` ALTER COLUMN `vendor_style_number` SET TAGS ('dbx_business_glossary_term' = 'Vendor Style Number');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_subdomain' = 'space_management');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_ssot_authoritative_owner' = 'store.store_planogram');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_ssot_role' = 'non_authoritative');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_ssot_owner' = 'store.store_planogram');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_ssot_reference' = 'store.store_planogram');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` SET TAGS ('dbx_ssot_authoritative_source' = 'store.store_planogram');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `merchandising_planogram_id` SET TAGS ('dbx_business_glossary_term' = 'Merchandising Planogram ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `compliance_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Required Flag');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `fixture_type` SET TAGS ('dbx_business_glossary_term' = 'Fixture Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `implementation_instructions` SET TAGS ('dbx_business_glossary_term' = 'Implementation Instructions');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `last_reset_date` SET TAGS ('dbx_business_glossary_term' = 'Last Reset Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `next_scheduled_reset_date` SET TAGS ('dbx_business_glossary_term' = 'Next Scheduled Reset Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `planogram_name` SET TAGS ('dbx_business_glossary_term' = 'Planogram Name');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `planogram_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `seasonal_flag` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `shelf_count` SET TAGS ('dbx_business_glossary_term' = 'Shelf Count');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `total_facings` SET TAGS ('dbx_business_glossary_term' = 'Total Facings');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`merchandising_planogram` ALTER COLUMN `visual_merchandising_notes` SET TAGS ('dbx_business_glossary_term' = 'Visual Merchandising Notes');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` SET TAGS ('dbx_subdomain' = 'space_management');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `bay_number` SET TAGS ('dbx_business_glossary_term' = 'Bay Number');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `capacity_units` SET TAGS ('dbx_business_glossary_term' = 'Capacity Units');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `display_orientation` SET TAGS ('dbx_business_glossary_term' = 'Display Orientation');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `facing_count` SET TAGS ('dbx_business_glossary_term' = 'Facing Count');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `fixture_type` SET TAGS ('dbx_business_glossary_term' = 'Fixture Type');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `is_hero_position` SET TAGS ('dbx_business_glossary_term' = 'Is Hero Position');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `maximum_facings` SET TAGS ('dbx_business_glossary_term' = 'Maximum Facings');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `merchandising_zone` SET TAGS ('dbx_business_glossary_term' = 'Merchandising Zone');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `minimum_facings` SET TAGS ('dbx_business_glossary_term' = 'Minimum Facings');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `position_status` SET TAGS ('dbx_business_glossary_term' = 'Position Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Priority Rank');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `sales_velocity_rank` SET TAGS ('dbx_business_glossary_term' = 'Sales Velocity Rank');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `shelf_number` SET TAGS ('dbx_business_glossary_term' = 'Shelf Number');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `signage_required` SET TAGS ('dbx_business_glossary_term' = 'Signage Required');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `space_productivity_index` SET TAGS ('dbx_business_glossary_term' = 'Space Productivity Index');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`planogram_position` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `assortment_item_id` SET TAGS ('dbx_business_glossary_term' = 'Assortment Item ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `assortment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Assortment Plan ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Featured Campaign Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `assortment_depth_tier` SET TAGS ('dbx_business_glossary_term' = 'Assortment Depth Tier');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `assortment_role` SET TAGS ('dbx_business_glossary_term' = 'Assortment Role');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `clearance_strategy` SET TAGS ('dbx_business_glossary_term' = 'Clearance Strategy');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `discontinuation_reason` SET TAGS ('dbx_business_glossary_term' = 'Discontinuation Reason');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `final_disposition_date` SET TAGS ('dbx_business_glossary_term' = 'Final Disposition Date');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `inclusion_status` SET TAGS ('dbx_business_glossary_term' = 'Inclusion Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `last_order_date` SET TAGS ('dbx_business_glossary_term' = 'Last Order Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `lifecycle_stage` SET TAGS ('dbx_business_glossary_term' = 'Lifecycle Stage');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `minimum_presentation_quantity` SET TAGS ('dbx_business_glossary_term' = 'Minimum Presentation Quantity');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `onboarding_status` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `planned_units` SET TAGS ('dbx_business_glossary_term' = 'Planned Units');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`assortment_item` ALTER COLUMN `private_label_flag` SET TAGS ('dbx_business_glossary_term' = 'Private Label Flag');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` SET TAGS ('dbx_subdomain' = 'buying_operations');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `vendor_negotiation_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Negotiation ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `allowance_amount` SET TAGS ('dbx_business_glossary_term' = 'Allowance Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `allowance_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `allowance_type` SET TAGS ('dbx_business_glossary_term' = 'Allowance Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `contract_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Contract Reference Number');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `coop_advertising_fund` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `cost_change_percentage` SET TAGS ('dbx_business_glossary_term' = 'Cost Change Percentage');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `cost_change_percentage` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `cost_change_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Change Reason Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `cost_change_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Cost Change Reason Description');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `fill_rate_commitment_percentage` SET TAGS ('dbx_business_glossary_term' = 'Fill Rate Commitment Percentage');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Lead Time Days');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `markdown_support_amount` SET TAGS ('dbx_business_glossary_term' = 'Markdown Support Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `markdown_support_amount` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `negotiation_end_date` SET TAGS ('dbx_business_glossary_term' = 'Negotiation End Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `negotiation_notes` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Notes');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `negotiation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Start Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `negotiation_status` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Status');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `negotiation_type` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `new_cost_price` SET TAGS ('dbx_business_glossary_term' = 'New Cost Price');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `new_cost_price` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `old_cost_price` SET TAGS ('dbx_business_glossary_term' = 'Old Cost Price');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `old_cost_price` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`vendor_negotiation` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_event_id` SET TAGS ('dbx_business_glossary_term' = 'Markdown Event ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `kpi_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Effectiveness Kpi Definition Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `actual_margin_impact` SET TAGS ('dbx_business_glossary_term' = 'Actual Margin Impact');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `actual_revenue_impact` SET TAGS ('dbx_business_glossary_term' = 'Actual Revenue Impact');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `actual_units_sold` SET TAGS ('dbx_business_glossary_term' = 'Actual Units Sold');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Approval Required Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_amount` SET TAGS ('dbx_business_glossary_term' = 'Markdown Amount');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_percentage` SET TAGS ('dbx_business_glossary_term' = 'Markdown Percentage');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_price` SET TAGS ('dbx_business_glossary_term' = 'Markdown Price');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_reason` SET TAGS ('dbx_business_glossary_term' = 'Markdown Reason');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_status` SET TAGS ('dbx_business_glossary_term' = 'Markdown Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `markdown_type` SET TAGS ('dbx_business_glossary_term' = 'Markdown Type');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `original_price` SET TAGS ('dbx_business_glossary_term' = 'Original Price');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `projected_margin_impact` SET TAGS ('dbx_business_glossary_term' = 'Projected Margin Impact');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `projected_revenue_impact` SET TAGS ('dbx_business_glossary_term' = 'Projected Revenue Impact');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `projected_units_sold` SET TAGS ('dbx_business_glossary_term' = 'Projected Units Sold');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`markdown_event` ALTER COLUMN `vendor_contribution_amount` SET TAGS ('dbx_business_glossary_term' = 'Vendor Contribution Amount');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `private_label_program_id` SET TAGS ('dbx_business_glossary_term' = 'Private Label Program ID');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `kpi_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Kpi Definition Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `assortment_breadth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Breadth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `assortment_depth_target` SET TAGS ('dbx_business_glossary_term' = 'Assortment Depth Target');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `competitive_positioning` SET TAGS ('dbx_business_glossary_term' = 'Competitive Positioning');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `discontinuation_date` SET TAGS ('dbx_business_glossary_term' = 'Discontinuation Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `exclusive_flag` SET TAGS ('dbx_business_glossary_term' = 'Exclusive Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `launch_date` SET TAGS ('dbx_business_glossary_term' = 'Launch Date');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Lead Time Days');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `markdown_strategy` SET TAGS ('dbx_business_glossary_term' = 'Markdown Strategy');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `marketing_investment_usd` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `otb_budget_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `packaging_specification` SET TAGS ('dbx_business_glossary_term' = 'Packaging Specification');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Program Code');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `program_description` SET TAGS ('dbx_business_glossary_term' = 'Program Description');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `program_status` SET TAGS ('dbx_business_glossary_term' = 'Program Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `quality_control_protocol` SET TAGS ('dbx_business_glossary_term' = 'Quality Control Protocol');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `quality_tier` SET TAGS ('dbx_business_glossary_term' = 'Quality Tier');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `seasonal_flag` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Flag');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `sustainability_certification_type` SET TAGS ('dbx_business_glossary_term' = 'Sustainability Certification Type');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`private_label_program` ALTER COLUMN `sustainability_certified_flag` SET TAGS ('dbx_business_glossary_term' = 'Sustainability Certified Flag');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category_campaign_placement` SET TAGS ('dbx_subdomain' = 'space_management');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category_campaign_placement` ALTER COLUMN `category_campaign_placement_id` SET TAGS ('dbx_business_glossary_term' = 'Category Campaign Placement ID');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category_campaign_placement` ALTER COLUMN `placement_status` SET TAGS ('dbx_business_glossary_term' = 'Placement Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` SET TAGS ('dbx_subdomain' = 'buying_operations');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` ALTER COLUMN `assigned_category_list` SET TAGS ('dbx_business_glossary_term' = 'Assigned Category List');
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` ALTER COLUMN `assignment_status` SET TAGS ('dbx_business_glossary_term' = 'Assignment Status');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`buyer_profit_center_assignment` ALTER COLUMN `otb_allocation_amount` SET TAGS ('dbx_business_glossary_term' = 'OTB Allocation Amount');
...
ALTER TABLE `vibe_retail_v1`.`merchandising`.`category_accrual_rule` ALTER COLUMN `category_created_by_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Assignment Creator Associate');
