...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_facility` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Manufacturing facilities typically map to cost centers for facility overhead allocation (utilities, rent, facility management). Consumer goods manufacturers track facility-level costs in cost centers ',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Manufacturing facilities often represent profit centers for P&L reporting by location/plant. Consumer goods companies with multi-site operations report profitability by manufacturing facility to evalu',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_line` (
    ...
    `product_category_id` BIGINT COMMENT 'Foreign key linking to product.product_category. Business justification: Production lines in consumer goods are dedicated to or primarily configured for specific product categories (liquid filling, powder, personal care). Line scheduling by category and capacity planning',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_bom` (
    ...
    `manufacturing_facility_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_facility. Business justification: Manufacturing BOMs are plant-specific in consumer goods ERP systems — the same product may have different BOMs at different facilities due to local sourcing, equipment capabilities, or regulatory requ',
    ...
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_site. Business justification: GMP and quality approvals in consumer goods are site-specific. A BOM component must reference the qualified supplier SITE (not just supplier) for sourcing compliance, quality audits, and approved supp',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`routing` (
    ...
    `manufacturing_facility_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_facility. Business justification: A production routing is plant-specific in ERP systems (SAP, Oracle). The routing table already carries a plant_code STRING column which is a denormalized reference to the manufacturing facility. Addin',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_order` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Production Planning Dashboard ties orders to brands to enable demand forecasting and coordinated marketing launch planning.',
    ...
    `manufacturing_facility_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_facility. Business justification: Production order compliance reporting requires linking each order to the products registration record to verify GMP compliance and traceability.',
    `product_category_id` BIGINT COMMENT 'Foreign key linking to product.product_category. Business justification: Pilot production orders are generated from R&D projects; the Production Planning system needs the project ID to track development cost and schedule.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`batch_record` (
    ...
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.product_formulation. Business justification: FDA 21 CFR Part 211 and EU GMP Annex 11 require batch records to cite the approved formulation version used during production for deviation investigation and recall traceability. batch_record links to',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Consumer goods GMP and FDA/EU recall regulations require lot genealogy: knowing which supplier goods receipt (and thus which supplier lot/batch) fed each production batch. Direct FK enables forward/ba',
    `manufacturing_bom_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_bom. Business justification: A batch manufacturing record (eBMR) is produced against a specific manufacturing BOM. The batch_record table carries bom_version (STRING) which is an execution-time version label — this is retained as',
    ...
    `routing_id` BIGINT COMMENT 'Foreign key linking to manufacturing.routing. Business justification: A batch manufacturing record (eBMR) captures execution against a specific routing. The batch_record table carries routing_version (STRING) which is an execution-time version label — this is retained a',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: Yield records capture input/output quantities at each manufacturing stage and are directly associated with a batch record (eBMR). The yield_record table already carries batch_number (STRING) and batch',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Yield losses and scrap post to specific GL accounts (typically COGS variance accounts) for financial reporting. Consumer goods ERP systems require GL account assignment for yield variance postings to ',
    `manufacturing_facility_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_facility. Business justification: Yield records are facility-scoped. The yield_record table carries plant_code (STRING) which is a denormalized reference to the manufacturing facility. Adding manufacturing_facility_id as a proper FK n',
    `production_line_id` BIGINT COMMENT 'Foreign key linking to manufacturing.production_line. Business justification: Yield is measured at a specific production line. The yield_record table carries production_line_code (STRING) which is a denormalized reference to the production_line entity. Adding production_line_id',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Yield reporting by SKU is a core manufacturing KPI in consumer goods (OEE dashboards, waste reduction programs). yield_record currently reaches SKU only via production_order join. Direct sku_id enable',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_confirmation` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: Each production confirmation is part of a batch manufacturing record (eBMR). The production_confirmation table carries batch_number (STRING) which is a denormalized reference to the batch_record entit',
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Production confirmations trigger inventory movements (WIP to finished goods) that post to specific GL accounts. Consumer goods manufacturers require GL account assignment for automated inventory valua',
    ...
    `routing_id` BIGINT COMMENT 'Foreign key linking to manufacturing.routing. Business justification: Production confirmations are posted against specific routing operations (operation_number STRING is retained as it identifies the specific step within the routing). Linking production_confirmation to ',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`planned_order` (
    ...
    `manufacturing_bom_id` BIGINT COMMENT 'Foreign key linking to manufacturing.manufacturing_bom. Business justification: MRP generates planned orders against a specific manufacturing BOM. Linking planned_order to manufacturing_bom enables material requirements explosion traceability from planning through execution. This',
    ...
    `product_category_id` BIGINT COMMENT 'Foreign key linking to product.product_category. Business justification: Capacity planning and S&OP reporting in consumer goods aggregate planned orders by product category. production_order already carries product_category_id; planned_order lacks it, creating an inconsist',
    `routing_id` BIGINT COMMENT 'Foreign key linking to manufacturing.routing. Business justification: Planned orders in MRP are generated against a specific routing (production version maps to a routing). Linking planned_order to routing enables planning-to-execution traceability and allows MRP to val',
    ...
    `distribution_facility_id` BIGINT COMMENT 'Foreign key linking to distribution.distribution_facility. Business justification: Supply planning allocates planned production to target distribution centers for replenishment and transfer orders. IBP and MRP systems require this link to balance network inventory and drive inter-fa',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`manufacturing`.`equipment` (
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.purchase_order. Business justification: Capital equipment assets are acquired via purchase orders. Linking equipment to its acquisition PO enables asset lifecycle management, warranty tracking against PO dates, depreciation reconciliation, ',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Equipment depreciation and maintenance costs allocate to cost centers for overhead absorption. Consumer goods manufacturers track equipment costs by cost center for production overhead rate calculatio',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Equipment assets require GL account assignment for depreciation posting and asset accounting. Consumer goods ERP systems post equipment depreciation to specific GL accounts (asset and accumulated depr',
    ...
    `work_center_id` BIGINT COMMENT 'Foreign key linking to manufacturing.work_center. Business justification: Equipment (machines, instruments) is physically located within and assigned to a work center. The equipment table already has production_line_id and manufacturing_facility_id FKs, but the intermediate',
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ADD CONSTRAINT `fk_manufacturing_yield_record_manufacturing_facility_id` FOREIGN KEY (`manufacturing_facility_id`) REFERENCES `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_facility`(`manufacturing_facility_id`);
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ADD CONSTRAINT `fk_manufacturing_yield_record_production_line_id` FOREIGN KEY (`production_line_id`) REFERENCES `vibe_consumer_goods_v1`.`manufacturing`.`production_line`(`production_line_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_facility` SET TAGS ('dbx_subdomain' = 'asset_configuration');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_facility` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_line` SET TAGS ('dbx_subdomain' = 'asset_configuration');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`work_center` SET TAGS ('dbx_subdomain' = 'asset_configuration');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_bom` SET TAGS ('dbx_subdomain' = 'recipe_planning');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_bom` ALTER COLUMN `manufacturing_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Manufacturing Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`manufacturing_bom` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`routing` SET TAGS ('dbx_subdomain' = 'recipe_planning');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`routing` ALTER COLUMN `manufacturing_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Manufacturing Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_order` SET TAGS ('dbx_subdomain' = 'batch_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_order` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_order` ALTER COLUMN `manufacturing_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Registration Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_order` ALTER COLUMN `product_category_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`batch_record` SET TAGS ('dbx_subdomain' = 'batch_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` SET TAGS ('dbx_subdomain' = 'batch_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ALTER COLUMN `manufacturing_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Manufacturing Facility Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ALTER COLUMN `production_line_id` SET TAGS ('dbx_business_glossary_term' = 'Production Line Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`yield_record` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_confirmation` SET TAGS ('dbx_subdomain' = 'batch_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_confirmation` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`production_confirmation` ALTER COLUMN `routing_id` SET TAGS ('dbx_business_glossary_term' = 'Routing Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`planned_order` SET TAGS ('dbx_subdomain' = 'recipe_planning');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`planned_order` ALTER COLUMN `product_category_id` SET TAGS ('dbx_business_glossary_term' = 'Product Category Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`planned_order` ALTER COLUMN `distribution_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Target Distribution Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`equipment` SET TAGS ('dbx_subdomain' = 'asset_configuration');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`equipment` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Acquisition Purchase Order Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`equipment` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`manufacturing`.`equipment` ALTER COLUMN `work_center_id` SET TAGS ('dbx_business_glossary_term' = 'Work Center Id (Foreign Key)');
