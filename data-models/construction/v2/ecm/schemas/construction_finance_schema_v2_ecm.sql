...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`journal_entry` (
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'The cost center to which costs or revenues are allocated. Used for internal management reporting and cost control.',
    `cost_code` DECIMAL(18,2) COMMENT 'Construction-specific cost code for detailed job costing. Categorizes costs by type of work (e.g., labor, materials, equipment, subcontractor).',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`journal_entry_line` (
    ...
    `cost_code` DECIMAL(18,2) COMMENT 'Construction-specific cost code classifying the nature of the expense (e.g., labor, materials, equipment, subcontractor). Used for job costing and BOQ variance analysis.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`finance_budget_revision` (
    ...
    `finance_budget_revision_description` STRING COMMENT '',
    ...
    `finance_budget_revision_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`job_cost_transaction` (
    ...
    `cost_code` DECIMAL(18,2) COMMENT 'Standardized cost code identifying the type of work or resource consumed (e.g., labor, material, equipment, subcontractor). Used for job costing classification and reporting.',
    ...
    `invoice_number` DECIMAL(18,2) COMMENT 'The vendor invoice number or internal billing document number associated with this cost transaction for accounts payable reconciliation.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`revenue_recognition_entry` (
    ...
    `billed_to_date` DATE COMMENT 'Cumulative amount billed to the client from contract inception through the end of the current recognition period, based on progress billing milestones or payment schedules.',
    ...
    `revenue_recognition_entry_description` STRING COMMENT '',
    ...
    `gross_profit_to_date` DATE COMMENT 'Cumulative gross profit recognized on the contract to date, calculated as revenue recognized to date minus cumulative costs incurred.',
    ...
    `revenue_recognition_entry_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`accounts_receivable_invoice` (
    ...
    `billing_period_end_date` DECIMAL(18,2) COMMENT 'End date of the work period covered by this invoice. Defines the cutoff for work included in this billing cycle.',
    `billing_period_start_date` DECIMAL(18,2) COMMENT 'Start date of the work period covered by this invoice. Used for progress billing and percentage-of-completion revenue recognition.',
    ...
    `accounts_receivable_invoice_description` STRING COMMENT '',
    ...
    `invoice_date` DECIMAL(18,2) COMMENT 'The date the invoice was formally issued to the client. This is the principal business event timestamp for revenue recognition and aging calculations.',
    `invoice_description` DECIMAL(18,2) COMMENT 'Detailed description of the work or milestone being billed. Provides context for the invoice line items and supports client review.',
    `invoice_number` DECIMAL(18,2) COMMENT 'Externally-known unique invoice number issued to the client. This is the business identifier printed on the invoice document and referenced in payment correspondence.',
    `invoice_status` DECIMAL(18,2) COMMENT 'Current lifecycle status of the invoice in the accounts receivable workflow. Tracks progression from draft through payment to closure or dispute resolution. [ENUM-REF-CANDIDATE: draft|issued|partially_paid|paid|overdue|disputed|cancelled|written_off — 8 candidates stripped; promote to reference product]',
    ...
    `last_payment_date` DECIMAL(18,2) COMMENT 'Date of the most recent payment received against this invoice. Null if no payments have been received.',
    ...
    `payment_certificate_date` DECIMAL(18,2) COMMENT 'Date the payment certificate was issued by the engineer or client representative. Typically precedes the invoice date.',
    `payment_certificate_number` DECIMAL(18,2) COMMENT 'Reference number of the payment certificate issued by the engineer or client representative approving the work value. This certificate authorizes the invoice issuance.',
    `payment_method` DECIMAL(18,2) COMMENT 'The payment instrument or method used by the client to settle this invoice (e.g., bank transfer, wire transfer, check, letter of credit).',
    ...
    `revenue_recognition_method` DECIMAL(18,2) COMMENT 'Accounting method used for revenue recognition on this invoice. Construction typically uses percentage-of-completion per IFRS 15 and GAAP ASC 606.',
    ...
    `accounts_receivable_invoice_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` (
    ...
    `contract_retention_ledger_id` BIGINT COMMENT '',
    `finance_contract_retention_ledger_ref_contract_retention_ledger_id` BIGINT COMMENT 'Reference to single source of truth contract.contract_retention_ledger (SSOT duplicate resolution).',
    `cost_code` DECIMAL(18,2) COMMENT 'The cost code associated with this retention entry for job costing and financial tracking.',
    `created_timestamp` TIMESTAMP COMMENT '',
    ...
    `finance_retention_ledger_description` STRING COMMENT '',
    ...
    `invoice_reference_number` DECIMAL(18,2) COMMENT 'The invoice number or payment application reference associated with this retention withholding or release.',
    ...
    `payment_certificate_number` DECIMAL(18,2) COMMENT 'The payment certificate number issued by the engineer or client authorizing payment and retention withholding.',
    ...
    `finance_retention_ledger_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`financial_guarantee` (
    ...
    `financial_guarantee_description` STRING COMMENT '',
    ...
    `premium_payment_frequency` DECIMAL(18,2) COMMENT 'Frequency at which premium payments are made: one-time, annual, semi-annual, quarterly, or monthly.',
    ...
    `financial_guarantee_status` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`profit_center` (
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'Associated cost center code used for internal cost allocation.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`billing_period` (
    ...
    `billing_cycle_code` DECIMAL(18,2) COMMENT 'Code that groups periods into a billing cycle (e.g., monthly cycle code).',
    ...
    `payment_due_days` DECIMAL(18,2) COMMENT 'Number of days from invoice date until payment is due for this period.',
    ...
    `remarks` STRING COMMENT '',
    `billing_period_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`company_code` (
    ...
    `cost_center` DECIMAL(18,2) COMMENT 'Identifier of the cost center associated with the entity.',
    ...
    `incorporation_date` DECIMAL(18,2) COMMENT 'Date the entity was legally incorporated.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`chart_of_accounts` (
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'Code linking the account to a cost center for internal cost allocation.',
    ...
    `is_budgetable` DECIMAL(18,2) COMMENT 'True if the account is included in budgeting and forecasting processes.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`commitment_item` (
    ...
    `commitment_construction_project_id` BIGINT COMMENT '',
    ...
    `expiration_date` DECIMAL(18,2) COMMENT 'Date on which the commitment line expires or is scheduled to end (nullable if open‑ended).',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`payment_run` (
    ...
    `number_of_payments` DECIMAL(18,2) COMMENT 'Total count of individual payment transactions included in the run.',
    `payment_channel` DECIMAL(18,2) COMMENT 'System or interface through which the payment run was initiated.',
    `payment_method` DECIMAL(18,2) COMMENT 'Instrument used to make the payments.',
    `remarks` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`finance`.`commitment` (
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'Code of the cost center charging the commitment.',
    ...
    `notes` STRING COMMENT 'Additional free‑form remarks or comments about the commitment.',
    `owner_role` STRING COMMENT 'Role of the employee who owns the commitment.',
    `remarks` STRING COMMENT '',
...
ALTER TABLE `vibe_construction_v1`.`finance`.`cost_center` ADD CONSTRAINT `fk_finance_cost_center_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `vibe_construction_v1`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `vibe_construction_v1`.`finance`.`job_cost_transaction` ADD CONSTRAINT `fk_finance_job_cost_transaction_commitment_item_id` FOREIGN KEY (`commitment_item_id`) REFERENCES `vibe_construction_v1`.`finance`.`commitment_item`(`commitment_item_id`);
...
ALTER TABLE `vibe_construction_v1`.`finance`.`revenue_recognition_entry` ADD CONSTRAINT `fk_finance_revenue_recognition_entry_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `vibe_construction_v1`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `vibe_construction_v1`.`finance`.`revenue_recognition_entry` ADD CONSTRAINT `fk_finance_revenue_recognition_entry_project_budget_id` FOREIGN KEY (`project_budget_id`) REFERENCES `vibe_construction_v1`.`finance`.`project_budget`(`project_budget_id`);
...
ALTER TABLE `vibe_construction_v1`.`finance`.`payment_record` ADD CONSTRAINT `fk_finance_payment_record_payment_run_id` FOREIGN KEY (`payment_run_id`) REFERENCES `vibe_construction_v1`.`finance`.`payment_run`(`payment_run_id`);
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` ADD CONSTRAINT `fk_finance_finance_retention_ledger_billing_period_id` FOREIGN KEY (`billing_period_id`) REFERENCES `vibe_construction_v1`.`finance`.`billing_period`(`billing_period_id`);
...
ALTER TABLE `vibe_construction_v1`.`finance`.`commitment_item` ADD CONSTRAINT `fk_finance_commitment_item_commitment_id` FOREIGN KEY (`commitment_id`) REFERENCES `vibe_construction_v1`.`finance`.`commitment`(`commitment_id`);
ALTER TABLE `vibe_construction_v1`.`finance`.`commitment_item` ADD CONSTRAINT `fk_finance_commitment_item_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `vibe_construction_v1`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `vibe_construction_v1`.`finance`.`job_cost_transaction` SET TAGS ('dbx_subdomain' = 'budget_planning');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`earned_value_record` SET TAGS ('dbx_subdomain' = 'budget_planning');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`progress_billing` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`progress_billing` ALTER COLUMN `engineer_approval_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`invoice` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`invoice` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Permit Id (Foreign Key)');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`accounts_receivable_invoice` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`payment_record` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_subdomain' = 'billing_collections');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_ref' = 'contract.contract_retention_ledger');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot' = 'authoritative');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_canonical' = 'contract.contract_retention_ledger');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_status' = 'duplicate');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_pair_resolved' = 'true');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_for' = 'contract.contract_retention_ledger');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_role' = 'owner');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` SET TAGS ('dbx_ssot_pair' = 'contract.contract_retention_ledger');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` ALTER COLUMN `contract_retention_ledger_id` SET TAGS ('dbx_ssot_reference_fk' = 'true');
ALTER TABLE `vibe_construction_v1`.`finance`.`finance_retention_ledger` ALTER COLUMN `finance_contract_retention_ledger_ref_contract_retention_ledger_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`financial_guarantee` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`financial_guarantee` ALTER COLUMN `beneficiary_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`financial_guarantee` ALTER COLUMN `issuer_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`profit_center` SET TAGS ('dbx_subdomain' = 'cost_control');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`profit_center` ALTER COLUMN `profit_center_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`profit_center` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`billing_period` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`billing_period` ALTER COLUMN `period_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `company_code_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`company_code` ALTER COLUMN `primary_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`chart_of_accounts` ALTER COLUMN `account_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`payment_run` SET TAGS ('dbx_subdomain' = 'billing_collections');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`payment_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_construction_v1`.`finance`.`payment_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`commitment` ALTER COLUMN `primary_original_commitment_id` SET TAGS ('dbx_business_glossary_term' = 'Original Commitment Id');
...
ALTER TABLE `vibe_construction_v1`.`finance`.`commitment` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
