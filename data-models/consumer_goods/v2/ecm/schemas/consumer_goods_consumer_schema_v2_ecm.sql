...
-- Generated on: 2026-06-24 00:22:16
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` (
    ...
    `registration_source_url` DECIMAL(18,2) COMMENT 'The URL or digital touchpoint from which the consumer completed their registration (e.g., brand website landing page, DTC checkout, loyalty sign-up page). Supports digital marketing attribution and campaign source tracking.',
...
) COMMENT 'A consumer shopper master record capturing identity, contact, consent, and loyalty attributes.';
...
) COMMENT 'A household entity grouping one or more shoppers sharing a residence, used for panel and loyalty analytics.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`address` (
    ...
    `dpv_confirmation` BOOLEAN COMMENT 'USPS Delivery Point Validation confirmation code indicating the deliverability of the address. Y=Confirmed deliverable, S=Confirmed deliverable (secondary info missing), D=Confirmed deliverable (secondary info present but unneeded), N=Not confirmed. Critical for DTC fulfillment and direct mail campaigns.',
    ...
    `geocoding_accuracy` DECIMAL(18,2) COMMENT 'Precision level of the geocoded latitude/longitude coordinates for this address. Rooftop indicates exact parcel-level geocoding; range_interpolated indicates interpolation between street numbers; geometric_center indicates centroid of a polygon. Impacts reliability of proximity-based analytics.',
    ...
    `residential_indicator` BOOLEAN COMMENT 'Classification of the address as residential or commercial. Impacts carrier surcharge calculations (UPS/FedEx residential delivery fees), marketing segmentation, and DTC fulfillment cost modeling.',
...
) COMMENT 'Physical or mailing address associated with a consumer shopper, including validation and geocoding metadata.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` (
    ...
    `primary_loyalty_program_id` BIGINT COMMENT '',
...
) COMMENT 'A loyalty program account held by a shopper, tracking points balance, tier, and enrollment details.';
...
) COMMENT 'A points earn or redemption transaction within a loyalty account.';
...
) COMMENT 'A consumer segment definition used for targeting, personalization, and analytics.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`segment_membership` (
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Marketing segment campaigns are budgeted to cost centers; linking enables expense tracking per segment.',
    ...
    `membership_score` DECIMAL(18,2) COMMENT '',
...
) COMMENT 'A record of a consumers membership in a segment, including assignment method and confidence score.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`preference` (
    ...
    `preference_type` STRING COMMENT '',
...
) COMMENT 'A consumer preference record capturing product, channel, and communication preferences.';
...
) COMMENT 'Current consent record for a consumer covering all consent types, legal bases, and regulatory jurisdictions.';
...
) COMMENT 'Immutable audit event capturing a change in consumer consent status, including legal basis and proof of consent.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` (
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Order processing costs are allocated to internal cost centers for budgeting and variance analysis.',
    ...
    `order_total` DECIMAL(18,2) COMMENT '',
...
) COMMENT 'A direct-to-consumer order placed by a shopper through a brand-owned channel.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` (
    ...
    `profit_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.profit_center. Business justification: Each line item contributes to a profit center; needed for profit‑center reporting per SKU.',
...
) COMMENT 'A line item within a direct-to-consumer order, capturing SKU, quantity, pricing, and fulfillment details.';
...
) COMMENT 'A return request initiated by a consumer for a DTC order, including disposition and refund details.';
...
) COMMENT 'A Net Promoter Score survey response submitted by a consumer.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`interaction` (
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Support interactions incur costs that must be charged to a cost center for service‑cost analysis.',
...
) COMMENT 'A consumer interaction event such as a call, chat, email, or in-store visit with the brand.';
...
) COMMENT 'A recurring subscription held by a consumer for a product or service.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`cltv_record` (
    ...
    `cltv_amount` DECIMAL(18,2) COMMENT '',
...
) COMMENT 'Customer lifetime value calculation record for a shopper, including predicted revenue, churn probability, and model metadata.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`communication_preference` (
    ...
    `consent_version` STRING COMMENT '',
    ...
    `frequency_preference` STRING COMMENT '',
...
) COMMENT 'Consumer communication channel preferences including opt-in/opt-out status per channel and brand.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`identity_link` (
    ...
    `confidence_score` DECIMAL(18,2) COMMENT '',
    ...
    `external_identifier` STRING COMMENT '',
    ...
    `identifier_type` STRING COMMENT '',
    ...
    `is_primary_identity` BOOLEAN COMMENT '',
    ...
    `linked_timestamp` TIMESTAMP COMMENT '',
    `match_confidence` DECIMAL(18,2) COMMENT '',
    ...
    `unlinked_timestamp` TIMESTAMP COMMENT '',
...
) COMMENT 'A cross-system identity linkage record connecting a consumer identity across multiple source systems.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` (
    ...
    `loyalty_program_id` BIGINT COMMENT 'Identifier of the specific referral program configuration under which this referral was created. Determines reward rules, expiry windows, eligible channels, and conversion criteria.',
    ...
    `referral_loyalty_program_id` BIGINT COMMENT 'Identifier of the loyalty program under which this referral was issued. Determines reward rules, eligibility criteria, and program-specific tracking.',
    `referral_referred_shopper_id` BIGINT COMMENT '',
    `referral_referrer_shopper_id` BIGINT COMMENT '',
    ...
    `referral_channel` STRING COMMENT '',
    ...
    `reward_amount` DECIMAL(18,2) COMMENT '',
    `reward_points_issued` DECIMAL(18,2) COMMENT '',
...
) COMMENT 'A consumer referral record tracking referrer, referee, and referral reward status.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` (
    ...
    `consumer_test_id` BIGINT COMMENT '',
    `market_research_study_id` BIGINT COMMENT '',
    `panel_id` BIGINT COMMENT '',
    ...
    `anonymized_flag` BOOLEAN COMMENT '',
    `channel` STRING COMMENT '',
    `compensation_amount` DECIMAL(18,2) COMMENT 'Monetary compensation paid to the shopper for participation',
    `compensation_currency` STRING COMMENT 'Currency of the compensation amount',
    `compensation_paid_flag` BOOLEAN COMMENT 'Whether compensation has been paid',
    ...
    `consent_obtained` BOOLEAN COMMENT '',
    `consent_version` STRING COMMENT '',
    `country_code` STRING COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency_code` STRING COMMENT '',
    `data_quality_score` DECIMAL(18,2) COMMENT 'Quality score of participant data',
    `data_usage_consent_flag` BOOLEAN COMMENT 'Whether the participant consented to data usage',
    `data_use_consent_flag` BOOLEAN COMMENT '',
    `eligibility_criteria_met_flag` BOOLEAN COMMENT 'Indicates if eligibility criteria were met',
    `end_date` DATE COMMENT 'Date the participant completed or exited the research',
    `enrollment_date` DATE COMMENT '',
    `feedback_score` STRING COMMENT '',
    `incentive_amount` DECIMAL(18,2) COMMENT '',
    `incentive_currency_code` STRING COMMENT '',
    ...
    `recruitment_channel` STRING COMMENT '',
    `recruitment_source` STRING COMMENT 'Source of participant recruitment',
    `screening_passed_flag` BOOLEAN COMMENT '',
    `source_channel` STRING COMMENT 'Channel through which the participant was recruited',
    `start_date` DATE COMMENT 'Date the participant started the research',
    `study_arm` STRING COMMENT 'Study arm or group the participant was assigned to',
    `study_name` STRING COMMENT '',
    `study_role` STRING COMMENT '',
    `study_type` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `v2_added_flag` BOOLEAN COMMENT 'Added by v2 mutator to ensure change',
    `withdrawal_date` DATE COMMENT 'Date the participant withdrew from the study',
    `withdrawal_reason` STRING COMMENT 'Reason for withdrawal if applicable',
    `research_participation_status` STRING COMMENT 'Lifecycle status of the research_participation record',
    `effective_date` DATE COMMENT 'Date the research_participation becomes effective',
    `expiration_date` DATE COMMENT 'Date the research_participation expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the research_participation',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the research_participation',
    `research_participation_description` STRING COMMENT 'Free-text description of the research_participation',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the research_participation was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the research_participation was last updated',
    CONSTRAINT pk_research_participation PRIMARY KEY(`research_participation_id`)
) COMMENT 'A record of a consumers participation in a research study or consumer test.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_program` (
    ...
    `loyalty_program_description` STRING COMMENT '',
    ...
    `is_coalition_program` BOOLEAN COMMENT '',
    ...
    `points_currency_name` STRING COMMENT '',
    ...
    `points_expiry_months` STRING COMMENT '',
    `points_expiry_policy` STRING COMMENT '',
...
) COMMENT 'A loyalty program definition including tiers, earn rules, and redemption policies.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` (
    ...
    `preference_center_id` BIGINT COMMENT '',
    `renewed_consent_session_id` BIGINT COMMENT 'Self-referencing FK on consent_session (renewed_consent_session_id)',
    `shopper_id` BIGINT COMMENT 'P8: Connect consumer.consent_session by adding column shopper_id (BIGINT) with FK to consumer.shopper.shopper_id because a consent session must identify which consumer it belongs to. P8: connect_table: consumer.consent_session** - add colum',
    ...
    `session_channel` STRING COMMENT '',
    ...
    `session_status` STRING COMMENT '',
    `session_timestamp` TIMESTAMP COMMENT '',
    `session_token` STRING COMMENT '',
...
) COMMENT 'A web or app session during which consent was captured or updated for a consumer.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` (
    ...
    `center_code` STRING COMMENT '',
    `center_name` STRING COMMENT '',
    `center_status` STRING COMMENT '',
    `center_url` STRING COMMENT '',
    ...
    `supported_channels` STRING COMMENT '',
...
) COMMENT 'A preference center record capturing a consumers opt-in/opt-out choices across communication channels.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` (
    ...
    `identity_verification_method` STRING COMMENT '',
    `identity_verified_flag` BOOLEAN COMMENT '',
    ...
    `requestor_email` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `v2_added_flag` BOOLEAN COMMENT 'Added by v2 mutator to ensure change',
...
) COMMENT 'A formal data subject request (DSR) submitted by a consumer under GDPR, CCPA, or similar regulation.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` (
    ...
    `age_range_max` STRING COMMENT '',
    `age_range_min` STRING COMMENT '',
...
) COMMENT 'A consumer research panel member record capturing demographic and participation attributes.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_tier` (
    ...
    `loyalty_program_id` BIGINT COMMENT 'P9: Connect consumer.loyalty_tier by adding column loyalty_program_id (BIGINT) with FK to consumer.loyalty_program.loyalty_program_id because tiers belong to a specific loyalty program. P9: connect_table: consumer.loyalty_tier** - add colum',
    ...
    `expiration_policy` DECIMAL(18,2) COMMENT 'Policy governing how tier status expires or rolls over.',
    ...
    `max_points_threshold` DECIMAL(18,2) COMMENT '',
    `min_points_threshold` DECIMAL(18,2) COMMENT '',
...
) COMMENT 'A tier level within a loyalty program defining qualification thresholds and benefits.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` (
    ...
    `marketing_brand_id` BIGINT COMMENT 'P10: Connect marketing.survey by adding column marketing_brand_id (BIGINT) with FK to marketing.marketing_brand.marketing_brand_id because surveys are typically brand-related. P10: connect_table: marketing.survey** - add column marketing_bran',
    `revised_survey_id` BIGINT COMMENT 'Self-referencing FK on survey (revised_survey_id)',
    `actual_sample_size` STRING COMMENT '',
    `anonymized_flag` BOOLEAN COMMENT '',
    `survey_category` STRING COMMENT 'Business category of the survey for reporting and analytics.',
    `channel` STRING COMMENT '',
    `survey_code` STRING COMMENT '',
    ...
    `target_sample_size` STRING COMMENT '',
...
) COMMENT 'A consumer survey definition associated with a brand or campaign.';
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ADD CONSTRAINT `fk_consumer_loyalty_account_referral_account_id` FOREIGN KEY (`referral_account_id`) REFERENCES `vibe_consumer_goods_v1`.`consumer`.`loyalty_account`(`loyalty_account_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` ADD CONSTRAINT `fk_consumer_referral_referral_loyalty_program_id` FOREIGN KEY (`referral_loyalty_program_id`) REFERENCES `vibe_consumer_goods_v1`.`consumer`.`loyalty_program`(`loyalty_program_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` SET TAGS ('dbx_reviewer_column_h' = 'no_notes_no_overrides');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `birth_date` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `birth_date` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `display_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `display_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `family_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `family_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `gender` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `gender` SET TAGS ('dbx_pii_category' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `gender` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `given_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `given_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_category' = 'address');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `primary_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `primary_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `primary_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `primary_phone` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`shopper` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` ALTER COLUMN `household_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` ALTER COLUMN `household_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_category' = 'address');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`household` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `address_id` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `shopper_id` SET TAGS ('dbx_natural_key' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `address_line_1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `address_line_2` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `address_status` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `address_type` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `carrier_route` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `census_tract` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `city` SET TAGS ('dbx_natural_key' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `city` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_pii_type' = 'address');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `latitude` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `line_1` SET TAGS ('dbx_natural_key' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `line_1` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `line_2` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `line_3` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `longitude` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `plus4_code` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `postal_code` SET TAGS ('dbx_natural_key' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `source` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `source_system_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `standardized_address` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `state_province` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`address` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `loyalty_program_id` SET TAGS ('dbx_business_role' = 'origin');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `referral_account_id` SET TAGS ('dbx_business_glossary_term' = 'Referral Account ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `account_number` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `linked_card_token` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `linked_card_token` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_account` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_transaction` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_transaction` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` SET TAGS ('dbx_subdomain' = 'analytics_insights');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` ALTER COLUMN `segment_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` ALTER COLUMN `segment_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment_membership` SET TAGS ('dbx_subdomain' = 'analytics_insights');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment_membership` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`segment_membership` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference` SET TAGS ('dbx_subdomain' = 'preference_engagement');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_record` SET TAGS ('dbx_subdomain' = 'privacy_governance');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_record` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_record` ALTER COLUMN `ip_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_record` ALTER COLUMN `ip_address` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_record` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` SET TAGS ('dbx_subdomain' = 'privacy_governance');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `consent_purpose_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `consent_purpose_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `ip_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `ip_address` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_event` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `carbon_emission_id` SET TAGS ('dbx_business_glossary_term' = 'Carbon Emission Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `billing_address_same_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `billing_address_same_flag` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `payment_transaction_ref` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_address_line1` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_city` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `ship_to_state_province` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` ALTER COLUMN `product_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` ALTER COLUMN `product_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_order_line` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` ALTER COLUMN `carrier_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` ALTER COLUMN `carrier_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`dtc_return` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` SET TAGS ('dbx_subdomain' = 'analytics_insights');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` ALTER COLUMN `brand_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` ALTER COLUMN `retailer_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` ALTER COLUMN `retailer_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`nps_response` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`interaction` SET TAGS ('dbx_subdomain' = 'preference_engagement');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`interaction` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`interaction` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Event Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`interaction` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` ALTER COLUMN `payment_token_reference` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`subscription` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`cltv_record` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`cltv_record` SET TAGS ('dbx_subdomain' = 'analytics_insights');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`cltv_record` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`cltv_record` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`communication_preference` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`communication_preference` SET TAGS ('dbx_subdomain' = 'preference_engagement');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`communication_preference` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`communication_preference` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`identity_link` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`identity_link` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` SET TAGS ('dbx_subdomain' = 'analytics_insights');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` ALTER COLUMN `loyalty_program_id` SET TAGS ('dbx_business_glossary_term' = 'Referral Program ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` ALTER COLUMN `referral_loyalty_program_id` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Program ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`referral` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` SET TAGS ('dbx_subdomain' = 'identity_management');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `compensation_currency` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `compensation_currency` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `compensation_paid_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `compensation_paid_flag` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `study_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `study_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_status` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_status` SET TAGS ('dbx_classification' = 'status');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_status` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `research_participation_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`research_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_program` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_program` ALTER COLUMN `program_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_program` ALTER COLUMN `program_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_program` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` SET TAGS ('dbx_subdomain' = 'privacy_governance');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `address_line` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `address_line` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`consent_session` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` SET TAGS ('dbx_subdomain' = 'preference_engagement');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_sensitivity' = 'email');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `email_opt_in` SET TAGS ('dbx_pii_kind' = 'email');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `phone_opt_in` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `phone_opt_in` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`preference_center` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` SET TAGS ('dbx_subdomain' = 'privacy_governance');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` ALTER COLUMN `requestor_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` ALTER COLUMN `requestor_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`data_subject_request` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `address_line1` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `address_line2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `address_line2` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `city` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_business_glossary_term' = 'Ethnicity');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_sensitivity' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_classification' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `ethnicity` SET TAGS ('dbx_pii_kind' = 'special_category');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `first_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `full_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `full_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_business_glossary_term' = 'Gender');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_sensitivity' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_pii_type' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_classification' = 'special_category');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `gender` SET TAGS ('dbx_pii_kind' = 'special_category');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `last_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `marital_status` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `marital_status` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `panel_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `panel_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`panel` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_tier` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_tier` ALTER COLUMN `tier_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_tier` ALTER COLUMN `tier_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`loyalty_tier` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` SET TAGS ('dbx_subdomain' = 'identity_management');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` SET TAGS ('dbx_reviewer_authority' = 'no_notes_column_H_consumer_goods');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `survey_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `survey_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `owner_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `owner_contact_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `owner_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `owner_contact_phone` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`consumer`.`survey` ALTER COLUMN `v2_added_flag` SET TAGS ('dbx_generated' = 'true');
