...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit` (
    ...
    `care_site_id` BIGINT COMMENT 'Reference to the facility or care setting where the visit occurred (hospital, clinic, outpatient center, telehealth hub). Links to the facility domain.',
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.audit. Business justification: Compliance audits routinely sample specific encounters for clinical documentation integrity, billing accuracy, and quality measure validation. Auditors reference visit records as evidence in findings.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Every visit incurs costs tracked to a specific cost center for departmental P&L, variance analysis, Medicare cost reporting, and service-line profitability. Core operational link in healthcare financi',
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Visit revenue and costs post to the general ledger for financial statement preparation, regulatory reporting (Medicare cost reports), and consolidated financial reporting across entities. Essential fi',
    `research_study_id` BIGINT COMMENT 'Foreign key linking to research.research_study. Business justification: Clinical encounters frequently occur as part of research protocols. Tracking which study a visit supports is essential for research billing determination, protocol compliance monitoring, regulatory re',
    ...
    `principal_icd10_diagnosis_code` STRING COMMENT 'ICD-10-CM code for the principal diagnosis — the condition established after study to be chiefly responsible for the admission. Required for DRG grouping, claims submission, and quality reporting.. Valid values are `^[A-Z][0-9A-Z]{1,6}(.[0-9A-Z]{1,4})?$`',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` (
    ...
    `message_log_id` BIGINT COMMENT 'Unique identifier of the originating HL7 ADT message from the source system (Epic or Cerner). Used for message traceability, deduplication, and reconciliation with upstream ADT feeds. Corresponds to MSH-10 in HL7 v2.x.',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Patient transport events require accountability tracking for safety incidents, OSHA reporting, and labor cost allocation. Transport staff (orderlies, techs) are workforce employees whose activity must',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Provider assignments drive RVU-based compensation and cost allocation models. Linking to cost centers enables provider productivity analysis, departmental expense tracking, and physician compensation ',
    `insurance_network_participation_id` BIGINT COMMENT 'Foreign key linking to insurance.network_participation. Business justification: Determines in-network vs out-of-network status for each provider on a visit, directly impacting reimbursement rates and patient cost-sharing. Critical for claims processing, payment accuracy, and pati',
    ...
    `provider_payer_enrollment_id` BIGINT COMMENT 'Foreign key linking to provider.payer_enrollment. Business justification: Validates that the assigned clinician is enrolled with the patients payer at time of service. Critical for billing compliance, claims adjudication, and preventing denials due to provider enrollment g',
    ...
    `stark_arrangement_id` BIGINT COMMENT 'Foreign key linking to compliance.stark_arrangement. Business justification: Stark Law compliance monitoring requires tracking which physicians provided designated health services under which financial arrangements. Referral volume tracking and FMV validation depend on linking',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Advanced practice providers (NPs, PAs) require documented nurse supervision in many states and for credentialing. Tracks supervisory relationships for compliance, privileging, and malpractice coverage',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` (
    ...
    `care_site_id` BIGINT COMMENT 'Reference to the hospital or care facility where the encounter occurred. Supports facility-level CMI benchmarking, regulatory reporting, and payer contract management.',
    ...
    `employee_id` BIGINT COMMENT 'Reference to the HIM (Health Information Management) coder who finalized the DRG assignment. Supports coding productivity reporting, quality audits, and compliance tracking.',
    ...
    `principal_diagnosis_code` STRING COMMENT 'The ICD-10-CM code for the principal diagnosis — the condition established after study to be chiefly responsible for the admission. Drives DRG grouping logic and is a key CDI target for reimbursement optimization.. Valid values are `^[A-Z][0-9A-Z]{2,6}(.[0-9A-Z]{1,4})?$`',
    `principal_diagnosis_description` STRING COMMENT 'Full text description of the principal ICD-10-CM diagnosis code. Supports CDI workflow review, clinical documentation audits, and payer communication.',
    ...
    `secondary_diagnosis_count` STRING COMMENT 'The total number of secondary (additional) ICD-10-CM diagnosis codes submitted on the claim. Influences CC/MCC determination and DRG tier assignment; a key CDI metric for documentation completeness.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` (
    ...
    `audit_finding_id` BIGINT COMMENT 'Foreign key linking to compliance.audit_finding. Business justification: Audit findings frequently cite specific diagnosis coding errors (upcoding, specificity, POA indicator errors). Direct linkage enables traceability from finding to the exact diagnosis record under revi',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Diagnosis coding drives reimbursement and HCC risk adjustment. Linking to cost centers enables service-line profitability analysis, clinical program financial performance, and diagnosis-related cost b',
    ...
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Diagnoses often require specific medications or supplies (diabetes→insulin, wounds→dressings). Formulary management, clinical pathway optimization, and diagnosis-related supply cost analysis depend on',
    ...
    `note_id` BIGINT COMMENT 'Reference to the clinical note or documentation that supports this diagnosis. Links the coded diagnosis back to the source clinical documentation for CDI, audit, and RAC defense purposes. Sourced from Epic ClinDoc.',
    `research_study_id` BIGINT COMMENT 'Foreign key linking to research.research_study. Business justification: Research protocols require tracking diagnoses occurring during study participation for safety monitoring, endpoint assessment, and adverse event reporting. Clinical research operations need to identif',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Procedures are performed in specific departments/cost centers. Essential for activity-based costing, OR suite profitability analysis, procedural revenue tracking, and CPT-based cost allocation for dep',
    ...
    `interventional_procedure_id` BIGINT COMMENT 'Foreign key linking to radiology.interventional_procedure. Business justification: Interventional radiology procedures (biopsies, drainages, embolizations) are billable encounter procedures requiring linkage for charge capture, procedural documentation, and outcomes tracking. IR pro',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Procedures consume specific supplies, devices, and implants that must be documented for billing (CDM charge codes), regulatory compliance (UDI tracking for implantables), and procedure cost analysis. ',
    ...
    `or_suite_id` BIGINT COMMENT 'Reference to the operating room or procedure room where the procedure was performed. Supports OR utilization reporting, room turnover analysis, and facility capacity planning.',
    ...
    `research_study_id` BIGINT COMMENT 'Foreign key linking to research.research_study. Business justification: Research billing requires distinguishing study-mandated procedures from standard-of-care for coverage analysis and sponsor billing. Protocol compliance monitoring needs to verify required procedures w',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Bed assignments are made by charge nurses or bed coordinators. Required for workflow accountability, assignment dispute resolution, and tracking coordinator productivity. Distinct from attending clini',
    `care_site_id` BIGINT COMMENT 'Reference to the facility (hospital or care site) where the bed is located. Supports multi-facility capacity planning and reporting.',
    ...
    `bed_id` BIGINT COMMENT 'Unique identifier of the physical bed as defined in the facilitys bed management system (Epic ADT or Cerner Bed Board). Used to track individual bed utilization, turnover, and HAI (Healthcare-Associated Infection) events.',
    ...
    `adt_event_type` STRING COMMENT 'HL7 ADT (Admit, Discharge, Transfer) event type code that triggered this bed assignment record. A01 = admit; A02 = transfer; A03 = discharge; A04 = register outpatient; A08 = update patient info; A11 = cancel admit; A12 = cancel transfer; A13 = cancel discharge. Supports HL7 FHIR interoperability and HIE (Health Information Exchange) event processing. [ENUM-REF-CANDIDATE: A01|A02|A03|A04|A08|A11|A12|A13 — 8 candidates stripped; promote to reference product]',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` (
    ...
    `visit_mpi_record_id` BIGINT COMMENT 'Reference to the patient whose insurance coverage is being captured for this visit. Links to the Master Patient Index (MPI) in the patient domain.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` (
    `encounter_authorization_id` BIGINT COMMENT 'Unique surrogate primary key for the prior authorization and concurrent review record within the encounter domain. Assigned by the lakehouse Silver layer upon ingestion.',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Utilization review coordinators process authorization requests. Required for audit trails, denial appeal preparation, productivity tracking, and compliance with payer submission timelines. Distinct fr',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Prior authorizations for services tie to the cost center delivering care. Needed for denied-service cost tracking, authorization yield financial analysis, and payer contract performance evaluation at ',
    ...
    `interface_channel_id` BIGINT COMMENT 'Foreign key linking to interoperability.interface_channel. Business justification: Authorization requests/responses are transmitted via specific payer interface channels. Tracking which channel processed each authorization enables SLA monitoring, payer-specific error analysis, routi',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Prior authorizations frequently specify approved devices, medications, or supplies by item number. Payer contract compliance and authorization workflow systems require tracking which specific material',
    ...
    `research_study_id` BIGINT COMMENT 'Foreign key linking to research.research_study. Business justification: Prior authorizations for research-related services must reference the study protocol for proper coverage determination under Medicare NCD 310.1 and commercial clinical trial policies. Research billing',
    ...
    `authorization_context` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` (
    `visit_status_history_id` BIGINT COMMENT 'Unique surrogate identifier for each visit status history record. Primary key for the visit_status_history data product in the encounter domain.',
    `bed_id` BIGINT COMMENT 'Reference to the specific bed assigned to the patient at the time of this status change. Supports bed management, occupancy tracking, and throughput analysis.',
    `clinician_id` BIGINT COMMENT 'Reference to the attending physician responsible for the patient at the time of this status change. Supports provider-level throughput and LOS analysis.',
    `corrected_record_visit_status_history_id` BIGINT COMMENT 'Reference to the original visit_status_history record that this record corrects or supersedes. Null if is_corrected is False. Enables correction chain tracing for audit and HIM workflows.',
    `demographics_id` BIGINT COMMENT 'Reference to the patient associated with the visit. Enables patient-level throughput and LOS milestone analysis without requiring a join to the visit table.',
    `drg_id` BIGINT COMMENT 'Foreign key linking to reference.drg. Business justification: Status change tracking—DRG codes captured during visit status changes must reference official DRG definitions for revenue cycle monitoring, case management tracking, and real-time financial impact ana',
    `employee_id` BIGINT COMMENT 'Reference to the system user (clinician, registrar, or automated process) who initiated or recorded the status change. Used for workflow auditing and accountability tracking.',
    `message_log_id` BIGINT COMMENT 'The HL7 message control ID (MSH-10) from the ADT message that triggered this status change. Enables traceability back to the originating HL7 ADT message for interface troubleshooting and audit.',
    `org_unit_id` BIGINT COMMENT 'Reference to the clinical department or unit associated with this status change event (e.g., ED, ICU, Med-Surg). Supports department-level throughput and capacity analysis.',
    `care_site_id` BIGINT COMMENT 'Reference to the facility or hospital where the visit status change occurred. Enables multi-facility throughput comparison and operational reporting.',
    `visit_id` BIGINT COMMENT 'Reference to the parent visit/encounter record for which this status change event is recorded. Links the status history to the core encounter.',
    `visit_transfer_destination_facility_care_site_id` BIGINT COMMENT 'Reference to the destination facility when the status change represents a patient transfer event. Null for non-transfer status changes. Supports inter-facility transfer tracking and care continuity analysis.',
    `admission_source_code` STRING COMMENT 'Standardized code indicating the source from which the patient was admitted at the time of an admission status event (e.g., emergency room, physician referral, transfer from another facility). Relevant for admit-type status transitions. Aligns with UB-04 FL-15.',
    `adt_event_reason_code` STRING COMMENT 'Coded reason for the ADT event or status change as defined by the source system or HL7 standard (e.g., elective admission, emergency, transfer for higher level of care). Supports root-cause analysis of status transitions.',
    `adt_event_type` STRING COMMENT 'HL7 ADT event type code that triggered this status change (e.g., A01=Admit, A02=Transfer, A03=Discharge, A04=Register Outpatient, A05=Pre-Admit, A06=Change Outpatient to Inpatient). Enables HL7-aligned event classification. [ENUM-REF-CANDIDATE: A01|A02|A03|A04|A05|A06|A07|A08|A09|A10|A11|A12|A13|A21|A22|A38|A40 — promote to reference product]. Valid values are `A01|A02|A03|A04|A05|A06`',
    `care_setting` STRING COMMENT 'Physical or virtual care setting where the patient was located at the time of this status change (e.g., ED, ICU, OR, inpatient unit, outpatient clinic, observation unit, telehealth). Supports care setting transition analysis. [ENUM-REF-CANDIDATE: ED|ICU|OR|inpatient_unit|outpatient_clinic|observation_unit|telehealth — 7 candidates stripped; promote to reference product]',
    `changed_by_role` STRING COMMENT 'Functional role of the user who performed the status change (e.g., physician, nurse, registrar, care coordinator, system). Enables role-based throughput and workflow analysis.. Valid values are `physician|nurse|registrar|care_coordinator|system|other`',
    `changed_by_user_name` STRING COMMENT 'Display name of the user who initiated the status change. Carried on the record to support audit reporting without requiring a join to the user directory.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this visit status history record was first created in the data platform. Distinct from status_change_timestamp which reflects the real-world event time. Used for audit trail and ETL lineage.',
    `department_name` STRING COMMENT 'Name of the clinical department or unit at the time of the status change. Carried on the record to support reporting without requiring a join to the department reference table.',
    `discharge_disposition_code` STRING COMMENT 'Standardized code indicating the patients disposition at the time of a discharge status event (e.g., home, skilled nursing facility, expired, AMA). Aligns with UB-04 FL-17 and CMS discharge disposition codes. Relevant for discharge-type status transitions.',
    `duration_in_status_minutes` STRING COMMENT 'Number of minutes the visit remained in the previous status before transitioning to the new status. Calculated at ingestion time from consecutive status_change_timestamps. Supports LOS milestone tracking and throughput bottleneck analysis.',
    `effective_date` DATE COMMENT 'Calendar date on which the status change became effective. Derived from status_change_timestamp for date-level partitioning, daily census reporting, and operational dashboards.',
    `encounter_number` STRING COMMENT 'Human-readable, externally visible encounter or visit number assigned by the source system (e.g., Epic CSN or Cerner FIN). Used for cross-system reconciliation and operational reporting.',
    `financial_class` STRING COMMENT 'Financial class or payer category of the visit at the time of this status change (e.g., Medicare, Medicaid, Commercial, Self-Pay). Supports payer-stratified throughput and LOS analysis.',
    `is_corrected` BOOLEAN COMMENT 'Indicates whether this status history record is a correction of a previously entered status change. True if this record supersedes an erroneous prior entry. Supports data quality auditing and HIM correction workflows.',
    `is_current_status` BOOLEAN COMMENT 'Indicates whether this record represents the most recent (current) status of the visit. True for the latest status record; False for all prior status records. Enables efficient current-state queries without window functions.',
    `is_system_generated` BOOLEAN COMMENT 'Indicates whether this status change was automatically generated by the source system (e.g., automated bed assignment, scheduled discharge trigger) rather than manually entered by a user. Supports audit quality and workflow automation analysis.',
    `los_at_event_hours` DECIMAL(18,2) COMMENT 'Cumulative length of stay in hours from the initial admission or arrival timestamp to the time of this status change event. Supports LOS milestone tracking and throughput benchmarking at each status transition point.',
    `mrn` STRING COMMENT 'Medical Record Number assigned to the patient by the facility. Carried on the status history record to support operational auditing and patient-level throughput reporting without requiring upstream joins.',
    `new_status` STRING COMMENT 'The visit status assigned as a result of this status change event. Represents the current state of the visit at the time of this record. [ENUM-REF-CANDIDATE: scheduled|pre-registered|arrived|admitted|in-progress|observation|transferred|discharged|cancelled|no-show|left-without-being-seen — promote to reference product]',
    `notes` STRING COMMENT 'Free-text notes or comments entered by the user at the time of the status change. May include clinical context, operational instructions, or escalation notes. Supports workflow auditing and care coordination.',
    `previous_status` STRING COMMENT 'The visit status immediately before this status change event occurred. Null for the initial status record. Used to reconstruct the full status transition chain. [ENUM-REF-CANDIDATE: scheduled|pre-registered|arrived|admitted|in-progress|observation|transferred|discharged|cancelled|no-show|left-without-being-seen — promote to reference product]',
    `room_number` STRING COMMENT 'Room number assigned to the patient at the time of this status change. Relevant for bed assignment, transfer, and discharge events. Supports bed management and capacity planning.',
    `sequence_number` STRING COMMENT 'Ordinal sequence number of this status change event within the visit lifecycle, starting at 1. Enables chronological ordering of status transitions for a given visit without relying solely on timestamps.',
    `source_system_event_code` STRING COMMENT 'The native event or transaction identifier from the originating source system (e.g., Epic ADT event ID, Cerner Millennium event ID). Used for cross-system reconciliation, deduplication, and ETL lineage tracking.',
    `status_change_reason` STRING COMMENT 'Free-text or coded narrative reason entered by the user or system explaining why the visit status was changed (e.g., Patient arrived at ED triage, Bed assigned in ICU, Patient left without being seen). Supports operational workflow auditing.',
    `status_change_timestamp` TIMESTAMP COMMENT 'The precise date and time when the visit status transition occurred. Principal event timestamp for this record. Used for LOS milestone tracking, throughput analysis, and ADT event sequencing.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this visit status history record was last modified in the data platform. Used for incremental ETL processing, change detection, and audit trail maintenance.',
    `vibe_added_flag` BOOLEAN COMMENT 'Flag added by VIBE mutator to ensure change',
    CONSTRAINT pk_visit_status_history PRIMARY KEY(`visit_status_history_id`)
) COMMENT 'Audit trail of all status changes for a visit, supporting compliance and operational reporting.';
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` (
    ...
    `care_site_id` BIGINT COMMENT 'Reference to the facility (ED or urgent care location) where the triage assessment was conducted. Supports multi-site throughput and capacity analytics.',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Triage nurses are often RNs tracked in workforce (not provider domain). Required for ED staffing models, competency validation (ESI certification), and labor productivity. Complements existing clinici',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`readmission` (
    ...
    `care_site_id` BIGINT COMMENT 'Foreign key reference to the facility where the readmission encounter occurred. Supports facility-level HRRP penalty calculation and Joint Commission reporting.',
    ...
    `primary_icd10_code` STRING COMMENT 'Primary ICD-10-CM diagnosis code assigned at the readmission encounter. Core field for determining whether the readmission is related to the index condition and for CMS HRRP measure attribution.. Valid values are `^[A-Z][0-9A-Z]{1,6}$`',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` (
    ...
    `care_site_id` BIGINT COMMENT 'Reference to the healthcare facility where the patient was discharged from.',
    `care_transition_notification_id` BIGINT COMMENT 'Foreign key linking to interoperability.care_transition_notification. Business justification: CMS Interoperability rules require discharge summaries to trigger care transition notifications. Linking enables compliance tracking, notification delivery verification, acknowledgment monitoring, and',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Case managers coordinate discharge planning, DME orders, home health referrals, and care transitions. Essential for readmission prevention programs, utilization management, and CMS care transition mea',
    `cda_document_id` BIGINT COMMENT 'The unique identifier of the discharge summary document in the source EHR system for traceability and audit purposes.',
    `compliance_regulatory_submission_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_submission. Business justification: Discharge summaries provide clinical documentation for quality measure submissions (CMS Core Measures, Joint Commission, state quality reporting). Specific discharge summaries are referenced in regula',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Discharge events close episodes of care with final cost accumulation to the treating cost center. Required for case costing, length-of-stay financial impact analysis, and discharge disposition cost tr',
    ...
    `principal_diagnosis_code` STRING COMMENT 'The ICD-10-CM diagnosis code representing the condition established after study to be chiefly responsible for the admission.',
    ...
    `secondary_diagnosis_count` STRING COMMENT 'The number of secondary diagnoses documented for this encounter, used for case complexity assessment.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` (
    `transfer_request_id` BIGINT COMMENT 'Unique identifier for the patient transfer request record. Primary key for the transfer_request product.',
    `bed_id` BIGINT COMMENT 'Reference to the specific bed assigned to the patient at the destination facility.',
    `care_site_id` BIGINT COMMENT 'Reference to the facility to which the patient is being transferred.',
    `employee_id` BIGINT COMMENT 'Reference to the user (administrator, physician, or case manager) who approved the transfer request.',
    `mpi_record_id` BIGINT COMMENT 'Reference to the patient being transferred.',
    `origin_bed_id` BIGINT COMMENT 'Reference to the specific bed the patient is being transferred from.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Inter-unit transfers move cost responsibility between cost centers. Required for accurate departmental cost accounting, transfer pricing between units, and cost allocation during multi-department epis',
    `original_transfer_request_id` BIGINT COMMENT 'Self-referencing FK on transfer_request (original_transfer_request_id)',
    `primary_transfer_care_site_id` BIGINT COMMENT 'Reference to the facility from which the patient is being transferred.',
    `clinician_id` BIGINT COMMENT 'Reference to the provider who initiated the transfer request.',
    `visit_id` BIGINT COMMENT 'Reference to the encounter (visit) for which the transfer is being requested.',
    `accepting_provider_npi` STRING COMMENT 'National Provider Identifier of the physician or clinician accepting the patient at the destination facility.',
    `actual_transfer_timestamp` TIMESTAMP COMMENT 'Actual date and time when the patient transfer was completed and the patient arrived at the destination.',
    `acuity_level` STRING COMMENT 'Clinical acuity classification of the patient at the time of transfer request, indicating urgency and level of care required.. Valid values are `critical|acute|stable|routine`',
    `approval_required` BOOLEAN COMMENT 'Indicates whether the transfer request requires administrative or clinical approval before proceeding.',
    `approval_timestamp` TIMESTAMP COMMENT 'Date and time when the transfer request was approved.',
    `bed_availability_confirmed` BOOLEAN COMMENT 'Indicates whether the destination facility has confirmed availability of an appropriate bed for the patient.',
    `cancellation_reason` STRING COMMENT 'Explanation for why the transfer request was cancelled after initial approval.',
    `clinical_indication` STRING COMMENT 'Detailed clinical justification for the transfer, including diagnosis, condition severity, and required interventions not available at the origin facility.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this transfer request record was first created in the system.',
    `denial_reason` STRING COMMENT 'Explanation for why the transfer request was denied, if applicable.',
    `destination_unit_code` STRING COMMENT 'Department or unit code at the destination facility where the patient will be admitted.',
    `emtala_certification_completed` BOOLEAN COMMENT 'Indicates whether the required EMTALA physician certification form has been completed, documenting that medical benefits of transfer outweigh risks.',
    `emtala_certification_timestamp` TIMESTAMP COMMENT 'Date and time when the EMTALA physician certification was signed.',
    `emtala_compliant` BOOLEAN COMMENT 'Indicates whether the transfer request meets EMTALA requirements for appropriate transfer, including medical screening, stabilization, and informed consent.',
    `isolation_required` BOOLEAN COMMENT 'Indicates whether the patient requires isolation precautions during and after transfer due to infectious disease or other clinical reasons.',
    `isolation_type` STRING COMMENT 'Type of isolation precautions required for the patient (e.g., contact, droplet, airborne, protective).. Valid values are `contact|droplet|airborne|protective|none`',
    `level_of_care_required` STRING COMMENT 'Specific level of care needed at the destination facility (e.g., ICU, step-down, telemetry, general medical-surgical).',
    `medical_records_sent` BOOLEAN COMMENT 'Indicates whether copies of medical records, imaging, and test results have been sent to the accepting facility.',
    `medical_records_sent_timestamp` TIMESTAMP COMMENT 'Date and time when medical records were transmitted to the destination facility.',
    `mrn` STRING COMMENT 'The patients medical record number at the originating facility.',
    `origin_unit_code` STRING COMMENT 'Department or unit code at the origin facility where the patient is currently located.',
    `patient_consent_obtained` BOOLEAN COMMENT 'Indicates whether informed consent for the transfer has been obtained from the patient or their legal representative.',
    `patient_consent_timestamp` TIMESTAMP COMMENT 'Date and time when patient or representative consent for transfer was documented.',
    `patient_stability_score` STRING COMMENT 'Numeric score representing the patients clinical stability at the time of transfer, used to assess transfer risk.',
    `primary_diagnosis_code` STRING COMMENT 'ICD-10 diagnosis code representing the primary condition necessitating the transfer.',
    `primary_diagnosis_description` STRING COMMENT 'Human-readable description of the primary diagnosis associated with the transfer request.',
    `request_timestamp` TIMESTAMP COMMENT 'Date and time when the transfer request was initiated. This is the principal business event timestamp for the transfer request transaction.',
    `requested_transfer_timestamp` TIMESTAMP COMMENT 'Desired date and time for the patient transfer to occur, as specified by the requesting provider.',
    `requesting_provider_npi` STRING COMMENT 'National Provider Identifier of the physician or clinician requesting the transfer.',
    `source_system_record_code` STRING COMMENT 'Unique identifier for this transfer request in the source operational system.',
    `specialty_service_required` STRING COMMENT 'Specialized medical service or capability required at the destination facility (e.g., neurosurgery, cardiac catheterization, burn unit, neonatal intensive care).',
    `transfer_reason` STRING COMMENT 'Clinical or operational reason for the transfer request. Free-text description of why the patient needs to be transferred.',
    `transfer_reason_code` STRING COMMENT 'Standardized code representing the reason for transfer (e.g., higher level of care, specialty service unavailable, patient request, capacity constraints).',
    `transfer_request_number` STRING COMMENT 'Business identifier for the transfer request, used for tracking and communication between facilities.',
    `transfer_status` STRING COMMENT 'Current lifecycle status of the transfer request. Tracks the request from initial submission through completion or cancellation. [ENUM-REF-CANDIDATE: requested|pending_approval|approved|in_progress|completed|cancelled|denied — 7 candidates stripped; promote to reference product]',
    `transfer_type` STRING COMMENT 'Classification of the transfer request based on scope and urgency. Inter-facility transfers occur between different healthcare organizations, intra-facility transfers occur within the same organization.. Valid values are `inter_facility|intra_facility|emergency|elective|specialty_care|higher_level_of_care`',
    `transport_mode` STRING COMMENT 'Method of transportation used or planned for the patient transfer.. Valid values are `ambulance|air_ambulance|wheelchair_van|private_vehicle|walk`',
    `transport_provider` STRING COMMENT 'Name of the ambulance service, air transport company, or other transportation provider responsible for the transfer.',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time when this transfer request record was last modified.',
    `vibe_added_flag` BOOLEAN COMMENT 'Flag added by VIBE mutator to ensure change',
    CONSTRAINT pk_transfer_request PRIMARY KEY(`transfer_request_id`)
) COMMENT 'Patient transfer request record tracking inter-facility and intra-facility transfers with EMTALA compliance.';

CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` (
    `visit_coverage_id` BIGINT COMMENT 'Primary key for the visit_coverage association',
    `payer_id` BIGINT COMMENT 'Foreign key linking to the insurance payer providing coverage for this visit',
    `visit_id` BIGINT COMMENT 'Foreign key linking to the patient visit record that this coverage applies to',
    `authorization_number` STRING COMMENT 'Unique authorization or referral number issued by the payer approving coverage for this specific visit. Required on claim submission for services requiring prior authorization. Obtained during pre-registration or admission process and specific to this visit-payer combination.',
    `authorization_status` STRING COMMENT 'Current status of prior authorization for this visit-payer combination. Approved indicates payer has authorized coverage for the planned services. Not Required indicates payer does not require authorization for this visit type or service category. Drives admission clearance and claim submission readiness.',
    `copay_amount` DECIMAL(18,2) COMMENT 'Fixed dollar amount the patient owes at time of service per the benefit plan for this payer. Collected at registration or discharge. Amount is specific to this visit-payer combination and varies by payer contract and patient benefit plan.',
    `coverage_effective_date` DATE COMMENT 'Date on which the patients coverage with this payer became active. Retrieved from eligibility verification. Used to validate that coverage was active on the date of service for this visit.',
    `coverage_sequence` STRING COMMENT 'Indicates the order of financial responsibility when multiple payers cover a single visit. Primary payer is billed first, secondary covers remaining balance per coordination of benefits rules, tertiary covers after secondary. Critical for claim submission sequencing and coordination of benefits processing.',
    `coverage_termination_date` DATE COMMENT 'Date on which the patients coverage with this payer ends or ended. Retrieved from eligibility verification. Null for active open-ended coverage. Used to validate coverage was active on date of service and to flag potential coverage gaps.',
    `deductible_met_amount` DECIMAL(18,2) COMMENT 'Dollar amount of the annual deductible the patient has already satisfied with this payer at the time of visit registration. Retrieved from eligibility verification response. Used to calculate patient financial responsibility and estimate out-of-pocket costs for this visit.',
    `eligibility_status` STRING COMMENT 'Status of real-time eligibility verification performed at visit registration or admission. Verified indicates active coverage confirmed via EDI 270/271 transaction or payer portal. Drives financial counseling, authorization requirements, and registration workflow completion.',
    `eligibility_verification_timestamp` TIMESTAMP COMMENT 'Date and time when real-time eligibility verification was performed with this payer for this visit. Documents when coverage was confirmed and provides audit trail for registration compliance and claim denial prevention.',
    `network_status` STRING COMMENT 'Indicates whether the facility or attending provider is in-network or out-of-network for this specific payer. Determines reimbursement rates, patient cost-sharing levels, and claim adjudication rules. Retrieved during eligibility verification and specific to this visit-payer relationship.',
    `subscriber_relationship` STRING COMMENT 'Relationship of the patient to the insurance subscriber (policyholder) for this payer. Self indicates patient is the policyholder. Required for claim submission and coordination of benefits processing. Specific to this visit-payer combination as patient may have different relationships with different payers.',
    `vibe_added_flag` BOOLEAN COMMENT 'Flag added by VIBE mutator to ensure change',
    CONSTRAINT pk_visit_coverage PRIMARY KEY(`visit_coverage_id`)
) COMMENT 'Summary of insurance coverage applicable to a visit for billing and eligibility purposes.';

CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` (
    `visit_recall_impact_id` BIGINT COMMENT 'Primary key for the visit_recall_impact association',
    `clinician_id` BIGINT COMMENT 'Reference to the clinician assigned to conduct the clinical review and patient notification for this visit-recall combination. Supports patient safety workflow assignment and accountability.',
    `recall_notice_id` BIGINT COMMENT 'Foreign key linking to the recall notice that potentially affects this visit based on supply usage during the encounter.',
    `visit_id` BIGINT COMMENT 'Foreign key linking to the patient visit that may have been exposed to the recalled medical supply, device, or implant during the encounter.',
    `clinical_review_completed` BOOLEAN COMMENT 'Indicates whether a clinical review has been completed to assess the actual patient impact for this specific visit-recall combination. Clinical review determines if the recalled product was actually used on this patient and what clinical follow-up is required.',
    `clinical_review_date` DATE COMMENT 'Date on which the clinical review was completed for this visit-recall combination. Used to track compliance with patient safety investigation timelines.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this visit-recall impact record was created in the system, typically when the recall was matched to potentially affected visits through supply chain traceability analysis.',
    `follow_up_due_date` DATE COMMENT 'Target date by which clinical follow-up should be completed for this visit-recall combination. Used for patient safety task management and compliance tracking.',
    `follow_up_required` BOOLEAN COMMENT 'Indicates whether ongoing clinical follow-up is required for this patient based on the recall impact assessment. Drives care coordination and patient safety monitoring workflows.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to this visit-recall impact record, tracking the progression of the patient safety investigation and notification workflow.',
    `lot_serial_match_confirmed` BOOLEAN COMMENT 'Indicates whether the specific lot number or serial number of the product used in this visit matches the affected lot/serial range specified in the recall notice. Critical for determining actual patient exposure.',
    `notification_date` DATE COMMENT 'Date on which the patient associated with this visit was formally notified of potential exposure to the recalled product. Used for regulatory compliance tracking and patient safety audit trails.',
    `patient_impact_assessment` STRING COMMENT 'Clinical assessment of the actual or potential impact of the recalled product on this specific patient during this visit. Categorizes risk level and drives notification urgency and remediation priority.',
    `patient_notification_status` STRING COMMENT 'Current status of the patient notification workflow for this visit-recall combination. Tracks whether the patient has been notified of potential exposure to the recalled product, supporting FDA MedWatch and patient safety regulatory requirements.',
    `product_actually_used` BOOLEAN COMMENT 'Indicates whether the recalled product was definitively confirmed to have been used on this patient during this visit. Determined through supply chain traceability, clinical documentation review, and UDI tracking.',
    `remediation_action_taken` STRING COMMENT 'Specific remediation action taken for this visit-recall combination based on clinical review findings. Documents the healthcare organizations response to the recall for this specific patient encounter.',
    `remediation_date` DATE COMMENT 'Date on which the remediation action was completed for this visit-recall combination. Used for regulatory compliance tracking and patient safety audit trails.',
    `vibe_added_flag` BOOLEAN COMMENT 'Flag added by VIBE mutator to ensure change',
    CONSTRAINT pk_visit_recall_impact PRIMARY KEY(`visit_recall_impact_id`)
) COMMENT 'Tracks the impact of supply/device recalls on specific patient visits for safety follow-up.';
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` SET TAGS ('pii_subdomain' = 'visit_management');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `care_site_id` SET TAGS ('pii_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `audit_id` SET TAGS ('pii_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `audit_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `cost_center_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `general_ledger_id` SET TAGS ('pii_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `general_ledger_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `research_study_id` SET TAGS ('pii_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Discharging Provider ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `clinician_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `admission_timestamp` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `admitting_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `consent_obtained` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `consent_obtained` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `converted_to_inpatient` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `converted_to_inpatient` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `discharge_timestamp` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `drg_type` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `drg_weight` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `encounter_number` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `financial_class` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `inpatient_conversion_timestamp` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `inpatient_conversion_timestamp` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `observation_hours` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `observation_status` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `principal_icd10_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `telehealth_connection_quality` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `telehealth_platform` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` SET TAGS ('pii_subdomain' = 'patient_movement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `clinician_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `demographics_id` SET TAGS ('pii_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `message_log_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `prior_event_adt_event_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `visit_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `accepting_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `drg_type` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `event_timestamp` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `patient_class_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `patient_class_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `patient_stability_score` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `patient_stability_score` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `source_system_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`adt_event` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` SET TAGS ('pii_subdomain' = 'visit_management');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `cost_center_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `insurance_network_participation_id` SET TAGS ('pii_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `insurance_network_participation_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `insurance_network_participation_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `network_affiliation_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `payer_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `clinician_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `provider_payer_enrollment_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `referral_order_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `stark_arrangement_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `tertiary_visit_supervising_provider_clinician_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `visit_id` SET TAGS ('pii_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `billing_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `drg_attribution_flag` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `participation_duration_minutes` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `rendering_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `rvu_credit_flag` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `telehealth_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_provider` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` SET TAGS ('pii_data_type' = 'transactional_data');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` SET TAGS ('pii_subdomain' = 'coverage_reimbursement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_assignment_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `base_payment_rate` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `base_payment_rate` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_changed_flag` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_description` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_version` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_version_number` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `drg_weight` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `expected_reimbursement` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `initial_drg_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `initial_drg_weight` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `outlier_payment` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `patient_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `patient_type` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `principal_diagnosis_description` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `principal_procedure_code` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `principal_procedure_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `procedure_count` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `secondary_diagnosis_count` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `secondary_diagnosis_count` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`drg_assignment` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `visit_diagnosis_id` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `visit_diagnosis_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `audit_finding_id` SET TAGS ('pii_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `cost_center_id` SET TAGS ('pii_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd_code_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `snomed_concept_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `coding_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_rank` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_rank` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_seq_num` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_seq_num` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_source` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_source` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_type` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `diagnosis_type` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `drg_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `drg_relevance_flag` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `drg_type` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `encounter_diagnosis_comment` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `encounter_diagnosis_comment` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `encounter_diagnosis_source_code` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `encounter_diagnosis_source_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd10_code` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd10_code` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd10_description` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd10_description` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `icd10_version` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `mental_health_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `onset_date` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `onset_date` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `primary_diagnosis_flag` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `primary_diagnosis_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `resolved_date` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `resolved_date` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `sdoh_flag` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `snomed_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `substance_use_flag` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_diagnosis` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `visit_procedure_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `cpt_code_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `hcpcs_code_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `icd_code_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `interventional_procedure_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_identifier' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `snomed_concept_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `charge_amount` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `consent_obtained_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `consent_obtained_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `cpt_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `cpt_modifier_1` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `cpt_modifier_2` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `drg_relevant_flag` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `hcpcs_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `icd10_pcs_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `is_principal_procedure` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `performing_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_date` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_description` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_end_timestamp` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_number` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_start_timestamp` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_status` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `procedure_type` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `snomed_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `source_system_procedure_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_procedure` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` SET TAGS ('pii_subdomain' = 'patient_movement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_identifier' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `bed_gender_designation` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `bed_gender_designation` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `bed_gender_designation` SET TAGS ('pii_sensitive' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `bed_gender_designation` SET TAGS ('pii_uc_classification' = 'sensitive');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `bed_label` SET TAGS ('pii_business_glossary_term' = 'Bed Label');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `is_observation_status` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `patient_class` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `patient_class` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `unit_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`bed_assignment` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` SET TAGS ('pii_subdomain' = 'coverage_reimbursement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `visit_insurance_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `visit_insurance_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `health_plan_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_id` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `visit_mpi_record_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `visit_mpi_record_id` SET TAGS ('pii_identifier' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `authorization_expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `billing_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `coinsurance_rate` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `coinsurance_rate` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `coinsurance_rate` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `copay_amount` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `copay_amount` SET TAGS ('pii_financial' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `deductible_amount` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `deductible_amount` SET TAGS ('pii_financial' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `deductible_met_amount` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `deductible_met_amount` SET TAGS ('pii_financial' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `financial_class` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_type_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_type_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_verification_source` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `insurance_verification_source` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `out_of_pocket_max` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `out_of_pocket_max` SET TAGS ('pii_financial' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `out_of_pocket_met_amount` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `out_of_pocket_met_amount` SET TAGS ('pii_financial' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `payer_phone` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `payer_phone` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_dob` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_dob` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_dob` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_relationship` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `subscriber_relationship` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_insurance` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` SET TAGS ('pii_data_type' = 'transactional_data');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` SET TAGS ('pii_subdomain' = 'coverage_reimbursement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` SET TAGS ('pii_mvm_alias' = 'authorization');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` SET TAGS ('pii_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` SET TAGS ('pii_mvm_included' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `encounter_authorization_id` SET TAGS ('pii_business_glossary_term' = 'Encounter Authorization ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `order_authorization_id` SET TAGS ('pii_business_glossary_term' = 'Order Authorization Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinical_order_id` SET TAGS ('pii_business_glossary_term' = 'Clinical Order Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinical_order_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Requesting Provider ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Authorization Coordinator Employee Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cost_center_id` SET TAGS ('pii_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `coverage_policy_id` SET TAGS ('pii_business_glossary_term' = 'Coverage Policy Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cpt_code_id` SET TAGS ('pii_business_glossary_term' = 'Cpt Code Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cpt_code_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_business_glossary_term' = 'Patient ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `drg_id` SET TAGS ('pii_business_glossary_term' = 'Drg Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `hcpcs_code_id` SET TAGS ('pii_business_glossary_term' = 'Hcpcs Code Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `hcpcs_code_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('pii_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd_code_id` SET TAGS ('pii_business_glossary_term' = 'Icd10 Diagnosis Code Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd_code_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_business_glossary_term' = 'Insurance Coverage Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `interface_channel_id` SET TAGS ('pii_business_glossary_term' = 'Interface Channel Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `material_master_id` SET TAGS ('pii_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_id` SET TAGS ('pii_business_glossary_term' = 'Payer ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `prior_auth_rule_id` SET TAGS ('pii_business_glossary_term' = 'Prior Auth Rule Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `research_study_id` SET TAGS ('pii_business_glossary_term' = 'Research Study Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter ID');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `appeal_decision` SET TAGS ('pii_value_regex' = 'upheld|overturned|partial|pending');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `appeal_decision_date` SET TAGS ('pii_business_glossary_term' = 'Appeal Decision Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `appeal_filed_date` SET TAGS ('pii_business_glossary_term' = 'Appeal Filed Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `appeal_status` SET TAGS ('pii_business_glossary_term' = 'Authorization Appeal Status');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `appeal_status` SET TAGS ('pii_value_regex' = 'not_appealed|appeal_pending|appeal_approved|appeal_denied|appeal_withdrawn');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `approved_quantity` SET TAGS ('pii_business_glossary_term' = 'Approved Service Quantity');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `approved_quantity_unit` SET TAGS ('pii_business_glossary_term' = 'Approved Quantity Unit of Measure');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `approved_quantity_unit` SET TAGS ('pii_value_regex' = 'visits|units|sessions|days|procedures|treatments');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `authorization_context` SET TAGS ('pii_ssot' = 'discriminator');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `authorization_status` SET TAGS ('pii_value_regex' = 'approved|denied|pending|appealed|retro_authorized|cancelled');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `authorization_type` SET TAGS ('pii_value_regex' = 'prior|concurrent|retrospective|expedited');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `authorized_amount` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinical_notes` SET TAGS ('pii_business_glossary_term' = 'Authorization Clinical Notes');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinical_notes` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `clinical_notes` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cpt_code` SET TAGS ('pii_business_glossary_term' = 'Current Procedural Terminology (CPT) Code');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cpt_code` SET TAGS ('pii_value_regex' = '^[0-9]{4}[0-9A-Z]$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `cpt_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `days_used` SET TAGS ('pii_business_glossary_term' = 'Days Used');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `decision_datetime` SET TAGS ('pii_business_glossary_term' = 'Authorization Decision Date and Time');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `effective_date` SET TAGS ('pii_business_glossary_term' = 'Authorization Effective Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `end_date` SET TAGS ('pii_business_glossary_term' = 'Authorization End Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `expiration_date` SET TAGS ('pii_business_glossary_term' = 'Authorization Expiration Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `extension_authorization_number` SET TAGS ('pii_business_glossary_term' = 'Extension Authorization Number');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `extension_requested_flag` SET TAGS ('pii_business_glossary_term' = 'Authorization Extension Requested Flag');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `facility_type` SET TAGS ('pii_business_glossary_term' = 'Authorized Facility Type');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `hcpcs_code` SET TAGS ('pii_business_glossary_term' = 'Healthcare Common Procedure Coding System (HCPCS) Code');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `hcpcs_code` SET TAGS ('pii_value_regex' = '^[A-Z][0-9]{4}$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `hcpcs_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd10_diagnosis_code` SET TAGS ('pii_business_glossary_term' = 'International Classification of Diseases 10th Revision (ICD-10) Diagnosis Code');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd10_diagnosis_code` SET TAGS ('pii_value_regex' = '^[A-Z][0-9]{2}(.[0-9A-Z]{1,4})?$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd10_diagnosis_code` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `icd10_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `mvm_alias_name` SET TAGS ('pii_reconciliation' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `mvm_alias_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `mvm_alias_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `mvm_ecm_reconciled_flag` SET TAGS ('pii_reconciliation' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Authorization Notes');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_business_glossary_term' = 'Patient Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_authorization_reference` SET TAGS ('pii_business_glossary_term' = 'Payer Authorization Reference Number');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_name` SET TAGS ('pii_business_glossary_term' = 'Payer Reviewer Name');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_npi` SET TAGS ('pii_business_glossary_term' = 'Payer Reviewer National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_npi` SET TAGS ('pii_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `payer_reviewer_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_conducted` SET TAGS ('pii_business_glossary_term' = 'Peer-to-Peer Review Conducted Flag');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_date` SET TAGS ('pii_business_glossary_term' = 'Peer-to-Peer Review Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_outcome` SET TAGS ('pii_business_glossary_term' = 'Peer-to-Peer (P2P) Review Outcome');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_outcome` SET TAGS ('pii_value_regex' = 'approved|denied|partial|pending');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_review_date` SET TAGS ('pii_business_glossary_term' = 'Peer-to-Peer (P2P) Review Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `peer_to_peer_review_flag` SET TAGS ('pii_business_glossary_term' = 'Peer-to-Peer (P2P) Review Flag');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `priority` SET TAGS ('pii_business_glossary_term' = 'Authorization Request Priority');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `priority` SET TAGS ('pii_value_regex' = 'routine|urgent|stat|emergent');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `reconciled_from_mvm` SET TAGS ('pii_reconciliation' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `reconciliation_source` SET TAGS ('pii_reconciliation' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `request_datetime` SET TAGS ('pii_business_glossary_term' = 'Authorization Request Date and Time');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `request_submitted_timestamp` SET TAGS ('pii_business_glossary_term' = 'Authorization Request Submitted Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requested_quantity` SET TAGS ('pii_business_glossary_term' = 'Requested Service Quantity');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_name` SET TAGS ('pii_business_glossary_term' = 'Requesting Provider Name');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('pii_business_glossary_term' = 'Requesting Provider National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('pii_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `retro_authorization_reason` SET TAGS ('pii_business_glossary_term' = 'Retrospective Authorization Reason');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `reviewer_name` SET TAGS ('pii_business_glossary_term' = 'Payer Reviewer Name');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `reviewer_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `reviewer_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `service_category` SET TAGS ('pii_business_glossary_term' = 'Service Category');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `service_type` SET TAGS ('pii_business_glossary_term' = 'Authorized Service Type');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_facility_name` SET TAGS ('pii_business_glossary_term' = 'Servicing Facility Name');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_facility_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_facility_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('pii_business_glossary_term' = 'Servicing Provider National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('pii_value_regex' = '^[0-9]{10}$');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `source_system_auth_code` SET TAGS ('pii_business_glossary_term' = 'Source System Authorization ID');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `ssot_canonical_reference` SET TAGS ('pii_business_glossary_term' = 'SSOT Canonical Reference');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `ssot_reconciliation_status` SET TAGS ('pii_business_glossary_term' = 'SSOT Reconciliation Status');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `start_date` SET TAGS ('pii_business_glossary_term' = 'Authorization Start Date');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `submission_method` SET TAGS ('pii_business_glossary_term' = 'Authorization Submission Method');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `submission_method` SET TAGS ('pii_value_regex' = 'electronic|phone|fax|portal|edi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `turnaround_time_hours` SET TAGS ('pii_business_glossary_term' = 'Authorization Turnaround Time in Hours');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `units_used` SET TAGS ('pii_business_glossary_term' = 'Units Used');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `urgency_level` SET TAGS ('pii_business_glossary_term' = 'Authorization Urgency Level');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `urgency_level` SET TAGS ('pii_value_regex' = 'routine|urgent|emergent|expedited');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`encounter_authorization` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` SET TAGS ('pii_data_type' = 'transactional_data');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` SET TAGS ('pii_subdomain' = 'visit_management');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `demographics_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `demographics_id` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `changed_by_user_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `changed_by_user_name` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `changed_by_user_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `changed_by_user_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `department_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `duration_in_status_minutes` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `financial_class` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `is_system_generated` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_status_history` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `acuity_change_reason` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `acuity_change_reason` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `chief_complaint` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `chief_complaint` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `diastolic_bp_mmhg` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `diastolic_bp_mmhg` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `glasgow_coma_score` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `glasgow_coma_score` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `heart_rate_bpm` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `heart_rate_bpm` SET TAGS ('pii_health' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `heart_rate_bpm` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `mental_health_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `pain_score` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `pain_score` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `respiratory_rate_bpm` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `respiratory_rate_bpm` SET TAGS ('pii_health' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `respiratory_rate_bpm` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `spo2_percent` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `spo2_percent` SET TAGS ('pii_health' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `spo2_percent` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `systolic_bp_mmhg` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `systolic_bp_mmhg` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `temperature_celsius` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `temperature_celsius` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `triage_nurse_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `weight_kg` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`triage_assessment` ALTER COLUMN `weight_kg` SET TAGS ('pii_health' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `demographics_id` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `demographics_id` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `drg_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `drg_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `estimated_penalty_amount` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `hrrp_excess_readmission_ratio` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `index_primary_icd10_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `medication_reconciliation_completed` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `primary_icd10_code` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `sdoh_flag` SET TAGS ('pii_sensitive' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`readmission` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `addended_discharge_summary_id` SET TAGS ('pii_self_ref_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `compliance_regulatory_submission_id` SET TAGS ('pii_business_glossary_term' = 'Regulatory Submission Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `drg_assignment_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `discharge_instructions_text` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `discharge_medications_prescribed` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `discharge_medications_prescribed` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `discharging_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `home_health_referral_made` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `hospital_course_narrative` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `medication_reconciliation_completed` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `patient_education_provided` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `patient_education_provided` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `patient_education_topics` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `patient_education_topics` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `principal_diagnosis_description` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `procedures_performed_summary` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `procedures_performed_summary` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `secondary_diagnosis_count` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `secondary_diagnosis_count` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `summary_of_hospitalization` SET TAGS ('pii_confidential' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`discharge_summary` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` SET TAGS ('pii_subdomain' = 'patient_movement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `original_transfer_request_id` SET TAGS ('pii_self_ref_fk' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `accepting_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `medical_records_sent` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `medical_records_sent` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `medical_records_sent_timestamp` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `medical_records_sent_timestamp` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_consent_obtained` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_consent_obtained` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_consent_timestamp` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_consent_timestamp` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_stability_score` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `patient_stability_score` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `primary_diagnosis_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `primary_diagnosis_description` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `requesting_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`transfer_request` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_data_type' = 'association_data');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_subdomain' = 'coverage_reimbursement');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_association_edges' = 'encounter.visit,insurance.payer');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` ALTER COLUMN `subscriber_relationship` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` ALTER COLUMN `subscriber_relationship` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_coverage` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` SET TAGS ('pii_data_type' = 'association_data');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` SET TAGS ('pii_subdomain' = 'visit_management');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` SET TAGS ('pii_['model_tier' = 'MVM');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` SET TAGS ('pii_'ecm_member' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` SET TAGS ('pii_'mvm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `notification_date` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `patient_impact_assessment` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `patient_impact_assessment` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `patient_notification_status` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `patient_notification_status` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `patient_notification_status` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`encounter`.`visit_recall_impact` ALTER COLUMN `vibe_added_flag` SET TAGS ('pii_vibe_added' = 'true');
