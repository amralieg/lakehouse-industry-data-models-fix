-- Metric views for domain: compliance | Business: Water_Utilities | Version: 2 | Generated on: 2026-06-19 23:41:31
...
CREATE OR REPLACE VIEW `vibe_water_utilities_v1`.`_metrics`.`compliance_ccr_exceedance_rate`
WITH METRICS
LANGUAGE YAML
AS $$
  version: 1.1
  comment: "Rate of contaminant MCL/treatment-technique exceedances reportable in the Consumer Confidence Report relative to all compliance determinations — flagship drinking-water quality KPI."
  source: "`vibe_water_utilities_v1`.`compliance`.`compliance_violation`"
  dimensions:
    - name: "Facility"
      expr: facility_id
      comment: "Facility where violation occurred"
    - name: "Contaminant"
      expr: contaminant_id
      comment: "Contaminant involved"
  measures:
    - name: "CCR Exceedance Rate %"
      expr: ROUND(100.0 * COUNT(1) / NULLIF(COUNT(DISTINCT facility_id), 0), 2)
      comment: "Exceedance events per facility (rate proxy)"
    - name: "Exceedance Count"
      expr: COUNT(1)
      comment: "Total CCR-reportable exceedances"
$$;