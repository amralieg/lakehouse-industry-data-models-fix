...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`header` (
    ...
    `benefit_package_id` BIGINT COMMENT 'Foreign key linking to plan.benefit_package. Business justification: Claim header is processed against a specific benefit package governing cost-sharing, network designation, and prior auth requirements. Benefit package context at the header level drives adjudication r',
    `group_practice_id` BIGINT COMMENT 'Foreign key linking to provider.group_practice. Business justification: Claims submitted by group practices require group-level aggregation for contract performance monitoring, capitation reconciliation, and value-based care reporting. billing_provider_npi is a denormaliz',
    `eligibility_verification_id` BIGINT COMMENT 'Foreign key linking to enrollment.eligibility_verification. Business justification: At claim intake, real-time eligibility verification is performed and the result governs whether the claim proceeds to adjudication. Linking enrollment.header to the eligibility_verification record supports',
    `grace_period_id` BIGINT COMMENT 'Foreign key linking to billing.grace_period. Business justification: ACA regulatory requirement: claims for services during a members grace period (months 2-3 for APTC recipients) must be pended per 45 CFR 156.270. Adjudicators must identify the active grace period at',
    `health_plan_id` BIGINT COMMENT 'Foreign key linking to plan.health_plan. Business justification: Claim header must reference the health plan for claim routing, LOB assignment, MLR reporting, and regulatory submissions. Adjudication has this FK but the header itself needs direct plan context for i',
    ...
    `pa_decision_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_decision. Business justification: Claim-level PA decision tracking is required for MLR reporting, auto-adjudication workflows, and regulatory audits. Direct FK from claim header to pa_decision enables claim-level authorization outcome',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: CMS and state regulators require PA-to-claim reconciliation reporting. The claim headers prior_authorization_number is a denormalized reference to pa_request. A direct FK enables automated PA valid',
    `par_agreement_id` BIGINT COMMENT 'Foreign key linking to network.par_agreement. Business justification: The PAR agreement governs the reimbursement methodology and contract terms applied to the entire claim. Contract compliance reporting, reimbursement accuracy audits, and VBC settlement calculations re',
    `plan_election_id` BIGINT COMMENT 'Foreign key linking to enrollment.plan_election. Business justification: Claims adjudication requires the specific plan election active at time of service to determine covered benefits, cost-sharing tiers, network status, and member responsibility. Every health insurance c',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: Claims are adjudicated against a specific member policy. Policy-level deductible accumulation, OOP max tracking, and coverage-type routing all require member.header to reference the active policy. Ever',
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Inpatient/outpatient claims must reference the rendering facility for DRG grouping, network tier determination, and facility fee adjudication. Health plan operations require facility-level claim aggre',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`line` (
    ...
    `cost_share_rule_id` BIGINT COMMENT 'Foreign key linking to plan.cost_share_rule. Business justification: Each claim line is adjudicated against a specific cost share rule determining copay, coinsurance, and deductible application. This link is essential for member liability calculation, accumulator track',
    `drug_master_id` BIGINT COMMENT 'Foreign key linking to pharmacy.drug_master. Business justification: Medical claim lines for physician-administered drugs (chemotherapy, biologics, J-codes) carry NDC codes. Linking to drug_master normalizes drug reference data, enabling cross-channel drug utilization ',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` (
    ...
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: Claim diagnoses are evaluated against clinical criteria during medical necessity review. Direct FK enables medical necessity audit trails linking specific diagnosis codes to the criteria version appli',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`procedure` (
    ...
    `benefit_id` BIGINT COMMENT 'Foreign key linking to plan.benefit. Business justification: Procedures must be validated against specific benefits for coverage determination, prior authorization, and quality measure reporting. procedure has prior_authorization_number and prior_authorization_',
    `header_id` BIGINT COMMENT 'Identifier of the parent claim to which this procedure belongs.',
    `medical_policy_id` BIGINT COMMENT 'Foreign key linking to utilization.medical_policy. Business justification: Procedure codes are adjudicated against medical policies for coverage determination. Direct FK from procedure to medical_policy enables policy-based procedure audits, experimental/investigational cove',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: Each procedure must be matched against a PA request to validate the specific CPT/HCPCS code was authorized. Procedure-level PA validation is required for accurate adjudication and ACA/state regulatory',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` (
    ...
    `benefit_package_id` BIGINT COMMENT 'Foreign key linking to plan.benefit_package. Business justification: Adjudication applies benefit package rules for cost-sharing, network tier, and prior auth. The adjudication record must reference the governing benefit package to support audit trails, accumulator man',
    `header_id` BIGINT COMMENT 'Unique identifier of the claim that this adjudication decision applies to.',
    `cost_share_rule_id` BIGINT COMMENT 'Foreign key linking to plan.cost_share_rule. Business justification: Adjudication tracks accumulator_deductible_impact, accumulator_oop_impact, deductible_amount, and oop_amount — all governed by a specific cost_share_rule. Linking adjudication to the applied rule is m',
    `eligibility_span_id` BIGINT COMMENT 'Foreign key linking to member.member_eligibility_span. Business justification: Adjudication must confirm the members eligibility span was active on the service date before paying a claim. Linking adjudication to the specific eligibility span used for coverage verification is a ',
    ...
    `line_id` BIGINT COMMENT 'Foreign key linking to claim.line. Business justification: The adjudication product description explicitly states it captures decisions for each claim and claim line. While adjudication.claim_header_id links to the header, there is no FK to the specific cla',
    `medical_policy_id` BIGINT COMMENT 'Foreign key linking to utilization.medical_policy. Business justification: Adjudication decisions are governed by specific medical policy versions. Direct FK from adjudication to medical_policy enables policy-based adjudication audits, tracks which policy version drove each ',
    `identity_id` BIGINT COMMENT 'Unique identifier of the insured member associated with the claim.',
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: Adjudication of risk-adjusted contracts (Medicare Advantage, ACA) applies the members RAF score to determine allowed amounts. Linking adjudication to the specific risk score used creates an auditable',
    `pa_decision_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_decision. Business justification: Adjudication outcomes are directly governed by PA decisions — authorized quantities, denial reason codes, and approval limits flow from pa_decision into adjudication. Direct FK enables adjudication-to',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: Adjudication must verify PA status before approving payment. The adjudication engine references the PA request to confirm authorization was granted and validate authorized quantities. prior_authoriza',
    `par_agreement_id` BIGINT COMMENT 'Foreign key linking to network.par_agreement. Business justification: Adjudication applies reimbursement methodology and fee schedules defined in the PAR agreement. Contract compliance audits and reimbursement accuracy reviews require knowing which PAR agreement governe',
    `participation_status_id` BIGINT COMMENT 'Foreign key linking to provider.participation_status. Business justification: Adjudication determines in-network vs out-of-network benefit application by referencing the providers participation status at time of service. Regulatory compliance (network adequacy, COB processing)',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: Adjudication applies policy-specific rules: deductible thresholds, OOP maximums, benefit limits, and copay tiers. Linking adjudication directly to policy enables accumulator impact calculations and re',
    `provider_id` BIGINT COMMENT 'National Provider Identifier (NPI) of the treating provider.',
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: Adjudication determines in-network vs out-of-network status against a specific provider network. Network-level adjudication reporting, MLR calculations, and regulatory filings require direct linkage. ',
    `tier_id` BIGINT COMMENT 'Foreign key linking to network.tier. Business justification: Adjudication applies tier-specific cost-sharing rules (deductible, copay, coinsurance differentials). The tier governs member responsibility amounts calculated during adjudication. A domain expert wou',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`eob` (
    ...
    `adverse_determination_id` BIGINT COMMENT 'Foreign key linking to appeal.adverse_determination. Business justification: EOBs communicate adverse determination outcomes to members and contain appeal_rights_text/appeal_rights_version fields that are directly derived from the adverse determination. Linking EOB to the spec',
    ...
    `health_plan_id` BIGINT COMMENT 'Foreign key linking to plan.health_plan. Business justification: EOB documents are plan-branded and plan-specific; plan name, SBC reference, and regulatory appeal rights content derive from the health plan. sbc_reference on eob is a denormalized plan attribute. Dir',
    `identity_id` BIGINT COMMENT 'Unique identifier of the covered member associated with the claim.',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: ACA and state regulations mandate that EOBs include PA information when services required prior authorization. Direct FK from eob to pa_request enables accurate PA reference on member-facing EOB docum',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: EOBs are generated per policy and must display policy number, coverage details, and benefit structure. Member portal EOB grouping by policy, ACA SBC references, and regulatory EOB delivery requirement',
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: ACA and ERISA require EOBs to disclose the network under which a claim was processed. The EOB must reference the specific provider network for member-facing network status disclosure and regulatory co',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: ACA-mandated EOBs must identify the rendering provider for member-facing communications. EOB generation and regulatory compliance require a direct FK to provider; provider_name and provider_npi are de',
    `subscriber_id` BIGINT COMMENT 'Identifier of the primary subscriber (often the employee or policyholder).',
    `tier_id` BIGINT COMMENT 'Foreign key linking to network.tier. Business justification: EOBs must disclose tier-based cost-sharing to members per SBC/ACA requirements. The tier determines member responsibility amounts shown on the EOB. A domain expert would expect the EOB to reference th',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to billing.account. Business justification: Overpayment recovery and retroactive claim adjustments are posted to billing accounts for GL reconciliation. Finance and compliance teams track which account absorbs each adjustment (e.g., employer gr',
    `adjudication_id` BIGINT COMMENT 'Foreign key linking to claim.adjudication. Business justification: Adjustments are post-adjudication corrections, recoveries, and audit findings. Every adjustment modifies or reverses a prior adjudication decision. Adding adjustment.adjudication_id -> claim.adjudicat',
    ...
    `transaction_id` BIGINT COMMENT 'Foreign key linking to enrollment.transaction. Business justification: Retroactive enrollment transactions (terminations, reinstatements, plan changes) directly trigger claim adjustments and reprocessing. Linking enrollment.adjustment to the enrollment transaction that caused',
    `health_plan_id` BIGINT COMMENT 'Foreign key linking to plan.health_plan. Business justification: Adjustment has compliance_60_day_rule field — CMS 60-day overpayment rule requires plan-level regulatory reporting of identified overpayments. Direct health_plan FK on adjustment enables plan-level ov',
    ...
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: Claim adjustments triggered by retroactive PA denial or approval changes must reference the originating PA request. Required for overpayment recovery audit trails, CMS 60-day repayment rule compliance',
    `par_agreement_id` BIGINT COMMENT 'Foreign key linking to network.par_agreement. Business justification: Claim adjustments (overpayments, recoveries, contract corrections) are frequently triggered by PAR agreement compliance audits or fee schedule corrections. Linking adjustments to the governing PAR agr',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`denial` (
    ...
    `adjudication_id` BIGINT COMMENT 'Foreign key linking to claim.adjudication. Business justification: A denial record is the direct outcome of an adjudication decision that resulted in non-payment. Linking denial.adjudication_id -> claim.adjudication.adjudication_id establishes the causal chain from a',
    `adverse_determination_id` BIGINT COMMENT 'Foreign key linking to appeal.adverse_determination. Business justification: Operational denials that escalate to formal adverse determinations require a direct link for regulatory reporting (NCQA UM standards, ACA §2719). Claims staff must identify which denials have been ele',
    `benefit_id` BIGINT COMMENT 'Foreign key linking to plan.benefit. Business justification: Denials are frequently benefit-specific — benefit limit exceeded, benefit exclusion, or non-covered service. Linking denial to the specific benefit enables benefit-level denial pattern analysis, NCQA/',
    `header_id` BIGINT COMMENT 'Unique identifier of the claim associated with this denial.',
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: Clinical denials must reference the specific clinical criteria not met. Direct FK enables denial reason documentation with criteria version tracking, supports member appeal letters citing specific cri',
    ...
    `pa_decision_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_decision. Business justification: When a claim is denied due to a PA denial, the denial record must reference the specific PA decision that drove it. This supports member appeals workflows, ERISA/state external review processes, and d',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: Denials are frequently issued because PA was not obtained or was denied. Linking denial to pa_request enables denial root-cause analysis, appeals processing workflows, and regulatory reporting on PA-r',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: Denial letters cite specific policy exclusions and benefit limitations. CMS and state regulatory denial rate reporting requires denial records to be attributed to the policy under which coverage was d',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: Provider-level denial rate tracking is a core operational metric driving network management and credentialing decisions. Regulatory reporting (state DOI, CMS) requires denial attribution by provider. ',
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: Out-of-network denials are a primary denial category. Regulatory reporting on denial rates by network, network adequacy gap analysis, and member appeal rights disclosures require linking denials to th',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`cob` (
    ...
    `adjudication_id` BIGINT COMMENT 'Foreign key linking to claim.adjudication. Business justification: COB (Coordination of Benefits) processing is a component of the adjudication workflow — the adjudication record already captures cob_adjusted_amount and cob_processing_result, indicating COB data flow',
    ...
    `eligibility_span_id` BIGINT COMMENT 'FK to member.member_eligibility_span.member_eligibility_span_id',
    `health_plan_id` BIGINT COMMENT 'Foreign key linking to plan.health_plan. Business justification: COB processing requires identifying which health plan is primary or secondary. COB records track msp_type, process_order, and primary_payer_paid_amount — all plan-specific. Direct plan FK enables MSP ',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: COB processing determines coordination order between primary and secondary payers by comparing the members active policies. Linking cob directly to the members policy enables liability calculation, ',
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: COB processing requires knowing the network under which each payer processed the claim to determine primary/secondary liability and crossover claim handling. Network-level COB reporting and MSP compli',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`payment` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to billing.account. Business justification: Financial reconciliation: claim payments (provider reimbursements) must be posted to the employer group or member billing account for GL reconciliation and cash flow reporting. Finance teams reconcile',
    `adjudication_id` BIGINT COMMENT 'Foreign key linking to claim.adjudication. Business justification: Payment is the financial execution of an adjudication decision. The payment product currently links to eob and claim header, but has no direct FK to the adjudication record that authorized the payment',
    `eob_id` BIGINT COMMENT 'add column eob_id (BIGINT) with FK to claim.eob.eob_id - payments are documented via EOB',
    `health_plan_id` BIGINT COMMENT 'Foreign key linking to plan.health_plan. Business justification: Claim payments must be attributed to a health plan for GL posting, financial reconciliation, and MLR ratio reporting. Payment is 3+ hops from health_plan via eob/adjudication; direct FK is required fo',
    `group_practice_id` BIGINT COMMENT 'Foreign key linking to provider.group_practice. Business justification: Capitation and bundled payments are made to group practices as a distinct payee entity. Value-based care financial reconciliation and group-level 1099 reporting require linking payments directly to gr',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: Provider payments require a direct FK to provider for 1099 tax reporting, overpayment recovery, and EFT remittance reconciliation. payee_type distinguishes individual vs group payees; role-prefix paye',
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `header_id` SET TAGS ('dbx_vibe_coding_demo' = 'scoped');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `benefit_package_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Package Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `group_practice_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Group Practice Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `eligibility_verification_id` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Verification Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `grace_period_id` SET TAGS ('dbx_business_glossary_term' = 'Grace Period Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `par_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Par Agreement Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `plan_election_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Election Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Claiming Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Rendering Facility Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_fhir_element' = 'identifier');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Line Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `revenue_code` SET TAGS ('dbx_value_regex' = '^[0-9]{4}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `clinical_criteria_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Policy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Surgeon Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_governance' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_subdomain' = 'processing_decisions');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `cost_share_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Share Rule Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `eligibility_span_id` SET TAGS ('dbx_business_glossary_term' = 'Member Eligibility Span Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Line Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `par_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Par Agreement Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `provider_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `provider_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `allowed_amount_method` SET TAGS ('dbx_value_regex' = 'fee_schedule|mac|rbp|capitation|custom');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_governance' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_subdomain' = 'processing_decisions');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `adverse_determination_id` SET TAGS ('dbx_business_glossary_term' = 'Adverse Determination Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Pa Request Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Rendering Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_fhir_element' = 'identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Transaction Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `par_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Par Agreement Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `overpayment_type` SET TAGS ('dbx_value_regex' = 'duplicate|incorrect_rate|ineligible_member|billing_error|fraud');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_subdomain' = 'processing_decisions');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `adverse_determination_id` SET TAGS ('dbx_business_glossary_term' = 'Adverse Determination Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `clinical_criteria_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` SET TAGS ('dbx_subdomain' = 'processing_decisions');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `group_practice_id` SET TAGS ('dbx_business_glossary_term' = 'Payee Group Practice Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_last_updated` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_profile_url` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_resource_identifier` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_resource_type` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_business_glossary_term' = 'FHIR Interoperability');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `fhir_version_code` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `master_entity_identifier` SET TAGS ('dbx_business_glossary_term' = 'Master Entity Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `method` SET TAGS ('dbx_value_regex' = 'eft|check|virtual_card|capitation_offset');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `payment_status` SET TAGS ('dbx_value_regex' = 'issued|cleared|voided|returned|stopped');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `payment_type` SET TAGS ('dbx_value_regex' = 'claim_payment|capitation|rebate|adjustment');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_created_at` SET TAGS ('dbx_business_glossary_term' = 'Record Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_status` SET TAGS ('dbx_governance' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Record Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `record_version` SET TAGS ('dbx_business_glossary_term' = 'Record Version');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `source` SET TAGS ('dbx_value_regex' = 'internal|external');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `trace_number` SET TAGS ('dbx_pii_phi' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `trace_number` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_fhir_element' = 'meta.lastUpdated');
