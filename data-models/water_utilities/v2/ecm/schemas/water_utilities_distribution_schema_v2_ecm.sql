...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` (
    ...
    `fixed_asset_id` BIGINT COMMENT 'Link to fixed asset',
    `vendor_id` BIGINT COMMENT 'Vendor who installed the pipe',
    `material_master_id` BIGINT COMMENT 'Link to material master',
    `network_node_id` BIGINT COMMENT 'Upstream network node',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` (
    ...
    `cip_project_id` BIGINT COMMENT 'Link to CIP project',
    `compliance_permit_id` BIGINT COMMENT 'Link to compliance permit',
    `customer_account_id` BIGINT COMMENT 'Link to customer account',
    `fixed_asset_id` BIGINT COMMENT 'Link to fixed asset',
    `vendor_id` BIGINT COMMENT 'Vendor who installed the service line',
    `material_master_id` BIGINT COMMENT 'Link to material master',
    `metering_meter_id` BIGINT COMMENT 'Link to meter',
    `pipe_main_id` BIGINT COMMENT 'Link to pipe main',
    `premise_id` BIGINT COMMENT 'Link to premise',
    `quality_sampling_point_id` BIGINT COMMENT 'Link to sampling point',
    ...
    `pressure_zone_code` STRING COMMENT 'Pressure zone code',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`dma` (
    ...
    `dma_responsible_operator_employee_id` BIGINT COMMENT 'Reference to the workforce employee or team responsible for monitoring and maintaining the DMA. Used for accountability and work assignment.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`prv_station` (
    ...
    `pressure_zone_id` BIGINT COMMENT 'Reference to the upstream hydraulic pressure zone feeding into the PRV station.',
    ...
    `bypass_configuration` DECIMAL(18,2) COMMENT 'Type of bypass arrangement installed at the PRV station for maintenance and emergency operations.',
    `calibration_frequency_months` DECIMAL(18,2) COMMENT 'Standard interval in months between required calibration activities for the PRV station.',
    ...
    `operational_status` DECIMAL(18,2) COMMENT 'Current operational state of the PRV station in the distribution network lifecycle.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`storage_tank` (
    ...
    `storage_dma_id` BIGINT COMMENT 'Reference to the District Metered Area (DMA) that this storage tank serves. DMAs are discrete zones used for water balance analysis and Non-Revenue Water (NRW) management.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` (
    ...
    `network_node_id` BIGINT COMMENT 'Hydraulic model node identifier for the reading location',
    `pipe_main_id` BIGINT COMMENT 'FK to the pipe main where reading was taken',
    ...
    `alarm_status` STRING COMMENT 'Alarm status if triggered',
    `alarm_threshold_value` DECIMAL(18,2) COMMENT 'Threshold value that triggers alarm',
    `alarm_triggered` BOOLEAN COMMENT 'Whether alarm was triggered',
    `alarm_triggered_flag` BOOLEAN COMMENT 'Whether the reading triggered an alarm',
    `alarm_type` STRING COMMENT 'Type of alarm if triggered',
    `average_value` DECIMAL(18,2) COMMENT 'Average value during interval',
    `avg_value` DECIMAL(18,2) COMMENT 'Average value in period',
    `battery_level_pct` DECIMAL(18,2) COMMENT 'Battery level percentage for remote sensors.',
    `chlorine_residual_mg_l` DECIMAL(18,2) COMMENT 'Chlorine residual mg/L.',
    ...
    `distance_from_plant_miles` DECIMAL(18,2) COMMENT 'Distance from the nearest treatment plant in miles.',
    `elevation_ft` DECIMAL(18,2) COMMENT 'Elevation in feet',
    `engineering_unit` STRING COMMENT '',
    `exceedance_flag` BOOLEAN COMMENT 'Whether the reading exceeds regulatory or operational limits',
    `exceeds_regulatory_limit` BOOLEAN COMMENT 'Whether reading exceeds regulatory limit',
    `flow_rate_gpm` DECIMAL(18,2) COMMENT 'Measured flow rate in gpm.',
    `flow_value` DECIMAL(18,2) COMMENT '',
    `gis_latitude` DECIMAL(18,2) COMMENT 'Latitude of the reading location',
    `gis_longitude` DECIMAL(18,2) COMMENT 'Longitude of the reading location',
    `high_alarm_limit` DECIMAL(18,2) COMMENT 'Upper alarm threshold for the parameter',
    `high_limit` DOUBLE COMMENT 'Upper alarm limit for this reading type',
    `high_threshold` DOUBLE COMMENT 'High alarm threshold',
    `ingestion_timestamp` TIMESTAMP COMMENT 'Data ingestion timestamp',
    `interval_duration_seconds` STRING COMMENT 'Duration of measurement interval in seconds',
    `is_alarm` BOOLEAN COMMENT 'Whether reading triggered an alarm.',
    `is_alarm_condition` BOOLEAN COMMENT 'Whether in alarm condition',
    `is_manual_entry` BOOLEAN COMMENT '',
    `is_regulatory_exceedance` BOOLEAN COMMENT 'Whether the reading exceeds the regulatory limit',
    `is_regulatory_sample_point` BOOLEAN COMMENT 'Whether this is a regulatory compliance sampling point.',
    `is_validated` BOOLEAN COMMENT '',
    `last_calibration_date` DATE COMMENT 'Date of last calibration',
    `latitude` DECIMAL(18,2) COMMENT 'Latitude of reading location',
    `location_description` STRING COMMENT 'Description of the reading location',
    `longitude` DECIMAL(18,2) COMMENT 'Longitude of reading location',
    `low_alarm_limit` DECIMAL(18,2) COMMENT 'Lower alarm threshold for the parameter',
    `low_limit` DOUBLE COMMENT 'Lower alarm limit for this reading type',
    `low_threshold` DOUBLE COMMENT 'Low alarm threshold',
    `max_value` DECIMAL(18,2) COMMENT 'Maximum value in period',
    `measured_value` DECIMAL(18,2) COMMENT 'Measured value of the reading',
    `measurement_timestamp` TIMESTAMP COMMENT 'measurement timestamp for network_reading',
    `min_value` DECIMAL(18,2) COMMENT 'Minimum value in period',
    `notes` STRING COMMENT 'Reading notes',
    `parameter_code` STRING COMMENT 'Standardized code for the measured parameter',
    `parameter_measured` STRING COMMENT 'Specific parameter measured (e.g., residual_chlorine, pressure_psi, flow_gpm)',
    `parameter_name` STRING COMMENT 'Name of parameter measured',
    `parameter_type` STRING COMMENT '',
    `pipe_diameter_inches` DECIMAL(18,2) COMMENT 'Diameter of the pipe at the reading location in inches.',
    `pipe_material` STRING COMMENT 'Material of the pipe at the reading location.',
    `pressure_psi` DECIMAL(18,2) COMMENT 'Measured pressure in psi.',
    `quality_code` STRING COMMENT 'Data quality code',
    `raw_value` DECIMAL(18,2) COMMENT 'Raw sensor value before scaling',
    `reading_date` DATE COMMENT '',
    `reading_interval_seconds` STRING COMMENT 'Duration of the reading interval in seconds',
    `reading_source` STRING COMMENT 'Source of reading (SCADA, manual, logger)',
    `reading_status` STRING COMMENT '',
    `reading_timestamp` TIMESTAMP COMMENT '',
    `reading_type` STRING COMMENT 'Type of reading (pressure, flow, chlorine residual, turbidity)',
    ...
    `regulatory_limit_value` DECIMAL(18,2) COMMENT 'Applicable regulatory limit for the parameter',
    `sample_interval_seconds` STRING COMMENT 'Time interval between readings in seconds.',
    `scada_tag` STRING COMMENT 'SCADA tag identifier',
    `scada_tag_name` STRING COMMENT 'SCADA tag name',
    `scaling_factor` DOUBLE COMMENT 'Scaling factor applied',
    `sensor_code` STRING COMMENT 'Identifier of the sensor that produced the reading',
    `sensor_location` STRING COMMENT 'Physical sensor location',
    `sensor_serial_number` STRING COMMENT 'Sensor serial number',
    `sensor_type` STRING COMMENT 'Type of sensor',
    `signal_strength_dbm` DECIMAL(18,2) COMMENT 'Communication signal strength in dBm.',
    `temperature_c` DECIMAL(18,2) COMMENT 'Water temperature Celsius.',
    `turbidity_ntu` DECIMAL(18,2) COMMENT 'Turbidity in NTU.',
    ...
    `validated_by` STRING COMMENT 'Person who validated the reading',
    `validated_flag` BOOLEAN COMMENT 'Whether reading has been validated',
    `validated_timestamp` TIMESTAMP COMMENT 'Timestamp of validation',
    `validation_method` STRING COMMENT 'Method used to validate the reading',
    `validation_status` STRING COMMENT 'Validation status',
    `validation_timestamp` TIMESTAMP COMMENT 'Timestamp of validation',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` (
    `distribution_nrw_water_balance_id` DECIMAL(18,2) COMMENT 'Unique identifier for the water balance record. Primary key for the NRW water balance data product.',
    ...
    `metering_nrw_water_balance_id` BIGINT COMMENT 'Cross-reference to metering NRW water balance',
    ...
    `balance_period_end` DECIMAL(18,2) COMMENT 'End of balance period',
    `balance_period_start` DECIMAL(18,2) COMMENT 'Start of balance period',
    ...
    `calculation_method` STRING COMMENT 'IWA/AWWA water balance method used',
    ...
    `confidence_grade` STRING COMMENT 'Confidence grade of the balance',
    ...
    `non_revenue_water_pct` DECIMAL(18,2) COMMENT 'NRW as percentage of system input',
    ...
    `number_of_connections` STRING COMMENT 'Number of service connections',
    `pipe_length_miles` DECIMAL(18,2) COMMENT 'Total pipe length in miles',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`leak_detection_survey` (
    ...
    `leak_vendor_id` BIGINT COMMENT 'Reference to the external contractor or vendor that performed the leak detection survey, if outsourced.',
    ...
    `repair_work_order_generated` DECIMAL(18,2) COMMENT 'Indicates whether a repair work order was automatically or manually generated as a result of leaks found during this survey.',
    ...
    `survey_cost_currency` DECIMAL(18,2) COMMENT 'Currency code for the survey cost amount. Defaults to USD for U.S. water utilities.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`main_break` (
    ...
    `quality_public_notification_id` BIGINT COMMENT 'FK to public notification issued for this event',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`valve_exercise` (
    ...
    `valve_technician_employee_id` BIGINT COMMENT 'Reference to the field technician or crew member who performed the valve exercise.',
    ...
    `duration_minutes` DECIMAL(18,2) COMMENT 'Total time in minutes required to complete the valve exercise activity, used for labor planning and efficiency analysis.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`hydrant_flow_test` (
    ...
    `flushing_duration_minutes` DECIMAL(18,2) COMMENT 'Duration in minutes that the hydrant was flushed before or during the test to clear sediment and ensure representative flow measurements.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`flushing_event` (
    ...
    `discharge_point_type` DECIMAL(18,2) COMMENT 'The type of infrastructure asset used as the discharge point for flushed water.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`hydraulic_model_run` (
    ...
    `calibration_status` DECIMAL(18,2) COMMENT 'Indicates the level of calibration and validation applied to the hydraulic model for this run: not calibrated (theoretical), preliminary (initial tuning), calibrated (field-verified), or validated (peer-reviewed and accepted).',
    ...
    `convergence_iterations` DECIMAL(18,2) COMMENT 'Number of solver iterations required to achieve hydraulic convergence, used to assess model complexity and computational efficiency.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`network_isolation_event` (
    ...
    `network_vendor_id` BIGINT COMMENT 'Reference to the external contractor performing the work, if applicable.',
    `pressure_zone_id` BIGINT COMMENT 'Reference to the primary pressure zone affected by this isolation event.',
    `quality_public_notification_id` BIGINT COMMENT 'FK to public notification',
    ...
    `affected_zones` STRING COMMENT 'Comma-separated list of affected zones',
    ...
    `population_at_risk` BIGINT COMMENT 'Estimated population affected by the isolation',
    ...
    `restoration_confirmed` DECIMAL(18,2) COMMENT 'Indicates whether full service restoration has been confirmed and all valves returned to normal operating position.',
    `restoration_confirmed_by` DECIMAL(18,2) COMMENT 'Reference to the employee who confirmed successful restoration of service.',
    `restoration_confirmed_datetime` DECIMAL(18,2) COMMENT 'Date and time when service restoration was confirmed and documented.',
    ...
    `valves_operated` DECIMAL(18,2) COMMENT 'Comma-separated list of valve identifiers that were operated (closed or opened) to achieve isolation.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_condition_assessment` (
    ...
    `condition_assessment_id` BIGINT COMMENT '',
    ...
    `pipe_technician_employee_id` BIGINT COMMENT 'Reference to the lead technician or inspector who conducted the assessment.',
    `pipe_vendor_id` BIGINT COMMENT 'Reference to the contractor or vendor who performed the assessment, if outsourced.',
    ...
    `corrosion_level` STRING COMMENT '',
    ...
    `structural_condition_score` DECIMAL(18,2) COMMENT '',
    `technician_name` STRING COMMENT 'Name of the lead technician or inspector, denormalized for reporting.',
    `wall_thickness_loss_pct` DECIMAL(18,2) COMMENT '',
    `wall_thickness_mm` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`dma_crew_coverage` (
    ...
    `is_primary_crew` BOOLEAN COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_procurement` (
    ...
    `ordered_length_feet` DECIMAL(18,2) COMMENT '',
    ...
    `quantity_feet` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`zone_operator_assignment` (
    ...
    `end_date` DATE COMMENT '',
    `is_primary` BOOLEAN COMMENT '',
    `is_primary_operator` BOOLEAN COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`maintenance_zone` (
    ...
    `pressure_zone_id` BIGINT COMMENT '',
    ...
    `boundary_description` STRING COMMENT '',
    ...
    `gis_polygon_boundary` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`nrw_program` (
    ...
    `dma_id` BIGINT COMMENT '',
    `finance_budget_id` BIGINT COMMENT 'FK to finance budget',
    ...
    `program_manager` STRING COMMENT '',
    ...
    `target_nrw_percentage` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`distribution`.`network_node` (
    ...
    `network_upstream_node_id` BIGINT COMMENT 'Identifier of the immediate upstream node in the hydraulic flow direction.',
    `primary_pressure_zone_id` BIGINT COMMENT 'Identifier of the broader network zone containing the node.',
    `primary_upstream_network_node_id` BIGINT COMMENT 'Self-referencing FK on network_node (upstream_network_node_id)',
    ...
    `elevation_ft` DECIMAL(18,2) COMMENT '',
    ...
    `gis_feature_code` STRING COMMENT '',
    `hydraulic_model_node_code` STRING COMMENT '',
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`maintenance_zone` ADD CONSTRAINT `fk_distribution_maintenance_zone_pressure_zone_id` FOREIGN KEY (`pressure_zone_id`) REFERENCES `vibe_water_utilities_v1`.`distribution`.`pressure_zone`(`pressure_zone_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`nrw_program` ADD CONSTRAINT `fk_distribution_nrw_program_dma_id` FOREIGN KEY (`dma_id`) REFERENCES `vibe_water_utilities_v1`.`distribution`.`dma`(`dma_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` SET TAGS ('dbx_water_distribution' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Installation Vendor');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_main` ALTER COLUMN `network_node_id` SET TAGS ('dbx_business_glossary_term' = 'Upstream Node');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` SET TAGS ('dbx_service_connection' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'CIP Project');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Installation Vendor');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `metering_meter_id` SET TAGS ('dbx_business_glossary_term' = 'Meter');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `quality_sampling_point_id` SET TAGS ('dbx_business_glossary_term' = 'Sampling Point');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `dma_code` SET TAGS ('dbx_business_glossary_term' = 'DMA Code');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`service_line` ALTER COLUMN `pressure_zone_code` SET TAGS ('dbx_business_glossary_term' = 'Pressure Zone Code');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pressure_zone` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_valve` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`prv_station` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`prv_station` ALTER COLUMN `address` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`prv_station` ALTER COLUMN `address` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`prv_station` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pump_station` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pump_station` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`storage_tank` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` SET TAGS ('dbx_structure_required' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` SET TAGS ('dbx_tier' = 'MVM');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `network_reading_id` SET TAGS ('dbx_reconciled_depth' = '91');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `network_reading_id` SET TAGS ('dbx_depth_tier' = 'canonical');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `network_node_id` SET TAGS ('dbx_business_glossary_term' = 'Network Node ID');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `alarm_threshold_value` SET TAGS ('dbx_business_glossary_term' = 'Alarm Threshold');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `alarm_triggered_flag` SET TAGS ('dbx_business_glossary_term' = 'Alarm Triggered');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `battery_level_pct` SET TAGS ('dbx_business_glossary_term' = 'Battery Level');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `distance_from_plant_miles` SET TAGS ('dbx_business_glossary_term' = 'Distance from Plant');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `exceedance_flag` SET TAGS ('dbx_business_glossary_term' = 'Exceedance Flag');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_latitude` SET TAGS ('dbx_business_glossary_term' = 'Latitude');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_longitude` SET TAGS ('dbx_business_glossary_term' = 'Longitude');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `gis_longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `high_limit` SET TAGS ('dbx_business_glossary_term' = 'High Limit');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `is_regulatory_exceedance` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Exceedance');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `is_regulatory_sample_point` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Sample Point');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `location_description` SET TAGS ('dbx_business_glossary_term' = 'Location Description');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `low_limit` SET TAGS ('dbx_business_glossary_term' = 'Low Limit');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `measured_value` SET TAGS ('dbx_business_glossary_term' = 'Measured Value');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `parameter_measured` SET TAGS ('dbx_business_glossary_term' = 'Parameter Measured');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `pipe_diameter_inches` SET TAGS ('dbx_business_glossary_term' = 'Pipe Diameter');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `pipe_material` SET TAGS ('dbx_business_glossary_term' = 'Pipe Material');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `reading_interval_seconds` SET TAGS ('dbx_business_glossary_term' = 'Reading Interval');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `reading_type` SET TAGS ('dbx_business_glossary_term' = 'Reading Type');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `regulatory_limit_value` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Limit');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `sample_interval_seconds` SET TAGS ('dbx_business_glossary_term' = 'Sample Interval');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `sensor_code` SET TAGS ('dbx_business_glossary_term' = 'Sensor ID');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `signal_strength_dbm` SET TAGS ('dbx_business_glossary_term' = 'Signal Strength');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_reading` ALTER COLUMN `validation_method` SET TAGS ('dbx_business_glossary_term' = 'Validation Method');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_ssot_master' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_ssot_role' = 'canonical');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_ssot_group' = 'distribution.distribution_nrw_water_balance');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_ssot_pair' = 'distribution.distribution_nrw_water_balance');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` SET TAGS ('dbx_metering_metering_nrw_water_balance' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`distribution_nrw_water_balance` ALTER COLUMN `metering_nrw_water_balance_id` SET TAGS ('dbx_ssot_xref' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`leak_detection_survey` ALTER COLUMN `grant_expenditure_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Expenditure Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`main_break` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`valve_exercise` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`valve_exercise` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`hydrant_flow_test` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`flushing_event` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`flushing_event` ALTER COLUMN `cost_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Allocation Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_isolation_event` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_isolation_event` SET TAGS ('dbx_structural_integrity_verified' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_isolation_event` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_condition_assessment` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_condition_assessment` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_condition_assessment` ALTER COLUMN `contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_condition_assessment` ALTER COLUMN `contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`dma_crew_coverage` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`dma_crew_coverage` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`dma_crew_coverage` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_procurement` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`pipe_procurement` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`zone_operator_assignment` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`zone_operator_assignment` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`maintenance_zone` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`maintenance_zone` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`maintenance_zone` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`nrw_program` SET TAGS ('dbx_subdomain' = 'maintenance_activities');
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`nrw_program` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`nrw_program` ALTER COLUMN `predecessor_nrw_program_id` SET TAGS ('dbx_self_ref_fk' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_node` SET TAGS ('dbx_structure_required' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`distribution`.`network_node` ALTER COLUMN `primary_upstream_network_node_id` SET TAGS ('dbx_self_ref_fk' = 'true');
