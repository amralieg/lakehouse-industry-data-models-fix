-- Schema for Domain: utilization | Business: Health_Insurance | Version: v3_mvm
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` (
    ...
    `coordinator_id` BIGINT COMMENT 'Foreign key linking to care.coordinator. Business justification: Care coordinators submit PA requests for members in care management programs. Essential workflow for tracking coordinator-initiated authorization requests and measuring program effectiveness.',
    `medical_policy_id` BIGINT COMMENT 'Foreign key linking to utilization.medical_policy. Business justification: PA requests are evaluated against specific medical policies (e.g., is prior auth required per policy, what coverage limits apply). pa_request currently has clinical_criteria_id but no direct link to t',
    ...
    `network_service_area_id` BIGINT COMMENT 'Foreign key linking to network.service_area. Business justification: PA requests must validate requested services are within members covered service area for authorization determination. PA reviewers check service area eligibility when members request services from ou',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` (
    ...
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: A PA decision is made by applying specific clinical criteria (InterQual/MCG). pa_decision currently stores clinical_criteria_set as a free-text string and clinical_criteria_version as a snapshot. Addi',
    `coordinator_id` BIGINT COMMENT 'Foreign key linking to care.coordinator. Business justification: Coordinators track PA decision outcomes to adjust care plans and escalate denials. Critical for care management reporting and member advocacy workflows.',
    ...
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: PA decision approval logic requires member risk score for medical necessity determination, payment accuracy calculation, and CMS risk adjustment reconciliation. Critical for authorization workflow and',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Authorized service lines specify the facility where care is rendered. linking to provider.facility enables network tier validation, accreditation checks, and facility-level cost reporting for autho',
    `clinical_criteria_id` BIGINT COMMENT 'Identifier of the specific clinical rule set (InterQual/MCG) applied to this line.',
    `coordinator_id` BIGINT COMMENT 'Foreign key linking to care.coordinator. Business justification: Coordinators monitor authorized service lines for care plan compliance, utilization tracking, and identifying over/under-utilization patterns in managed populations.',
    ...
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: Service line authorization pricing and approval thresholds vary by member risk tier and HCC status. Essential for accurate authorized_price calculation and risk-adjusted utilization management decisio',
    `pa_decision_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_decision. Business justification: auth_service_line is the line-item detail for services authorized within a PA request. It already references pa_request_id (the request header), but the authorization outcome (approval, denial, partia',
    ...
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: Each authorized service line must track rendering providers network participation for accurate claims adjudication and member cost-sharing calculation. Claims systems use service line network assignm',
    `reimbursement_policy_id` BIGINT COMMENT 'Foreign key linking to contract.reimbursement_policy. Business justification: Each authorized service line must comply with applicable reimbursement policies for NCCI edits, bundling rules, modifier logic, and payment methodology. Required for accurate claims adjudication and p',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` (
    ...
    `provider_contract_id` BIGINT COMMENT 'Foreign key linking to contract.provider_contract. Business justification: Concurrent reviews validate inpatient services against provider contract terms including authorization requirements, LOS targets, payment methodology, and discharge planning obligations. Essential for',
    ...
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: Concurrent review LOS benchmarking and discharge planning must account for member risk profile and HCC burden. Risk-adjusted readmission_risk_score calculation and care coordination resource allocatio',
    `subscriber_id` BIGINT COMMENT 'Unique identifier of the member whose stay is under review.',
    `network_service_area_id` BIGINT COMMENT 'Foreign key linking to network.service_area. Business justification: Concurrent reviews must validate admitting facility is within members covered service area for out-of-area authorization decisions. UM coordinators check service area boundaries when members are admi',
    `pa_request_id` BIGINT COMMENT 'Identifier of the originating prior‑authorization request.',
    `plan_election_id` BIGINT COMMENT 'Foreign key linking to enrollment.plan_election. Business justification: Concurrent reviews require direct access to members current plan election for real-time benefit verification, authorization limit checks, and cost-sharing calculations during ongoing inpatient stays.',
    `care_plan_id` BIGINT COMMENT 'Foreign key linking to care.care_plan. Business justification: Inpatient concurrent reviews directly inform care plan updates, discharge planning, and post-acute service coordination. Standard clinical workflow for care management programs.',
    ...
    `provider_network_id` BIGINT COMMENT 'Foreign key linking to network.provider_network. Business justification: Concurrent reviews require network validation for authorization and payment determination. UM nurses verify network participation status during inpatient stay reviews to determine coverage levels and ',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` (
    ...
    `care_enrollment_id` BIGINT COMMENT 'Foreign key linking to care.care_enrollment. Business justification: UM cases for enrolled members require tracking enrollment status for program-specific protocols, acuity-based review criteria, and care management reporting.',
    ...
    `inpatient_admission_id` BIGINT COMMENT 'Foreign key linking to utilization.inpatient_admission. Business justification: For inpatient UM cases, the um_case (overarching workflow container) should directly reference the inpatient_admission it manages. While concurrent_review bridges um_case and inpatient_admission, a di',
    `medical_policy_id` BIGINT COMMENT 'Foreign key linking to utilization.medical_policy. Business justification: A UM case is the overarching workflow container for utilization management, and its clinical determinations are governed by medical policies. um_case currently stores clinical_criteria_set and clinica',
    `identity_id` BIGINT COMMENT 'Unique identifier of the member associated with the case.',
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: UM case prioritization, severity stratification, and care management resource allocation driven by member risk score. Case_priority and case_severity assignment require risk-adjusted clinical context ',
    `network_service_area_id` BIGINT COMMENT 'Foreign key linking to network.service_area. Business justification: UM cases track geographic service area to determine network adequacy compliance and out-of-area coverage rules. Case managers verify service area coverage when coordinating care across regions, especi',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` (
    ...
    `coordinator_id` BIGINT COMMENT 'Foreign key linking to care.coordinator. Business justification: Coordinators manage discharge planning, post-acute transitions, and readmission prevention for assigned members. Standard care management responsibility for inpatient episodes.',
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Inpatient admissions occur at a specific facility. linking to provider.facility enables DRG benchmarking by facility type, trauma level, and accreditation status — core UM inpatient review and netw',
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: Inpatient admission authorization and DRG payment reconciliation requires member risk score for accurate expected_cost_amount calculation and risk-adjusted LOS benchmarking. Essential for payment accu',
    `pa_request_id` BIGINT COMMENT 'Foreign key linking to utilization.pa_request. Business justification: An inpatient admission subject to UM review is typically tied to a prior authorization request. inpatient_admission currently stores authorization_number and payer_authorization_status as free-text st',
    `plan_election_id` BIGINT COMMENT 'Foreign key linking to enrollment.plan_election. Business justification: Admissions require immediate plan benefit verification and authorization status at point of service. Direct link supports real-time eligibility checks and cost-sharing calculations for emergency/urgen',
    `care_plan_id` BIGINT COMMENT 'Foreign key linking to care.care_plan. Business justification: Admissions trigger mandatory care plan reviews and updates, especially for chronic condition programs. Required for discharge planning and readmission prevention workflows.',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: Admission eligibility checks need the members enrollment eligibility span to confirm benefit coverage dates, essential for clinical validation and regulatory audit of inpatient services.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` (
    ...
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: Medical policies are backed by specific clinical criteria sets (InterQual, MCG). medical_policy currently stores clinical_criteria_source (e.g., InterQual, MCG) and clinical_criteria_version as st',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` (
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Clinical criteria must reference regulatory obligations (CMS NCDs/LCDs, state mandates) that legally require specific coverage rules. Accreditation and regulatory audits verify criteria align with app',
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ADD CONSTRAINT `fk_utilization_pa_request_medical_policy_id` FOREIGN KEY (`medical_policy_id`) REFERENCES `vibe_health_insurance_v1`.`utilization`.`medical_policy`(`medical_policy_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ADD CONSTRAINT `fk_utilization_um_case_medical_policy_id` FOREIGN KEY (`medical_policy_id`) REFERENCES `vibe_health_insurance_v1`.`utilization`.`medical_policy`(`medical_policy_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `coordinator_id` SET TAGS ('dbx_business_glossary_term' = 'Coordinator Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Policy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `coordinator_id` SET TAGS ('dbx_business_glossary_term' = 'Coordinator Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `coordinator_id` SET TAGS ('dbx_business_glossary_term' = 'Coordinator Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` SET TAGS ('dbx_subdomain' = 'inpatient_oversight');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Provider Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `discharge_destination` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_subdomain' = 'authorization_management');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `care_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Care Enrollment Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Policy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `medical_policy_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_subdomain' = 'inpatient_oversight');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `coordinator_id` SET TAGS ('dbx_business_glossary_term' = 'Coordinator Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Eligibility Span Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_subdomain' = 'clinical_standards');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_subdomain' = 'clinical_standards');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
