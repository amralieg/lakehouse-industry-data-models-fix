...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`order_confirmation` (
    ...
    `order_confirmation_code` STRING COMMENT 'Unique confirmation code for the order confirmation',
    ...
    `order_confirmation_description` STRING COMMENT 'Description of the order confirmation',
    ...
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when the order confirmation was last modified',
    `order_confirmation_name` STRING COMMENT 'Name or identifier for the order confirmation',
    ...
    `order_confirmation_status` STRING COMMENT 'Status of the order confirmation',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`production_schedule` (
    ...
    `planning_strategy` DECIMAL(18,2) COMMENT 'Manufacturing strategy governing how this schedule is planned and executed. Make-to-stock builds for inventory; make-to-order builds against customer orders; assemble-to-order configures from standard components; engineer-to-order designs and builds custom products.. Valid values are `make_to_stock|make_to_order|assemble_to_order|engineer_to_order`',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`routing` (
    ...
    `cost_currency_code` DECIMAL(18,2) COMMENT 'ISO 4217 three-letter currency code for the standard cost amount.. Valid values are `^[A-Z]{3}$`',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`version` (
    ...
    `version_code` STRING COMMENT 'Version code identifier for the production version',
    ...
    `is_costing_relevant` DECIMAL(18,2) COMMENT 'Flag indicating whether this production version is used for product costing calculations. True means this version is included in standard cost estimates.',
    ...
    `modified_timestamp` TIMESTAMP COMMENT '',
    `version_name` STRING COMMENT 'The name or identifier of the version',
    ...
    `version_status` STRING COMMENT 'Status of the version record',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`shift_report` (
    ...
    `shift_report_code` STRING COMMENT '',
    ...
    `shift_report_description` STRING COMMENT '',
    ...
    `modified_timestamp` TIMESTAMP COMMENT '',
    `shift_report_name` STRING COMMENT '',
    ...
    `shift_report_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` (
    ...
    `production_downtime_event_code` STRING COMMENT '',
    ...
    `production_downtime_event_description` STRING COMMENT '',
    ...
    `production_downtime_event_name` STRING COMMENT '',
    ...
    `production_downtime_event_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`resource_tool` (
    ...
    `calibration_status` DECIMAL(18,2) COMMENT 'Current calibration compliance status. Current = within calibration interval; Due = approaching calibration date; Overdue = past calibration date and blocked from use; Not Applicable = calibration not required for this tool type.. Valid values are `current|due|overdue|not_applicable`',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`production_line` (
    ...
    `operational_status` DECIMAL(18,2) COMMENT 'Current operational state of the production line indicating availability for production scheduling and capacity planning.. Valid values are `active|inactive|maintenance|standby|decommissioned`',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`work_order_allocation` (
    ...
    `allocated_by` STRING COMMENT 'User/system that performed the allocation',
    `allocated_by_name` STRING COMMENT 'Business attribute for work_order_allocation.',
    ...
    `allocation_reason_code` STRING COMMENT '',
    `allocation_sequence` STRING COMMENT '',
    ...
    `allocation_type` STRING COMMENT 'Type of resource/material being allocated',
    `allocation_uom` STRING COMMENT 'Business attribute for work_order_allocation.',
    `work_order_allocation_code` STRING COMMENT '',
    `confirmed_quantity` DECIMAL(18,2) COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `work_order_allocation_description` STRING COMMENT '',
    `is_partial_flag` BOOLEAN COMMENT 'Whether allocation is partial',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `modified_timestamp` TIMESTAMP COMMENT '',
    `work_order_allocation_name` STRING COMMENT '',
    `notes` STRING COMMENT '',
    `planned_completion_date` TIMESTAMP COMMENT '',
    `priority` BIGINT COMMENT 'Allocation priority rank',
    `priority_rank` STRING COMMENT '',
    `released_flag` BOOLEAN COMMENT 'Business attribute for work_order_allocation.',
    `released_quantity` DECIMAL(18,2) COMMENT '',
    `remaining_quantity` DECIMAL(18,2) COMMENT '',
    `work_order_allocation_status` STRING COMMENT '',
    `unit_of_measure` STRING COMMENT 'Unit of measure for allocated quantity',
    `allocated_by_user_id` BIGINT COMMENT 'User who performed the allocation',
    `allocation_notes` STRING COMMENT 'Additional notes about the allocation',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` (
    ...
    `work_center_group_code` STRING COMMENT '',
    ...
    `work_center_group_description` STRING COMMENT '',
    ...
    `modified_timestamp` TIMESTAMP COMMENT '',
    `work_center_group_name` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`production`.`calendar` (
    ...
    `calendar_description` STRING COMMENT '',
    ...
    `shift_indicator` BOOLEAN COMMENT 'Designated shift for the date in production planning (day, night, or swing).',
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ADD CONSTRAINT `fk_production_version_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `vibe_manufacturing_v1`.`production`.`plant`(`plant_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ADD CONSTRAINT `fk_production_work_center_group_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `vibe_manufacturing_v1`.`production`.`plant`(`plant_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_work_order` SET TAGS ('dbx_structure_preserved' = 'v2');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_work_order` SET TAGS ('dbx_ssot_group' = 'work_order');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_work_order` SET TAGS ('dbx_ssot_role' = 'master');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_work_order` SET TAGS ('dbx_ssot_status' = 'resolved');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_work_order` SET TAGS ('dbx_ssot_owner' = 'asset.asset_work_order');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`order_confirmation` SET TAGS ('dbx_subdomain' = 'shop_execution');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`order_confirmation` ALTER COLUMN `order_confirmation_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_schedule` SET TAGS ('dbx_subdomain' = 'resource_planning');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_schedule` ALTER COLUMN `planning_strategy` SET TAGS ('dbx_value_regex' = 'make_to_stock|make_to_order|assemble_to_order|engineer_to_order');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` SET TAGS ('dbx_subdomain' = 'resource_planning');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant ID');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ALTER COLUMN `version_production_scheduler_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Version Production Scheduler Employee Id');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ALTER COLUMN `version_production_scheduler_employee_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ALTER COLUMN `issue_storage_location` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`version` ALTER COLUMN `version_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`shift` SET TAGS ('dbx_subdomain' = 'performance_reporting');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`shift_report` SET TAGS ('dbx_subdomain' = 'performance_reporting');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`shift_report` ALTER COLUMN `shift_report_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` SET TAGS ('dbx_subdomain' = 'performance_reporting');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` SET TAGS ('dbx_ssot_group' = 'downtime_event');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` SET TAGS ('dbx_ssot_role' = 'consumer');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` SET TAGS ('dbx_ssot_status' = 'resolved');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` SET TAGS ('dbx_ssot_owner' = 'asset.asset_downtime_event');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` ALTER COLUMN `ecn_id` SET TAGS ('dbx_business_glossary_term' = 'Ecn Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_downtime_event` ALTER COLUMN `production_downtime_event_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`resource_tool` ALTER COLUMN `calibration_status` SET TAGS ('dbx_value_regex' = 'current|due|overdue|not_applicable');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`run` SET TAGS ('dbx_subdomain' = 'shop_execution');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`run` ALTER COLUMN `run_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_line` ALTER COLUMN `historian_config_id` SET TAGS ('dbx_business_glossary_term' = 'Historian Config Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_line` ALTER COLUMN `modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_line` ALTER COLUMN `modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`production_line` ALTER COLUMN `modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_order_allocation` SET TAGS ('dbx_subdomain' = 'shop_execution');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_order_allocation` ALTER COLUMN `work_order_allocation_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` SET TAGS ('dbx_subdomain' = 'resource_planning');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ALTER COLUMN `manager_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ALTER COLUMN `manager_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ALTER COLUMN `manager_phone` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`work_center_group` ALTER COLUMN `work_center_group_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_group' = 'plant');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_role' = 'consumer');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_master' = 'asset.plant');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot' = 'reference');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_canonical' = 'asset.plant');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_status' = 'resolved');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`plant` SET TAGS ('dbx_ssot_owner' = 'asset.plant');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`calendar` SET TAGS ('dbx_subdomain' = 'performance_reporting');
ALTER TABLE `vibe_manufacturing_v1`.`production`.`calendar` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`calendar` ALTER COLUMN `calendar_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`production`.`shift_sequence` SET TAGS ('dbx_subdomain' = 'performance_reporting');
