...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`test`.`ate_configuration` (
    ...
    `site_id` BIGINT COMMENT 'add column site_id (BIGINT) with FK to shared.site.site_id - ATE configurations are physically located at test floors and need site association; currently isolated with zero outbound FKs',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`test`.`insertion` (
    ...
    `process_step_id` BIGINT COMMENT 'add column process_step_id (BIGINT) with FK to process.process_step.process_step_id - test insertions occur at specific manufacturing process steps',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`test`.`adaptive_test_flow` (
    ...
    `ic_catalog_id` BIGINT COMMENT 'add column ic_catalog_id (BIGINT) with FK to product.ic_catalog.ic_catalog_id - adaptive test flows are product-specific',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`test`.`test_step` (
    ...
    `ate_configuration_id` BIGINT COMMENT 'add column ate_configuration_id (BIGINT) with FK to test.ate_configuration.ate_configuration_id - test steps execute on specific ATE configurations',
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`test_step` ADD CONSTRAINT `fk_test_test_step_ate_configuration_id` FOREIGN KEY (`ate_configuration_id`) REFERENCES `vibe_semiconductors_v1`.`test`.`ate_configuration`(`ate_configuration_id`);
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`test_program` ALTER COLUMN `test_program_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`correlation_study` ALTER COLUMN `study_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`insertion` ALTER COLUMN `insertion_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`adaptive_test_flow` ALTER COLUMN `flow_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`adaptive_test_flow` ALTER COLUMN `ml_model_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`test_case` ALTER COLUMN `test_case_name` SET TAGS ('dbx_pii_detected' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`test`.`test_step` ALTER COLUMN `step_name` SET TAGS ('dbx_pii_detected' = 'true');
