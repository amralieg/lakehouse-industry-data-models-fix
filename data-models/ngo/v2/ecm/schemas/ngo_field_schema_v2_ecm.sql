...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` (
    ...
    `supply_distribution_line_id` BIGINT COMMENT 'FK to SSOT owner supply.supply_distribution_line for entity distribution_line',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` (
    ...
    `mobile_project_site_id` BIGINT COMMENT 'Reference to the program project under which this mobile health outreach session was conducted.',
    ...
    `outreach_project_site_id` BIGINT COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`wash_intervention` (
    ...
    `wash_intervention_description` STRING COMMENT 'Detailed narrative description of the WASH intervention activity, including technical specifications, community engagement approach, and any special considerations or challenges encountered during implementation.',
    ...
    `wash_intervention_status` STRING COMMENT 'Current lifecycle status of the WASH intervention activity, tracking progress from planning through completion or closure.. Valid values are `planned|in_progress|completed|suspended|cancelled`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`assessment_response` (
    ...
    `registrant_id` BIGINT COMMENT 'Anonymized identifier for the survey respondent to enable longitudinal tracking while protecting personally identifiable information. Used for follow-up assessments and trend analysis.',
    `assessment_respondent_registrant_id` BIGINT COMMENT 'Foreign key linking to beneficiary.registrant. Business justification: PDM and assessment surveys collect individual beneficiary responses. Required for beneficiary satisfaction tracking, accountability to affected populations (AAP), and individual-level feedback analysi',
    ...
    `respondent_registrant_id` BIGINT COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` (
    ...
    `emergency_id` BIGINT COMMENT 'add column emergency_id (BIGINT) with FK to field.emergency.emergency_id - cluster coordination is activated in response to emergencies and this link is missing',
    ...
    `meeting_day` STRING COMMENT 'Day of the week or month when cluster coordination meetings are typically held.',
    `meeting_frequency` STRING COMMENT 'Scheduled frequency of cluster coordination meetings (e.g., weekly, monthly).. Valid values are `Weekly|Bi-Weekly|Monthly|Quarterly|Ad-Hoc|As Needed`',
    `meeting_location` STRING COMMENT 'Physical or virtual location where cluster coordination meetings are held (e.g., OCHA office, Zoom link).',
    `meeting_time` TIMESTAMP COMMENT 'Time of day when cluster coordination meetings are typically held (e.g., 10:00 AM local time).',
    ...
    `notes` STRING COMMENT 'Free-text notes capturing key coordination issues, decisions, action items, or strategic positioning considerations related to this cluster participation.',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`access_constraint` (
    ...
    `access_constraint_code` STRING COMMENT 'Externally-known unique code for the access constraint, typically formatted as AC-{COUNTRY}-{SEQUENCE} for tracking and reporting purposes.. Valid values are `^AC-[A-Z]{3}-[0-9]{6}$`',
    ...
    `end_date` DATE COMMENT 'Date when the access constraint was resolved or mitigated, allowing resumption of normal operations. Null if constraint remains active.',
    ...
    `notes` STRING COMMENT 'Additional contextual notes, observations, or lessons learned related to the access constraint. May include details on community dynamics, seasonal patterns, or coordination challenges.',
    ...
    `start_date` DATE COMMENT 'Date when the access constraint was first identified or reported. Marks the beginning of the impediment to humanitarian operations.',
    `access_constraint_status` STRING COMMENT 'Current lifecycle status of the access constraint. Active indicates ongoing impediment; monitoring indicates under observation; mitigated indicates workaround implemented; resolved indicates constraint removed; escalated indicates elevated to senior management or OCHA coordination.. Valid values are `active|monitoring|mitigated|resolved|escalated`',
    `access_constraint_type` STRING COMMENT 'Classification of the humanitarian access constraint affecting field operations. Active conflict includes armed hostilities; road blockage includes physical barriers; administrative denial includes government refusals; armed actor checkpoint includes non-state armed group controls; bureaucratic impediment includes visa/permit delays; weather/natural hazard includes floods, landslides, or extreme weather.. Valid values are `active_conflict|road_blockage|administrative_denial|armed_actor_checkpoint|bureaucratic_impediment|weather_natural_hazard`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`pdm_survey` (
    ...
    `pdm_survey_code` STRING COMMENT 'Externally-known unique business identifier for the PDM survey, typically following organizational naming conventions for tracking and reporting purposes.. Valid values are `^PDM-[A-Z0-9]{6,12}$`',
    ...
    `pdm_survey_date` DATE COMMENT 'The principal date when the PDM survey field data collection was conducted or completed. Represents the business event timestamp for accountability reporting.',
    ...
    `end_date` DATE COMMENT 'Date when the PDM survey field data collection activities were completed.',
    ...
    `start_date` DATE COMMENT 'Date when the PDM survey field data collection activities commenced.',
    `pdm_survey_status` STRING COMMENT 'Current lifecycle status of the PDM survey indicating its progress through planning, field data collection, analysis, and approval stages.. Valid values are `planned|in_progress|completed|cancelled|under_review|approved`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`assessment_participation` (
    ...
    `assessment_participation_date` DATE COMMENT 'The date this volunteer participated in the assessment fieldwork. May differ from assessment_date if the assessment spanned multiple days and this volunteer only participated on specific days. Supports attendance tracking and per-diem calculation.',
    ...
    `assessment_participation_status` STRING COMMENT 'Current status of this volunteers participation in this assessment. Tracks lifecycle from recruitment through completion. Values: confirmed (volunteer committed), completed (fieldwork done), cancelled (volunteer withdrew), no_show (volunteer did not appear). Supports operational coordination and volunteer reliability tracking.',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`field`.`emergency` (
    ...
    `intervention_id` BIGINT COMMENT 'add column program_intervention_id (BIGINT) with FK to program.intervention.intervention_id - emergencies trigger humanitarian interventions and this critical link is missing',
...
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ADD CONSTRAINT `fk_field_mobile_health_outreach_outreach_project_site_id` FOREIGN KEY (`outreach_project_site_id`) REFERENCES `vibe_ngo_v1`.`field`.`project_site`(`project_site_id`);
...
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ADD CONSTRAINT `fk_field_cluster_coordination_emergency_id` FOREIGN KEY (`emergency_id`) REFERENCES `vibe_ngo_v1`.`field`.`emergency`(`emergency_id`);
...
ALTER TABLE `vibe_ngo_v1`.`field`.`project_site` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`project_site` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`project_site` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`project_site` ALTER COLUMN `network_site_id` SET TAGS ('dbx_business_glossary_term' = 'Network Site Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`project_site` ALTER COLUMN `statutory_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Statutory Registration Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`country_office` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`country_office` ALTER COLUMN `network_site_id` SET TAGS ('dbx_business_glossary_term' = 'Network Site Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` SET TAGS ('dbx_subdomain' = 'service_delivery');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `cluster_coordination_id` SET TAGS ('dbx_business_glossary_term' = 'Cluster ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `it_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Vehicle It Asset Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `plan_id` SET TAGS ('dbx_business_glossary_term' = 'Communication Plan Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_event` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` SET TAGS ('dbx_subdomain' = 'service_delivery');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `field_distribution_line_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Line ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `distribution_event_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Event ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `actual_quantity_distributed` SET TAGS ('dbx_business_glossary_term' = 'Actual Quantity Distributed');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `batch_number` SET TAGS ('dbx_business_glossary_term' = 'Batch Number');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `beneficiary_count` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `cluster_sector` SET TAGS ('dbx_business_glossary_term' = 'Cluster Sector');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `distribution_method` SET TAGS ('dbx_business_glossary_term' = 'Distribution Method');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `distribution_method` SET TAGS ('dbx_value_regex' = 'direct_hand|voucher|cash_transfer|mobile_distribution|fixed_site');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `distribution_status` SET TAGS ('dbx_business_glossary_term' = 'Distribution Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `distribution_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|cancelled|partially_distributed');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `donor_earmark` SET TAGS ('dbx_business_glossary_term' = 'Donor Earmark');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Expiry Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `iati_transaction_type` SET TAGS ('dbx_business_glossary_term' = 'International Aid Transparency Initiative (IATI) Transaction Type');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `item_category` SET TAGS ('dbx_business_glossary_term' = 'Item Category');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `item_category` SET TAGS ('dbx_value_regex' = 'NFI|Food|WASH|Shelter|Health|Education');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `item_code` SET TAGS ('dbx_business_glossary_term' = 'Item Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `item_description` SET TAGS ('dbx_business_glossary_term' = 'Item Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `modified_by` SET TAGS ('dbx_business_glossary_term' = 'Modified By');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `number` SET TAGS ('dbx_business_glossary_term' = 'Line Number');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `pipeline_source` SET TAGS ('dbx_business_glossary_term' = 'Pipeline Source');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `planned_quantity` SET TAGS ('dbx_business_glossary_term' = 'Planned Quantity');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `quality_check_date` SET TAGS ('dbx_business_glossary_term' = 'Quality Check Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `quality_check_status` SET TAGS ('dbx_business_glossary_term' = 'Quality Check Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `quality_check_status` SET TAGS ('dbx_value_regex' = 'passed|failed|not_checked|conditional');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `sdg_alignment` SET TAGS ('dbx_business_glossary_term' = 'Sustainable Development Goal (SDG) Alignment');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `total_value` SET TAGS ('dbx_business_glossary_term' = 'Total Value');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `unit_value` SET TAGS ('dbx_business_glossary_term' = 'Unit Value');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `variance_quantity` SET TAGS ('dbx_business_glossary_term' = 'Variance Quantity');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `variance_reason` SET TAGS ('dbx_business_glossary_term' = 'Variance Reason');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_distribution_line` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` SET TAGS ('dbx_subdomain' = 'service_delivery');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_health_outreach_id` SET TAGS ('dbx_business_glossary_term' = 'Mobile Health Outreach ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_health_outreach_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_health_outreach_id` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Award Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `chs_self_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `field_team_id` SET TAGS ('dbx_business_glossary_term' = 'Team Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Intervention Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Site ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `project_site_id` SET TAGS ('dbx_classification' = 'restricted');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `project_site_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_project_site_id` SET TAGS ('dbx_classification' = 'restricted');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `mobile_project_site_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Org Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `registrant_id` SET TAGS ('dbx_business_glossary_term' = 'Registrant Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `safeguarding_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Safeguarding Incident Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Community Health Worker (CHW) Team Lead ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'KoboToolbox Form ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `outreach_project_site_id` SET TAGS ('dbx_renamed_from' = 'project_site_id');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `admin_level_1` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 1');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `admin_level_2` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 2');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_female_18_to_59` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Female 18 to 59');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_female_5_to_17` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Female 5 to 17');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_female_60_plus` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Female 60 Plus');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_female_under_5` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Female Under 5');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_male_18_to_59` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Male 18 to 59');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_male_5_to_17` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Male 5 to 17');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_male_60_plus` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Male 60 Plus');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `beneficiaries_reached_male_under_5` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Reached Male Under 5');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `chw_team_size` SET TAGS ('dbx_business_glossary_term' = 'Community Health Worker (CHW) Team Size');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `data_collection_method` SET TAGS ('dbx_business_glossary_term' = 'Data Collection Method');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `data_collection_method` SET TAGS ('dbx_value_regex' = 'KoboToolbox|CommCare|Paper Form|Mobile App|Other');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `dhis2_reporting_period` SET TAGS ('dbx_business_glossary_term' = 'District Health Information System 2 (DHIS2) Reporting Period');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `dhis2_reporting_period` SET TAGS ('dbx_value_regex' = '^[0-9]{4}-(Q[1-4]|M(0[1-9]|1[0-2])|W([0-4][0-9]|5[0-3]))$');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `health_cluster_reported` SET TAGS ('dbx_business_glossary_term' = 'Health Cluster Reported');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `health_cluster_reported` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `health_cluster_reported` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `latitude` SET TAGS ('dbx_business_glossary_term' = 'Latitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `location_name` SET TAGS ('dbx_business_glossary_term' = 'Location Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `location_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `longitude` SET TAGS ('dbx_business_glossary_term' = 'Longitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `referral_facility_primary` SET TAGS ('dbx_business_glossary_term' = 'Referral Facility Primary');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `referrals_made_count` SET TAGS ('dbx_business_glossary_term' = 'Referrals Made Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_anc_provided` SET TAGS ('dbx_business_glossary_term' = 'Antenatal Care (ANC) Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_gbv_referral_provided` SET TAGS ('dbx_business_glossary_term' = 'Gender-Based Violence (GBV) Referral Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_immunization_provided` SET TAGS ('dbx_business_glossary_term' = 'Immunization Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_muac_screening_provided` SET TAGS ('dbx_business_glossary_term' = 'Mid-Upper Arm Circumference (MUAC) Screening Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_pss_provided` SET TAGS ('dbx_business_glossary_term' = 'Psychosocial Support (PSS) Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `service_wash_hygiene_promotion_provided` SET TAGS ('dbx_business_glossary_term' = 'Water Sanitation and Hygiene (WASH) Hygiene Promotion Service Provided');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_code` SET TAGS ('dbx_business_glossary_term' = 'Session Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_code` SET TAGS ('dbx_value_regex' = '^MHO-[A-Z]{3}-[0-9]{6}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_date` SET TAGS ('dbx_business_glossary_term' = 'Session Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_end_time` SET TAGS ('dbx_business_glossary_term' = 'Session End Time');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_notes` SET TAGS ('dbx_business_glossary_term' = 'Session Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_start_time` SET TAGS ('dbx_business_glossary_term' = 'Session Start Time');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_status` SET TAGS ('dbx_business_glossary_term' = 'Session Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `session_status` SET TAGS ('dbx_value_regex' = 'Completed|Partially Completed|Cancelled|Postponed');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `sphere_compliant` SET TAGS ('dbx_business_glossary_term' = 'SPHERE Compliant');
ALTER TABLE `vibe_ngo_v1`.`field`.`mobile_health_outreach` ALTER COLUMN `total_beneficiaries_reached` SET TAGS ('dbx_business_glossary_term' = 'Total Beneficiaries Reached');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` SET TAGS ('dbx_subdomain' = 'service_delivery');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `wash_intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Water Sanitation and Hygiene (WASH) Intervention ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `chs_self_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `coordination_meeting_id` SET TAGS ('dbx_business_glossary_term' = 'Coordination Meeting ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `field_team_id` SET TAGS ('dbx_business_glossary_term' = 'Team Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `impact_story_id` SET TAGS ('dbx_business_glossary_term' = 'Impact Story Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Implementing Partner Organization ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Staff Member ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `actual_beneficiaries_reached` SET TAGS ('dbx_business_glossary_term' = 'Actual Beneficiaries Reached');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `actual_expenditure_usd` SET TAGS ('dbx_business_glossary_term' = 'Actual Expenditure United States Dollars (USD)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `actual_expenditure_usd` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `assessment_method` SET TAGS ('dbx_business_glossary_term' = 'Assessment Method');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `assessment_method` SET TAGS ('dbx_value_regex' = 'fgd|kii|household_survey|observation|pdm');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `budget_allocated_usd` SET TAGS ('dbx_business_glossary_term' = 'Budget Allocated United States Dollars (USD)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `budget_allocated_usd` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `community_contribution` SET TAGS ('dbx_business_glossary_term' = 'Community Contribution');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `contractor_name` SET TAGS ('dbx_business_glossary_term' = 'Contractor Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `data_collection_tool` SET TAGS ('dbx_business_glossary_term' = 'Data Collection Tool');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `data_collection_tool` SET TAGS ('dbx_value_regex' = 'kobo_toolbox|odk_collect|commcare|paper_form|other');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `wash_intervention_description` SET TAGS ('dbx_business_glossary_term' = 'WASH Intervention Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `disability_inclusion` SET TAGS ('dbx_business_glossary_term' = 'Disability Inclusion Measures');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `disability_inclusion` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `disability_inclusion` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Intervention End Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `environmental_impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Environmental Impact Assessment Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `environmental_impact_assessment` SET TAGS ('dbx_value_regex' = 'not_required|conducted|pending');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `gender_considerations` SET TAGS ('dbx_business_glossary_term' = 'Gender Considerations');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `gender_considerations` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `gender_considerations` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `gender_considerations` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `hygiene_promotion_conducted` SET TAGS ('dbx_business_glossary_term' = 'Hygiene Promotion Conducted Flag');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_latitude` SET TAGS ('dbx_business_glossary_term' = 'Infrastructure GPS Latitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_longitude` SET TAGS ('dbx_business_glossary_term' = 'Infrastructure GPS Longitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `infrastructure_gps_longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `nfi_distributed_quantity` SET TAGS ('dbx_business_glossary_term' = 'Non-Food Item (NFI) Distributed Quantity');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `ocha_wash_cluster_code` SET TAGS ('dbx_business_glossary_term' = 'Office for the Coordination of Humanitarian Affairs (OCHA) WASH Cluster Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `ocha_wash_cluster_code` SET TAGS ('dbx_value_regex' = '^WASH-[A-Z]{2}-[0-9]{4}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sitrep_reporting_period` SET TAGS ('dbx_business_glossary_term' = 'Situation Report (SitRep) Reporting Period');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sitrep_reporting_period` SET TAGS ('dbx_value_regex' = '^[0-9]{4}-Q[1-4]$|^[0-9]{4}-M(0[1-9]|1[0-2])$');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sphere_latrine_coverage_ratio` SET TAGS ('dbx_business_glossary_term' = 'SPHERE Latrine Coverage Ratio');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sphere_latrine_coverage_ratio` SET TAGS ('dbx_value_regex' = '^1:[0-9]+$');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sphere_latrine_coverage_ratio` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sphere_water_quantity_lpd` SET TAGS ('dbx_business_glossary_term' = 'SPHERE Water Quantity Liters Per Person Per Day (LPD)');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Intervention Start Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `wash_intervention_status` SET TAGS ('dbx_business_glossary_term' = 'WASH Intervention Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `wash_intervention_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|suspended|cancelled');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `sustainability_plan` SET TAGS ('dbx_business_glossary_term' = 'Sustainability Plan');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `target_population_count` SET TAGS ('dbx_business_glossary_term' = 'Target Population Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `wash_intervention_type` SET TAGS ('dbx_business_glossary_term' = 'WASH Intervention Type');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `water_quality_test_result` SET TAGS ('dbx_business_glossary_term' = 'Water Quality Test Result');
ALTER TABLE `vibe_ngo_v1`.`field`.`wash_intervention` ALTER COLUMN `water_quality_test_result` SET TAGS ('dbx_value_regex' = 'safe|unsafe|not_tested');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` SET TAGS ('dbx_subdomain' = 'deployment_management');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `evaluation_id` SET TAGS ('dbx_business_glossary_term' = 'Evaluation Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `internal_review_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Review Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `plan_id` SET TAGS ('dbx_business_glossary_term' = 'Communication Plan Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `safeguarding_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Safeguarding Incident Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'KoboToolbox Form ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Assessment Team Lead Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` SET TAGS ('dbx_subdomain' = 'deployment_management');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `registrant_id` SET TAGS ('dbx_business_glossary_term' = 'Respondent Anonymized ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `registrant_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `registrant_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `assessment_respondent_registrant_id` SET TAGS ('dbx_business_glossary_term' = 'Registrant Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Enumerator ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `respondent_registrant_id` SET TAGS ('dbx_renamed_from' = 'registrant_id');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_response` ALTER COLUMN `secondary_need_category` SET TAGS ('dbx_business_glossary_term' = 'Secondary Need Category');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` SET TAGS ('dbx_subdomain' = 'deployment_management');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `plan_id` SET TAGS ('dbx_business_glossary_term' = 'Communication Plan Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `admin_level_1_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `admin_level_2_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `cluster_lead_agency` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`sitrep` ALTER COLUMN `hrp_progress_percentage` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` SET TAGS ('dbx_subdomain' = 'site_operations');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_coordination_id` SET TAGS ('dbx_business_glossary_term' = 'Cluster Coordination ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Org Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'NGO (Non-Governmental Organization) Focal Point Staff ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_activation_date` SET TAGS ('dbx_business_glossary_term' = 'Cluster Activation Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_activation_status` SET TAGS ('dbx_business_glossary_term' = 'Cluster Activation Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_activation_status` SET TAGS ('dbx_value_regex' = 'Active|Inactive|Suspended|Deactivated|Pending Activation');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_code` SET TAGS ('dbx_business_glossary_term' = 'Cluster Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_deactivation_date` SET TAGS ('dbx_business_glossary_term' = 'Cluster Deactivation Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_name` SET TAGS ('dbx_business_glossary_term' = 'Cluster Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_strategy_alignment_status` SET TAGS ('dbx_business_glossary_term' = 'Cluster Strategy Alignment Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_strategy_alignment_status` SET TAGS ('dbx_value_regex' = 'Fully Aligned|Partially Aligned|Not Aligned|Under Review');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `cluster_working_group_membership` SET TAGS ('dbx_business_glossary_term' = 'Cluster Working Group Membership');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `co_lead_agency_name` SET TAGS ('dbx_business_glossary_term' = 'Co-Lead Agency Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `co_lead_agency_name` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `hpc_commitment_amount` SET TAGS ('dbx_business_glossary_term' = 'HPC (Humanitarian Programme Cycle) Commitment Amount');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `hpc_commitment_currency_code` SET TAGS ('dbx_business_glossary_term' = 'HPC (Humanitarian Programme Cycle) Commitment Currency Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `hpc_commitment_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `hpc_commitment_description` SET TAGS ('dbx_business_glossary_term' = 'HPC (Humanitarian Programme Cycle) Commitment Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `information_sharing_agreement_date` SET TAGS ('dbx_business_glossary_term' = 'Information Sharing Agreement Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `information_sharing_agreement_signed_flag` SET TAGS ('dbx_business_glossary_term' = 'Information Sharing Agreement Signed Flag');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `last_meeting_attendance_date` SET TAGS ('dbx_business_glossary_term' = 'Last Meeting Attendance Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `last_sitrep_contribution_date` SET TAGS ('dbx_business_glossary_term' = 'Last SitRep (Situation Report) Contribution Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `last_three_w_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Last 3W (Who does What Where) Submission Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `lead_agency_name` SET TAGS ('dbx_business_glossary_term' = 'Lead Agency Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `lead_agency_name` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `meeting_day` SET TAGS ('dbx_business_glossary_term' = 'Coordination Meeting Day');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `meeting_frequency` SET TAGS ('dbx_business_glossary_term' = 'Coordination Meeting Frequency');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `meeting_frequency` SET TAGS ('dbx_value_regex' = 'Weekly|Bi-Weekly|Monthly|Quarterly|Ad-Hoc|As Needed');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `meeting_location` SET TAGS ('dbx_business_glossary_term' = 'Coordination Meeting Location');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `meeting_time` SET TAGS ('dbx_business_glossary_term' = 'Coordination Meeting Time');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `next_three_w_submission_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next 3W (Who does What Where) Submission Due Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `ngo_participation_end_date` SET TAGS ('dbx_business_glossary_term' = 'NGO (Non-Governmental Organization) Participation End Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `ngo_participation_start_date` SET TAGS ('dbx_business_glossary_term' = 'NGO (Non-Governmental Organization) Participation Start Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `ngo_participation_status` SET TAGS ('dbx_business_glossary_term' = 'NGO (Non-Governmental Organization) Participation Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `ngo_participation_status` SET TAGS ('dbx_value_regex' = 'Active|Inactive|Suspended|Withdrawn');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Coordination Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `sitrep_contribution_flag` SET TAGS ('dbx_business_glossary_term' = 'SitRep (Situation Report) Contribution Flag');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `three_w_reporting_frequency` SET TAGS ('dbx_business_glossary_term' = '3W (Who does What Where) Reporting Frequency');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `three_w_reporting_frequency` SET TAGS ('dbx_value_regex' = 'Weekly|Monthly|Quarterly|Bi-Annual|Annual|On-Demand');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `three_w_reporting_obligation_flag` SET TAGS ('dbx_business_glossary_term' = '3W (Who does What Where) Reporting Obligation Flag');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `three_w_submission_compliance_status` SET TAGS ('dbx_business_glossary_term' = '3W (Who does What Where) Submission Compliance Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `three_w_submission_compliance_status` SET TAGS ('dbx_value_regex' = 'Compliant|Overdue|Pending|Not Applicable');
ALTER TABLE `vibe_ngo_v1`.`field`.`cluster_coordination` ALTER COLUMN `total_meetings_attended_count` SET TAGS ('dbx_business_glossary_term' = 'Total Meetings Attended Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` SET TAGS ('dbx_subdomain' = 'deployment_management');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `field_team_id` SET TAGS ('dbx_business_glossary_term' = 'Team ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `it_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Vehicle ID');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Team Leader ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `secondary_languages` SET TAGS ('dbx_business_glossary_term' = 'Secondary Languages');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_team` ALTER COLUMN `secondary_languages` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` SET TAGS ('dbx_subdomain' = 'deployment_management');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Deployed Staff Member ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `safeguarding_training_completion_id` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`field_deployment` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'KoboToolbox Form ID');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` SET TAGS ('dbx_subdomain' = 'site_operations');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_id` SET TAGS ('dbx_business_glossary_term' = 'Access Constraint ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Shipment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `compliance_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Incident Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Org Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `security_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Security Incident Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'KoboToolbox Form ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `admin_level_1` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 1');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `admin_level_2` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 2');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `affected_activities_description` SET TAGS ('dbx_business_glossary_term' = 'Affected Activities Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `affected_location_name` SET TAGS ('dbx_business_glossary_term' = 'Affected Location Name');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `affected_location_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `affected_pcode` SET TAGS ('dbx_business_glossary_term' = 'Affected Place Code (P-Code)');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `affected_pcode` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}[0-9]{2,8}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `alternative_route_available` SET TAGS ('dbx_business_glossary_term' = 'Alternative Route Available');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `alternative_route_description` SET TAGS ('dbx_business_glossary_term' = 'Alternative Route Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `armed_actor_involved` SET TAGS ('dbx_business_glossary_term' = 'Armed Actor Involved');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `armed_actor_involved` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `cluster_affiliation` SET TAGS ('dbx_business_glossary_term' = 'Cluster Affiliation');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_code` SET TAGS ('dbx_business_glossary_term' = 'Access Constraint Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_code` SET TAGS ('dbx_value_regex' = '^AC-[A-Z]{3}-[0-9]{6}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `data_source_system` SET TAGS ('dbx_business_glossary_term' = 'Data Source System');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `donor_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Donor Notification Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `donor_notification_required` SET TAGS ('dbx_business_glossary_term' = 'Donor Notification Required');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Constraint End Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `escalation_status` SET TAGS ('dbx_business_glossary_term' = 'Escalation Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `escalation_status` SET TAGS ('dbx_value_regex' = 'not_escalated|escalated_country_director|escalated_regional_director|escalated_ocha|escalated_hq');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `estimated_beneficiaries_unreached` SET TAGS ('dbx_business_glossary_term' = 'Estimated Beneficiaries Unreached');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `impact_on_program_timeline` SET TAGS ('dbx_business_glossary_term' = 'Impact on Program Timeline');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `latitude` SET TAGS ('dbx_business_glossary_term' = 'Latitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `longitude` SET TAGS ('dbx_business_glossary_term' = 'Longitude');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `mitigation_actions_taken` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Actions Taken');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `negotiation_required` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Required');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Access Constraint Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `reporting_source` SET TAGS ('dbx_business_glossary_term' = 'Reporting Source');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `reporting_source_contact` SET TAGS ('dbx_business_glossary_term' = 'Reporting Source Contact');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `reporting_source_contact` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `security_incident_linked` SET TAGS ('dbx_business_glossary_term' = 'Security Incident Linked');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `sitrep_included` SET TAGS ('dbx_business_glossary_term' = 'Situation Report (SitRep) Included');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Constraint Start Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_status` SET TAGS ('dbx_business_glossary_term' = 'Access Constraint Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_status` SET TAGS ('dbx_value_regex' = 'active|monitoring|mitigated|resolved|escalated');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_type` SET TAGS ('dbx_business_glossary_term' = 'Access Constraint Type');
ALTER TABLE `vibe_ngo_v1`.`field`.`access_constraint` ALTER COLUMN `access_constraint_type` SET TAGS ('dbx_value_regex' = 'active_conflict|road_blockage|administrative_denial|armed_actor_checkpoint|bureaucratic_impediment|weather_natural_hazard');
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` SET TAGS ('dbx_subdomain' = 'site_operations');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Shipment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `compliance_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Incident Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `crisis_communication_id` SET TAGS ('dbx_business_glossary_term' = 'Crisis Communication Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Security Focal Point Staff ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`security_incident` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` SET TAGS ('dbx_subdomain' = 'service_delivery');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_id` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `distribution_event_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Event ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `feedback_case_id` SET TAGS ('dbx_business_glossary_term' = 'Feedback Case Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `household_id` SET TAGS ('dbx_business_glossary_term' = 'Household Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Org Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `safeguarding_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Safeguarding Incident Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Surveyor Team Lead Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `aap_score` SET TAGS ('dbx_business_glossary_term' = 'Accountability to Affected Populations (AAP) Score');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `adequacy_score` SET TAGS ('dbx_business_glossary_term' = 'Adequacy Score');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `age_disaggregation_available` SET TAGS ('dbx_business_glossary_term' = 'Age Disaggregation Available');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `age_disaggregation_available` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `chs_compliance_rating` SET TAGS ('dbx_business_glossary_term' = 'Core Humanitarian Standard (CHS) Compliance Rating');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `chs_compliance_rating` SET TAGS ('dbx_value_regex' = 'fully_compliant|substantially_compliant|partially_compliant|non_compliant|not_assessed');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `cluster_sector` SET TAGS ('dbx_business_glossary_term' = 'Cluster Sector');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_code` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey Code');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_code` SET TAGS ('dbx_value_regex' = '^PDM-[A-Z0-9]{6,12}$');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `complaints_received_count` SET TAGS ('dbx_business_glossary_term' = 'Complaints Received Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `corrective_actions_required` SET TAGS ('dbx_business_glossary_term' = 'Corrective Actions Required');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `data_collection_tool` SET TAGS ('dbx_business_glossary_term' = 'Data Collection Tool');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `data_source_system` SET TAGS ('dbx_business_glossary_term' = 'Data Source System');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_date` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `days_post_distribution` SET TAGS ('dbx_business_glossary_term' = 'Days Post-Distribution');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey End Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `gender_disaggregation_available` SET TAGS ('dbx_business_glossary_term' = 'Gender Disaggregation Available');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `gender_disaggregation_available` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `gender_disaggregation_available` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `gender_disaggregation_available` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `key_findings_summary` SET TAGS ('dbx_business_glossary_term' = 'Key Findings Summary');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `methodology` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey Methodology');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `methodology` SET TAGS ('dbx_value_regex' = 'household_interview|focus_group_discussion|key_informant_interview|phone_survey|mobile_data_collection|mixed_methods');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `protection_concern_description` SET TAGS ('dbx_business_glossary_term' = 'Protection Concern Description');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `protection_concern_description` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `protection_concerns_noted` SET TAGS ('dbx_business_glossary_term' = 'Protection Concerns Noted');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `recommendations` SET TAGS ('dbx_business_glossary_term' = 'Recommendations');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `report_approved_date` SET TAGS ('dbx_business_glossary_term' = 'Report Approved Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `report_submitted_date` SET TAGS ('dbx_business_glossary_term' = 'Report Submitted Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `response_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Response Rate Percentage');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `sample_size_actual` SET TAGS ('dbx_business_glossary_term' = 'Sample Size Actual');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `sample_size_planned` SET TAGS ('dbx_business_glossary_term' = 'Sample Size Planned');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `sampling_method` SET TAGS ('dbx_business_glossary_term' = 'Sampling Method');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `sampling_method` SET TAGS ('dbx_value_regex' = 'random|systematic|stratified|cluster|purposive|convenience');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `satisfaction_score` SET TAGS ('dbx_business_glossary_term' = 'Satisfaction Score');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `sdg_alignment` SET TAGS ('dbx_business_glossary_term' = 'Sustainable Development Goal (SDG) Alignment');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey Start Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_status` SET TAGS ('dbx_business_glossary_term' = 'Post-Distribution Monitoring (PDM) Survey Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `pdm_survey_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|cancelled|under_review|approved');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `surveyor_organization` SET TAGS ('dbx_business_glossary_term' = 'Surveyor Organization');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `utilization_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Utilization Rate Percentage');
ALTER TABLE `vibe_ngo_v1`.`field`.`pdm_survey` ALTER COLUMN `vulnerability_disaggregation_available` SET TAGS ('dbx_business_glossary_term' = 'Vulnerability Disaggregation Available');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` SET TAGS ('dbx_subdomain' = 'deployment_management');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` SET TAGS ('dbx_association_edges' = 'field.assessment,volunteer.volunteer');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `assessment_participation_id` SET TAGS ('dbx_business_glossary_term' = 'Assessment Participation ID');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Assessment Participation - Assessment Id');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `volunteer_id` SET TAGS ('dbx_business_glossary_term' = 'Assessment Participation - Volunteer Id');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `assigned_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Assigned Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `completed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completed Timestamp');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `assessment_participation_date` SET TAGS ('dbx_business_glossary_term' = 'Participation Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `enumerator_role` SET TAGS ('dbx_business_glossary_term' = 'Enumerator Role');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `assessment_participation_status` SET TAGS ('dbx_business_glossary_term' = 'Participation Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `surveys_completed_count` SET TAGS ('dbx_business_glossary_term' = 'Surveys Completed Count');
ALTER TABLE `vibe_ngo_v1`.`field`.`assessment_participation` ALTER COLUMN `training_completion_status` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` SET TAGS ('dbx_subdomain' = 'service_delivery');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` SET TAGS ('dbx_association_edges' = 'field.distribution_event,volunteer.volunteer');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `distribution_participation_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Participation Identifier');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `distribution_event_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Participation - Distribution Event Id');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `volunteer_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Participation - Volunteer Id');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `assignment_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Date');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `attendance_status` SET TAGS ('dbx_business_glossary_term' = 'Attendance Status');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `beneficiaries_served` SET TAGS ('dbx_business_glossary_term' = 'Beneficiaries Served');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `hours_worked` SET TAGS ('dbx_business_glossary_term' = 'Hours Worked');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Participation Notes');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `performance_rating` SET TAGS ('dbx_business_glossary_term' = 'Performance Rating');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `shift_assignment` SET TAGS ('dbx_business_glossary_term' = 'Shift Assignment');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `shift_end_time` SET TAGS ('dbx_business_glossary_term' = 'Shift End Time');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `shift_start_time` SET TAGS ('dbx_business_glossary_term' = 'Shift Start Time');
ALTER TABLE `vibe_ngo_v1`.`field`.`distribution_participation` ALTER COLUMN `volunteer_role` SET TAGS ('dbx_business_glossary_term' = 'Volunteer Role');
ALTER TABLE `vibe_ngo_v1`.`field`.`country` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`field`.`country` SET TAGS ('dbx_subdomain' = 'site_operations');
...
ALTER TABLE `vibe_ngo_v1`.`field`.`emergency` SET TAGS ('dbx_subdomain' = 'site_operations');
