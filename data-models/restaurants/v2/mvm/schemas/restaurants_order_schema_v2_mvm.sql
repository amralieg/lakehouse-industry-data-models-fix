...
-- Generated on: 2026-07-10 20:02:56
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`guest_order` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to guest.address. Business justification: Delivery zone analytics, address-level order frequency, and delivery performance reporting require linking guest_order to a validated guest address record. Denormalized delivery_address_line1 and deli',
    ...
    `pos_terminal_id` BIGINT COMMENT 'Foreign key linking to restaurant.pos_terminal. Business justification: POS terminal-level order attribution supports terminal sales reconciliation, PCI audit trails at the order level, and speed-of-service analysis by terminal — all named operational reporting requiremen',
    ...
    `shift_id` BIGINT COMMENT 'Foreign key linking to workforce.shift. Business justification: Shift-level sales summary (total orders, revenue, ticket times per shift) is a standard restaurant management report used for labor-to-sales ratio analysis. Deriving shift from server_employee_id + pl',
    `supplier_id` BIGINT COMMENT 'Identifier of the marketing promotion, coupon, or Limited Time Offer (LTO) applied to this order. Null if no promotion was applied. Used for promotional campaign effectiveness and discount attribution analysis.',
    ...
    `visit_id` BIGINT COMMENT 'Foreign key linking to guest.guest_visit. Business justification: Visit-level analytics (average check per visit, items per visit, visit frequency) require linking orders to the parent visit. Operations reports and loyalty visit-counting depend on this. No existing ',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`order_item` (
    ...
    `allergen_declaration_id` BIGINT COMMENT 'Foreign key linking to menu.allergen_declaration. Business justification: Food safety liability and illness investigation (guest_order.illness_report_id) require knowing which allergen_declaration was active when an order item was prepared. Allergen incident response, regul',
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: order_item has a service_channel STRING column that denormalizes channel information at the line-item level. Adding channel_id FK normalizes this to the channel reference master, enabling consistent c',
    ...
    `item_cost_id` BIGINT COMMENT 'Foreign key linking to menu.item_cost. Business justification: Theoretical vs. actual COGS variance reporting — a core restaurant P&L process — requires linking each sold order item to the item_cost record active at time of sale. Food cost reconciliation and menu',
    `item_price_id` BIGINT COMMENT 'Foreign key linking to menu.item_price. Business justification: Revenue auditing and franchise price compliance require linking each order line to the exact price record (channel, daypart, effective date) in effect at sale time. Price override tracking and price e',
    ...
    `nutrition_profile_id` BIGINT COMMENT 'Foreign key linking to menu.nutrition_profile. Business justification: FDA menu labeling regulations require traceability of which nutrition_profile version was disclosed at point of sale for each order item. Regulatory audit trails and calorie disclosure compliance repo',
    ...
    `recipe_id` BIGINT COMMENT 'Foreign key linking to menu.recipe. Business justification: Food safety and HACCP compliance require tracing which specific recipe version was used to prepare each order item. Kitchen operations, food safety audits, and waste tracking all depend on knowing the',
    `uom_id` BIGINT COMMENT 'Foreign key linking to inventory.uom. Business justification: Accurate inventory depletion math requires order_item quantities expressed in a UOM consistent with inventory tracking. Without sold_uom_id, depletion calculations cannot convert sold quantities to in',
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: Theoretical usage / sales-driven inventory depletion reporting requires linking each sold order_item to its inventory stock_item. Restaurant operators run daily depletion reports (sales × recipe yield',
    `unit_id` BIGINT COMMENT 'Foreign key linking to restaurant.unit. Business justification: Needed for Limited‑Time Offer (LTO) effectiveness tracking: linking each order line to its LTO campaign enables calculation of lift, cost, and ROI for promotional items.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`order_modifier` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: order_modifier has an order_channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling channel-level modifier analytics',
    `discount_id` BIGINT COMMENT 'Foreign key linking to order.discount. Business justification: order_modifier has an is_comped flag indicating modifier-level comps/discounts, but no FK to the discount record that authorized the comp. Adding discount_id to order_modifier enables traceability fro',
    ...
    `ingredient_id` BIGINT COMMENT 'Foreign key linking to supply.ingredient. Business justification: Food safety traceability and allergen management require knowing which ingredient backs each modifier (e.g., add bacon, substitute cheese). If an ingredient is recalled or flagged, operators must iden',
    `ingredient_lot_id` BIGINT COMMENT 'Foreign key linking to supply.ingredient_lot. Business justification: Lot-level recall traceability requires identifying which specific ingredient lot was used in each modifier. FDA/FSMA traceability rules mandate lot-level tracking for all food components served. order',
    `kitchen_station_id` BIGINT COMMENT 'Foreign key linking to restaurant.kitchen_station. Business justification: Modifiers are KDS-routed to specific kitchen stations (grill, expo, etc.) for preparation. Station-level modifier throughput and KDS routing accuracy reporting require a proper FK. kds_station_name is',
    ...
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: Modifier-level inventory depletion (e.g., extra cheese, added protein) requires linking each order_modifier to its stock_item. Restaurant COGS and inventory accuracy reports must account for modifier ',
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to inventory.uom. Business justification: Modifier quantities (e.g., 2 oz extra sauce) must reference inventory UOM for accurate depletion. Role-prefix modifier_ distinguishes from order_items sold_uom_id. Required for modifier-level COGS ',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`payment` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: payment has a channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling consistent channel-level payment tender analyt',
    `discount_id` BIGINT COMMENT 'Foreign key linking to order.discount. Business justification: payment has a discount_amount column capturing the discount applied at payment time, but no FK to the discount record. Adding discount_id to payment enables traceability from a payment tender back to ',
    ...
    `shift_id` BIGINT COMMENT 'Foreign key linking to workforce.shift. Business justification: End-of-shift cash drawer reconciliation is a critical named restaurant process: managers must balance all payments processed during a shift against the drawer count. Deriving shift from cashier_employ',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`status_event` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: status_event has a service_channel STRING column that denormalizes channel information. Adding channel_id FK to the channel reference table normalizes this relationship, enabling consistent channel-le',
    ...
    `kitchen_station_id` BIGINT COMMENT 'Foreign key linking to restaurant.kitchen_station. Business justification: Order status events include KDS state transitions tied to specific kitchen stations. Station-level SOS breach analysis, throughput dashboards, and KDS performance reporting — all named restaurant oper',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`channel` (
    ...
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Channels are configured against specific loyalty programs (mobile app → primary program; third-party delivery → no program). This drives loyalty integration setup, channel-level program eligibility ru',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`kds_ticket` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: kds_ticket has an order_channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling channel-level KDS performance and ti',
    ...
    `recipe_id` BIGINT COMMENT 'Foreign key linking to menu.recipe. Business justification: Kitchen display systems must surface the correct recipe version (cook temperature, hold time, plating instructions) for each ticket. Food safety compliance and kitchen accuracy reporting require the K',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`delivery_order` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to guest.address. Business justification: Delivery address management requires normalizing the denormalized address fields on delivery_order to guest.address for address validation, repeat-delivery-address analytics, and delivery-zone perform',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`catering_order` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to guest.address. Business justification: Catering delivery logistics require a validated, reusable address record for repeat venue tracking, large-order logistics planning, and address validation. Denormalized address fields on catering_orde',
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: catering_order has an order_channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling channel-level catering order ana',
    ...
    `menu_id` BIGINT COMMENT 'Foreign key linking to menu.menu. Business justification: Catering operations use a distinct catering menu for quoting, fulfillment, and pricing. Linking catering_order to the specific menu version used ensures accurate quoted_price validation, catering menu',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`discount` (
    ...
    `offer_id` BIGINT COMMENT 'Foreign key linking to loyalty.offer. Business justification: Discounts generated from loyalty offers require traceability back to the originating offer for offer redemption reporting, P&L attribution by offer, and fraud detection. Restaurant loyalty operations ',
    ...
    `reward_id` BIGINT COMMENT 'Foreign key linking to loyalty.reward. Business justification: Loyalty reward redemptions directly generate discount records at POS. Linking discount to reward enables loyalty liability reconciliation, reward fulfillment auditing, and COGS impact reporting for re',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Supplier-funded promotional discounts (trade spend / promotional allowances) are authorized by specific supplier contracts. Linking discount to supplier_contract enables trade spend reconciliation, co',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`refund` (
    ...
    `allergen_incident_id` BIGINT COMMENT 'Foreign key linking to foodsafety.allergen_incident. Business justification: Refunds triggered by allergen reactions must be traceable to the allergen incident for financial impact analysis, legal liability tracking, and FDA compliance reporting. This link enables quantifying ',
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: refund has an order_channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling channel-level refund rate analytics. The',
    ...
    `member_id` BIGINT COMMENT 'Foreign key linking to loyalty.member. Business justification: Refunds for loyalty members require points reversal processing. refund already has loyalty_points_refunded column but no FK to identify which member account to credit. Member-level refund history also',
    `order_item_id` BIGINT COMMENT 'Foreign key linking to order.order_item. Business justification: A refund is frequently issued at the item level (e.g., wrong item, quality complaint on a specific menu item). refund currently only links to guest_order (header level), which prevents item-level refu',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`order`.`tax` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: tax has an order_channel STRING column that denormalizes channel information. Adding channel_id FK normalizes this to the channel reference master, enabling channel-level tax compliance and remittance',
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_item` ADD CONSTRAINT `fk_order_order_item_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ADD CONSTRAINT `fk_order_order_modifier_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ADD CONSTRAINT `fk_order_order_modifier_discount_id` FOREIGN KEY (`discount_id`) REFERENCES `vibe_restaurants_v1`.`order`.`discount`(`discount_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`payment` ADD CONSTRAINT `fk_order_payment_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`status_event` ADD CONSTRAINT `fk_order_status_event_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`kds_ticket` ADD CONSTRAINT `fk_order_kds_ticket_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ADD CONSTRAINT `fk_order_refund_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`tax` ADD CONSTRAINT `fk_order_tax_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `vibe_restaurants_v1`.`order`.`channel`(`channel_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Address Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion ID');
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Restaurant ID');
ALTER TABLE `vibe_restaurants_v1`.`order`.`guest_order` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Guest Visit Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_item` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_item` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Sold Uom Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_item` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Item Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_item` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Lto Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `discount_id` SET TAGS ('dbx_business_glossary_term' = 'Discount Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `ingredient_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Ingredient Lot Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `kitchen_station_id` SET TAGS ('dbx_business_glossary_term' = 'Kitchen Station Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Item Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`order_modifier` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Modifier Uom Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`payment` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`status_event` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`channel` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`kds_ticket` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`kds_ticket` ALTER COLUMN `recipe_id` SET TAGS ('dbx_business_glossary_term' = 'Recipe Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`delivery_order` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Guest Address Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`catering_order` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Catering Delivery Address Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`catering_order` ALTER COLUMN `menu_id` SET TAGS ('dbx_business_glossary_term' = 'Menu Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`discount` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ALTER COLUMN `allergen_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Allergen Incident Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Member Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`order`.`refund` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`order`.`tax` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
