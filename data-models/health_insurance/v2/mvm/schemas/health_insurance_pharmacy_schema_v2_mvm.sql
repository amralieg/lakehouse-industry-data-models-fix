...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_master` (
    ...
    `hcc_mapping_id` BIGINT COMMENT 'Foreign key linking to risk.hcc_mapping. Business justification: drug_master.hcc_relevant flag signals HCC-associated drugs used in CMS risk adjustment reporting. Formalizing this as a FK to hcc_mapping enables risk-adjustment drug utilization analytics and support',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`formulary_drug_tier` (
    ...
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: Formulary management process: each drug tiers PA, step therapy, and quantity limit requirements are governed by specific clinical criteria versions. Formulary teams must link tier-level coverage rule',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_pricing` (
    ...
    `formulary_id` BIGINT COMMENT 'Foreign key linking to pharmacy.formulary. Business justification: drug_pricing carries a denormalized formulary_tier STRING column. Adding formulary_id FK to formulary normalizes the formulary reference, enabling direct joins to retrieve tier and formulary metadata.',
    `pbm_contract_id` BIGINT COMMENT 'Foreign key linking to pharmacy.pbm_contract. Business justification: drug_pricing carries mac_list_version and mac_list_reference as denormalized attributes. PBM contracts define the MAC list reference, AWP discount percentages, and pricing methodology that govern drug',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` (
    ...
    `benefit_package_id` BIGINT COMMENT 'Foreign key linking to plan.benefit_package. Business justification: Pharmacy claim adjudication applies cost-sharing rules defined at the benefit_package level (retail copay tiers, deductible applicability, specialty drug management). Linking claims to benefit_package',
    `cost_share_rule_id` BIGINT COMMENT 'Foreign key linking to plan.cost_share_rule. Business justification: During pharmacy claim adjudication, a specific cost_share_rule determines member copay/coinsurance. Regulatory audits and member grievance processes require an audit trail of which rule was applied to',
    ...
    `formulary_id` BIGINT COMMENT 'Foreign key linking to pharmacy.formulary. Business justification: pharmacy_claim carries formulary_tier as a denormalized STRING. Each pharmacy claim is adjudicated against a specific formulary (the members plan formulary). Adding formulary_id FK to formulary norma',
    ...
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: Pharmacy claims are adjudicated against a specific member policy to determine deductible application, OOP accumulation, and plan-paid amounts. EOB generation, COB adjudication, and CMS Part D reportin',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: Every pharmacy claim must identify the prescribing provider for CMS Part D reporting, HEDIS prescribing measures, fraud/waste/abuse detection, and provider profiling. prescriber_npi is a denormalized ',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`claim_line` (
    ...
    `cost_share_rule_id` BIGINT COMMENT 'Foreign key linking to plan.cost_share_rule. Business justification: Claim line adjudication applies drug-specific cost_share_rules (e.g., specialty drug coinsurance, HSA-compatible deductible rules). Linking claim_line to cost_share_rule enables line-level audit trail',
    ...
    `formulary_drug_tier_id` BIGINT COMMENT 'Foreign key linking to pharmacy.formulary_drug_tier. Business justification: claim_line carries formulary_tier as a denormalized STRING. formulary_drug_tier is the authoritative association entity linking a specific NDC to a formulary with its assigned tier, cost-sharing rules',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`prior_authorization` (
    ...
    `benefit_id` BIGINT COMMENT 'Foreign key linking to plan.benefit. Business justification: A prior authorization is triggered by a specific benefit rule (authorization_required, prior_auth_review_level on benefit). Linking PA to benefit enables clinical criteria validation against the benef',
    `clinical_criteria_id` BIGINT COMMENT 'Foreign key linking to utilization.clinical_criteria. Business justification: Pharmacy PA review process: each pharmacy prior authorization is evaluated against a specific clinical criteria set and version. PA reviewers and auditors must trace which clinical_criteria governed a',
    `drug_master_id` BIGINT COMMENT 'Foreign key linking to pharmacy.drug_master. Business justification: prior_authorization carries drug_ndc as a denormalized NDC code STRING. drug_master is the authoritative NDC-level drug master record. Adding drug_master_id FK normalizes the drug reference on the PA ',
    ...
    `hcc_mapping_id` BIGINT COMMENT 'Foreign key linking to risk.hcc_mapping. Business justification: PA clinical criteria for specialty and HCC-relevant drugs are validated against HCC-mapped diagnoses (prior_authorization.icd_diagnosis_code maps to hcc_mapping.icd10_code). Risk-adjustment-aware PA r',
    ...
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: PA decisions are made against a specific policys benefit design and coverage year. CMS PA reporting, denial letters, and appeal processing all require policy context. prior_authorization links to sub',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider. Business justification: Every pharmacy PA is submitted by a prescribing provider. Health insurance PA workflows, CMS Part D audit trails, and fraud/abuse detection require linking the PA to the credentialed prescriber in the',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` (
    ...
    `network_config_id` BIGINT COMMENT 'Foreign key linking to plan.network_config. Business justification: Dispensing pharmacy network tier assignment (in-network vs. out-of-network, preferred vs. standard) is governed by the plans network_config. This link enables accurate in/out-of-network determination',
    `party_id` BIGINT COMMENT 'Foreign key linking to contract.party. Business justification: Dispensing pharmacies are contracted network parties. Health plan pharmacy network management — credentialing, NPI validation, sanction checks, network participation status — requires linking each dis',
    `pbm_contract_id` BIGINT COMMENT 'Foreign key linking to pharmacy.pbm_contract. Business justification: dispensing_pharmacy carries contract_type, contract_effective_date, and contract_termination_date as denormalized attributes. pbm_contract is the authoritative record governing PBM network participati',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`pbm_contract` (
    ...
    `party_id` BIGINT COMMENT 'Foreign key linking to contract.party. Business justification: The PBM is a contracted legal entity tracked as a contract.party (TIN, NPI, legal name, credentialing status). PBM contract management, vendor oversight, and regulatory reporting (CMS PBM transparency',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` (
    ...
    `benefit_package_id` BIGINT COMMENT 'Foreign key linking to plan.benefit_package. Business justification: Accumulator limits (deductible_limit_amt, moop_limit_amt) are defined at the benefit_package level. Multi-package plans require benefit_accumulator to reference the specific package to apply correct t',
    `eligibility_span_id` BIGINT COMMENT 'Foreign key linking to enrollment.enrollment_eligibility_span. Business justification: Benefit accumulators (deductible, OOP, MOOP) are scoped to a members eligibility span — reset dates, benefit period, and coverage category on the accumulator must align with the governing eligibility',
    `formulary_id` BIGINT COMMENT 'Foreign key linking to pharmacy.formulary. Business justification: benefit_accumulator carries formulary_tier_applied as a denormalized STRING. Accumulator balances are tracked per benefit period and are formulary-specific (different formularies have different cost-s',
    ...
    `medicare_entitlement_id` BIGINT COMMENT 'Foreign key linking to enrollment.medicare_entitlement. Business justification: Medicare Part D accumulators (TrOOP, ICL, catastrophic phase) are exclusively governed by Medicare entitlement. benefit_accumulator carries part_d_benefit_phase, troop_applied_amt, icl_applied_amt — a',
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: CMS Part D payment reconciliation and LIS tier assignment (lis_level, lis_copay_tier, part_d_benefit_phase on benefit_accumulator) are directly driven by member risk scores. CMS requires linking accum',
    ...
    `plan_election_id` BIGINT COMMENT 'Foreign key linking to enrollment.plan_election. Business justification: Accumulators are scoped to a members specific plan election — the elected plan determines deductible, OOP, and MOOP limits. Mid-year plan changes via QLE require new accumulator records per election.',
    `policy_id` BIGINT COMMENT 'Foreign key linking to member.policy. Business justification: Accumulators (deductible, OOP, TROOP) are policy-specific. COB scenarios require per-policy accumulator tracking. Annual reset logic and CMS Part D reconciliation depend on policy linkage. benefit_acc',
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_pricing` ADD CONSTRAINT `fk_pharmacy_drug_pricing_formulary_id` FOREIGN KEY (`formulary_id`) REFERENCES `vibe_health_insurance_v1`.`pharmacy`.`formulary`(`formulary_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ADD CONSTRAINT `fk_pharmacy_pharmacy_claim_formulary_id` FOREIGN KEY (`formulary_id`) REFERENCES `vibe_health_insurance_v1`.`pharmacy`.`formulary`(`formulary_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`claim_line` ADD CONSTRAINT `fk_pharmacy_claim_line_formulary_drug_tier_id` FOREIGN KEY (`formulary_drug_tier_id`) REFERENCES `vibe_health_insurance_v1`.`pharmacy`.`formulary_drug_tier`(`formulary_drug_tier_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` ADD CONSTRAINT `fk_pharmacy_dispensing_pharmacy_pbm_contract_id` FOREIGN KEY (`pbm_contract_id`) REFERENCES `vibe_health_insurance_v1`.`pharmacy`.`pbm_contract`(`pbm_contract_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ADD CONSTRAINT `fk_pharmacy_benefit_accumulator_formulary_id` FOREIGN KEY (`formulary_id`) REFERENCES `vibe_health_insurance_v1`.`pharmacy`.`formulary`(`formulary_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_master` ALTER COLUMN `hcc_mapping_id` SET TAGS ('dbx_business_glossary_term' = 'Hcc Mapping Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_pricing` ALTER COLUMN `formulary_id` SET TAGS ('dbx_business_glossary_term' = 'Formulary Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`drug_pricing` ALTER COLUMN `pbm_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Pbm Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` SET TAGS ('dbx_subdomain' = 'claim_benefits');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ALTER COLUMN `benefit_package_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Package Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ALTER COLUMN `cost_share_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Share Rule Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ALTER COLUMN `formulary_id` SET TAGS ('dbx_business_glossary_term' = 'Formulary Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pharmacy_claim` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Prescriber Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`claim_line` SET TAGS ('dbx_subdomain' = 'claim_benefits');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`claim_line` ALTER COLUMN `formulary_drug_tier_id` SET TAGS ('dbx_business_glossary_term' = 'Formulary Drug Tier Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`prior_authorization` SET TAGS ('dbx_subdomain' = 'claim_benefits');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`prior_authorization` ALTER COLUMN `hcc_mapping_id` SET TAGS ('dbx_business_glossary_term' = 'Hcc Mapping Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` SET TAGS ('dbx_subdomain' = 'formulary_management');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` ALTER COLUMN `network_config_id` SET TAGS ('dbx_business_glossary_term' = 'Network Config Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Party Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`dispensing_pharmacy` ALTER COLUMN `pbm_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Pbm Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pbm_contract` SET TAGS ('dbx_subdomain' = 'formulary_management');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`pbm_contract` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Party Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` SET TAGS ('dbx_subdomain' = 'claim_benefits');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `benefit_package_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Package Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `eligibility_span_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Eligibility Span Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `formulary_id` SET TAGS ('dbx_business_glossary_term' = 'Formulary Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `medicare_entitlement_id` SET TAGS ('dbx_business_glossary_term' = 'Medicare Entitlement Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `plan_election_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Election Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`pharmacy`.`benefit_accumulator` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
