...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`employee` (
    ...
    `benefit_plan_id` BIGINT COMMENT 'Foreign key linking to workforce.benefit_plan. Business justification: An employee is enrolled in a primary benefit plan. The employee table has no benefit plan linkage despite benefit_plan being a core workforce product. This FK captures the employees primary/default b',
    ...
    `payroll_group_id` BIGINT COMMENT 'Foreign key linking to workforce.payroll_group. Business justification: Every employee belongs to a payroll group that governs their pay frequency, currency, tax treatment, and benefit eligibility. Without this FK, the employee record has no direct link to payroll group —',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`position` (
    ...
    `workforce_department_id` BIGINT COMMENT 'Foreign key linking to workforce.workforce_department. Business justification: A position is defined within a specific department (e.g., Line Cook belongs to BOH, Server belongs to FOH). The position table currently stores department as a denormalized STRING. Adding workforce_de',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`schedule` (
    ...
    `labor_forecast_id` BIGINT COMMENT 'Foreign key linking to workforce.labor_forecast. Business justification: A labor schedule is built to fulfill a labor forecast (projected demand by daypart and week). Linking schedule to labor_forecast enables variance analysis between forecasted labor demand and the actua',
    ...
    `scheduling_template_id` BIGINT COMMENT 'Foreign key linking to workforce.scheduling_template. Business justification: A published schedule is generated from a scheduling template that defines standard shift patterns, FTE targets, and daypart configurations. Without this FK, there is no traceability from a published s',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`payroll_record` (
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Financial cost‑center to which payroll costs are charged.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` (
    ...
    `position_id` BIGINT COMMENT 'Foreign key linking to workforce.position. Business justification: A recruitment requisition is opened to fill a specific authorized position. The recruitment table currently stores position_title as a denormalized STRING. Adding position_id FK normalizes this to the',
    ...
    `workforce_department_id` BIGINT COMMENT 'Foreign key linking to workforce.workforce_department. Business justification: A recruitment requisition is opened for a specific department. The recruitment table currently stores department as a denormalized STRING. Adding workforce_department_id FK normalizes this to the auth',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` (
    ...
    `recruitment_id` BIGINT COMMENT 'Foreign key linking to workforce.recruitment. Business justification: Onboarding is the direct continuation of the recruitment lifecycle — a new hire who was recruited goes through onboarding. Linking onboarding to the originating recruitment record enables end-to-end t',
    ...
    `method` STRING COMMENT 'Mode used to deliver onboarding activities.. Valid values are `in_person|virtual|hybrid`',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` (
    ...
    `training_employee_id` BIGINT COMMENT 'Unique identifier of the trainer or facilitator.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` (
    ...
    `position_id` BIGINT COMMENT 'Foreign key linking to workforce.position. Business justification: A performance review evaluates an employee in the context of their current position. The performance_review table has no position_id FK despite having position-specific scoring criteria (food_safety_s',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` (
    `labor_budget_id` DECIMAL(18,2) COMMENT 'System-generated unique identifier for each labor budget record.',
    ...
    `workforce_department_id` BIGINT COMMENT 'Foreign key linking to workforce.workforce_department. Business justification: Labor budgets are set at the department level (BOH labor budget vs. FOH labor budget) in addition to the restaurant unit level. The labor_budget table already has unit-level FKs but no department FK. ',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` (
    `tip_compliance_id` DECIMAL(18,2) COMMENT 'Unique identifier for the tip_compliance data product (auto-inserted pre-linking).',
    `corrected_tip_compliance_id` DECIMAL(18,2) COMMENT 'Self-referencing FK on tip_compliance (corrected_tip_compliance_id)',
    ...
    `allocated_tips` DECIMAL(18,2) COMMENT '',
    `allocated_tips_amount` DECIMAL(18,2) COMMENT '',
    `calculated_at` TIMESTAMP COMMENT '',
    `cash_tips` DECIMAL(18,2) COMMENT '',
    `compliance_status` STRING COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `credit_card_tips` DECIMAL(18,2) COMMENT '',
    `declared_tips` DECIMAL(18,2) COMMENT '',
    `declared_tips_amount` DECIMAL(18,2) COMMENT 'Business attribute declared_tips_amount derived from business context.',
    `is_compliant` BOOLEAN COMMENT '',
    `minimum_wage_makeup` DECIMAL(18,2) COMMENT 'Business attribute minimum_wage_makeup for tip_compliance',
    `minimum_wage_shortfall` STRING COMMENT '',
    `minimum_wage_shortfall_amount` DECIMAL(18,2) COMMENT 'Shortfall vs minimum wage',
    `pay_period_end` DATE COMMENT '',
    `pay_period_end_date` DATE COMMENT 'Pay period end',
    `pay_period_start` DATE COMMENT '',
    `pay_period_start_date` DATE COMMENT 'Pay period start',
    `period_end` STRING COMMENT '',
    `period_end_date` DATE COMMENT '',
    `period_start` STRING COMMENT '',
    `period_start_date` DATE COMMENT '',
    `reported_tips` STRING COMMENT '',
    `reported_tips_amount` DECIMAL(18,2) COMMENT '',
    ...
    `reviewed_at` TIMESTAMP COMMENT '',
    `reviewed_by` STRING COMMENT 'Reviewer name',
    `reviewed_date` DATE COMMENT '',
    `reviewer_name` STRING COMMENT '',
    `shortfall_amount` DECIMAL(18,2) COMMENT 'Business attribute shortfall_amount for workforce.tip_compliance',
    `source_system_code` STRING COMMENT '',
    `tip_credit_amount` DECIMAL(18,2) COMMENT '',
    `tips_allocated_amount` DECIMAL(18,2) COMMENT '',
    `tips_reported_amount` DECIMAL(18,2) COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `violation_reason` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` (
    ...
    `position_id` BIGINT COMMENT 'Foreign key linking to workforce.position. Business justification: A scheduling template defines shift patterns for a specific position or role (e.g., a Cashier template vs. a Kitchen Manager template). The scheduling_template table currently stores role_applicabilit',
    `workforce_department_id` BIGINT COMMENT 'Foreign key linking to workforce.workforce_department. Business justification: A scheduling template is scoped to a specific department (e.g., BOH template vs. FOH template). The scheduling_template table has location_scope STRING but no department FK. Adding workforce_departmen',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` (
    `workforce_department_id` BIGINT COMMENT 'Primary key for department',
    ...
    `secondary_canonical_department_id` BIGINT COMMENT 'Canonical reference to authoritative restaurant.department (SSOT).',
...
) COMMENT 'Master reference table for department. Referenced by department_id. [SSOT] Non-authoritative duplicate of restaurant.department (concept department); authoritative SSOT is restaurant.department. SSOT: authoritative source is restaurant.department.';
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` (
    ...
    `total_employee_count` DECIMAL(18,2) COMMENT 'Number of employees included in this payroll run.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` (
    ...
    `replaces_benefit_plan_id` BIGINT COMMENT 'Self-referencing FK on benefit_plan (replaces_benefit_plan_id)',
    `carrier_name` STRING COMMENT 'Name of the insurance carrier or plan administrator providing this benefit (e.g., Aetna, BlueCross BlueShield, Fidelity, MetLife). Used for carrier invoicing reconciliation and employee ID card generation.',
    `carrier_plan_code` STRING COMMENT 'The insurance carriers own plan identification number or group number assigned to this benefit plan. Required for claims processing, EOB reconciliation, and carrier EDI transactions.',
    `cobra_eligible_flag` BOOLEAN COMMENT 'Indicates whether this benefit plan is subject to COBRA continuation coverage requirements. Applies to employers with 20+ employees. Drives COBRA notice generation and premium calculation upon qualifying events.',
    `cobra_premium_pct` DECIMAL(18,2) COMMENT 'Percentage of total premium charged to COBRA participants (typically 102% of total premium: 100% cost + 2% administrative fee). Used in COBRA billing calculations.',
    `coinsurance_pct` DECIMAL(18,2) COMMENT 'Percentage of covered medical expenses the employee pays after meeting the deductible (e.g., 20.00 means employee pays 20%, plan pays 80%). Used in employee benefits communications and cost modeling.',
    `copay_primary_care_amount` DECIMAL(18,2) COMMENT 'Fixed dollar copay amount an employee pays per primary care physician visit. Applicable to HMO and PPO medical plans.',
    `copay_specialist_amount` DECIMAL(18,2) COMMENT 'Fixed dollar copay amount an employee pays per specialist physician visit.',
    `coverage_tier` STRING COMMENT 'Enrollment tier indicating who is covered under this benefit plan option. Determines premium amounts and dependent eligibility. Common tiers in restaurant industry: employee only, employee + spouse, employee + children, employee + family.',
    `deductible_family_amount` DECIMAL(18,2) COMMENT 'Annual out-of-pocket deductible amount for a family enrollment tier before the plan begins paying covered expenses.',
    `deductible_individual_amount` DECIMAL(18,2) COMMENT 'Annual out-of-pocket deductible amount for an individual enrollee before the plan begins paying covered expenses. Applicable to medical and dental plans.',
    `disability_benefit_pct` DECIMAL(18,2) COMMENT 'For short-term or long-term disability plans, the percentage of pre-disability base salary paid as a benefit (e.g., 60.00 = 60% of salary). Null for non-disability plan types.',
    `disability_max_weekly_benefit` DECIMAL(18,2) COMMENT 'Maximum weekly dollar benefit payable under a disability plan, regardless of the salary percentage calculation. Null for non-disability plan types.',
    `effective_date` DATE COMMENT 'Date on which this benefit plan becomes active and available for employee enrollment. Marks the start of the plan year or the date a new plan was introduced.',
    `eligibility_class` STRING COMMENT 'Employee classification tier that qualifies for this benefit plan. Restaurant operators commonly differentiate between full-time (30+ hrs/week), part-time, management, and executive tiers per ACA and ERISA nondiscrimination rules.',
    `employee_premium_amount` DECIMAL(18,2) COMMENT 'Per-pay-period dollar amount deducted from the employees paycheck for this benefit plan (employee share of premium). Used in payroll integration with Workday HCM for pre-tax or post-tax deduction setup.',
    `employer_hsa_contribution_amount` DECIMAL(18,2) COMMENT 'Annual dollar amount the employer seeds into the employees Health Savings Account (HSA) for HDHP-enrolled employees. Used in total compensation reporting and payroll integration.',
    `employer_premium_amount` DECIMAL(18,2) COMMENT 'Per-pay-period dollar amount contributed by the employer toward this benefit plan. Used for labor cost modeling, benefits cost reporting, and ACA affordability calculations.',
    `erisa_plan_number` STRING COMMENT 'Three-digit plan number assigned by the employer for ERISA reporting purposes, used on DOL Form 5500 filings. Welfare benefit plans typically use numbers 501–999.',
    `expiration_date` DATE COMMENT 'Date on which this benefit plan expires or is discontinued. Null for open-ended plans. Used to manage plan year transitions and open enrollment windows.',
    `fsa_eligible_flag` BOOLEAN COMMENT 'Indicates whether employees enrolled in this plan may also participate in a Flexible Spending Account (FSA) for healthcare or dependent care expenses.',
    `funding_type` STRING COMMENT 'Indicates how the benefit plan is financially structured. Fully insured plans transfer risk to the carrier; self-funded plans retain risk with the employer. Impacts stop-loss insurance requirements and DOL Form 5500 reporting.',
    `group_number` STRING COMMENT 'Employer group number assigned by the insurance carrier, printed on employee insurance cards and used for claims submission and provider verification.',
    `hsa_eligible_flag` BOOLEAN COMMENT 'Indicates whether this plan qualifies as a High-Deductible Health Plan (HDHP) making enrollees eligible to open and contribute to a Health Savings Account (HSA). Drives IRS contribution limit enforcement.',
    `is_aca_affordable` BOOLEAN COMMENT 'Indicates whether this plan meets the ACA affordability threshold (employee premium does not exceed the IRS-published percentage of household income). Critical for avoiding ACA employer shared responsibility penalties.',
    `is_aca_minimum_essential` BOOLEAN COMMENT 'Indicates whether this plan qualifies as Minimum Essential Coverage (MEC) under the Affordable Care Act. Required for ACA employer mandate compliance reporting on IRS Forms 1094-C and 1095-C.',
    `is_aca_minimum_value` BOOLEAN COMMENT 'Indicates whether this plan meets the ACA Minimum Value standard (covers at least 60% of total allowed costs). Required for ACA affordability and employer shared responsibility compliance.',
    `life_insurance_benefit_multiple` DECIMAL(18,2) COMMENT 'For life insurance plans, the multiple of annual salary used to calculate the death benefit (e.g., 1.0 = 1x salary, 2.0 = 2x salary). Null for non-life-insurance plan types.',
    `life_insurance_max_benefit_amount` DECIMAL(18,2) COMMENT 'Maximum dollar cap on the life insurance death benefit regardless of salary multiple. Amounts above IRS Section 79 threshold ($50,000) trigger imputed income calculations.',
    `network_type` STRING COMMENT 'Indicates the provider network scope for this plan. Relevant for restaurant employees who may work across multiple geographic locations and need portable coverage.',
    `notes` STRING COMMENT 'Free-text field for HR administrators to capture plan-specific notes, special eligibility conditions, carrier contact information, or renewal negotiation details not captured in structured fields.',
    `open_enrollment_end_date` DATE COMMENT 'Date on which the annual open enrollment window closes. Elections made after this date require a qualifying life event (QLE) to be processed.',
    `open_enrollment_start_date` DATE COMMENT 'Date on which the annual open enrollment window opens for this benefit plan, allowing employees to elect, change, or waive coverage for the upcoming plan year.',
    `out_of_pocket_max_family` DECIMAL(18,2) COMMENT 'Maximum annual out-of-pocket cost for a family enrollment tier, after which the plan covers 100% of covered expenses.',
    `out_of_pocket_max_individual` DECIMAL(18,2) COMMENT 'Maximum annual out-of-pocket cost for an individual enrollee, after which the plan covers 100% of covered expenses. ACA mandates annual limits for essential health benefits.',
    `plan_code` STRING COMMENT 'Externally-known alphanumeric code uniquely identifying the benefit plan, used in HR systems, payroll integration, and employee communications (e.g., MED-PPO-FULL, DEN-BASIC-PT).',
    `plan_name` STRING COMMENT 'Human-readable name of the benefit plan as displayed to employees during open enrollment and in HR portals (e.g., Full-Time Medical PPO Gold, Part-Time Dental Basic).',
    `plan_status` STRING COMMENT 'Current lifecycle state of the benefit plan. Active plans are available for enrollment; discontinued plans retain historical enrollments but accept no new members; grandfathered plans are ACA-exempt legacy offerings.',
    `plan_subtype` STRING COMMENT 'Further classification within the plan type, such as PPO vs HMO for medical, or term vs whole for life insurance. Determines network access, cost-sharing structure, and IRS contribution limits.',
    `plan_type` STRING COMMENT 'Category of benefit covered by this plan. Drives eligibility rules, tax treatment, and payroll deduction logic. [ENUM-REF-CANDIDATE: medical|dental|vision|life_insurance|disability|401k|fsa|hsa|commuter|supplemental|eap|pet|legal — promote to reference product]',
    `plan_year_start_month` STRING COMMENT 'Calendar month (1–12) in which the plan year begins. Determines open enrollment timing, FSA/HSA contribution reset dates, and deductible accumulation periods.',
    `premium_frequency` STRING COMMENT 'Frequency at which premium deductions are taken from employee paychecks. Must align with the restaurants payroll cycle configured in Workday HCM.',
    `qualifying_life_event_allowed` BOOLEAN COMMENT 'Indicates whether mid-year enrollment changes are permitted upon a qualifying life event (e.g., marriage, birth, loss of other coverage). Required for HIPAA special enrollment compliance.',
    `retirement_employer_match_pct` DECIMAL(18,2) COMMENT 'For 401(k) or retirement plans, the percentage of employee contributions matched by the employer up to the match cap (e.g., 50.00 = employer matches 50 cents per dollar contributed). Null for non-retirement plans.',
    `retirement_match_cap_pct` DECIMAL(18,2) COMMENT 'For 401(k) plans, the maximum percentage of employee compensation up to which the employer match applies (e.g., 6.00 = employer matches on first 6% of salary contributed). Null for non-retirement plans.',
    `summary_plan_description_url` STRING COMMENT 'URL or document path to the ERISA-required Summary Plan Description document that must be distributed to plan participants. Required to be provided within 90 days of enrollment.',
    `tax_treatment` STRING COMMENT 'Indicates whether employee premium contributions are deducted on a pre-tax basis (Section 125 cafeteria plan), post-tax basis, or a combination. Drives payroll deduction coding in Workday HCM.',
    `total_premium_amount` DECIMAL(18,2) COMMENT 'Total per-pay-period premium cost for this benefit plan (employee + employer contributions combined). Used for carrier invoice reconciliation and benefits cost benchmarking.',
    `vesting_cliff_months` STRING COMMENT 'Number of months of service required before any employer contributions vest under a cliff vesting schedule. Null for immediate or graded vesting schedules.',
    `vesting_schedule_type` STRING COMMENT 'Type of vesting schedule applied to employer contributions for retirement plans. Immediate vesting is common in restaurant industry to attract hourly workers; cliff and graded vesting reward tenure.',
    `waiting_period_days` STRING COMMENT 'Number of calendar days an employee must be employed before becoming eligible to enroll in this benefit plan. ACA limits medical waiting periods to 90 days maximum.',
    `workday_benefit_plan_code` STRING COMMENT 'The benefit plan identifier as configured in Workday HCM, used for payroll deduction mapping, benefits administration, and ACA reporting integration. Enables reconciliation between the data lakehouse and the system of record.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`employee` ADD CONSTRAINT `fk_workforce_employee_benefit_plan_id` FOREIGN KEY (`benefit_plan_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`benefit_plan`(`benefit_plan_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`schedule` ADD CONSTRAINT `fk_workforce_schedule_scheduling_template_id` FOREIGN KEY (`scheduling_template_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`scheduling_template`(`scheduling_template_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ADD CONSTRAINT `fk_workforce_onboarding_recruitment_id` FOREIGN KEY (`recruitment_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`recruitment`(`recruitment_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ADD CONSTRAINT `fk_workforce_scheduling_template_position_id` FOREIGN KEY (`position_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`position`(`position_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ADD CONSTRAINT `fk_workforce_workforce_department_secondary_canonical_department_id` FOREIGN KEY (`secondary_canonical_department_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`workforce_department`(`workforce_department_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ADD CONSTRAINT `fk_workforce_benefit_plan_replaces_benefit_plan_id` FOREIGN KEY (`replaces_benefit_plan_id`) REFERENCES `vibe_restaurants_v1`.`workforce`.`benefit_plan`(`benefit_plan_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`employee` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`employee` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`employee` ALTER COLUMN `benefit_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Plan Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`employee` ALTER COLUMN `payroll_group_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Group Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`position` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`position` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`position` ALTER COLUMN `workforce_department_id` SET TAGS ('dbx_business_glossary_term' = 'Workforce Department Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`shift` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`shift` ALTER COLUMN `station` SET TAGS ('dbx_business_glossary_term' = 'Work Station');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`shift` ALTER COLUMN `station` SET TAGS ('dbx_value_regex' = 'BOH|FOH|drive_thru|delivery');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`schedule` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`schedule` ALTER COLUMN `scheduling_template_id` SET TAGS ('dbx_business_glossary_term' = 'Scheduling Template Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`time_entry` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`time_entry` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Location Identifier (LOC_ID)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_forecast` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_forecast` ALTER COLUMN `lto_flag` SET TAGS ('dbx_business_glossary_term' = 'Limited Time Offer Flag');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_forecast` ALTER COLUMN `promotion_flag` SET TAGS ('dbx_business_glossary_term' = 'Promotion Flag');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_record` SET TAGS ('dbx_subdomain' = 'payroll_administration');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_record` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_record` ALTER COLUMN `other_deductions` SET TAGS ('dbx_business_glossary_term' = 'Other Deductions Amount');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `position_id` SET TAGS ('dbx_business_glossary_term' = 'Position Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `workforce_department_id` SET TAGS ('dbx_business_glossary_term' = 'Workforce Department Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `job_level` SET TAGS ('dbx_business_glossary_term' = 'Job Level');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `recruitment_status` SET TAGS ('dbx_business_glossary_term' = 'Recruitment Status');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `recruitment_status` SET TAGS ('dbx_value_regex' = 'open|closed|on_hold|cancelled|filled');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `required_certifications` SET TAGS ('dbx_business_glossary_term' = 'Required Certifications');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `required_education_level` SET TAGS ('dbx_business_glossary_term' = 'Required Education Level');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`recruitment` ALTER COLUMN `required_education_level` SET TAGS ('dbx_value_regex' = 'high_school|associate|bachelor|master|doctorate');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `primary_onboarding_assigned_buddy_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `recruitment_id` SET TAGS ('dbx_business_glossary_term' = 'Recruitment Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `tertiary_onboarding_manager_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Manager ID');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `tertiary_onboarding_manager_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `tertiary_onboarding_manager_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `method` SET TAGS ('dbx_value_regex' = 'in_person|virtual|hybrid');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Notes');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `onboarding_status` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Status (NOT_STARTED, IN_PROGRESS, COMPLETED, CANCELLED)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `onboarding_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|cancelled');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `onboarding_type` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Type (NEW_HIRE, TRANSFER, SEASONAL, INTERN)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `onboarding_type` SET TAGS ('dbx_value_regex' = 'new_hire|transfer|seasonal|intern');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `pos_access_provision_date` SET TAGS ('dbx_business_glossary_term' = 'POS Access Provision Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `pos_access_provisioned` SET TAGS ('dbx_business_glossary_term' = 'POS Access Provisioned Flag');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `record_audit_created` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `record_audit_updated` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `servsafe_completion_date` SET TAGS ('dbx_business_glossary_term' = 'ServSafe Completion Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `servsafe_enrolled` SET TAGS ('dbx_business_glossary_term' = 'ServSafe Enrolled Flag');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Start Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `training_completed` SET TAGS ('dbx_business_glossary_term' = 'Training Completed Flag');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`onboarding` ALTER COLUMN `training_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Timestamp');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`certification` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`certification` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` ALTER COLUMN `training_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Trainer Identifier');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` ALTER COLUMN `training_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`training_completion` ALTER COLUMN `training_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `position_id` SET TAGS ('dbx_business_glossary_term' = 'Position Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `actual_hours` SET TAGS ('dbx_business_glossary_term' = 'Actual Worked Hours');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `attendance_score` SET TAGS ('dbx_business_glossary_term' = 'Attendance Score');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Status (Valid, Expired, Pending)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `certification_status` SET TAGS ('dbx_value_regex' = 'valid|expired|pending');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Comments and Feedback');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `competency_score_total` SET TAGS ('dbx_business_glossary_term' = 'Total Competency Score');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `confidentiality_level` SET TAGS ('dbx_business_glossary_term' = 'Confidentiality Level of Review Record');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `labor_percentage_actual` SET TAGS ('dbx_business_glossary_term' = 'Actual Labor Cost Percentage');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `labor_percentage_target` SET TAGS ('dbx_business_glossary_term' = 'Target Labor Cost Percentage');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `next_review_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Due Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `overall_rating` SET TAGS ('dbx_business_glossary_term' = 'Overall Rating Score');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `overtime_hours` SET TAGS ('dbx_business_glossary_term' = 'Overtime Hours Worked');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `performance_review_status` SET TAGS ('dbx_business_glossary_term' = 'Review Status');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `performance_review_status` SET TAGS ('dbx_value_regex' = 'draft|completed|cancelled|in_progress');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `promotion_recommendation` SET TAGS ('dbx_business_glossary_term' = 'Promotion Recommendation Flag');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `rating_scale` SET TAGS ('dbx_business_glossary_term' = 'Rating Scale (e.g., 1‑5, 1‑10, Percentage)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `rating_scale` SET TAGS ('dbx_value_regex' = '1-5|1-10|percentage');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date and Time');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_document_url` SET TAGS ('dbx_business_glossary_term' = 'URL to Review Document');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_method` SET TAGS ('dbx_business_glossary_term' = 'Review Method (Self, Manager, 360‑degree, Peer)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_method` SET TAGS ('dbx_value_regex' = 'self|manager|360|peer');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_number` SET TAGS ('dbx_business_glossary_term' = 'Performance Review Number (PRN)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_period_end` SET TAGS ('dbx_business_glossary_term' = 'Review Period End Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_period_start` SET TAGS ('dbx_business_glossary_term' = 'Review Period Start Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_type` SET TAGS ('dbx_business_glossary_term' = 'Review Type (e.g., 90‑Day, Annual, Promotion, Probation, Exit)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `review_type` SET TAGS ('dbx_value_regex' = '90_day|annual|promotion|probation|exit');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `scheduled_hours` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Work Hours');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `shift_daypart` SET TAGS ('dbx_business_glossary_term' = 'Shift Daypart (Breakfast, Lunch, Dinner, Late Night)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `shift_daypart` SET TAGS ('dbx_value_regex' = 'breakfast|lunch|dinner|late_night');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`performance_review` ALTER COLUMN `speed_score` SET TAGS ('dbx_business_glossary_term' = 'Speed Competency Score');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`leave_request` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`leave_request` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` SET TAGS ('dbx_subdomain' = 'compliance_tracking');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `actual_break_time` SET TAGS ('dbx_business_glossary_term' = 'Actual Break Time');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `compliance_reported` SET TAGS ('dbx_business_glossary_term' = 'Compliance Reported Flag');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `corrective_action_due_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Due Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `corrective_action_taken` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Taken');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `daypart` SET TAGS ('dbx_business_glossary_term' = 'Daypart of Violation Occurrence');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `daypart` SET TAGS ('dbx_value_regex' = 'breakfast|lunch|dinner|late_night');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `labor_violation_description` SET TAGS ('dbx_business_glossary_term' = 'Violation Description');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `detection_method` SET TAGS ('dbx_business_glossary_term' = 'Detection Method');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `detection_method` SET TAGS ('dbx_value_regex' = 'automated|manual');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `resolved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Violation Resolved Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `scheduled_break_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Break Time');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `severity` SET TAGS ('dbx_business_glossary_term' = 'Violation Severity Level');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `severity` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `violation_code` SET TAGS ('dbx_business_glossary_term' = 'Standardized Violation Code');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_violation` ALTER COLUMN `violation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Violation Occurrence Timestamp');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `workforce_department_id` SET TAGS ('dbx_business_glossary_term' = 'Workforce Department Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `daypart` SET TAGS ('dbx_business_glossary_term' = 'Daypart');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `daypart` SET TAGS ('dbx_value_regex' = 'breakfast|lunch|dinner|late_night|all_day');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `fte_budget_boh` SET TAGS ('dbx_business_glossary_term' = 'Back‑of‑House FTE Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `fte_budget_foh` SET TAGS ('dbx_business_glossary_term' = 'Front‑of‑House FTE Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `fte_budget_total` SET TAGS ('dbx_business_glossary_term' = 'Total FTE Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `hours_budget_boh` SET TAGS ('dbx_business_glossary_term' = 'Back‑of‑House Hours Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `hours_budget_foh` SET TAGS ('dbx_business_glossary_term' = 'Front‑of‑House Hours Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `hours_budget_total` SET TAGS ('dbx_business_glossary_term' = 'Total Labor Hours Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_cost_estimate` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Estimate');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_cost_estimate_boh` SET TAGS ('dbx_business_glossary_term' = 'Back‑of‑House Labor Cost Estimate');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_cost_estimate_foh` SET TAGS ('dbx_business_glossary_term' = 'Front‑of‑House Labor Cost Estimate');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_dollar_budget` SET TAGS ('dbx_business_glossary_term' = 'Labor Dollar Budget');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_percent_target` SET TAGS ('dbx_business_glossary_term' = 'Target Labor Percentage');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_percent_target_boh` SET TAGS ('dbx_business_glossary_term' = 'Back‑of‑House Labor Percentage Target');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_percent_target_foh` SET TAGS ('dbx_business_glossary_term' = 'Front‑of‑House Labor Percentage Target');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Budget Notes');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Period End Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Period Start Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `scenario` SET TAGS ('dbx_business_glossary_term' = 'Budget Scenario');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`labor_budget` ALTER COLUMN `scenario` SET TAGS ('dbx_value_regex' = 'base|optimistic|pessimistic|promotion|lto');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` SET TAGS ('dbx_subdomain' = 'compliance_tracking');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `credit_card_tips` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `minimum_wage_makeup` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `minimum_wage_makeup` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `minimum_wage_shortfall_amount` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `minimum_wage_shortfall_amount` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`tip_compliance` ALTER COLUMN `reviewer_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ALTER COLUMN `position_id` SET TAGS ('dbx_business_glossary_term' = 'Position Id (Foreign Key)');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ALTER COLUMN `workforce_department_id` SET TAGS ('dbx_business_glossary_term' = 'Workforce Department Id (Foreign Key)');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`scheduling_template` ALTER COLUMN `fte_target` SET TAGS ('dbx_business_glossary_term' = 'Fte Target');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_subdomain' = 'compliance_tracking');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_superseded_by' = 'restaurant.department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_role' = 'duplicate');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_authoritative_owner' = 'restaurant.department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_owner_for' = 'department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot' = 'owner');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_concept' = 'department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_alias_of' = 'restaurant.department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_ssot_owner' = 'department');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `secondary_canonical_department_id` SET TAGS ('dbx_ssot_canonical_ref' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `secondary_canonical_department_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `department_type` SET TAGS ('dbx_business_glossary_term' = 'Department Type');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `workforce_department_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `floor_area_sqft` SET TAGS ('dbx_business_glossary_term' = 'Floor Area Sqft');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `headcount` SET TAGS ('dbx_business_glossary_term' = 'Headcount');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`workforce_department` ALTER COLUMN `is_remote` SET TAGS ('dbx_business_glossary_term' = 'Is Remote');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` SET TAGS ('dbx_subdomain' = 'payroll_administration');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `batch_number` SET TAGS ('dbx_business_glossary_term' = 'Batch Number');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `deductions_amount` SET TAGS ('dbx_business_glossary_term' = 'Deductions Amount');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_run_description` SET TAGS ('dbx_business_glossary_term' = 'Payroll Run Description');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `gross_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Amount');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `is_finalized` SET TAGS ('dbx_business_glossary_term' = 'Is Finalized');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `net_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Amount');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_cycle` SET TAGS ('dbx_business_glossary_term' = 'Pay Cycle');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period End Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Start Date');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_type` SET TAGS ('dbx_business_glossary_term' = 'Payroll Type');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `record_audit_created` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `record_audit_updated` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `run_number` SET TAGS ('dbx_business_glossary_term' = 'Run Number');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `run_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Run Timestamp');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_run_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_group` SET TAGS ('dbx_subdomain' = 'payroll_administration');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_group` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_group` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`payroll_group` ALTER COLUMN `group_code` SET TAGS ('dbx_business_glossary_term' = 'Group Code');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` SET TAGS ('dbx_subdomain' = 'payroll_administration');
...
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `replaces_benefit_plan_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `deductible_family_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `deductible_individual_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `disability_benefit_pct` SET TAGS ('dbx_classification' = 'confidential');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `disability_benefit_pct` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `disability_benefit_pct` SET TAGS ('dbx_sensitivity' = 'high');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `disability_max_weekly_benefit` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `employee_premium_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `employer_hsa_contribution_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `employer_premium_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `life_insurance_max_benefit_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `out_of_pocket_max_family` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `out_of_pocket_max_individual` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`workforce`.`benefit_plan` ALTER COLUMN `total_premium_amount` SET TAGS ('dbx_confidential' = 'true');
