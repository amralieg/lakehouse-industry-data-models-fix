...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`supplychain`.`replenishment_plan` (
    ...
    `promo_campaign_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_campaign. Business justification: Promotional inventory builds are planned against specific campaigns (campaign dates, scope, target units). The existing `promotion_flag` boolean is insufficient — buyers need the campaign_id to size t',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`supplychain`.`demand_forecast` (
    ...
    `cluster_id` BIGINT COMMENT 'Foreign key linking to store.cluster. Business justification: Cluster-level demand forecasting is standard retail practice — similar stores are grouped for statistical lift and new-item forecasting. A cluster_id FK on demand_forecast enables cluster-level foreca',
    ...
    `price_list_id` BIGINT COMMENT 'Foreign key linking to pricing.price_list. Business justification: Demand forecasts are channel and price-list specific — forecasted_revenue depends on which price_list was active. Linking demand_forecast to price_list supports price-elasticity modeling, promotional ',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`supplychain`.`outbound_order_line` (
    ...
    `department_id` BIGINT COMMENT 'Foreign key linking to store.department. Business justification: Outbound order lines represent store replenishment by SKU; linking to store department enables department-level fill-rate reporting, shrinkage analysis, and replenishment performance tracking — a stan',
