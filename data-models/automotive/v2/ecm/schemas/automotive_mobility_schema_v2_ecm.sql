...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` (
    ...
    `aftersales_nameplate_id` BIGINT COMMENT 'Foreign key linking to aftersales.nameplate. Business justification: OTA campaign planning requires linking each connected vehicle to its product nameplate to apply correct software version and compliance reporting.',
    ...
    `subscription_active_flag` BOOLEAN COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`telematics_device` (
    ...
    `subscription_context_flag` BOOLEAN COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`service` (
    ...
    `aftersales_nameplate_id` BIGINT COMMENT 'Foreign key linking to aftersales.nameplate. Business justification: Service eligibility matrix links services to specific vehicle nameplates for subscription eligibility and regulatory reporting.',
    ...
    `field_service_enabled_flag` BOOLEAN COMMENT 'Whether this mobility service supports field service dispatch integration',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` (
    ...
    `subscription_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` (
    ...
    `speed_kmh` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`trip` (
    ...
    `trip_connected_vehicle_id` BIGINT COMMENT '',
    ...
    `distance_km` DECIMAL(18,2) COMMENT 'Total distance covered during the trip.',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` (
    ...
    `cleared_flag` BOOLEAN COMMENT '',
    `cleared_timestamp` TIMESTAMP COMMENT 'Timestamp when DTC was cleared',
    `created_timestamp` TIMESTAMP COMMENT '',
    `data_quality_flag` BOOLEAN COMMENT '',
    `mobility_dtc_event_description` STRING COMMENT '',
    `detected_timestamp` TIMESTAMP COMMENT '',
    `dtc_code` STRING COMMENT 'Diagnostic trouble code (OBD-II format)',
    `dtc_description` STRING COMMENT '',
    `ecu_source` STRING COMMENT '',
    `event_status` STRING COMMENT '',
    `event_timestamp` TIMESTAMP COMMENT 'Timestamp when DTC was triggered',
    `freeze_frame_data` STRING COMMENT 'Freeze frame data captured at time of DTC',
    `is_active` BOOLEAN COMMENT 'Whether DTC is currently active',
    `mil_illuminated_flag` BOOLEAN COMMENT '',
    `mileage` STRING COMMENT '',
    `mileage_at_detection` STRING COMMENT '',
    `mileage_at_event` STRING COMMENT 'Vehicle mileage when DTC occurred',
    `obd_mode` STRING COMMENT '',
    `severity` STRING COMMENT 'Severity level (informational, warning, critical)',
    `severity_level` STRING COMMENT '',
    `subscription_vehicle_flag` BOOLEAN COMMENT '',
    `system_affected` STRING COMMENT 'Vehicle system affected (engine, transmission, brakes, ADAS)',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `vin` STRING COMMENT '',
    CONSTRAINT pk_mobility_dtc_event PRIMARY KEY(`mobility_dtc_event_id`)
) COMMENT '[FIELD SERVICES COVERAGE] connected-vehicle DTC building block feeding proactive field service triggers | Diagnostic Trouble Code (DTC) event record captured via OBD (On-Board Diagnostics) from connected vehicles. Each record stores the DTC code, SAE/OEM fault description, ECU source module, fault severity level, first occurrence timestamp, last occurrence timestamp, occurrence count, freeze frame data snapshot, and resolution status. Feeds remote diagnostics services and predictive maintenance alert generation. Sourced from Geotab and Bosch IoT OBD data streams. This product is the single source of truth (SSOT) for its concept in the ECM Lakehouse.';
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`predictive_maintenance_alert` (
    ...
    `alert_timestamp` TIMESTAMP COMMENT '',
    `alert_type` STRING COMMENT '',
    ...
    `predicted_failure_date` DATE COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_ota_deployment` (
    ...
    `deployment_status` STRING COMMENT '',
    `deployment_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`tpms_reading` (
    ...
    `data_quality_flag` BOOLEAN COMMENT 'Quality assessment of the TPMS reading after validation checks.',
    ...
    `pressure_kpa` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`geofence` (
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`geofence_event` (
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` (
    ...
    `account_name` STRING COMMENT 'Name of the mobility fleet account',
    `account_number` STRING COMMENT 'Account number',
    `account_status` STRING COMMENT 'Status (active, suspended, closed)',
    `billing_currency_code` STRING COMMENT '',
    ...
    `fleet_account_status` STRING COMMENT '',
    `fleet_size` STRING COMMENT 'Number of vehicles in the fleet',
    `monthly_fee` DECIMAL(18,2) COMMENT 'Monthly fleet management fee',
    `monthly_fee_amount` DECIMAL(18,2) COMMENT '',
    `subscription_sales_channel` STRING COMMENT '',
    `subscription_tier` STRING COMMENT 'Service subscription tier',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `vehicle_count` STRING COMMENT 'Number of vehicles in the fleet',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` (
    ...
    `created_timestamp` TIMESTAMP COMMENT '',
    `driver_name` STRING COMMENT 'Assigned driver name',
    `end_date` DATE COMMENT '',
    `mileage_at_assignment` STRING COMMENT 'Mileage when assigned',
    `mileage_at_return` STRING COMMENT 'Mileage when returned',
    `odometer_at_assignment_km` DECIMAL(18,2) COMMENT '',
    `return_date` DATE COMMENT 'Date vehicle was returned',
    `start_date` DATE COMMENT '',
    `subscription_order_ref` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `usage_type` STRING COMMENT 'Usage type (dedicated, pool, shared)',
    `vin` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`adas_feature_entitlement` (
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`usage_record` (
    ...
    `service_subscription_id` BIGINT COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`remote_command` (
    ...
    `issued_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` (
    ...
    `primary_ev_charger_id` BIGINT COMMENT '',
    `charge_cost_amount` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`service_incident` (
    ...
    `affected_service` STRING COMMENT 'Service affected by the incident',
    `affected_vehicle_count` STRING COMMENT 'Number of vehicles affected',
    `created_timestamp` TIMESTAMP COMMENT '',
    `incident_date` DATE COMMENT '',
    `incident_description` STRING COMMENT '',
    `incident_number` STRING COMMENT '',
    `incident_severity` STRING COMMENT '',
    `incident_status` STRING COMMENT '',
    `incident_timestamp` TIMESTAMP COMMENT 'Timestamp when incident occurred',
    `incident_type` STRING COMMENT 'Type of service incident (outage, degradation, security)',
    `reported_at` TIMESTAMP COMMENT '',
    `reported_timestamp` TIMESTAMP COMMENT '',
    `resolution_notes` STRING COMMENT '',
    `resolution_timestamp` TIMESTAMP COMMENT 'Timestamp when incident was resolved',
    `resolved_at` TIMESTAMP COMMENT '',
    `resolved_timestamp` TIMESTAMP COMMENT '',
    `root_cause` STRING COMMENT 'Root cause of the incident',
    `severity` STRING COMMENT 'Severity (P1-critical, P2-high, P3-medium, P4-low)',
    `severity_level` STRING COMMENT '',
    `service_incident_status` STRING COMMENT 'Status (open, investigating, resolved, closed)',
    `subscription_covered_flag` BOOLEAN COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` (
    ...
    `consent_granted_timestamp` TIMESTAMP COMMENT '',
    `consent_revoked_timestamp` TIMESTAMP COMMENT '',
    `consent_status` STRING COMMENT 'Status (granted, withdrawn, expired)',
    `consent_timestamp` TIMESTAMP COMMENT '',
    `consent_type` STRING COMMENT 'Type of consent (telematics, location, diagnostics, marketing)',
    `consent_version` STRING COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `data_categories` STRING COMMENT 'Categories of data covered by consent',
    `data_subject_email` STRING COMMENT '',
    `expiry_date` DATE COMMENT 'Date consent expires',
    `granted_date` DATE COMMENT 'Date consent was granted',
    `legal_basis` STRING COMMENT 'Legal basis for data processing',
    `opt_in_flag` BOOLEAN COMMENT '',
    `revoked_timestamp` TIMESTAMP COMMENT '',
    `subscription_context_flag` BOOLEAN COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `withdrawal_date` DATE COMMENT 'Date consent was withdrawn',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`software_version` (
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`service_tier` (
    ...
    `annual_price` DECIMAL(18,2) COMMENT 'Annual subscription price',
    `service_tier_code` STRING COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `data_allowance_gb` DECIMAL(18,2) COMMENT 'Monthly data allowance in GB',
    `dealer_sellable_flag` BOOLEAN COMMENT '',
    `service_tier_description` STRING COMMENT '',
    `feature_list` STRING COMMENT '',
    `feature_set` STRING COMMENT '',
    `included_services` STRING COMMENT 'Services included in this tier',
    `is_active` BOOLEAN COMMENT 'Whether this tier is currently available',
    `max_vehicles` STRING COMMENT 'Maximum vehicles allowed on this tier',
    `monthly_price` DECIMAL(18,2) COMMENT 'Monthly subscription price',
    `monthly_price_amount` DECIMAL(18,2) COMMENT '',
    `service_tier_name` STRING COMMENT '',
    `sales_eligible_flag` BOOLEAN COMMENT '',
    `tier_code` STRING COMMENT 'Unique code for the tier',
    `tier_description` STRING COMMENT '',
    `tier_level` STRING COMMENT '',
    `tier_name` STRING COMMENT 'Name of the service tier (basic, standard, premium, enterprise)',
    `tier_rank` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` (
    `vehicle_service_subscription_id` BIGINT COMMENT 'Primary key',
    `connected_vehicle_id` BIGINT COMMENT 'FK to connected vehicle',
    `party_id` BIGINT COMMENT 'FK to customer party',
    `pricing_plan_id` BIGINT COMMENT 'FK to pricing plan',
    `service_id` BIGINT COMMENT 'FK to mobility service',
    `service_tier_id` BIGINT COMMENT 'FK to service tier',
    `activation_timestamp` TIMESTAMP COMMENT 'Timestamp of activation',
    `auto_renew_flag` BOOLEAN COMMENT 'Whether subscription auto-renews',
    `billing_amount` DECIMAL(18,2) COMMENT 'Billing amount per cycle',
    `billing_cycle` STRING COMMENT 'Billing cycle (monthly, quarterly, annual)',
    ...
    `data_allowance_gb` DECIMAL(18,2) COMMENT 'Monthly data allowance in gigabytes',
    `data_usage_gb` DECIMAL(18,2) COMMENT 'Data usage in gigabytes',
    `dealer_originated_flag` BOOLEAN COMMENT '',
    `end_date` DATE COMMENT 'Subscription end date',
    `included_features` STRING COMMENT 'Comma-separated list of features included in subscription',
    `included_mileage` STRING COMMENT '',
    `last_billing_date` DATE COMMENT 'Date of last billing',
    `monthly_fee_amount` DECIMAL(18,2) COMMENT '',
    `next_billing_date` DATE COMMENT 'Date of next billing',
    `overage_rate_per_gb` DECIMAL(18,2) COMMENT 'Overage charge per GB beyond allowance',
    `payment_method` STRING COMMENT 'Payment method',
    `promo_code` STRING COMMENT 'Promotional code applied',
    `renewal_count` STRING COMMENT 'Number of renewals',
    ...
    `subscription_sales_link_id` BIGINT COMMENT 'FK to the sales subscription link connecting this subscription to the sales flow.',
    `subscription_status` STRING COMMENT '',
    `subscription_type` STRING COMMENT 'Type of subscription',
    `suspension_date` DATE COMMENT 'Date subscription was suspended if applicable',
    `suspension_reason` STRING COMMENT 'Reason for subscription suspension',
    `swap_eligible_flag` BOOLEAN COMMENT '',
    `total_billed_amount` DECIMAL(18,2) COMMENT 'Total amount billed over subscription lifetime',
    `trial_end_date` DATE COMMENT 'Trial period end date',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
    `vehicle_service_subscription_status` STRING COMMENT 'Current subscription status',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`mobility_route` (
    ...
    `distance_km` DECIMAL(18,2) COMMENT 'Total length of the route in kilometers.',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` (
    ...
    `charger_name` DECIMAL(18,2) COMMENT '',
    `charger_status` DECIMAL(18,2) COMMENT '',
    `charger_type` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_automotive_v1`.`mobility`.`pricing_plan` (
    `pricing_plan_id` BIGINT COMMENT 'Primary key for pricing_plan',
    `service_tier_id` BIGINT COMMENT '',
    ...
    `dealer_eligible_flag` BOOLEAN COMMENT '',
    ...
    `external_system_code` STRING COMMENT 'Identifier of the pricing plan in an external billing or CRM system.',
    ...
    `online_purchasable_flag` BOOLEAN COMMENT '',
    ...
    `sales_channel_eligible` STRING COMMENT 'Sales channels eligible for this pricing plan (online, dealer, both).',
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` ADD CONSTRAINT `fk_mobility_connected_vehicle_connected_mobility_fleet_account_id` FOREIGN KEY (`connected_mobility_fleet_account_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_fleet_account`(`mobility_fleet_account_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` ADD CONSTRAINT `fk_mobility_connected_vehicle_software_version_id` FOREIGN KEY (`software_version_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`software_version`(`software_version_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` ADD CONSTRAINT `fk_mobility_connected_vehicle_telematics_device_id` FOREIGN KEY (`telematics_device_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`telematics_device`(`telematics_device_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ADD CONSTRAINT `fk_mobility_service_service_tier_id` FOREIGN KEY (`service_tier_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service_tier`(`service_tier_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` ADD CONSTRAINT `fk_mobility_service_subscription_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` ADD CONSTRAINT `fk_mobility_service_subscription_service_id` FOREIGN KEY (`service_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service`(`service_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` ADD CONSTRAINT `fk_mobility_telemetry_event_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` ADD CONSTRAINT `fk_mobility_telemetry_event_trip_id` FOREIGN KEY (`trip_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`trip`(`trip_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` ADD CONSTRAINT `fk_mobility_trip_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` ADD CONSTRAINT `fk_mobility_trip_mobility_route_id` FOREIGN KEY (`mobility_route_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_route`(`mobility_route_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` ADD CONSTRAINT `fk_mobility_trip_telematics_device_id` FOREIGN KEY (`telematics_device_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`telematics_device`(`telematics_device_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` ADD CONSTRAINT `fk_mobility_trip_trip_connected_vehicle_id` FOREIGN KEY (`trip_connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ADD CONSTRAINT `fk_mobility_mobility_dtc_event_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` ADD CONSTRAINT `fk_mobility_remote_diagnostic_session_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`predictive_maintenance_alert` ADD CONSTRAINT `fk_mobility_predictive_maintenance_alert_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ota_campaign` ADD CONSTRAINT `fk_mobility_ota_campaign_software_version_id` FOREIGN KEY (`software_version_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`software_version`(`software_version_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_ota_deployment` ADD CONSTRAINT `fk_mobility_mobility_ota_deployment_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_ota_deployment` ADD CONSTRAINT `fk_mobility_mobility_ota_deployment_ota_campaign_id` FOREIGN KEY (`ota_campaign_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`ota_campaign`(`ota_campaign_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`tpms_reading` ADD CONSTRAINT `fk_mobility_tpms_reading_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence` ADD CONSTRAINT `fk_mobility_geofence_mobility_fleet_account_id` FOREIGN KEY (`mobility_fleet_account_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_fleet_account`(`mobility_fleet_account_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence_event` ADD CONSTRAINT `fk_mobility_geofence_event_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence_event` ADD CONSTRAINT `fk_mobility_geofence_event_geofence_id` FOREIGN KEY (`geofence_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`geofence`(`geofence_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence_event` ADD CONSTRAINT `fk_mobility_geofence_event_telematics_device_id` FOREIGN KEY (`telematics_device_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`telematics_device`(`telematics_device_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ADD CONSTRAINT `fk_mobility_mobility_fleet_vehicle_assignment_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ADD CONSTRAINT `fk_mobility_mobility_fleet_vehicle_assignment_mobility_fleet_account_id` FOREIGN KEY (`mobility_fleet_account_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_fleet_account`(`mobility_fleet_account_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`adas_feature_entitlement` ADD CONSTRAINT `fk_mobility_adas_feature_entitlement_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`usage_record` ADD CONSTRAINT `fk_mobility_usage_record_service_id` FOREIGN KEY (`service_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service`(`service_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`usage_record` ADD CONSTRAINT `fk_mobility_usage_record_service_subscription_id` FOREIGN KEY (`service_subscription_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service_subscription`(`service_subscription_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ADD CONSTRAINT `fk_mobility_remote_command_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ADD CONSTRAINT `fk_mobility_remote_command_mobility_fleet_account_id` FOREIGN KEY (`mobility_fleet_account_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_fleet_account`(`mobility_fleet_account_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ADD CONSTRAINT `fk_mobility_remote_command_remote_mobility_fleet_account_id` FOREIGN KEY (`remote_mobility_fleet_account_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`mobility_fleet_account`(`mobility_fleet_account_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` ADD CONSTRAINT `fk_mobility_ev_charging_session_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` ADD CONSTRAINT `fk_mobility_ev_charging_session_ev_charger_id` FOREIGN KEY (`ev_charger_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`ev_charger`(`ev_charger_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` ADD CONSTRAINT `fk_mobility_ev_charging_session_primary_ev_charger_id` FOREIGN KEY (`primary_ev_charger_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`ev_charger`(`ev_charger_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ADD CONSTRAINT `fk_mobility_service_incident_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ADD CONSTRAINT `fk_mobility_mobility_consent_record_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` ADD CONSTRAINT `fk_mobility_vehicle_service_subscription_connected_vehicle_id` FOREIGN KEY (`connected_vehicle_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`connected_vehicle`(`connected_vehicle_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` ADD CONSTRAINT `fk_mobility_vehicle_service_subscription_pricing_plan_id` FOREIGN KEY (`pricing_plan_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`pricing_plan`(`pricing_plan_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` ADD CONSTRAINT `fk_mobility_vehicle_service_subscription_service_id` FOREIGN KEY (`service_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service`(`service_id`);
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` ADD CONSTRAINT `fk_mobility_vehicle_service_subscription_service_tier_id` FOREIGN KEY (`service_tier_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service_tier`(`service_tier_id`);
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`pricing_plan` ADD CONSTRAINT `fk_mobility_pricing_plan_service_tier_id` FOREIGN KEY (`service_tier_id`) REFERENCES `vibe_automotive_v1`.`mobility`.`service_tier`(`service_tier_id`);
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`connected_vehicle` ALTER COLUMN `aftersales_nameplate_id` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telematics_device` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telematics_device` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telematics_device` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telematics_device` ALTER COLUMN `equipment_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment Registry Id (Foreign Key)');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` SET TAGS ('dbx_subdomain' = 'subscription_management');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_id` SET TAGS ('dbx_business_glossary_term' = 'Mobility Service Identifier (MSID)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Service Manager Employee Id (Foreign Key)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `aftersales_nameplate_id` SET TAGS ('dbx_business_glossary_term' = 'Nameplate Id (Foreign Key)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `aftersales_nameplate_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_tier_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Tier Identifier (PTID)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `supply_supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `billing_cycle` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle (BC)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `billing_cycle` SET TAGS ('dbx_value_regex' = 'monthly|annual|quarterly');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_category` SET TAGS ('dbx_business_glossary_term' = 'Service Category (SCAT)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_code` SET TAGS ('dbx_business_glossary_term' = 'Service Code (SC)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp (RCT)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `currency` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (CC)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `data_governance_ref` SET TAGS ('dbx_business_glossary_term' = 'Data Governance Reference');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `data_privacy_level` SET TAGS ('dbx_business_glossary_term' = 'Data Privacy Level (DPL)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `data_privacy_level` SET TAGS ('dbx_value_regex' = 'public|internal|confidential|restricted');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `data_retention_period_days` SET TAGS ('dbx_business_glossary_term' = 'Data Retention Period (Days) (DRPD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_description` SET TAGS ('dbx_business_glossary_term' = 'Service Description (SD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `device_compatibility` SET TAGS ('dbx_business_glossary_term' = 'Device Compatibility (DC)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `eligibility_rules` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Rules (ER)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `end_of_service_date` SET TAGS ('dbx_business_glossary_term' = 'Service End‑of‑Production Date (SEOPD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `is_premium` SET TAGS ('dbx_business_glossary_term' = 'Premium Service Flag (PSF)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `launch_date` SET TAGS ('dbx_business_glossary_term' = 'Service Launch Date (SLD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `max_simultaneous_devices` SET TAGS ('dbx_business_glossary_term' = 'Max Simultaneous Devices (MSD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_name` SET TAGS ('dbx_business_glossary_term' = 'Service Name (SN)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `ota_update_capability` SET TAGS ('dbx_business_glossary_term' = 'OTA Update Capability (OTA)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `predictive_maintenance_enabled` SET TAGS ('dbx_business_glossary_term' = 'Predictive Maintenance Enabled (PME)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `price` SET TAGS ('dbx_business_glossary_term' = 'Service Price (SP)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `provider` SET TAGS ('dbx_business_glossary_term' = 'Service Provider (SPRV)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `regulatory_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Approval Date (RAD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `regulatory_approval_status` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Approval Status (RAS)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `regulatory_approval_status` SET TAGS ('dbx_value_regex' = 'approved|pending|rejected');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `remote_diagnostics_enabled` SET TAGS ('dbx_business_glossary_term' = 'Remote Diagnostics Enabled (RDE)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_status` SET TAGS ('dbx_business_glossary_term' = 'Service Lifecycle Status (SLS)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `service_status` SET TAGS ('dbx_value_regex' = 'active|inactive|retired|draft|pending');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `sla_description` SET TAGS ('dbx_business_glossary_term' = 'Service SLA Description (SSLD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `sla_hours` SET TAGS ('dbx_business_glossary_term' = 'Service SLA Response Time (Hours) (SSLA)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `subscription_type` SET TAGS ('dbx_business_glossary_term' = 'Subscription Type (ST)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `subscription_type` SET TAGS ('dbx_value_regex' = 'subscription|pay_per_use|one_time|tiered');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `supported_vehicle_models` SET TAGS ('dbx_business_glossary_term' = 'Supported Vehicle Models (SVM)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `tax_included` SET TAGS ('dbx_business_glossary_term' = 'Tax Included Flag (TIF)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `tax_rate` SET TAGS ('dbx_business_glossary_term' = 'Tax Rate (Percentage) (TR)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `terms_url` SET TAGS ('dbx_business_glossary_term' = 'Service Terms URL (STU)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp (RUT)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `usage_based_insurance_enabled` SET TAGS ('dbx_business_glossary_term' = 'Usage‑Based Insurance Enabled (UBIE)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `usage_limit` SET TAGS ('dbx_business_glossary_term' = 'Usage Limit (UL)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `usage_metric` SET TAGS ('dbx_business_glossary_term' = 'Usage Metric (UM)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `usage_unit` SET TAGS ('dbx_business_glossary_term' = 'Usage Unit (UU)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `usage_unit` SET TAGS ('dbx_value_regex' = 'miles|km|hours|events');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `v2x_enabled` SET TAGS ('dbx_business_glossary_term' = 'V2X Communication Enabled (V2X)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `version` SET TAGS ('dbx_business_glossary_term' = 'Service Version (SV)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service` ALTER COLUMN `version_release_date` SET TAGS ('dbx_business_glossary_term' = 'Service Version Release Date (SVRD)');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` SET TAGS ('dbx_subdomain' = 'subscription_management');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` ALTER COLUMN `service_id` SET TAGS ('dbx_business_glossary_term' = 'Mobility Service Identifier (SERVICE_ID)');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_subscription` ALTER COLUMN `data_governance_ref` SET TAGS ('dbx_business_glossary_term' = 'Data Governance Reference');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` SET TAGS ('dbx_field_services_candidate' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` SET TAGS ('dbx_value_chain_gap' = 'field_services');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`telemetry_event` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`trip` ALTER COLUMN `equipment_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment Registry Id (Foreign Key)');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_ssot_owner' = 'mobility');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_ssot_concept' = 'dtc_event');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_ssot_authoritative' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_field_services_candidate' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_value_chain_gap' = 'field_services');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` SET TAGS ('dbx_ssot_role' = 'owner');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `cleared_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cleared Timestamp');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `dtc_code` SET TAGS ('dbx_business_glossary_term' = 'DTC Code');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event Timestamp');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `freeze_frame_data` SET TAGS ('dbx_business_glossary_term' = 'Freeze Frame Data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `mileage_at_event` SET TAGS ('dbx_business_glossary_term' = 'Mileage at Event');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `severity` SET TAGS ('dbx_business_glossary_term' = 'Severity');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_dtc_event` ALTER COLUMN `system_affected` SET TAGS ('dbx_business_glossary_term' = 'System Affected');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_field_services_candidate' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_value_chain_gap' = 'field_services');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_mvm_status' = 'include');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_diagnostic_session` SET TAGS ('dbx_mvm_scope' = 'basic_connected_vehicle');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`predictive_maintenance_alert` SET TAGS ('dbx_subdomain' = 'connected_telematics');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`predictive_maintenance_alert` SET TAGS ('dbx_field_services_candidate' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`predictive_maintenance_alert` SET TAGS ('dbx_value_chain_gap' = 'field_services');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ota_campaign` SET TAGS ('dbx_subdomain' = 'ota_deployment');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_ota_deployment` SET TAGS ('dbx_subdomain' = 'ota_deployment');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`tpms_reading` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence` ALTER COLUMN `geofence_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`geofence_event` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `account_name` SET TAGS ('dbx_business_glossary_term' = 'Account Name');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `account_number` SET TAGS ('dbx_business_glossary_term' = 'Account Number');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `account_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `account_status` SET TAGS ('dbx_business_glossary_term' = 'Account Status');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `contract_end_date` SET TAGS ('dbx_business_glossary_term' = 'Contract End Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `contract_start_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Start Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `fleet_account_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `fleet_account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `fleet_size` SET TAGS ('dbx_business_glossary_term' = 'Fleet Size');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `monthly_fee` SET TAGS ('dbx_business_glossary_term' = 'Monthly Fee');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_account` ALTER COLUMN `subscription_tier` SET TAGS ('dbx_business_glossary_term' = 'Subscription Tier');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `assigned_driver_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `assignment_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `assignment_status` SET TAGS ('dbx_business_glossary_term' = 'Assignment Status');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `driver_name` SET TAGS ('dbx_business_glossary_term' = 'Driver Name');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `driver_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `mileage_at_assignment` SET TAGS ('dbx_business_glossary_term' = 'Mileage at Assignment');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `mileage_at_return` SET TAGS ('dbx_business_glossary_term' = 'Mileage at Return');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `return_date` SET TAGS ('dbx_business_glossary_term' = 'Return Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_fleet_vehicle_assignment` ALTER COLUMN `usage_type` SET TAGS ('dbx_business_glossary_term' = 'Usage Type');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`adas_feature_entitlement` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`adas_feature_entitlement` SET TAGS ('dbx_subdomain' = 'ota_deployment');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`adas_feature_entitlement` ALTER COLUMN `feature_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`usage_record` SET TAGS ('dbx_subdomain' = 'subscription_management');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`usage_record` ALTER COLUMN `service_id` SET TAGS ('dbx_business_glossary_term' = 'Service ID');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_[enum_ref_candidate' = 'remote_start');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_remote_stop' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_lock' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_unlock' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_climate_preconditioning' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_horn' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_lights' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_charge_schedule' = 'true');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`remote_command` ALTER COLUMN `command_type` SET TAGS ('dbx_speed_limit_set_—_promote_to_reference_product]' = 'true');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` SET TAGS ('dbx_subdomain' = 'fleet_operations');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charging_session` ALTER COLUMN `station_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `affected_service` SET TAGS ('dbx_business_glossary_term' = 'Affected Service');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `affected_vehicle_count` SET TAGS ('dbx_business_glossary_term' = 'Affected Vehicle Count');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `incident_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Incident Timestamp');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `incident_type` SET TAGS ('dbx_business_glossary_term' = 'Incident Type');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `resolution_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Resolution Timestamp');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `root_cause` SET TAGS ('dbx_business_glossary_term' = 'Root Cause');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `severity` SET TAGS ('dbx_business_glossary_term' = 'Severity');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_incident` ALTER COLUMN `service_incident_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` SET TAGS ('dbx_subdomain' = 'subscription_management');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `consent_status` SET TAGS ('dbx_business_glossary_term' = 'Consent Status');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `consent_type` SET TAGS ('dbx_business_glossary_term' = 'Consent Type');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `data_categories` SET TAGS ('dbx_business_glossary_term' = 'Data Categories');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `data_subject_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Expiry Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `granted_date` SET TAGS ('dbx_business_glossary_term' = 'Granted Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `legal_basis` SET TAGS ('dbx_business_glossary_term' = 'Legal Basis');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`mobility_consent_record` ALTER COLUMN `withdrawal_date` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Date');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`software_version` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`software_version` SET TAGS ('dbx_subdomain' = 'connected_telematics');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` SET TAGS ('dbx_subdomain' = 'subscription_management');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `annual_price` SET TAGS ('dbx_business_glossary_term' = 'Annual Price');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `data_allowance_gb` SET TAGS ('dbx_business_glossary_term' = 'Data Allowance GB');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `included_services` SET TAGS ('dbx_business_glossary_term' = 'Included Services');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `max_vehicles` SET TAGS ('dbx_business_glossary_term' = 'Max Vehicles');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `monthly_price` SET TAGS ('dbx_business_glossary_term' = 'Monthly Price');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `service_tier_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `tier_code` SET TAGS ('dbx_business_glossary_term' = 'Tier Code');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `tier_name` SET TAGS ('dbx_business_glossary_term' = 'Tier Name');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`service_tier` ALTER COLUMN `tier_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` SET TAGS ('dbx_subdomain' = 'subscription_management');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`vehicle_service_subscription` ALTER COLUMN `subscription_sales_link_id` SET TAGS ('dbx_business_glossary_term' = 'Subscription Sales Link');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` SET TAGS ('dbx_subdomain' = 'fleet_operations');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` ALTER COLUMN `address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` ALTER COLUMN `charger_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`ev_charger` ALTER COLUMN `ev_charger_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`pricing_plan` SET TAGS ('dbx_subdomain' = 'subscription_management');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`pricing_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_automotive_v1`.`mobility`.`pricing_plan` ALTER COLUMN `sales_channel_eligible` SET TAGS ('dbx_business_glossary_term' = 'Sales Channel Eligible');
