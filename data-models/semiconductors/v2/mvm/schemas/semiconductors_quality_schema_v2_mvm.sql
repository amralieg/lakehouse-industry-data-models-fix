...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`inspection_lot` (
    ...
    `calibration_record_id` BIGINT COMMENT 'Foreign key linking to equipment.calibration_record. Business justification: IATF 16949 requires traceability of measurement equipment calibration status at the time of inspection. Linking inspection_lot to the active calibration_record of the inspection tool enables auditors ',
    `control_plan_id` BIGINT COMMENT 'Foreign key linking to quality.control_plan. Business justification: An inspection lot is conducted according to a control plan that defines the sampling rate, measurement criteria, tolerances, and inspection tool. Adding control_plan_id to inspection_lot normalizes th',
    ...
    `raw_material_id` BIGINT COMMENT 'Foreign key linking to inventory.raw_material. Business justification: Incoming quality inspection of raw materials (silicon wafers, process chemicals) is a mandatory IQC process. inspection_lot.lot_type and material_type indicate raw material coverage. This link support',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`defect_record` (
    ...
    `die_bank_id` BIGINT COMMENT 'Foreign key linking to inventory.die_bank. Business justification: Defect records with die_x/die_y coordinates track die-level defects that directly affect die bank quality and KGD status. This link enables die bank quality holds and KGD re-certification decisions wh',
    ...
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: Supplier Corrective Action Requests (SCARs) and supplier quality scorecards require defect records to be directly traceable to the responsible supplier. IATF 16949 supplier monitoring mandates this tr',
    `wafer_id` BIGINT COMMENT 'Identifier of the wafer on which the defect was observed.',
    `wafer_probe_run_id` BIGINT COMMENT 'Foreign key linking to test.wafer_probe_run. Business justification: Defects are detected during wafer probe runs. Quality engineers must correlate defect records to the specific probe run for yield loss analysis, probe card qualification, and root cause investigation.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`wafer_map` (
    ...
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Design-correlated defect pattern analysis: wafer maps are generated for specific design projects; design engineers analyze wafer maps to distinguish systematic (design-driven) from random defect patte',
    `inspection_lot_id` BIGINT COMMENT 'Foreign key linking to quality.inspection_lot. Business justification: A wafer map is a spatial defect/bin distribution map produced for an individual wafer within an inspection lot. wafer_map currently links to fabrication_wafer_lot and wafer but has no direct FK to the',
    `tapeout_id` BIGINT COMMENT 'Foreign key linking to design.tapeout. Business justification: Tapeout-level defect density tracking: wafer maps are produced from tapeout-specific mask sets. Linking wafer_map to tapeout enables tapeout-level defect density trending, mask defect attribution, and',
    ...
    `map_generation_timestamp` DECIMAL(18,2) COMMENT 'Date and time when the wafer map was generated.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`yield_record` (
    ...
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Design-for-yield feedback loop: yield engineers correlate yield loss (fallout_rate, yield_gap_percent, yield_loss_category) to specific IC design projects to trigger design ECOs. Standard semiconducto',
    ...
    `quality_spec_id` BIGINT COMMENT 'Foreign key linking to quality.quality_spec. Business justification: A yield record measures yield against a defined quality gate specification. quality_spec defines the yield targets, acceptance criteria, and measurement parameters for each process step or SKU. Adding',
    ...
    `tool_qualification_id` BIGINT COMMENT 'Foreign key linking to equipment.tool_qualification. Business justification: Yield records from qualification wafer runs are the primary evidence for tool qualification acceptance. Process engineers reference the tool qualification to contextualize yield data, distinguish qual',
    `wafer_id` BIGINT COMMENT 'Unique identifier for the individual wafer.',
    `wafer_map_id` BIGINT COMMENT 'Foreign key linking to quality.wafer_map. Business justification: A yield record captures yield measurement outcomes at quality gates. The spatial bin distribution captured in a wafer map is the source data for yield calculations. Linking yield_record.wafer_map_id →',
    `wafer_probe_run_id` BIGINT COMMENT 'Foreign key linking to test.wafer_probe_run. Business justification: Yield records capture yield at wafer probe stage. yield_record already has final_test_run_id; adding wafer_probe_run_id completes yield traceability across both test stages. Yield engineers need to co',
    ...
    `bin_yield_pct` DECIMAL(18,2) COMMENT 'Bin yield as a percentage',
    `calibration_status` DECIMAL(18,2) COMMENT 'Indicates whether the measurement equipment was calibrated.',
    ...
    `fallout_rate` DECIMAL(18,2) COMMENT 'Fallout rate as a decimal ratio',
    ...
    `yield_loss_category` DECIMAL(18,2) COMMENT 'Root cause classification for yield loss.',
    ...
    `yield_record_status` DECIMAL(18,2) COMMENT 'Current validation status of the yield record.',
    `yield_target_percent` DECIMAL(18,2) COMMENT 'Target yield percentage defined for the product/process.',
    `yield_type` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` (
    ...
    `assembly_lot_id` BIGINT COMMENT 'Foreign key linking to packaging.assembly_lot. Business justification: Package-level reliability tests (HTOL, thermal cycling, HAST) are performed on assembled lots. Linking reliability_test to assembly_lot enables traceability of test results back to specific OSAT vendo',
    `die_bank_id` BIGINT COMMENT 'Foreign key linking to inventory.die_bank. Business justification: Reliability tests (HTOL, ESD, latch-up) are performed on die sampled from die banks for JEDEC/AEC-Q qualification. This link provides traceability from reliability test results to the specific die ban',
    `finished_good_id` BIGINT COMMENT 'Foreign key linking to inventory.finished_good. Business justification: AEC-Q100/Q101 and JEDEC qualification require reliability testing of packaged finished goods. This link ties reliability test results directly to finished good inventory records, enabling qualificatio',
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: JEDEC/AEC-Q100 qualification tracking: reliability qualification tests (HTOL, TDDB, EM) are executed against a specific IC design project. Qualification engineers must link test results to the design ',
    `inventory_wafer_lot_id` BIGINT COMMENT 'Foreign key linking to inventory.inventory_wafer_lot. Business justification: Reliability tests on wafer-level samples require traceability to the inventory wafer lot for hold/release decisions. This link enables inventory disposition based on reliability test outcomes, support',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: Reliability qualification programs for outsourced or purchased components require test results to be traceable to the supplying entity. Supplier qualification status and JEDEC reliability compliance r',
    `tapeout_id` BIGINT COMMENT 'Foreign key linking to design.tapeout. Business justification: Reliability qualification is performed on silicon from a specific tapeout (mask revision, process node). JEDEC qualification reports must identify the exact tapeout under test; this link enables tapeo',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` (
    ...
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: KGD qualification traceability to design: KGD certification is performed on dies from a specific IC design project. Design engineers need to track KGD qualification status per project for customer com',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: KGD certifications for purchased dies must be traceable to the supplying foundry or die vendor. Supplier qualification programs and customer-facing KGD certification documents require direct supplier ',
    `tapeout_id` BIGINT COMMENT 'Foreign key linking to design.tapeout. Business justification: KGD certification tied to tapeout revision: KGD qualification is silicon-revision-specific; a new tapeout requires re-qualification. Linking quality_kgd_certification to tapeout enables tapeout-level ',
    `wafer_map_id` BIGINT COMMENT '',
    `wafer_probe_run_id` BIGINT COMMENT 'Foreign key linking to test.wafer_probe_run. Business justification: KGD certification is issued based on wafer probe test results demonstrating die quality. The certification must reference the specific probe run that provided the test evidence. This is a fundamental ',
    ...
    `expiration_reason` DECIMAL(18,2) COMMENT 'Reason why the certification expired or was terminated.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` (
    ...
    `assembly_lot_id` BIGINT COMMENT 'Foreign key linking to packaging.assembly_lot. Business justification: Field DPPM records must trace back to the originating assembly lot to identify the OSAT vendor, package type, and process conditions responsible for field failures. This traceability is required for 8',
    ...
    `design_win_id` BIGINT COMMENT 'Foreign key linking to customer.customer_design_win. Business justification: DPPM performance in semiconductors is tracked at the design win/program level for customer quality scorecards and SLA compliance. dppm_record has account_id and ic_catalog_id but no program-level link',
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: DPPM reporting in semiconductors requires formal notification to a specific customer quality contact. The 8D report and corrective action responses are sent to a named contact. dppm_record already has',
    `final_test_run_id` BIGINT COMMENT 'Foreign key linking to test.final_test_run. Business justification: DPPM metrics are computed from final test results and customer returns. Linking dppm_record to final_test_run enables direct traceability from customer-facing quality KPIs to the test execution data, ',
    `finished_good_id` BIGINT COMMENT 'Foreign key linking to inventory.finished_good. Business justification: DPPM records track customer-reported defects on shipped finished goods. This link ties DPPM metrics directly to finished_good inventory records, enabling customer quality scorecards, field return anal',
    ...
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Customer quality scorecard reporting: DPPM records are tracked per design project for customer-facing quality KPIs and corrective action programs. Design-level DPPM trending is a standard automotive/i',
    `rma_id` BIGINT COMMENT 'Foreign key linking to order.rma. Business justification: DPPM calculation in semiconductor quality is directly driven by customer returns. Linking dppm_record to the originating RMA enables accurate DPPM computation, customer quality scorecards, and supplie',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: DPPM is a primary supplier quality KPI in semiconductor manufacturing. Every DPPM record must be directly traceable to the responsible supplier for supplier scorecards, IATF 16949 supplier monitoring,',
    ...
    `compliance_iso9001` BOOLEAN COMMENT 'Compliance status of the record with ISO 9001 quality management requirements.',
    ...
    `defect_rate` DECIMAL(18,2) COMMENT 'Defect rate as a decimal ratio',
    ...
    `reject_rate` DECIMAL(18,2) COMMENT 'Reject rate as a decimal ratio',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` (
    ...
    `final_test_run_id` BIGINT COMMENT 'Foreign key linking to test.final_test_run. Business justification: CAPAs are frequently initiated from final test failures. The CAPA record must reference the test run that triggered the corrective action to enable effectiveness verification — re-running the same tes',
    `finished_good_id` BIGINT COMMENT 'Foreign key linking to inventory.finished_good. Business justification: CAPAs triggered by finished good quality escapes (customer returns, ATE failures) must reference the affected finished good inventory. This link supports inventory quarantine and rework decisions driv',
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Design-triggered CAPA closure: CAPAs caused by systematic design issues (e.g., design rule violations causing yield loss) must reference the design project to assign corrective actions to design engin',
    `inventory_wafer_lot_id` BIGINT COMMENT 'Foreign key linking to inventory.inventory_wafer_lot. Business justification: CAPAs triggered by wafer lot nonconformances must reference the affected inventory wafer lot to manage inventory holds and disposition. This link supports the CAPA-driven inventory hold/release workfl',
    ...
    `raw_material_id` BIGINT COMMENT 'Foreign key linking to inventory.raw_material. Business justification: CAPAs triggered by incoming raw material nonconformances (e.g., silicon wafer defects, chemical purity failures) must reference the specific raw material. This link supports supplier corrective action',
    `maintenance_event_id` BIGINT COMMENT 'Foreign key linking to equipment.maintenance_event. Business justification: CAPA corrective actions in semiconductor fabs frequently require specific maintenance events (tool repair, chamber replacement, recalibration). Quality engineers link the CAPA to the maintenance event',
    `sku_id` BIGINT COMMENT 'Identifier of the product or design associated with the CAPA.',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: CAPAs in semiconductors are frequently issued as Supplier Corrective Action Requests (SCARs). The CAPA must reference the responsible supplier for IATF 16949 compliance, supplier performance tracking,',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`nonconformance_report` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: NCRs with customer impact (is_customer_impact=true) require notification to a specific customer contact. Fields customer_notification_required and customer_notification_sent_timestamp exist but no FK ',
    `defect_record_id` BIGINT COMMENT 'Foreign key linking to quality.defect_record. Business justification: A Non-Conformance Report is typically raised in response to a specific defect event captured in a defect record. Adding defect_record_id to nonconformance_report links the NCR to the originating defec',
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: IATF 16949 NCR traceability: nonconformances originating from design-related issues (DRC violations, design-rule non-compliance at incoming inspection) must reference the design project for root cause',
    ...
    `quality_spec_id` BIGINT COMMENT 'Foreign key linking to quality.quality_spec. Business justification: An NCR documents a deviation from a quality specification. nonconformance_report.specification_violated is a free-text string that should be normalized to a FK reference to quality_spec. Adding qualit',
    `raw_material_id` BIGINT COMMENT 'Foreign key linking to inventory.raw_material. Business justification: NCRs are raised against incoming raw materials (silicon wafers, photoresist, process gases) failing incoming inspection. This link supports incoming quality control disposition, supplier notification,',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supplier. Business justification: NCRs for incoming material nonconformances must directly reference the supplier for SCAR issuance, supplier notification workflows, and IATF 16949 supplier quality management. Direct FK enables suppli',
    ...
    `ncr_number` STRING COMMENT '',
    `ncr_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` (
    ...
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Design-specific quality specification governance: quality specs define acceptance criteria, measurement parameters, and tolerance limits for a specific IC design project. Design review processes requi',
    `limit_id` BIGINT COMMENT 'Foreign key linking to test.limit. Business justification: Quality specs define engineering acceptance criteria that test limits must implement. Quality engineers must verify that test limits conform to quality specs during product qualification and spec chan',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Quality specs define acceptance criteria for specific materials in incoming inspection. Linking quality_spec to material_master enables material-level quality specification management, supporting inco',
    ...
    `pdk_id` BIGINT COMMENT 'Foreign key linking to design.pdk. Business justification: PDK-level quality governance: quality specs for DRC/LVS acceptance criteria, process corner tolerances, and reliability limits are PDK-specific. Foundry qualification requires linking quality specs to',
    `process_step_id` BIGINT COMMENT 'Foreign key linking to fabrication.process_step. Business justification: Specification documents are applied to a particular process step; linking supports step‑specific spec compliance checks.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`quality`.`failure_analysis_report` (
    ...
    `assembly_lot_id` BIGINT COMMENT 'Foreign key linking to packaging.assembly_lot. Business justification: Failure analysis reports on packaged devices must trace to the assembly lot to identify OSAT vendor, package type, material lot, and process conditions as root cause inputs. FA engineers require this ',
    ...
    `defect_record_id` BIGINT COMMENT '',
    `die_bank_id` BIGINT COMMENT 'Foreign key linking to inventory.die_bank. Business justification: Failure analysis is performed on die sampled from die banks when KGD failures are detected. This link provides traceability from FA findings to the specific die bank lot, enabling die bank quarantine ',
    `final_test_run_id` BIGINT COMMENT 'Foreign key linking to test.final_test_run. Business justification: Failure analysis reports are initiated from final test failures. The FAR must reference the test run to access bin distribution, test conditions, and parametric data needed for root cause analysis. Th',
    `finished_good_id` BIGINT COMMENT 'Foreign key linking to inventory.finished_good. Business justification: Failure analysis reports are routinely conducted on failed finished goods from customer returns or ATE escapes. This link ties FA findings to specific finished good inventory, enabling quarantine of a',
    `ic_design_project_id` BIGINT COMMENT 'Foreign key linking to design.ic_design_project. Business justification: Failure analysis feedback to design: FA reports on silicon failures trigger design ECOs and design rule updates. Design engineers must be able to retrieve all FA reports against their project. Standar',
    `inventory_wafer_lot_id` BIGINT COMMENT 'Foreign key linking to inventory.inventory_wafer_lot. Business justification: Failure analysis reports on wafer lot failures require direct traceability to the inventory wafer lot for disposition decisions. This link enables inventory hold/scrap decisions based on FA findings a',
    `physical_layout_id` BIGINT COMMENT 'Foreign key linking to design.physical_layout. Business justification: Layout-driven failure analysis: FA reports on metal shorts, via failures, or antenna violations reference the specific physical layout version (routing, metal fill) that caused the failure. Layout eng',
    ...
    `unit_test_result_id` BIGINT COMMENT 'Foreign key linking to test.unit_test_result. Business justification: A FAR is initiated for a specific failing unit identified by its unit test result. This atomic link enables FA engineers to access the exact parametric measurements, bin assignment, and test condition',
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ADD CONSTRAINT `fk_quality_kgd_certification_wafer_map_id` FOREIGN KEY (`wafer_map_id`) REFERENCES `vibe_semiconductors_v1`.`quality`.`wafer_map`(`wafer_map_id`);
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`nonconformance_report` ADD CONSTRAINT `fk_quality_nonconformance_report_defect_record_id` FOREIGN KEY (`defect_record_id`) REFERENCES `vibe_semiconductors_v1`.`quality`.`defect_record`(`defect_record_id`);
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`inspection_lot` SET TAGS ('dbx_subdomain' = 'inspection_yield');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`inspection_lot` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`inspection_lot` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`inspection_lot` ALTER COLUMN `quality_engineer` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`defect_record` SET TAGS ('dbx_subdomain' = 'inspection_yield');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`defect_record` ALTER COLUMN `die_bank_id` SET TAGS ('dbx_business_glossary_term' = 'Die Bank Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`wafer_map` SET TAGS ('dbx_subdomain' = 'inspection_yield');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`wafer_map` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`wafer_map` ALTER COLUMN `tapeout_id` SET TAGS ('dbx_business_glossary_term' = 'Tapeout Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`yield_record` SET TAGS ('dbx_subdomain' = 'inspection_yield');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`yield_record` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`yield_record` ALTER COLUMN `tool_qualification_id` SET TAGS ('dbx_business_glossary_term' = 'Tool Qualification Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` SET TAGS ('dbx_subdomain' = 'reliability_certification');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` ALTER COLUMN `inventory_wafer_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Inventory Wafer Lot Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` ALTER COLUMN `tapeout_id` SET TAGS ('dbx_business_glossary_term' = 'Tapeout Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`reliability_test` ALTER COLUMN `operator_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` SET TAGS ('dbx_subdomain' = 'reliability_certification');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `kgd_certification_id` SET TAGS ('dbx_business_glossary_term' = 'Known Good Die Certification ID (KGD_CERT_ID)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `die_bank_id` SET TAGS ('dbx_business_glossary_term' = 'Die Lot Identifier (DIE_LOT_ID)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `tapeout_id` SET TAGS ('dbx_business_glossary_term' = 'Tapeout Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `wafer_probe_run_id` SET TAGS ('dbx_business_glossary_term' = 'Wafer Probe Run Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Certification Approval Timestamp (APPROVAL_TS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `burn_in_status` SET TAGS ('dbx_business_glossary_term' = 'Burn‑In Status (BURN_IN_STS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `burn_in_status` SET TAGS ('dbx_value_regex' = 'completed|not_required|pending');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_batch` SET TAGS ('dbx_business_glossary_term' = 'Certification Batch Identifier (CERT_BATCH)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Date (CERT_DATE)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_location` SET TAGS ('dbx_business_glossary_term' = 'Certification Facility Location (CERT_LOC)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Number (CERT_NUM)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_reference` SET TAGS ('dbx_business_glossary_term' = 'Certification Reference Code (CERT_REF)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_standard` SET TAGS ('dbx_business_glossary_term' = 'Certification Standard (CERT_STD)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `certification_standard` SET TAGS ('dbx_value_regex' = 'JEDEC_JESD49|JEDEC_JESD20|CUSTOM');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `compliance_iatf16949` SET TAGS ('dbx_business_glossary_term' = 'IATF 16949 Compliance Indicator (COMPLY_IATF16949)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `compliance_iso9001` SET TAGS ('dbx_business_glossary_term' = 'ISO 9001 Compliance Indicator (COMPLY_ISO9001)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `compliance_jedec` SET TAGS ('dbx_business_glossary_term' = 'JEDEC Compliance Indicator (COMPLY_JEDEC)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp (CREATED_TS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `customer_requirements` SET TAGS ('dbx_business_glossary_term' = 'Customer Specific KGD Requirements (CUSTOMER_REQ)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `expiration_reason` SET TAGS ('dbx_business_glossary_term' = 'Reason for Certification Expiration (EXPIRY_REASON)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Expiry Date (EXPIRY_DATE)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `kgd_version` SET TAGS ('dbx_business_glossary_term' = 'KGD Version Identifier (KGD_VER)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Additional Notes (NOTES)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `parametric_screen_result` SET TAGS ('dbx_business_glossary_term' = 'Parametric Screen Result (PARAM_SCREEN_RES)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `parametric_screen_result` SET TAGS ('dbx_value_regex' = 'pass|fail|conditional');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `part_number` SET TAGS ('dbx_business_glossary_term' = 'Product Part Number (PART_NUM)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `quality_kgd_certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Lifecycle Status (CERT_STATUS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `quality_kgd_certification_status` SET TAGS ('dbx_value_regex' = 'active|expired|revoked');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `reliability_grade` SET TAGS ('dbx_business_glossary_term' = 'Reliability Grade (REL_GRADE)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `reliability_grade` SET TAGS ('dbx_value_regex' = 'A|B|C|D|E');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `revision_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Revision Number (REV_NUM)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `test_coverage_percent` SET TAGS ('dbx_business_glossary_term' = 'Test Coverage Percentage (TEST_COV_PCT)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `test_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Test Duration in Hours (TEST_DUR_HRS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `test_temperature_c` SET TAGS ('dbx_business_glossary_term' = 'Test Temperature in Celsius (TEST_TEMP_C)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `test_type` SET TAGS ('dbx_business_glossary_term' = 'Test Type Conducted (TEST_TYPE)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `test_type` SET TAGS ('dbx_value_regex' = 'electrical|parametric|reliability');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`kgd_certification` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp (UPDATED_TS)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` SET TAGS ('dbx_subdomain' = 'corrective_action');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` ALTER COLUMN `assembly_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Assembly Lot Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` ALTER COLUMN `design_win_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Design Win Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Dppm Contact Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`dppm_record` ALTER COLUMN `rma_id` SET TAGS ('dbx_business_glossary_term' = 'Rma Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` ALTER COLUMN `final_test_run_id` SET TAGS ('dbx_business_glossary_term' = 'Final Test Run Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` ALTER COLUMN `raw_material_id` SET TAGS ('dbx_business_glossary_term' = 'Raw Material Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` ALTER COLUMN `maintenance_event_id` SET TAGS ('dbx_business_glossary_term' = 'Required Maintenance Event Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`capa_record` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`nonconformance_report` ALTER COLUMN `defect_record_id` SET TAGS ('dbx_business_glossary_term' = 'Defect Record Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`nonconformance_report` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`nonconformance_report` ALTER COLUMN `raw_material_id` SET TAGS ('dbx_business_glossary_term' = 'Raw Material Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` SET TAGS ('dbx_subdomain' = 'corrective_action');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` ALTER COLUMN `ic_design_project_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Design Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` ALTER COLUMN `process_step_id` SET TAGS ('dbx_business_glossary_term' = 'Process Step Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`quality_spec` ALTER COLUMN `parameter_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`failure_analysis_report` SET TAGS ('dbx_subdomain' = 'reliability_certification');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`failure_analysis_report` ALTER COLUMN `die_bank_id` SET TAGS ('dbx_business_glossary_term' = 'Die Bank Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`failure_analysis_report` ALTER COLUMN `inventory_wafer_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Inventory Wafer Lot Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`control_plan` ALTER COLUMN `author_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`control_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`control_plan` ALTER COLUMN `sampling_rate` SET TAGS ('dbx_type_verified' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`control_plan` ALTER COLUMN `updated_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`quality`.`control_plan` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
