...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`header` (
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Regulatory reporting process requires each claim to be linked to the specific regulatory obligation it satisfies for CMS/State filings.',
    ...
    `group_id` BIGINT COMMENT 'Identifier linking related claim adjustments.',
    `header_group_id` BIGINT COMMENT 'Foreign key linking to employer.employer_group. Business justification: Required for employer‑sponsored plan cost reporting and ACA employer reporting; each claim must be attributed to the employer group that provided the coverage.',
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Required for network participation and quality reporting; each claim must be linked to the provider entity for regulatory and performance analytics.',
    ...
    `rendering_provider_practice_location_id` BIGINT COMMENT 'FK to provider.provider.provider_id — Every claim must resolve to a rendering provider for network status determination, fee schedule lookup, and provider payment. This is the second most critical operational join in health insurance.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`line` (
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Line‑level provider attribution enables detailed utilization, cost, and service‑type analysis required by utilization review and case‑mix reporting.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`procedure` (
    ...
    `surgeon_provider_provider_provider_practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Links surgeon NPI to provider entity for surgeon performance, outcome, and credentialing dashboards mandated by quality‑measure reporting.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` (
    ...
    `practice_location_id` BIGINT COMMENT 'National Provider Identifier (NPI) of the treating provider.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`cob` (
    ...
    `claim_payer_id` BIGINT COMMENT 'Internal identifier of the primary insurance payer.',
    ...
    `secondary_payer_code` DECIMAL(18,2) COMMENT 'Internal identifier of the secondary insurance payer.',
    `secondary_payer_denial_reason` DECIMAL(18,2) COMMENT 'Reason code or description for any denial by the secondary payer.',
    `secondary_payer_name` DECIMAL(18,2) COMMENT 'Legal name of the secondary insurance payer.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` (
    ...
    `reserve_period_month` DECIMAL(18,2) COMMENT 'Month (1‑12) of the reserve reporting period.',
    `reserve_period_year` DECIMAL(18,2) COMMENT 'Calendar year of the reserve reporting period.',
    `reserve_run_date` DECIMAL(18,2) COMMENT 'Date on which the IBNR reserve calculation was performed.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` (
    ...
    `fee_schedule_id` DECIMAL(18,2) COMMENT 'FK to contract.fee_schedule',
    `adjudication_rate_status` DOUBLE COMMENT 'Current lifecycle status of the rate.',
    ...
    `currency_code` STRING COMMENT 'Three‑letter ISO currency code for monetary amounts.. Valid values are `^[A-Z]{3}$`',
    `adjudication_rate_description` DOUBLE COMMENT 'Human‑readable description or notes about the rate.',
    ...
    `rate_source` DOUBLE COMMENT 'Origin of the rate definition (e.g., CMS, plan‑specific, contracted).',
    `rate_type` DOUBLE COMMENT 'Category of the rate (procedure, drug, bundle, or service).',
...
) COMMENT 'Immutable audit trail of claim lifecycle status transitions — received, pended, adjudicated, paid, denied, appealed, adjusted. Captures status code, transition timestamp, triggering user or system, and reason code. Supports 277 claim status inquiry responses. [FHIR-aligned]';

CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` (
    `claim_payer_id` BIGINT COMMENT 'Primary key for payer',
    `billing_payer_id` BIGINT COMMENT 'Single-source-of-truth reference to canonical payer record.',
    `vendor_id` BIGINT COMMENT 'external payers are third-party entities that should link to the vendor master',
    ...
    `payer_payment_cycle` DECIMAL(18,2) COMMENT 'Description of the payment cycle for the payer.',
    `payer_payment_frequency` DECIMAL(18,2) COMMENT 'Frequency of payments to the payer.',
    `payer_payment_method` DECIMAL(18,2) COMMENT 'Preferred method of payment to the payer.',
    `payer_payment_terms` DECIMAL(18,2) COMMENT 'Payment terms agreed with the payer.',
...
) COMMENT '[SSOT:payer] Reference to canonical SSOT billing.payer for the payer concept; do not duplicate authoritative data here. Master reference table for payer. Referenced by primary_payer_id. SSOT NOTE: This product is the single source of truth (canonical) for the payer concept across domains. [FHIR-aligned] Designated SSOT for the payer concept.';
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ADD CONSTRAINT `fk_claim_cob_claim_payer_id` FOREIGN KEY (`claim_payer_id`) REFERENCES `vibe_health_insurance_v1`.`claim`.`claim_payer`(`claim_payer_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` SET TAGS ('dbx_['fhir_resource' = 'Claim');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `care_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Care Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Changed By (User ID)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `enrollment_eligibility_span_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Eligibility Span Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Adjustment Group ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `header_group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `inpatient_admission_id` SET TAGS ('dbx_business_glossary_term' = 'Inpatient Admission Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `plan_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Claiming Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Care Program Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Network Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `regulatory_submission_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `rendering_provider_practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Header Rendering Provider Id');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `pool_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Pool Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_business_glossary_term' = 'Subscriber Id');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_pii_type' = 'identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `um_case_id` SET TAGS ('dbx_business_glossary_term' = 'Um Case Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `year_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Year Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`header` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` SET TAGS ('dbx_['fhir_resource' = 'Claim');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` SET TAGS ('dbx_'fhir_resource' = 'Claim.item');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Ledger Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Line Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Network Tier Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'National Drug Code (NDC)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_fhir' = 'medicationCodeableConcept');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`line` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` SET TAGS ('dbx_['fhir_resource' = 'Condition');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`diagnosis` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` SET TAGS ('dbx_['fhir_resource' = 'Procedure');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Surgeon Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `surgeon_provider_provider_provider_practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Surgeon Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_status` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_status` SET TAGS ('dbx_value_regex' = 'approved|denied|pending');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `prior_authorization_status` SET TAGS ('dbx_fhir_element' = 'status');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_business_glossary_term' = 'Secondary Procedure Codes');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_sensitivity' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_fhir_element' = 'Procedure.code');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `surgeon_name` SET TAGS ('dbx_business_glossary_term' = 'Surgeon Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `surgeon_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`procedure` ALTER COLUMN `surgeon_name` SET TAGS ('dbx_pii_type' = 'name');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_fhir_resource' = 'ClaimResponse');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Adjudicator ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Plan ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `network_status` SET TAGS ('dbx_business_glossary_term' = 'Network Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `network_status` SET TAGS ('dbx_value_regex' = 'in_network|out_of_network|preferred|non_preferred');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication` ALTER COLUMN `network_status` SET TAGS ('dbx_fhir_element' = 'status');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_['fhir_resource' = 'ExplanationOfBenefit');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_'interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` SET TAGS ('dbx_'fhir_resource' = 'ClaimResponse/ExplanationOfBenefit]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Credential Record Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_name` SET TAGS ('dbx_business_glossary_term' = 'Provider Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_name` SET TAGS ('dbx_pii_type' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_business_glossary_term' = 'National Provider Identifier (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `provider_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`eob` ALTER COLUMN `sbc_reference` SET TAGS ('dbx_business_glossary_term' = 'SBC Reference');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Initiator ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjustment` ALTER COLUMN `prior_authorization_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` SET TAGS ('dbx_interop' = 'fhir');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Resolved By User Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_business_glossary_term' = 'Member Identifier (MEMBER_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`denial` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` SET TAGS ('dbx_fhir_resource' = 'Basic');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Processing User Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `claim_payer_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Payer Identifier');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `other_insurance_type` SET TAGS ('dbx_business_glossary_term' = 'Other Insurance Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `other_insurance_type` SET TAGS ('dbx_value_regex' = 'employer_group|medicare|medicaid|auto|workers_comp');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_allowed_amount` SET TAGS ('dbx_business_glossary_term' = 'Secondary Payer Allowed Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_code` SET TAGS ('dbx_business_glossary_term' = 'Secondary Payer Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_denial_reason` SET TAGS ('dbx_business_glossary_term' = 'Secondary Payer Denial Reason');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_name` SET TAGS ('dbx_business_glossary_term' = 'Secondary Payer Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_name` SET TAGS ('dbx_pii_type' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`cob` ALTER COLUMN `secondary_payer_paid_amount` SET TAGS ('dbx_business_glossary_term' = 'Secondary Payer Paid Amount');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` SET TAGS ('dbx_['fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` SET TAGS ('dbx_'fhir_resource' = 'PaymentReconciliation');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_sensitivity' = 'financial_pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_fhir_interop_ready' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_fhir_resource' = 'Claim');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_ibnr_grouper' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_mvm' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_scope' = 'MVM');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` SET TAGS ('dbx_mvm_scope' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `ibnr_id` SET TAGS ('dbx_business_glossary_term' = 'Ibnr Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Actuary ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `actuarial_method` SET TAGS ('dbx_business_glossary_term' = 'Actuarial Method');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `actuarial_method` SET TAGS ('dbx_value_regex' = 'Chain-Ladder|Bornhuetter-Ferguson');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `completion_factor` SET TAGS ('dbx_business_glossary_term' = 'Completion Factor');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = 'USD|EUR|GBP|CAD|AUD|JPY');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `currency_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `currency_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `ibnr_description` SET TAGS ('dbx_business_glossary_term' = 'Reserve Description');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `development_factor` SET TAGS ('dbx_business_glossary_term' = 'Development Factor');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `estimated_ibnr_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated IBNR Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `ibnr_status` SET TAGS ('dbx_business_glossary_term' = 'Reserve Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `ibnr_status` SET TAGS ('dbx_value_regex' = 'draft|active|approved|closed|rejected');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `ibnr_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `incurred_month` SET TAGS ('dbx_business_glossary_term' = 'Incurred Month');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `line_of_business` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `line_of_business` SET TAGS ('dbx_value_regex' = 'Medical|Dental|Vision|Behavioral');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `mlr_numerator_allocation` SET TAGS ('dbx_business_glossary_term' = 'MLR Numerator Allocation');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `paid_to_date_amount` SET TAGS ('dbx_business_glossary_term' = 'Paid To Date Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `plan_type` SET TAGS ('dbx_business_glossary_term' = 'Plan Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `plan_type` SET TAGS ('dbx_value_regex' = 'HMO|PPO|EPO|POS|HDHP');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `prior_period_reserve_amount` SET TAGS ('dbx_business_glossary_term' = 'Prior Period Reserve Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `reserve_change_amount` SET TAGS ('dbx_business_glossary_term' = 'Reserve Change Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `reserve_period_month` SET TAGS ('dbx_business_glossary_term' = 'Reserve Period Month');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `reserve_period_year` SET TAGS ('dbx_business_glossary_term' = 'Reserve Period Year');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `reserve_run_date` SET TAGS ('dbx_business_glossary_term' = 'Reserve Run Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `service_category` SET TAGS ('dbx_business_glossary_term' = 'Service Category');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `service_category` SET TAGS ('dbx_value_regex' = 'Inpatient|Outpatient|Professional|Pharmacy|Dental');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`ibnr` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_id` SET TAGS ('dbx_business_glossary_term' = 'Subrogation ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Attorney ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_city` SET TAGS ('dbx_business_glossary_term' = 'Accident City');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_city` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_country` SET TAGS ('dbx_business_glossary_term' = 'Accident Country');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_country` SET TAGS ('dbx_fhir_element' = 'address.country');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_date` SET TAGS ('dbx_business_glossary_term' = 'Accident Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_business_glossary_term' = 'Accident State Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_fhir_element' = 'address.state');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `accident_state_code` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `currency_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `currency_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `demand_amount` SET TAGS ('dbx_business_glossary_term' = 'Demand Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `demand_sent_date` SET TAGS ('dbx_business_glossary_term' = 'Demand Sent Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `gross_recovery_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Recovery Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `initiated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Initiated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `is_lien_applied` SET TAGS ('dbx_business_glossary_term' = 'Is Lien Applied');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `is_recovery_closed` SET TAGS ('dbx_business_glossary_term' = 'Is Recovery Closed');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `is_settlement_agreed` SET TAGS ('dbx_business_glossary_term' = 'Is Settlement Agreed');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `legal_fees_amount` SET TAGS ('dbx_business_glossary_term' = 'Legal Fees Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `liability_type` SET TAGS ('dbx_business_glossary_term' = 'Liability Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `liability_type` SET TAGS ('dbx_value_regex' = 'primary|secondary|joint');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_amount` SET TAGS ('dbx_business_glossary_term' = 'Lien Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_holder_name` SET TAGS ('dbx_business_glossary_term' = 'Lien Holder Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_holder_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_holder_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_holder_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_status` SET TAGS ('dbx_business_glossary_term' = 'Lien Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_status` SET TAGS ('dbx_value_regex' = 'pending|applied|released');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `lien_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `negotiation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `net_recovery_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Recovery Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `recovery_method` SET TAGS ('dbx_business_glossary_term' = 'Recovery Method');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `recovery_method` SET TAGS ('dbx_value_regex' = 'direct|offset|court');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `recovery_notes` SET TAGS ('dbx_business_glossary_term' = 'Recovery Notes');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `regulatory_reporting_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Flag');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `regulatory_reporting_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_business_glossary_term' = 'Reporting State Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_fhir_element' = 'address.state');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `reporting_state_code` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `settlement_amount` SET TAGS ('dbx_business_glossary_term' = 'Settlement Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_number` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_status` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_status` SET TAGS ('dbx_value_regex' = 'identified|demand_sent|negotiating|settled|closed|rejected');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_type` SET TAGS ('dbx_business_glossary_term' = 'Subrogation Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `subrogation_type` SET TAGS ('dbx_value_regex' = 'auto|workers_comp|premises|other');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `third_party_insurer_code` SET TAGS ('dbx_business_glossary_term' = 'Third‑Party Insurer ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `third_party_insurer_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `third_party_insurer_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`subrogation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` SET TAGS ('dbx_fhir_resource' = 'Basic');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_business_glossary_term' = 'Principal Diagnosis Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_value_regex' = '^[A-TV-Z][0-9][0-9A-Z](.[0-9A-Z]{1,4})?$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_sensitivity' = 'phi');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_fhir_element' = 'Condition.code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_health' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_diagnosis_code` SET TAGS ('dbx_fhir' = 'code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_business_glossary_term' = 'Principal Procedure Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_sensitivity' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`drg` ALTER COLUMN `principal_procedure_code` SET TAGS ('dbx_fhir_element' = 'Procedure.code');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`accumulator` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`accumulator` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`accumulator` SET TAGS ('dbx_fhir_resource' = 'Basic');
...
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` SET TAGS ('dbx_subdomain' = 'adjudication_processing');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` SET TAGS ('dbx_fhir_resource' = 'Claim');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `adjudication_rate_id` SET TAGS ('dbx_business_glossary_term' = 'Adjudication Rate ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `fee_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Fee Schedule Id');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `fee_schedule_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `adjudication_rate_status` SET TAGS ('dbx_business_glossary_term' = 'Rate Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `adjudication_rate_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `allowed_amount` SET TAGS ('dbx_business_glossary_term' = 'Allowed Amount');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `awp_percentage` SET TAGS ('dbx_business_glossary_term' = 'Average Wholesale Price (AWP) Percentage');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `currency_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `currency_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `adjudication_rate_description` SET TAGS ('dbx_business_glossary_term' = 'Rate Description');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `effective_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `geographic_cbsa` SET TAGS ('dbx_business_glossary_term' = 'Core Based Statistical Area (CBSA) Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `geographic_county` SET TAGS ('dbx_business_glossary_term' = 'Geographic County');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `geographic_state` SET TAGS ('dbx_business_glossary_term' = 'Geographic State');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `geographic_state` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `geographic_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `mac_rate` SET TAGS ('dbx_business_glossary_term' = 'Maximum Allowable Cost (MAC) Rate');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `medicare_fee_schedule_percent` SET TAGS ('dbx_business_glossary_term' = 'Medicare Fee Schedule Percentage');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `network_tier` SET TAGS ('dbx_business_glossary_term' = 'Network Tier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `network_tier` SET TAGS ('dbx_value_regex' = 'in_network|out_of_network|preferred|standard|specialty');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `place_of_service_code` SET TAGS ('dbx_business_glossary_term' = 'Place of Service Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `place_of_service_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `place_of_service_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_business_glossary_term' = 'Procedure Code (CPT/HCPCS)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$|^[A-Z0-9]{5}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_sensitivity' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_code` SET TAGS ('dbx_fhir_element' = 'Procedure.code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_modifier` SET TAGS ('dbx_business_glossary_term' = 'Procedure Modifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `procedure_modifier` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{0,2}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `provider_specialty_code` SET TAGS ('dbx_business_glossary_term' = 'Provider Specialty Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `provider_specialty_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `provider_specialty_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `provider_type` SET TAGS ('dbx_business_glossary_term' = 'Provider Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `provider_type` SET TAGS ('dbx_value_regex' = 'physician|hospital|lab|pharmacy|clinic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `rate_source` SET TAGS ('dbx_business_glossary_term' = 'Rate Source');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `rate_type` SET TAGS ('dbx_business_glossary_term' = 'Rate Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `rvu_based_flag` SET TAGS ('dbx_business_glossary_term' = 'RVU‑Based Calculation Flag');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Termination Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `termination_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`adjudication_rate` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` SET TAGS ('dbx_subdomain' = 'claim_intake');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `attachment_id` SET TAGS ('dbx_business_glossary_term' = 'Attachment ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Related Claim Line ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `parent_attachment_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Attachment Id');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `parent_attachment_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Related Claim ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `access_count` SET TAGS ('dbx_business_glossary_term' = 'Access Count');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `attachment_status` SET TAGS ('dbx_business_glossary_term' = 'Attachment Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `attachment_status` SET TAGS ('dbx_value_regex' = 'pending|received|reviewed|rejected|approved|archived');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `attachment_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `checksum_sha256` SET TAGS ('dbx_business_glossary_term' = 'SHA-256 Checksum');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `checksum_sha256` SET TAGS ('dbx_value_regex' = '^[A-Fa-f0-9]{64}$');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `confidentiality_level` SET TAGS ('dbx_business_glossary_term' = 'Confidentiality Level');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `confidentiality_level` SET TAGS ('dbx_value_regex' = 'public|confidential|restricted');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `attachment_description` SET TAGS ('dbx_business_glossary_term' = 'Attachment Description');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `document_category` SET TAGS ('dbx_business_glossary_term' = 'Document Category');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `document_category` SET TAGS ('dbx_value_regex' = 'medical_record|itemized_bill|operative_report|lab_result|prescription|authorization');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `document_subtype` SET TAGS ('dbx_business_glossary_term' = 'Document Subtype');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `encryption_method` SET TAGS ('dbx_business_glossary_term' = 'Encryption Method');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `encryption_method` SET TAGS ('dbx_value_regex' = 'aes256|des|none');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `expiration_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Expiration Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `external_document_reference` SET TAGS ('dbx_business_glossary_term' = 'External Document Identifier (DOC_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `file_name` SET TAGS ('dbx_business_glossary_term' = 'File Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `file_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `file_name` SET TAGS ('dbx_pii_type' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `file_size_bytes` SET TAGS ('dbx_business_glossary_term' = 'File Size (Bytes)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `file_type` SET TAGS ('dbx_business_glossary_term' = 'File Type (MIME)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `is_encrypted` SET TAGS ('dbx_business_glossary_term' = 'Encryption Indicator');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `is_original` SET TAGS ('dbx_business_glossary_term' = 'Original Document Indicator');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `is_sensitive` SET TAGS ('dbx_business_glossary_term' = 'Sensitive Flag');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `last_access_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Access Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Attachment Notes');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `received_method` SET TAGS ('dbx_business_glossary_term' = 'Reception Method');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `received_method` SET TAGS ('dbx_value_regex' = 'portal|fax|mail|edi|email');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `retention_period_days` SET TAGS ('dbx_business_glossary_term' = 'Retention Period (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `source_system_reference` SET TAGS ('dbx_business_glossary_term' = 'Source System Reference');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `upload_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Upload Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `virus_scan_result` SET TAGS ('dbx_business_glossary_term' = 'Virus Scan Result');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `virus_scan_result` SET TAGS ('dbx_value_regex' = 'clean|infected|unknown');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`attachment` ALTER COLUMN `virus_scanned` SET TAGS ('dbx_business_glossary_term' = 'Virus Scan Performed');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` SET TAGS ('dbx_subdomain' = 'claim_intake');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `status_event_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Status Event ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'User ID');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `parent_status_event_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Status Event Id');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `parent_status_event_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `audit_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `audit_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Event Comments');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `event_source` SET TAGS ('dbx_business_glossary_term' = 'Event Source System');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `event_source` SET TAGS ('dbx_value_regex' = 'EDI|CORE|PBM|UM|CAREMGMT|OTHER');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `is_manual` SET TAGS ('dbx_business_glossary_term' = 'Manual Override Flag');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `previous_status_code` SET TAGS ('dbx_business_glossary_term' = 'Previous Claim Status Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `previous_status_code` SET TAGS ('dbx_value_regex' = 'received|pended|adjudicated|paid|denied|adjusted');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `previous_status_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `previous_status_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Status Transition Reason Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `reason_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `reason_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Status Transition Reason Description');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `status_code` SET TAGS ('dbx_business_glossary_term' = 'Current Claim Status Code');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `status_code` SET TAGS ('dbx_value_regex' = 'received|pended|adjudicated|paid|denied|adjusted');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `status_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `status_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `user_role` SET TAGS ('dbx_business_glossary_term' = 'User Role');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`status_event` ALTER COLUMN `user_role` SET TAGS ('dbx_value_regex' = 'system|operator|provider|member');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_subdomain' = 'payment_recovery');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_status' = 'canonical');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_alias_of' = 'billing.payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_source' = 'billing.payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_concept' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_role' = 'reference');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_canonical' = 'billing.payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_fhir_resource' = 'Organization');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_ref' = 'billing.payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_owner' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` SET TAGS ('dbx_ssot_group' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `claim_payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `billing_payer_id` SET TAGS ('dbx_business_glossary_term' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `billing_payer_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `billing_payer_id` SET TAGS ('dbx_ssot_role' = 'reference');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `billing_payer_id` SET TAGS ('dbx_ssot_group' = 'payer');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_balance` SET TAGS ('dbx_business_glossary_term' = 'Payer Account Balance');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_balance` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_balance` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_currency` SET TAGS ('dbx_business_glossary_term' = 'Payer Account Currency');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Account Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_sensitivity' = 'financial_pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_number` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_status` SET TAGS ('dbx_business_glossary_term' = 'Payer Account Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_account_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Payer Address Line1');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_fhir_element' = 'address.line');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line1` SET TAGS ('dbx_fhir' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Payer Address Line2');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_fhir_element' = 'address.line');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_address_line2` SET TAGS ('dbx_fhir' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_business_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Business Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_business_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_business_number` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_business_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_business_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_city` SET TAGS ('dbx_business_glossary_term' = 'Payer City');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_city` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_business_glossary_term' = 'Payer Contact Email');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_fhir_element' = 'telecom.email');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_email' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_email` SET TAGS ('dbx_fhir' = 'telecom');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_business_glossary_term' = 'Payer Contact Name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_pii' = 'person_name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_fhir_element' = 'name.text');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_name` SET TAGS ('dbx_fhir' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_business_glossary_term' = 'Payer Contact Phone');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_fhir_element' = 'telecom.phone');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_phone' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contact_phone` SET TAGS ('dbx_fhir' = 'telecom');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_end_date` SET TAGS ('dbx_business_glossary_term' = 'Payer Contract End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Contract Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_start_date` SET TAGS ('dbx_business_glossary_term' = 'Payer Contract Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_status` SET TAGS ('dbx_business_glossary_term' = 'Payer Contract Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_contract_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_country` SET TAGS ('dbx_business_glossary_term' = 'Payer Country');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_country` SET TAGS ('dbx_fhir_element' = 'address.country');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_coverage_end_date` SET TAGS ('dbx_business_glossary_term' = 'Payer Coverage End Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_coverage_end_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_coverage_start_date` SET TAGS ('dbx_business_glossary_term' = 'Payer Coverage Start Date');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_coverage_start_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_created_at` SET TAGS ('dbx_business_glossary_term' = 'Payer Created At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_deleted_at` SET TAGS ('dbx_business_glossary_term' = 'Payer Deleted At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_lei` SET TAGS ('dbx_business_glossary_term' = 'Payer Lei');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_lei` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_lei` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_logo_url` SET TAGS ('dbx_business_glossary_term' = 'Payer Logo Url');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_notes` SET TAGS ('dbx_business_glossary_term' = 'Payer Notes');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_payment_cycle` SET TAGS ('dbx_business_glossary_term' = 'Payer Payment Cycle');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_payment_frequency` SET TAGS ('dbx_business_glossary_term' = 'Payer Payment Frequency');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payer Payment Method');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payer Payment Terms');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_rating` SET TAGS ('dbx_business_glossary_term' = 'Payer Rating');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_rating` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_rating_source` SET TAGS ('dbx_business_glossary_term' = 'Payer Rating Source');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_rating_source` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_risk_category` SET TAGS ('dbx_business_glossary_term' = 'Payer Risk Category');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Payer Risk Score');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_state` SET TAGS ('dbx_business_glossary_term' = 'Payer State');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_status` SET TAGS ('dbx_business_glossary_term' = 'Payer Status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_tax_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Tax Number');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_tax_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_tax_number` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_tax_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_tax_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_type` SET TAGS ('dbx_business_glossary_term' = 'Payer Type');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_updated_at` SET TAGS ('dbx_business_glossary_term' = 'Payer Updated At');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_website` SET TAGS ('dbx_business_glossary_term' = 'Payer Website');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_zip` SET TAGS ('dbx_business_glossary_term' = 'Payer Zip');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_zip` SET TAGS ('dbx_fhir_element' = 'address.postalCode');
ALTER TABLE `vibe_health_insurance_v1`.`claim`.`claim_payer` ALTER COLUMN `payer_zip` SET TAGS ('dbx_pii_type' = 'address');
