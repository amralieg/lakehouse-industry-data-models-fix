...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` (
    ...
    `account_class` STRING COMMENT 'Account classification',
    `account_close_date` DATE COMMENT 'Date account was closed',
    `account_holder_name` STRING COMMENT 'The account holder name for the customer account record.',
    `account_name` STRING COMMENT 'The account name of the customer account.',
    `account_number` STRING COMMENT '',
    `account_open_date` DATE COMMENT 'Date account was opened',
    `account_status` STRING COMMENT '',
    `account_type` STRING COMMENT '',
    `autopay_enrolled` BOOLEAN COMMENT 'Whether enrolled in autopay',
    `autopay_enrolled_flag` BOOLEAN COMMENT '',
    `autopay_flag` BOOLEAN COMMENT 'Whether enrolled in autopay',
    `average_monthly_usage_gallons` DECIMAL(18,2) COMMENT 'Average monthly water usage in gallons',
    `bill_format` STRING COMMENT 'Preferred bill format (paper, electronic, etc.)',
    `billing_address` STRING COMMENT '',
    `billing_cycle_code` STRING COMMENT 'Billing cycle assignment',
    `billing_email` STRING COMMENT 'Account billing email.',
    `billing_method` STRING COMMENT 'Preferred billing method',
    `billing_phone` STRING COMMENT 'Account billing phone.',
    `budget_billing_amount` DECIMAL(18,2) COMMENT 'Monthly budget billing amount',
    `budget_billing_enrolled` BOOLEAN COMMENT 'Whether enrolled in budget billing',
    `budget_billing_flag` BOOLEAN COMMENT '',
    `close_date` DATE COMMENT '',
    `closed_date` DATE COMMENT 'closed date for customer_account',
    `collection_status` STRING COMMENT 'Collection status if applicable',
    `created_at` TIMESTAMP COMMENT '',
    `created_date` TIMESTAMP COMMENT 'Record creation timestamp',
    `created_timestamp` TIMESTAMP COMMENT '',
    `credit_limit` STRING COMMENT '',
    `credit_rating` STRING COMMENT 'Internal credit rating',
    `credit_score` STRING COMMENT '',
    `credit_score_band` STRING COMMENT 'Credit risk band.',
    `current_balance` DECIMAL(18,2) COMMENT '',
    `current_balance_amount` DECIMAL(18,2) COMMENT '',
    `current_balance_amt` DECIMAL(18,2) COMMENT 'Current account balance',
    `current_balance_usd` DECIMAL(18,2) COMMENT '',
    `customer_class` STRING COMMENT '',
    `customer_name` STRING COMMENT '',
    `data_quality_flag` BOOLEAN COMMENT 'True when record passed data validation.',
    `days_delinquent` STRING COMMENT 'Number of days delinquent',
    `delinquency_count` STRING COMMENT 'Number of delinquency events',
    `delinquency_status` STRING COMMENT '',
    `deposit_amount` DECIMAL(18,2) COMMENT 'Security deposit amount held',
    `deposit_required_flag` BOOLEAN COMMENT 'Whether deposit is required',
    `ebill_enrolled` BOOLEAN COMMENT '',
    `email` STRING COMMENT '',
    `email_address` STRING COMMENT 'Primary email address',
    `enrolled_date` DATE COMMENT 'enrolled date for customer_account',
    `is_active` BOOLEAN COMMENT 'Whether account is active.',
    `is_lifeline_eligible` BOOLEAN COMMENT 'Whether eligible for lifeline/assistance rates',
    `is_paperless` BOOLEAN COMMENT '',
    `is_senior_citizen` BOOLEAN COMMENT 'Whether account holder qualifies as senior',
    `language_preference` STRING COMMENT 'Preferred language for communications',
    `last_bill_amount` DECIMAL(18,2) COMMENT 'Amount of last bill',
    `last_bill_date` DATE COMMENT 'Date of last bill generated',
    ...
    `life_support_flag` BOOLEAN COMMENT 'Whether life support equipment present',
    `low_income_flag` BOOLEAN COMMENT 'Whether low income assistance applies',
    `mailing_address_line_1` STRING COMMENT 'Mailing address line 1',
    `mailing_address_line_2` STRING COMMENT 'Mailing address line 2',
    ...
    `mailing_zip` STRING COMMENT 'Mailing ZIP code',
    `medical_baseline_flag` BOOLEAN COMMENT 'Whether medical baseline applies',
    `meter_count` STRING COMMENT 'Number of meters associated with account',
    `notes` STRING COMMENT 'Account notes',
    `open_date` DATE COMMENT '',
    `opened_date` DATE COMMENT 'opened date for customer_account',
    `paperless_billing` BOOLEAN COMMENT 'Whether enrolled in paperless billing',
    `paperless_billing_flag` BOOLEAN COMMENT '',
    `paperless_enrolled` BOOLEAN COMMENT 'Whether enrolled in paperless billing',
    `past_due_amount` DECIMAL(18,2) COMMENT 'Past due amount',
    `past_due_balance_amt` DECIMAL(18,2) COMMENT 'Past due balance amount',
    `payment_method` STRING COMMENT 'Default payment method',
    `payment_terms_days` STRING COMMENT 'Payment terms in days',
    ...
    `primary_email` STRING COMMENT 'Primary email address',
    `primary_phone` STRING COMMENT 'Primary phone number',
    `rate_schedule_code` STRING COMMENT 'Assigned rate schedule',
    `record_status` STRING COMMENT '',
    `senior_citizen_flag` BOOLEAN COMMENT 'Whether senior citizen discount applies',
    `service_address_count` STRING COMMENT 'Number of service addresses on account',
    `service_end_date` DATE COMMENT 'Date service ended if applicable',
    `service_start_date` DATE COMMENT 'Date service started',
    `service_type` STRING COMMENT 'Service type (water, sewer, both, reclaimed)',
    `shutoff_protected` BOOLEAN COMMENT 'Whether protected from shutoff',
    `shutoff_protection_reason` STRING COMMENT 'Reason for shutoff protection',
    `tax_exempt` BOOLEAN COMMENT 'Whether account is tax exempt',
    `tax_exempt_certificate` STRING COMMENT 'Tax exemption certificate number',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`person` (
    ...
    `government_id_expiration_date` DECIMAL(18,2) COMMENT 'The expiration date of the government-issued identification document in yyyy-MM-dd format. Used to ensure identity verification documents remain current and valid.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`organization` (
    ...
    `annual_revenue_range` DECIMAL(18,2) COMMENT 'Estimated annual revenue range of the organization. Used for credit assessment, account prioritization, and business development targeting.',
    ...
    `federal_tax_number` DECIMAL(18,2) COMMENT 'IRS-issued Employer Identification Number (EIN) for tax reporting and identification purposes. Nine-digit number in format XX-XXXXXXX.',
    ...
    `incorporation_state` DECIMAL(18,2) COMMENT 'Two-letter state code where the organization is legally incorporated or registered. Used for jurisdictional compliance and legal correspondence.',
    ...
    `payment_terms_days` DECIMAL(18,2) COMMENT 'Number of days allowed for payment after invoice date. Standard terms are typically 30 days; extended terms may be granted based on credit tier.',
    ...
    `tax_exempt_certificate_number` DECIMAL(18,2) COMMENT 'State-issued tax exemption certificate number for organizations claiming tax-exempt status. Required for audit compliance.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` (
    ...
    `billing_rate_schedule_id` DECIMAL(18,2) COMMENT 'Applicable billing rate schedule',
    `customer_account_id` BIGINT COMMENT 'Associated customer account',
    ...
    `auto_pay_enrolled` BOOLEAN COMMENT 'Whether enrolled in automatic payment',
    `auto_pay_flag` BOOLEAN COMMENT '',
    `auto_renew` BOOLEAN COMMENT 'Auto renew',
    `auto_renew_flag` BOOLEAN COMMENT '',
    `backflow_device_required` BOOLEAN COMMENT 'Whether backflow prevention required',
    `backflow_prevention_required` BOOLEAN COMMENT 'Whether backflow prevention device is required',
    `backflow_required` BOOLEAN COMMENT 'Whether backflow prevention required',
    `backflow_required_flag` BOOLEAN COMMENT 'Backflow prevention required',
    `billing_cycle_code` STRING COMMENT 'Assigned billing cycle code',
    `billing_frequency` STRING COMMENT 'Billing frequency (monthly, quarterly)',
    `capacity_fee_amount` DECIMAL(18,2) COMMENT 'Capacity fee amount paid',
    `capacity_fee_paid` BOOLEAN COMMENT 'Whether capacity/impact fee has been paid',
    `connection_fee_amount` DECIMAL(18,2) COMMENT 'Connection fee paid',
    `connection_size_inches` DECIMAL(18,2) COMMENT 'Service connection size in inches',
    `contract_term_months` STRING COMMENT '',
    `contracted_demand_gpd` DECIMAL(18,2) COMMENT '',
    `contracted_volume` STRING COMMENT '',
    `contracted_volume_gallons` BIGINT COMMENT 'Contracted monthly volume in gallons if applicable',
    `created_at` TIMESTAMP COMMENT '',
    `created_date` TIMESTAMP COMMENT 'Record creation timestamp',
    `created_timestamp` TIMESTAMP COMMENT '',
    `data_quality_flag` BOOLEAN COMMENT 'True when record passed data validation.',
    `demand_charge_amount` DECIMAL(18,2) COMMENT 'Demand charge if applicable',
    `deposit_amount` DECIMAL(18,2) COMMENT 'Required deposit amount',
    `deposit_amount_usd` DECIMAL(18,2) COMMENT '',
    `deposit_required` BOOLEAN COMMENT 'Whether deposit is required',
    `deposit_required_flag` BOOLEAN COMMENT 'Whether a deposit is required',
    `effective_date` DATE COMMENT 'Date the agreement becomes effective',
    `effective_end_date` DATE COMMENT '',
    `effective_start_date` DATE COMMENT '',
    `end_date` DATE COMMENT '',
    `estimated_annual_usage_gal` DECIMAL(18,2) COMMENT 'Estimated annual usage in gallons',
    `estimated_annual_usage_gallons` DECIMAL(18,2) COMMENT 'Estimated annual usage',
    `expiration_date` DATE COMMENT 'Date the agreement expires',
    `fire_service_flag` BOOLEAN COMMENT 'Whether includes fire service',
    `flat_rate_amount` DECIMAL(18,2) COMMENT 'Flat rate amount if applicable',
    `flat_rate_flag` BOOLEAN COMMENT 'Whether flat rate billing applies',
    `irrigation_flag` BOOLEAN COMMENT 'Whether includes irrigation service',
    `is_active` BOOLEAN COMMENT 'Whether agreement is active.',
    `is_auto_renew` BOOLEAN COMMENT '',
    `is_tax_exempt` BOOLEAN COMMENT 'Whether the account is tax exempt',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `meter_size_inches` DECIMAL(18,2) COMMENT 'Meter size in inches for this service',
    `minimum_charge` DECIMAL(18,2) COMMENT 'Minimum monthly charge',
    `minimum_charge_amount` DECIMAL(18,2) COMMENT 'Minimum monthly charge',
    `minimum_charge_usd` DECIMAL(18,2) COMMENT 'Minimum monthly charge in USD',
    `monthly_base_charge_usd` DECIMAL(18,2) COMMENT '',
    `monthly_charge` DECIMAL(18,2) COMMENT '',
    `monthly_charge_amount` DECIMAL(18,2) COMMENT '',
    `monthly_charge_usd` STRING COMMENT '',
    `monthly_service_charge` DECIMAL(18,2) COMMENT 'Monthly service charge',
    `notes` STRING COMMENT 'Agreement notes',
    `paperless_billing` BOOLEAN COMMENT 'Whether enrolled in paperless billing',
    `rate_class` STRING COMMENT '',
    `rate_code` STRING COMMENT 'Applied rate code.',
    `rate_schedule_code` STRING COMMENT 'Assigned rate schedule',
    `reclaimed_water_flag` BOOLEAN COMMENT 'Whether this is reclaimed water service',
    `record_status` STRING COMMENT '',
    `seasonal_end_month` STRING COMMENT 'Seasonal service end month',
    `seasonal_flag` BOOLEAN COMMENT 'Whether this is a seasonal service',
    `seasonal_start_month` STRING COMMENT 'Seasonal service start month',
    `service_class` STRING COMMENT 'Service class designation',
    `service_class_code` STRING COMMENT 'Service class code',
    `service_type` STRING COMMENT 'Type of service (water, sewer, stormwater, reclaimed)',
    `special_conditions` STRING COMMENT 'Special conditions or notes on the agreement',
    `start_date` DATE COMMENT '',
    `service_agreement_status` STRING COMMENT 'Current agreement status (active, pending, terminated, suspended)',
    `termination_date` DATE COMMENT 'Actual termination date if terminated early',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_person_rel` (
    ...
    `landlord_tenant_indicator` BOOLEAN COMMENT 'Specifies whether this person is the property owner (landlord), renter (tenant), owner-occupant, or property manager. Used for billing responsibility determination and compliance reporting.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`segment` (
    ...
    `rate_case_docket_number` DECIMAL(18,2) COMMENT 'Regulatory docket or case number associated with the rate case that established or modified this customer segment. Used for audit trail and regulatory compliance.',
    `rate_tier` DECIMAL(18,2) COMMENT 'Applicable rate tier or rate schedule identifier associated with this customer segment. Links segment to pricing structure for billing purposes.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`service_application` (
    ...
    `service_assigned_to_user_employee_id` BIGINT COMMENT 'User ID of the utility staff member currently assigned to review and process this service application.',
    ...
    `service_customer_customer_account_id` BIGINT COMMENT 'Reference to the customer (applicant) who submitted this service application. Links to the customer master record.',
    `service_employee_id` BIGINT COMMENT 'User ID of the utility staff member who approved the service application.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_status_history` (
    ...
    `employee_id` BIGINT COMMENT 'The system user identifier of the person or process that created this status history record. Supports audit trail and data quality investigations.',
    `account_employee_id` BIGINT COMMENT 'The system user identifier of the person who initiated or approved the status transition. May be a customer service representative, billing clerk, or system administrator.',
    ...
    `account_payment_plan_id` BIGINT COMMENT 'Reference to an active payment arrangement or installment plan associated with the status transition. Used to track compliance with payment agreements.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`contact` (
    ...
    `value` DECIMAL(18,2) COMMENT 'The actual contact information value - phone number, email address, mailing address, SMS number, portal username, or fax number. Format varies by contact_channel. This is the primary contact data point.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`communication_preference` (
    ...
    `communication_customer_customer_account_id` BIGINT COMMENT 'Reference to the customer account for which these communication preferences apply.',
    ...
    `payment_confirmation_channel` DECIMAL(18,2) COMMENT 'Preferred channel for payment confirmation receipts and acknowledgments.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` (
    ...
    `active_flag` BOOLEAN COMMENT 'Whether program is active',
    `administering_agency` STRING COMMENT 'Agency administering the program',
    `administering_department` STRING COMMENT '',
    `annual_budget` DECIMAL(18,2) COMMENT 'Annual program budget',
    `annual_budget_amount` DECIMAL(18,2) COMMENT 'Annual budget allocated to this program',
    `annual_budget_usd` DECIMAL(18,2) COMMENT '',
    `application_deadline` DATE COMMENT 'Application deadline date',
    `application_deadline_date` DATE COMMENT 'Application deadline',
    `application_required` BOOLEAN COMMENT 'Whether application is required',
    `application_required_flag` BOOLEAN COMMENT '',
    `benefit_amount` DECIMAL(18,2) COMMENT '',
    `benefit_amount_usd` STRING COMMENT '',
    `benefit_frequency` STRING COMMENT 'How often benefit is applied (one-time, monthly, annual)',
    `benefit_pct` DECIMAL(18,2) COMMENT '',
    `benefit_percentage` DECIMAL(18,2) COMMENT 'Benefit percentage.',
    `benefit_period_months` STRING COMMENT 'Benefit period in months',
    `benefit_type` STRING COMMENT '',
    `contact_email` STRING COMMENT 'Program contact email',
    `contact_phone` STRING COMMENT 'Program contact phone number',
    `created_at` TIMESTAMP COMMENT '',
    `created_date` TIMESTAMP COMMENT 'Record creation timestamp',
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp',
    `current_enrollment` STRING COMMENT 'Current enrollment count',
    `current_enrollment_count` STRING COMMENT 'Current number of active enrollees',
    `data_quality_flag` BOOLEAN COMMENT 'True when record passed data validation.',
    `assistance_program_description` STRING COMMENT '',
    `discount_pct` DECIMAL(18,2) COMMENT '',
    `discount_percent` DECIMAL(18,2) COMMENT '',
    `discount_percentage` DECIMAL(18,2) COMMENT 'Percentage discount applied to bills',
    `discount_rate_pct` DECIMAL(18,2) COMMENT '',
    `documentation_required` BOOLEAN COMMENT 'Required documentation',
    `effective_date` DATE COMMENT 'Date the program becomes effective',
    `effective_end_date` DATE COMMENT '',
    `effective_start_date` DATE COMMENT '',
    `eligibility_criteria` STRING COMMENT '',
    `eligibility_income_threshold_pct` DECIMAL(18,2) COMMENT '',
    `end_date` DATE COMMENT '',
    `enrollment_cap` STRING COMMENT 'Maximum number of enrollees allowed',
    `enrollment_end_date` DATE COMMENT '',
    `enrollment_period_end` DATE COMMENT 'Enrollment period end date',
    `enrollment_period_start` DATE COMMENT 'Enrollment period start date',
    `enrollment_start_date` DATE COMMENT '',
    `expiration_date` DATE COMMENT 'Date the program expires',
    `funding_cap_amount` DECIMAL(18,2) COMMENT '',
    `funding_source` STRING COMMENT 'Source of program funding (federal, state, local, utility)',
    `funding_source_type` STRING COMMENT '',
    `household_size_max` STRING COMMENT 'Maximum household size eligible',
    `income_threshold_pct_fpl` DECIMAL(18,2) COMMENT 'Income eligibility threshold as percent of Federal Poverty Level',
    `is_active` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `max_benefit_amount` DECIMAL(18,2) COMMENT 'Maximum benefit amount per enrollment',
    `max_benefit_amount_usd` DECIMAL(18,2) COMMENT '',
    `max_enrollment` STRING COMMENT 'Maximum enrollment capacity',
    `max_enrollment_count` STRING COMMENT 'Maximum number of enrollees',
    `program_code` STRING COMMENT '',
    `program_description` STRING COMMENT 'Detailed program description',
    `program_end_date` DATE COMMENT 'Program end date',
    `program_name` STRING COMMENT '',
    `program_start_date` DATE COMMENT 'Program effective start date',
    `program_status` STRING COMMENT 'Current status (active, closed, suspended)',
    `program_type` STRING COMMENT '',
    `recertification_interval_months` STRING COMMENT 'Months between recertification',
    `recertification_period_months` STRING COMMENT 'Recertification period in months',
    `recertification_required` BOOLEAN COMMENT 'Whether recertification is required',
    `recertification_required_flag` BOOLEAN COMMENT '',
    `record_status` STRING COMMENT '',
    `regulatory_authority` STRING COMMENT '',
    `remaining_budget` DECIMAL(18,2) COMMENT 'Remaining budget for current period',
    `renewal_period_months` STRING COMMENT 'Renewal period in months',
    `renewal_required` BOOLEAN COMMENT 'Whether annual renewal required',
    `requires_annual_recertification` BOOLEAN COMMENT 'Whether annual recertification is required',
    `requires_recertification` BOOLEAN COMMENT 'Whether periodic recertification is required',
    `start_date` DATE COMMENT '',
    `updated_at` TIMESTAMP COMMENT '',
    `updated_date` TIMESTAMP COMMENT 'Record last update timestamp',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
    `website_url` STRING COMMENT 'Program website URL',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` (
    ...
    `employee_id` BIGINT COMMENT 'Employee who approved enrollment',
    ...
    `customer_account_id` BIGINT COMMENT 'Enrolled customer account',
    ...
    `annual_household_income` DECIMAL(18,2) COMMENT 'Annual household income',
    `annual_household_income_usd` DECIMAL(18,2) COMMENT '',
    `application_date` DATE COMMENT 'Date of application',
    `approval_date` DATE COMMENT '',
    `approved_benefit_amount` DECIMAL(18,2) COMMENT 'Approved benefit amount.',
    `approved_benefit_amount_usd` DECIMAL(18,2) COMMENT '',
    `approved_date` DATE COMMENT 'Approved date',
    `approved_flag` BOOLEAN COMMENT 'Whether enrollment approved',
    `benefit_amount` DECIMAL(18,2) COMMENT '',
    `benefit_amount_applied` DECIMAL(18,2) COMMENT '',
    `benefit_type` STRING COMMENT 'Type of benefit (credit, discount_pct, fixed_amount)',
    `billing_assistance_enrollment_id` BIGINT COMMENT 'References the single source of truth master record in billing.billing_assistance_enrollment',
    `case_worker_name` STRING COMMENT 'Assigned case worker name',
    `certification_expiration_date` DATE COMMENT '',
    `created_at` TIMESTAMP COMMENT '',
    `created_date` TIMESTAMP COMMENT 'Record creation timestamp',
    `created_timestamp` TIMESTAMP COMMENT '',
    `customer_ssot_billing_assistance_enrollment_ref_id` BIGINT COMMENT 'Canonical single-source-of-truth reference to billing.billing_assistance_enrollment (billing_assistance_enrollment_id); this entity defers authoritative attribute values to that master entity.',
    `data_quality_flag` BOOLEAN COMMENT 'True when record passed data validation.',
    `denial_reason` STRING COMMENT 'Reason for denial if applicable',
    `discount_pct` DECIMAL(18,2) COMMENT '',
    `discount_pct_applied` DECIMAL(18,2) COMMENT 'Discount percentage applied',
    `discount_percent_applied` DECIMAL(18,2) COMMENT '',
    `discount_percentage` DECIMAL(18,2) COMMENT 'Applied discount percentage',
    `documentation_on_file` BOOLEAN COMMENT 'Whether documentation is on file',
    `effective_date` DATE COMMENT 'Benefit effective date',
    `effective_end_date` DATE COMMENT 'Benefit effective end date',
    `effective_start_date` DATE COMMENT 'Benefit effective start date',
    `eligibility_review_date` DATE COMMENT '',
    `eligibility_verified_flag` BOOLEAN COMMENT '',
    `enrollment_date` DATE COMMENT '',
    `enrollment_number` STRING COMMENT 'Unique enrollment number',
    `enrollment_status` STRING COMMENT '',
    `expiration_date` DATE COMMENT '',
    `household_income` DECIMAL(18,2) COMMENT 'Reported household income',
    `household_size` STRING COMMENT 'Household size',
    `income_verification_date` DATE COMMENT 'Date income was verified',
    `income_verification_method` STRING COMMENT 'Method of income verification',
    `income_verified` BOOLEAN COMMENT 'Whether income was verified.',
    `income_verified_flag` BOOLEAN COMMENT '',
    `is_active` BOOLEAN COMMENT '',
    `is_auto_renewed` BOOLEAN COMMENT 'Whether enrollment auto-renews',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_recertification_date` DATE COMMENT 'Date of last recertification',
    `last_renewal_date` DATE COMMENT 'Date of last renewal',
    `monthly_benefit_amount` DECIMAL(18,2) COMMENT 'Monthly benefit amount',
    `next_recertification_date` DATE COMMENT 'Next recertification due date',
    `notes` STRING COMMENT 'Enrollment notes',
    `recertification_date` DATE COMMENT 'Recertification date',
    `recertification_due_date` DATE COMMENT 'Recertification due date.',
    `recertification_status` STRING COMMENT 'Recertification status',
    `record_status` STRING COMMENT '',
    `remaining_benefit` DECIMAL(18,2) COMMENT 'Remaining benefit amount',
    `renewal_due_date` DATE COMMENT 'Date renewal is due',
    `termination_date` DATE COMMENT 'Actual termination date',
    `termination_reason` STRING COMMENT 'Reason for termination',
    `total_benefit_received` DECIMAL(18,2) COMMENT 'Total benefit received to date',
    `total_benefits_received` DECIMAL(18,2) COMMENT 'Total benefits received to date',
    `total_disbursed` DECIMAL(18,2) COMMENT 'Total amount disbursed to date',
    `updated_at` TIMESTAMP COMMENT '',
    `updated_date` TIMESTAMP COMMENT 'Record last update timestamp',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_note` (
    ...
    `note_type` STRING COMMENT 'Attribute note_type for account_note',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`interaction` (
    ...
    `interaction_employee_id` BIGINT COMMENT 'Identifier of the customer service agent or representative who handled the interaction. Null for self-service interactions.',
    `interaction_person_id` BIGINT COMMENT 'FK to person',
    ...
    `duration_seconds` DECIMAL(18,2) COMMENT 'Total duration of the interaction in seconds, applicable primarily to call and chat channels. Null for asynchronous channels like email.',
    `escalated_flag` BOOLEAN COMMENT 'Escalated flag',
    ...
    `follow_up_required_flag` BOOLEAN COMMENT 'Follow up required flag',
    `interaction_channel` STRING COMMENT 'Interaction channel',
    `interaction_date` DATE COMMENT 'Interaction date',
    ...
    `notes` STRING COMMENT 'Attribute notes for interaction',
    ...
    `resolution_code` STRING COMMENT 'Resolution code',
    ...
    `resolution_status` STRING COMMENT 'Attribute resolution_status for interaction',
    ...
    `satisfaction_score` DECIMAL(18,2) COMMENT 'Customer satisfaction score',
    ...
    `summary` STRING COMMENT 'Interaction summary',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` (
    ...
    `customer_assigned_to_user_employee_id` BIGINT COMMENT 'Reference to the employee or user assigned as the resolution owner for this complaint.',
    `customer_employee_id` BIGINT COMMENT 'Reference to the employee or user assigned as the resolution owner for this complaint.',
    `person_id` BIGINT COMMENT 'FK to person',
    `customer_reported_by_person_id` BIGINT COMMENT 'Foreign key linking to customer.person. Business justification: Complaints capture reporter details that should reference the person master when the reporter is a known person. This eliminates duplication and enables proper reporter tracking. Nullable as some comp',
    ...
    `metering_complaint_id` BIGINT COMMENT 'Cross-reference to metering complaint',
    ...
    `complaint_date` DATE COMMENT 'Complaint date',
    ...
    `complaint_type` STRING COMMENT 'Complaint type',
    ...
    `customer_satisfaction_score` DECIMAL(18,2) COMMENT 'Customer satisfaction score',
    `days_to_resolve` STRING COMMENT 'Days to resolve',
    `customer_complaint_description` STRING COMMENT 'Attribute description for customer_complaint',
    `escalated_flag` BOOLEAN COMMENT 'Escalated flag',
    ...
    `received_date` DATE COMMENT 'Attribute received_date for customer_complaint',
    ...
    `regulatory_reportable_flag` BOOLEAN COMMENT 'Regulatory reportable flag',
    ...
    `resolution_notes` STRING COMMENT 'Attribute resolution_notes for customer_complaint',
    ...
    `resolved_date` DATE COMMENT 'Attribute resolved_date for customer_complaint',
    ...
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
    `water_quality_related_flag` BOOLEAN COMMENT 'Water quality related flag',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_hierarchy` (
    ...
    `child_customer_account_id` BIGINT COMMENT 'FK to child account',
    `parent_customer_account_id` BIGINT COMMENT 'FK to parent account',
    ...
    `billing_consolidated_flag` BOOLEAN COMMENT 'Billing consolidated flag',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`deposit` (
    ...
    `accrued_interest` DECIMAL(18,2) COMMENT 'Total accrued interest',
    `amount` DECIMAL(18,2) COMMENT '',
    `amount_applied_to_balance` DECIMAL(18,2) COMMENT 'Amount applied to account balance',
    `amount_held` DECIMAL(18,2) COMMENT 'Current amount held',
    `amount_held_usd` DECIMAL(18,2) COMMENT '',
    `amount_refunded` STRING COMMENT '',
    `amount_refunded_usd` DECIMAL(18,2) COMMENT '',
    `amount_usd` DECIMAL(18,2) COMMENT '',
    `applied_amount` DECIMAL(18,2) COMMENT 'Amount applied to balance',
    `applied_to_balance_date` DATE COMMENT 'Date applied to account balance',
    `applied_to_balance_flag` BOOLEAN COMMENT 'Applied to balance flag',
    `check_number` STRING COMMENT 'Check number if paid by check',
    `collected_date` DATE COMMENT '',
    `created_at` TIMESTAMP COMMENT '',
    `created_date` TIMESTAMP COMMENT 'Record creation timestamp',
    `created_timestamp` TIMESTAMP COMMENT '',
    `credit_score_at_deposit` STRING COMMENT 'Credit score at time of deposit',
    `current_balance` DECIMAL(18,2) COMMENT 'Current deposit balance',
    `data_quality_flag` BOOLEAN COMMENT 'True when record passed data validation.',
    `deposit_date` DATE COMMENT '',
    `deposit_number` STRING COMMENT 'Unique deposit reference number',
    `deposit_status` STRING COMMENT '',
    `deposit_type` STRING COMMENT '',
    `eligible_for_refund` BOOLEAN COMMENT 'Whether currently eligible for refund',
    `forfeiture_date` DATE COMMENT 'Date deposit was forfeited',
    `forfeiture_reason` STRING COMMENT 'Reason for forfeiture',
    `interest_accrued` DECIMAL(18,2) COMMENT 'Accrued interest amount.',
    `interest_accrued_amount` DECIMAL(18,2) COMMENT '',
    `interest_accrued_usd` DECIMAL(18,2) COMMENT '',
    `interest_rate` DECIMAL(18,2) COMMENT '',
    `interest_rate_pct` DECIMAL(18,2) COMMENT '',
    `is_refundable` BOOLEAN COMMENT 'Whether deposit is refundable.',
    `is_refunded` BOOLEAN COMMENT '',
    `last_interest_calc_date` DATE COMMENT 'Date of last interest calculation',
    `last_interest_date` DATE COMMENT 'Date interest was last calculated',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `months_good_payment_history` STRING COMMENT 'Months of good payment history',
    `notes` STRING COMMENT 'Deposit notes',
    `payment_method` STRING COMMENT 'Method of payment',
    `reason` STRING COMMENT 'Reason deposit was required',
    `reason_code` STRING COMMENT 'Reason code for deposit requirement',
    `receipt_number` STRING COMMENT 'Payment receipt number',
    `received_date` DATE COMMENT 'Date deposit received.',
    `record_status` STRING COMMENT '',
    `refund_amount` DECIMAL(18,2) COMMENT 'Amount refunded',
    `refund_date` DATE COMMENT '',
    `refund_due_date` DATE COMMENT '',
    `refund_eligible_date` DATE COMMENT 'Date deposit becomes refund eligible',
    `refund_method` STRING COMMENT 'Method of refund',
    `refund_threshold_months` STRING COMMENT 'Months required for refund eligibility',
    `refunded_amount` DECIMAL(18,2) COMMENT '',
    `refunded_date` DATE COMMENT '',
    `required_by_policy` BOOLEAN COMMENT 'Whether deposit was required by policy',
    `review_date` DATE COMMENT 'Date deposit eligibility was last reviewed',
    `updated_at` TIMESTAMP COMMENT '',
    `updated_date` TIMESTAMP COMMENT 'Record last update timestamp',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
    `waiver_approved` BOOLEAN COMMENT 'Whether a waiver was approved',
    `waiver_approved_by` STRING COMMENT 'Waiver approved by',
    `waiver_reason` STRING COMMENT 'Waiver reason',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` (
    ...
    `third_person_id` BIGINT COMMENT 'FK to person',
    ...
    `enrollment_date` DATE COMMENT 'Attribute enrollment_date for third_party_notification',
    ...
    `notification_status` STRING COMMENT 'Notification status',
    `notification_trigger` STRING COMMENT 'Trigger event for notification',
    ...
    `notification_type` STRING COMMENT 'Notification type',
    ...
    `third_party_address` STRING COMMENT 'Third party address',
    `third_party_email` STRING COMMENT 'Third party email',
    ...
    `third_party_phone` STRING COMMENT 'Third party phone',
    `third_party_relationship` STRING COMMENT 'Relationship to account holder',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_document` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user who approved the document.',
    `account_employee_id` BIGINT COMMENT 'Identifier of the user or system account that uploaded the document.',
    `account_last_modified_by_user_employee_id` BIGINT COMMENT 'Identifier of the user who last modified the document record.',
    `account_uploaded_by_employee_id` BIGINT COMMENT 'FK to uploading employee',
    ...
    `account_document_description` STRING COMMENT 'Document description',
    ...
    `document_url` STRING COMMENT 'Attribute document_url for account_document',
    ...
    `upload_date` DATE COMMENT 'Attribute upload_date for account_document',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` (
    ...
    `customer_program_id` BIGINT COMMENT 'FK to program',
    `customer_service_program_enrollment_id` BIGINT COMMENT 'References the single source of truth record in service.service_program_enrollment',
    `service_address_id` BIGINT COMMENT 'FK to service address for program enrollment',
    ...
    `territory_id` BIGINT COMMENT 'Service territory associated with the enrollment.',
    `actual_reduction_pct` DECIMAL(18,2) COMMENT 'Actual percentage reduction in water usage achieved.',
    `actual_reduction_percent` DECIMAL(18,2) COMMENT 'Actual percentage reduction achieved',
    `annual_savings_gallons` BIGINT COMMENT 'Projected annual water savings in gallons',
    `application_date` DATE COMMENT 'Date customer applied for program',
    `approval_date` DATE COMMENT 'Date enrollment was approved',
    `audit_completed_flag` BOOLEAN COMMENT 'Whether water audit was completed',
    `auto_enrolled_flag` BOOLEAN COMMENT 'Auto enrolled flag',
    `auto_renewal_flag` BOOLEAN COMMENT 'Whether enrollment auto-renews',
    `baseline_usage_gallons` BIGINT COMMENT 'Baseline water usage before program enrollment',
    `benefit_amount` DECIMAL(18,2) COMMENT 'Benefit amount',
    `carbon_offset_tons` DECIMAL(18,2) COMMENT 'Estimated carbon offset in tons from program participation',
    ...
    `co_benefit_program_flag` BOOLEAN COMMENT 'Whether enrolled in co-benefit program',
    `communication_preference` STRING COMMENT 'Preferred communication method',
    `contractor_name` STRING COMMENT 'Name of installation contractor',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the enrollment record was created.',
    `customer_satisfaction_score` DECIMAL(18,2) COMMENT 'Customer satisfaction score (1-5)',
    `denial_reason` STRING COMMENT 'Reason for denial if application rejected',
    `device_installed_flag` BOOLEAN COMMENT 'Whether a conservation device was installed',
    `device_type` STRING COMMENT 'Type of conservation device installed (e.g., low-flow showerhead, smart irrigation controller).',
    `disability_accommodation_flag` BOOLEAN COMMENT 'Whether disability accommodation is needed',
    `discount_percentage` DECIMAL(18,2) COMMENT 'Discount percentage',
    `disenrollment_date` DATE COMMENT 'Date the customer was disenrolled from the program.',
    `disenrollment_reason` STRING COMMENT 'Reason for program disenrollment if applicable.',
    `effective_date` DATE COMMENT 'Effective date',
    `eligibility_verification_date` DATE COMMENT 'Eligibility verification date',
    `eligibility_verified_flag` BOOLEAN COMMENT 'Eligibility verified flag',
    `enrollment_channel` STRING COMMENT 'Channel through which the customer enrolled (e.g., online, phone, in-person, mail).',
    ...
    `enrollment_number` STRING COMMENT '',
    ...
    `equipment_serial_number` STRING COMMENT 'Serial number of installed equipment',
    `estimated_payback_months` STRING COMMENT 'Estimated payback period in months for the program investment',
    ...
    `income_qualified_flag` BOOLEAN COMMENT 'Whether customer is income-qualified for program',
    `inspection_date` DATE COMMENT 'Date of the most recent program compliance inspection.',
    `language_preference` STRING COMMENT 'Preferred language for program communications',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `lifetime_savings_gallons` BIGINT COMMENT 'Total lifetime water savings achieved',
    `marketing_channel` STRING COMMENT 'Marketing channel through which customer enrolled',
    `notes` STRING COMMENT 'Enrollment notes',
    ...
    `post_enrollment_usage_gallons` BIGINT COMMENT 'Water usage in gallons after program enrollment',
    `post_installation_inspection_date` DATE COMMENT 'Date of post-installation inspection',
    `pre_enrollment_usage_gallons` BIGINT COMMENT 'Water usage in gallons before program enrollment',
    `program_tier` STRING COMMENT 'Tier level within the program',
    `program_type` STRING COMMENT 'Type of program enrolled in',
    ...
    `recertification_due_date` DATE COMMENT 'Recertification due date',
    `referral_source` STRING COMMENT 'Source of program referral',
    `renewal_reminder_sent_date` DATE COMMENT 'Date renewal reminder was sent',
    `senior_discount_eligible` BOOLEAN COMMENT 'Whether customer is eligible for senior discount',
    `ssot_source_domain` STRING COMMENT 'Indicates SSOT source is service.service_program_enrollment',
    `target_reduction_pct` DECIMAL(18,2) COMMENT 'Target percentage reduction in water usage for the program.',
    `target_reduction_percent` DECIMAL(18,2) COMMENT 'Target percentage reduction in water usage',
    `target_savings_gallons` DECIMAL(18,2) COMMENT 'Target water savings in gallons',
    `termination_date` DATE COMMENT 'Termination date',
    `termination_reason` STRING COMMENT 'Termination reason',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the enrollment record was last updated.',
    `verification_date` DATE COMMENT 'Date savings were verified',
    `verification_method` STRING COMMENT 'Method used to verify savings',
    `veteran_status` BOOLEAN COMMENT 'Whether customer is a veteran',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_enforcement_impact` (
    ...
    `customer_account_id` BIGINT COMMENT 'Foreign key linking to the affected customer account',
    ...
    `fine_amount` DECIMAL(18,2) COMMENT 'Fine amount',
    `impact_date` DATE COMMENT 'Attribute impact_date for account_enforcement_impact',
    `impact_description` STRING COMMENT 'Impact description',
    `impact_end_date` DATE COMMENT 'Impact end date',
    ...
    `impact_start_date` DATE COMMENT 'Impact start date',
    `impact_type` STRING COMMENT 'Impact type',
    ...
    `service_restriction_flag` BOOLEAN COMMENT 'Service restriction flag',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` (
    ...
    `case_id` BIGINT COMMENT 'Customer case opened for the overflow impact.',
    `customer_account_id` BIGINT COMMENT 'FK to customer account impacted',
    ...
    `quality_public_notification_id` BIGINT COMMENT 'FK to public notification issued',
    `service_address_id` BIGINT COMMENT 'FK to service address impacted',
    `affected_area_sqft` DECIMAL(18,2) COMMENT 'Area affected by overflow in square feet',
    `alternative_water_provided` BOOLEAN COMMENT 'Whether alternative water was provided',
    `basement_flooding_flag` BOOLEAN COMMENT 'Whether basement flooding occurred',
    `boil_water_notice_issued` BOOLEAN COMMENT 'Whether boil water notice was issued',
    `claim_status` STRING COMMENT 'Status of the insurance or compensation claim.',
    ...
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the impact record was created.',
    ...
    `customer_contacted_timestamp` TIMESTAMP COMMENT 'Timestamp when customer was contacted',
    `damage_assessment_amount` DECIMAL(18,2) COMMENT 'Amount from damage assessment',
    `duration_hours` DECIMAL(18,2) COMMENT 'Duration of overflow impact in hours',
    `duration_of_impact_hours` DECIMAL(18,2) COMMENT 'Duration of the overflow impact in hours.',
    `environmental_damage_flag` BOOLEAN COMMENT 'Indicates if environmental damage occurred',
    `estimated_damage_amount` DECIMAL(18,2) COMMENT 'Estimated monetary value of property damage.',
    `estimated_volume_gallons` DECIMAL(18,2) COMMENT 'Estimated overflow volume affecting the premise in gallons',
    `evacuation_required` BOOLEAN COMMENT 'Indicates whether evacuation of the premise was required.',
    `evacuation_required_flag` BOOLEAN COMMENT 'Indicates whether evacuation of the premise was required.',
    `follow_up_date` DATE COMMENT 'Scheduled date for follow-up',
    `follow_up_required_flag` BOOLEAN COMMENT 'Indicates if follow-up action is required',
    `health_impact_flag` BOOLEAN COMMENT 'Indicates whether health impacts were reported.',
    `health_risk_flag` BOOLEAN COMMENT 'Whether a health risk was identified',
    `impact_end_timestamp` TIMESTAMP COMMENT 'Impact end timestamp',
    ...
    `impact_start_timestamp` TIMESTAMP COMMENT 'Impact start timestamp',
    ...
    `insurance_claim_filed` BOOLEAN COMMENT 'Indicates whether an insurance claim was filed for the damage.',
    `insurance_claim_filed_flag` BOOLEAN COMMENT 'Whether an insurance claim was filed',
    `insurance_claim_number` STRING COMMENT 'Insurance claim number if filed',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `legal_action_pending` BOOLEAN COMMENT 'Indicates if legal action is pending',
    `media_coverage_flag` BOOLEAN COMMENT 'Indicates if the incident received media coverage',
    `notes` STRING COMMENT 'Additional notes about the overflow impact on this premise.',
    ...
    `notification_sent_flag` BOOLEAN COMMENT 'Notification sent flag',
    `odor_complaint_flag` BOOLEAN COMMENT 'Whether odor complaint was filed',
    `overflow_end_timestamp` TIMESTAMP COMMENT 'Timestamp when overflow impact ended at premise',
    `overflow_start_timestamp` TIMESTAMP COMMENT 'Timestamp when overflow impact began at premise',
    `photo_documentation_flag` BOOLEAN COMMENT 'Whether photo documentation was collected',
    `population_affected` STRING COMMENT 'Population affected',
    `property_damage_description` STRING COMMENT 'Description of property damage',
    `property_damage_flag` BOOLEAN COMMENT 'Whether property damage occurred',
    `regulatory_report_required` BOOLEAN COMMENT 'Indicates if regulatory reporting is required',
    `relocation_days` STRING COMMENT 'Number of days of temporary relocation',
    `remediation_cost` DECIMAL(18,2) COMMENT 'Cost of remediation activities',
    `resolution_status` STRING COMMENT 'Current resolution status',
    `restoration_contractor_name` STRING COMMENT 'Name of the restoration contractor engaged',
    `restoration_date` DATE COMMENT 'Date when service was fully restored to the premise.',
    `restoration_start_date` DATE COMMENT 'Date when restoration/cleanup work began.',
    `service_disruption_flag` BOOLEAN COMMENT 'Service disruption flag',
    `service_interruption_hours` DECIMAL(18,2) COMMENT 'Duration of service interruption in hours.',
    `sewage_backup_flag` BOOLEAN COMMENT 'Whether sewage backup into premise occurred',
    `street_flooding_flag` BOOLEAN COMMENT 'Whether street flooding occurred',
    `temporary_relocation_required` BOOLEAN COMMENT 'Whether temporary relocation was required',
    `third_party_damage_assessment` BOOLEAN COMMENT 'Whether third party damage assessment was conducted',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
    `video_documentation_flag` BOOLEAN COMMENT 'Whether video documentation was collected',
    `water_quality_advisory_issued` BOOLEAN COMMENT 'Whether water quality advisory was issued',
    `yard_flooding_flag` BOOLEAN COMMENT 'Whether yard flooding occurred',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`sampling_participation` (
    ...
    `consent_date` DATE COMMENT 'Consent date',
    `consent_flag` BOOLEAN COMMENT '',
    `consent_method` STRING COMMENT 'Consent method',
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp',
    ...
    `last_sample_date` DATE COMMENT 'Attribute last_sample_date for sampling_participation',
    ...
    `result_notification_date` DATE COMMENT 'Result notification date',
    `result_notification_method` STRING COMMENT 'Result notification method',
    `sample_collection_date` DATE COMMENT 'Sample collection date',
    `sample_kit_returned_date` DATE COMMENT 'Sample kit returned date',
    `sample_kit_sent_date` DATE COMMENT 'Sample kit sent date',
    ...
    `sampling_program_type` STRING COMMENT 'Sampling program type (LCR, UCMR, etc.)',
    `tier_classification` STRING COMMENT 'Tier classification',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`account_asset_responsibility` (
    ...
    `agreement_reference` STRING COMMENT 'Agreement reference',
    ...
    `cost_allocation_method` DECIMAL(18,2) COMMENT 'Method used to allocate asset-related costs (maintenance, replacement, depreciation) to this account: PERCENTAGE (based on ownership_percentage), EQUAL_SPLIT (divided equally among all responsible accounts), USAGE_BASED (proportional to metered usage), FIXED_AMOUNT (predetermined fixed cost), CUSTOM (special arrangement)',
    `cost_responsibility_pct` DECIMAL(18,2) COMMENT 'Cost responsibility percentage',
    ...
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `maintenance_obligation_flag` BOOLEAN COMMENT 'Maintenance obligation flag',
    ...
    `replacement_obligation_flag` BOOLEAN COMMENT 'Replacement obligation flag',
    `responsibility_status` STRING COMMENT 'Responsibility status',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` (
    ...
    `building_type` STRING COMMENT 'Building type',
    `construction_year` STRING COMMENT 'Construction year',
    ...
    `internal_plumbing_material` STRING COMMENT 'Internal plumbing material',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    ...
    `site_status` STRING COMMENT 'Site status',
    `site_type` STRING COMMENT 'Site type (tap, entry point, etc.)',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` (
    ...
    `assistance_program_id` BIGINT COMMENT 'FK to related assistance program',
    `employee_id` BIGINT COMMENT 'FK to case worker handling enrollment',
    ...
    `service_address_id` BIGINT COMMENT 'FK to service address for grant enrollment',
    `territory_id` BIGINT COMMENT 'Service territory for the grant enrollment.',
    `amount_disbursed_to_date` DECIMAL(18,2) COMMENT 'Total amount disbursed to date',
    `annual_household_income` DECIMAL(18,2) COMMENT 'Annual household income for eligibility',
    `appeal_decision_date` DATE COMMENT 'Date of appeal decision',
    `appeal_filed_flag` BOOLEAN COMMENT 'Whether an appeal has been filed',
    `application_date` DATE COMMENT 'Date customer applied for grant',
    `approval_date` DATE COMMENT 'Date grant enrollment was approved',
    ...
    `benefit_calculation_method` STRING COMMENT 'Method used to calculate benefit amount',
    ...
    `co_applicant_name` STRING COMMENT 'Name of co-applicant if applicable',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the enrollment record was created.',
    `crisis_assistance_flag` BOOLEAN COMMENT 'Indicates if this is crisis assistance enrollment',
    `denial_reason` STRING COMMENT 'Reason for denial if application rejected',
    `disability_benefit_recipient_flag` BOOLEAN COMMENT 'Whether applicant receives disability benefits',
    `disbursement_date` DATE COMMENT 'Date the grant benefit was disbursed to the customer.',
    `disbursement_method` STRING COMMENT 'How the benefit is disbursed (bill credit, direct payment)',
    `documentation_complete_flag` BOOLEAN COMMENT 'Whether all required documentation is complete',
    `documentation_on_file` BOOLEAN COMMENT 'Documentation on file',
    `effective_end_date` DATE COMMENT 'Effective end date',
    `effective_start_date` DATE COMMENT 'Effective start date',
    `eligibility_criteria_met` BOOLEAN COMMENT 'Eligibility criteria met',
    ...
    `eligibility_verified_flag` BOOLEAN COMMENT '',
    `employment_status` STRING COMMENT 'Employment status of primary applicant',
    ...
    `enrollment_number` STRING COMMENT '',
    `enrollment_status` STRING COMMENT 'Current enrollment status (active, expired, revoked)',
    `expiration_date` DATE COMMENT 'Expiration date',
    `federal_poverty_level_pct` DECIMAL(18,2) COMMENT 'Household income as a percentage of the federal poverty level.',
    `federal_poverty_level_percent` DECIMAL(18,2) COMMENT 'Household income as percent of federal poverty level',
    `grant_amount_awarded` DECIMAL(18,2) COMMENT 'Grant amount awarded',
    `grant_amount_disbursed` DECIMAL(18,2) COMMENT 'Grant amount disbursed',
    `grant_program_name` STRING COMMENT 'Name of the grant program the customer is enrolled in.',
    `homeowner_flag` BOOLEAN COMMENT 'Whether applicant is homeowner',
    `household_size` STRING COMMENT 'Number of people in household',
    `income_source` STRING COMMENT 'Primary source of household income',
    `income_verification_date` DATE COMMENT 'Date income was verified for eligibility',
    `income_verification_flag` BOOLEAN COMMENT 'Indicates whether income verification was completed.',
    `income_verification_status` STRING COMMENT 'Status of income verification for eligibility (e.g., verified, pending, waived).',
    `landlord_consent_flag` BOOLEAN COMMENT 'Whether landlord consent was obtained if renter',
    `last_disbursement_date` DATE COMMENT 'Date of most recent disbursement',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `liheap_recipient_flag` BOOLEAN COMMENT 'Whether applicant is LIHEAP recipient',
    `max_benefit_amount` DECIMAL(18,2) COMMENT 'Maximum benefit amount allowed',
    `medicaid_recipient_flag` BOOLEAN COMMENT 'Whether applicant is Medicaid recipient',
    `notes` STRING COMMENT 'Enrollment notes',
    `prior_assistance_amount` DECIMAL(18,2) COMMENT 'Amount of prior assistance received',
    `prior_assistance_flag` BOOLEAN COMMENT 'Whether applicant received prior assistance',
    `purpose_description` STRING COMMENT 'Purpose description',
    `recertification_due_date` DATE COMMENT 'Date recertification is due',
    `referral_agency_name` STRING COMMENT 'Name of agency that referred the customer',
    `renewal_date` DATE COMMENT 'Date when the grant enrollment must be renewed.',
    `renter_flag` BOOLEAN COMMENT 'Whether applicant is renter',
    `snap_recipient_flag` BOOLEAN COMMENT 'Whether applicant is SNAP recipient',
    `ssi_recipient_flag` BOOLEAN COMMENT 'Whether applicant is SSI recipient',
    `tanf_recipient_flag` BOOLEAN COMMENT 'Whether applicant is TANF recipient',
    `termination_reason` STRING COMMENT 'Reason for enrollment termination if applicable',
    `total_disbursed_amount` DECIMAL(18,2) COMMENT 'Total amount disbursed',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the enrollment record was last updated.',
    `utility_arrears_amount` DECIMAL(18,2) COMMENT 'Amount of utility arrears at time of enrollment',
    `verification_date` DATE COMMENT 'Date eligibility was verified',
    `verification_method` STRING COMMENT 'Method used to verify eligibility',
    `veteran_benefit_recipient_flag` BOOLEAN COMMENT 'Whether applicant receives veteran benefits',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`project_stakeholder` (
    ...
    `impact_level` STRING COMMENT 'Impact level',
    ...
    `stakeholder_type` STRING COMMENT 'Stakeholder type',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`parcel` (
    ...
    `assessed_value` DECIMAL(18,2) COMMENT 'Assessed value',
    `assessed_value_amount` DECIMAL(18,2) COMMENT '',
    ...
    `flood_zone_designation` STRING COMMENT 'Flood zone designation',
    ...
    `land_use_code` STRING COMMENT 'Land use code',
    ...
    `lot_size_acres` DECIMAL(18,2) COMMENT 'Lot size in acres',
    `lot_size_square_feet` DECIMAL(18,2) COMMENT 'Lot size in square feet',
    ...
    `owner_mailing_address` STRING COMMENT 'Owner mailing address',
    ...
    `parcel_area_sqft` DECIMAL(18,2) COMMENT 'Attribute parcel_area_sqft for parcel',
    ...
    `tax_assessment_year` DECIMAL(18,2) COMMENT 'Fiscal year of the tax assessment.',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
    `valuation_usd` DECIMAL(18,2) COMMENT 'Assessed monetary value of the parcel in US dollars.',
    `within_service_area_flag` BOOLEAN COMMENT 'Within service area flag',
    `zip_code` STRING COMMENT 'Postal code for the parcel location.',
    `zoning_classification` STRING COMMENT 'Zoning classification',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`case` (
    ...
    `employee_id` BIGINT COMMENT 'FK to assigned employee',
    `case_assigned_to_employee_id` BIGINT COMMENT '',
    `case_employee_id` BIGINT COMMENT 'Identifier of the employee or contractor assigned to resolve the case.',
    ...
    `days_open` STRING COMMENT 'Days open',
    `case_description` STRING COMMENT 'Case description',
    `due_date` DATE COMMENT 'Case due date',
    `escalated_flag` BOOLEAN COMMENT 'Escalated flag',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `notes` STRING COMMENT 'Notes',
    `open_date` DATE COMMENT 'Case open date',
    ...
    `subject` STRING COMMENT 'Case subject',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`rotation_pool` (
    ...
    `account_count` STRING COMMENT 'Number of accounts in pool',
    ...
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_rotation_date` DATE COMMENT 'Last rotation date',
    `member_count` STRING COMMENT '',
    `rotation_pool_name` STRING COMMENT 'Descriptive name of the rotation pool.',
    `next_rotation_date` DATE COMMENT 'Next rotation date',
    `notes` STRING COMMENT 'Notes',
    `pool_code` STRING COMMENT 'Pool code',
    `pool_description` STRING COMMENT 'Pool description',
    `pool_name` STRING COMMENT 'Pool name',
    `pool_status` STRING COMMENT 'Pool status',
    `pool_type` STRING COMMENT 'Pool type',
    `region_code` STRING COMMENT 'Three-letter country code representing the region of the rotation pool.',
    `rotation_basis` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` (
    `main_break_customer_impact_id` BIGINT COMMENT '',
    `main_break_id` BIGINT COMMENT '',
    `population_at_risk` BIGINT COMMENT '',
    `affected_zones` STRING COMMENT '',
    `expected_restoration_at` TIMESTAMP COMMENT '',
    `public_notification_id` BIGINT COMMENT '',
    CONSTRAINT pk_main_break_customer_impact PRIMARY KEY(`main_break_customer_impact_id`)
) COMMENT '';

CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` (
    `network_isolation_customer_impact_id` BIGINT COMMENT '',
    `network_isolation_event_id` BIGINT COMMENT '',
    `population_at_risk` BIGINT COMMENT '',
    `affected_zones` STRING COMMENT '',
    `expected_restoration_at` TIMESTAMP COMMENT '',
    `public_notification_id` BIGINT COMMENT '',
    CONSTRAINT pk_network_isolation_customer_impact PRIMARY KEY(`network_isolation_customer_impact_id`)
) COMMENT '';
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`interaction` ADD CONSTRAINT `fk_customer_interaction_interaction_person_id` FOREIGN KEY (`interaction_person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ADD CONSTRAINT `fk_customer_customer_complaint_customer_reported_by_person_id` FOREIGN KEY (`customer_reported_by_person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_hierarchy` ADD CONSTRAINT `fk_customer_account_hierarchy_child_customer_account_id` FOREIGN KEY (`child_customer_account_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`customer_account`(`customer_account_id`);
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_hierarchy` ADD CONSTRAINT `fk_customer_account_hierarchy_parent_customer_account_id` FOREIGN KEY (`parent_customer_account_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`customer_account`(`customer_account_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` ADD CONSTRAINT `fk_customer_third_party_notification_third_person_id` FOREIGN KEY (`third_person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ADD CONSTRAINT `fk_customer_customer_program_enrollment_service_address_id` FOREIGN KEY (`service_address_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`service_address`(`service_address_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_participation` ADD CONSTRAINT `fk_customer_sampling_participation_premise_id` FOREIGN KEY (`premise_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`premise`(`premise_id`);
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_participation` ADD CONSTRAINT `fk_customer_sampling_participation_sampling_site_id` FOREIGN KEY (`sampling_site_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`sampling_site`(`sampling_site_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ADD CONSTRAINT `fk_customer_sampling_site_premise_id` FOREIGN KEY (`premise_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`premise`(`premise_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ADD CONSTRAINT `fk_customer_sampling_site_service_address_id` FOREIGN KEY (`service_address_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`service_address`(`service_address_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`project_stakeholder` ADD CONSTRAINT `fk_customer_project_stakeholder_customer_account_id` FOREIGN KEY (`customer_account_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`customer_account`(`customer_account_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`project_stakeholder` ADD CONSTRAINT `fk_customer_project_stakeholder_person_id` FOREIGN KEY (`person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ADD CONSTRAINT `fk_customer_case_person_id` FOREIGN KEY (`person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` SET TAGS ('dbx_ssot_role' = 'canonical');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` SET TAGS ('dbx_ssot_group' = 'customer.customer_account');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` SET TAGS ('dbx_ssot_master' = 'billing.billing_account');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Wtp Facility Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `account_holder_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `account_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `billing_address` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `billing_email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `billing_phone` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `credit_limit` SET TAGS ('dbx_pii' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `customer_name` SET TAGS ('dbx_PII' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `email_address` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `mailing_address_line_1` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `mailing_address_line_2` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `medical_baseline_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `medical_baseline_flag` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `phone` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `primary_email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `primary_phone` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `tax_identification_number` SET TAGS ('dbx_PII' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`person` ALTER COLUMN `marketing_consent_date` SET TAGS ('dbx_pii' = 'tax_id');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`person` ALTER COLUMN `marketing_consent_flag` SET TAGS ('dbx_pii' = 'tax_id');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`organization` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_address` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'Cip Project Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `billing_rate_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Schedule');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `customer_account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Account');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `auto_pay_enrolled` SET TAGS ('dbx_business_glossary_term' = 'Auto Pay');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `backflow_prevention_required` SET TAGS ('dbx_business_glossary_term' = 'Backflow Prevention Required');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `billing_cycle_code` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `contracted_volume_gallons` SET TAGS ('dbx_business_glossary_term' = 'Contracted Volume');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `deposit_amount` SET TAGS ('dbx_business_glossary_term' = 'Deposit Amount');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `deposit_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Deposit Required');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `is_tax_exempt` SET TAGS ('dbx_business_glossary_term' = 'Tax Exempt');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `meter_size_inches` SET TAGS ('dbx_business_glossary_term' = 'Meter Size');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `minimum_charge_usd` SET TAGS ('dbx_business_glossary_term' = 'Minimum Charge');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `paperless_billing` SET TAGS ('dbx_business_glossary_term' = 'Paperless Billing');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `service_type` SET TAGS ('dbx_business_glossary_term' = 'Service Type');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `special_conditions` SET TAGS ('dbx_business_glossary_term' = 'Special Conditions');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `service_agreement_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `updated_date` SET TAGS ('dbx_business_glossary_term' = 'Updated Date');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`segment` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_segment_assignment` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_status_history` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Violation Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`contact` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`communication_preference` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` SET TAGS ('dbx_subdomain' = 'program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `annual_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Annual Budget');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `contact_email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `contact_phone` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Created Date');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `current_enrollment_count` SET TAGS ('dbx_business_glossary_term' = 'Current Enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `enrollment_cap` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Cap');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `enrollment_period_end` SET TAGS ('dbx_business_glossary_term' = 'Enrollment End');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `enrollment_period_start` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Start');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `funding_source` SET TAGS ('dbx_business_glossary_term' = 'Funding Source');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `income_threshold_pct_fpl` SET TAGS ('dbx_business_glossary_term' = 'Income Threshold');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `max_benefit_amount` SET TAGS ('dbx_business_glossary_term' = 'Max Benefit Amount');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `max_enrollment_count` SET TAGS ('dbx_business_glossary_term' = 'Max Enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `program_status` SET TAGS ('dbx_business_glossary_term' = 'Program Status');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `requires_annual_recertification` SET TAGS ('dbx_business_glossary_term' = 'Recertification Required');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`assistance_program` ALTER COLUMN `updated_date` SET TAGS ('dbx_business_glossary_term' = 'Updated Date');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_subdomain' = 'program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_ssot_role' = 'canonical');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_ssot_group' = 'customer.customer_assistance_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_ssot_master' = 'billing.billing_assistance_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` SET TAGS ('dbx_ssot_master_ref' = 'billing.billing_assistance_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` ALTER COLUMN `annual_household_income_usd` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` ALTER COLUMN `billing_assistance_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Foreign key linking to SSOT master billing.billing_assistance_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` ALTER COLUMN `billing_assistance_enrollment_id` SET TAGS ('dbx_ssot_fk' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` ALTER COLUMN `billing_assistance_enrollment_id` SET TAGS ('dbx_ssot_mvm_resolved' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_assistance_enrollment` ALTER COLUMN `customer_ssot_billing_assistance_enrollment_ref_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_note` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_note` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_note` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Violation Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_note` ALTER COLUMN `enforcement_action_id` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Action Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`interaction` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_ssot_group' = 'customer.customer_complaint');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_ssot_pair' = 'customer.customer_complaint');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` SET TAGS ('dbx_metering_metering_complaint' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_assigned_to_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User Identifier (ID)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_assigned_to_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_assigned_to_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User Identifier (ID)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `customer_reported_by_person_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By Person Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_complaint` ALTER COLUMN `metering_complaint_id` SET TAGS ('dbx_ssot_xref' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`deposit` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`deposit` ALTER COLUMN `amount_refunded` SET TAGS ('dbx_pii' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` SET TAGS ('dbx_subdomain' = 'communication_services');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` ALTER COLUMN `third_party_address` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` ALTER COLUMN `third_party_email` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`third_party_notification` ALTER COLUMN `third_party_phone` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_document` ALTER COLUMN `account_uploaded_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_document` ALTER COLUMN `account_uploaded_by_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_subdomain' = 'program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_thin_product_expanded' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_master' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_references' = 'service.service_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_role' = 'alias');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_canonical' = 'service.service_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_group' = 'service.service_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_pair' = 'customer.customer_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_service_service_program_enrollment' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_ref' = 'service.service_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` SET TAGS ('dbx_ssot_master_ref' = 'service.service_program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `customer_service_program_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Foreign key linking to SSOT master service.service_program_enrollment');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `customer_service_program_enrollment_id` SET TAGS ('dbx_ssot_fk' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `service_address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `service_address_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `service_program_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Program Enrollment Identifier');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `service_program_enrollment_id` SET TAGS ('dbx_renamed_from' = 'service_program_enrollment_id');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `annual_savings_gallons` SET TAGS ('dbx_business_glossary_term' = 'Annual Savings');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `device_installed_flag` SET TAGS ('dbx_business_glossary_term' = 'Device Installed');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `disability_accommodation_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `disability_accommodation_flag` SET TAGS ('dbx_pii_health' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `enrollment_channel` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Channel');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Inspection Date');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_program_enrollment` ALTER COLUMN `ssot_source_domain` SET TAGS ('dbx_ssot_source' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`account_enforcement_impact` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` SET TAGS ('dbx_subdomain' = 'impact_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` SET TAGS ('dbx_thin_product_expanded' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `service_address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `service_address_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `claim_status` SET TAGS ('dbx_business_glossary_term' = 'Claim Status');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `estimated_damage_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Damage Amount');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `evacuation_required` SET TAGS ('dbx_business_glossary_term' = 'Evacuation Required');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `evacuation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Evacuation Required Flag');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `health_impact_flag` SET TAGS ('dbx_business_glossary_term' = 'Health Impact Flag');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `health_impact_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `health_impact_flag` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `health_risk_flag` SET TAGS ('dbx_business_glossary_term' = 'Health Risk');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `insurance_claim_filed` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Filed');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `resolution_status` SET TAGS ('dbx_business_glossary_term' = 'Resolution Status');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `restoration_contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `restoration_contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `restoration_date` SET TAGS ('dbx_business_glossary_term' = 'Restoration Date');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `restoration_start_date` SET TAGS ('dbx_business_glossary_term' = 'Restoration Start Date');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `street_flooding_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise_overflow_impact` ALTER COLUMN `street_flooding_flag` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_participation` SET TAGS ('dbx_subdomain' = 'program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` SET TAGS ('dbx_subdomain' = 'impact_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `service_address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `service_address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`sampling_site` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` SET TAGS ('dbx_subdomain' = 'program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` SET TAGS ('dbx_thin_product_expanded' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `amount_disbursed_to_date` SET TAGS ('dbx_business_glossary_term' = 'Amount Disbursed');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `co_applicant_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `disability_benefit_recipient_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `disability_benefit_recipient_flag` SET TAGS ('dbx_pii_health' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `federal_poverty_level_pct` SET TAGS ('dbx_business_glossary_term' = 'Federal Poverty Level Percent');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `recertification_due_date` SET TAGS ('dbx_business_glossary_term' = 'Recertification Due');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`grant_enrollment` ALTER COLUMN `renewal_date` SET TAGS ('dbx_business_glossary_term' = 'Renewal Date');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`project_stakeholder` SET TAGS ('dbx_subdomain' = 'impact_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`parcel` ALTER COLUMN `owner_mailing_address` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` SET TAGS ('dbx_subdomain' = 'communication_services');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` SET TAGS ('dbx_model_structure_verified_domains' = '15');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` SET TAGS ('dbx_products' = '379');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Case Identifier');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `case_assigned_to_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `case_assigned_to_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `case_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Id');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `customer_account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Account Id');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `parent_case_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Case Id');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `parent_case_id` SET TAGS ('dbx_self_ref_fk' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`case` ALTER COLUMN `case_type` SET TAGS ('dbx_business_glossary_term' = 'Case Type');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`rotation_pool` SET TAGS ('dbx_subdomain' = 'program_enrollment');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `main_break_customer_impact_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `main_break_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `main_break_id` SET TAGS ('dbx_links_incident' = 'main_break');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `population_at_risk` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `affected_zones` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `expected_restoration_at` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`main_break_customer_impact` ALTER COLUMN `public_notification_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `network_isolation_customer_impact_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `network_isolation_event_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `network_isolation_event_id` SET TAGS ('dbx_links_incident' = 'network_isolation_event');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `population_at_risk` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `affected_zones` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `expected_restoration_at` SET TAGS ('dbx_subdomain' = 'impact_tracking');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`network_isolation_customer_impact` ALTER COLUMN `public_notification_id` SET TAGS ('dbx_subdomain' = 'impact_tracking');
