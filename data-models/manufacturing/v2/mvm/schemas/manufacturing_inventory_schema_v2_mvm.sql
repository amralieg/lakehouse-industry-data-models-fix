...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`stock_location` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: In VMI/consignment manufacturing operations, a stock_location physically resides at a customer site. Linking to customer.address enables replenishment logistics, delivery scheduling, and VMI site mana',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`material_master` (
    ...
    `abc_indicator` BOOLEAN COMMENT 'ABC classification for inventory management prioritization. A items are high-value requiring tight control, B items are moderate-value, C items are low-value with relaxed controls. Used for cycle counting frequency and safety stock strategies.',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`stock_balance` (
    ...
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: stock_balance.batch_number is a denormalized string that should be replaced with a proper FK to inventory.lot_batch. Stock balances for batch-managed materials must reference the lot/batch master to s',
    ...
    `plant_data_id` BIGINT COMMENT 'Foreign key linking to product.plant_data. Business justification: MRP controllers compare stock_balance quantities against plant_data parameters (reorder_point, safety_stock_quantity, lot_size_procedure) daily to trigger replenishment. This plant-level stock evaluat',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` (
    ...
    `header_id` BIGINT COMMENT 'Foreign key linking to order.order_header. Business justification: In make-to-order manufacturing, lots/batches are produced for a specific sales order; this FK enables direct MTO lot-to-order traceability, customer-specific batch reporting, and order fulfillment sta',
    ...
    `order_intake_id` BIGINT COMMENT 'Foreign key linking to sales.order_intake. Business justification: Make-to-order manufacturing traceability: links a produced/received lot directly to the customer sales order that triggered it. Enables lot-level customer traceability, recall management, and customer',
    `planned_order_id` BIGINT COMMENT 'Foreign key linking to supply.planned_order. Business justification: Lot/batch records are created when planned orders are converted and goods received. Traceability from planned supply to actual received lot is a core manufacturing regulatory and quality requirement —',
    `plant_data_id` BIGINT COMMENT 'Foreign key linking to product.plant_data. Business justification: Lot/batch creation and management is governed by plant_data (batch_management_required, minimum_remaining_shelf_life_days, inspection_setup_required). Manufacturing batch traceability requires knowing',
    ...
    `revision_id` BIGINT COMMENT 'Foreign key linking to engineering.engineering_revision. Business justification: PPAP, first article inspection, and regulatory traceability require knowing which engineering revision was active when a production lot was manufactured. lot_batch has ppap-relevant fields implicitly ',
    ...
    `wip_lot_id` BIGINT COMMENT 'Foreign key linking to production.wip_lot. Business justification: A finished goods or semi-finished lot/batch is produced FROM a specific WIP lot. This link enables forward/backward batch traceability (which WIP lot produced this batch), required for quality recalls',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`stock_movement` (
    ...
    `asset_work_order_id` BIGINT COMMENT 'Foreign key linking to asset.asset_work_order. Business justification: Maintenance work order actual material cost reporting requires tracing which stock movements (goods issues) were triggered by which asset work order. Enables planned vs. actual material cost variance ',
    ...
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: stock_movement.batch_number is a denormalized string reference to the lot/batch master. Adding lot_batch_id as a proper FK to inventory.lot_batch normalizes this relationship, enabling full traceabili',
    ...
    `plant_data_id` BIGINT COMMENT 'Foreign key linking to product.plant_data. Business justification: Every goods receipt and goods issue in manufacturing is posted against a plant/material combination. plant_data governs the inspection_setup_required, valuation_class, and quality_inspection_type that',
    ...
    `wip_lot_id` BIGINT COMMENT 'Foreign key linking to production.wip_lot. Business justification: WIP transfer stock movements (e.g., SAP movement types 311/312) must reference the specific WIP lot being moved for batch traceability and WIP inventory reconciliation. Lot genealogy and WIP aging rep',
    ...
    `movement_indicator` BOOLEAN COMMENT 'High-level classification of the movement direction: receipt (inbound), issue (outbound), transfer (location-to-location), or adjustment (correction/write-off).',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`cycle_count` (
    ...
    `warehouse_id` BIGINT COMMENT 'Foreign key linking to inventory.warehouse. Business justification: cycle_count.warehouse_number is a denormalized string identifier for the warehouse in which the cycle count is being performed. cycle_count already has stock_location_id FK (for the specific location ',
    `abc_indicator` BOOLEAN COMMENT 'ABC classification of materials included in this count. A=high-value/high-frequency, B=medium, C=low-value/low-frequency, X=unclassified. Drives cycle count frequency per ABC methodology.',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`cycle_count_line` (
    ...
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: cycle_count_line.batch_number is a denormalized string reference to the lot/batch being counted. In batch-managed inventory environments, physical counts are performed at the batch level — each count ',
    ...
    `special_stock_indicator` BOOLEAN COMMENT 'Indicator for special stock categories that require separate tracking. Consignment stock is vendor-owned, project stock is allocated to specific projects, sales order stock is reserved for customer orders.',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`inventory`.`replenishment_order` (
    ...
    `header_id` BIGINT COMMENT 'Foreign key linking to order.order_header. Business justification: In make-to-order manufacturing, replenishment orders are triggered by specific sales orders; formalizing this FK enables MTO demand pegging, order-driven replenishment reporting, and customer delivery',
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: replenishment_order.batch_number is a denormalized string that should reference inventory.lot_batch via a proper FK. Replenishment orders can be batch-specific — for example, a kanban-triggered replen',
    `material_master_id` BIGINT COMMENT 'Reference to the material or SKU being replenished. Links to the material master record.',
    `order_intake_id` BIGINT COMMENT 'Foreign key linking to sales.order_intake. Business justification: Demand-driven MRP: in make-to-order manufacturing, replenishment orders are triggered by confirmed customer order intake. This link enables ATP (Available-to-Promise) reporting, demand pegging, and on',
    ...
    `plant_data_id` BIGINT COMMENT 'Foreign key linking to product.plant_data. Business justification: Replenishment orders are generated by MRP based on plant_data parameters: mrp_type, reorder_point, safety_stock_quantity, lot_size_procedure, planned_delivery_time_days. The replenishment_order must r',
    `production_work_order_id` BIGINT COMMENT 'Foreign key linking to production.production_work_order. Business justification: Replenishment orders are frequently triggered by specific production work orders material shortages. Material planners use this link to trace which work order drove a replenishment, enabling on-time ',
    `purchase_requisition_id` BIGINT COMMENT 'Foreign key linking to procurement.purchase_requisition. Business justification: In manufacturing MRP/ERP workflows, an external replenishment order directly triggers a purchase requisition. This FK enables end-to-end supply chain traceability from inventory replenishment demand t',
    `sourcing_rule_id` BIGINT COMMENT 'Foreign key linking to supply.sourcing_rule. Business justification: Replenishment orders are generated based on sourcing rules that define source type, supplier, and lead time. Tracing which sourcing rule drove each replenishment order is required for procurement comp',
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_location` SET TAGS ('dbx_subdomain' = 'warehouse_master');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_location` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Consignment Address Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_location` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_location` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`warehouse` SET TAGS ('dbx_subdomain' = 'warehouse_master');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`material_master` SET TAGS ('dbx_subdomain' = 'warehouse_master');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_balance` SET TAGS ('dbx_subdomain' = 'warehouse_master');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` SET TAGS ('dbx_subdomain' = 'warehouse_master');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order Header Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` ALTER COLUMN `order_intake_id` SET TAGS ('dbx_business_glossary_term' = 'Order Intake Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` ALTER COLUMN `planned_order_id` SET TAGS ('dbx_business_glossary_term' = 'Planned Order Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` ALTER COLUMN `plant_data_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Data Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`lot_batch` ALTER COLUMN `wip_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Wip Lot Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_movement` SET TAGS ('dbx_subdomain' = 'stock_operations');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_movement` ALTER COLUMN `plant_data_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Data Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`stock_movement` ALTER COLUMN `wip_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Wip Lot Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`cycle_count` SET TAGS ('dbx_subdomain' = 'stock_operations');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`cycle_count_line` SET TAGS ('dbx_subdomain' = 'stock_operations');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`replenishment_order` SET TAGS ('dbx_subdomain' = 'stock_operations');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`replenishment_order` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order Header Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`replenishment_order` ALTER COLUMN `production_work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Production Work Order Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`inventory`.`replenishment_order` ALTER COLUMN `purchase_requisition_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Requisition Id (Foreign Key)');
