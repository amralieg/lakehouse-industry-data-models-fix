...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` (
    ...
    `pa_requesting_provider_practice_location_id` BIGINT COMMENT 'FK to provider.provider.provider_id — PA requests must resolve to the requesting provider for network status verification, gold card eligibility, and provider communication.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` (
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Links PA decision to the originating provider for audit trails, regulatory reporting, and provider‑specific denial analysis.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` (
    ...
    `fee_schedule_id` DECIMAL(18,2) COMMENT 'Foreign key linking to contract.fee_schedule. Business justification: Each authorized service line uses a fee schedule to calculate allowed amount; linking enables accurate payment calculation and regulatory fee‑schedule reporting.',
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Associates each authorized service line with the ordering provider, enabling utilization analytics and provider reimbursement validation.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` (
    ...
    `practice_location_id` BIGINT COMMENT 'Unique identifier of the provider who delivered the service under review.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` (
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Captures the requesting provider in peer‑to‑peer reviews for peer performance tracking and compliance with clinical peer‑review regulations.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` (
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Turn‑around‑time events are measured against specific regulatory standards; linking ties each event to its governing obligation.',
    ...
    `practice_location_id` BIGINT COMMENT 'Unique identifier for the provider who submitted or is linked to the request.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` (
    ...
    `practice_location_id` BIGINT COMMENT 'Identifier of the provider who rendered the service under review.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` (
    ...
    `record_id` BIGINT COMMENT 'UM reviewers require clinical credentials',
    `employee_id` BIGINT COMMENT 'UM reviewers are internal staff',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` (
    ...
    `practice_location_id` BIGINT COMMENT 'Unique identifier of the health care provider receiving the notification, when applicable.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` (
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Programs are designed to meet regulatory requirements; the FK supports program‑to‑obligation reporting.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` (
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Medical policies must align with regulatory obligations; linking provides traceability for compliance audits.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` (
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Links bed‑day review to the responsible provider to support length‑of‑stay variance analysis and readmission risk reporting.',
    ...
    `denial_reason_description` STRING COMMENT 'Human-readable description of the denial reason.',
    `discharge_planning_notes` DECIMAL(18,2) COMMENT 'Narrative notes regarding discharge planning and anticipated needs.',
...
) COMMENT 'Tracks individual bed day reviews for inpatient concurrent review cases — daily clinical assessments of continued stay medical necessity. Captures review date, reviewer, clinical status, Milliman/InterQual criteria met, extension days approved, and discharge planning notes. [FHIR-aligned]';

CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` (
    `um_program_enrollment_id` BIGINT COMMENT 'Primary key',
    `group_id` BIGINT COMMENT 'FK to employer group',
    `um_program_id` BIGINT COMMENT 'FK to UM program',
    `contribution_strategy` STRING COMMENT 'The contribution strategy attribute capturing relevant data for the um program enrollment in the utilization domain.',
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp',
    `disenrollment_date` DATE COMMENT 'The date value representing disenrollment date for this um program enrollment record.',
    `effective_end_date` DATE COMMENT 'The date value representing effective end date for this um program enrollment record.',
    `effective_start_date` DATE COMMENT 'The date value representing effective start date for this um program enrollment record.',
    `enrollment_date` DATE COMMENT 'The date value representing enrollment date for this um program enrollment record.',
    `enrollment_reason` STRING COMMENT 'Reason for enrollment',
    `enrollment_status` STRING COMMENT 'The current status indicator for the enrollment within the workflow.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` (
    ...
    `practice_location_id` BIGINT COMMENT 'System identifier for the healthcare provider delivering services in this episode.',
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` SET TAGS ('dbx_['fhir_resource' = 'ServiceRequest');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` SET TAGS ('dbx_'fhir_resource' = 'Coverage');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Decision Maker Identifier (Decision Maker ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `pa_requesting_provider_practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Pa Requesting Provider Id');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `pa_requesting_provider_practice_location_id` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `related_request_pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Related Prior Authorization Request Identifier (Related Request ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `additional_information_requests_count` SET TAGS ('dbx_business_glossary_term' = 'Count of Additional Information Requests (AIR Count)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_request` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` SET TAGS ('dbx_fhir_resource' = 'ClaimResponse');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_decision` ALTER COLUMN `clinical_criteria_set` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Set (CRIT_SET)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` SET TAGS ('dbx_fhir_resource' = 'Coverage.preAuthRef');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `care_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Care Plan Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `fee_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Fee Schedule Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_business_glossary_term' = 'Authorized Facility NPI');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`auth_service_line` ALTER COLUMN `authorized_facility_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`concurrent_review` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` SET TAGS ('dbx_subdomain' = 'clinical_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `retrospective_review_id` SET TAGS ('dbx_business_glossary_term' = 'Retrospective Review ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `gap_id` SET TAGS ('dbx_business_glossary_term' = 'Care Gap Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Network Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `um_reviewer_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `um_reviewer_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `um_reviewer_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `adjusted_amount` SET TAGS ('dbx_business_glossary_term' = 'Adjusted Amount (USD)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `adjusted_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `clinical_criteria_applied` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Applied');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `clinical_criteria_applied` SET TAGS ('dbx_value_regex' = 'InterQual|MCG');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `compliance_state` SET TAGS ('dbx_business_glossary_term' = 'Compliance State');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `compliance_state` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|exempt');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `compliance_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (ISO 4217)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `currency_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `currency_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Description');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `documentation_completeness_flag` SET TAGS ('dbx_business_glossary_term' = 'Documentation Completeness Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `documentation_received_date` SET TAGS ('dbx_business_glossary_term' = 'Documentation Received Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `medical_necessity_outcome` SET TAGS ('dbx_business_glossary_term' = 'Medical Necessity Outcome');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `medical_necessity_outcome` SET TAGS ('dbx_value_regex' = 'approved|denied|partial');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `medical_necessity_outcome` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `medical_necessity_outcome` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `retro_review_deadline_flag` SET TAGS ('dbx_business_glossary_term' = 'Retro Review Deadline Compliance Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Review Completion Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_initiation_date` SET TAGS ('dbx_business_glossary_term' = 'Review Initiation Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_notes` SET TAGS ('dbx_business_glossary_term' = 'Review Notes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_number` SET TAGS ('dbx_business_glossary_term' = 'Retrospective Review Number');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_status` SET TAGS ('dbx_business_glossary_term' = 'Review Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_status` SET TAGS ('dbx_value_regex' = 'pending|in_review|completed|closed');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_type` SET TAGS ('dbx_business_glossary_term' = 'Review Type');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `review_type` SET TAGS ('dbx_value_regex' = 'retrospective|post_admission');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `service_date` SET TAGS ('dbx_business_glossary_term' = 'Date of Service');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`retrospective_review` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` SET TAGS ('dbx_subdomain' = 'authorization_management');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `peer_to_peer_review_id` SET TAGS ('dbx_business_glossary_term' = 'Peer To Peer Review Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Associated Claim Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `header_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `header_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Peer-to-Peer Review Request Identifier (REQ_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `peer_pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Associated Prior Authorization Request ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `peer_pa_request_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `peer_pa_request_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Requesting Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `completed_date` SET TAGS ('dbx_business_glossary_term' = 'Completed Review Date and Time');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `deadline_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Deadline Compliance Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Review Notes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `outcome_description` SET TAGS ('dbx_business_glossary_term' = 'Review Outcome Description');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `regulatory_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Compliance Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `request_date` SET TAGS ('dbx_business_glossary_term' = 'Request Date and Time');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_duration_days` SET TAGS ('dbx_business_glossary_term' = 'Review Duration (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_outcome` SET TAGS ('dbx_business_glossary_term' = 'Peer-to-Peer Review Outcome');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_outcome` SET TAGS ('dbx_value_regex' = 'upheld_denial|partial_approval|full_reversal');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_status` SET TAGS ('dbx_business_glossary_term' = 'Peer-to-Peer Review Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_status` SET TAGS ('dbx_value_regex' = 'pending|in_progress|completed|cancelled');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `review_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Provider National Provider Identifier (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `reviewer_provider_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Review Date and Time');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_business_glossary_term' = 'State Code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_fhir_element' = 'address.state');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_fhir' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `state_code` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`peer_to_peer_review` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` SET TAGS ('dbx_subdomain' = 'clinical_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `tat_compliance_event_id` SET TAGS ('dbx_business_glossary_term' = 'Turnaround Time (TAT) Compliance Event ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `identity_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `identity_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Request ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Network Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Flag (True if within TAT)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `compliance_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `decision_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Decision Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `due_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Decision Due Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Compliance Event Log Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `event_type` SET TAGS ('dbx_business_glossary_term' = 'Compliance Event Type');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction (State or Federal)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `line_of_business` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `line_of_business` SET TAGS ('dbx_value_regex' = 'medical|dental|vision|pharmacy|behavioral');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Additional Notes or Comments');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `receipt_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Request Receipt Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `regulatory_reporting_period` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Period');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `regulatory_reporting_period` SET TAGS ('dbx_value_regex' = '^d{4}Q[1-4]$');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `regulatory_reporting_period` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `review_type` SET TAGS ('dbx_business_glossary_term' = 'Review Type (e.g., Prior Authorization, Concurrent Review)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `review_type` SET TAGS ('dbx_value_regex' = 'prior_authorization|concurrent_review|retrospective_review|utilization_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Category for Non-Compliance');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_value_regex' = 'staffing|documentation|escalation|system|other');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `root_cause_detail` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Detail Description');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `tat_standard_days` SET TAGS ('dbx_business_glossary_term' = 'Configured TAT Standard (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `tat_standard_hours` SET TAGS ('dbx_business_glossary_term' = 'Configured TAT Standard (Hours)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `urgency_level` SET TAGS ('dbx_business_glossary_term' = 'Urgency Level');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `urgency_level` SET TAGS ('dbx_value_regex' = 'routine|urgent|stat');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `variance_days` SET TAGS ('dbx_business_glossary_term' = 'Variance from TAT Standard (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`tat_compliance_event` ALTER COLUMN `variance_hours` SET TAGS ('dbx_business_glossary_term' = 'Variance from TAT Standard (Hours)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_subdomain' = 'program_operations');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_fhir_resource' = 'Task');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned UM Nurse ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Provider ID');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `um_program_id` SET TAGS ('dbx_business_glossary_term' = 'Um Program Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `primary_provider_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `primary_provider_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `primary_provider_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `primary_provider_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_case` ALTER COLUMN `primary_provider_npi` SET TAGS ('dbx_pii_type' = 'identifier');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_subdomain' = 'program_operations');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `um_reviewer_id` SET TAGS ('dbx_business_glossary_term' = 'UM Reviewer ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `assigned_review_queue` SET TAGS ('dbx_business_glossary_term' = 'Assigned Review Queue');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_fhir_element' = 'address.city');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `city` SET TAGS ('dbx_fhir' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `compliance_state` SET TAGS ('dbx_business_glossary_term' = 'Compliance State');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `compliance_state` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending|under_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `compliance_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `country` SET TAGS ('dbx_business_glossary_term' = 'Country');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `country` SET TAGS ('dbx_value_regex' = 'USA|CAN|MEX');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `country` SET TAGS ('dbx_fhir_element' = 'address.country');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `credential_type` SET TAGS ('dbx_business_glossary_term' = 'Credential Type');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `credential_type` SET TAGS ('dbx_value_regex' = 'RN|MD|PharmD|LCSW|NP|PA');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_business_glossary_term' = 'Date of Birth (DOB) (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_pii_dob' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_fhir_element' = 'birthDate');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_dob' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `date_of_birth` SET TAGS ('dbx_fhir' = 'birthDate');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `delegation_authority_level` SET TAGS ('dbx_business_glossary_term' = 'Delegation Authority Level');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `delegation_authority_level` SET TAGS ('dbx_value_regex' = 'none|review|approve|escalate|admin');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_business_glossary_term' = 'Email Address (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_value_regex' = '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+.[A-Za-z]{2,}$');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_fhir_element' = 'telecom.email');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_email' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `email_address` SET TAGS ('dbx_fhir' = 'telecom');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_business_glossary_term' = 'First Name (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_pii' = 'person_name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_fhir_element' = 'name.given');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_person_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `first_name` SET TAGS ('dbx_fhir' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_business_glossary_term' = 'Full Name (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_pii' = 'person_name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_fhir_element' = 'name.text');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_person_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `full_name` SET TAGS ('dbx_fhir' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `hire_date` SET TAGS ('dbx_business_glossary_term' = 'Hire Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_business_glossary_term' = 'Last Name (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_pii' = 'person_name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_fhir_element' = 'name.family');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_person_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_name` SET TAGS ('dbx_fhir' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `last_training_date` SET TAGS ('dbx_business_glossary_term' = 'Last Training Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'License Expiration Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_number` SET TAGS ('dbx_business_glossary_term' = 'Professional License Number (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_number` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_state` SET TAGS ('dbx_business_glossary_term' = 'License State');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `license_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_business_glossary_term' = 'Middle Name (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_pii' = 'person_name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_fhir_element' = 'name.given');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_person_name' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `middle_name` SET TAGS ('dbx_fhir' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `ncqa_qualified_flag` SET TAGS ('dbx_business_glossary_term' = 'NCQA Qualification Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `next_training_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Training Due Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Notes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_business_glossary_term' = 'National Provider Identifier (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `npi` SET TAGS ('dbx_fhir' = 'identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_business_glossary_term' = 'Phone Number (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_fhir_element' = 'telecom.phone');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_phone' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `phone_number` SET TAGS ('dbx_fhir' = 'telecom');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `reviewer_status` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `reviewer_status` SET TAGS ('dbx_value_regex' = 'active|inactive|suspended|retired|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `reviewer_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `specialty` SET TAGS ('dbx_business_glossary_term' = 'Clinical Specialty');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_business_glossary_term' = 'State (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_fhir_element' = 'address.state');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `state` SET TAGS ('dbx_fhir' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_business_glossary_term' = 'Street Address (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `street_address` SET TAGS ('dbx_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Termination Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `termination_date` SET TAGS ('dbx_fhir_element' = 'period.end');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `urac_qualified_flag` SET TAGS ('dbx_business_glossary_term' = 'URAC Qualification Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Record Version Number');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_business_glossary_term' = 'ZIP Code (PII)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_reviewer` ALTER COLUMN `zip_code` SET TAGS ('dbx_fhir_element' = 'address.postalCode');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` SET TAGS ('dbx_subdomain' = 'authorization_management');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `pa_notification_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Notification ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Claim ID (CLAIM_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `header_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `header_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility ID (FAC_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `facility_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `facility_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID (MEM_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Pa Request Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID (PROV_ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID (CREATED_BY_UID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `acknowledgment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Timestamp (ACK_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `approval_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Expiration Date (APPROVAL_EXP_DT)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `attachment_indicator` SET TAGS ('dbx_business_glossary_term' = 'Attachment Indicator (ATTACH_IND)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `compliance_state` SET TAGS ('dbx_business_glossary_term' = 'Compliance State (COMP_STATE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `compliance_state` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `compliance_state` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp (CREATED_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `decision_reference` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Decision Reference (PA_DEC_REF)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_channel` SET TAGS ('dbx_business_glossary_term' = 'Delivery Channel (DELIV_CHAN)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_channel` SET TAGS ('dbx_value_regex' = 'mail|fax|portal|phone|email');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_status` SET TAGS ('dbx_business_glossary_term' = 'Delivery Status (DELIV_STATUS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_status` SET TAGS ('dbx_value_regex' = 'sent|delivered|failed|pending_acknowledgment');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `delivery_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Delivery Timestamp (DELIV_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Code (DENIAL_CODE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Description (DENIAL_DESC)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `is_urgent` SET TAGS ('dbx_business_glossary_term' = 'Urgent Flag (URGENT_FLG)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code (LANG_CD)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `language_code` SET TAGS ('dbx_value_regex' = 'EN|ES|FR|DE|ZH|JA');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `language_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `language_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `language_code` SET TAGS ('dbx_fhir_element' = 'communication.language');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `message_body` SET TAGS ('dbx_business_glossary_term' = 'Message Body (MSG_BODY)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_number` SET TAGS ('dbx_business_glossary_term' = 'Notification Number (NOTIF_NUM)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_status` SET TAGS ('dbx_business_glossary_term' = 'Notification Lifecycle Status (NOTIF_STATUS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_status` SET TAGS ('dbx_value_regex' = 'draft|queued|sent|completed|cancelled');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_type` SET TAGS ('dbx_business_glossary_term' = 'Notification Type (NOTIF_TYPE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `notification_type` SET TAGS ('dbx_value_regex' = 'approval|denial|pend|expedited_acknowledgment');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level (PRIORITY_LVL)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `recipient_type` SET TAGS ('dbx_business_glossary_term' = 'Recipient Type (RECIP_TYPE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `recipient_type` SET TAGS ('dbx_value_regex' = 'member|provider|facility');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `regulatory_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference (REG_REF)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `regulatory_reference` SET TAGS ('dbx_value_regex' = 'ACA|CMS_MA|STATE_MANDATE|NONE');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `sent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Sent Timestamp (SENT_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_notification` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp (UPDATED_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_subdomain' = 'episode_tracking');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_['fhir_resource' = 'Encounter');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_'fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` SET TAGS ('dbx_'interop' = 'fhir]');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `enrollment_eligibility_span_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Eligibility Span Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_business_glossary_term' = 'Facility NPI (National Provider Identifier)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `facility_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_business_glossary_term' = 'Attending Physician NPI');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`inpatient_admission` ALTER COLUMN `physician_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` SET TAGS ('dbx_subdomain' = 'program_operations');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `um_program_id` SET TAGS ('dbx_business_glossary_term' = 'Utilization Management Program Identifier (UM Program ID)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Program Owner Employee Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `pool_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Pool Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_category` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Category (UM Accreditation Category)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_category` SET TAGS ('dbx_value_regex' = 'tat|criteria|notification|delegation|p2p');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_status` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Status (UM Accreditation Status)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `accreditation_survey_date` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Survey Date (UM Accreditation Survey Date)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `clinical_criteria_set` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Set (UM Clinical Criteria)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Compliance Notes (UM Compliance Notes)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp (UM Record Created)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `criteria_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Criteria Effective Date (UM Criteria Effective Date)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `criteria_effective_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `criteria_version` SET TAGS ('dbx_business_glossary_term' = 'Criteria Version (UM Criteria Version)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `um_program_description` SET TAGS ('dbx_business_glossary_term' = 'Program Description (UM Program Description)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date (UM Effective From)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until Date (UM Effective Until)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `gap_analysis_date` SET TAGS ('dbx_business_glossary_term' = 'Gap Analysis Date (UM Gap Analysis Date)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `gap_analysis_status` SET TAGS ('dbx_business_glossary_term' = 'Gap Analysis Status (UM Gap Analysis Status)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `gap_analysis_status` SET TAGS ('dbx_value_regex' = 'open|closed|in_progress');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `gap_analysis_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `line_of_business` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `line_of_business` SET TAGS ('dbx_value_regex' = 'individual|group|medicare|medicaid|exchange|self_funded');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `next_review_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Due Date (UM Review Due Date)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `pa_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required (PA Required)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Program Code (UM Program Code)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_name` SET TAGS ('dbx_business_glossary_term' = 'Program Name (UM Program Name)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_name` SET TAGS ('dbx_pii_type' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_notes` SET TAGS ('dbx_business_glossary_term' = 'Program Notes (UM Program Notes)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Program Type (UM Program Type)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_type` SET TAGS ('dbx_value_regex' = 'inpatient|outpatient|behavioral|pharmacy|specialty');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `program_version` SET TAGS ('dbx_business_glossary_term' = 'Program Version (UM Program Version)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `regulatory_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference (UM Regulatory Reference)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `review_type_scope` SET TAGS ('dbx_business_glossary_term' = 'Review Type Scope (UM Review Scope)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `review_type_scope` SET TAGS ('dbx_value_regex' = 'concurrent|retrospective|both');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `services_covered` SET TAGS ('dbx_business_glossary_term' = 'Services Covered (UM Services Covered)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `um_program_status` SET TAGS ('dbx_business_glossary_term' = 'Program Status (UM Program Status)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `um_program_status` SET TAGS ('dbx_value_regex' = 'active|inactive|retired|draft');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `um_program_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp (UM Record Updated)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` SET TAGS ('dbx_subdomain' = 'authorization_management');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_required_service_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required Service ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Pa Request Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `clinical_criteria_set` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Set');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `effective_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `exemption_type` SET TAGS ('dbx_business_glossary_term' = 'Exemption Type');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `exemption_type` SET TAGS ('dbx_value_regex' = 'state_mandate|cms_requirement|gold_card|plan_policy|none');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_approval_rate_met_flag` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Approval Rate Met Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_approval_rate_met_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_approval_rate_threshold` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Approval Rate Threshold (%)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_approval_rate_threshold` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Effective Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_effective_date` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_effective_date` SET TAGS ('dbx_fhir_element' = 'period.start');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Expiration Date');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_expiration_date` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_flag` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Override Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_flag` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Provider NPI');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_provider_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_revocation_reason` SET TAGS ('dbx_business_glossary_term' = 'Gold Card Revocation Reason');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `gold_card_revocation_reason` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `line_of_business` SET TAGS ('dbx_business_glossary_term' = 'Line of Business (LOB)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `line_of_business` SET TAGS ('dbx_value_regex' = 'commercial|medicare_advantage|medicaid|group|individual');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_required_service_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_required_service_status` SET TAGS ('dbx_value_regex' = 'active|inactive|retired|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_required_service_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_type` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Type');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `pa_type` SET TAGS ('dbx_value_regex' = 'standard|expedited|concurrent|none');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `provider_specialty_exemption_flag` SET TAGS ('dbx_business_glossary_term' = 'Provider Specialty Exemption Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `record_audit_created` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `record_audit_updated` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `regulatory_citation` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Citation');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `service_code` SET TAGS ('dbx_business_glossary_term' = 'Current Procedural Terminology (CPT) or Healthcare Common Procedure Coding System (HCPCS) Code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `service_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `service_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `service_description` SET TAGS ('dbx_business_glossary_term' = 'Service Description (Full Name)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `updated_by` SET TAGS ('dbx_business_glossary_term' = 'Updated By User ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`pa_required_service` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` SET TAGS ('dbx_subdomain' = 'program_operations');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `um_delegation_id` SET TAGS ('dbx_business_glossary_term' = 'Utilization Management Delegation ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `um_program_id` SET TAGS ('dbx_business_glossary_term' = 'Um Program Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `audit_result_status` SET TAGS ('dbx_business_glossary_term' = 'Audit Result Status (AUDIT_RESULT)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `audit_result_status` SET TAGS ('dbx_value_regex' = 'pass|fail|conditional|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `audit_result_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `audit_schedule_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Schedule Date (AUDIT_SCHED_DATE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'NCQA Compliance Status (COMPLIANCE_STATUS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|under_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `compliance_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `contract_terms_description` SET TAGS ('dbx_business_glossary_term' = 'Contract Terms Description (CONTRACT_TERMS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `corrective_action_plan_reference` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan Reference (CAP_REF)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp (CREATED_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_entity_name` SET TAGS ('dbx_business_glossary_term' = 'Delegated Entity Name (ENTITY_NAME)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_entity_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_entity_name` SET TAGS ('dbx_pii_type' = 'name');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_entity_type` SET TAGS ('dbx_business_glossary_term' = 'Delegated Entity Type (ENTITY_TYPE)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_entity_type` SET TAGS ('dbx_value_regex' = 'tpa|health_system|aco|ipa|specialty_vendor');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_functions` SET TAGS ('dbx_business_glossary_term' = 'Delegated UM Functions (DELEGATED_FUNCS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegated_functions` SET TAGS ('dbx_value_regex' = 'prior_authorization|concurrent_review|retrospective_review|utilization_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_number` SET TAGS ('dbx_business_glossary_term' = 'Delegation Number (DELEGATION_NO)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_status` SET TAGS ('dbx_business_glossary_term' = 'Delegation Status (STATUS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_status` SET TAGS ('dbx_value_regex' = 'active|inactive|terminated|suspended|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `delegation_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date (EFF_FROM)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until Date (EFF_UNTIL)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `performance_metric_actual_days` SET TAGS ('dbx_business_glossary_term' = 'Performance Metric Actual (PERF_ACTUAL_DAYS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `performance_metric_target_days` SET TAGS ('dbx_business_glossary_term' = 'Performance Metric Target (PERF_TARGET_DAYS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `sub_delegation_allowed_flag` SET TAGS ('dbx_business_glossary_term' = 'Sub‑Delegation Allowed Flag (SUB_DELEG_ALLOWED)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `termination_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Termination Reason Code (TERM_REASON_CD)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `termination_reason_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `termination_reason_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `termination_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Termination Reason Description (TERM_REASON_DESC)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_delegation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp (UPDATED_TS)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_subdomain' = 'clinical_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_fhir_resource' = 'Coverage');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`medical_policy` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` SET TAGS ('dbx_subdomain' = 'clinical_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` SET TAGS ('dbx_fhir_resource' = 'Basic');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `bed_day_review_id` SET TAGS ('dbx_business_glossary_term' = 'Bed Day Review ID');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `concurrent_review_id` SET TAGS ('dbx_business_glossary_term' = 'Concurrent Review Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `coordinator_id` SET TAGS ('dbx_business_glossary_term' = 'Care Coordinator Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member Identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `identity_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `parent_bed_day_review_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Bed Day Review Id');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `parent_bed_day_review_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Network Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `approval_decision` SET TAGS ('dbx_business_glossary_term' = 'Approval Decision');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `approval_decision` SET TAGS ('dbx_value_regex' = 'approved|denied|pending');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `bed_day_review_status` SET TAGS ('dbx_business_glossary_term' = 'Review Status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `bed_day_review_status` SET TAGS ('dbx_value_regex' = 'pending|in_progress|completed|closed|denied');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `bed_day_review_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_criteria_met_flag` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Met Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_criteria_set` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Set');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_criteria_version` SET TAGS ('dbx_business_glossary_term' = 'Clinical Criteria Version');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_status` SET TAGS ('dbx_business_glossary_term' = 'Clinical Status of Patient');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_status` SET TAGS ('dbx_value_regex' = 'stable|improving|deteriorating|critical');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `clinical_status` SET TAGS ('dbx_fhir_element' = 'status');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Description');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `discharge_planning_notes` SET TAGS ('dbx_business_glossary_term' = 'Discharge Planning Notes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `extension_days_approved` SET TAGS ('dbx_business_glossary_term' = 'Approved Extension Days');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `is_critical` SET TAGS ('dbx_business_glossary_term' = 'Critical Review Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `length_of_stay_benchmark_met_flag` SET TAGS ('dbx_business_glossary_term' = 'Length of Stay Benchmark Met Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `length_of_stay_current` SET TAGS ('dbx_business_glossary_term' = 'Current Length of Stay (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `length_of_stay_target` SET TAGS ('dbx_business_glossary_term' = 'Target Length of Stay (Days)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `readmission_risk_category` SET TAGS ('dbx_business_glossary_term' = 'Readmission Risk Category');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `readmission_risk_category` SET TAGS ('dbx_value_regex' = 'low|medium|high');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `readmission_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Readmission Risk Score');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `regulatory_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Compliance Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `review_notes` SET TAGS ('dbx_business_glossary_term' = 'Review Notes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `review_number` SET TAGS ('dbx_business_glossary_term' = 'Bed Day Review Number (BDR#)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `review_number` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `review_number` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `review_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Review Date and Time');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_business_glossary_term' = 'Reviewer National Provider Identifier (NPI)');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_denormalized_natural_key' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_ssot' = 'owning_entity_via_fk');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_fhir_element' = 'identifier.npi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `reviewer_npi` SET TAGS ('dbx_pii_type' = 'identifier');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `social_work_involved` SET TAGS ('dbx_business_glossary_term' = 'Social Work Involvement Flag');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`bed_day_review` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_subdomain' = 'program_operations');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_association_edges' = 'utilization.um_program,employer.employer_group');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_fhir_resource' = 'Coverage');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`um_program_enrollment` SET TAGS ('dbx_preserved' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_subdomain' = 'clinical_review');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_fhir_interop' = 'ready');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_fhir_resource' = 'Condition');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_interop' = 'fhir');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`clinical_criteria` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` SET TAGS ('dbx_subdomain' = 'episode_tracking');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` SET TAGS ('dbx_mvm_scope' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` SET TAGS ('dbx_mvm' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` SET TAGS ('dbx_scope' = 'mvm');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` SET TAGS ('dbx_preserved' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Id');
...
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_business_glossary_term' = 'Secondary Diagnosis Codes');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_classification' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_health' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_pii_class' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_pii_tag' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_sensitivity_class' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_sensitivity_detail' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_sensitivity' = 'phi');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_fhir_element' = 'Condition.code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_fhir' = 'code');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_phi' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_sensitivity_subtype' = 'health');
ALTER TABLE `vibe_health_insurance_v1`.`utilization`.`episode_of_care` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_pii_category' = 'health');
