...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`contact` (
    ...
    `manager_contact_id` BIGINT COMMENT 'Self-referencing identifier pointing to the contacts organizational superior within the same account. Enables modeling of the customer-side org chart for multi-stakeholder enterprise sales and service engagement strategies.',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`segment` (
    ...
    `payment_term_id` BIGINT COMMENT 'Foreign key linking to billing.payment_term. Business justification: Customer segments define default payment terms applied to all accounts in the segment (e.g., strategic accounts get Net-60, standard accounts Net-30). Normalizing segment.payment_terms_code to a prope',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`address` (
    ...
    `contact_id` BIGINT COMMENT 'Reference to the individual contact person associated with this address, when the address belongs to a specific contact rather than the account-level organization. Nullable for account-level addresses.',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` (
    ...
    `family_id` BIGINT COMMENT 'Foreign key linking to product.family. Business justification: SLA agreements in manufacturing are scoped to product families for service contract management, warranty coverage reporting, and SLA compliance tracking by product line. The plain-text product_line_s',
    `sales_contract_id` BIGINT COMMENT 'Foreign key linking to sales.sales_contract. Business justification: In manufacturing, SLA agreements are governed by and derived from sales contracts. Service delivery managers must trace which contract defines the commercial obligations underpinning an SLA. Contract ',
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: SLA agreements in industrial manufacturing define service commitments for specific physical locations — a plant, facility, or installation site where field service, preventive maintenance, and on-site',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`interaction` (
    ...
    `header_id` BIGINT COMMENT 'Foreign key linking to order.order_header. Business justification: Customer service interactions in manufacturing (complaints, delivery disputes, quality escalations) are routinely linked to specific orders. This FK enables order-level complaint tracking, CAPA linkag',
    `lead_id` BIGINT COMMENT 'Foreign key linking to customer.customer_lead. Business justification: In CRM-driven B2B industrial sales, interactions (sales calls, site visits, demos) frequently occur with prospective customers who are still in the lead stage — before conversion to a customer_account',
    ...
    `quote_id` BIGINT COMMENT 'Foreign key linking to sales.quote. Business justification: Customer interactions in manufacturing sales frequently relate to specific quotes (negotiation calls, quote review meetings, price objection handling). Linking interactions to quotes enables quote neg',
    `sku_master_id` BIGINT COMMENT 'Foreign key linking to product.sku_master. Business justification: Customer interactions in manufacturing (complaints, technical support, CAPA) are tied to specific SKUs. The interaction table has capa_reference and is_customer_complaint confirming this process. ',
...
CREATE OR REPLACE TABLE `vibe_manufacturing_v1`.`customer`.`lead` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.customer_contact. Business justification: A customer lead, once qualified or during conversion, is associated with a specific customer contact record. In B2B industrial sales, the lead person (first_name, last_name, email on customer_lead) ma',
    `opportunity_id` BIGINT COMMENT 'Foreign key linking to sales.opportunity. Business justification: Lead-to-opportunity conversion is the core CRM process in manufacturing sales. When a lead converts, the resulting opportunity must be traceable back to the originating lead for pipeline attribution, ',
    ...
    `family_id` BIGINT COMMENT 'Foreign key linking to product.family. Business justification: Lead qualification in manufacturing begins at the product family level before narrowing to specific SKUs. The plain-text product_interest_area column is a denormalized product family reference. Link',
    ...
    `segment_id` BIGINT COMMENT 'Foreign key linking to customer.segment. Business justification: Leads should be pre-classified into market segments for routing, scoring, and prioritization. In B2B industrial manufacturing, leads are often categorized by industry vertical, company size, or produc',
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ADD CONSTRAINT `fk_customer_contact_manager_contact_id` FOREIGN KEY (`manager_contact_id`) REFERENCES `vibe_manufacturing_v1`.`customer`.`contact`(`contact_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` ADD CONSTRAINT `fk_customer_sla_agreement_address_id` FOREIGN KEY (`address_id`) REFERENCES `vibe_manufacturing_v1`.`customer`.`address`(`address_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`interaction` ADD CONSTRAINT `fk_customer_interaction_lead_id` FOREIGN KEY (`lead_id`) REFERENCES `vibe_manufacturing_v1`.`customer`.`lead`(`lead_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ADD CONSTRAINT `fk_customer_lead_contact_id` FOREIGN KEY (`contact_id`) REFERENCES `vibe_manufacturing_v1`.`customer`.`contact`(`contact_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ADD CONSTRAINT `fk_customer_lead_segment_id` FOREIGN KEY (`segment_id`) REFERENCES `vibe_manufacturing_v1`.`customer`.`segment`(`segment_id`);
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `manager_contact_id` SET TAGS ('dbx_business_glossary_term' = 'Reports To Contact ID');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `contact_type` SET TAGS ('dbx_value_regex' = 'technical_evaluator|commercial_buyer|executive_approver|end_user|influencer|other');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Contact Record Created Timestamp');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `crm_contact_code` SET TAGS ('dbx_business_glossary_term' = 'Customer Relationship Management (CRM) Contact ID');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `crm_contact_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `customer_contact_status` SET TAGS ('dbx_business_glossary_term' = 'Contact Status');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `customer_contact_status` SET TAGS ('dbx_value_regex' = 'active|inactive|do_not_contact|pending_verification|deceased');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Contact Data Quality Score');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `department` SET TAGS ('dbx_business_glossary_term' = 'Contact Department');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `do_not_call` SET TAGS ('dbx_business_glossary_term' = 'Do Not Call Flag');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `email` SET TAGS ('dbx_business_glossary_term' = 'Contact Business Email Address');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `email_opt_out` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `is_decision_maker` SET TAGS ('dbx_business_glossary_term' = 'Decision Maker Flag');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `is_primary_contact` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Flag');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `is_primary_contact` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `is_primary_contact` SET TAGS ('dbx_mask' = 'non-prod');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `job_title` SET TAGS ('dbx_business_glossary_term' = 'Contact Job Title');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Contact Preferred Language Code');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `language_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}(-[A-Z]{2})?$');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `last_activity_date` SET TAGS ('dbx_business_glossary_term' = 'Contact Last Activity Date');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Contact Last Modified Timestamp');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_country_code` SET TAGS ('dbx_business_glossary_term' = 'Contact Mailing Country Code');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Contact Mailing Postal Code');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_state` SET TAGS ('dbx_business_glossary_term' = 'Contact Mailing State or Province');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_state` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_state` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_street` SET TAGS ('dbx_business_glossary_term' = 'Contact Mailing Street Address');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_street` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mailing_street` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `marketing_opt_in` SET TAGS ('dbx_business_glossary_term' = 'Marketing Opt-In Consent Flag');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `marketing_opt_in_date` SET TAGS ('dbx_business_glossary_term' = 'Marketing Opt-In Consent Date');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mdm_golden_record_code` SET TAGS ('dbx_business_glossary_term' = 'Master Data Management (MDM) Golden Record ID');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mdm_golden_record_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `mobile_phone` SET TAGS ('dbx_business_glossary_term' = 'Contact Mobile Phone Number');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `phone` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `salutation` SET TAGS ('dbx_value_regex' = 'Mr.|Ms.|Mrs.|Dr.|Prof.');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `sla_tier` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Tier');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `sla_tier` SET TAGS ('dbx_value_regex' = 'platinum|gold|silver|bronze|standard');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`contact` ALTER COLUMN `technical_contact_flag` SET TAGS ('dbx_business_glossary_term' = 'Technical Contact Flag');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`segment` ALTER COLUMN `payment_term_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Term Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`credit_profile` SET TAGS ('dbx_subdomain' = 'credit_terms');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` SET TAGS ('dbx_subdomain' = 'credit_terms');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` ALTER COLUMN `sales_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Contract Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Service Address Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`sla_agreement` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`interaction` ALTER COLUMN `lead_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Lead Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Contact Id (Foreign Key)');
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `opportunity_id` SET TAGS ('dbx_business_glossary_term' = 'Converted Opportunity Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `family_id` SET TAGS ('dbx_business_glossary_term' = 'Family Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment Id (Foreign Key)');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `email_opt_out` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_manufacturing_v1`.`customer`.`lead` ALTER COLUMN `phone` SET TAGS ('dbx_sensitivity' = 'pii');
