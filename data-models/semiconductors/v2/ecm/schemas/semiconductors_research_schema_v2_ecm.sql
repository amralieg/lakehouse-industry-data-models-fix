...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` (
    ...
    `design_milestone_id` BIGINT COMMENT 'SSOT FK reference to canonical design.design_milestone; this table is a secondary view and must not duplicate master data.',
...
) COMMENT 'Milestone tracking records for R&D programs and projects capturing key technical and business gate reviews. Captures milestone ID, associated program or project, milestone name, milestone type (technical gate, management review, customer demo, tapeout, silicon return, qualification), planned date, actual completion date, completion criteria, gate review outcome (pass, conditional pass, fail, deferred), action items, and approving authority. Supports technology roadmap execution tracking and CHIPS Act program reporting. References SSOT: design.design_milestone';
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`research`.`budget_allocation` (
    `budget_allocation_id` DECIMAL(18,2) COMMENT 'Primary key for budget_allocation',
    ...
    `budget_category` DECIMAL(18,2) COMMENT 'The primary budget category for this allocation (e.g., headcount, equipment, materials, external services, fab costs, IP licensing).',
    `budget_for_program` DECIMAL(18,2) COMMENT 'FK to research.program.program_id — Budget allocations fund R&D programs. Critical for financial tracking, CHIPS Act compliance reporting, and program cost management.',
    `budget_funds_program` DECIMAL(18,2) COMMENT 'FK to research.program.program_id — Budget allocations are assigned to R&D programs (and their constituent projects). This is essential for financial tracking and CHIPS Act compliance reporting.',
    `budget_period_end_date` DECIMAL(18,2) COMMENT 'The end date of the budget allocation period.',
    `budget_period_start_date` DECIMAL(18,2) COMMENT 'The start date of the budget allocation period.',
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'The cost center code to which this budget allocation is assigned for financial tracking and reporting.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` (
    ...
    `currency_id` BIGINT COMMENT 'Foreign key to the currency used for funding amounts.',
    `employee_id` BIGINT COMMENT 'Foreign key to the employee who owns/manages this collaboration.',
    `research_program_id` BIGINT COMMENT 'Foreign key linking to the R&D program',
    `supplier_id` BIGINT COMMENT 'Foreign key to the supplier/partner organization.',
    `agreement_reference` STRING COMMENT 'Reference number or identifier for the formal collaboration agreement.',
    `agreement_type` STRING COMMENT 'Type of collaboration agreement (e.g., joint_development, licensing, co_investment, research_services).',
    `collaboration_end_date` DATE COMMENT '',
    `collaboration_role` STRING COMMENT '',
    `collaboration_start_date` DATE COMMENT '',
    `collaboration_status` STRING COMMENT 'Current lifecycle status of the collaboration (e.g., proposed, active, on_hold, completed, terminated).',
    `collaboration_type` STRING COMMENT 'Type of collaboration (Joint Development, Licensing, Co-Investment)',
    `contact_email` STRING COMMENT 'Email address of the primary contact person at the partner organization.',
    `contact_person_name` STRING COMMENT 'Name of the primary contact person at the partner organization.',
    `contribution_amount` DECIMAL(18,2) COMMENT '',
    `contribution_scope` STRING COMMENT 'Description of what each partner contributes (e.g., IP, equipment, funding, personnel).',
    `contribution_type` STRING COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the collaboration record was created in the system.',
    `currency_code` STRING COMMENT 'ISO currency code for the funding contribution',
    `deliverable_description` STRING COMMENT 'Description of key deliverables expected from the partner',
    `deliverable_summary` STRING COMMENT 'Summary of key deliverables expected from the collaboration.',
    `effective_end_date` DATE COMMENT 'End date of the collaboration',
    `effective_start_date` DATE COMMENT 'Start date of the collaboration',
    `end_date` DATE COMMENT 'Planned or actual end date of the collaboration agreement.',
    `export_control_flag` BOOLEAN COMMENT 'Indicates whether the collaboration involves export-controlled technology requiring compliance review.',
    `funding_amount` DECIMAL(18,2) COMMENT 'Total monetary funding committed to this collaboration by all parties.',
    `funding_contribution` DECIMAL(18,2) COMMENT 'Monetary contribution from the partner',
    `funding_contribution_amount` DECIMAL(18,2) COMMENT 'Monetary contribution from the partner',
    `funding_share_amount` DECIMAL(18,2) COMMENT '',
    `funding_share_percentage` DECIMAL(18,2) COMMENT '',
    `governance_model` STRING COMMENT 'Governance structure for the collaboration (e.g., joint_steering_committee, single_lead, consortium).',
    `ip_ownership_percent` DECIMAL(18,2) COMMENT 'Percentage of IP ownership held by the primary organization.',
    `ip_ownership_split` STRING COMMENT 'Description of how intellectual property ownership is divided among partners.',
    `ip_ownership_split_percent` DECIMAL(18,2) COMMENT 'Percentage of IP ownership allocated to the partner',
    `ip_ownership_terms` STRING COMMENT 'Summary of intellectual property ownership and licensing terms governing collaboration outputs.',
    `ip_sharing_terms` STRING COMMENT '',
    `is_exclusive` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when the record was last modified.',
    `milestone_count` STRING COMMENT 'Total number of defined milestones in the collaboration plan.',
    `milestones_completed` STRING COMMENT 'Number of milestones completed so far.',
    `nda_reference` STRING COMMENT 'Reference to the governing NDA',
    `nda_required_flag` BOOLEAN COMMENT 'Indicates whether a non-disclosure agreement is required for this collaboration.',
    `nda_signed_date` DATE COMMENT 'Date when the NDA was signed.',
    `notes` STRING COMMENT 'Free-text notes about the collaboration.',
    `partner_name` STRING COMMENT '',
    `partner_organization_name` STRING COMMENT 'Legal name of the external partner organization involved in the collaboration.',
    `partner_role` STRING COMMENT 'Role of the channel partner within the program (e.g., co‑developer, co‑seller)',
    `partner_type` STRING COMMENT '',
    `partnership_type` STRING COMMENT 'Type of partnership (e.g., joint_development, licensing, consortium, co_investment).',
    `primary_contact_email` STRING COMMENT 'Email of the primary contact at the partner organization',
    `primary_contact_name` STRING COMMENT 'Name of the primary contact at the partner organization',
    `publication_rights` STRING COMMENT 'Description of publication rights and restrictions for research outputs.',
    `revenue_share_percent` DECIMAL(18,2) COMMENT 'Percentage of revenue shared with the partner from commercialized collaboration outputs.',
    `review_frequency` STRING COMMENT 'How often the collaboration is reviewed (e.g., monthly, quarterly).',
    `risk_level` STRING COMMENT 'Assessed risk level of the collaboration (e.g., low, medium, high, critical).',
    `scope_description` STRING COMMENT 'Detailed description of the technical and business scope of the collaboration.',
    `start_date` DATE COMMENT 'Effective start date of the collaboration agreement.',
    `program_partner_collaboration_status` STRING COMMENT 'Current status of the collaboration (e.g., proposed, active, completed, terminated).',
    `technology_area` STRING COMMENT 'Primary technology area or domain of the collaboration (e.g., advanced_packaging, EUV_litho, 3nm_process).',
    `termination_clause` STRING COMMENT 'Summary of conditions under which the collaboration can be terminated.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the collaboration record was last updated.',
    `deliverables` STRING COMMENT 'Expected deliverables',
    `collaboration_name` STRING COMMENT 'Name of the collaboration between program and partner',
    `deliverables_description` STRING COMMENT 'Description of expected deliverables from the collaboration',
    `contact_person` STRING COMMENT 'Primary contact person for the collaboration',
    `created_by` STRING COMMENT 'User or system identifier that created this collaboration record.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` (
    ...
    `employee_id` BIGINT COMMENT 'Employee who performed the assessment',
    `obligation_register_id` BIGINT COMMENT 'Foreign key linking to the compliance obligation',
    `research_program_id` BIGINT COMMENT '',
    `project_id` BIGINT COMMENT 'Foreign key linking to the research project',
    `approval_status` STRING COMMENT 'Approval status of the assessment (Approved, Pending, Rejected)',
    `assessment_date` DATE COMMENT 'Date the compliance assessment was performed for the project‑obligation pair',
    `assessment_scope` STRING COMMENT 'Scope of the assessment (full project, phase, deliverable)',
    `assessment_status` STRING COMMENT '',
    `assessment_type` STRING COMMENT 'Type of compliance assessment (Export Control, Environmental, Safety)',
    `assessor_name` STRING COMMENT '',
    `completion_date` DATE COMMENT 'Date the assessment was completed',
    `compliance_status` STRING COMMENT 'Result of the assessment (e.g., compliant, partially compliant, non‑compliant)',
    `compliant_flag` BOOLEAN COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp',
    `critical_finding_count` STRING COMMENT 'Number of critical findings requiring immediate action',
    `due_date` DATE COMMENT 'Due date for completing remediation actions',
    `export_control_classification` STRING COMMENT '',
    `finding_count` STRING COMMENT 'Number of findings identified during assessment',
    `findings_summary` STRING COMMENT '',
    `is_compliant` BOOLEAN COMMENT '',
    `next_review_date` DATE COMMENT 'Date of the next scheduled compliance review',
    `notes` STRING COMMENT 'Additional notes from the compliance assessment',
    `regulation_reference` STRING COMMENT '',
    `regulatory_framework` STRING COMMENT 'Applicable regulatory framework (EAR, ITAR, REACH, CHIPS Act)',
    `remediation_complete_flag` BOOLEAN COMMENT 'Whether all remediation actions have been completed',
    ...
    `risk_score` DECIMAL(18,2) COMMENT 'Quantitative risk score from the assessment',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
    `compliance_score` DECIMAL(18,2) COMMENT 'Compliance score',
    `compliance_assessment_status` STRING COMMENT 'Assessment status',
    `findings` STRING COMMENT 'Assessment findings',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` (
    ...
    `research_test_plan_description` STRING COMMENT 'Detailed narrative describing the purpose, scope, and methodology of the test plan.',
    ...
    `research_test_plan_status` STRING COMMENT 'Current lifecycle state of the test plan.',
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_program` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Program Director Employee ID');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`project` SET TAGS ('dbx_domain' = 'research');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_technology_node` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_technology_node` SET TAGS ('dbx_ssot' = 'secondary');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_technology_node` ALTER COLUMN `fabrication_technology_node_id` SET TAGS ('dbx_ssot_ref' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`technology_roadmap` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`technology_roadmap` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`experimental_lot` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`experimental_lot` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Experimental Project Id');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_integration_run` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_integration_run` ALTER COLUMN `fabrication_wafer_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Wafer Lot Identifier (ID)');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`device_architecture` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`materials_research` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`materials_research` ALTER COLUMN `supplier_vendor_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`ip_core_development` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`ip_core_development` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Ip Project Id');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`ip_core_development` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`patent_filing` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`invention_disclosure` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`invention_disclosure` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Inventor Employee Identifier (ID)');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`invention_disclosure` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`invention_disclosure` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Related Project Identifier (ID)');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`invention_disclosure` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`collaboration` ALTER COLUMN `partner_technical_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`collaboration` ALTER COLUMN `partner_technical_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`tapeout_experiment` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`tapeout_experiment` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Tapeout Project Id');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`tapeout_experiment` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`tapeout_experiment` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`characterization_result` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`characterization_result` SET TAGS ('dbx_subdomain' = 'results_analysis');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`pdk_development` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`pdk_development` ALTER COLUMN `pdk_custodian_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` SET TAGS ('dbx_ssot' = 'secondary');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` SET TAGS ('dbx_ssot_owner' = 'design.design_milestone');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` ALTER COLUMN `design_milestone_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` ALTER COLUMN `design_milestone_id` SET TAGS ('dbx_ssot_ref' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approving Authority Employee Identifier (ID)');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_milestone` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Research Project Identifier (ID)');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`publication` SET TAGS ('dbx_subdomain' = 'program_management');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`publication` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Publication Project Id');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`budget_allocation` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`packaging_research` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`yield_learning_record` SET TAGS ('dbx_subdomain' = 'results_analysis');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`yield_learning_record` ALTER COLUMN `learning_engineer_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`competitive_benchmark` SET TAGS ('dbx_subdomain' = 'technology_roadmap');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`competitive_benchmark` ALTER COLUMN `approved_by_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`government_grant` ALTER COLUMN `principal_investigator_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_flow_experiment` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_flow_experiment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Author Employee ID');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_flow_experiment` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Related Research Project ID');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `program_partner_collaboration_id` SET TAGS ('dbx_business_glossary_term' = 'Programpartnercollaboration - Collaboration Id');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `channel_partner_id` SET TAGS ('dbx_business_glossary_term' = 'Programpartnercollaboration - Channel Partner Id');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `collaboration_id` SET TAGS ('dbx_business_glossary_term' = 'Collaboration Agreement ID');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `collaboration_id` SET TAGS ('dbx_none' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `currency_id` SET TAGS ('dbx_business_glossary_term' = 'Funding Currency ID');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Owner Employee ID');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `research_program_id` SET TAGS ('dbx_business_glossary_term' = 'Programpartnercollaboration - Rd Program Id');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier ID');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `agreement_type` SET TAGS ('dbx_business_glossary_term' = 'Agreement Type');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `collaboration_type` SET TAGS ('dbx_business_glossary_term' = 'Collaboration Type');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `contact_email` SET TAGS ('dbx_business_glossary_term' = 'Contact Email');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `contact_person_name` SET TAGS ('dbx_business_glossary_term' = 'Contact Person Name');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `contact_person_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `contribution_scope` SET TAGS ('dbx_business_glossary_term' = 'Contribution Scope');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `deliverable_description` SET TAGS ('dbx_business_glossary_term' = 'Deliverable Description');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `deliverable_summary` SET TAGS ('dbx_business_glossary_term' = 'Deliverable Summary');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `export_control_flag` SET TAGS ('dbx_business_glossary_term' = 'Export Control Flag');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `funding_amount` SET TAGS ('dbx_business_glossary_term' = 'Funding Amount');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `funding_contribution` SET TAGS ('dbx_business_glossary_term' = 'Funding Contribution');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `funding_contribution_amount` SET TAGS ('dbx_business_glossary_term' = 'Funding Contribution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `ip_ownership_percent` SET TAGS ('dbx_business_glossary_term' = 'IP Ownership Percent');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `ip_ownership_split` SET TAGS ('dbx_business_glossary_term' = 'IP Ownership Split');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `ip_ownership_split_percent` SET TAGS ('dbx_business_glossary_term' = 'IP Ownership Split');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `milestones_completed` SET TAGS ('dbx_business_glossary_term' = 'Milestones Completed');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `nda_reference` SET TAGS ('dbx_business_glossary_term' = 'NDA Reference');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `nda_required_flag` SET TAGS ('dbx_business_glossary_term' = 'NDA Required Flag');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `nda_signed_date` SET TAGS ('dbx_business_glossary_term' = 'NDA Signed Date');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `partner_organization_name` SET TAGS ('dbx_business_glossary_term' = 'Partner Organization');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `partner_role` SET TAGS ('dbx_none' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `partnership_type` SET TAGS ('dbx_business_glossary_term' = 'Partnership Type');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Email');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `publication_rights` SET TAGS ('dbx_business_glossary_term' = 'Publication Rights');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `revenue_share_percent` SET TAGS ('dbx_business_glossary_term' = 'Revenue Share Percent');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `scope_description` SET TAGS ('dbx_business_glossary_term' = 'Scope Description');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `program_partner_collaboration_status` SET TAGS ('dbx_business_glossary_term' = 'Collaboration Status');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `technology_area` SET TAGS ('dbx_business_glossary_term' = 'Technology Area');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `termination_clause` SET TAGS ('dbx_business_glossary_term' = 'Termination Clause');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`program_partner_collaboration` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` SET TAGS ('dbx_subdomain' = 'results_analysis');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `compliance_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Assessment - Assessment Id');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assessor');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `obligation_register_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Assessment - Compliance Obligation Id');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Assessment - Research Project Id');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `assessment_scope` SET TAGS ('dbx_business_glossary_term' = 'Assessment Scope');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `critical_finding_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Finding Count');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `finding_count` SET TAGS ('dbx_business_glossary_term' = 'Finding Count');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `remediation_complete_flag` SET TAGS ('dbx_business_glossary_term' = 'Remediation Complete');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `risk_score` SET TAGS ('dbx_business_glossary_term' = 'Risk Score');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`compliance_assessment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`test_structure` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`test_structure` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`process_split` SET TAGS ('dbx_subdomain' = 'experimental_execution');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` SET TAGS ('dbx_subdomain' = 'results_analysis');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` ALTER COLUMN `research_test_plan_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` ALTER COLUMN `research_test_plan_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` ALTER COLUMN `updated_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`research`.`research_test_plan` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`test_suite` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_semiconductors_v1`.`research`.`test_suite` SET TAGS ('dbx_subdomain' = 'results_analysis');
