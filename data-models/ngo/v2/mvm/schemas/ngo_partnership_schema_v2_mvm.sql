...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`partner_org` (
    ...
    `country_id` BIGINT COMMENT 'Foreign key linking to field.country. Business justification: Partner organizations must maintain valid statutory registrations (NGO registration, tax-exempt status) for due diligence and partnership eligibility. Links partner vetting to legal compliance status.',
    ...
    `acronym` STRING COMMENT 'Commonly used acronym or short name for the partner organization (e.g., CARE, IRC, MSF). Widely used in SitReps, cluster coordination, and OCHA reporting to identify partners concisely.',
    ...
    `website` STRING COMMENT 'Official public website URL of the partner organization. Used for due diligence verification, public profile validation, and IATI transparency reporting.. Valid values are `^https?://[^s/$.?#].[^s]*$`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`agreement` (
    ...
    `capacity_assessment_id` BIGINT COMMENT 'Foreign key linking to partnership.capacity_assessment. Business justification: partnership_agreement carries denormalized capacity_assessment_date and capacity_assessment_status fields that represent the capacity assessment conducted as a prerequisite to or during the agreement.',
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Partnership agreements are signed and managed by a specific country office, which is the responsible NGO entity for compliance, reporting, and agreement oversight. Country directors sign agreements; c',
    `due_diligence_record_id` BIGINT COMMENT 'Foreign key linking to partnership.due_diligence_record. Business justification: partnership_agreement carries a denormalized due_diligence_risk_rating field that is sourced from the due_diligence_record conducted on the partner prior to agreement execution. Adding due_diligence_r',
    `funding_source_id` BIGINT COMMENT 'Foreign key linking to grant.funding_source. Business justification: Partnership agreements in nonprofits are funded through specific restricted or unrestricted funds. Financial reporting, fund accounting, and donor compliance require tracking which fund finances each ',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` (
    ...
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Capacity assessments are commissioned and used by specific country offices for partner management decisions (payment modality, enhanced monitoring). Country offices own the assessment process; this li',
    `intervention_id` BIGINT COMMENT 'Foreign key linking to program.intervention. Business justification: Capacity assessments routinely include CHS (Core Humanitarian Standard) self-assessment as a component for partner quality assurance. Direct operational link in partner vetting processes. CHS complian',
    ...
    `end_date` DATE COMMENT 'Date on which the capacity assessment process was formally concluded and the final report was issued.',
    ...
    `reference_code` STRING COMMENT 'Externally-known alphanumeric reference code assigned to this capacity assessment, used in correspondence, donor reporting, and audit trails (e.g., NGO-CA-2024-000123).. Valid values are `^[A-Z]{2,6}-CA-[0-9]{4}-[0-9]{4,6}$`',
    `report_url` STRING COMMENT 'URL or document management system path to the full capacity assessment report. Classified as confidential due to sensitive organizational information about the partner.',
    `scope` STRING COMMENT 'Indicates the depth and modality of the assessment: FULL (comprehensive on-site assessment covering all functional areas), ABBREVIATED (targeted review of selected areas), DESK_REVIEW (document-based review without site visit), FIELD_VISIT (on-site verification component).. Valid values are `FULL|ABBREVIATED|DESK_REVIEW|FIELD_VISIT`',
    ...
    `start_date` DATE COMMENT 'Date on which the capacity assessment process formally commenced, including document review and stakeholder engagement initiation.',
    `tool_version` STRING COMMENT 'Version number or edition of the assessment methodology tool used (e.g., USAID OCA v3.0, DFID OCAT 2019). Ensures comparability of scores across assessments conducted using different tool versions.',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` (
    ...
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Due diligence records are initiated and owned by the country office responsible for the partner relationship. Country offices manage partner compliance portfolios; this link enables country-level due ',
    ...
    `reference_number` STRING COMMENT 'Externally-known unique alphanumeric reference code assigned to this due diligence record, used in correspondence, audit trails, and donor reporting. Format: DDR-YYYY-NNNNNN.. Valid values are `^DDR-[0-9]{4}-[0-9]{6}$`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` (
    ...
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Partner contacts are assigned to interact with specific country offices (duty station). Country office staff need to identify their designated partner contacts for coordination, reporting follow-up, a',
    ...
    `partner_contact_status` STRING COMMENT 'Current lifecycle status of the partner contact record. Indicates whether the contact is actively engaged in partnership coordination, temporarily unavailable, or no longer with the organization.. Valid values are `active|inactive|on_leave|departed|pending_verification`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` (
    ...
    `component_id` BIGINT COMMENT 'Foreign key linking to program.component. Business justification: Partner performance reviews are scoped to specific program components in multi-component interventions. This link supports component-level partner accountability reporting and corrective action tracki',
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Partner performance reviews are conducted by and attributed to the country office responsible for the partnership. Country offices own the review process, and country-level performance reporting requi',
    ...
    `intervention_id` BIGINT COMMENT 'Foreign key linking to program.intervention. Business justification: Partner performance reviews may trigger or reference internal compliance reviews when issues are identified. Links performance monitoring to compliance oversight. Essential for integrated risk managem',
    ...
    `project_site_id` BIGINT COMMENT 'Foreign key linking to field.project_site. Business justification: Partner performance reviews are frequently scoped to specific project sites where the partner operates. Site-level performance reporting enables program managers to assess partner quality at individua',
    ...
    `method` STRING COMMENT 'Methodology used to conduct the performance review. Field visits provide direct observation; desk reviews rely on submitted documentation; joint reviews involve donor or cluster lead participation.. Valid values are `desk_review|field_visit|joint_review|remote_assessment`',
    ...
    `partner_performance_review_date` DATE COMMENT 'The actual calendar date on which the performance review meeting or assessment was conducted with the partner.',
    `partner_performance_review_status` STRING COMMENT 'Current lifecycle state of the performance review record. Drives workflow routing for approval and archiving.. Valid values are `draft|in_progress|completed|approved|cancelled`',
    `partner_performance_review_type` STRING COMMENT 'Classification of the review occasion. Scheduled reviews occur at fixed intervals per the SoW; triggered reviews are initiated by risk events or compliance concerns; final reviews occur at agreement close-out.. Valid values are `scheduled|ad_hoc|triggered|final|mid_term`',
    ...
    `period_end_date` DATE COMMENT 'Last day of the performance period being assessed in this review. Together with review_period_start_date defines the assessment window.',
    `period_start_date` DATE COMMENT 'First day of the performance period being assessed in this review. Aligns with the reporting cycle defined in the partner SoW.',
    ...
    `reference_number` STRING COMMENT 'Externally-known alphanumeric reference code assigned to this review for tracking in correspondence, donor reports, and audit trails. Format: PPR-YYYY-NNNNNN.. Valid values are `^PPR-[0-9]{4}-[0-9]{6}$`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`consortium` (
    ...
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Consortia operate under the coordination of a specific country office, which manages consortium governance, reporting, and donor liaison. Country offices are the operational anchor for consortium mana',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to partnership.partnership_agreement. Business justification: consortium_member carries a sub_award_agreement_number string field that is a text reference to the partnership_agreement governing that members participation in the consortium. Adding partnership_ag',
    `capacity_assessment_id` BIGINT COMMENT 'Foreign key linking to partnership.capacity_assessment. Business justification: consortium_member carries denormalized capacity_assessment_date and capacity_assessment_score fields that are clearly sourced from a capacity_assessment record conducted on the member partner prior to',
    `component_id` BIGINT COMMENT 'Foreign key linking to program.component. Business justification: In consortium-implemented programs, specific members are responsible for specific components (geographic or thematic). This link supports consortium governance reporting — tracking which member delive',
    `consortium_id` BIGINT COMMENT 'Reference to the consortium arrangement that this member belongs to.',
    `due_diligence_record_id` BIGINT COMMENT 'Foreign key linking to partnership.due_diligence_record. Business justification: consortium_member carries denormalized due_diligence_completion_date and due_diligence_status fields that map directly to a due_diligence_record for that partner within the consortium context. Adding ',
    `partner_org_id` BIGINT COMMENT 'Reference to the partner organization participating as a member of this consortium.',
    `subaward_id` BIGINT COMMENT 'Foreign key linking to grant.subaward. Business justification: Each consortium member receives a corresponding subaward under the prime award. Linking consortium_member to subaward enables financial tracking of member funding allocations against actual subaward o',
    ...
    `role` STRING COMMENT 'The functional role that this partner organization plays within the consortium (e.g., lead agency, co-implementer, technical advisor, sub-recipient, associate member, observer).. Valid values are `lead|co_implementer|technical_advisor|sub_recipient|associate_member|observer`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` (
    ...
    `component_id` BIGINT COMMENT 'Foreign key linking to program.component. Business justification: Partner reports are submitted at the component level in multi-component programs. partner_report_submission has intervention_id but no component_id. This supports component-level financial and narrati',
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Partner report submissions are received, reviewed, and accepted by the responsible country office. Country offices track reporting compliance per partner; this link is essential for country-level repo',
    `donor_report_id` BIGINT COMMENT 'Foreign key linking to grant.donor_report. Business justification: Partner report submissions are the source data for donor reports. Program teams reconcile partner submissions against donor reports for financial and narrative accuracy. donor_report_reference is a pl',
    ...
    `method` STRING COMMENT 'The channel or mechanism through which the partner submitted the report, such as email, online portal, GMS (Grant Management System) upload, hard copy, or KoboToolbox.. Valid values are `email|online_portal|gms_upload|hard_copy|kobo_toolbox`',
    ...
    `partner_report_submission_date` DATE COMMENT 'The actual calendar date on which the partner submitted this report to the organization. Used to calculate timeliness and compliance with reporting obligations.',
    ...
    `reference_number` STRING COMMENT 'Externally-known alphanumeric reference code assigned to this report submission, used in correspondence with the partner and in donor reporting systems such as Unit4 ERP or GMS.',
    ...
    `timestamp` TIMESTAMP COMMENT 'The precise date and time at which the partner report was received or uploaded into the grant management system, capturing the exact moment of submission for audit trail purposes.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ADD CONSTRAINT `fk_partnership_agreement_capacity_assessment_id` FOREIGN KEY (`capacity_assessment_id`) REFERENCES `vibe_ngo_v1`.`partnership`.`capacity_assessment`(`capacity_assessment_id`);
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ADD CONSTRAINT `fk_partnership_agreement_due_diligence_record_id` FOREIGN KEY (`due_diligence_record_id`) REFERENCES `vibe_ngo_v1`.`partnership`.`due_diligence_record`(`due_diligence_record_id`);
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Statutory Registration Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `acronym` SET TAGS ('dbx_business_glossary_term' = 'Organization Acronym');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `bank_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `hq_city` SET TAGS ('dbx_pii_location' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `partner_org_name` SET TAGS ('dbx_business_glossary_term' = 'Organization Legal Name');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `partner_org_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `partner_org_type` SET TAGS ('dbx_business_glossary_term' = 'Organization Type Classification');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `preferred_language` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `tax_identifier` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `website` SET TAGS ('dbx_business_glossary_term' = 'Organization Website URL');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_org` ALTER COLUMN `website` SET TAGS ('dbx_value_regex' = '^https?://[^s/$.?#].[^s]*$');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Award Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `capacity_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `due_diligence_record_id` SET TAGS ('dbx_business_glossary_term' = 'Due Diligence Record Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `funding_source_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Intervention Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `cluster_lead_agency` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `lead_organization_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `partnership_agreement_status` SET TAGS ('dbx_business_glossary_term' = 'Agreement Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `partnership_agreement_type` SET TAGS ('dbx_business_glossary_term' = 'Agreement Type');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`agreement` ALTER COLUMN `partnership_agreement_type` SET TAGS ('dbx_value_regex' = 'MoU|LoA|SoW|sub_award|consortium|other');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `capacity_assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `capacity_assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `capacity_assessment_status` SET TAGS ('dbx_value_regex' = 'DRAFT|IN_PROGRESS|COMPLETED|VALIDATED|ARCHIVED');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `capacity_assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Type');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `capacity_assessment_type` SET TAGS ('dbx_value_regex' = 'PRE_AWARD|PERIODIC|TRIGGERED|CLOSE_OUT');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment End Date');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `lead_assessor_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `methodology` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Methodology');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `methodology` SET TAGS ('dbx_value_regex' = 'USAID_OCA|DFID_OCAT|ECHO_PACA|IRC_CCAT|UNDP_CAPACITY_SCORECARD|OTHER');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `payment_modality_recommendation` SET TAGS ('dbx_value_regex' = 'ADVANCE|REIMBURSEMENT|DIRECT_PAYMENT|MIXED');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `reference_code` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Reference Code');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `reference_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2,6}-CA-[0-9]{4}-[0-9]{4,6}$');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `report_url` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Report Document URL');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `report_url` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `scope` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Scope');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `scope` SET TAGS ('dbx_value_regex' = 'FULL|ABBREVIATED|DESK_REVIEW|FIELD_VISIT');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Start Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`capacity_assessment` ALTER COLUMN `tool_version` SET TAGS ('dbx_business_glossary_term' = 'Assessment Tool Version');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` ALTER COLUMN `ngo_registration_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'NGO Registration Expiry Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` ALTER COLUMN `ngo_registration_number` SET TAGS ('dbx_business_glossary_term' = 'National NGO Registration Number');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` ALTER COLUMN `reference_number` SET TAGS ('dbx_business_glossary_term' = 'Due Diligence Record Reference Number');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`due_diligence_record` ALTER COLUMN `reference_number` SET TAGS ('dbx_value_regex' = '^DDR-[0-9]{4}-[0-9]{6}$');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `gender` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `nationality` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `partner_contact_status` SET TAGS ('dbx_business_glossary_term' = 'Partner Contact Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `partner_contact_status` SET TAGS ('dbx_value_regex' = 'active|inactive|on_leave|departed|pending_verification');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_contact` ALTER COLUMN `preferred_language` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Review Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `corrective_action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `corrective_action_due_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Due Date');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `corrective_action_status` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `corrective_action_status` SET TAGS ('dbx_value_regex' = 'not_required|pending|in_progress|completed|overdue');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `method` SET TAGS ('dbx_business_glossary_term' = 'Review Method');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `method` SET TAGS ('dbx_value_regex' = 'desk_review|field_visit|joint_review|remote_assessment');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `partner_performance_review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Conducted Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `partner_performance_review_status` SET TAGS ('dbx_business_glossary_term' = 'Review Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `partner_performance_review_status` SET TAGS ('dbx_value_regex' = 'draft|in_progress|completed|approved|cancelled');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `partner_performance_review_type` SET TAGS ('dbx_business_glossary_term' = 'Review Type');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `partner_performance_review_type` SET TAGS ('dbx_value_regex' = 'scheduled|ad_hoc|triggered|final|mid_term');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Review Period End Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Review Period Start Date');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `reference_number` SET TAGS ('dbx_business_glossary_term' = 'Performance Review Reference Number');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_performance_review` ALTER COLUMN `reference_number` SET TAGS ('dbx_value_regex' = '^PPR-[0-9]{4}-[0-9]{6}$');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `due_diligence_date` SET TAGS ('dbx_business_glossary_term' = 'Due Diligence Completion Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `due_diligence_status` SET TAGS ('dbx_business_glossary_term' = 'Partner Due Diligence Status');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `due_diligence_status` SET TAGS ('dbx_value_regex' = 'pending|in_progress|completed|expired');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `localization_percentage` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `consortium_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium` ALTER COLUMN `short_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `contribution_type` SET TAGS ('dbx_value_regex' = 'financial|in_kind|technical_assistance|capacity_building|advocacy|coordination');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `cost_share_percentage` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `funding_allocation_percentage` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `role` SET TAGS ('dbx_business_glossary_term' = 'Member Role');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `role` SET TAGS ('dbx_value_regex' = 'lead|co_implementer|technical_advisor|sub_recipient|associate_member|observer');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`consortium_member` ALTER COLUMN `signing_authority_name` SET TAGS ('dbx_pii_personal' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `geographic_coverage` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `method` SET TAGS ('dbx_business_glossary_term' = 'Submission Method');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `method` SET TAGS ('dbx_value_regex' = 'email|online_portal|gms_upload|hard_copy|kobo_toolbox');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `partner_report_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Submission Date');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `reference_number` SET TAGS ('dbx_business_glossary_term' = 'Submission Reference Number');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `reporting_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period End Date');
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `reporting_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Start Date');
...
ALTER TABLE `vibe_ngo_v1`.`partnership`.`partner_report_submission` ALTER COLUMN `timestamp` SET TAGS ('dbx_business_glossary_term' = 'Submission Timestamp');
