...
-- Generated on: 2026-06-24 00:22:19
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` (
    ...
    `product_brand_id` BIGINT COMMENT 'FK to authoritative product.product_brand; resolves SSOT duplicate',
    ...
    `logo_asset_url` DECIMAL(18,2) COMMENT 'URL or file path to the official brand logo asset in the digital asset management system. Used for marketing materials, packaging design, and digital campaigns.',
...
) COMMENT 'Brand entity from marketing perspective - brand equity, positioning, campaigns, and consumer perception. SSOT for brand marketing strategy. Distinct from product.product_brand which is the product portfolio/PLM brand assignment.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` (
    ...
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Campaign expense allocation requires charging campaign spend to a specific cost center for budgeting.',
    ...
    `campaign_objective` STRING COMMENT '',
    ...
    `total_budget` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_flight` (
    ...
    `creative_asset_id` DECIMAL(18,2) COMMENT 'Reference to the creative asset used in this flight. Links to creative asset management system for creative performance analysis.',
    ...
    `allocated_budget` DECIMAL(18,2) COMMENT '',
    ...
    `budget_spent` DECIMAL(18,2) COMMENT '',
    ...
    `start_date` DATE COMMENT '',
    `campaign_flight_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` (
    ...
    `planning_period` STRING COMMENT '',
    ...
    `start_date` DATE COMMENT '',
    `media_plan_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` (
    `media_spend_id` DECIMAL(18,2) COMMENT 'Unique identifier for the media spend transaction record. Primary key for the media spend data product.',
    `campaign_flight_id` BIGINT COMMENT '',
    `campaign_id` BIGINT COMMENT 'Reference to the marketing campaign associated with this media spend transaction.',
    `cost_center_id` DECIMAL(18,2) COMMENT 'Foreign key linking to finance.cost_center. Business justification: Media spend invoices are posted against cost centers to track advertising costs per organizational unit.',
    ...
    `impressions` BIGINT COMMENT '',
    `impressions_delivered` BIGINT COMMENT 'The total number of ad impressions delivered for this spend transaction.',
    `invoice_number` DECIMAL(18,2) COMMENT 'The invoice number from the media owner or agency billing system for this spend transaction.',
    ...
    `payment_status` DECIMAL(18,2) COMMENT 'The payment status of the invoice associated with this media spend transaction.',
    ...
    `spend_period` DECIMAL(18,2) COMMENT 'The fiscal or calendar period (e.g., 2024-Q1, 2024-03) to which this spend is allocated for reporting purposes.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` (
    `creative_asset_id` DECIMAL(18,2) COMMENT 'Unique identifier for the creative asset record. Primary key.',
    ...
    `parent_asset_creative_asset_id` DECIMAL(18,2) COMMENT 'Identifier of the parent creative asset if this asset is a derivative, localized version, or variant of another asset.',
    ...
    `asset_code` DECIMAL(18,2) COMMENT 'Unique business identifier or code assigned to the creative asset for tracking and reference purposes.',
    `asset_format` DECIMAL(18,2) COMMENT '',
    `asset_name` DECIMAL(18,2) COMMENT 'Human-readable name or title of the creative asset (e.g., Spring Campaign Hero Banner, Product Launch Video 30s).',
    `asset_type` DECIMAL(18,2) COMMENT 'Classification of the creative asset by media type (video, image, audio, document, HTML, social post, email template). [ENUM-REF-CANDIDATE: video|image|audio|document|html|social_post|email_template — 7 candidates stripped; promote to reference product]',
    ...
    `creative_asset_description` DECIMAL(18,2) COMMENT 'Detailed textual description of the creative asset content, purpose, and key messaging.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` (
    ...
    `influencer_category` STRING COMMENT '',
    ...
    `engagement_rate` DECIMAL(18,2) COMMENT '',
    ...
    `fee_per_post` DECIMAL(18,2) COMMENT '',
    ...
    `influencer_name` STRING COMMENT '',
    ...
    `payment_terms` DECIMAL(18,2) COMMENT 'Standard payment terms agreed with the influencer for campaign engagements.',
    ...
    `tier` STRING COMMENT '',
    `total_campaigns_participated` DECIMAL(18,2) COMMENT 'Cumulative count of campaigns the influencer has participated in with Consumer Goods. Used for partnership history and performance tracking.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`consumer_segment` (
    ...
    `consumer_segment_description` STRING COMMENT '',
    ...
    `price_sensitivity_level` DECIMAL(18,2) COMMENT 'Degree of price sensitivity and responsiveness to promotions and discounts for consumers in this segment.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` (
    ...
    `nps_score` DECIMAL(18,2) COMMENT 'The Net Promoter Score calculated from the research study, measuring customer loyalty and likelihood to recommend the brand on a scale from -100 to +100.',
    ...
    `start_date` DATE COMMENT '',
    `market_research_study_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_health_tracker` (
    ...
    `awareness_percent` DECIMAL(18,2) COMMENT '',
    `awareness_score` DECIMAL(18,2) COMMENT '',
    ...
    `consideration_percent` DECIMAL(18,2) COMMENT '',
    `consideration_score` DECIMAL(18,2) COMMENT '',
    ...
    `preference_score` DECIMAL(18,2) COMMENT '',
    ...
    `purchase_intent_score` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` (
    ...
    `sov_measurement_category` STRING COMMENT '',
    `channel` STRING COMMENT '',
    `competitor_brand` STRING COMMENT '',
    ...
    `sov_percent` DECIMAL(18,2) COMMENT '',
    `sov_percentage` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`digital_performance` (
    ...
    `creative_asset_id` DECIMAL(18,2) COMMENT 'Identifier for the specific creative asset used in this campaign execution. Links performance to creative master data for creative effectiveness analysis.',
    ...
    `click_through_rate` DECIMAL(18,2) COMMENT '',
    ...
    `ctr_percent` DECIMAL(18,2) COMMENT '',
    ...
    `revenue_attributed` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`attribution` (
    ...
    `creative_asset_id` DECIMAL(18,2) COMMENT 'Identifier of the creative asset (ad, banner, video) shown at this touchpoint.',
    ...
    `attributed_clicks` BIGINT COMMENT '',
    ...
    `attributed_impressions` BIGINT COMMENT '',
    `attributed_revenue` DECIMAL(18,2) COMMENT '',
    ...
    `cost_per_acquisition` DECIMAL(18,2) COMMENT '',
    ...
    `measurement_date` DATE COMMENT '',
    `measurement_period_end` DATE COMMENT '',
    `measurement_period_start` DATE COMMENT '',
    ...
    `total_touchpoints` DECIMAL(18,2) COMMENT 'Total number of marketing touchpoints in the consumer journey leading to this conversion.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`social_listening_record` (
    ...
    `negative_mentions` STRING COMMENT '',
    ...
    `neutral_mentions` STRING COMMENT '',
    ...
    `sentiment_score` DECIMAL(18,2) COMMENT '',
    ...
    `total_mention_volume` DECIMAL(18,2) COMMENT 'Total number of social media mentions captured during the observation period.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` (
    ...
    `nielsen_panel_insight_category` STRING COMMENT '',
    ...
    `dollar_sales` DECIMAL(18,2) COMMENT '',
    ...
    `household_penetration_pct` DECIMAL(18,2) COMMENT '',
    ...
    `market_share_pct` DECIMAL(18,2) COMMENT '',
    `market_share_percent` DECIMAL(18,2) COMMENT '',
    ...
    `total_panel_households` DECIMAL(18,2) COMMENT 'Total number of households in the Nielsen panel for this geography and period.',
    `total_purchase_occasions` DECIMAL(18,2) COMMENT 'Total number of purchase transactions recorded for the category, brand, or SKU during the panel period.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` (
    ...
    `supplier_id` BIGINT COMMENT 'P6: Connect procurement.agency by adding column supplier_id (BIGINT) with FK to procurement.supplier.supplier_id because agencies are external vendors and should link to the supplier master. P6: connect_table: procurement.agency** - add column ',
    ...
    `fee_structure` DECIMAL(18,2) COMMENT 'Description of the agencys fee arrangement, including rate type (hourly, monthly retainer, project-based, commission percentage, performance-based) and any tiered or volume-based pricing.',
    ...
    `payment_terms` DECIMAL(18,2) COMMENT 'Standard payment terms agreed with the agency (e.g., Net 30, Net 60, milestone-based, advance payment). Defines when invoices are due.',
    ...
    `retainer_amount` DECIMAL(18,2) COMMENT '',
    ...
    `agency_status` STRING COMMENT '',
    `tax_number` DECIMAL(18,2) COMMENT 'Tax identification number or employer identification number (EIN) of the agency for tax reporting and compliance purposes.',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` (
    `marketing_budget_id` DECIMAL(18,2) COMMENT 'Unique identifier for the marketing budget allocation record. Primary key.',
    `campaign_id` BIGINT COMMENT '',
    ...
    `finance_budget_id` DECIMAL(18,2) COMMENT 'FK to authoritative finance.finance_budget; resolves SSOT duplicate',
    ...
    `budget_amount` DECIMAL(18,2) COMMENT '',
    `budget_category` STRING COMMENT '',
    `budget_code` DECIMAL(18,2) COMMENT 'Externally-known unique code for this marketing budget allocation, used for tracking and reference across planning systems.',
    `budget_context` STRING COMMENT 'Marketing-specific budget context',
    `budget_name` DECIMAL(18,2) COMMENT 'Human-readable name or title for this marketing budget allocation, typically describing the scope or purpose.',
    `budget_period` DECIMAL(18,2) COMMENT '',
    `budget_status` DECIMAL(18,2) COMMENT 'Current lifecycle status of the budget allocation in the approval and execution workflow.',
    `budget_type` DECIMAL(18,2) COMMENT 'Classification of the budget allocation by planning horizon and purpose (annual, quarterly, campaign-specific, project-based, tactical, or strategic).',
    `budget_year` DECIMAL(18,2) COMMENT '',
    ...
    `spent_amount` DECIMAL(18,2) COMMENT '',
    `marketing_budget_status` STRING COMMENT '',
...
) COMMENT 'Marketing department budget for campaigns, media, and brand activities. SSOT for marketing spend planning. Distinct from finance.finance_budget which is the corporate-level financial budget.';

CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` (
    `market_share_record_id` DECIMAL(18,2) COMMENT 'Unique identifier for the market share measurement record.',
    ...
    `market_share_record_category` STRING COMMENT '',
    ...
    `market_share_percent` DECIMAL(18,2) COMMENT '',
    `market_share_value_pct` DECIMAL(18,2) COMMENT '',
    `market_share_volume_pct` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` (
    ...
    `payment_schedule` DECIMAL(18,2) COMMENT 'Description of the payment terms and schedule for the sponsorship contract, including installment amounts and due dates.',
    ...
    `sponsorship_type` STRING COMMENT '',
    `start_date` DATE COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` (
    ...
    `promotion_event_id` BIGINT COMMENT 'FK to promotion.promotion_event when this marketing event is trade-linked; resolves SSOT duplicate',
    ...
    `event_context` STRING COMMENT 'Marketing-specific event context',
    ...
    `marketing_event_status` STRING COMMENT '',
...
) COMMENT 'Marketing event such as product launches, brand activations, sponsorships, and experiential marketing. SSOT for marketing event management. Distinct from promotion.promotion_event which is a trade promotion execution event.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` (
    ...
    `annual_volume_target` DECIMAL(18,2) COMMENT 'Annual volume target for the brand at this account',
    `approval_status` STRING COMMENT '',
    `approved_by` STRING COMMENT '',
    `assignment_date` DATE COMMENT 'Date of brand assignment',
    `assignment_end_date` DATE COMMENT '',
    `assignment_reason` STRING COMMENT '',
    ...
    `channel_code` STRING COMMENT 'Channel code for the brand assignment',
    `created_timestamp` TIMESTAMP COMMENT '',
    `distribution_priority` STRING COMMENT 'Priority ranking for distribution',
    `distribution_tier` STRING COMMENT '',
    `effective_date` DATE COMMENT '',
    `effective_from` STRING COMMENT '',
    `effective_until` STRING COMMENT '',
    `end_date` DATE COMMENT '',
    `exclusivity_flag` BOOLEAN COMMENT 'Indicates exclusive brand rights',
    `expiry_date` DATE COMMENT '',
    `is_active` BOOLEAN COMMENT '',
    `is_primary` BOOLEAN COMMENT '',
    `is_primary_brand` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the assignment was last updated',
    ...
    `priority_rank` STRING COMMENT 'Priority ranking of brand',
    `region_code` STRING COMMENT '',
    `revenue_share_percentage` DECIMAL(18,2) COMMENT 'Revenue share percentage for this assignment',
    `target_shelf_share_pct` DECIMAL(18,2) COMMENT 'Target shelf share percentage for the brand at this account',
    `territory_code` STRING COMMENT 'Territory code for assignment',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `expiration_date` DATE COMMENT 'Date the brand_assignment expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the brand_assignment',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the brand_assignment',
    `brand_assignment_description` STRING COMMENT 'Free-text description of the brand_assignment',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the brand_assignment was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the brand_assignment was last updated',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` (
    `brand_distribution_allocation_id` DECIMAL(18,2) COMMENT 'Primary key for the brand_distribution_allocation association',
    `employee_id` BIGINT COMMENT 'Employee who approved allocation',
    ...
    `allocation_basis` STRING COMMENT 'Basis on which the allocation was determined.',
    `allocation_driver` STRING COMMENT 'Driver for allocation (sales, population, etc.)',
    `allocation_frequency` STRING COMMENT 'Frequency of allocation (weekly, monthly, etc.)',
    `allocation_method` STRING COMMENT 'Method used for allocation (proportional, fixed, etc.)',
    `allocation_percent` DECIMAL(18,2) COMMENT 'Percentage of the nodes capacity allocated to the brand',
    `allocation_percentage` STRING COMMENT '',
    `allocation_period` STRING COMMENT '',
    `allocation_quantity` DECIMAL(18,2) COMMENT '',
    `allocation_reason` STRING COMMENT 'Reason for allocation',
    `allocation_revenue_target` DECIMAL(18,2) COMMENT 'Revenue target for this brand distribution allocation',
    `allocation_status` STRING COMMENT '',
    `allocation_type` DECIMAL(18,2) COMMENT '',
    `allocation_variance_pct` DECIMAL(18,2) COMMENT 'Variance from planned allocation',
    `allocation_volume_units` DECIMAL(18,2) COMMENT 'Allocated volume in units for the brand at this node',
    `approval_date` DATE COMMENT 'Date allocation was approved',
    `approval_required_flag` BOOLEAN COMMENT 'Whether approval is required',
    `approved_flag` BOOLEAN COMMENT 'Whether the allocation has been approved',
    `channel_code` STRING COMMENT '',
    `channel_type` STRING COMMENT 'Distribution channel type',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency_code` STRING COMMENT 'Currency code for monetary values',
    `effective_date` DATE COMMENT 'Start date',
    ...
    `effective_until` STRING COMMENT '',
    `expiry_date` DATE COMMENT 'End date',
    `historical_sales_weight_pct` DECIMAL(18,2) COMMENT 'Weight given to historical sales',
    `is_active` BOOLEAN COMMENT '',
    `last_allocation_date` DATE COMMENT 'Date of last allocation',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp of last update to this allocation',
    `market_potential_weight_pct` DECIMAL(18,2) COMMENT 'Weight given to market potential',
    `maximum_allocation_units` DECIMAL(18,2) COMMENT 'Maximum allocation in units',
    `minimum_allocation_units` DECIMAL(18,2) COMMENT 'Minimum allocation in units',
    `next_allocation_date` DATE COMMENT 'Date of next allocation',
    `notes` STRING COMMENT '',
    ...
    `reallocation_allowed_flag` BOOLEAN COMMENT 'Whether reallocation is allowed',
    `region_code` STRING COMMENT '',
    `revenue_share_pct` DECIMAL(18,2) COMMENT 'Revenue share percentage',
    `brand_distribution_allocation_status` STRING COMMENT '',
    `strategic_priority_weight_pct` DECIMAL(18,2) COMMENT 'Weight given to strategic priority',
    `territory_code` STRING COMMENT 'Territory code for allocation',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
    `volume_cap_units` DECIMAL(18,2) COMMENT 'Maximum volume cap in units',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` (
    `campaign_inventory_allocation_id` DECIMAL(18,2) COMMENT 'Primary key for the campaign_inventory_allocation association',
    ...
    `allocation_reason` DECIMAL(18,2) COMMENT '',
    `allocation_start_date` DATE COMMENT 'Start date of the allocation period',
    `allocation_status` STRING COMMENT '',
    `allocation_type` STRING COMMENT 'Type of allocation (sample, display, promotional pack)',
    `allocation_uom` STRING COMMENT '',
    `approval_date` DATE COMMENT 'Date the allocation was approved',
    `approved_by` STRING COMMENT 'Person who approved the allocation',
    `consumed_quantity` DECIMAL(18,2) COMMENT 'Quantity consumed from allocation',
    `cost_per_unit` DECIMAL(18,2) COMMENT 'Cost per unit of allocated inventory',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency_code` STRING COMMENT 'Currency of the cost amounts',
    `estimated_cost_amount` DECIMAL(18,2) COMMENT 'Estimated cost of the allocated inventory',
    `expiry_date` DATE COMMENT '',
    `fulfilled_quantity` STRING COMMENT '',
    `fulfillment_date` DATE COMMENT 'Date when the allocation was fulfilled',
    `is_active` BOOLEAN COMMENT '',
    `is_fulfilled` BOOLEAN COMMENT 'Whether the allocation has been fully fulfilled',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the record was last updated',
    `notes` STRING COMMENT '',
    `priority_level` STRING COMMENT 'Priority level of allocation',
    `priority_rank` STRING COMMENT 'Priority ranking for allocation fulfillment',
    `remaining_quantity` DECIMAL(18,2) COMMENT 'Remaining quantity available',
    `reserved_quantity` STRING COMMENT '',
    `total_cost_amount` DECIMAL(18,2) COMMENT 'Total cost of the inventory allocation',
    `unit_of_measure` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `campaign_inventory_allocation_status` STRING COMMENT 'Lifecycle status of the campaign_inventory_allocation record',
    `effective_date` DATE COMMENT 'Date the campaign_inventory_allocation becomes effective',
    `expiration_date` DATE COMMENT 'Date the campaign_inventory_allocation expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the campaign_inventory_allocation',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the campaign_inventory_allocation',
    `campaign_inventory_allocation_description` STRING COMMENT 'Free-text description of the campaign_inventory_allocation',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the campaign_inventory_allocation was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the campaign_inventory_allocation was last updated',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` (
    `marketing_offset_allocation_id` DECIMAL(18,2) COMMENT 'Primary key for the offset_allocation association',
    `campaign_id` BIGINT COMMENT 'Foreign key linking to the campaign',
    `carbon_offset_id` BIGINT COMMENT 'Foreign key linking to the carbon offset',
    `marketing_brand_id` BIGINT COMMENT '',
    `allocated_tonnes_co2e` DECIMAL(18,2) COMMENT '',
    `allocation_amount` DECIMAL(18,2) COMMENT 'Amount of credits allocated from this offset to the campaign (tCO2e)',
    `allocation_cost` DECIMAL(18,2) COMMENT '',
    `allocation_currency` STRING COMMENT 'Currency of the allocation amount',
    `allocation_date` DATE COMMENT '',
    ...
    `allocation_status` STRING COMMENT '',
    `allocation_type` DECIMAL(18,2) COMMENT '',
    `carbon_tonnes_offset` DECIMAL(18,2) COMMENT 'Tonnes of carbon offset allocated',
    `certification_standard` STRING COMMENT 'Certification standard (Gold Standard, VCS, etc.)',
    `cost_amount` DECIMAL(18,2) COMMENT '',
    `cost_per_unit` DECIMAL(18,2) COMMENT 'Cost per offset unit',
    `created_timestamp` TIMESTAMP COMMENT '',
    ...
    `expiry_date` DATE COMMENT '',
    `is_active` BOOLEAN COMMENT '',
    `is_verified` BOOLEAN COMMENT 'Whether the offset allocation has been verified',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the record was last updated',
    `notes` STRING COMMENT '',
    `offset_amount` DECIMAL(18,2) COMMENT '',
    `offset_project_name` STRING COMMENT 'Name of the offset project',
    `offset_quantity_tonnes` DECIMAL(18,2) COMMENT '',
    `offset_reason` STRING COMMENT '',
    `offset_standard` STRING COMMENT 'Standard used for the offset (Gold Standard, VCS)',
    `offset_type` STRING COMMENT '',
    `offset_unit_of_measure` STRING COMMENT 'Unit of measure for the offset',
    `offset_units` DECIMAL(18,2) COMMENT 'Number of offset units',
    `period_end_date` DATE COMMENT 'End date of the allocation period',
    `period_start_date` DATE COMMENT 'Start date of the allocation period',
    `purchase_date` DATE COMMENT 'Date the carbon offset was purchased for the campaign',
    `reporting_period` STRING COMMENT '',
    `ssot_note` STRING COMMENT 'Authoritative marketing offset allocation',
    `marketing_offset_allocation_status` STRING COMMENT '',
    `total_cost_amount` DECIMAL(18,2) COMMENT 'Total cost of the offset allocation',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `verification_date` DATE COMMENT 'Date of verification',
    `verification_status` STRING COMMENT 'Verification status',
    `expiration_date` DATE COMMENT 'Date the marketing_offset_allocation expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the marketing_offset_allocation',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the marketing_offset_allocation',
    `marketing_offset_allocation_description` STRING COMMENT 'Free-text description of the marketing_offset_allocation',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the marketing_offset_allocation was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the marketing_offset_allocation was last updated',
    CONSTRAINT pk_marketing_offset_allocation PRIMARY KEY(`marketing_offset_allocation_id`)
) COMMENT 'Carbon offset allocation to marketing activities (events, campaigns, media production). SSOT for marketing carbon offset tracking. Distinct from distribution.distribution_offset_allocation which covers distribution operations.';
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` (
    ...
    `creative_asset_id` DECIMAL(18,2) COMMENT '',
    ...
    `country_code` STRING COMMENT 'Country code for the submission jurisdiction',
    `created_timestamp` TIMESTAMP COMMENT '',
    `current_submissions` STRING COMMENT '',
    `end_date` DATE COMMENT '',
    `expiry_date` DATE COMMENT 'Date when the approval expires',
    `is_active` BOOLEAN COMMENT '',
    `is_mandatory` BOOLEAN COMMENT 'Whether this submission is mandatory for the campaign',
    `jurisdiction_code` STRING COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the link was last updated',
    `link_status` STRING COMMENT '',
    `link_type` STRING COMMENT '',
    `max_submissions` STRING COMMENT '',
    `notes` STRING COMMENT '',
    `priority_level` STRING COMMENT 'Priority level of submission',
    `regulatory_body` STRING COMMENT 'Regulatory body the submission was sent to',
    `rejection_reason` STRING COMMENT 'Reason for rejection if applicable',
    `response_date` DATE COMMENT 'Date the regulatory response was received',
    `resubmission_required_flag` BOOLEAN COMMENT 'Indicates if resubmission is required',
    `review_date` DATE COMMENT 'Date of review',
    `reviewer_name` STRING COMMENT '',
    `start_date` DATE COMMENT '',
    `submission_country` STRING COMMENT 'Country for which the submission was made',
    `submission_date` DATE COMMENT '',
    `submission_status` STRING COMMENT '',
    `submission_type` STRING COMMENT '',
    `submission_url` STRING COMMENT '',
    `submitted_by` STRING COMMENT '',
    `submitted_date` DATE COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `campaign_submission_link_status` STRING COMMENT 'Lifecycle status of the campaign_submission_link record',
    `effective_date` DATE COMMENT 'Date the campaign_submission_link becomes effective',
    `expiration_date` DATE COMMENT 'Date the campaign_submission_link expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the campaign_submission_link',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the campaign_submission_link',
    `campaign_submission_link_description` STRING COMMENT 'Free-text description of the campaign_submission_link',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the campaign_submission_link was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the campaign_submission_link was last updated',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` (
    ...
    `agency_id` BIGINT COMMENT '',
    `employee_id` BIGINT COMMENT 'Backup employee for this assignment',
    `campaign_employee_id` BIGINT COMMENT 'Foreign key linking to the employee',
    `campaign_id` BIGINT COMMENT 'Foreign key linking to the campaign',
    `campaign_replacement_for_employee_id` BIGINT COMMENT 'Employee being replaced',
    `actual_hours` DECIMAL(18,2) COMMENT 'Actual hours spent on this assignment',
    `actual_performance` STRING COMMENT 'Actual performance achieved',
    `allocation_hours` DECIMAL(18,2) COMMENT 'Allocated hours',
    `allocation_percent` DECIMAL(18,2) COMMENT '',
    `allocation_percentage` DECIMAL(18,2) COMMENT 'Percentage of the employees time allocated to the campaign',
    `approval_required_flag` BOOLEAN COMMENT 'Indicates if approval required',
    `approval_status` STRING COMMENT '',
    `assigned_budget` DECIMAL(18,2) COMMENT '',
    `assignment_end_date` DATE COMMENT 'Date when the employee stopped working on the campaign',
    `assignment_location` STRING COMMENT 'Location of assignment',
    `assignment_priority` STRING COMMENT 'Priority of assignment (high, medium, low)',
    `assignment_reason` STRING COMMENT 'Reason for assignment',
    `assignment_role` STRING COMMENT '',
    `assignment_start_date` DATE COMMENT 'Date when the employee started working on the campaign',
    `assignment_status` STRING COMMENT 'Status',
    `assignment_type` STRING COMMENT '',
    `billable_flag` BOOLEAN COMMENT 'Indicates whether the assignment is billable.',
    `budget_allocated` DECIMAL(18,2) COMMENT 'Budget allocated to assignee',
    `completion_date` DATE COMMENT 'Actual completion date of the assignment',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency_code` STRING COMMENT '',
    `deliverables` STRING COMMENT 'Key deliverables expected from the assignee',
    `effective_date` DATE COMMENT 'Start date',
    `effective_from` STRING COMMENT '',
    `effective_until` STRING COMMENT '',
    `expiry_date` DATE COMMENT 'End date',
    `hours_allocated` DECIMAL(18,2) COMMENT 'Hours allocated to campaign',
    `is_active` BOOLEAN COMMENT '',
    `is_lead` BOOLEAN COMMENT 'Whether this person is the lead for the campaign',
    `is_lead_flag` BOOLEAN COMMENT 'Whether this person is the lead for the campaign',
    `is_primary` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `notes` STRING COMMENT '',
    `overtime_eligible_flag` BOOLEAN COMMENT 'Whether overtime is eligible',
    `overtime_hours_worked` DECIMAL(18,2) COMMENT 'Overtime hours worked',
    `performance_rating` STRING COMMENT 'Performance rating (1-5)',
    `performance_target` STRING COMMENT 'Performance target for assignment',
    `planned_hours` DECIMAL(18,2) COMMENT 'Planned hours allocated for this assignment',
    `primary_contact_flag` BOOLEAN COMMENT 'Indicates if primary contact for campaign',
    `remote_work_allowed_flag` BOOLEAN COMMENT 'Whether remote work is allowed',
    `responsibility_area` STRING COMMENT 'Area of responsibility',
    `role` STRING COMMENT 'The role of the employee on the campaign (e.g., Planner, Creative, Analyst)',
    `skill_match_score` STRING COMMENT 'Score indicating skill match (1-100)',
    `campaign_assignment_status` STRING COMMENT '',
    `training_completion_date` DATE COMMENT 'Date training was completed',
    `training_required_flag` BOOLEAN COMMENT 'Whether training is required',
    `travel_budget_amount` DECIMAL(18,2) COMMENT 'Budget for travel',
    `travel_required_flag` BOOLEAN COMMENT 'Whether travel is required',
    `updated_timestamp` TIMESTAMP COMMENT 'Record update timestamp',
    `workload_pct` DECIMAL(18,2) COMMENT 'Percentage of workload allocated',
...
CREATE OR REPLACE TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` (
    `event_participation_id` BIGINT COMMENT '',
    `campaign_id` BIGINT COMMENT '',
    `employee_id` BIGINT COMMENT '',
    `marketing_event_id` BIGINT COMMENT '',
    `shopper_id` BIGINT COMMENT '',
    `trade_account_id` BIGINT COMMENT '',
    `accommodation_cost` DECIMAL(18,2) COMMENT 'Accommodation cost for the event participation',
    `attendance_confirmed_flag` BOOLEAN COMMENT '',
    `attendance_date` DATE COMMENT '',
    `attendance_duration_minutes` STRING COMMENT 'Total duration of attendance in minutes',
    `attendance_flag` BOOLEAN COMMENT '',
    `attendance_status` STRING COMMENT 'Status of attendance (e.g., confirmed, attended, no-show)',
    `cancellation_reason` STRING COMMENT 'Reason for cancellation if participation was cancelled',
    `check_in_time` TIMESTAMP COMMENT 'Timestamp when the participant checked in at the event',
    `check_in_timestamp` TIMESTAMP COMMENT '',
    `check_out_timestamp` TIMESTAMP COMMENT 'Check-out timestamp at the event',
    `cost_amount` DECIMAL(18,2) COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency_code` STRING COMMENT '',
    `engagement_score` DECIMAL(18,2) COMMENT 'Score measuring participant engagement level',
    `expense_amount` DECIMAL(18,2) COMMENT 'Expense amount incurred',
    `feedback_comments` STRING COMMENT 'Free-text feedback comments from the participant',
    `feedback_score` STRING COMMENT '',
    `hours_committed` DECIMAL(18,2) COMMENT '',
    `hours_contributed` DECIMAL(18,2) COMMENT 'Hours contributed by the participant',
    `invitation_sent_date` DATE COMMENT 'Date when the invitation was sent to the participant',
    `is_confirmed` BOOLEAN COMMENT '',
    `is_external` BOOLEAN COMMENT 'Whether the participant is external to the organization',
    `is_organizer` BOOLEAN COMMENT 'Whether the participant was an event organizer',
    `is_speaker` BOOLEAN COMMENT '',
    `last_modified_timestamp` TIMESTAMP COMMENT '',
    `last_updated_timestamp` TIMESTAMP COMMENT '',
    `leads_generated` STRING COMMENT 'Number of leads generated by this participant at the event',
    `leads_generated_count` STRING COMMENT 'Number of leads generated by this participant at the event',
    `notes` STRING COMMENT '',
    `participant_email` STRING COMMENT '',
    `participant_name` STRING COMMENT '',
    `participant_type` STRING COMMENT '',
    `participation_cost_amount` DECIMAL(18,2) COMMENT '',
    `participation_date` DATE COMMENT 'Date of participation in the marketing event',
    `participation_role` STRING COMMENT 'Role of the participant in the event (e.g., speaker, attendee, sponsor)',
    `participation_status` STRING COMMENT '',
    `participation_type` STRING COMMENT '',
    `registration_date` DATE COMMENT '',
    `responsibility_area` STRING COMMENT '',
    `role` STRING COMMENT '',
    `rsvp_date` DATE COMMENT 'Date the participant responded',
    `session_count` STRING COMMENT 'Number of sessions attended by the participant',
    `total_expense` DECIMAL(18,2) COMMENT 'Total expense incurred for the participation',
    `travel_cost` DECIMAL(18,2) COMMENT 'Travel cost incurred for participation',
    `travel_cost_amount` DECIMAL(18,2) COMMENT 'Travel cost for the participation',
    `travel_required_flag` BOOLEAN COMMENT 'Indicates if travel was required',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `event_participation_status` STRING COMMENT 'Lifecycle status of the event_participation record',
    `effective_date` DATE COMMENT 'Date the event_participation becomes effective',
    `expiration_date` DATE COMMENT 'Date the event_participation expires',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity associated with the event_participation',
    `amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the event_participation',
    `event_participation_description` STRING COMMENT 'Free-text description of the event_participation',
    `created_at` TIMESTAMP COMMENT 'Timestamp when the event_participation was created',
    `updated_at` TIMESTAMP COMMENT 'Timestamp when the event_participation was last updated',
    `attendance_count` BIGINT COMMENT 'Business attribute attendance_count',
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ADD CONSTRAINT `fk_marketing_influencer_campaign_id` FOREIGN KEY (`campaign_id`) REFERENCES `vibe_consumer_goods_v1`.`marketing`.`campaign`(`campaign_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ADD CONSTRAINT `fk_marketing_marketing_budget_campaign_id` FOREIGN KEY (`campaign_id`) REFERENCES `vibe_consumer_goods_v1`.`marketing`.`campaign`(`campaign_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ADD CONSTRAINT `fk_marketing_event_participation_campaign_id` FOREIGN KEY (`campaign_id`) REFERENCES `vibe_consumer_goods_v1`.`marketing`.`campaign`(`campaign_id`);
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` SET TAGS ('dbx_subdomain' = 'brand_management');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` SET TAGS ('dbx_ssot_owner' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` SET TAGS ('dbx_ssot_canonical' = 'product.product_brand');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` SET TAGS ('dbx_ssot_role' = 'brand_ssot');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` SET TAGS ('dbx_ssot' = 'marketing.marketing_brand');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Manager Employee Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `parent_brand_marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `parent_brand_marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Supplier Id (Foreign Key)');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `product_brand_id` SET TAGS ('dbx_business_glossary_term' = 'Product Brand Reference');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `product_brand_id` SET TAGS ('dbx_ssot' = 'reference');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `annual_marketing_budget` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `annual_marketing_budget` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `brand_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `discontinuation_date` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_brand` ALTER COLUMN `discontinuation_date` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` ALTER COLUMN `rep_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Rep Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` ALTER COLUMN `campaign_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign` ALTER COLUMN `campaign_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_flight` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_flight` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_flight` ALTER COLUMN `flight_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_flight` ALTER COLUMN `flight_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` SET TAGS ('dbx_subdomain' = 'campaign_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` ALTER COLUMN `rd_project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` SET TAGS ('dbx_subdomain' = 'campaign_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `agency_name` SET TAGS ('dbx_business_glossary_term' = 'Agency Name');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `agency_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `agency_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `market_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `market_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_sensitivity' = 'person_name');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`media_spend` ALTER COLUMN `media_owner_name` SET TAGS ('dbx_pii_kind' = 'person_name');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `asset_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `asset_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `creative_agency_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`creative_asset` ALTER COLUMN `creative_agency_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` SET TAGS ('dbx_subdomain' = 'campaign_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `agent_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `agent_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `audience_gender_split` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `audience_gender_split` SET TAGS ('dbx_pii_category' = 'special_category');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `email_address` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `full_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `full_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `influencer_tier` SET TAGS ('dbx_business_glossary_term' = 'Influencer Tier Classification');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `influencer_tier` SET TAGS ('dbx_value_regex' = 'Nano|Micro|Macro|Mega|Celebrity');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `influencer_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `performance_rating` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `performance_rating` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`influencer` ALTER COLUMN `phone_number` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`consumer_segment` SET TAGS ('dbx_subdomain' = 'brand_management');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`consumer_segment` ALTER COLUMN `segment_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`consumer_segment` ALTER COLUMN `segment_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `data_provider_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `data_provider_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `research_agency_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `research_agency_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `study_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_research_study` ALTER COLUMN `study_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_health_tracker` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_health_tracker` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_health_tracker` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` ALTER COLUMN `product_category_id` SET TAGS ('dbx_business_glossary_term' = 'Product Category ID');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` ALTER COLUMN `top_competitor_brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sov_measurement` ALTER COLUMN `top_competitor_brand_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`digital_performance` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`digital_performance` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`digital_performance` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`attribution` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`attribution` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`attribution` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`social_listening_record` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`social_listening_record` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`social_listening_record` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `brand_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `gtin` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`nielsen_panel_insight` ALTER COLUMN `gtin` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` SET TAGS ('dbx_subdomain' = 'partner_relationships');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `address_line1` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `address_line2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `address_line2` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `agency_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `agency_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `performance_rating` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `performance_rating` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_category' = 'address');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`agency` ALTER COLUMN `primary_contact_phone` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` SET TAGS ('dbx_subdomain' = 'brand_management');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` SET TAGS ('dbx_ssot_owner' = 'marketing');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` SET TAGS ('dbx_budget_scope' = 'marketing_spend');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` SET TAGS ('dbx_ssot_role' = 'marketing_budget_view');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` SET TAGS ('dbx_ssot_reference' = 'finance.finance_budget');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_budget_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_budget_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_budget_id` SET TAGS ('dbx_ssot_superseded_by' = 'finance.finance_budget');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_budget_id` SET TAGS ('dbx_ssot_status' = 'duplicate_resolved');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Budget Reference');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_ssot' = 'reference');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `actual_spend_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Spend Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `agency_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Agency Fee Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `budget_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `budget_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `budget_status` SET TAGS ('dbx_business_glossary_term' = 'Budget Status');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `budget_type` SET TAGS ('dbx_business_glossary_term' = 'Budget Type');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `campaign_type` SET TAGS ('dbx_business_glossary_term' = 'Campaign Type');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `channel` SET TAGS ('dbx_business_glossary_term' = 'Marketing Channel');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `committed_spend_amount` SET TAGS ('dbx_business_glossary_term' = 'Committed Spend Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `contingency_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Contingency Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `contingency_budget_amount` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `contingency_budget_amount` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `country_codes` SET TAGS ('dbx_business_glossary_term' = 'Country Codes');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Quarter');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_value_regex' = 'Q1|Q2|Q3|Q4');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `geography_scope` SET TAGS ('dbx_business_glossary_term' = 'Geography Scope');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `geography_scope` SET TAGS ('dbx_value_regex' = 'global|regional|national|local');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_value_regex' = '^[0-9]{6,10}$');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `non_working_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Non-Working Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Budget Notes');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `production_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Production Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `research_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Research Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `total_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `variance_amount` SET TAGS ('dbx_business_glossary_term' = 'Variance Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `variance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Variance Percentage');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_budget` ALTER COLUMN `working_media_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Working Media Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` SET TAGS ('dbx_subdomain' = 'performance_analytics');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` ALTER COLUMN `geography_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`market_share_record` ALTER COLUMN `geography_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` SET TAGS ('dbx_subdomain' = 'partner_relationships');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `agency_partner_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `agency_partner_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `sponsorship_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `sponsorship_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `property_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`sponsorship` ALTER COLUMN `property_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` SET TAGS ('dbx_subdomain' = 'campaign_execution');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` SET TAGS ('dbx_ssot_owner' = 'marketing');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` SET TAGS ('dbx_event_scope' = 'brand_activation');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` SET TAGS ('dbx_ssot_role' = 'marketing_event_view');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` SET TAGS ('dbx_ssot' = 'marketing.marketing_event');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `marketing_event_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `marketing_event_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `product_category_id` SET TAGS ('dbx_business_glossary_term' = 'Product Category ID');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Event Reference');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_ssot' = 'reference');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `promotion_event_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `actual_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Cost Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `actual_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `actual_end_date` SET TAGS ('dbx_business_glossary_term' = 'Actual End Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line1` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `address_line2` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `budget_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `event_description` SET TAGS ('dbx_business_glossary_term' = 'Event Description');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `event_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `event_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'planned|confirmed|in_progress|completed|cancelled|postponed');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `event_type` SET TAGS ('dbx_value_regex' = 'product_launch|consumer_activation|trade_show|sampling_event|pop_up_experience|retailer_presentation');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `lead_count` SET TAGS ('dbx_business_glossary_term' = 'Lead Count');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `market_geography` SET TAGS ('dbx_business_glossary_term' = 'Market Geography');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `nps_score` SET TAGS ('dbx_business_glossary_term' = 'Net Promoter Score (NPS)');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `objective` SET TAGS ('dbx_business_glossary_term' = 'Event Objective');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `objective` SET TAGS ('dbx_value_regex' = 'brand_awareness|product_trial|lead_generation|retailer_engagement|consumer_education|market_research');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `outcome` SET TAGS ('dbx_business_glossary_term' = 'Event Outcome');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `outcome` SET TAGS ('dbx_value_regex' = 'exceeded_expectations|met_expectations|below_expectations|cancelled|postponed');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `planned_end_date` SET TAGS ('dbx_business_glossary_term' = 'Planned End Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `planned_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Start Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_category' = 'address');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `registration_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Registration Required Flag');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `sample_units_distributed` SET TAGS ('dbx_business_glossary_term' = 'Sample Units Distributed');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State or Province');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `target_attendance` SET TAGS ('dbx_business_glossary_term' = 'Target Attendance');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `venue_name` SET TAGS ('dbx_business_glossary_term' = 'Venue Name');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `venue_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `venue_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `venue_type` SET TAGS ('dbx_business_glossary_term' = 'Venue Type');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_event` ALTER COLUMN `venue_type` SET TAGS ('dbx_value_regex' = 'retail_store|convention_center|outdoor_location|virtual_platform|corporate_office|pop_up_space');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` SET TAGS ('dbx_subdomain' = 'brand_management');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `brand_assignment_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `brand_assignment_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `brand_assignment_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_assignment` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` SET TAGS ('dbx_subdomain' = 'brand_management');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `allocation_driver` SET TAGS ('dbx_business_glossary_term' = 'Allocation Driver');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `allocation_frequency` SET TAGS ('dbx_business_glossary_term' = 'Allocation Frequency');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `allocation_reason` SET TAGS ('dbx_business_glossary_term' = 'Allocation Reason');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `allocation_variance_pct` SET TAGS ('dbx_business_glossary_term' = 'Allocation Variance Percent');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Approval Required');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `historical_sales_weight_pct` SET TAGS ('dbx_business_glossary_term' = 'Historical Sales Weight');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `last_allocation_date` SET TAGS ('dbx_business_glossary_term' = 'Last Allocation Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `market_potential_weight_pct` SET TAGS ('dbx_business_glossary_term' = 'Market Potential Weight');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `maximum_allocation_units` SET TAGS ('dbx_business_glossary_term' = 'Maximum Allocation Units');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `minimum_allocation_units` SET TAGS ('dbx_business_glossary_term' = 'Minimum Allocation Units');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `next_allocation_date` SET TAGS ('dbx_business_glossary_term' = 'Next Allocation Date');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `reallocation_allowed_flag` SET TAGS ('dbx_business_glossary_term' = 'Reallocation Allowed');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`brand_distribution_allocation` ALTER COLUMN `strategic_priority_weight_pct` SET TAGS ('dbx_business_glossary_term' = 'Strategic Priority Weight');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_status` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_status` SET TAGS ('dbx_classification' = 'status');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_status` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `effective_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `effective_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `effective_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `campaign_inventory_allocation_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_inventory_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` SET TAGS ('dbx_subdomain' = 'brand_management');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` SET TAGS ('dbx_ssot_owner' = 'true');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` SET TAGS ('dbx_ssot_role' = 'marketing_offset_view');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` SET TAGS ('dbx_ssot_reference' = 'distribution.distribution_offset_allocation');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_id` SET TAGS ('dbx_ssot_superseded_by' = 'distribution.distribution_offset_allocation');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_id` SET TAGS ('dbx_ssot_status' = 'duplicate_resolved');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_brand_id` SET TAGS ('dbx_pii_type' = 'tax_id');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `ssot_note` SET TAGS ('dbx_business_glossary_term' = 'SSOT Offset Allocation');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `ssot_note` SET TAGS ('dbx_ssot' = 'authoritative');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `marketing_offset_allocation_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`marketing_offset_allocation` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_status` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_status` SET TAGS ('dbx_classification' = 'status');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_status` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `effective_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `effective_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `effective_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `campaign_submission_link_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_submission_link` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Backup Employee');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `campaign_replacement_for_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Replacement For Employee');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `campaign_replacement_for_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `campaign_replacement_for_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `actual_performance` SET TAGS ('dbx_business_glossary_term' = 'Actual Performance');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `assignment_location` SET TAGS ('dbx_business_glossary_term' = 'Assignment Location');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `assignment_priority` SET TAGS ('dbx_business_glossary_term' = 'Assignment Priority');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `assignment_reason` SET TAGS ('dbx_business_glossary_term' = 'Assignment Reason');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `overtime_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Overtime Eligible');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `overtime_hours_worked` SET TAGS ('dbx_business_glossary_term' = 'Overtime Hours Worked');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `performance_rating` SET TAGS ('dbx_business_glossary_term' = 'Performance Rating');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `performance_target` SET TAGS ('dbx_business_glossary_term' = 'Performance Target');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `remote_work_allowed_flag` SET TAGS ('dbx_business_glossary_term' = 'Remote Work Allowed');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `skill_match_score` SET TAGS ('dbx_business_glossary_term' = 'Skill Match Score');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `training_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `training_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Required');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `travel_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Travel Budget Amount');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `travel_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Travel Required');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`campaign_assignment` ALTER COLUMN `workload_pct` SET TAGS ('dbx_business_glossary_term' = 'Workload Percent');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` SET TAGS ('dbx_subdomain' = 'campaign_execution');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `marketing_event_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `marketing_event_id` SET TAGS ('dbx_pii_type' = 'tax_id');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `accommodation_cost` SET TAGS ('dbx_business_glossary_term' = 'Accommodation Cost');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `attendance_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Attendance Duration Minutes');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `feedback_comments` SET TAGS ('dbx_business_glossary_term' = 'Feedback Comments');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `invitation_sent_date` SET TAGS ('dbx_business_glossary_term' = 'Invitation Sent Date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `is_organizer` SET TAGS ('dbx_business_glossary_term' = 'Is Organizer');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `leads_generated` SET TAGS ('dbx_business_glossary_term' = 'Leads Generated');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `participant_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `participant_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `session_count` SET TAGS ('dbx_business_glossary_term' = 'Session Count');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `total_expense` SET TAGS ('dbx_business_glossary_term' = 'Total Expense');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_status` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_status` SET TAGS ('dbx_classification' = 'status');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_status` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_classification' = 'date');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `expiration_date` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `quantity` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `amount` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `amount` SET TAGS ('dbx_classification' = 'measure');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `amount` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_description` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_description` SET TAGS ('dbx_classification' = 'text');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `event_participation_description` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `created_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_derived' = 'true');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_classification' = 'audit');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `updated_at` SET TAGS ('dbx_source' = 'business_context');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `attendance_count` SET TAGS ('dbx_added_by' = 'VREQ-049');
ALTER TABLE `vibe_consumer_goods_v1`.`marketing`.`event_participation` ALTER COLUMN `attendance_count` SET TAGS ('dbx_attribute_class' = 'business');
