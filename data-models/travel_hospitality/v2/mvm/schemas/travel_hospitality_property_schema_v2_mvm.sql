...
-- Generated on: 2026-06-22 19:42:21
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`property` (
    ...
    `phone_number` BIGINT COMMENT 'The main public-facing telephone number for the property in E.164 international format. Used for guest communications, GDS/OTA display, emergency contact, and regulatory filings.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`facility` (
    `facility_id` BIGINT COMMENT 'Unique surrogate identifier for each physical facility or amenity record within the property domain. Primary key for the facility data product.',
    ...
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: A property facility (pool, spa, fitness center) operates within the propertys defined seasonal calendar periods. Linking property_facility to seasonal_calendar allows the data model to associate faci',
    ...
    `operating_hours_close` BIGINT COMMENT 'Standard daily closing time of the facility in HH:MM (24-hour) format. For facilities operating past midnight, this may be 23:59 with a separate overnight flag. Used alongside operating_hours_open for guest-facing display and staffing.',
    `operating_hours_open` BIGINT COMMENT 'Standard daily opening time of the facility in HH:MM (24-hour) format. Used for guest-facing amenity display, operational staffing schedules, and housekeeping preparation windows. For 24-hour facilities, set to 00:00.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`franchise_agreement` (
    ...
    `governing_law_country` BIGINT COMMENT 'ISO 3166-1 alpha-3 country code of the jurisdiction whose laws govern the interpretation and enforcement of this agreement. Critical for cross-border portfolio legal compliance and dispute resolution.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`meeting_space` (
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: Meeting spaces generate F&B catering revenue (minimum_catering_spend, catering_required exist on meeting_space). Linking to the F&B revenue center enables banquet catering revenue attribution, event P',
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: Meeting and event spaces have seasonal demand patterns that align with the propertys seasonal calendar (e.g., peak conference season, holiday blackout periods, reduced-demand shoulder seasons). Linki',
    ...
    `rental_rate_tier` DECIMAL(18,2) COMMENT 'Pricing tier classification for the meeting space rental. Actual rates are managed in rate tables and vary by season, day of week, and booking terms.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`property_outlet` (
    ...
    `fnb_outlet_id` BIGINT COMMENT 'Foreign key linking to fnb.fnb_outlet. Business justification: property_outlet is the property-domain physical/operational record; fnb_outlet is the F&B-domain operational record for the same restaurant or bar. Cross-domain outlet reporting, GDS content syndicati',
    `property_id` BIGINT COMMENT 'Identifier of the property where this outlet is located. Links to the property master data.',
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: Property outlet revenue is posted to an F&B revenue center for USALI P&L reporting and POS reconciliation. Revenue managers and F&B controllers require this link to attribute outlet-level revenue corr',
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: A revenue-generating outlet (restaurant, bar, spa) operates within the propertys seasonal calendar framework. Linking property_outlet to seasonal_calendar connects outlet seasonal operations to the g',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`gds_profile` (
    ...
    `franchise_agreement_id` BIGINT COMMENT 'Foreign key linking to property.franchise_agreement. Business justification: A GDS/CRS property profile is governed by the franchise agreement that defines the brand standards, distribution channel requirements (crs_connectivity_required), and brand codes under which the prope',
    ...
    `fax_number` BIGINT COMMENT 'The fax number for the property as listed in GDS channel profiles. Used by travel agents and corporate accounts for reservation confirmations and group booking correspondence. Classified as confidential organizational contact data.',
    ...
    `phone_number` BIGINT COMMENT 'The primary reservations or front desk phone number for the property as listed in GDS and OTA channels. Used by travel agents and guests to contact the property directly. Classified as confidential organizational contact data.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`seasonal_calendar` (
    ...
    `staffing_level` BIGINT COMMENT 'Planned staffing level for this seasonal period. Drives workforce scheduling and seasonal hiring decisions. Values: full (standard staffing), reduced (lower than standard), minimal (skeleton crew), peak-augmented (additional seasonal staff).',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`property`.`currency` (
    ...
    `exchange_rate_update_frequency` DECIMAL(18,2) COMMENT 'The frequency at which exchange rates are refreshed for this currency. Critical for accurate revenue recognition, dynamic pricing in Revenue Management System (RMS), and financial consolidation under International Financial Reporting Standards (IFRS) and Generally Accepted Accounting Principles (GAAP).',
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ADD CONSTRAINT `fk_property_facility_currency_id` FOREIGN KEY (`currency_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`currency`(`currency_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ADD CONSTRAINT `fk_property_facility_property_id` FOREIGN KEY (`property_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`property`(`property_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ADD CONSTRAINT `fk_property_facility_property_outlet_id` FOREIGN KEY (`property_outlet_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`property_outlet`(`property_outlet_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ADD CONSTRAINT `fk_property_facility_seasonal_calendar_id` FOREIGN KEY (`seasonal_calendar_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`seasonal_calendar`(`seasonal_calendar_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`meeting_space` ADD CONSTRAINT `fk_property_meeting_space_seasonal_calendar_id` FOREIGN KEY (`seasonal_calendar_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`seasonal_calendar`(`seasonal_calendar_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`property_outlet` ADD CONSTRAINT `fk_property_property_outlet_seasonal_calendar_id` FOREIGN KEY (`seasonal_calendar_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`seasonal_calendar`(`seasonal_calendar_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`gds_profile` ADD CONSTRAINT `fk_property_gds_profile_franchise_agreement_id` FOREIGN KEY (`franchise_agreement_id`) REFERENCES `vibe_travel_hospitality_v1`.`property`.`franchise_agreement`(`franchise_agreement_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`property` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`hierarchy` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` SET TAGS ('dbx_subdomain' = 'venue_operations');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `currency_id` SET TAGS ('dbx_business_glossary_term' = 'Currency Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `property_id` SET TAGS ('dbx_business_glossary_term' = 'Property ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `property_outlet_id` SET TAGS ('dbx_business_glossary_term' = 'Property Outlet Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `ada_features` SET TAGS ('dbx_business_glossary_term' = 'ADA (Americans with Disabilities Act) Accessibility Features');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `age_restriction` SET TAGS ('dbx_business_glossary_term' = 'Age Restriction Policy');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `age_restriction` SET TAGS ('dbx_value_regex' = 'none|adults_only|minimum_age_18|minimum_age_16|children_only');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `area_sqft` SET TAGS ('dbx_business_glossary_term' = 'Facility Area (Square Feet)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `av_equipment_available` SET TAGS ('dbx_business_glossary_term' = 'Audio-Visual (AV) Equipment Available Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `building_wing` SET TAGS ('dbx_business_glossary_term' = 'Building Wing or Zone');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `capacity` SET TAGS ('dbx_business_glossary_term' = 'Facility Capacity');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_category` SET TAGS ('dbx_business_glossary_term' = 'Facility Category');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_code` SET TAGS ('dbx_business_glossary_term' = 'Facility Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_-]{2,20}$');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `dress_code` SET TAGS ('dbx_business_glossary_term' = 'Facility Dress Code');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `dress_code` SET TAGS ('dbx_value_regex' = 'none|smart_casual|formal|resort_casual|swimwear_only');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_type` SET TAGS ('dbx_business_glossary_term' = 'Facility Type');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `floor_number` SET TAGS ('dbx_business_glossary_term' = 'Floor Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `inspection_result` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Result');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `inspection_result` SET TAGS ('dbx_value_regex' = 'pass|pass_with_conditions|fail|pending');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `is_24_hour` SET TAGS ('dbx_business_glossary_term' = '24-Hour Operation Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `is_ada_compliant` SET TAGS ('dbx_business_glossary_term' = 'ADA (Americans with Disabilities Act) Compliance Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `is_fee_based` SET TAGS ('dbx_business_glossary_term' = 'Fee-Based Facility Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `is_reservation_required` SET TAGS ('dbx_business_glossary_term' = 'Reservation Required Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `is_seasonal` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Availability Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `last_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `license_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Facility License Expiry Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `license_number` SET TAGS ('dbx_business_glossary_term' = 'Facility Operating License Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `license_number` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `max_occupancy_pct` SET TAGS ('dbx_business_glossary_term' = 'Maximum Occupancy Percentage');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_business_glossary_term' = 'Facility Name');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `facility_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `natural_light` SET TAGS ('dbx_business_glossary_term' = 'Natural Light Available Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `next_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Next Scheduled Inspection Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `operating_hours_close` SET TAGS ('dbx_business_glossary_term' = 'Operating Hours Closing Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `operating_hours_close` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `operating_hours_open` SET TAGS ('dbx_business_glossary_term' = 'Operating Hours Opening Time');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `operating_hours_open` SET TAGS ('dbx_typed' = 'numeric_correction');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `outdoor_indoor` SET TAGS ('dbx_business_glossary_term' = 'Indoor/Outdoor Classification');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `outdoor_indoor` SET TAGS ('dbx_value_regex' = 'indoor|outdoor|semi_outdoor');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `property_facility_description` SET TAGS ('dbx_business_glossary_term' = 'Facility Description');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `property_facility_status` SET TAGS ('dbx_business_glossary_term' = 'Facility Operational Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `property_facility_status` SET TAGS ('dbx_value_regex' = 'active|inactive|seasonal|under_renovation|closed|pending_opening');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `renovation_end_date` SET TAGS ('dbx_business_glossary_term' = 'Renovation End Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `renovation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Renovation Start Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `renovation_status` SET TAGS ('dbx_business_glossary_term' = 'Renovation Status');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `renovation_status` SET TAGS ('dbx_value_regex' = 'not_scheduled|planned|in_progress|completed');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `seasonal_close_date` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Closing Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `seasonal_open_date` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Opening Date');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `seating_configuration` SET TAGS ('dbx_business_glossary_term' = 'Seating Configuration');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `smoking_policy` SET TAGS ('dbx_business_glossary_term' = 'Facility Smoking Policy');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `smoking_policy` SET TAGS ('dbx_value_regex' = 'non_smoking|smoking_permitted|designated_area');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `sort_order` SET TAGS ('dbx_business_glossary_term' = 'Display Sort Order');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`facility` ALTER COLUMN `usage_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Facility Usage Fee Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`franchise_agreement` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`franchise_agreement` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`meeting_space` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`meeting_space` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`property_outlet` ALTER COLUMN `fnb_outlet_id` SET TAGS ('dbx_business_glossary_term' = 'Fnb Outlet Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`property_outlet` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`property_outlet` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`gds_profile` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`gds_profile` ALTER COLUMN `franchise_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Franchise Agreement Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`seasonal_calendar` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`property`.`currency` SET TAGS ('dbx_subdomain' = 'portfolio_identity');
