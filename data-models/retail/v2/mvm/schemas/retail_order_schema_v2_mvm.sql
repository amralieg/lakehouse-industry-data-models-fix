...
-- Generated on: 2026-06-24 00:49:12
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`header` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: Order billing address normalization: linking to customer.address enforces referential integrity, enables address-level order history reporting, and eliminates denormalized flat billing address columns',
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: Order confirmation and shipping notification delivery should use the customers verified, consent-managed contact record. header.customer_email and customer_phone are denormalized. Linking to customer',
    ...
    `marketplace_listing_id` BIGINT COMMENT 'Foreign key linking to ecommerce.marketplace_listing. Business justification: Marketplace order attribution: orders originating from marketplace listings must link to the specific listing for marketplace fee reconciliation, sell-through rate calculation, buy-box performance ana',
    `price_list_id` BIGINT COMMENT 'Foreign key linking to pricing.price_list. Business justification: Order headers are fulfilled under a specific price list (e.g., wholesale, loyalty tier, regional). Retail margin reporting and order audit require knowing which price list governed the entire order. N',
    ...
    `subscription_id` BIGINT COMMENT 'Foreign key linking to order.subscription. Business justification: Subscription orders (subscribe-and-save, auto-replenishment) generate recurring order headers on each delivery cycle. The header table currently has no FK back to the subscription that generated it, m',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`order_line` (
    ...
    `digital_catalog_id` BIGINT COMMENT 'Foreign key linking to ecommerce.digital_catalog. Business justification: Ecommerce order attribution: retailers must trace which digital catalog listing (product page, content version, SEO entry) drove each order line purchase. Required for catalog content performance repo',
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supplier.vendor. Business justification: Direct Ship Vendor (DSV) fulfillment: in retail, drop-ship order lines are routed to a specific vendor for direct-to-consumer fulfillment. This FK enables EDI order transmission, vendor performance tr',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`pos_transaction` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: B2B in-store purchases on account (net-30 terms, employee discount accounts, corporate charge accounts) require linking POS transactions to customer accounts. pos_transaction links to profile but not ',
    ...
    `price_list_id` BIGINT COMMENT 'Foreign key linking to pricing.price_list. Business justification: POS transactions execute under a specific price list (regular, employee, clearance). Stamping the active price list on the transaction enables price list performance reporting, in-store pricing audit,',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`pos_transaction_line` (
    ...
    `coupon_id` BIGINT COMMENT 'Foreign key linking to promotion.coupon. Business justification: pos_transaction_line.coupon_code is a denormalized coupon reference. A proper FK to promotion.coupon enables coupon redemption reconciliation, single-use coupon enforcement, and vendor-funded coupon c',
    `department_id` BIGINT COMMENT 'Foreign key linking to store.department. Business justification: Department-level POS sales reporting, shrinkage tracking, and licensed-department regulatory compliance all require knowing which store department each transaction line belongs to. A retail domain exp',
    `markdown_id` BIGINT COMMENT 'Foreign key linking to pricing.markdown. Business justification: POS line items sold at marked-down prices must reference the markdown event for sell-through reporting (units sold per markdown event) — a core retail KPI. The existing sku_price_id path does not dire',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: In-store POS line-item promotional reporting requires linking each sold line to the specific promo offer applied. Enables offer-level redemption analytics, vendor chargeback calculations, and promotio',
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to product.uom. Business justification: Retail POS sells items by weight (lb/kg), volume, or each — requiring UOM master reference for variable-measure items, scale-integrated POS, and cross-channel UOM-consistent sales reporting. order_lin',
    `vendor_item_id` BIGINT COMMENT 'Foreign key linking to supplier.vendor_item. Business justification: DSD (Direct Store Delivery) cost reconciliation and chargeback processing: POS transaction lines for DSD items must reference the specific vendor item record to validate unit cost, pack configuration,',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`payment` (
    ...
    `payment_method_id` DECIMAL(18,2) COMMENT 'Foreign key linking to customer.payment_method. Business justification: Payment reconciliation, chargeback management, and tokenized card lifecycle tracking require linking captured order payments to the customers stored payment method. Enables all orders charged to thi',
    `pos_transaction_id` BIGINT COMMENT 'Foreign key linking to order.pos_transaction. Business justification: A payment tender record in a retail POS environment is directly associated with a specific POS transaction. While payment.header_id links to the order header, POS transactions are the immediate transa',
    `shipment_id` BIGINT COMMENT 'Foreign key linking to fulfillment.shipment. Business justification: Ship-and-capture payment model: retail payments are authorized at order placement and captured on shipment. Linking payment to shipment enables automated capture triggers, financial reconciliation of ',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`discount` (
    ...
    `coupon_id` BIGINT COMMENT 'Foreign key linking to promotion.coupon. Business justification: discount.discount_code is a denormalized coupon reference when the discount is coupon-driven. A FK to promotion.coupon enables coupon-driven discount reconciliation, single-use coupon validation, and ',
    ...
    `markdown_id` BIGINT COMMENT 'Foreign key linking to pricing.markdown. Business justification: Discounts applied at order or POS level may originate from a markdown event. Linking discount to the source markdown enables markdown sell-through tracking and markdown effectiveness reporting — core ',
    ...
    `pos_transaction_id` BIGINT COMMENT 'Foreign key linking to order.pos_transaction. Business justification: Discounts applied during POS transactions need a direct link to the POS transaction record. The discount table already has header_id (for order-level context) and order_line_id (for e-commerce line-le',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: discount already links to promo_campaign but not the specific promo_offer that drove it. Offer-level discount attribution is required for vendor chargeback calculations, promotional P&L reporting, and',
    `promotion_banner_id` BIGINT COMMENT 'Foreign key linking to ecommerce.promotion_banner. Business justification: Marketing ROI reporting: retailers attribute applied discounts to the specific promotion banner that drove the conversion. Required for banner click-to-discount conversion analysis, campaign spend jus',
    `rebate_id` BIGINT COMMENT 'Foreign key linking to promotion.rebate. Business justification: Rebate programs generate discount records at point of sale or post-purchase. Linking discount to rebate enables rebate accrual reconciliation against actual discounts applied, vendor rebate settlement',
    `rule_id` BIGINT COMMENT 'Foreign key linking to pricing.rule. Business justification: Discounts applied to orders are generated by pricing rules. Linking discount to the triggering pricing rule enables rule effectiveness reporting, audit of rule-driven discounts, and compliance validat',
    `vendor_allowance_id` BIGINT COMMENT 'Foreign key linking to supplier.vendor_allowance. Business justification: Vendor-funded discount accrual reconciliation: retail finance tracks which vendor allowance authorized each applied discount for co-op billing and accrual accounting. Linking discount records to the a',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`cancellation` (
    ...
    `fulfillment_order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: Order cancellation workflow: cancellations must directly reference the fulfillment order to trigger WMS cancellation signals, halt pick/pack tasks, and track fulfillment cancellation costs. Retail OMS',
    ...
    `pos_transaction_id` BIGINT COMMENT 'Foreign key linking to order.pos_transaction. Business justification: In-store cancellations and returns are initiated at the POS terminal and are directly tied to a specific POS transaction. The cancellation table has header_id and order_line_id for e-commerce cancella',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`promise` (
    ...
    `inbound_shipment_id` BIGINT COMMENT 'Foreign key linking to supplychain.inbound_shipment. Business justification: Available-to-Promise (ATP) calculation: delivery promises depend on inbound shipment arrival dates to determine when stock will be available. Retail ATP engines link promises to inbound shipments to c',
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: Delivery promise accuracy analysis by address/region requires linking promise records to the customer address used for SLA calculation. Eliminates denormalized postal_code and country_code columns. En',
    `location_id` BIGINT COMMENT 'Foreign key linking to store.location. Business justification: BOPIS/ROPIS promise management requires associating each delivery promise with the specific store location where pickup is promised. Store-level SLA breach reporting and omnichannel promise accuracy K',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`gift_card` (
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Physical and digital gift cards are sold as SKUs with UPCs, shelf placement, and inventory tracking. Linking gift_card to the product SKU enables gift card sales reporting by product, inventory deplet',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`gift_card_transaction` (
    ...
    `payment_id` BIGINT COMMENT 'Foreign key linking to order.payment. Business justification: When a gift card is redeemed as a payment instrument, both a payment record (capturing the tender) and a gift_card_transaction record (capturing the balance change) are created. These two records repr',
    ...
    `storefront_id` BIGINT COMMENT 'Foreign key linking to ecommerce.storefront. Business justification: Channel-level gift card liability and fraud monitoring: gift card activations, redemptions, and reloads on ecommerce storefronts must be attributed to the specific storefront for per-channel liability',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`order`.`subscription` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: B2B/corporate account subscription management requires linking subscriptions to customer accounts for credit limit enforcement, account status checks, and account-level subscription spend reporting. s',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: subscription.promotion_code is a denormalized promo offer reference. Subscriptions are frequently acquired via promotional offers (e.g., first 3 months free). A FK to promo_offer enables subscriptio',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Subscribe-and-save / auto-replenishment subscriptions are tied to a specific SKU. Without sku_id on subscription, subscription revenue by product, SKU discontinuation impact on active subscriptions, a',
...
ALTER TABLE `vibe_retail_v1`.`order`.`cancellation` ADD CONSTRAINT `fk_order_cancellation_pos_transaction_id` FOREIGN KEY (`pos_transaction_id`) REFERENCES `vibe_retail_v1`.`order`.`pos_transaction`(`pos_transaction_id`);
...
ALTER TABLE `vibe_retail_v1`.`order`.`header` SET TAGS ('dbx_subdomain' = 'order_capture');
...
ALTER TABLE `vibe_retail_v1`.`order`.`header` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Address Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`header` ALTER COLUMN `marketplace_listing_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Listing Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`order`.`header` ALTER COLUMN `price_list_id` SET TAGS ('dbx_business_glossary_term' = 'Price List Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`order_line` SET TAGS ('dbx_subdomain' = 'order_capture');
...
ALTER TABLE `vibe_retail_v1`.`order`.`order_line` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`pos_transaction` SET TAGS ('dbx_subdomain' = 'order_capture');
...
ALTER TABLE `vibe_retail_v1`.`order`.`pos_transaction` ALTER COLUMN `price_list_id` SET TAGS ('dbx_business_glossary_term' = 'Price List Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`pos_transaction_line` SET TAGS ('dbx_subdomain' = 'order_capture');
...
ALTER TABLE `vibe_retail_v1`.`order`.`pos_transaction_line` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Uom Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`order`.`pos_transaction_line` ALTER COLUMN `vendor_item_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Item Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`payment` SET TAGS ('dbx_subdomain' = 'payment_fulfillment');
...
ALTER TABLE `vibe_retail_v1`.`order`.`payment` ALTER COLUMN `payment_method_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Method Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`discount` SET TAGS ('dbx_subdomain' = 'payment_fulfillment');
...
ALTER TABLE `vibe_retail_v1`.`order`.`discount` ALTER COLUMN `coupon_id` SET TAGS ('dbx_business_glossary_term' = 'Coupon Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`discount` ALTER COLUMN `markdown_id` SET TAGS ('dbx_business_glossary_term' = 'Markdown Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`discount` ALTER COLUMN `promotion_banner_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Banner Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`order`.`discount` ALTER COLUMN `rebate_id` SET TAGS ('dbx_business_glossary_term' = 'Rebate Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`cancellation` SET TAGS ('dbx_subdomain' = 'payment_fulfillment');
...
ALTER TABLE `vibe_retail_v1`.`order`.`cancellation` ALTER COLUMN `pos_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Pos Transaction Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`promise` SET TAGS ('dbx_subdomain' = 'payment_fulfillment');
...
ALTER TABLE `vibe_retail_v1`.`order`.`promise` ALTER COLUMN `inbound_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Shipment Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`promise` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`order`.`promise` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_retail_v1`.`order`.`promise` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_retail_v1`.`order`.`promise` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store Location Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`gift_card` SET TAGS ('dbx_subdomain' = 'subscription_loyalty');
...
ALTER TABLE `vibe_retail_v1`.`order`.`gift_card` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`gift_card_transaction` SET TAGS ('dbx_subdomain' = 'subscription_loyalty');
...
ALTER TABLE `vibe_retail_v1`.`order`.`gift_card_transaction` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`order`.`subscription` SET TAGS ('dbx_subdomain' = 'subscription_loyalty');
...
ALTER TABLE `vibe_retail_v1`.`order`.`subscription` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
