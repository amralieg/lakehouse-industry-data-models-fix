...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`target_population` (
    ...
    `community_id` BIGINT COMMENT 'add column community_id (BIGINT) with FK to beneficiary.community.community_id - target populations are defined in terms of communities',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`program_amendment` (
    ...
    `grant_amendment_id` BIGINT COMMENT 'FK to SSOT owner grant.grant_amendment for entity amendment',
    ...
    `number` STRING COMMENT 'Sequential or coded identifier for this amendment within the program (e.g., AMD-001, Amendment 1).',
    ...
    `program_amendment_status` STRING COMMENT 'Current lifecycle state of the amendment: draft (being prepared), submitted (sent for approval), under review (being evaluated), approved (accepted and effective), rejected (denied), or withdrawn (cancelled by submitter).. Valid values are `draft|submitted|under_review|approved|rejected|withdrawn`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`design_assessment` (
    ...
    `staff_member_id` BIGINT COMMENT 'add column staff_member_id (BIGINT) with FK to workforce.staff_member.staff_member_id - design assessments need an assessor/owner',
    `program_id` BIGINT COMMENT '',
    ...
    `design_assessment_code` STRING COMMENT 'Externally-known unique code or identifier for this design assessment, used in reports and donor communications.. Valid values are `^[A-Z0-9]{6,20}$`',
    ...
    `team_lead` STRING COMMENT 'Name or identifier of the individual or organization leading the assessment team.',
    `team_size` STRING COMMENT 'Number of enumerators, surveyors, or field staff involved in conducting the assessment.',
    `title` STRING COMMENT 'Descriptive title of the design assessment (e.g., Rapid Nutrition Assessment - Northern Province).',
    `design_assessment_type` STRING COMMENT 'Classification of the assessment approach: rapid (quick situational), comprehensive (in-depth multi-sector), sectoral (single sector focus), baseline (pre-intervention), midline (mid-intervention), or endline (post-intervention).. Valid values are `rapid|comprehensive|sectoral|baseline|midline|endline`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`program_closeout` (
    ...
    `grant_closeout_id` BIGINT COMMENT 'FK to SSOT owner grant.grant_closeout for entity closeout',
    ...
    `number` STRING COMMENT 'Externally-known unique identifier for this closeout process, used in donor communications and compliance reporting.',
    ...
    `program_closeout_status` STRING COMMENT 'Current lifecycle status of the closeout process: initiated, in progress, pending donor review, donor approved, completed, or archived.. Valid values are `initiated|in_progress|pending_donor_review|donor_approved|completed|archived`',
    `program_closeout_type` STRING COMMENT 'Classification of the closeout reason: planned completion, early termination, no-cost extension expiry, emergency suspension, donor-initiated closure, or force majeure event.. Valid values are `planned|early_termination|no_cost_extension_expiry|emergency_suspension|donor_initiated|force_majeure`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`review_event` (
    ...
    `review_event_code` STRING COMMENT 'Business identifier or reference code for the review event, often used in donor correspondence and internal tracking systems.. Valid values are `^[A-Z0-9_-]{3,20}$`',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`program_partnership` (
    ...
    `role` STRING COMMENT 'The specific role this partner plays in the intervention (e.g., Lead Implementer, Sub-Recipient, Technical Partner, Local CBO, Consortium Member). Defines the partners primary responsibility and authority level.',
    ...
    `program_partnership_status` STRING COMMENT 'Current lifecycle status of this specific partnership arrangement within the intervention. Tracks whether the partner is actively implementing, suspended due to performance issues, or completed their scope of work.',
    ...
    `program_partnership_type` STRING COMMENT 'Classification of the partnership arrangement type (e.g., Direct Implementation, Sub-Award, Technical Assistance, Capacity Building, Coordination). Determines reporting and compliance requirements.',
...
CREATE OR REPLACE TABLE `vibe_ngo_v1`.`program`.`component_system_usage` (
    ...
    `component_system_usage_status` STRING COMMENT 'Current status of this component-system usage relationship (Active, Inactive, Pilot, Sunset, Decommissioned). Tracks the lifecycle of the usage relationship independently from the component or system status.',
...
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ADD CONSTRAINT `fk_program_theory_of_change_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ADD CONSTRAINT `fk_program_program_amendment_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ADD CONSTRAINT `fk_program_design_assessment_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ADD CONSTRAINT `fk_program_program_closeout_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ADD CONSTRAINT `fk_program_review_event_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ADD CONSTRAINT `fk_program_program_partnership_program_id` FOREIGN KEY (`program_id`) REFERENCES `vibe_ngo_v1`.`program`.`program`(`program_id`);
...
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` SET TAGS ('dbx_subdomain' = 'intervention_design');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Implementing Organizational Unit (Org Unit) ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'Primary System Platform Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Program Manager ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` SET TAGS ('dbx_subdomain' = 'intervention_design');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `chs_self_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `gender_integration_approach` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`theory_of_change` ALTER COLUMN `stakeholder_engagement_approach` SET TAGS ('dbx_pii_demographic' = 'true');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_logframe` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_logframe` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_logframe` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_logframe` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_logframe` ALTER COLUMN `chs_self_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`logframe_row` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`component` SET TAGS ('dbx_subdomain' = 'intervention_design');
ALTER TABLE `vibe_ngo_v1`.`program`.`component` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`component` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Finding Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`component` ALTER COLUMN `donor_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`component` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`component` ALTER COLUMN `it_project_id` SET TAGS ('dbx_business_glossary_term' = 'It Project Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`target_population` SET TAGS ('dbx_subdomain' = 'intervention_design');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`target_population` ALTER COLUMN `chs_self_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Chs Self Assessment Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `program_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Program Amendment Identifier (ID)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Award Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program Identifier (ID)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Amendment Approval Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `audit_trail_notes` SET TAGS ('dbx_business_glossary_term' = 'Audit Trail Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `budget_change_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Change Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `budget_change_currency` SET TAGS ('dbx_business_glossary_term' = 'Budget Change Currency Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `budget_change_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `compliance_impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Compliance Impact Assessment');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Amendment Record Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `donor_approval_reference` SET TAGS ('dbx_business_glossary_term' = 'Donor Approval Reference Number');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `donor_approving_authority` SET TAGS ('dbx_business_glossary_term' = 'Donor Approving Authority');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Amendment Effective Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `geographic_coverage_change` SET TAGS ('dbx_business_glossary_term' = 'Geographic Coverage Change Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `geographic_coverage_change` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `grant_requirement_flag` SET TAGS ('dbx_business_glossary_term' = 'Grant Requirement Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `internal_approving_authority` SET TAGS ('dbx_business_glossary_term' = 'Internal Approving Authority');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `justification_narrative` SET TAGS ('dbx_business_glossary_term' = 'Amendment Justification Narrative');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Amendment Last Modified By User');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Amendment Record Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `logframe_revision_flag` SET TAGS ('dbx_business_glossary_term' = 'Logical Framework (LogFrame) Revision Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `logframe_revision_summary` SET TAGS ('dbx_business_glossary_term' = 'Logical Framework (LogFrame) Revision Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `number` SET TAGS ('dbx_business_glossary_term' = 'Amendment Number');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `original_end_date` SET TAGS ('dbx_business_glossary_term' = 'Original Program End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `original_target_beneficiaries` SET TAGS ('dbx_business_glossary_term' = 'Original Target Beneficiaries Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Amendment Rejection Reason');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `revised_end_date` SET TAGS ('dbx_business_glossary_term' = 'Revised Program End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `revised_target_beneficiaries` SET TAGS ('dbx_business_glossary_term' = 'Revised Target Beneficiaries Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `risk_assessment` SET TAGS ('dbx_business_glossary_term' = 'Amendment Risk Assessment');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `scope_change_description` SET TAGS ('dbx_business_glossary_term' = 'Scope Change Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `sdg_alignment_change` SET TAGS ('dbx_business_glossary_term' = 'Sustainable Development Goal (SDG) Alignment Change');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `sector_classification_change` SET TAGS ('dbx_business_glossary_term' = 'Sector Classification Change Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `stakeholder_consultation_summary` SET TAGS ('dbx_business_glossary_term' = 'Stakeholder Consultation Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `program_amendment_status` SET TAGS ('dbx_business_glossary_term' = 'Amendment Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `program_amendment_status` SET TAGS ('dbx_value_regex' = 'draft|submitted|under_review|approved|rejected|withdrawn');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Amendment Submission Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `supporting_documentation_reference` SET TAGS ('dbx_business_glossary_term' = 'Supporting Documentation Reference');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `target_population_change` SET TAGS ('dbx_business_glossary_term' = 'Target Population Change Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `timeline_extension_days` SET TAGS ('dbx_business_glossary_term' = 'Timeline Extension Days');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `program_amendment_type` SET TAGS ('dbx_business_glossary_term' = 'Amendment Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `program_amendment_type` SET TAGS ('dbx_value_regex' = 'no_cost_extension|scope_change|budget_reallocation|geographic_expansion|target_population_adjustment|logframe_revision');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_amendment` ALTER COLUMN `withdrawal_reason` SET TAGS ('dbx_business_glossary_term' = 'Amendment Withdrawal Reason');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` SET TAGS ('dbx_subdomain' = 'intervention_design');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Design Assessment ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `admin_level_1` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 1');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `admin_level_2` SET TAGS ('dbx_business_glossary_term' = 'Administrative Level 2');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_code` SET TAGS ('dbx_business_glossary_term' = 'Assessment Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `dac_sector_code` SET TAGS ('dbx_business_glossary_term' = 'Development Assistance Committee (DAC) Sector Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `dac_sector_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `data_collection_end_date` SET TAGS ('dbx_business_glossary_term' = 'Data Collection End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `data_collection_start_date` SET TAGS ('dbx_business_glossary_term' = 'Data Collection Start Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `data_sharing_restrictions` SET TAGS ('dbx_business_glossary_term' = 'Data Sharing Restrictions');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `donor_visibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Donor Visibility Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `ethical_approval_obtained` SET TAGS ('dbx_business_glossary_term' = 'Ethical Approval Obtained');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `gam_prevalence_percent` SET TAGS ('dbx_business_glossary_term' = 'Global Acute Malnutrition (GAM) Prevalence Percent');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `geographic_scope` SET TAGS ('dbx_business_glossary_term' = 'Geographic Scope');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `humanitarian_principles_compliance` SET TAGS ('dbx_business_glossary_term' = 'Humanitarian Principles Compliance');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `humanitarian_principles_compliance` SET TAGS ('dbx_value_regex' = 'full|partial|not_applicable');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `key_findings_summary` SET TAGS ('dbx_business_glossary_term' = 'Key Findings Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `methodology` SET TAGS ('dbx_business_glossary_term' = 'Methodology');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `population_assessed` SET TAGS ('dbx_business_glossary_term' = 'Population Assessed');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `priority_needs_ranking` SET TAGS ('dbx_business_glossary_term' = 'Priority Needs Ranking');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `protection_risks_identified` SET TAGS ('dbx_business_glossary_term' = 'Protection Risks Identified');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `report_url` SET TAGS ('dbx_business_glossary_term' = 'Report Uniform Resource Locator (URL)');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `sam_prevalence_percent` SET TAGS ('dbx_business_glossary_term' = 'Severe Acute Malnutrition (SAM) Prevalence Percent');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `sdg_alignment` SET TAGS ('dbx_business_glossary_term' = 'Sustainable Development Goal (SDG) Alignment');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `sector_focus` SET TAGS ('dbx_business_glossary_term' = 'Sector Focus');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|validated|archived');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `target_population_description` SET TAGS ('dbx_business_glossary_term' = 'Target Population Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `team_lead` SET TAGS ('dbx_business_glossary_term' = 'Assessment Team Lead');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `team_size` SET TAGS ('dbx_business_glossary_term' = 'Assessment Team Size');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Assessment Title');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `design_assessment_type` SET TAGS ('dbx_value_regex' = 'rapid|comprehensive|sectoral|baseline|midline|endline');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `validated_by` SET TAGS ('dbx_business_glossary_term' = 'Validated By');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `validation_date` SET TAGS ('dbx_business_glossary_term' = 'Validation Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `wash_coverage_gap_percent` SET TAGS ('dbx_business_glossary_term' = 'Water Sanitation and Hygiene (WASH) Coverage Gap Percent');
ALTER TABLE `vibe_ngo_v1`.`program`.`design_assessment` ALTER COLUMN `wash_coverage_gap_percent` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`risk_register` ALTER COLUMN `compliance_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Incident Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `donor_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Donor Compliance Req Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `it_project_id` SET TAGS ('dbx_business_glossary_term' = 'It Project Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`implementation_plan` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'System Platform Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `budget_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `donor_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `it_project_id` SET TAGS ('dbx_business_glossary_term' = 'It Project Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan` ALTER COLUMN `other_direct_costs` SET TAGS ('dbx_business_glossary_term' = 'Other Direct Costs (ODC)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` SET TAGS ('dbx_subdomain' = 'planning_monitoring');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `budget_line_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Line Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `donor_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`budget_plan_line` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'System Platform Id (Foreign Key)');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_closeout_id` SET TAGS ('dbx_business_glossary_term' = 'Program Closeout ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Award Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Certified By Staff Member Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `archiving_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Archiving Completion Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `archiving_reference` SET TAGS ('dbx_business_glossary_term' = 'Archiving Reference');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `asset_disposition_plan` SET TAGS ('dbx_business_glossary_term' = 'Asset Disposition Plan');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `asset_disposition_status` SET TAGS ('dbx_business_glossary_term' = 'Asset Disposition Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `asset_disposition_status` SET TAGS ('dbx_value_regex' = 'pending|in_progress|completed|donor_approval_required|approved');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `audit_completion_status` SET TAGS ('dbx_business_glossary_term' = 'Audit Completion Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `audit_completion_status` SET TAGS ('dbx_value_regex' = 'not_required|pending|in_progress|completed|findings_issued|resolved');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `audit_findings_summary` SET TAGS ('dbx_business_glossary_term' = 'Audit Findings Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `budget_utilization_percentage` SET TAGS ('dbx_business_glossary_term' = 'Budget Utilization Percentage');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `budget_utilization_percentage` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `budget_variance_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Variance Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `completion_date` SET TAGS ('dbx_business_glossary_term' = 'Closeout Completion Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `compliance_certification_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Certification Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `compliance_certification_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Certification Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `donor_signoff_conditions` SET TAGS ('dbx_business_glossary_term' = 'Donor Sign-Off Conditions');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `donor_signoff_date` SET TAGS ('dbx_business_glossary_term' = 'Donor Sign-Off Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `donor_signoff_status` SET TAGS ('dbx_business_glossary_term' = 'Donor Sign-Off Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `donor_signoff_status` SET TAGS ('dbx_value_regex' = 'pending|submitted|under_review|approved|conditional_approval|rejected');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_beneficiary_count` SET TAGS ('dbx_business_glossary_term' = 'Final Beneficiary Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_beneficiary_count_female` SET TAGS ('dbx_business_glossary_term' = 'Final Beneficiary Count Female');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_beneficiary_count_male` SET TAGS ('dbx_business_glossary_term' = 'Final Beneficiary Count Male');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_beneficiary_count_other` SET TAGS ('dbx_business_glossary_term' = 'Final Beneficiary Count Other');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Final Budget Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_expenditure_amount` SET TAGS ('dbx_business_glossary_term' = 'Final Expenditure Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_financial_reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Final Financial Reconciliation Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_financial_reconciliation_status` SET TAGS ('dbx_value_regex' = 'pending|in_progress|reconciled|discrepancies_identified|resolved|audited');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_report_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Final Report Approval Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `final_report_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Final Report Submission Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `initiation_date` SET TAGS ('dbx_business_glossary_term' = 'Closeout Initiation Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `knowledge_management_outputs` SET TAGS ('dbx_business_glossary_term' = 'Knowledge Management Outputs');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `knowledge_management_outputs` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `lessons_learned_summary` SET TAGS ('dbx_business_glossary_term' = 'Lessons Learned Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Closeout Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `number` SET TAGS ('dbx_business_glossary_term' = 'Closeout Number');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `outstanding_obligations_description` SET TAGS ('dbx_business_glossary_term' = 'Outstanding Obligations Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `outstanding_obligations_flag` SET TAGS ('dbx_business_glossary_term' = 'Outstanding Obligations Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_end_date` SET TAGS ('dbx_business_glossary_term' = 'Program End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `record_retention_end_date` SET TAGS ('dbx_business_glossary_term' = 'Record Retention End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_closeout_status` SET TAGS ('dbx_business_glossary_term' = 'Closeout Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_closeout_status` SET TAGS ('dbx_value_regex' = 'initiated|in_progress|pending_donor_review|donor_approved|completed|archived');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_closeout_type` SET TAGS ('dbx_business_glossary_term' = 'Closeout Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_closeout` ALTER COLUMN `program_closeout_type` SET TAGS ('dbx_value_regex' = 'planned|early_termination|no_cost_extension_expiry|emergency_suspension|donor_initiated|force_majeure');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partner_linkage_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Linkage Identifier (ID)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program Identifier (ID)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `constituent_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Constituent Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Organization Identifier (ID)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partner_psea_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Psea Assessment Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Staff Member Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `sanctions_screening_id` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Screening Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `beneficiary_reached_count` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Reached Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `beneficiary_target_count` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Target Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `budget_allocated_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Allocated Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `budget_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Currency Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `budget_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `capacity_assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `capacity_assessment_score` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Score');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `capacity_assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Capacity Assessment Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `capacity_assessment_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|approved|conditional_approval|failed');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `capacity_building_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Capacity Building Required Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `community_based_organization_flag` SET TAGS ('dbx_business_glossary_term' = 'Community-Based Organization (CBO) Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|conditional|under_review|not_assessed');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Partnership End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `geographic_scope` SET TAGS ('dbx_business_glossary_term' = 'Geographic Scope of Partnership');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `last_monitoring_visit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Monitoring Visit Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `last_report_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Last Report Submission Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `local_partner_flag` SET TAGS ('dbx_business_glossary_term' = 'Local Partner Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `monitoring_visit_count` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Visit Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Partnership Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_role` SET TAGS ('dbx_business_glossary_term' = 'Partnership Role');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_role` SET TAGS ('dbx_value_regex' = 'lead_implementer|sub_grantee|technical_partner|government_counterpart|consortium_member|capacity_building_partner');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_status` SET TAGS ('dbx_business_glossary_term' = 'Partnership Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_status` SET TAGS ('dbx_value_regex' = 'active|pending|suspended|terminated|completed|under_review');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_type` SET TAGS ('dbx_business_glossary_term' = 'Partnership Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `partnership_type` SET TAGS ('dbx_value_regex' = 'direct_implementation|sub_award|technical_assistance|coordination|joint_implementation|government_collaboration');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `performance_rating` SET TAGS ('dbx_business_glossary_term' = 'Partner Performance Rating');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `performance_rating` SET TAGS ('dbx_value_regex' = 'outstanding|exceeds_expectations|meets_expectations|needs_improvement|unsatisfactory|not_rated');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `performance_review_date` SET TAGS ('dbx_business_glossary_term' = 'Performance Review Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `reporting_frequency` SET TAGS ('dbx_business_glossary_term' = 'Reporting Frequency');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Partnership Risk Rating');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `sector_focus` SET TAGS ('dbx_business_glossary_term' = 'Sector Focus');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Partnership Start Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `sub_award_reference` SET TAGS ('dbx_business_glossary_term' = 'Sub-Award Reference Number');
ALTER TABLE `vibe_ngo_v1`.`program`.`partner_linkage` ALTER COLUMN `termination_reason` SET TAGS ('dbx_business_glossary_term' = 'Partnership Termination Reason');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_event_id` SET TAGS ('dbx_business_glossary_term' = 'Review Event ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Program ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `partnership_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'Reporting System Platform Id (Foreign Key)');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `approved_by_name` SET TAGS ('dbx_business_glossary_term' = 'Approved By Name');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `approved_by_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `beneficiary_reach_children` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Reach Children');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `beneficiary_reach_female` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Reach Female');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `beneficiary_reach_male` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Reach Male');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `beneficiary_reach_total` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Reach Total');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `challenges_narrative` SET TAGS ('dbx_business_glossary_term' = 'Challenges Narrative');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `chs_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Core Humanitarian Standard (CHS) Compliance Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `cluster_submission_flag` SET TAGS ('dbx_business_glossary_term' = 'Cluster Submission Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_event_code` SET TAGS ('dbx_business_glossary_term' = 'Review Event Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_event_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_-]{3,20}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `dissemination_date` SET TAGS ('dbx_business_glossary_term' = 'Dissemination Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `document_url` SET TAGS ('dbx_business_glossary_term' = 'Document URL');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `donor_visibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Donor Visibility Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `executive_summary` SET TAGS ('dbx_business_glossary_term' = 'Executive Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `financial_summary_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Summary Budget Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `financial_summary_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Financial Summary Currency Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `financial_summary_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `financial_summary_expenditure_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Summary Expenditure Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `follow_up_actions` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Actions');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `key_findings` SET TAGS ('dbx_business_glossary_term' = 'Key Findings');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `lead_author_name` SET TAGS ('dbx_business_glossary_term' = 'Lead Author Name');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `lead_author_name` SET TAGS ('dbx_pii_personal' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `lessons_learned_summary` SET TAGS ('dbx_business_glossary_term' = 'Lessons Learned Summary');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `management_response` SET TAGS ('dbx_business_glossary_term' = 'Management Response');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `management_response` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `progress_narrative` SET TAGS ('dbx_business_glossary_term' = 'Progress Narrative');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `recommendations` SET TAGS ('dbx_business_glossary_term' = 'Recommendations');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `reporting_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period End Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `reporting_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Start Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_team_members` SET TAGS ('dbx_business_glossary_term' = 'Review Team Members');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `scope_description` SET TAGS ('dbx_business_glossary_term' = 'Scope Description');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `sphere_standards_applied_flag` SET TAGS ('dbx_business_glossary_term' = 'Sphere Standards Applied Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_event_status` SET TAGS ('dbx_business_glossary_term' = 'Review Event Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Submission Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`review_event` ALTER COLUMN `review_event_type` SET TAGS ('dbx_business_glossary_term' = 'Review Event Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` SET TAGS ('dbx_association_edges' = 'program.intervention,partnership.partner_org');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership - Intervention Id');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ALTER COLUMN `role` SET TAGS ('dbx_business_glossary_term' = 'Partnership Role');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ALTER COLUMN `program_partnership_status` SET TAGS ('dbx_business_glossary_term' = 'Partnership Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ALTER COLUMN `sub_award_reference` SET TAGS ('dbx_business_glossary_term' = 'Sub-Award Reference');
ALTER TABLE `vibe_ngo_v1`.`program`.`program_partnership` ALTER COLUMN `program_partnership_type` SET TAGS ('dbx_business_glossary_term' = 'Partnership Type');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` SET TAGS ('dbx_subdomain' = 'lifecycle_governance');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` SET TAGS ('dbx_association_edges' = 'program.intervention,compliance.donor_requirement');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `intervention_compliance_id` SET TAGS ('dbx_business_glossary_term' = 'Intervention Compliance ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `donor_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Intervention Compliance - Donor Compliance Req Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `intervention_id` SET TAGS ('dbx_business_glossary_term' = 'Intervention Compliance - Intervention Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Staff Member');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `actual_effort_hours` SET TAGS ('dbx_business_glossary_term' = 'Actual Effort Hours');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `associated_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Associated Cost Amount');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Created Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `deliverable_format` SET TAGS ('dbx_business_glossary_term' = 'Deliverable Format');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Due Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `last_modified_date` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `requirement_status` SET TAGS ('dbx_business_glossary_term' = 'Requirement Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Submission Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `waiver_granted_flag` SET TAGS ('dbx_business_glossary_term' = 'Waiver Granted Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`intervention_compliance` ALTER COLUMN `waiver_justification` SET TAGS ('dbx_business_glossary_term' = 'Waiver Justification');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` SET TAGS ('dbx_subdomain' = 'component_linkage');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` SET TAGS ('dbx_association_edges' = 'program.component,mel.indicator');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `component_indicator_id` SET TAGS ('dbx_business_glossary_term' = 'Component Indicator Assignment ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component Indicator - Component Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `indicator_id` SET TAGS ('dbx_business_glossary_term' = 'Component Indicator - Indicator Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `assignment_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `baseline_value` SET TAGS ('dbx_business_glossary_term' = 'Component-Specific Baseline Value');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `data_collection_method` SET TAGS ('dbx_business_glossary_term' = 'Component Data Collection Method');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `is_mandatory` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `logframe_reference` SET TAGS ('dbx_business_glossary_term' = 'LogFrame Reference');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Assignment Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `reporting_frequency` SET TAGS ('dbx_business_glossary_term' = 'Component Reporting Frequency');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `responsible_unit` SET TAGS ('dbx_business_glossary_term' = 'Responsible Unit');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `component_indicator_status` SET TAGS ('dbx_business_glossary_term' = 'Assignment Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_indicator` ALTER COLUMN `target_value` SET TAGS ('dbx_business_glossary_term' = 'Component-Specific Target Value');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` SET TAGS ('dbx_subdomain' = 'component_linkage');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` SET TAGS ('dbx_association_edges' = 'program.component,technology.system_platform');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `component_system_usage_id` SET TAGS ('dbx_business_glossary_term' = 'Component System Usage ID');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component System Usage - Component Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `system_platform_id` SET TAGS ('dbx_business_glossary_term' = 'Component System Usage - System Platform Id');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `activation_date` SET TAGS ('dbx_business_glossary_term' = 'Activation Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `component_system_role` SET TAGS ('dbx_business_glossary_term' = 'Component System Role');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `data_classification_level` SET TAGS ('dbx_business_glossary_term' = 'Data Classification Level');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `data_classification_level` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `deactivation_date` SET TAGS ('dbx_business_glossary_term' = 'Deactivation Date');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `integration_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Integration Required Flag');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `primary_contact` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `component_system_usage_status` SET TAGS ('dbx_business_glossary_term' = 'Usage Status');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `component_system_usage_status` SET TAGS ('dbx_pii_demographic' = 'true');
ALTER TABLE `vibe_ngo_v1`.`program`.`component_system_usage` ALTER COLUMN `user_access_count` SET TAGS ('dbx_business_glossary_term' = 'User Access Count');
ALTER TABLE `vibe_ngo_v1`.`program`.`program` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_ngo_v1`.`program`.`program` SET TAGS ('dbx_subdomain' = 'intervention_design');
...
ALTER TABLE `vibe_ngo_v1`.`program`.`program` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Program Manager Id');
