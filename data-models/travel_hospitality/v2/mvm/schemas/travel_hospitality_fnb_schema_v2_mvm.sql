...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`fnb_outlet` (
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: fnb_outlet currently carries revenue_center_code (DECIMAL(18,2)) as a denormalized code. Each F&B outlet maps to a USALI revenue center for departmental P&L reporting. This is a fundamental relationsh',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`menu` (
    ...
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: Menus carry a seasonal_flag indicating seasonal applicability. F&B directors plan menu activations around property-level seasonal demand periods (holiday menus, summer menus). Linking menu to seasonal',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`menu_item` (
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: menu_item currently carries revenue_center_code (DECIMAL(18,2)) as a denormalized code. This should be normalized to a proper FK revenue_center_id referencing fnb.revenue_center.revenue_center_id. Men',
    ...
    `spice_level` BIGINT COMMENT 'Indicates the heat or spiciness level of the menu item. Used for guest preference matching and expectation setting.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`pos_check` (
    ...
    `beo_id` BIGINT COMMENT 'Foreign key linking to event.beo. Business justification: POS checks generated for banquet functions must reference the BEO for billing reconciliation, attrition tracking, and F&B minimum verification. Hotel banquet operations post charges through POS agains',
    ...
    `meeting_space_id` BIGINT COMMENT 'Foreign key linking to property.meeting_space. Business justification: Banquet and catering events generate POS checks tied to specific meeting rooms. Linking pos_check to meeting_space enables meeting space catering revenue attribution, event P&L reporting, and minimum ',
    `property_id` BIGINT COMMENT 'Identifier of the property where the check was opened.',
    `reservation_group_block_id` BIGINT COMMENT 'Foreign key linking to reservation.reservation_group_block. Business justification: Group master billing: F&B charges for group events must be attributed to the contracted group block for attrition calculation, master account posting, and group revenue reconciliation. Existing guest_',
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: A POS check is always opened within a specific USALI revenue center (e.g., Restaurant, Bar, Room Service). While pos_check_line already carries revenue_center_id for line-level allocation, the check h',
    ...
    `stay_history_id` BIGINT COMMENT 'Foreign key linking to guest.stay_history. Business justification: POS checks for in-house guests are posted to the stay folio and contribute to stay_history.fb_revenue. Linking pos_check to stay_history enables stay-level F&B revenue attribution and folio reconcilia',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`revenue_center` (
    ...
    `tax_rate_group_code` DECIMAL(18,2) COMMENT 'Tax rate group code applied to transactions in this revenue center. Maps to jurisdiction-specific tax rules.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`room_service_order` (
    ...
    `corporate_account_id` BIGINT COMMENT 'Foreign key linking to guest.corporate_account. Business justification: Corporate accounts with direct billing arrangements have room service charges posted to their master account. This FK enables corporate F&B billing reconciliation, a standard hospitality accounts-rece',
    `fnb_outlet_id` BIGINT COMMENT 'Reference to the Food and Beverage (F&B) outlet fulfilling the room service order.',
    `guest_group_block_id` BIGINT COMMENT 'Foreign key linking to guest.guest_group_block. Business justification: Group blocks frequently include room service charges billed to the master group folio. This FK enables group F&B spend tracking and master-folio posting, a standard group billing process. No existing ',
    `profile_id` BIGINT COMMENT 'Reference to the guest profile who placed the room service order.',
    `member_id` BIGINT COMMENT 'Foreign key linking to loyalty.member. Business justification: Room service orders must be linked to loyalty members for points accrual, member F&B spend reporting, and personalization. loyalty_member_number is a denormalized text field that should be replaced ',
    ...
    `stay_history_id` BIGINT COMMENT 'Foreign key linking to guest.stay_history. Business justification: Room service orders must be attributed to a specific stay for accurate F&B revenue per stay reporting and folio reconciliation. stay_history.fb_revenue aggregates this data. A domain expert expects ro',
    `vip_designation_id` BIGINT COMMENT 'Foreign key linking to guest.vip_designation. Business justification: VIP guests have specific room service protocols (priority dispatch, amenity inclusions, special handling). Linking to vip_designation enables VIP service-level compliance tracking and pre-arrival F&B ',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`stock_transaction` (
    ...
    `room_amenity_id` BIGINT COMMENT 'Foreign key linking to inventory.room_amenity. Business justification: Minibar restocking operations require tracking which room amenity unit (minibar) was replenished. F&B stock transactions for minibar consumption/restocking must reference the specific room_amenity rec',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`food_safety_inspection` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to property.property_facility. Business justification: Food safety inspections cover specific physical facilities (kitchens, storage rooms, prep areas) tracked in property_facility. Regulatory inspection reports reference the physical space. This link ena',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` (
    `recipe_line_id` BIGINT COMMENT 'Primary key for the recipe_line association',
    `inventory_item_id` BIGINT COMMENT 'Foreign key linking this recipe line to the specific inventory item (ingredient) used in the menu item.',
    `menu_item_id` BIGINT COMMENT 'Foreign key linking this recipe line to the parent menu item whose ingredient composition this record defines.',
    `is_primary_ingredient` BOOLEAN COMMENT 'Indicates whether this ingredient is a primary/key component of the menu item (true) versus a garnish, condiment, or modifier (false). Used for food cost reporting, allergen prominence flagging, and menu labeling decisions.',
    `preparation_notes` STRING COMMENT 'Free-text instructions specific to how this ingredient should be prepared or handled within this particular recipe (e.g., julienned, blanched before use, added at plating only). Belongs to the recipe line, not to the ingredient master or menu item description.',
    `quantity_per_portion` DECIMAL(18,2) COMMENT 'The gross quantity of this inventory item required to produce one standard portion of the menu item, before yield adjustment. Drives standard cost calculation and purchasing requirements.',
    `sequence_number` BIGINT COMMENT 'Integer defining the order in which this ingredient is used or added during preparation of the menu item. Used by kitchen staff to follow standardised recipe procedures and for recipe card printing.',
    `unit_of_measure` STRING COMMENT 'The unit of measure used to express quantity_per_portion for this ingredient in this recipe context (e.g., grams, ml, each). May differ from the inventory items base stock UOM and requires a conversion factor for cost calculation.',
    `yield_percent` DECIMAL(18,2) COMMENT 'The usable yield percentage of this ingredient after preparation loss (trimming, cooking, peeling). Used to calculate the net quantity needed from gross quantity. For example, a yield of 70% means 100g raw produces 70g usable product. Critical for accurate food cost calculation.',
    CONSTRAINT pk_recipe_line PRIMARY KEY(`recipe_line_id`)
) COMMENT 'This association product represents the Recipe Line (bill of materials) between a menu_item and an inventory_item. It captures the ingredient composition of each menu item — defining which inventory items are required, in what quantities, with what yield adjustments and preparation instructions. Each record links one menu_item to one inventory_item and carries attributes that exist only in the context of that specific ingredient-in-recipe relationship. Used by chefs for preparation, food cost controllers for standard cost calculation, and procurement for purchasing requirements and ISO 22000 traceability.. Existence Justification: In F&B operations, a menu item (e.g., Grilled Salmon) is composed of multiple inventory ingredients (salmon fillet, butter, lemon, herbs), and each inventory item (e.g., salmon fillet) is used across multiple menu items. This recipe/bill-of-materials relationship is a first-class operational concept actively managed by chefs, food cost controllers, and procurement teams — it drives standard cost calculation, purchasing requirements, and ISO 22000 traceability. The relationship carries its own data (quantity per portion, unit of measure, yield percent, preparation notes) that belongs to neither the menu item nor the inventory item alone.';
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ADD CONSTRAINT `fk_fnb_recipe_line_inventory_item_id` FOREIGN KEY (`inventory_item_id`) REFERENCES `vibe_travel_hospitality_v1`.`fnb`.`inventory_item`(`inventory_item_id`);
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ADD CONSTRAINT `fk_fnb_recipe_line_menu_item_id` FOREIGN KEY (`menu_item_id`) REFERENCES `vibe_travel_hospitality_v1`.`fnb`.`menu_item`(`menu_item_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`fnb_outlet` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`menu` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`pos_check` ALTER COLUMN `reservation_group_block_id` SET TAGS ('dbx_business_glossary_term' = 'Reservation Group Block Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`pos_check` ALTER COLUMN `stay_history_id` SET TAGS ('dbx_business_glossary_term' = 'Stay History Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`room_service_order` ALTER COLUMN `corporate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Corporate Account Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`room_service_order` ALTER COLUMN `guest_group_block_id` SET TAGS ('dbx_business_glossary_term' = 'Guest Group Block Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`room_service_order` ALTER COLUMN `stay_history_id` SET TAGS ('dbx_business_glossary_term' = 'Stay History Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`room_service_order` ALTER COLUMN `vip_designation_id` SET TAGS ('dbx_business_glossary_term' = 'Vip Designation Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`food_safety_inspection` SET TAGS ('dbx_subdomain' = 'inventory_control');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`food_safety_inspection` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Property Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` SET TAGS ('dbx_subdomain' = 'inventory_control');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` SET TAGS ('dbx_association_edges' = 'fnb.menu_item,fnb.inventory_item');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `recipe_line_id` SET TAGS ('dbx_business_glossary_term' = 'Recipe Line - Recipe Line Id');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `inventory_item_id` SET TAGS ('dbx_business_glossary_term' = 'Recipe Line - Inventory Item Id');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `menu_item_id` SET TAGS ('dbx_business_glossary_term' = 'Recipe Line - Menu Item Id');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `is_primary_ingredient` SET TAGS ('dbx_business_glossary_term' = 'Primary Ingredient Flag');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `preparation_notes` SET TAGS ('dbx_business_glossary_term' = 'Preparation Notes');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `quantity_per_portion` SET TAGS ('dbx_business_glossary_term' = 'Quantity Per Portion');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Preparation Sequence Number');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Recipe Unit of Measure');
ALTER TABLE `vibe_travel_hospitality_v1`.`fnb`.`recipe_line` ALTER COLUMN `yield_percent` SET TAGS ('dbx_business_glossary_term' = 'Yield Percentage');
