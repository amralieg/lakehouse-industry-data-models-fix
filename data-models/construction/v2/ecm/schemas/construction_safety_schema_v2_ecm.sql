...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`safety_audit` (
    ...
    `safety_audit_description` STRING COMMENT '',
    ...
    `safety_audit_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`hse_inspection` (
    ...
    `hse_inspection_description` STRING COMMENT '',
    ...
    `hse_inspection_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`ppe_register` (
    ...
    `cost_code` DECIMAL(18,2) COMMENT 'Project cost code or Work Breakdown Structure (WBS) element to which the PPE cost is allocated. Enables job costing and HSE cost tracking in SAP S/4HANA Project Systems and Viewpoint Vista.',
    ...
    `ppe_register_description` STRING COMMENT '',
    ...
    `ppe_register_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`environmental_monitoring` (
    ...
    `data_quality_flag` BOOLEAN COMMENT 'Quality assurance flag indicating the reliability and validity of the measurement value. Suspect indicates potential instrument malfunction or anomalous conditions; Below Detection Limit indicates the parameter was not detectable at the instruments minimum detection level. Used in QA/QC review.',
    `environmental_monitoring_description` STRING COMMENT '',
    ...
    `instrument_calibration_date` DECIMAL(18,2) COMMENT 'The date on which the monitoring instrument was last calibrated prior to this measurement. Used to verify that measurements were taken with a calibrated instrument within the required calibration interval, supporting regulatory defensibility.',
    `instrument_calibration_due_date` DECIMAL(18,2) COMMENT 'The date by which the monitoring instrument must next be calibrated to remain within its valid calibration period. Measurements taken after this date may be flagged as non-compliant for regulatory reporting purposes.',
    ...
    `sample_duration_minutes` DECIMAL(18,2) COMMENT 'The duration in minutes over which the sample or measurement was collected or averaged. Relevant for time-averaged parameters such as noise (LAeq), air quality (24-hour PM10), and vibration. Null for instantaneous grab samples.',
    ...
    `environmental_monitoring_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`training` (
    ...
    `firm_profile_id` BIGINT COMMENT 'Foreign key linking to bid.firm_profile. Business justification: Regulatory HSE reporting requires linking each safety training record to the subcontractor that provided or is responsible for the training.',
    ...
    `training_description` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`chemical_register` (
    ...
    `chemical_register_description` STRING COMMENT '',
    ...
    `expiration_date` DECIMAL(18,2) COMMENT 'Date after which the chemical is considered expired and must not be used.',
...
CREATE OR REPLACE TABLE `vibe_construction_v1`.`safety`.`incident_subcontractor_involvement` (
    ...
    `corrective_action_required` BOOLEAN COMMENT 'Whether corrective action is required from the subcontractor.',
    `corrective_action_required_flag` BOOLEAN COMMENT '',
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp.',
    `incident_subcontractor_involvement_description` STRING COMMENT '',
    `fault_percentage` DECIMAL(18,2) COMMENT '',
    `involvement_description` STRING COMMENT 'Description of the subcontractors involvement.',
    `involvement_role` STRING COMMENT 'The role of the subcontractor in the incident (e.g., contractor, equipment provider, laborer)',
    `involvement_status` STRING COMMENT '',
    `involvement_type` STRING COMMENT 'Type of involvement (direct, indirect, witness).',
    `is_at_fault` BOOLEAN COMMENT 'Whether the subcontractor was found at fault.',
    `is_primary_responsible` BOOLEAN COMMENT '',
    `liability_percentage` DECIMAL(18,2) COMMENT 'Percentage of liability assigned to the subcontractor for the incident',
    `notification_date` DATE COMMENT 'Date the subcontractor was notified of involvement.',
    ...
    `workers_involved_count` STRING COMMENT '',
    `created_by` STRING COMMENT '',
...
ALTER TABLE `vibe_construction_v1`.`safety`.`incident` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Bid Submission Id (Foreign Key)');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`swms` ALTER COLUMN `tertiary_swms_prepared_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Prepared By (Employee ID)');
ALTER TABLE `vibe_construction_v1`.`safety`.`swms` ALTER COLUMN `tertiary_swms_prepared_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_construction_v1`.`safety`.`swms` ALTER COLUMN `tertiary_swms_prepared_by_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`permit_to_work` ALTER COLUMN `performing_authority_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`toolbox_meeting` SET TAGS ('dbx_subdomain' = 'field_compliance');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`toolbox_meeting` ALTER COLUMN `firm_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Subcontractor ID');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`safety_audit` SET TAGS ('dbx_subdomain' = 'field_compliance');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`safety_audit` ALTER COLUMN `auditee_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`safety_audit` ALTER COLUMN `lead_auditor_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`hse_inspection` SET TAGS ('dbx_subdomain' = 'field_compliance');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`hse_inspection` ALTER COLUMN `firm_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Sub Firm Id (Foreign Key)');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`ppe_register` SET TAGS ('dbx_subdomain' = 'worker_protection');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`ppe_register` ALTER COLUMN `item_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`hse_plan` SET TAGS ('dbx_subdomain' = 'field_compliance');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`hse_plan` ALTER COLUMN `firm_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Sub Firm Id (Foreign Key)');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`hazard_register` ALTER COLUMN `firm_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Subcontractor ID');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`environmental_monitoring` SET TAGS ('dbx_subdomain' = 'risk_control');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`environmental_monitoring` ALTER COLUMN `data_quality_flag` SET TAGS ('dbx_type_corrected' = 'true');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`environmental_monitoring` ALTER COLUMN `laboratory_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`environmental_monitoring` ALTER COLUMN `monitoring_location_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`training` SET TAGS ('dbx_subdomain' = 'worker_protection');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`training` ALTER COLUMN `firm_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Sub Firm Id (Foreign Key)');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`chemical_register` SET TAGS ('dbx_subdomain' = 'worker_protection');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`chemical_register` ALTER COLUMN `chemical_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_construction_v1`.`safety`.`incident_subcontractor_involvement` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_construction_v1`.`safety`.`incident_subcontractor_involvement` ALTER COLUMN `created_by` SET TAGS ('dbx_mask_in_nonprod' = 'true');
