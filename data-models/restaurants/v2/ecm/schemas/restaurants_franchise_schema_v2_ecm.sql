...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`franchisee` (
    ...
    `credit_rating` DECIMAL(18,2) COMMENT 'Credit rating assigned to the franchisee by a rating agency. [ENUM-REF-CANDIDATE: aaa|aa|a|bbb|bb|b|ccc|cc|c|d — promote to reference product]',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`billing` (
    ...
    `agreement_id` BIGINT COMMENT 'Connect franchise.billing by adding column agreement_id (BIGINT) with FK to franchise.agreement.agreement_id because franchise billing must reference the governing agreement. P17: connect_table: franchise.billing** - add column agreement_id',
    ...
    `amount_due` DECIMAL(18,2) COMMENT '',
    `amount_paid` DECIMAL(18,2) COMMENT '',
    `billed_amount` DECIMAL(18,2) COMMENT 'Billed amount',
    `billing_date` DATE COMMENT '',
    `billing_number` STRING COMMENT '',
    `billing_status` STRING COMMENT '',
    ...
    `marketing_fee_amount` DECIMAL(18,2) COMMENT '',
    ...
    `period_end_date` DATE COMMENT 'Billing period end',
    `period_start` DATE COMMENT '',
    `period_start_date` DATE COMMENT 'Billing period start',
    `royalty_amount` DECIMAL(18,2) COMMENT '',
    `source_system_code` STRING COMMENT '',
    `technology_fee_amount` DECIMAL(18,2) COMMENT 'Technology fee billed',
    `total_amount` DECIMAL(18,2) COMMENT '',
    `total_amount_due` DECIMAL(18,2) COMMENT '',
    `total_due` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`sales_report` (
    ...
    `daypart_sales_breakdown` DECIMAL(18,2) COMMENT 'JSON string summarizing sales by daypart (e.g., breakfast, lunch, dinner).',
    ...
    `sales_report_status` DECIMAL(18,2) COMMENT 'Current lifecycle status of the sales report.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` (
    ...
    `health_inspection_score` DECIMAL(18,2) COMMENT 'Score from the most recent health inspection (0‑100).',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`development_schedule` (
    ...
    `total_units_committed` DECIMAL(18,2) COMMENT 'Total number of restaurant units the franchisee has agreed to open under the schedule.',
    `units_opened_to_date` DATE COMMENT 'Cumulative count of units the franchisee has opened to date under this schedule.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` (
    ...
    `compliance_audit_id` BIGINT COMMENT 'Connect franchise.franchise_corrective_action by adding column compliance_audit_id (BIGINT) with FK to franchise.compliance_audit.compliance_audit_id. - P19: connect_table: franchise.franchise_corrective_action** - add column compliance_aud',
    `foodsafety_corrective_action_id` BIGINT COMMENT 'FK to canonical SSOT owner foodsafety.foodsafety_corrective_action for concept corrective_action.',
    ...
    `root_cause` STRING COMMENT 'Business attribute root_cause for franchise.franchise_corrective_action',
    `severity` STRING COMMENT '',
    `severity_level` STRING COMMENT '',
    `source_system_code` STRING COMMENT '',
    `franchise_corrective_action_status` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
    `verification_notes` STRING COMMENT 'Business attribute verification_notes for franchise.franchise_corrective_action',
    `verification_status` STRING COMMENT 'Business attribute verification_status for franchise_corrective_action',
    CONSTRAINT pk_franchise_corrective_action PRIMARY KEY(`franchise_corrective_action_id`)
) COMMENT 'Tracks corrective action plans (CAPs) issued to franchisees following compliance audit failures, brand standards violations, or contractual defaults. Captures violation description, severity level, corrective action required, due date, responsible party, resolution status, follow-up audit date, escalation level, and cure period expiration. Supports IFA best practices for franchisee remediation and FTC Franchise Rule default notice requirements. [SSOT] Aliases canonical owner foodsafety.foodsafety_corrective_action for concept corrective_action. SSOT: authoritative source is foodsafety.foodsafety_corrective_action. SSOT: authoritative owner is foodsafety.foodsafety_corrective_action for concept corrective_action. [SSOT: authoritative owner is foodsafety.foodsafety_corrective_action] distinct context-corrective actions differ by trigger; keep both but clarify ownership scope';
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` (
    ...
    `calculation_basis` STRING COMMENT '',
    ...
    `expiry_date` DATE COMMENT 'Business attribute expiry_date derived from business context.',
    `fee_amount` DECIMAL(18,2) COMMENT '',
    `fee_name` DECIMAL(18,2) COMMENT '',
    `fee_percentage` DECIMAL(18,2) COMMENT '',
    `fee_rate` DECIMAL(18,2) COMMENT '',
    `fee_rate_pct` DECIMAL(18,2) COMMENT '',
    `fee_rate_percent` DECIMAL(18,2) COMMENT '',
    `fee_type` DECIMAL(18,2) COMMENT '',
    `flat_amount` DECIMAL(18,2) COMMENT '',
    `flat_fee_amount` DECIMAL(18,2) COMMENT '',
    `frequency` STRING COMMENT '',
    `is_active` BOOLEAN COMMENT '',
    `marketing_fee_rate_pct` DECIMAL(18,2) COMMENT 'Marketing fee rate percent',
    `fee_schedule_name` DECIMAL(18,2) COMMENT 'Name of the fee schedule',
    `rate_percent` DECIMAL(18,2) COMMENT '',
    `rate_percentage` STRING COMMENT 'Rate percentage',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`renewal_event` (
    ...
    `renewal_fee_currency` DECIMAL(18,2) COMMENT 'Three‑letter ISO 4217 currency code for the renewal fee.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`termination_event` (
    ...
    `outstanding_royalty_currency_code` DECIMAL(18,2) COMMENT 'Three-letter ISO currency code for the outstanding royalty balance.',
    ...
    `termination_fee_currency_code` DECIMAL(18,2) COMMENT 'ISO currency code for the termination fee.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`performance_scorecard` (
    ...
    `net_promoter_score` DECIMAL(18,2) COMMENT 'Net promoter score for the franchisee, ranging from -100 to 100.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`fdd_disclosure` (
    ...
    `state_registration_status` DECIMAL(18,2) COMMENT 'Indicates whether the disclosure has been registered with the state regulatory authority.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`area_representative` (
    ...
    `fee_structure_description` DECIMAL(18,2) COMMENT 'Narrative description of fees charged to the representative.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`support_visit` (
    ...
    `expense_category` DECIMAL(18,2) COMMENT 'Category of the expense associated with the support visit.',
    ...
    `visit_duration_minutes` DECIMAL(18,2) COMMENT 'Total time spent on the support visit, measured in minutes.',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`marketing_fund_contribution` (
    ...
    `calculation_basis` STRING COMMENT 'Business attribute calculation_basis for marketing_fund_contribution',
    `contribution_amount` DECIMAL(18,2) COMMENT '',
    `contribution_date` DATE COMMENT '',
    `contribution_period` DECIMAL(18,2) COMMENT '',
    `contribution_period_end` DECIMAL(18,2) COMMENT '',
    `contribution_period_end_date` DATE COMMENT 'Contribution period end',
    `contribution_period_start` DECIMAL(18,2) COMMENT '',
    `contribution_period_start_date` DATE COMMENT 'Contribution period start',
    `contribution_rate` DECIMAL(18,2) COMMENT '',
    `contribution_rate_pct` DECIMAL(18,2) COMMENT '',
    `contribution_rate_percent` DECIMAL(18,2) COMMENT '',
    `contribution_status` DECIMAL(18,2) COMMENT 'Lifecycle status indicator for the contribution on this marketing fund contribution record, capturing values such as pending, approved, posted, reconciled, disputed, or cancelled.',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency` STRING COMMENT '',
    `currency_code` STRING COMMENT '',
    `due_date` DATE COMMENT '',
    `fund_balance_amount` DECIMAL(18,2) COMMENT '',
    `gross_sales_amount` DECIMAL(18,2) COMMENT 'Gross sales basis',
    `gross_sales_basis` DECIMAL(18,2) COMMENT '',
    `is_paid` BOOLEAN COMMENT '',
    `paid_at` TIMESTAMP COMMENT '',
    `paid_date` DATE COMMENT '',
    ...
    `period` STRING COMMENT '',
    `rate_percent` DECIMAL(18,2) COMMENT '',
    `sales_basis` STRING COMMENT '',
    `sales_basis_amount` DECIMAL(18,2) COMMENT '',
    `source_system_code` STRING COMMENT '',
    `marketing_fund_contribution_status` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` (
    ...
    `annual_escalation_rate` DECIMAL(18,2) COMMENT '',
    `base_rent` DECIMAL(18,2) COMMENT 'Business attribute base_rent for franchise.lease_agreement',
    `base_rent_amount` DECIMAL(18,2) COMMENT 'Monthly base rent amount agreed in the lease',
    `cam_charge_amount` DECIMAL(18,2) COMMENT '',
    `cam_charges` DECIMAL(18,2) COMMENT '',
    `commencement_date` DATE COMMENT 'Derived business attribute for lease_agreement.',
    `created_timestamp` TIMESTAMP COMMENT '',
    `currency` STRING COMMENT '',
    `currency_code` STRING COMMENT '',
    `effective_date` DATE COMMENT '',
    `end_date` DATE COMMENT '',
    `escalation_rate_percent` DECIMAL(18,2) COMMENT '',
    `expiration_date` DATE COMMENT 'Derived business attribute for lease_agreement.',
    `expiry_date` DATE COMMENT '',
    `guarantor` STRING COMMENT '',
    `guarantor_name` STRING COMMENT '',
    `is_subleased` BOOLEAN COMMENT '',
    `lease_end_date` DATE COMMENT 'Date the lease ends',
    `lease_number` STRING COMMENT '',
    `lease_start_date` DATE COMMENT 'Date the lease begins',
    `lease_status` STRING COMMENT '',
    ...
    `monthly_rent` DECIMAL(18,2) COMMENT '',
    `monthly_rent_amount` DECIMAL(18,2) COMMENT '',
    `notes` STRING COMMENT '',
    `percentage_rent_breakpoint_amount` DECIMAL(18,2) COMMENT '',
    `percentage_rent_rate` DECIMAL(18,2) COMMENT '',
    `renewal_option` STRING COMMENT '',
    `renewal_option_count` STRING COMMENT '',
    `renewal_option_flag` BOOLEAN COMMENT '',
    `renewal_option_term_months` STRING COMMENT '',
    `renewal_options` STRING COMMENT '',
    `rent_escalation_percent` DECIMAL(18,2) COMMENT '',
    `rent_escalation_rate` DECIMAL(18,2) COMMENT 'Annual rent escalation percentage',
    `security_deposit` DECIMAL(18,2) COMMENT '',
    `security_deposit_amount` DECIMAL(18,2) COMMENT '',
    `signed_at` TIMESTAMP COMMENT '',
    `square_footage` DECIMAL(18,2) COMMENT '',
    `start_date` DATE COMMENT '',
    `lease_agreement_status` STRING COMMENT '',
    `term_months` STRING COMMENT '',
    `updated_timestamp` TIMESTAMP COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_restaurants_v1`.`franchise`.`franchise_remodel_project` (
    `franchise_remodel_project_id` BIGINT COMMENT '',
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`marketing_fund_contribution` ADD CONSTRAINT `fk_franchise_marketing_fund_contribution_agreement_id` FOREIGN KEY (`agreement_id`) REFERENCES `vibe_restaurants_v1`.`franchise`.`agreement`(`agreement_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` ADD CONSTRAINT `fk_franchise_lease_agreement_agreement_id` FOREIGN KEY (`agreement_id`) REFERENCES `vibe_restaurants_v1`.`franchise`.`agreement`(`agreement_id`);
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchisee` SET TAGS ('dbx_ensured' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchisee` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`agreement` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`territory` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`billing` SET TAGS ('dbx_subdomain' = 'financial_operations');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`billing` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`sales_report` SET TAGS ('dbx_subdomain' = 'financial_operations');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`sales_report` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` SET TAGS ('dbx_subdomain' = 'growth_development');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii_subtype' = 'person_name');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii_kind' = 'person_name');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `last_milestone_name` SET TAGS ('dbx_pii_category' = 'person_name');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `project_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`nro_pipeline` ALTER COLUMN `project_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`development_schedule` SET TAGS ('dbx_subdomain' = 'growth_development');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`development_schedule` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`compliance_audit` SET TAGS ('dbx_subdomain' = 'compliance_performance');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`compliance_audit` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_subdomain' = 'compliance_performance');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_authoritative_owner' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_alias_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot' = 'alias');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_concept' = 'corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_owner' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_dependent_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_role' = 'duplicate');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_status' = 'deprecated');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `franchise_corrective_action_id` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `compliance_audit_id` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_ssot_canonical_ref' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_ssot' = 'reference');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_ssot_owner' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_canonical' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `foodsafety_corrective_action_id` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `franchisee_id` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `action_code` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `action_description` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `action_number` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `action_status` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `assigned_to` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `assigned_to` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `closed_at` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `closed_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `completed_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `completion_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `corrective_steps` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `franchise_corrective_action_description` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `due_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `is_resolved` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `issue_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `issue_description` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `issued_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `resolution_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `resolution_status` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `resolved_at` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `resolved_date` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `responsible_party` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `root_cause` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `severity` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `severity_level` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `source_system_code` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `franchise_corrective_action_status` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `verification_notes` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_corrective_action` ALTER COLUMN `verification_status` SET TAGS ('dbx_ssot_duplicate_of' = 'foodsafety.foodsafety_corrective_action');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` SET TAGS ('dbx_subdomain' = 'financial_operations');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` ALTER COLUMN `fee_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fee_schedule` ALTER COLUMN `fee_schedule_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`training_enrollment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`training_enrollment` SET TAGS ('dbx_subdomain' = 'compliance_performance');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`training_enrollment` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`transfer_event` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`renewal_event` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`termination_event` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`performance_scorecard` SET TAGS ('dbx_subdomain' = 'compliance_performance');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`performance_scorecard` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`fdd_disclosure` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`prospect` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`prospect` ALTER COLUMN `address_line2` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`area_representative` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`area_representative` ALTER COLUMN `primary_contact_method` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`support_visit` SET TAGS ('dbx_subdomain' = 'compliance_performance');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`support_visit` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`marketing_fund_contribution` SET TAGS ('dbx_subdomain' = 'financial_operations');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`marketing_fund_contribution` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` SET TAGS ('dbx_structure_locked' = 'v2');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` ALTER COLUMN `lease_id` SET TAGS ('dbx_role' = 'fk');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` ALTER COLUMN `guarantor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` ALTER COLUMN `guarantor_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`lease_agreement` ALTER COLUMN `created_at` SET TAGS ('dbx_role' = 'audit');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_remodel_project` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_remodel_project` SET TAGS ('dbx_ssot_role' = 'authoritative');
...
ALTER TABLE `vibe_restaurants_v1`.`franchise`.`franchise_remodel_project` ALTER COLUMN `franchise_remodel_project_id` SET TAGS ('dbx_pk' = 'true');
