...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`member` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to channel.channel. Business justification: Loyalty enrollment attribution reporting requires linking each member to the channel through which they enrolled. The plain-text enrollment_channel attribute is a denormalization of channel.channel. M',
    ...
    `membership_number` BIGINT COMMENT 'Externally-known unique membership identifier assigned to the loyalty program member. Used for member recognition and program transactions.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`points_ledger` (
    ...
    `event_booking_id` BIGINT COMMENT 'Foreign key linking to event.event_booking. Business justification: MICE/group loyalty programs award points for event bookings separately from hotel stays. Points_ledger already has reservation_booking_id for stays; a parallel event_booking_id FK is needed to record ',
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: Points ledger entries from F&B transactions must reference the originating revenue center for F&B-level loyalty reporting (points earned per outlet), USALI reconciliation, and revenue center profitabi',
    ...
    `stay_history_id` BIGINT COMMENT 'Foreign key linking to guest.stay_history. Business justification: Loyalty audit and points dispute resolution require direct reconciliation of points ledger entries against stay history records. Points posting teams and member services agents use this link to valida',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`accrual_rule` (
    ...
    `booking_source_id` BIGINT COMMENT 'Foreign key linking to channel.booking_source. Business justification: Accrual rules are frequently restricted to specific booking sources (e.g., OTA bookings earn fewer points than direct). The plain-text booking_channel_restriction attribute is a denormalization of cha',
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: Loyalty accrual rules are scoped to specific F&B revenue centers in hotel operations (e.g., 2x points at signature restaurant vs. bar). This FK enables revenue-center-specific points earning configura',
    `segment_id` BIGINT COMMENT 'Foreign key linking to guest.segment. Business justification: Loyalty program configuration includes segment-targeted accrual rules (e.g., corporate segment earns 3x points). Revenue and loyalty managers configure and report on segment-specific earning rules. A ',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption_rule` (
    ...
    `revenue_center_id` BIGINT COMMENT 'Foreign key linking to fnb.revenue_center. Business justification: Redemption rules in hotel loyalty programs are scoped to specific F&B revenue centers (e.g., points redeemable only at the signature restaurant). This FK enables outlet-specific redemption configurati',
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: Redemption blackout dates align with peak seasonal demand periods. Linking redemption_rule to seasonal_calendar replaces the unstructured blackout_dates text field with a structured FK, enabling autom',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption` (
    ...
    `event_booking_id` BIGINT COMMENT 'Foreign key linking to event.event_booking. Business justification: Members redeem points against event bookings (meeting room rental, AV packages, catering). Redemption already has reservation_booking_id for hotel stays; a distinct event_booking_id FK tracks MICE red',
    `member_id` BIGINT COMMENT 'Identifier of the loyalty program member who initiated the redemption.',
    `property_outlet_id` BIGINT COMMENT 'Foreign key linking to property.property_outlet. Business justification: When members redeem points for F&B rewards (complimentary dining, F&B credits), the redemption must reference the specific outlet for outlet-level loyalty redemption reporting, revenue offset accounti',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`reward_catalog` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to channel.channel. Business justification: Reward catalog items are redeemable only through specific channels (e.g., direct booking only, not OTA). The plain-text redemption_channel attribute is a denormalization of channel.channel. Channel-sp',
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: Free Night Award Fulfillment: when a member redeems a free-night reward, the system must allocate inventory for a specific room type. This FK links the reward catalog entry to the eligible room type, ',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`benefit_entitlement` (
    ...
    `promotion_id` BIGINT COMMENT 'Foreign key linking to loyalty.promotion. Business justification: A benefit entitlement can be granted as a direct result of a loyalty promotion (e.g., a promotional campaign that awards a complimentary night, room upgrade, or amenity benefit). The existing entitlem',
    `reservation_booking_id` BIGINT COMMENT 'Foreign key linking to reservation.reservation_booking. Business justification: When a loyalty benefit (free breakfast, room upgrade, late checkout) is consumed on a specific stay, the entitlement must reference the booking where it was applied. This enables benefit utilization r',
    `stay_history_id` BIGINT COMMENT 'Foreign key linking to guest.stay_history. Business justification: Benefit fulfillment tracking requires knowing which stay consumed a benefit entitlement (e.g., complimentary breakfast, room upgrade). Loyalty operations and audit teams report on benefit utilization ',
    ...
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: Loyalty Benefit Fulfillment — Room Upgrade: benefit_entitlement tracks room upgrade entitlements for tier members. The role-prefix upgrade_ disambiguates the target room type the member is entitled ',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to channel.channel. Business justification: Loyalty promotions are distributed through specific channels (OTA, direct, GDS). The plain-text marketing_channel attribute is a denormalization of channel.channel. Channel-level promotion performance',
    ...
    `seasonal_calendar_id` BIGINT COMMENT 'Foreign key linking to property.seasonal_calendar. Business justification: Loyalty promotions (double points, bonus nights) are coordinated with seasonal demand periods. Linking promotion to seasonal_calendar enables revenue management teams to align loyalty incentives with ',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`member_preference` (
    ...
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: Member Preference Management — Room Type: member_preference records room-type preferences (preference_category=room_type) with preference_value as a denormalized string. A proper FK to inventory.roo',
    ...
    `confidence_level` BIGINT COMMENT 'The confidence or reliability level of this preference. Stated preferences have highest confidence; inferred preferences are scored based on frequency and recency of observed behavior.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion_enrollment` (
    ...
    `member_id` BIGINT COMMENT 'Foreign key linking this enrollment record to the enrolled loyalty member.',
    `promotion_id` BIGINT COMMENT 'Foreign key linking this enrollment record to the parent loyalty promotion.',
    `award_date` DATE COMMENT 'Date when the bonus points or nights were posted to the members account for this promotion enrollment. Used for fulfillment tracking and reconciliation.',
    `completion_date` DATE COMMENT 'Date when the member successfully completed all qualifying criteria for this promotion. Null if not yet completed. Source for promotion.total_completions aggregate.',
    `enrollment_channel` STRING COMMENT 'Channel through which the member enrolled in the promotion (e.g., web, mobile_app, front_desk, email_link, auto_enrolled). Belongs to the enrollment event, not to the member or promotion.',
    `enrollment_date` DATE COMMENT 'Date when the member enrolled or was auto-enrolled in the promotion. Belongs to the enrollment relationship, not to the member or promotion alone. Source for promotion.total_enrollments aggregate.',
    `exclusion_list` STRING COMMENT 'Comma-separated list of member IDs or segments explicitly excluded from this promotion (e.g., employees, test accounts, members who opted out). Null if no exclusions apply. [Moved from promotion: The exclusion_list on promotion is an anti-pattern (comma-separated member IDs stored as STRING). Member-level exclusion state belongs in the enrollment association as a status value (e.g., status = excluded) or as a separate exclusion flag on the enrollment record. This attribute should be removed from promotion and replaced by querying enrollment records with excluded status.]',
    `opt_out_date` DATE COMMENT 'Date when the member opted out of this specific promotion enrollment. Null if still active or completed. Belongs to the enrollment lifecycle.',
    `points_awarded` DECIMAL(18,2) COMMENT 'Actual bonus points awarded to this member upon completion of this promotion. May differ from promotion.bonus_points_amount if pro-rated or adjusted. Belongs to the enrollment outcome, not to the members lifetime balance or the promotions budget.',
    `promotion_enrollment_status` STRING COMMENT 'Current lifecycle state of this members participation in the promotion. Tracks progress from enrollment through completion or expiry.',
    `qualifying_spend_amount` DECIMAL(18,2) COMMENT 'Cumulative qualifying spend the member has accumulated toward this promotions minimum_spend_amount requirement. Belongs to the enrollment relationship.',
    `qualifying_stays_count` STRING COMMENT 'Number of qualifying stays the member has accumulated toward this promotions minimum_nights requirement. Belongs to the enrollment relationship as it tracks progress specific to this member-promotion pair.',
    `registration_timestamp` TIMESTAMP COMMENT 'Precise timestamp of the enrollment event, capturing the exact moment the member registered. Used for audit trail and channel attribution.',
    CONSTRAINT pk_promotion_enrollment PRIMARY KEY(`promotion_enrollment_id`)
) COMMENT 'This association product represents the Enrollment event between a loyalty promotion and a loyalty member. It captures the full lifecycle of a members participation in a specific promotion — from registration through qualifying activity progress to bonus award or expiry. Each record links one promotion to one member and carries attributes that exist only in the context of that specific member-promotion participation: enrollment channel, status, qualifying progress, and award outcome. This is the operational source of truth for promotion fulfillment tracking and the source for the aggregate counters (total_enrollments, total_completions) on the promotion entity.. Existence Justification: In loyalty program operations, a member can enroll in multiple promotions simultaneously (e.g., a birthday bonus AND a spend challenge AND a double nights offer), and each promotion is designed to attract and track hundreds or thousands of members. The business actively manages each member-promotion enrollment as a distinct operational record — tracking enrollment date, progress toward qualifying criteria, completion status, and bonus award. This is a textbook operational M:N: the enrollment itself is a named business concept with rich lifecycle data that belongs to neither the promotion nor the member alone.';
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`points_ledger` SET TAGS ('dbx_subdomain' = 'points_activity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`points_ledger` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`points_ledger` ALTER COLUMN `stay_history_id` SET TAGS ('dbx_business_glossary_term' = 'Stay History Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`accrual_rule` SET TAGS ('dbx_subdomain' = 'points_activity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`accrual_rule` ALTER COLUMN `booking_source_id` SET TAGS ('dbx_business_glossary_term' = 'Booking Source Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`accrual_rule` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption_rule` SET TAGS ('dbx_subdomain' = 'points_activity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption_rule` ALTER COLUMN `revenue_center_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Center Id (Foreign Key)');
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption_rule` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption` SET TAGS ('dbx_subdomain' = 'points_activity');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`redemption` ALTER COLUMN `property_outlet_id` SET TAGS ('dbx_business_glossary_term' = 'Outlet Property Outlet Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`reward_catalog` SET TAGS ('dbx_subdomain' = 'reward_promotions');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`benefit_entitlement` ALTER COLUMN `stay_history_id` SET TAGS ('dbx_business_glossary_term' = 'Stay History Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`benefit_entitlement` ALTER COLUMN `room_type_id` SET TAGS ('dbx_business_glossary_term' = 'Upgrade Room Type Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion` SET TAGS ('dbx_subdomain' = 'reward_promotions');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion` ALTER COLUMN `seasonal_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Calendar Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`member_preference` ALTER COLUMN `room_type_id` SET TAGS ('dbx_business_glossary_term' = 'Room Type Id (Foreign Key)');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion_enrollment` SET TAGS ('dbx_subdomain' = 'reward_promotions');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion_enrollment` ALTER COLUMN `qualifying_spend_amount` SET TAGS ('dbx_business_glossary_term' = 'Qualifying Spend Amount');
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion_enrollment` ALTER COLUMN `qualifying_stays_count` SET TAGS ('dbx_business_glossary_term' = 'Qualifying Stays Count');
ALTER TABLE `vibe_travel_hospitality_v1`.`loyalty`.`promotion_enrollment` ALTER COLUMN `registration_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Registration Timestamp');
