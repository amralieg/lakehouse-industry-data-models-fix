...
-- Generated on: 2026-06-23 22:46:20
...
-- ========= TABLES =========
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`storefront` (
    ...
    `digital_catalog_id` BIGINT COMMENT 'Identifier of the product catalog assigned to this storefront within the e-commerce platform or the PIM system. Determines which assortment of SKUs, categories, and product content is surfaced to customers on this channel. Aligns with Oracle Retail Merchandising System (ORMS) assortment planning and the customer master data system product golden records.',
    ...
    `created_timestamp` TIMESTAMP COMMENT 'The timestamp when this storefront record was first created in the system of record (the e-commerce platform or the customer master data system). Represents the audit trail creation event for data governance and lineage tracking purposes. Format: yyyy-MM-ddTHH:mm:ss.SSSXXX.',
    ...
    `free_shipping_threshold` DECIMAL(18,2) COMMENT 'The minimum order value (in the storefronts primary currency) at which free standard shipping is automatically applied at checkout. A key lever for AOV (Average Order Value) optimization and customer acquisition. Configured in Oracle Retail Price Management (RPM) or the e-commerce platform promotion rules.',
    `gdpr_consent_required` BOOLEAN COMMENT 'Indicates whether this storefront is subject to GDPR (General Data Protection Regulation) consent requirements, triggering cookie consent banners, data processing notices, and opt-in workflows for customers in EU/EEA jurisdictions. Drives compliance configuration in the e-commerce platform and CRM systems.',
    ...
    `platform_version` STRING COMMENT 'The version identifier of the the e-commerce platform (SFCC) platform release deployed for this storefront (e.g., SFCC 23.7, B2C Commerce 22.10). Used for release management, compatibility tracking, and upgrade planning across the digital commerce estate.',
    ...
    `seo_meta_title` STRING COMMENT 'The default HTML meta title tag content for the storefronts homepage, used by search engines for indexing and SERP (Search Engine Results Page) display. Impacts organic search traffic and digital marketing performance. Managed within the e-commerce platform site preferences.',
    `session_timeout_minutes` STRING COMMENT 'The number of minutes of customer inactivity after which a shopping session is automatically expired on this storefront. Governs cart abandonment timing, session-based analytics, and security compliance for authenticated customer sessions. Configured in the e-commerce platform site security settings.',
    `site_code` STRING COMMENT 'The externally-known unique site identifier assigned within the e-commerce platform (SFCC) for this storefront. Used as the operational reference key across digital commerce platform configurations, API calls, and integration touchpoints. Aligns to the SFCC Site objects siteID field.',
    `source_system_code` STRING COMMENT 'Code identifying the operational system of record from which this storefront record was sourced or last updated (e.g., SFCC for the e-commerce platform, MDM for the customer master data system, MANUAL for manually entered records). Used for data lineage tracking, reconciliation, and master data governance within the Silver Layer lakehouse.. Valid values are `SFCC|ORMS|MDM|SAP|MANUAL`',
    ...
    `supported_payment_methods` STRING COMMENT 'Comma-delimited list of payment methods accepted at checkout on this storefront (e.g., credit_card,debit_card,paypal,apple_pay,google_pay,gift_card,buy_now_pay_later). Governs payment gateway configuration and PCI DSS compliance scope for this channel. Managed within the e-commerce platform payment processor settings. [ENUM-REF-CANDIDATE: credit_card|debit_card|paypal|apple_pay|google_pay|gift_card|buy_now_pay_later|bank_transfer — promote to reference product]',
    `tax_jurisdiction_code` STRING COMMENT 'Code identifying the primary tax jurisdiction applicable to this storefront for sales tax, VAT, or GST calculation purposes (e.g., US-CA, CA-ON, GB). Used by the tax engine integrated with the e-commerce platform to apply correct tax rates at checkout. Aligns with FASB ASC 606 revenue recognition and FTC pricing transparency requirements.',
    `theme_code` STRING COMMENT 'Identifier referencing the visual theme or template applied to this storefront within the e-commerce platform (e.g., sfra-retail-v3, pwa-kit-2024). Controls the storefronts UI/UX presentation layer, including layout, typography, and brand styling. Used by digital merchandising and visual merchandising teams for planogram-equivalent digital shelf configuration.',
    `time_zone` STRING COMMENT 'IANA time zone identifier for the storefronts primary operating time zone (e.g., America/New_York, America/Los_Angeles). Used for scheduling promotional campaigns, flash sales, maintenance windows, and business-hours-based SLA calculations. Aligns with the e-commerce platform site time zone configuration.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` (
    ...
    `last_published_timestamp` TIMESTAMP COMMENT 'The exact date and time when this digital catalog record was most recently published or re-published to the live storefront. Used for content freshness tracking, cache invalidation, and audit trails in the e-commerce platform.',
    `locale_code` STRING COMMENT 'IETF BCP 47 locale code specifying the language and regional variant for this digital catalog record (e.g., en-US, fr-CA, es-MX). Supports multi-locale storefront content management and international e-commerce operations in the e-commerce platform.. Valid values are `^[a-z]{2}-[A-Z]{2}$`',
    `long_description` STRING COMMENT 'Full consumer-facing product description displayed on the product detail page (PDP), including features, benefits, materials, and usage instructions. Supports rich HTML formatting. Managed in the e-commerce platform content management.',
    ...
    `publish_start_date` DATE COMMENT 'The date on which this digital catalog record is scheduled to become or became active and visible on the storefront. Used for pre-launch product scheduling and promotional launch coordination in the e-commerce platform.',
    ...
    `seo_title` STRING COMMENT 'The HTML page title tag for the product detail page (PDP), optimized for search engine indexing. Typically limited to 60 characters per SEO best practices. Managed in the e-commerce platform content management.',
    ...
    `source_system_code` STRING COMMENT 'Code identifying the operational system of record from which this digital catalog record originated or was last updated. Supports data lineage tracking across the e-commerce platform (SFCC), Oracle Retail Merchandising System (ORMS), the customer master data system, and Oracle Retail Price Management (RPM).. Valid values are `SFCC|ORMS|MDM|RPM|MANUAL`',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`web_session` (
    ...
    `is_cart_created` BOOLEAN COMMENT 'Indicates whether the visitor created a shopping cart (added at least one item) during this session. True = cart was created. Key funnel stage flag for CR (Conversion Rate) and abandoned cart analysis in the e-commerce platform.',
    ...
    `session_token` STRING COMMENT 'Externally-known alphanumeric session identifier generated by the e-commerce platform (SFCC) at session initiation. Used for cross-system session correlation and digital journey stitching across touchpoints.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` (
    ...
    `sku_id` BIGINT COMMENT 'Reference to the master product record associated with this cart line item. Links to the product catalog managed in the customer master data system and Oracle Retail Merchandising System (ORMS).',
    ...
    `web_session_id` BIGINT COMMENT 'The web or app session identifier active when this item was added to the cart. Used for digital customer journey tracking, Click-Through Rate (CTR) attribution, and abandoned cart session-level recovery workflows in the e-commerce platform.',
    ...
    `add_to_cart_timestamp` TIMESTAMP COMMENT 'The date and time when the customer added this item to the shopping cart. Principal business event timestamp for this entity. Used for abandoned cart analysis, session duration analytics, and digital customer journey tracking in the e-commerce platform.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`checkout` (
    ...
    `web_session_id` BIGINT COMMENT 'Unique identifier for the digital browsing session in which this checkout was initiated. Sourced from the e-commerce platform session tracking. Enables online-to-offline attribution and digital customer journey analysis.',
    ...
    `checkout_number` STRING COMMENT 'Human-readable, externally visible reference number for the checkout session. Used in customer communications, customer service case management, and cross-system reconciliation with the e-commerce platform. Format: CHK- followed by 10 digits.. Valid values are `^CHK-[0-9]{10}$`',
    ...
    `current_step` STRING COMMENT 'The most recent checkout funnel step reached by the customer. Tracks progression through: cart_review → address_entry → shipping_selection → payment_entry → order_review → order_placed. Core field for Conversion Rate (CR) funnel step analysis in the e-commerce platform.. Valid values are `cart_review|address_entry|shipping_selection|payment_entry|order_review|order_placed`',
    ...
    `shipping_method_code` STRING COMMENT 'Code identifying the selected shipping method (e.g., STANDARD_5DAY, EXPRESS_2DAY, OVERNIGHT, SAME_DAY). Sourced from the e-commerce platform shipping method catalog. Applicable when fulfillment_mode is ship_to_home or drop_ship. Null for BOPIS/ROPIS.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` (
    ...
    `payment_method_id` DECIMAL(18,2) COMMENT 'Foreign key linking to customer.payment_method. Business justification: Links payment transaction to stored payment instrument. Business need: payment method performance analysis (authorization rates by card type), fraud pattern detection by instrument, PCI compliance aud',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`product_review` (
    ...
    `storefront_id` BIGINT COMMENT 'Reference to the digital storefront or e-commerce site on which the review was submitted. Supports multi-banner and multi-locale storefront segmentation within the e-commerce platform.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` (
    ...
    `storefront_id` BIGINT COMMENT 'Reference to the digital storefront on which this test is executed. Links to the e-commerce storefront master record in the e-commerce platform.',
    ...
    `test_code` STRING COMMENT 'Externally-known alphanumeric business identifier for the test, used in downstream analytics pipelines, tagging systems, and the e-commerce platform experiment references.. Valid values are `^[A-Z0-9_-]{3,30}$`',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` (
    ...
    `sku_id` BIGINT COMMENT 'Reference to the internal SKU (Stock Keeping Unit) that this marketplace listing represents. Links the marketplace listing to Retails internal product master record managed in Oracle Retail Merchandising System (ORMS) and the customer master data system.',
    ...
    `brand_name` STRING COMMENT 'The brand name associated with the marketplace listing. Used for brand registry enforcement, private label identification, and marketplace brand analytics. Sourced from the customer master data system product golden record.',
    ...
    `ean` STRING COMMENT 'The 13-digit European Article Number (EAN) / GTIN-13 for the listed product. Required for international marketplace listings (e.g., eBay international, Google Shopping). Sourced from the customer master data system product golden record.. Valid values are `^[0-9]{13}$`',
    ...
    `last_sync_timestamp` TIMESTAMP COMMENT 'The timestamp of the most recent successful data synchronization between Retails internal systems (the e-commerce platform / the customer master data system) and the marketplace platform API. Used to detect stale listings and trigger re-sync workflows. Format: yyyy-MM-ddTHH:mm:ss.SSSXXX.',
    ...
    `shipping_weight_kg` DECIMAL(18,2) COMMENT 'The shipping weight of the product in kilograms as declared on the marketplace listing. Used for marketplace fulfillment fee calculation (FBA, WFS), shipping cost estimation, and hazmat classification. Sourced from the customer master data system product master.',
    ...
    `upc` STRING COMMENT 'The 12-digit Universal Product Code (UPC) associated with this listing. Used by marketplaces for product catalog matching and compliance. Sourced from the customer master data system product golden record.. Valid values are `^[0-9]{12}$`',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` (
    ...
    `source_system_code` STRING COMMENT 'Code identifying the operational system of record where this personalization rule originated (e.g., SFCC for the e-commerce platform).',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` (
    ...
    `rank` STRING COMMENT 'Position or rank of this recommended product within the recommendation set. Lower numbers indicate higher priority or relevance.',
    ...
    `source` STRING COMMENT 'Source or generation method of the recommendation. Indicates whether the recommendation was generated in real-time, from batch processing, or as a fallback.. Valid values are `real_time|batch|cached|fallback|manual`',
    `source_system_code` STRING COMMENT 'Identifier of the source system that generated or recorded this recommendation event. Typically the e-commerce platform or recommendation engine identifier.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` (
    ...
    `source_system_code` STRING COMMENT 'Code identifying the operational system of record that originated this banner data (e.g., SFCC for the e-commerce platform, CMS for Content Management System).',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` (
    `storefront_assortment_id` BIGINT COMMENT 'Unique identifier for the storefront assortment record',
    `ab_test_id` BIGINT COMMENT 'Reference to an A/B test if this assortment is part of an experiment',
    `assortment_plan_id` BIGINT COMMENT 'Reference to the merchandising assortment plan being published',
    `category_id` BIGINT COMMENT 'Reference to the primary merchandise category for this assortment',
    `price_zone_id` BIGINT COMMENT 'Reference to the pricing zone for price display',
    `associate_id` BIGINT COMMENT 'Reference to the associate who published this assortment',
    `season_id` BIGINT COMMENT 'Reference to the merchandising season this assortment covers',
    `storefront_id` BIGINT COMMENT 'Reference to the digital storefront displaying this assortment',
    `active_sku_count` STRING COMMENT 'Current count of active SKUs published in this storefront assortment.',
    `actual_sku_count` STRING COMMENT 'Actual number of SKUs currently in this assortment',
    `add_to_cart_rate` DECIMAL(18,2) COMMENT 'Rate at which viewed items in this assortment are added to cart.',
    `approval_status` STRING COMMENT 'Approval status of the assortment publication (draft, pending, approved, live).',
    `assortment_status` STRING COMMENT 'Operational status (e.g. active, paused, pending_review)',
    `assortment_type` STRING COMMENT 'Type of assortment such as curated, algorithmic, seasonal, or clearance.',
    `average_selling_price` DECIMAL(18,2) COMMENT 'Average selling price of items in this storefront assortment.',
    `conversion_rate` DECIMAL(18,2) COMMENT 'Measured conversion rate for this assortment configuration',
    ...
    `display_sort_order` STRING COMMENT 'Default sort order for products (e.g. relevance, price_asc, new_arrivals)',
    `effective_end_date` DATE COMMENT 'Date when this assortment is removed from the storefront',
    `effective_start_date` DATE COMMENT 'Date when this assortment becomes visible on the storefront',
    `exclusion_count` STRING COMMENT 'Number of SKUs explicitly excluded from this assortment.',
    `exclusion_list_count` STRING COMMENT 'Number of SKUs explicitly excluded from this assortment.',
    `exclusion_reason_code` STRING COMMENT 'Reason code if the assortment is restricted or unpublished.',
    `exclusion_rule` STRING COMMENT 'Business rule expression defining product exclusions',
    `geo_market_code` STRING COMMENT 'Geographic market code for regional assortment targeting',
    `inventory_threshold_qty` STRING COMMENT 'Minimum inventory quantity required to display a product',
    `last_sync_timestamp` TIMESTAMP COMMENT 'Timestamp of the last synchronization with the product catalog',
    `locale_code` STRING COMMENT 'Locale or language code for which this assortment is configured.',
    `localized_assortment_strategy` STRING COMMENT 'Numeric strategy weight for localized assortment optimization',
    `localized_otb_budget_amount` DECIMAL(18,2) COMMENT 'Open-to-buy budget localized for this storefront market',
    `max_items_displayed` STRING COMMENT 'Maximum number of items displayed per page or section',
    `merchandising_theme` STRING COMMENT 'Merchandising theme or collection name associated with this assortment.',
    `notes` STRING COMMENT 'Free-text notes about assortment strategy or seasonal adjustments.',
    `out_of_stock_handling` STRING COMMENT 'Strategy for handling out-of-stock items such as hide, show_notify, or substitute.',
    `page_view_count` STRING COMMENT 'Total page views for items in this assortment during the active period.',
    `personalization_enabled_flag` BOOLEAN COMMENT 'Indicates if personalized product ordering is enabled',
    `priority_rank` STRING COMMENT 'Display priority when multiple assortments overlap',
    `publication_status` STRING COMMENT 'Current publication status (e.g. draft, scheduled, live, archived)',
    `published_date` DATE COMMENT 'Date when the assortment was published to the storefront',
    `refresh_frequency` STRING COMMENT 'How often the assortment is refreshed (e.g. daily, weekly, real_time)',
    `region_code` STRING COMMENT 'Geographic region code for localized assortment.',
    `return_rate` DECIMAL(18,2) COMMENT 'Return rate for items sold from this assortment.',
    `revenue_target_amount` DECIMAL(18,2) COMMENT 'Revenue target for this storefront assortment',
    `sku_count_target` STRING COMMENT 'Target number of SKUs to display in this assortment',
    `total_revenue_amount` DECIMAL(18,2) COMMENT 'Total revenue generated by this assortment since publication.',
...
) COMMENT 'Links merchandising assortment plans to digital storefronts, controlling which products are visible online with geo-market targeting, personalization settings, inventory thresholds, and conversion tracking.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`ecommerce`.`message_template` (
    ...
    `message_last_modified_by_user_associate_id` BIGINT COMMENT 'Identifier of the user who last modified this message template.',
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ADD CONSTRAINT `fk_ecommerce_checkout_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ADD CONSTRAINT `fk_ecommerce_digital_payment_cart_id` FOREIGN KEY (`cart_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`cart`(`cart_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ADD CONSTRAINT `fk_ecommerce_digital_payment_checkout_id` FOREIGN KEY (`checkout_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`checkout`(`checkout_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ADD CONSTRAINT `fk_ecommerce_digital_payment_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ADD CONSTRAINT `fk_ecommerce_product_page_view_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ADD CONSTRAINT `fk_ecommerce_product_page_view_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ADD CONSTRAINT `fk_ecommerce_search_query_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ADD CONSTRAINT `fk_ecommerce_search_query_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ADD CONSTRAINT `fk_ecommerce_product_review_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ADD CONSTRAINT `fk_ecommerce_ab_test_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ADD CONSTRAINT `fk_ecommerce_ab_test_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ADD CONSTRAINT `fk_ecommerce_abandoned_cart_recovery_cart_id` FOREIGN KEY (`cart_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`cart`(`cart_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ADD CONSTRAINT `fk_ecommerce_abandoned_cart_recovery_checkout_id` FOREIGN KEY (`checkout_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`checkout`(`checkout_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ADD CONSTRAINT `fk_ecommerce_abandoned_cart_recovery_message_template_id` FOREIGN KEY (`message_template_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`message_template`(`message_template_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ADD CONSTRAINT `fk_ecommerce_abandoned_cart_recovery_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ADD CONSTRAINT `fk_ecommerce_personalization_rule_ab_test_id` FOREIGN KEY (`ab_test_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`ab_test`(`ab_test_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ADD CONSTRAINT `fk_ecommerce_personalization_rule_fallback_personalization_rule_id` FOREIGN KEY (`fallback_personalization_rule_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`personalization_rule`(`personalization_rule_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ADD CONSTRAINT `fk_ecommerce_personalization_rule_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ADD CONSTRAINT `fk_ecommerce_recommendation_cart_id` FOREIGN KEY (`cart_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`cart`(`cart_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ADD CONSTRAINT `fk_ecommerce_recommendation_personalization_rule_id` FOREIGN KEY (`personalization_rule_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`personalization_rule`(`personalization_rule_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ADD CONSTRAINT `fk_ecommerce_recommendation_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ADD CONSTRAINT `fk_ecommerce_recommendation_superseded_recommendation_id` FOREIGN KEY (`superseded_recommendation_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`recommendation`(`recommendation_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ADD CONSTRAINT `fk_ecommerce_recommendation_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ADD CONSTRAINT `fk_ecommerce_promotion_banner_replaced_promotion_banner_id` FOREIGN KEY (`replaced_promotion_banner_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`promotion_banner`(`promotion_banner_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ADD CONSTRAINT `fk_ecommerce_promotion_banner_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ADD CONSTRAINT `fk_ecommerce_site_notification_follow_up_site_notification_id` FOREIGN KEY (`follow_up_site_notification_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`site_notification`(`site_notification_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ADD CONSTRAINT `fk_ecommerce_site_notification_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ADD CONSTRAINT `fk_ecommerce_site_notification_web_session_id` FOREIGN KEY (`web_session_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`web_session`(`web_session_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ADD CONSTRAINT `fk_ecommerce_storefront_fulfillment_network_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ADD CONSTRAINT `fk_ecommerce_storefront_assortment_ab_test_id` FOREIGN KEY (`ab_test_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`ab_test`(`ab_test_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ADD CONSTRAINT `fk_ecommerce_storefront_assortment_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ADD CONSTRAINT `fk_ecommerce_storefront_responsibility_storefront_id` FOREIGN KEY (`storefront_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`storefront`(`storefront_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ADD CONSTRAINT `fk_ecommerce_catalog_node_inventory_digital_catalog_id` FOREIGN KEY (`digital_catalog_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`digital_catalog`(`digital_catalog_id`);
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ADD CONSTRAINT `fk_ecommerce_message_template_parent_message_template_id` FOREIGN KEY (`parent_message_template_id`) REFERENCES `vibe_retail_v1`.`ecommerce`.`message_template`(`message_template_id`);

-- ========= TAGS =========
ALTER SCHEMA `vibe_retail_v1`.`ecommerce` SET TAGS ('dbx_division' = 'business');
ALTER SCHEMA `vibe_retail_v1`.`ecommerce` SET TAGS ('dbx_domain' = 'ecommerce');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` SET TAGS ('dbx_subdomain' = 'channel_management');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `digital_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Product Catalog ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `inventory_node_id` SET TAGS ('dbx_business_glossary_term' = 'Inventory Node Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `pci_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Pci Assessment Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Storefront Country Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Storefront Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `customer_service_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `default_language_code` SET TAGS ('dbx_business_glossary_term' = 'Default Language Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_drop_ship_enabled` SET TAGS ('dbx_business_glossary_term' = 'Drop Ship Fulfillment Enabled Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_loyalty_enabled` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Program Enabled Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_ropis_enabled` SET TAGS ('dbx_business_glossary_term' = 'Reserve Online Pick Up In Store (ROPIS) Enabled Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_ship_from_store_enabled` SET TAGS ('dbx_business_glossary_term' = 'Ship-from-Store (SFS) Enabled Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_ship_to_home_enabled` SET TAGS ('dbx_business_glossary_term' = 'Ship-to-Home Fulfillment Enabled Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `is_tax_inclusive_pricing` SET TAGS ('dbx_business_glossary_term' = 'Tax-Inclusive Pricing Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `launch_date` SET TAGS ('dbx_business_glossary_term' = 'Storefront Launch Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Storefront Locale Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `locale_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}_[A-Z]{2}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `max_cart_line_items` SET TAGS ('dbx_business_glossary_term' = 'Maximum Cart Line Items');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `storefront_name` SET TAGS ('dbx_business_glossary_term' = 'Storefront Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `storefront_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `supported_payment_methods` SET TAGS ('dbx_business_glossary_term' = 'Supported Payment Methods');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `tax_jurisdiction_code` SET TAGS ('dbx_business_glossary_term' = 'Tax Jurisdiction Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `theme_code` SET TAGS ('dbx_business_glossary_term' = 'Storefront Theme ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront` ALTER COLUMN `time_zone` SET TAGS ('dbx_business_glossary_term' = 'Storefront Time Zone');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` SET TAGS ('dbx_subdomain' = 'channel_management');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `digital_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Digital Catalog ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `sku_price_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Price Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `age_restriction` SET TAGS ('dbx_business_glossary_term' = 'Age Restriction');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `age_restriction` SET TAGS ('dbx_value_regex' = 'none|18+|21+');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `brand_name` SET TAGS ('dbx_business_glossary_term' = 'Brand Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `canonical_url` SET TAGS ('dbx_business_glossary_term' = 'Canonical URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `content_completeness_score` SET TAGS ('dbx_business_glossary_term' = 'Digital Content Completeness Score');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `has_video` SET TAGS ('dbx_business_glossary_term' = 'Has Product Video Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `hazmat_flag` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Material (Hazmat) Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `image_count` SET TAGS ('dbx_business_glossary_term' = 'Product Image Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `is_drop_ship_eligible` SET TAGS ('dbx_business_glossary_term' = 'Drop Ship Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `is_featured` SET TAGS ('dbx_business_glossary_term' = 'Featured Product Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `is_online_available` SET TAGS ('dbx_business_glossary_term' = 'Online Availability Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `is_private_label` SET TAGS ('dbx_business_glossary_term' = 'Private Label Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `is_searchable` SET TAGS ('dbx_business_glossary_term' = 'Searchable Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `last_published_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Published Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Locale Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `locale_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}-[A-Z]{2}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `long_description` SET TAGS ('dbx_business_glossary_term' = 'Long Product Description');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `primary_image_url` SET TAGS ('dbx_business_glossary_term' = 'Primary Product Image URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `product_name` SET TAGS ('dbx_business_glossary_term' = 'Digital Product Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `product_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `publication_status` SET TAGS ('dbx_business_glossary_term' = 'Publication Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `publication_status` SET TAGS ('dbx_value_regex' = 'draft|published|unpublished|scheduled|archived|suspended');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `publish_end_date` SET TAGS ('dbx_business_glossary_term' = 'Publish End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `publish_start_date` SET TAGS ('dbx_business_glossary_term' = 'Publish Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `rating_average` SET TAGS ('dbx_business_glossary_term' = 'Average Customer Rating');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `review_count` SET TAGS ('dbx_business_glossary_term' = 'Customer Review Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `seo_meta_description` SET TAGS ('dbx_business_glossary_term' = 'Search Engine Optimization (SEO) Meta Description');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `seo_title` SET TAGS ('dbx_business_glossary_term' = 'Search Engine Optimization (SEO) Title');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `short_description` SET TAGS ('dbx_business_glossary_term' = 'Short Product Description');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `sort_order` SET TAGS ('dbx_business_glossary_term' = 'Sort Order');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `source_system_code` SET TAGS ('dbx_value_regex' = 'SFCC|ORMS|MDM|RPM|MANUAL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `tax_category_code` SET TAGS ('dbx_business_glossary_term' = 'Tax Category Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `url_slug` SET TAGS ('dbx_business_glossary_term' = 'URL Slug');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_catalog` ALTER COLUMN `url_slug` SET TAGS ('dbx_value_regex' = '^[a-z0-9]+(?:-[a-z0-9]+)*$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` SET TAGS ('dbx_subdomain' = 'shopper_engagement');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `loyalty_program_id` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Program ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `browser_name` SET TAGS ('dbx_business_glossary_term' = 'Browser Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `browser_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `browser_version` SET TAGS ('dbx_business_glossary_term' = 'Browser Version');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `consent_version` SET TAGS ('dbx_business_glossary_term' = 'Consent Version');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `device_type` SET TAGS ('dbx_value_regex' = 'desktop|mobile|tablet|smart_tv|other');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `exit_page_url` SET TAGS ('dbx_business_glossary_term' = 'Exit Page URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `ip_address` SET TAGS ('dbx_business_glossary_term' = 'IP Address');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `is_bot` SET TAGS ('dbx_business_glossary_term' = 'Is Bot');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `is_bounce` SET TAGS ('dbx_business_glossary_term' = 'Is Bounce');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `is_cart_created` SET TAGS ('dbx_business_glossary_term' = 'Is Cart Created');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `is_checkout_initiated` SET TAGS ('dbx_business_glossary_term' = 'Is Checkout Initiated');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `is_order_placed` SET TAGS ('dbx_business_glossary_term' = 'Is Order Placed');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `landing_page_url` SET TAGS ('dbx_business_glossary_term' = 'Landing Page URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `last_search_query` SET TAGS ('dbx_business_glossary_term' = 'Last Search Query');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `operating_system` SET TAGS ('dbx_business_glossary_term' = 'Operating System');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `page_view_count` SET TAGS ('dbx_business_glossary_term' = 'Page View Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `referral_source` SET TAGS ('dbx_business_glossary_term' = 'Referral Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `referral_url` SET TAGS ('dbx_business_glossary_term' = 'Referral URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `search_query_count` SET TAGS ('dbx_business_glossary_term' = 'Search Query Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `session_end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Session End Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `session_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Session Start Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `session_status` SET TAGS ('dbx_business_glossary_term' = 'Session Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `session_token` SET TAGS ('dbx_business_glossary_term' = 'Session Token');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `utm_campaign` SET TAGS ('dbx_business_glossary_term' = 'UTM Campaign');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `utm_content` SET TAGS ('dbx_business_glossary_term' = 'UTM Content');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `utm_medium` SET TAGS ('dbx_business_glossary_term' = 'UTM Medium');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `utm_source` SET TAGS ('dbx_business_glossary_term' = 'UTM Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `utm_term` SET TAGS ('dbx_business_glossary_term' = 'UTM Term');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`web_session` ALTER COLUMN `visitor_type` SET TAGS ('dbx_business_glossary_term' = 'Visitor Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `abandoned_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cart Abandoned Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `abandonment_reason` SET TAGS ('dbx_business_glossary_term' = 'Cart Abandonment Reason');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `abandonment_reason` SET TAGS ('dbx_value_regex' = 'session_timeout|inactivity|explicit_exit|payment_failure|out_of_stock|price_change');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `cart_number` SET TAGS ('dbx_business_glossary_term' = 'Cart Number');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `cart_number` SET TAGS ('dbx_value_regex' = '^CART-[0-9]{10}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `cart_status` SET TAGS ('dbx_business_glossary_term' = 'Cart Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `cart_status` SET TAGS ('dbx_value_regex' = 'active|abandoned|converted|expired|merged');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `channel` SET TAGS ('dbx_business_glossary_term' = 'Commerce Channel');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `channel` SET TAGS ('dbx_value_regex' = 'web|mobile_app|mobile_web|email_link|bopis_kiosk');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `converted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cart Converted Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `coupon_code` SET TAGS ('dbx_business_glossary_term' = 'Coupon Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `coupon_redemption_channel` SET TAGS ('dbx_business_glossary_term' = 'Coupon Redemption Channel');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `coupon_redemption_channel` SET TAGS ('dbx_value_regex' = 'web|mobile_app|email_link|push_notification|sms');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `coupon_redemption_status` SET TAGS ('dbx_business_glossary_term' = 'Coupon Redemption Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `coupon_redemption_status` SET TAGS ('dbx_value_regex' = 'applied|validated|rejected|reversed');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cart Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `estimated_shipping_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Shipping Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `estimated_tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Tax Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Locale Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `locale_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}_[A-Z]{2}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `loyalty_points_redeemed` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Redeemed');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `recovery_email_sent` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `session_id` SET TAGS ('dbx_business_glossary_term' = 'Session ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `utm_campaign` SET TAGS ('dbx_business_glossary_term' = 'UTM Campaign');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart` ALTER COLUMN `utm_source` SET TAGS ('dbx_business_glossary_term' = 'UTM Source');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `cart_item_id` SET TAGS ('dbx_business_glossary_term' = 'Cart Item ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Session ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `add_to_cart_source` SET TAGS ('dbx_business_glossary_term' = 'Add-to-Cart Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `add_to_cart_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Add-to-Cart Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `gift_message` SET TAGS ('dbx_business_glossary_term' = 'Gift Message');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `is_private_label` SET TAGS ('dbx_business_glossary_term' = 'Private Label Indicator');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `item_status` SET TAGS ('dbx_business_glossary_term' = 'Cart Item Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `loyalty_points_earned` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Earned');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `price_override_flag` SET TAGS ('dbx_business_glossary_term' = 'Price Override Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `product_department` SET TAGS ('dbx_business_glossary_term' = 'Product Department');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `promotion_type` SET TAGS ('dbx_business_glossary_term' = 'Promotion Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`cart_item` ALTER COLUMN `promotion_type` SET TAGS ('dbx_value_regex' = 'percentage_off|fixed_amount_off|buy_x_get_y|free_shipping|bundle');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `checkout_id` SET TAGS ('dbx_business_glossary_term' = 'Checkout ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `checkout_number` SET TAGS ('dbx_value_regex' = '^CHK-[0-9]{10}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `checkout_status` SET TAGS ('dbx_business_glossary_term' = 'Checkout Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `checkout_status` SET TAGS ('dbx_value_regex' = 'initiated|in_progress|completed|abandoned|expired');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `completed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Checkout Completed Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Discount Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `fulfillment_mode` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Mode');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `loyalty_points_redeemed` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Points Redeemed');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `order_total_amount` SET TAGS ('dbx_business_glossary_term' = 'Order Total Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `shipping_method_code` SET TAGS ('dbx_business_glossary_term' = 'Shipping Method Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `shipping_method_name` SET TAGS ('dbx_business_glossary_term' = 'Shipping Method Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `shipping_method_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`checkout` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` SET TAGS ('dbx_subdomain' = 'digital_transactions');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `digital_payment_id` SET TAGS ('dbx_business_glossary_term' = 'Digital Payment ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `authorization_code` SET TAGS ('dbx_business_glossary_term' = 'Payment Authorization Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `authorization_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Payment Authorization Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `billing_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Billing Postal Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `card_expiry_month` SET TAGS ('dbx_business_glossary_term' = 'Card Expiry Month');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `card_expiry_year` SET TAGS ('dbx_business_glossary_term' = 'Card Expiry Year');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `card_network` SET TAGS ('dbx_business_glossary_term' = 'Card Network');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `card_network` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `fraud_screening_result` SET TAGS ('dbx_business_glossary_term' = 'Fraud Screening Result');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `gateway_transaction_reference` SET TAGS ('dbx_business_glossary_term' = 'Payment Gateway Transaction ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `initiated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Payment Initiated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `installment_count` SET TAGS ('dbx_business_glossary_term' = 'Buy Now Pay Later (BNPL) Installment Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `payment_amount` SET TAGS ('dbx_business_glossary_term' = 'Payment Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `payment_channel` SET TAGS ('dbx_business_glossary_term' = 'Payment Channel');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `payment_gateway` SET TAGS ('dbx_business_glossary_term' = 'Payment Gateway');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `payment_method_type` SET TAGS ('dbx_business_glossary_term' = 'Payment Method Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `payment_status` SET TAGS ('dbx_business_glossary_term' = 'Payment Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `refund_amount` SET TAGS ('dbx_business_glossary_term' = 'Refund Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `three_ds_version` SET TAGS ('dbx_value_regex' = '1.0|2.1|2.2|2.3');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `transaction_reference` SET TAGS ('dbx_business_glossary_term' = 'Transaction Reference Number');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `wallet_provider` SET TAGS ('dbx_business_glossary_term' = 'Digital Wallet Provider');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`digital_payment` ALTER COLUMN `wallet_provider` SET TAGS ('dbx_value_regex' = 'apple_pay|google_pay|samsung_pay|paypal|venmo|other');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` SET TAGS ('dbx_subdomain' = 'shopper_engagement');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `product_page_view_id` SET TAGS ('dbx_business_glossary_term' = 'Product Page View ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `fulfillment_node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Product ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `brand_name` SET TAGS ('dbx_business_glossary_term' = 'Brand Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `category_path` SET TAGS ('dbx_business_glossary_term' = 'Category Path');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `displayed_price` SET TAGS ('dbx_business_glossary_term' = 'Displayed Price');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `displayed_price` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `interaction_type` SET TAGS ('dbx_business_glossary_term' = 'Interaction Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `interaction_type` SET TAGS ('dbx_value_regex' = 'organic_browse|recommendation_click|search_result_click|direct_url|email_link|promotion_link');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `inventory_status` SET TAGS ('dbx_business_glossary_term' = 'Inventory Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `inventory_status` SET TAGS ('dbx_value_regex' = 'in_stock|low_stock|out_of_stock|backorder|preorder');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_add_to_cart` SET TAGS ('dbx_business_glossary_term' = 'Add to Cart Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_bot` SET TAGS ('dbx_business_glossary_term' = 'Bot Traffic Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_markdown_price` SET TAGS ('dbx_business_glossary_term' = 'Markdown Price Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_recommendation_clicked` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Clicked Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_recommendation_served` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Served Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `is_wishlist_add` SET TAGS ('dbx_business_glossary_term' = 'Wishlist Add Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `page_url` SET TAGS ('dbx_business_glossary_term' = 'Product Detail Page (PDP) URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `personalization_segment` SET TAGS ('dbx_business_glossary_term' = 'Personalization Segment');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `recommendation_algorithm` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Algorithm');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `recommendation_placement` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Placement');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `recommendation_placement` SET TAGS ('dbx_value_regex' = 'pdp_carousel|pdp_bottom|pdp_sidebar|pdp_popup|none');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `referral_source` SET TAGS ('dbx_business_glossary_term' = 'Referral Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `search_query` SET TAGS ('dbx_business_glossary_term' = 'Search Query');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `search_result_rank` SET TAGS ('dbx_business_glossary_term' = 'Search Result Rank');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `sku` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `sku` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{4,30}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `time_on_page_seconds` SET TAGS ('dbx_business_glossary_term' = 'Time on Page (Seconds)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `upc` SET TAGS ('dbx_business_glossary_term' = 'Universal Product Code (UPC)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `upc` SET TAGS ('dbx_value_regex' = '^[0-9]{12}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_page_view` ALTER COLUMN `visitor_type` SET TAGS ('dbx_business_glossary_term' = 'Visitor Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `search_query_id` SET TAGS ('dbx_business_glossary_term' = 'Search Query ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `demand_forecast_id` SET TAGS ('dbx_business_glossary_term' = 'Demand Forecast Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Search Category ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `applied_filter_count` SET TAGS ('dbx_business_glossary_term' = 'Applied Filter Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `applied_sort_order` SET TAGS ('dbx_business_glossary_term' = 'Applied Sort Order');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `clicked_result_position` SET TAGS ('dbx_business_glossary_term' = 'Clicked Result Position');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `device_type` SET TAGS ('dbx_value_regex' = 'desktop|mobile|tablet|kiosk|other');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `geo_country_code` SET TAGS ('dbx_business_glossary_term' = 'Geographic Country Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `geo_country_code` SET TAGS ('dbx_value_regex' = '[A-Z]{3}');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_add_to_cart` SET TAGS ('dbx_business_glossary_term' = 'Add-to-Cart Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_bot` SET TAGS ('dbx_business_glossary_term' = 'Bot Traffic Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_click_through` SET TAGS ('dbx_business_glossary_term' = 'Click-Through Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_purchase` SET TAGS ('dbx_business_glossary_term' = 'Purchase Conversion Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_redirected` SET TAGS ('dbx_business_glossary_term' = 'Search Redirect Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_spell_corrected` SET TAGS ('dbx_business_glossary_term' = 'Spell Correction Applied Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_synonym_expanded` SET TAGS ('dbx_business_glossary_term' = 'Synonym Expansion Applied Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `is_zero_results` SET TAGS ('dbx_business_glossary_term' = 'Zero Results Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `normalized_query_text` SET TAGS ('dbx_business_glossary_term' = 'Normalized Query Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_language_code` SET TAGS ('dbx_business_glossary_term' = 'Query Language Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_language_code` SET TAGS ('dbx_value_regex' = '[a-z]{2}(-[A-Z]{2})?');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_source` SET TAGS ('dbx_business_glossary_term' = 'Query Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_status` SET TAGS ('dbx_business_glossary_term' = 'Query Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_status` SET TAGS ('dbx_value_regex' = 'completed|timeout|error|blocked');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Query Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_type` SET TAGS ('dbx_business_glossary_term' = 'Query Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `query_type` SET TAGS ('dbx_value_regex' = 'keyword|natural_language|barcode_scan|voice|autocomplete_selection');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `raw_query_text` SET TAGS ('dbx_business_glossary_term' = 'Raw Query Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `redirect_url` SET TAGS ('dbx_business_glossary_term' = 'Search Redirect URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `response_time_ms` SET TAGS ('dbx_business_glossary_term' = 'Search Response Time (Milliseconds)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `result_count` SET TAGS ('dbx_business_glossary_term' = 'Search Result Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `result_page_number` SET TAGS ('dbx_business_glossary_term' = 'Result Page Number');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`search_query` ALTER COLUMN `search_engine_version` SET TAGS ('dbx_business_glossary_term' = 'Search Engine Version');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `product_review_id` SET TAGS ('dbx_business_glossary_term' = 'Product Review ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU) ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `abuse_report_count` SET TAGS ('dbx_business_glossary_term' = 'Abuse Report Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `external_review_reference` SET TAGS ('dbx_business_glossary_term' = 'External Review ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `fit_feedback` SET TAGS ('dbx_business_glossary_term' = 'Fit Feedback');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `fit_feedback` SET TAGS ('dbx_value_regex' = 'runs_small|true_to_size|runs_large|not_applicable');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `has_media` SET TAGS ('dbx_business_glossary_term' = 'Has Media Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `helpful_vote_count` SET TAGS ('dbx_business_glossary_term' = 'Helpful Vote Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `incentive_type` SET TAGS ('dbx_business_glossary_term' = 'Incentive Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `incentive_type` SET TAGS ('dbx_value_regex' = 'loyalty_points|discount_coupon|free_product|sweepstakes_entry|none');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `is_deleted` SET TAGS ('dbx_business_glossary_term' = 'Soft Delete Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `is_incentivized` SET TAGS ('dbx_business_glossary_term' = 'Incentivized Review Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `is_verified_purchase` SET TAGS ('dbx_business_glossary_term' = 'Verified Purchase Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `media_attachment_count` SET TAGS ('dbx_business_glossary_term' = 'Media Attachment Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderated_by` SET TAGS ('dbx_business_glossary_term' = 'Moderated By');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Moderation Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderation_notes` SET TAGS ('dbx_business_glossary_term' = 'Moderation Notes');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderation_rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Moderation Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderation_status` SET TAGS ('dbx_business_glossary_term' = 'Moderation Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `moderation_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected|flagged');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `product_size_purchased` SET TAGS ('dbx_business_glossary_term' = 'Product Size Purchased');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `published_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Review Published Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `purchase_channel` SET TAGS ('dbx_business_glossary_term' = 'Purchase Channel');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `review_language_code` SET TAGS ('dbx_business_glossary_term' = 'Review Language Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `review_title` SET TAGS ('dbx_business_glossary_term' = 'Review Title');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `reviewer_display_name` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Display Name');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `reviewer_expertise_level` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Expertise Level');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `reviewer_location_country_code` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Location Country Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `sentiment_label` SET TAGS ('dbx_business_glossary_term' = 'Sentiment Label');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `sentiment_score` SET TAGS ('dbx_business_glossary_term' = 'Sentiment Score');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `star_rating` SET TAGS ('dbx_business_glossary_term' = 'Star Rating');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `submission_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Review Submission Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `syndication_partner_review_reference` SET TAGS ('dbx_business_glossary_term' = 'Syndication Partner Review ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `syndication_source` SET TAGS ('dbx_business_glossary_term' = 'Syndication Source');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`product_review` ALTER COLUMN `unhelpful_vote_count` SET TAGS ('dbx_business_glossary_term' = 'Unhelpful Vote Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` SET TAGS ('dbx_subdomain' = 'digital_transactions');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `privacy_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Privacy Assessment Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `assigned_variant` SET TAGS ('dbx_business_glossary_term' = 'Assigned Variant');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `assigned_variant` SET TAGS ('dbx_value_regex' = 'control|treatment_a|treatment_b|treatment_c');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `assignment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Variant Assignment Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `conclusion_reason` SET TAGS ('dbx_business_glossary_term' = 'Test Conclusion Reason');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `conclusion_reason` SET TAGS ('dbx_value_regex' = 'significance_reached|duration_elapsed|manual_stop|inconclusive|cancelled');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `confidence_level` SET TAGS ('dbx_business_glossary_term' = 'Statistical Confidence Level');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `conversion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Conversion Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `conversion_value` SET TAGS ('dbx_business_glossary_term' = 'Conversion Value');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `device_targeting` SET TAGS ('dbx_business_glossary_term' = 'Device Targeting');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `device_targeting` SET TAGS ('dbx_value_regex' = 'all|desktop|mobile|tablet');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Test End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Test End Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `geo_targeting_country_code` SET TAGS ('dbx_business_glossary_term' = 'Geographic Targeting Country Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `geo_targeting_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `hypothesis` SET TAGS ('dbx_business_glossary_term' = 'Test Hypothesis');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `is_converted` SET TAGS ('dbx_business_glossary_term' = 'Is Converted Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `is_multivariate` SET TAGS ('dbx_business_glossary_term' = 'Is Multivariate Test Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `is_personalized` SET TAGS ('dbx_business_glossary_term' = 'Is Personalized Test Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `minimum_detectable_effect` SET TAGS ('dbx_business_glossary_term' = 'Minimum Detectable Effect (MDE)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `owner_team` SET TAGS ('dbx_business_glossary_term' = 'Test Owner Team');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `page_type_target` SET TAGS ('dbx_business_glossary_term' = 'Page Type Target');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `primary_metric` SET TAGS ('dbx_business_glossary_term' = 'Primary Success Metric');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `sample_size_target` SET TAGS ('dbx_business_glossary_term' = 'Target Sample Size');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `secondary_metric` SET TAGS ('dbx_business_glossary_term' = 'Secondary Success Metric');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `significance_threshold` SET TAGS ('dbx_business_glossary_term' = 'Statistical Significance Threshold');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Test Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Test Start Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `target_audience_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Audience Segment');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_code` SET TAGS ('dbx_business_glossary_term' = 'A/B Test Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_-]{3,30}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_description` SET TAGS ('dbx_business_glossary_term' = 'Test Description');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_name` SET TAGS ('dbx_business_glossary_term' = 'A/B Test Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_status` SET TAGS ('dbx_business_glossary_term' = 'A/B Test Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_status` SET TAGS ('dbx_value_regex' = 'draft|running|paused|concluded|cancelled');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_type` SET TAGS ('dbx_business_glossary_term' = 'A/B Test Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `test_type` SET TAGS ('dbx_value_regex' = 'layout|pricing|copy|recommendation|navigation|personalization');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `total_traffic_allocation_pct` SET TAGS ('dbx_business_glossary_term' = 'Total Traffic Allocation Percentage');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `variant_count` SET TAGS ('dbx_business_glossary_term' = 'Variant Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`ab_test` ALTER COLUMN `winning_variant` SET TAGS ('dbx_business_glossary_term' = 'Winning Variant');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `abandoned_cart_recovery_id` SET TAGS ('dbx_business_glossary_term' = 'Abandoned Cart Recovery ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `abandoned_cart_item_count` SET TAGS ('dbx_business_glossary_term' = 'Abandoned Cart Item Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Recovery Device Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `device_type` SET TAGS ('dbx_value_regex' = 'desktop|mobile|tablet|unknown');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `incentive_type` SET TAGS ('dbx_business_glossary_term' = 'Recovery Incentive Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `incentive_type` SET TAGS ('dbx_value_regex' = 'none|percentage_discount|fixed_discount|free_shipping|loyalty_points|gift_with_purchase');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `incentive_value` SET TAGS ('dbx_business_glossary_term' = 'Recovery Incentive Value');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `is_first_recovery_attempt` SET TAGS ('dbx_business_glossary_term' = 'Is First Recovery Attempt Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `is_incentive_redeemed` SET TAGS ('dbx_business_glossary_term' = 'Is Incentive Redeemed Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `message_clicked_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Recovery Message Clicked Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `message_opened_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Recovery Message Opened Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `message_sent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Recovery Message Sent Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovered_order_discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Recovered Order Discount Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovered_order_net_amount` SET TAGS ('dbx_business_glossary_term' = 'Recovered Order Net Amount');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovery_channel` SET TAGS ('dbx_business_glossary_term' = 'Recovery Channel');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovery_expiry_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Recovery Expiry Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovery_sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Recovery Sequence Number');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovery_status` SET TAGS ('dbx_business_glossary_term' = 'Recovery Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `recovery_workflow_number` SET TAGS ('dbx_business_glossary_term' = 'Recovery Workflow Number');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `trigger_delay_minutes` SET TAGS ('dbx_business_glossary_term' = 'Recovery Trigger Delay (Minutes)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `utm_campaign` SET TAGS ('dbx_business_glossary_term' = 'UTM Campaign');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`abandoned_cart_recovery` ALTER COLUMN `utm_source` SET TAGS ('dbx_business_glossary_term' = 'UTM Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` SET TAGS ('dbx_subdomain' = 'channel_management');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_listing_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Listing ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Buyer Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Location Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU) ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `age_restriction_flag` SET TAGS ('dbx_business_glossary_term' = 'Age Restriction Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `brand_name` SET TAGS ('dbx_business_glossary_term' = 'Brand Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `content_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Content Compliance Flag');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `ean` SET TAGS ('dbx_business_glossary_term' = 'European Article Number (EAN)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `ean` SET TAGS ('dbx_value_regex' = '^[0-9]{13}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `fulfillment_method` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Fulfillment Method');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `fulfillment_method` SET TAGS ('dbx_value_regex' = 'fbm|fba|wfs|sfs|dropship|3pl');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `hazmat_flag` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Material (Hazmat) Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `image_count` SET TAGS ('dbx_business_glossary_term' = 'Listing Image Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `inventory_quantity` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Inventory Quantity');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `is_buy_box_owner` SET TAGS ('dbx_business_glossary_term' = 'Buy Box Ownership Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `is_private_label` SET TAGS ('dbx_business_glossary_term' = 'Private Label Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `last_sync_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Synchronization Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Listing Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listing_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Listing Quality Score');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listing_status` SET TAGS ('dbx_business_glossary_term' = 'Listing Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listing_status` SET TAGS ('dbx_value_regex' = 'active|suppressed|inactive|out_of_stock|pending_review|removed');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listing_title` SET TAGS ('dbx_business_glossary_term' = 'Listing Title');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `listing_url` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Listing URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Locale Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_platform` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Platform');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_platform` SET TAGS ('dbx_value_regex' = 'amazon|ebay|walmart_marketplace|google_shopping|target_plus|other');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_price` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Price');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_storefront_name` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Storefront Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `marketplace_storefront_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `promotion_eligibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Promotion Eligibility Flag');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `sell_through_rate` SET TAGS ('dbx_business_glossary_term' = 'Sell-Through Rate');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `shipping_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Shipping Weight (kg)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `suppression_reason` SET TAGS ('dbx_business_glossary_term' = 'Listing Suppression Reason');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`marketplace_listing` ALTER COLUMN `upc` SET TAGS ('dbx_business_glossary_term' = 'Universal Product Code (UPC)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `personalization_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Personalization Rule ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Segment ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `fallback_personalization_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Fallback Personalization Rule Id');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `algorithm_type` SET TAGS ('dbx_business_glossary_term' = 'Algorithm Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `attributed_revenue_amount` SET TAGS ('dbx_business_glossary_term' = 'Attributed Revenue Amount');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `click_count` SET TAGS ('dbx_business_glossary_term' = 'Click Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `confidence_threshold` SET TAGS ('dbx_business_glossary_term' = 'Confidence Threshold');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `conversion_count` SET TAGS ('dbx_business_glossary_term' = 'Conversion Count');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `ctr` SET TAGS ('dbx_business_glossary_term' = 'Click-Through Rate (CTR)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `device_type_filter` SET TAGS ('dbx_business_glossary_term' = 'Device Type Filter');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `exclusion_criteria` SET TAGS ('dbx_business_glossary_term' = 'Exclusion Criteria');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `fallback_strategy` SET TAGS ('dbx_business_glossary_term' = 'Fallback Strategy');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `fallback_strategy` SET TAGS ('dbx_value_regex' = 'bestsellers|new_arrivals|trending|category_default|manual_curation|none');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `geo_country_codes` SET TAGS ('dbx_business_glossary_term' = 'Geographic Country Codes');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `impression_count` SET TAGS ('dbx_business_glossary_term' = 'Impression Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `is_device_specific` SET TAGS ('dbx_business_glossary_term' = 'Is Device Specific');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `is_geo_targeted` SET TAGS ('dbx_business_glossary_term' = 'Is Geo Targeted');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `last_executed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Executed Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `max_recommendation_count` SET TAGS ('dbx_business_glossary_term' = 'Maximum Recommendation Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `min_recommendation_count` SET TAGS ('dbx_business_glossary_term' = 'Minimum Recommendation Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `placement_location` SET TAGS ('dbx_business_glossary_term' = 'Placement Location');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `placement_location` SET TAGS ('dbx_value_regex' = 'homepage|pdp|cart|checkout|category_page|search_results');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `placement_zone` SET TAGS ('dbx_business_glossary_term' = 'Placement Zone');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Priority Rank');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `rule_code` SET TAGS ('dbx_business_glossary_term' = 'Rule Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `rule_name` SET TAGS ('dbx_business_glossary_term' = 'Rule Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `rule_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `rule_status` SET TAGS ('dbx_business_glossary_term' = 'Rule Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `rule_type` SET TAGS ('dbx_business_glossary_term' = 'Rule Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `targeting_criteria` SET TAGS ('dbx_business_glossary_term' = 'Targeting Criteria');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `updated_by` SET TAGS ('dbx_business_glossary_term' = 'Updated By');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`personalization_rule` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `recommendation_id` SET TAGS ('dbx_business_glossary_term' = 'Recommendation ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `cart_id` SET TAGS ('dbx_business_glossary_term' = 'Cart ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `personalization_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Personalization Rule Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `superseded_recommendation_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Recommendation Id');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `added_to_cart_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Added to Cart Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `brand_name` SET TAGS ('dbx_business_glossary_term' = 'Brand Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `brand_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `category_path` SET TAGS ('dbx_business_glossary_term' = 'Category Path');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `context` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Context');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `device_type` SET TAGS ('dbx_value_regex' = 'desktop|mobile|tablet|app|other');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `displayed_price` SET TAGS ('dbx_business_glossary_term' = 'Displayed Price');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `inventory_status` SET TAGS ('dbx_business_glossary_term' = 'Inventory Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `inventory_status` SET TAGS ('dbx_value_regex' = 'in_stock|low_stock|out_of_stock|backorder|preorder|discontinued');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `is_added_to_cart` SET TAGS ('dbx_business_glossary_term' = 'Is Added to Cart Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `is_clicked` SET TAGS ('dbx_business_glossary_term' = 'Is Clicked Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `is_private_label` SET TAGS ('dbx_business_glossary_term' = 'Is Private Label Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `is_purchased` SET TAGS ('dbx_business_glossary_term' = 'Is Purchased Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `model_version` SET TAGS ('dbx_business_glossary_term' = 'Model Version');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `personalization_segment` SET TAGS ('dbx_business_glossary_term' = 'Personalization Segment');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `placement` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Placement');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `purchased_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Purchased Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `rank` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Rank');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `score` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Score');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `served_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Served Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `source` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Source');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `source` SET TAGS ('dbx_value_regex' = 'real_time|batch|cached|fallback|manual');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `strategy` SET TAGS ('dbx_business_glossary_term' = 'Recommendation Strategy');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `time_to_click_seconds` SET TAGS ('dbx_business_glossary_term' = 'Time to Click (Seconds)');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`recommendation` ALTER COLUMN `visitor_type` SET TAGS ('dbx_business_glossary_term' = 'Visitor Type');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `promotion_banner_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Banner ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `replaced_promotion_banner_id` SET TAGS ('dbx_business_glossary_term' = 'Replaced Promotion Banner Id');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `attributed_revenue` SET TAGS ('dbx_business_glossary_term' = 'Attributed Revenue');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_description` SET TAGS ('dbx_business_glossary_term' = 'Banner Description');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_name` SET TAGS ('dbx_business_glossary_term' = 'Banner Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_status` SET TAGS ('dbx_business_glossary_term' = 'Banner Status');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_title` SET TAGS ('dbx_business_glossary_term' = 'Banner Title');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_type` SET TAGS ('dbx_business_glossary_term' = 'Banner Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `banner_type` SET TAGS ('dbx_value_regex' = 'hero|carousel|sidebar|footer|popup|interstitial');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `call_to_action_text` SET TAGS ('dbx_business_glossary_term' = 'Call to Action (CTA) Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `click_count` SET TAGS ('dbx_business_glossary_term' = 'Click Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `click_through_url` SET TAGS ('dbx_business_glossary_term' = 'Click-Through URL (Uniform Resource Locator)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `conversion_count` SET TAGS ('dbx_business_glossary_term' = 'Conversion Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `creative_asset_url` SET TAGS ('dbx_business_glossary_term' = 'Creative Asset URL (Uniform Resource Locator)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `device_targeting` SET TAGS ('dbx_business_glossary_term' = 'Device Targeting');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `display_priority` SET TAGS ('dbx_business_glossary_term' = 'Display Priority');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'End Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `impression_count` SET TAGS ('dbx_business_glossary_term' = 'Impression Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `is_personalized` SET TAGS ('dbx_business_glossary_term' = 'Is Personalized');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Locale Code');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `placement_zone` SET TAGS ('dbx_business_glossary_term' = 'Placement Zone');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Start Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `target_audience_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Audience Segment');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `updated_by` SET TAGS ('dbx_business_glossary_term' = 'Updated By');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`promotion_banner` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `site_notification_id` SET TAGS ('dbx_business_glossary_term' = 'Site Notification ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `follow_up_site_notification_id` SET TAGS ('dbx_business_glossary_term' = 'Follow Up Site Notification Id');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Shipment Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Product ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `web_session_id` SET TAGS ('dbx_business_glossary_term' = 'Web Session ID');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `call_to_action_text` SET TAGS ('dbx_business_glossary_term' = 'Call to Action (CTA) Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `call_to_action_url` SET TAGS ('dbx_business_glossary_term' = 'Call to Action (CTA) URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `clicked_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Clicked Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `current_price` SET TAGS ('dbx_business_glossary_term' = 'Current Price');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `data_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Consent Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `delivered_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Delivered Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `delivery_channel` SET TAGS ('dbx_business_glossary_term' = 'Delivery Channel');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `device_type` SET TAGS ('dbx_value_regex' = 'desktop|mobile|tablet|app');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `discount_percentage` SET TAGS ('dbx_business_glossary_term' = 'Discount Percentage');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `dismissed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Dismissed Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `expiry_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Expiry Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `image_url` SET TAGS ('dbx_business_glossary_term' = 'Image URL');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `is_clicked` SET TAGS ('dbx_business_glossary_term' = 'Is Clicked Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `is_dismissed` SET TAGS ('dbx_business_glossary_term' = 'Is Dismissed Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `is_personalized` SET TAGS ('dbx_business_glossary_term' = 'Is Personalized Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `is_read` SET TAGS ('dbx_business_glossary_term' = 'Is Read Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `locale_code` SET TAGS ('dbx_business_glossary_term' = 'Locale Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `message_body` SET TAGS ('dbx_business_glossary_term' = 'Message Body');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `notification_number` SET TAGS ('dbx_business_glossary_term' = 'Notification Number');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `notification_status` SET TAGS ('dbx_business_glossary_term' = 'Notification Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `notification_type` SET TAGS ('dbx_business_glossary_term' = 'Notification Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `personalization_segment` SET TAGS ('dbx_business_glossary_term' = 'Personalization Segment');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `previous_price` SET TAGS ('dbx_business_glossary_term' = 'Previous Price');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `product_name` SET TAGS ('dbx_business_glossary_term' = 'Product Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `product_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `read_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Read Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `sent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Sent Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `sku` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU)');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Notification Title');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `trigger_event` SET TAGS ('dbx_business_glossary_term' = 'Trigger Event');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `triggered_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Triggered Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`site_notification` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` SET TAGS ('dbx_data_type' = 'association_data');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `storefront_fulfillment_network_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Fulfillment Network ID');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `cutoff_time` SET TAGS ('dbx_business_glossary_term' = 'Order Cutoff Time');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `fulfillment_priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Priority Rank');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Active Status Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `max_daily_order_capacity` SET TAGS ('dbx_business_glossary_term' = 'Maximum Daily Order Capacity');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `service_level_agreement` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_fulfillment_network` ALTER COLUMN `shipping_cost_tier` SET TAGS ('dbx_business_glossary_term' = 'Shipping Cost Tier');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` SET TAGS ('dbx_subdomain' = 'channel_management');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` SET TAGS ('dbx_association_edges' = 'merchandising.assortment_plan,ecommerce.storefront');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `storefront_assortment_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Assortment Identifier');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `assortment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Assortment - Assortment Plan Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Publisher');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Assortment - Storefront Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Storefront End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Storefront Launch Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `localized_assortment_strategy` SET TAGS ('dbx_business_glossary_term' = 'Channel Assortment Strategy');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `localized_otb_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Storefront OTB Budget');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Rollout Priority');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `publication_status` SET TAGS ('dbx_business_glossary_term' = 'Publication Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `published_date` SET TAGS ('dbx_business_glossary_term' = 'Publication Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_assortment` ALTER COLUMN `sku_count_target` SET TAGS ('dbx_business_glossary_term' = 'Localized SKU Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` SET TAGS ('dbx_subdomain' = 'channel_management');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` SET TAGS ('dbx_association_edges' = 'merchandising.buyer,ecommerce.storefront');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `storefront_responsibility_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Responsibility Identifier');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `buyer_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Responsibility - Buyer Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `storefront_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Responsibility - Storefront Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `approval_authority_override` SET TAGS ('dbx_business_glossary_term' = 'Storefront Approval Authority Override');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `assigned_date` SET TAGS ('dbx_business_glossary_term' = 'Responsibility Assignment Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `budget_allocation` SET TAGS ('dbx_business_glossary_term' = 'Storefront Budget Allocation');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `category_list` SET TAGS ('dbx_business_glossary_term' = 'Assigned Category List');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Responsibility End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `performance_target` SET TAGS ('dbx_business_glossary_term' = 'Storefront Performance Target');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`storefront_responsibility` ALTER COLUMN `responsibility_scope` SET TAGS ('dbx_business_glossary_term' = 'Responsibility Scope');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `fulfillment_node_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Node Inventory - Fulfillment Node Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `inventory_quantity` SET TAGS ('dbx_business_glossary_term' = 'On-Hand Inventory Quantity');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `is_available_for_bopis` SET TAGS ('dbx_business_glossary_term' = 'BOPIS Availability Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `is_available_for_ship_from_store` SET TAGS ('dbx_business_glossary_term' = 'Ship-From-Store Availability Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `is_bopis_eligible` SET TAGS ('dbx_business_glossary_term' = 'Buy Online Pick Up In Store (BOPIS) Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `is_ship_from_store_eligible` SET TAGS ('dbx_business_glossary_term' = 'Ship From Store (SFS) Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`catalog_node_inventory` ALTER COLUMN `last_inventory_update_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Inventory Update Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `message_last_modified_by_user_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Associate Id');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `parent_message_template_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Message Template Id');
...
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `a_b_test_enabled` SET TAGS ('dbx_business_glossary_term' = 'A B Test Enabled');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `a_b_test_variant` SET TAGS ('dbx_business_glossary_term' = 'A B Test Variant');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `average_click_through_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Average Click Through Rate Percent');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `average_conversion_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Average Conversion Rate Percent');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `average_open_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Average Open Rate Percent');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `body_content` SET TAGS ('dbx_business_glossary_term' = 'Body Content');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `call_to_action_text` SET TAGS ('dbx_business_glossary_term' = 'Call To Action Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `call_to_action_url` SET TAGS ('dbx_business_glossary_term' = 'Call To Action Url');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `character_count` SET TAGS ('dbx_business_glossary_term' = 'Character Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `compliance_reviewed` SET TAGS ('dbx_business_glossary_term' = 'Compliance Reviewed');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `compliance_reviewed_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Reviewed Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `design_template_code` SET TAGS ('dbx_business_glossary_term' = 'Design Template Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `estimated_send_time_seconds` SET TAGS ('dbx_business_glossary_term' = 'Estimated Send Time Seconds');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `last_sent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Sent Timestamp');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `merge_fields` SET TAGS ('dbx_business_glossary_term' = 'Merge Fields');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `message_category` SET TAGS ('dbx_business_glossary_term' = 'Message Category');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `personalization_enabled` SET TAGS ('dbx_business_glossary_term' = 'Personalization Enabled');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `preheader_text` SET TAGS ('dbx_business_glossary_term' = 'Preheader Text');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `reply_to_email` SET TAGS ('dbx_business_glossary_term' = 'Reply To Email');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `reply_to_email` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `reply_to_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `sender_email` SET TAGS ('dbx_business_glossary_term' = 'Sender Email');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `sender_email` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `sender_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `sender_name` SET TAGS ('dbx_business_glossary_term' = 'Sender Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `sender_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `message_template_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `subject_line` SET TAGS ('dbx_business_glossary_term' = 'Subject Line');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `template_code` SET TAGS ('dbx_business_glossary_term' = 'Template Code');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `template_name` SET TAGS ('dbx_business_glossary_term' = 'Template Name');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `template_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `template_type` SET TAGS ('dbx_business_glossary_term' = 'Template Type');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `total_sends_count` SET TAGS ('dbx_business_glossary_term' = 'Total Sends Count');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `unsubscribe_link_required` SET TAGS ('dbx_business_glossary_term' = 'Unsubscribe Link Required');
ALTER TABLE `vibe_retail_v1`.`ecommerce`.`message_template` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
