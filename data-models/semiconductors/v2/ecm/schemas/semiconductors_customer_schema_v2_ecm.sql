...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`account` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: Normalize primary address fields by referencing address table.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`segment` (
    ...
    `employee_id` BIGINT COMMENT 'P6: Add column owner_employee_id (BIGINT) to workforce.segment with FK to workforce.employee.employee_id because segment is a lookup table with zero outbound references. P6: connect_table: workforce.segment** - add column owner_employee_id (B',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` (
    ...
    `sales_design_win_id` BIGINT COMMENT 'SSOT FK reference to canonical sales.sales_design_win; this table is a secondary view and must not duplicate master data.',
    ...
    `design_win_reference` STRING COMMENT '',
    ...
    `estimated_annual_volume` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` (
    ...
    `sales_design_registration_id` BIGINT COMMENT 'SSOT FK reference to canonical sales.sales_design_registration; this table is a secondary view and must not duplicate master data.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` (
    ...
    `qualification_account_id` BIGINT COMMENT '',
    ...
    `qualification_level` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`engagement_activity` (
    ...
    `risk_flag` BOOLEAN COMMENT 'Risk level associated with the engagement.',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` (
    ...
    `nda_number` STRING COMMENT '',
    `nda_status` STRING COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`credit_profile` (
    ...
    `credit_limit_usd` DECIMAL(18,2) COMMENT '',
    ...
    `credit_used_usd` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`customer_sample_request` (
    ...
    `product_sample_request_id` BIGINT COMMENT '',
...
) COMMENT 'Customer requests for engineering samples, evaluation kits, and pre-production devices prior to volume purchase. Captures requesting contact, account, part number requested, quantity, application description, sample purpose (evaluation, design-in, qualification), requested delivery date, shipping address, approval status, approving FAE, sample cost (if charged), and fulfillment status. Tracks the early-stage design-in funnel from sample to design registration to design-win. References SSOT: product.product_sample_request';
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` (
    ...
    `unit_price_usd` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` (
    ...
    `product_ltb_notification_id` BIGINT COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` (
    ...
    `employee_id` BIGINT COMMENT 'Employee who approved the allocation',
    `fab_tool_id` BIGINT COMMENT 'Foreign key linking to the fab tool',
    `actual_utilization_percent` DECIMAL(18,2) COMMENT 'Actual utilization percentage achieved',
    `allocated_capacity_percent` DECIMAL(18,2) COMMENT '',
    `allocated_wafer_starts_per_week` STRING COMMENT '',
    ...
    `allocation_type` STRING COMMENT 'Type of allocation (Dedicated, Shared, Priority)',
    `contract_reference` STRING COMMENT 'Reference to the governing contract or agreement',
    `created_timestamp` TIMESTAMP COMMENT '',
    `effective_from` DATE COMMENT '',
    `effective_until` DATE COMMENT '',
    `is_dedicated` BOOLEAN COMMENT '',
    `is_guaranteed_flag` BOOLEAN COMMENT 'Whether the allocation is contractually guaranteed',
    `notes` STRING COMMENT 'Additional notes about the allocation',
    `priority_class` STRING COMMENT '',
    `priority_level` STRING COMMENT 'Priority level for scheduling (High, Medium, Low)',
    `process_flow_name` STRING COMMENT 'Name of the process flow the allocation covers',
    `reserved_capacity_hours` DECIMAL(18,2) COMMENT '',
    `reserved_capacity_wafers` STRING COMMENT '',
    `reserved_capacity_wph` DECIMAL(18,2) COMMENT '',
    `technology_node_nm` STRING COMMENT 'Technology node in nanometers for the allocation',
    `updated_timestamp` TIMESTAMP COMMENT 'Record last update timestamp',
    `utilization_rate` DECIMAL(18,2) COMMENT 'Actual utilization rate as a ratio of allocated capacity',
    `utilization_target_percent` DECIMAL(18,2) COMMENT 'Target utilization percentage for the allocated capacity',
    `wafer_starts_per_week` STRING COMMENT 'Number of wafer starts per week allocated to this customer',
    `wafers_per_week_capacity` STRING COMMENT 'Number of wafers per week allocated to this customer',
    `start_date` DATE COMMENT 'Allocation start date',
    `end_date` DATE COMMENT 'Allocation end date',
    `allocated_hours` DECIMAL(18,2) COMMENT 'Hours allocated',
    `tool_allocation_status` STRING COMMENT 'Allocation status',
    `utilization_target` DECIMAL(18,2) COMMENT 'Target utilization percentage',
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ADD CONSTRAINT `fk_customer_account_address_id` FOREIGN KEY (`address_id`) REFERENCES `vibe_semiconductors_v1`.`customer`.`address`(`address_id`);
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` ADD CONSTRAINT `fk_customer_qualification_status_qualification_account_id` FOREIGN KEY (`qualification_account_id`) REFERENCES `vibe_semiconductors_v1`.`customer`.`account`(`account_id`);
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Address Id (Foreign Key)');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `primary_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account` ALTER COLUMN `primary_phone` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `address_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Identifier (EMP_ID)');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `first_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `first_name` SET TAGS ('dbx_pii_class' = 'person_name');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `full_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `full_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `full_name` SET TAGS ('dbx_pii_class' = 'person_name');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `last_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `last_name` SET TAGS ('dbx_pii_class' = 'person_name');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`contact` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account_hierarchy` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account_hierarchy` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account_hierarchy` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`segment` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`segment` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` SET TAGS ('dbx_domain_verified' = 'customer');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` SET TAGS ('dbx_ssot' = 'customer.address');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` SET TAGS ('dbx_mvm_ssot_resolved' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_id` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_id` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_status` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_status` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_type` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_type` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `city` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_code` SET TAGS ('dbx_natural_key' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `address_code` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_email` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_name` SET TAGS ('dbx_mask_nonprod' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `contact_phone` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code (ISO 3166‑1 Alpha‑3)');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_denormalized' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `country_code` SET TAGS ('dbx_normalize_to' = 'shared.country.country_code');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `lineage_code` SET TAGS ('dbx_natural_key' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `region` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `state_province` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `street_line1` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `street_line2` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `tax_jurisdiction_code` SET TAGS ('dbx_denormalized_natural_key' = 'tax_jurisdiction');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `updated_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`address` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` SET TAGS ('dbx_ssot' = 'secondary');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` ALTER COLUMN `sales_design_win_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` ALTER COLUMN `sales_design_win_id` SET TAGS ('dbx_ssot_ref' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_win` ALTER COLUMN `sales_design_win_id` SET TAGS ('dbx_ssot_consolidated' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` SET TAGS ('dbx_ssot' = 'secondary');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` ALTER COLUMN `fab_tool_id` SET TAGS ('dbx_business_glossary_term' = 'Fab Tool Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Research Project Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` ALTER COLUMN `sales_design_registration_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` ALTER COLUMN `sales_design_registration_id` SET TAGS ('dbx_ssot_ref' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_design_registration` ALTER COLUMN `sales_design_registration_id` SET TAGS ('dbx_ssot_consolidated' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` SET TAGS ('dbx_subdomain' = 'account_management');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Account Id');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`qualification_status` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Qualification Owner Employee Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`engagement_activity` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`engagement_activity` ALTER COLUMN `participant_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`engagement_activity` ALTER COLUMN `risk_flag` SET TAGS ('dbx_type_verified' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` SET TAGS ('dbx_subdomain' = 'account_management');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `signatory_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `signatory_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `signatory_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `signed_by_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `updated_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`nda_agreement` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`account_team` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`distributor_agreement` SET TAGS ('dbx_subdomain' = 'account_management');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`distributor_agreement` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`distributor_agreement` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`credit_profile` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`credit_profile` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`credit_profile` ALTER COLUMN `updated_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`credit_profile` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_sample_request` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_sample_request` SET TAGS ('dbx_ssot' = 'product.product_sample_request');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_sample_request` ALTER COLUMN `product_sample_request_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` SET TAGS ('dbx_subdomain' = 'commercial_terms');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` ALTER COLUMN `approved_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`price_agreement` ALTER COLUMN `created_by` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` SET TAGS ('dbx_subdomain' = 'commercial_terms');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` SET TAGS ('dbx_domain_verified' = 'customer');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` SET TAGS ('dbx_ssot' = 'product.product_ltb_notification');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` ALTER COLUMN `product_ltb_notification_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`customer_ltb_notification` ALTER COLUMN `technology_roadmap_id` SET TAGS ('dbx_business_glossary_term' = 'Technology Roadmap Id (Foreign Key)');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` SET TAGS ('dbx_subdomain' = 'commercial_terms');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` SET TAGS ('dbx_domain_verified' = 'customer');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `tool_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Tool Allocation - Tool Allocation Id');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Tool Allocation - Account Id');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `fab_tool_id` SET TAGS ('dbx_business_glossary_term' = 'Tool Allocation - Fab Tool Id');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `actual_utilization_percent` SET TAGS ('dbx_business_glossary_term' = 'Actual Utilization');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `contract_reference` SET TAGS ('dbx_business_glossary_term' = 'Contract Reference');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `is_guaranteed_flag` SET TAGS ('dbx_business_glossary_term' = 'Guaranteed Flag');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `process_flow_name` SET TAGS ('dbx_business_glossary_term' = 'Process Flow Name');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `technology_node_nm` SET TAGS ('dbx_business_glossary_term' = 'Technology Node');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `utilization_rate` SET TAGS ('dbx_business_glossary_term' = 'Utilization Rate');
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `utilization_target_percent` SET TAGS ('dbx_business_glossary_term' = 'Utilization Target');
...
ALTER TABLE `vibe_semiconductors_v1`.`customer`.`tool_allocation` ALTER COLUMN `wafers_per_week_capacity` SET TAGS ('dbx_business_glossary_term' = 'Weekly Wafer Capacity');
