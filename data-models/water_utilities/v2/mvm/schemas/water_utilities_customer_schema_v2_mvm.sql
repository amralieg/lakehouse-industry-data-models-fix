...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` (
    ...
    `close_date` DATE COMMENT 'Date account was closed',
    ...
    `holder_name` STRING COMMENT 'The account holder name for the customer account record.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`organization` (
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: Industrial pretreatment program management: organizations subject to IUP permits must be linked to their compliance_permit record. organization already carries denormalized iup_permit_number and iup_e',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Industrial users (organization.industrial_user_flag=true) discharge to the sewer system under EPA 40 CFR Part 403 industrial pretreatment programs. Tracking which WWTP receives each industrial users ',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`service_address` (
    ...
    `sewershed_basin_id` BIGINT COMMENT 'Foreign key linking to wastewater.sewershed_basin. Business justification: service_address.sewer_basin is a denormalized text representation of the sewershed_basin entity. SSO notifications, wastewater billing, and regulatory capacity reporting require a proper FK. sewer_bas',
    `water_system_id` BIGINT COMMENT 'Foreign key linking to quality.water_system. Business justification: Each service address is served by a specific public water system (PWS). This link is required for CCR delivery targeting, LCRR compliance site classification, and regulatory reporting — all of which r',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`premise` (
    ...
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: LCRR lead service line inventory and field operations require knowing which asset registry record (meter, service line) is installed at each premise. Premise already carries denormalized meter_size_in',
    ...
    `sewershed_basin_id` BIGINT COMMENT 'Foreign key linking to wastewater.sewershed_basin. Business justification: Capacity planning, SSO regulatory reporting, and wastewater billing require knowing which sewershed basin drains each premise. premise.wastewater_service_available_flag confirms wastewater service con',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` (
    ...
    `billing_account_id` BIGINT COMMENT 'Foreign key linking to billing.billing_account. Business justification: A service agreement is the contractual basis for a billing account in water utilities. AR teams, collections staff, and payment plan processors navigate from service agreement to billing account daily',
    ...
    `rate_schedule_id` DECIMAL(18,2) COMMENT 'Applicable billing rate schedule',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Utility CIS/AM integration requires knowing which physical metered asset (meter, service connection) serves each service agreement. This drives meter-read-to-bill reconciliation, meter exchange work o',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`service_application` (
    ...
    `organization_id` BIGINT COMMENT 'Foreign key linking to customer.organization. Business justification: service_application currently has applicant_person_id for individual applicants but no FK for organizational applicants (commercial, industrial, municipal). When a business or municipality applies for',
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: Industrial service applications require a valid pretreatment/IUP compliance permit before service can be approved. Linking service_application to compliance_permit enables permit-verification during a',
    ...
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: New service provisioning requires assigning a specific meter or service asset from the asset registry to the approved application. This link supports asset allocation tracking, meter set scheduling, a',
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: A service_application, when approved, results in the creation of a service_agreement. This lifecycle FK links the application record to the agreement it produced, enabling traceability from applicatio',
    ...
    `pipe_main_id` BIGINT COMMENT 'Foreign key linking to distribution.pipe_main. Business justification: During new service connection approval, engineering must identify the specific pipe main to be tapped. Capacity, material compatibility, and operating pressure of the tap main are reviewed as part of ',
    `facility_id` BIGINT COMMENT 'Foreign key linking to treatment.facility. Business justification: New service connection approval requires a WTP capacity check against the serving facilitys design_capacity_mgd and permitted_capacity_mgd. Capacity planning, connection fee calculation, and regulato',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`customer`.`complaint` (
    ...
    `complaint_customer_reported_by_person_id` BIGINT COMMENT 'Foreign key linking to customer.person. Business justification: Complaints capture reporter details that should reference the person master when the reporter is a known person. This eliminates duplication and enables proper reporter tracking. Nullable as some comp',
    ...
    `main_break_id` BIGINT COMMENT 'Foreign key linking to distribution.main_break. Business justification: Customer complaints about outages, discoloration, and pressure loss are causally linked to main breaks. Root cause analysis, regulatory CCR reporting, and customer communication workflows require link',
    ...
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Water quality, pressure, and service complaints are frequently caused by a specific infrastructure asset (main, meter, valve). Linking complaint to the implicated asset registry record enables asset p',
    `regulatory_inspection_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_inspection. Business justification: Water quality or service complaints frequently trigger regulatory inspections. Linking customer_complaint to the resulting regulatory_inspection supports complaint-to-inspection traceability reporting',
    ...
    `service_line_id` BIGINT COMMENT 'Foreign key linking to distribution.service_line. Business justification: Under EPA LCRR regulations, lead-related customer complaints must be linked to the specific service line for material verification, remediation tracking, and regulatory reporting. Low-pressure and dis',
    `sewer_network_id` BIGINT COMMENT 'Foreign key linking to wastewater.sewer_network. Business justification: Sewer backup, blockage, and SSO complaints must be linked to the specific sewer network segment for field dispatch, root cause analysis, and EPA SSO regulatory reporting. A water utility domain expert',
    ...
    `water_sample_id` BIGINT COMMENT 'Foreign key linking to quality.water_sample. Business justification: Water quality complaints (taste, odor, discoloration) trigger field sampling under LCRR/SDWA. The complaint record must reference the resulting water_sample to close the complaint-to-resolution workfl',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Wastewater-related complaints (sewage odor, effluent quality) must be tracked against the responsible WWTP for NPDES compliance reporting and regulatory response. customer_complaint already links to t',
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ADD CONSTRAINT `fk_customer_service_application_organization_id` FOREIGN KEY (`organization_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`organization`(`organization_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`complaint` ADD CONSTRAINT `fk_customer_complaint_complaint_customer_reported_by_person_id` FOREIGN KEY (`complaint_customer_reported_by_person_id`) REFERENCES `vibe_water_utilities_v1`.`customer`.`person`(`person_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`customer_account` ALTER COLUMN `holder_name` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`organization` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_address` ALTER COLUMN `sewershed_basin_id` SET TAGS ('dbx_business_glossary_term' = 'Sewershed Basin Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_address` ALTER COLUMN `water_system_id` SET TAGS ('dbx_business_glossary_term' = 'Water System Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`premise` ALTER COLUMN `sewershed_basin_id` SET TAGS ('dbx_business_glossary_term' = 'Sewershed Basin Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `rate_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Schedule');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_agreement` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ALTER COLUMN `organization_id` SET TAGS ('dbx_business_glossary_term' = 'Applicant Organization Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ALTER COLUMN `pipe_main_id` SET TAGS ('dbx_business_glossary_term' = 'Tap Pipe Main Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`service_application` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Wtp Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`contact` SET TAGS ('dbx_subdomain' = 'service_delivery');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`complaint` ALTER COLUMN `complaint_customer_reported_by_person_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By Person Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`complaint` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`complaint` ALTER COLUMN `regulatory_inspection_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Inspection Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`customer`.`complaint` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
