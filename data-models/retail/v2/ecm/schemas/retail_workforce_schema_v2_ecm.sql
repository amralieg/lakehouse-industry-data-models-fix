...
-- ========= TABLES =========
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`associate` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`time_entry` (
    ...
    `associate_id` BIGINT COMMENT 'Unique identifier for the employee who recorded this time entry. Links to the employee master record in Workday HCM.',
    ...
    `time_associate_id` BIGINT COMMENT 'Identifier for the user (employee or manager) who last modified this time entry record. Used for audit trail and accountability.',
    ...
    `source` STRING COMMENT 'The system or method used to capture this time entry. Indicates data quality and audit trail for time tracking.. Valid values are `time_clock|mobile_app|manual_entry|workday_hcm|pos_terminal|biometric_scanner`',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`payroll_record` (
    ...
    `pay_period_id` BIGINT COMMENT 'Foreign key linking to workforce.pay_period. Business justification: payroll_record captures pay period dates as denormalized strings (pay_period_start_date, pay_period_end_date) but has no FK to the authoritative pay_period table. Adding pay_period_id normalizes this ',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`leave_request` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`performance_review` (
    ...
    `merit_cycle_id` BIGINT COMMENT 'Foreign key linking to workforce.merit_cycle. Business justification: Performance reviews conducted during a merit cycle should reference that cycle directly. The merit_cycle drives the review cadence, calibration requirements, and budget guidelines. compensation_change',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`requisition` (
    ...
    `labor_budget_id` BIGINT COMMENT 'Foreign key linking to workforce.labor_budget. Business justification: Open position requisitions must be authorized against a labor budget. The labor_budget is the SSOT for approved headcount, FTE targets, and budgeted labor cost. Linking requisition.labor_budget_id → l',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`job_application` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`labor_budget` (
    ...
    `staffing_plan_id` BIGINT COMMENT 'Foreign key linking to workforce.staffing_plan. Business justification: A labor budget is the financial expression of a staffing plan — the staffing plan defines headcount allocation by org unit and job profile, while the labor budget translates that into dollar amounts. ',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`compensation_change` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`org_unit` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`wf_certification` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` (
    `dashboard_access_id` BIGINT COMMENT 'Unique identifier for the dashboard access record',
    `access_request_id` BIGINT COMMENT 'Reference to the access request ticket that initiated this grant',
    `associate_id` BIGINT COMMENT 'Reference to the associate granted dashboard access',
    `dashboard_config_id` BIGINT COMMENT 'Reference to the analytics dashboard configuration',
    `dashboard_delegated_to_associate_id` BIGINT COMMENT 'Reference to the associate this access is delegated to',
    `dashboard_granted_by_associate_id` BIGINT COMMENT 'Reference to the associate who granted this access',
    `org_unit_id` BIGINT COMMENT 'Reference to the organizational unit for role-based access',
    `access_count` BIGINT COMMENT 'Number of times the associate has accessed this dashboard',
    `access_granted_date` DATE COMMENT 'Date when access was initially granted',
    `access_level` STRING COMMENT 'Level of access (e.g. viewer, editor, admin, owner)',
    `access_reason` STRING COMMENT 'Business justification for granting this access',
    `access_reason_code` STRING COMMENT 'Reason for granting access (role_based, request, temporary, training).',
    `access_revoked_date` DATE COMMENT 'Date when access was revoked if applicable',
    `access_source` STRING COMMENT 'How access was granted (e.g. role_based, manual, self_service)',
    `access_status` STRING COMMENT 'Current access status (e.g. active, suspended, revoked, expired)',
    `audit_log_enabled_flag` BOOLEAN COMMENT 'Indicates if all access events are logged for audit',
    `can_export_flag` BOOLEAN COMMENT 'Whether the user can export data from this dashboard.',
    `can_share_flag` BOOLEAN COMMENT 'Whether the user can share this dashboard with others.',
    `compliance_attestation_date` DATE COMMENT 'Date of the most recent compliance attestation for this access',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the access record was created',
    `custom_filter_settings` STRING COMMENT 'JSON or serialized custom filter configuration for this user',
    `data_scope_filter` STRING COMMENT 'Data scope restriction applied to this access (e.g. region, store)',
    `delegation_allowed_flag` BOOLEAN COMMENT 'Indicates if this access can be delegated to another associate',
    `delivery_email` STRING COMMENT 'Email address for scheduled dashboard report delivery',
    `delivery_frequency` STRING COMMENT 'Frequency of scheduled deliveries (e.g. daily, weekly, monthly)',
    `expiration_date` DATE COMMENT 'Date when this access automatically expires',
    `export_allowed_flag` BOOLEAN COMMENT 'Whether the associate is allowed to export data from this dashboard.',
    `export_enabled_flag` BOOLEAN COMMENT 'Indicates if data export is permitted for this access',
    `favorite_flag` BOOLEAN COMMENT 'Indicates if the associate has marked this dashboard as a favorite',
    `ip_restriction` STRING COMMENT 'IP address or CIDR range restriction for access',
    `is_default_dashboard` BOOLEAN COMMENT 'Indicates if this is the associates default landing dashboard',
    `last_accessed_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent dashboard access',
    `last_export_timestamp` TIMESTAMP COMMENT 'Timestamp of the last data export from this dashboard',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp of the last modification to this record',
    `max_data_retention_days` STRING COMMENT 'Maximum number of days of data visible through this access',
    `notes` STRING COMMENT 'Free-text notes about access conditions or restrictions.',
    `notification_enabled_flag` BOOLEAN COMMENT 'Indicates if the associate receives dashboard alert notifications',
    `notification_preference` STRING COMMENT 'How the associate prefers to receive dashboard alerts (e.g. email, in_app, none).',
    `pin_order` STRING COMMENT 'Display order position if the dashboard is pinned',
    `refresh_frequency` STRING COMMENT 'Data refresh frequency for this dashboard view',
    `role_name` STRING COMMENT 'Role name that triggered this access grant',
    `row_level_security_filter` STRING COMMENT 'Row-level security filter expression applied to data',
    `schedule_delivery_flag` BOOLEAN COMMENT 'Indicates if scheduled report delivery is enabled',
    `session_timeout_minutes` STRING COMMENT 'Session timeout in minutes for this dashboard access',
    `sharing_enabled_flag` BOOLEAN COMMENT 'Indicates if the associate can share this dashboard with others',
    `terms_accepted_flag` BOOLEAN COMMENT 'Indicates if the associate accepted data usage terms',
    `two_factor_required_flag` BOOLEAN COMMENT 'Indicates if two-factor authentication is required for access',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the last modification to this access record.',
    `watermark_enabled_flag` BOOLEAN COMMENT 'Indicates if exported data includes a watermark',
    CONSTRAINT pk_dashboard_access PRIMARY KEY(`dashboard_access_id`)
) COMMENT 'Controls and audits associate access to analytics dashboards, managing permissions, security filters, delegation, compliance attestation, and usage tracking for data governance.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` (
    `workforce_kpi_target_id` BIGINT COMMENT 'Primary key for workforce_kpi_target',
    `analytics_kpi_target_id` BIGINT COMMENT 'FK to the authoritative analytics KPI target record that this workforce target assignment references.',
    `job_profile_id` BIGINT COMMENT 'Foreign key linking to workforce.job_profile. Business justification: Workforce KPI targets are frequently defined at the job profile/role level (e.g., sales per hour targets for store associates vs. DC throughput targets for warehouse staff). Adding job_profile_id to w',
...
) COMMENT '[References SSOT: analytics.analytics_kpi_target] Workforce-specific KPI target assignments that reference the authoritative analytics.analytics_kpi_target for target definitions. Links KPI targets to org units and associates for accountability tracking.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` (
    ...
    `org_program_compliance_program_id` BIGINT COMMENT 'Foreign key linking to the compliance program that applies to this organizational unit',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`candidate` (
    ...
    `source` STRING COMMENT 'Channel or source through which the candidate was identified or applied, used for recruitment effectiveness analysis.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`payroll_run` (
    `payroll_run_id` BIGINT COMMENT 'Primary key for payroll_run',
    `associate_id` BIGINT COMMENT 'Identifier of the user who approved this payroll run for processing.',
    `payroll_associate_id` BIGINT COMMENT 'Identifier of the user who initiated or created this payroll run.',
    ...
    `payroll_reversal_of_run_id` BIGINT COMMENT 'Reference to the original payroll run that this run reverses or corrects, if applicable.',
    `primary_reversal_payroll_run_id` BIGINT COMMENT 'Self-referencing FK on payroll_run (reversal_payroll_run_id)',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` (
    ...
    `shift_schedule_id` BIGINT COMMENT 'The shift assignment that the requesting employee is willing to take in exchange. May be null for one-way swap requests.',
    `associate_id` BIGINT COMMENT 'The manager or supervisor who approved or rejected the shift swap request.',
    `shift_associate_id` BIGINT COMMENT 'The employee who is initiating the shift swap request and seeking to exchange their assigned shift.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`coverage_request` (
    `coverage_request_id` BIGINT COMMENT 'Primary key for coverage_request',
    `associate_id` BIGINT COMMENT 'Identifier of the manager or supervisor who approved or rejected the coverage request.',
    `coverage_associate_id` BIGINT COMMENT 'Identifier of the employee who is requesting coverage or whose shift needs coverage.',
    `coverage_covering_employee_associate_id` BIGINT COMMENT 'Identifier of the employee who has been assigned or volunteered to provide coverage.',
    `coverage_requesting_employee_associate_id` BIGINT COMMENT 'Identifier of the employee who initiated the coverage request, if different from the employee needing coverage.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`pay_period` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`union` (
    `union_id` BIGINT COMMENT 'Primary key for union',
    `parent_union_id` BIGINT COMMENT 'Self-referencing FK on union (parent_union_id)',
    `abbreviation` STRING COMMENT 'Commonly used abbreviation or acronym for the union name (e.g., UFCW, SEIU, Teamsters).',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` (
    `payroll_calendar_id` BIGINT COMMENT 'Primary key for payroll_calendar',
    `org_unit_id` BIGINT COMMENT 'Reference to the business unit or division that uses this payroll calendar.',
    ...
    `created_by_user_code` STRING COMMENT 'Identifier of the user or system account that created this payroll calendar record.',
    ...
    `last_modified_by_user_code` STRING COMMENT 'Identifier of the user or system account that last modified this payroll calendar record.',
    ...
    `workday_calendar_code` STRING COMMENT 'External identifier for this payroll calendar in Workday HCM system for integration and synchronization.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`workforce`.`access_request` (
    `access_request_id` BIGINT COMMENT 'Primary key for access_request',
    `approver_employee_associate_id` BIGINT COMMENT 'Reference to the workforce employee (typically a manager or system owner) who approved or rejected the access request. Null if the request has not yet reached an approval decision.',
    `associate_id` BIGINT COMMENT 'Reference to the workforce employee record for the individual requesting or on whose behalf access is being requested. Links to the employee master in the workforce domain.',
    `location_id` BIGINT COMMENT 'Reference to the retail store or location for which access is being requested, when the request is scoped to a specific store or distribution center. Null for corporate-wide or system-level access requests.',
    `requester_employee_associate_id` BIGINT COMMENT 'Reference to the workforce employee who submitted the access request. May differ from employee_id when a manager or HR business partner submits on behalf of another employee.',
    `delegated_from_access_request_id` BIGINT COMMENT 'Self-referencing FK on access_request (delegated_from_access_request_id)',
    `access_category` STRING COMMENT 'Broad category of the resource to which access is being requested: system_application (POS, WMS, HCM, ERP), physical_facility (store, warehouse, server room), data_resource (database, data lake, report), network (VPN, Wi-Fi, VLAN), reporting_tool (BI dashboards, analytics platforms).',
    `access_duration_days` STRING COMMENT 'The number of days for which access is requested, calculated as the difference between expiry_date and effective_date. Null for permanent access grants. Supports time-limited access policy enforcement for contractors and seasonal retail staff.',
    `access_level` STRING COMMENT 'Granular permission level being requested: read (view only), read_write (view and modify), admin (full administrative control), execute (run processes or reports), no_access (explicit denial or removal). Supports least-privilege enforcement.',
    `approval_notes` STRING COMMENT 'Free-text notes added by the approver during the review process. May include conditions placed on the approval, scope limitations, or instructions for the provisioning team.',
    `approval_timestamp` TIMESTAMP COMMENT 'The date and time when the approver rendered an approval or rejection decision on the access request. Null if no decision has been made yet.',
    `cancellation_reason` STRING COMMENT 'Free-text reason provided when an access request is cancelled before a final approval decision. Captures context such as employee departure, role change, or duplicate request scenarios.',
    `cancelled_timestamp` TIMESTAMP COMMENT 'The date and time when the access request was cancelled. Null if the request was not cancelled. Supports lifecycle completeness in audit reporting.',
    `compliance_framework` STRING COMMENT 'The primary regulatory or compliance framework governing the access being requested (e.g., SOX, PCI DSS, GDPR, HIPAA, OSHA). Used to route requests through appropriate compliance-aware approval workflows and to tag access grants for regulatory reporting.',
    `cost_center_code` STRING COMMENT 'The cost center code associated with the employees department at the time of the request. Used for labor cost allocation of IT access management activities and chargebacks.',
    `created_timestamp` TIMESTAMP COMMENT 'The date and time when the access request record was first created in the system. May precede submitted_timestamp if the request was saved as a draft.',
    `data_classification_scope` STRING COMMENT 'The highest data classification level of the data or system resources covered by this access request: public, internal, confidential, or restricted. Drives approval workflow complexity and audit requirements.',
    `department` STRING COMMENT 'The organizational department of the employee at the time the access request was submitted (e.g., Store Operations, Finance, Supply Chain, Human Resources). Snapshot value for audit trail integrity.',
    `effective_date` DATE COMMENT 'The date from which the requested access should be active. May be a future date for onboarding scenarios or role transitions. Supports scheduled provisioning workflows.',
    `expiry_date` DATE COMMENT 'The date on which the requested access should automatically expire or be reviewed for renewal. Null for permanent access grants. Supports time-limited access for contractors, seasonal workers, and emergency access scenarios.',
    `external_hris_transaction_reference` STRING COMMENT 'The transaction or business process ID from the Workday HCM system that triggered or is associated with this access request (e.g., a hire, transfer, or termination business process ID). Enables end-to-end traceability between HR events and access provisioning actions.',
    `is_emergency_access` BOOLEAN COMMENT 'Indicates whether this request is a break-glass or emergency access request that bypasses standard approval workflows. Emergency access requests require post-hoc review and are subject to enhanced audit scrutiny.',
    `is_privileged_access` BOOLEAN COMMENT 'Indicates whether the requested access is classified as privileged (e.g., system administrator, database administrator, payroll administrator). Privileged access requests require additional approval steps and enhanced monitoring.',
    `job_title` STRING COMMENT 'The job title of the employee at the time the access request was submitted. Captured as a snapshot to support audit reviews even if the employees role changes after the request is processed.',
    `justification` STRING COMMENT 'Free-text business justification provided by the requester explaining why the access is needed. Required for audit and compliance purposes, particularly for elevated or sensitive access requests.',
    `next_review_date` DATE COMMENT 'The scheduled date for the next periodic access review or certification of this access grant. Supports access recertification campaigns and compliance with least-privilege policies.',
    `priority` STRING COMMENT 'Business urgency assigned to the access request: low (routine, standard SLA), medium (elevated urgency), high (business-impacting delay), critical (emergency or break-glass scenario requiring immediate action). Drives SLA and escalation routing.',
    `provisioned_by` STRING COMMENT 'The identity (username or system name) that executed the provisioning action in the target system. May be a human IT administrator username or an automated provisioning system identifier. Supports accountability in access audit trails.',
    `provisioned_timestamp` TIMESTAMP COMMENT 'The date and time when the approved access was actually provisioned in the target system. Used to measure provisioning SLA compliance (time from approval to provisioning).',
    `provisioning_method` STRING COMMENT 'Indicates how the access was provisioned once approved: automated (via identity governance platform or SCIM), manual (IT administrator action), or hybrid (partial automation with manual steps). Supports process efficiency analysis.',
    `rejection_reason` STRING COMMENT 'Free-text explanation provided by the approver when an access request is rejected. Documents the rationale for denial for audit, compliance, and requester feedback purposes.',
    `related_hr_event` STRING COMMENT 'The HR lifecycle event that triggered this access request, if applicable: new_hire (onboarding provisioning), transfer (location or department change), promotion (role elevation), termination (deprovisioning), leave_of_absence (temporary suspension), rehire (re-provisioning). Null for ad-hoc requests not tied to an HR event.',
    `request_number` STRING COMMENT 'Human-readable, externally-known reference number assigned to the access request at submission (e.g., AR-2024-000123). Used in communications, audit trails, and helpdesk tickets.',
    `request_status` STRING COMMENT 'Current workflow state of the access request: draft (saved but not submitted), submitted (awaiting review), pending_approval (under manager/owner review), approved (decision made, awaiting provisioning), rejected (denied), provisioned (access granted in target system), cancelled (withdrawn by requester). [ENUM-REF-CANDIDATE: draft|submitted|pending_approval|approved|rejected|provisioned|cancelled — promote to reference product]',
    `request_type` STRING COMMENT 'Categorizes the nature of the access request: new_access (first-time provisioning), modify_access (change to existing permissions), remove_access (deprovisioning/offboarding), emergency_access (break-glass or temporary elevated access), role_change (job role transition requiring access realignment).',
    `requested_role` STRING COMMENT 'The specific role, permission set, or access profile being requested within the target system (e.g., Store Manager Read-Only, Payroll Administrator, Inventory Viewer). Aligns with role-based access control (RBAC) definitions.',
    `requires_mfa` BOOLEAN COMMENT 'Indicates whether the requested access requires multi-factor authentication (MFA) enforcement. Typically true for privileged access, remote access, and access to systems containing sensitive retail or financial data.',
    `review_required_flag` BOOLEAN COMMENT 'Indicates whether this access grant requires a scheduled post-provisioning review (e.g., quarterly access certification). Typically true for privileged access, emergency access, and access to sensitive financial or HR systems.',
    `scope_geographic` STRING COMMENT 'The geographic scope of the access being requested (e.g., a specific store region, country, or global). Relevant for multi-region retail enterprises where access may be scoped to specific geographies for data residency or operational reasons.',
    `sla_breach_flag` BOOLEAN COMMENT 'Indicates whether the access request exceeded its SLA target fulfillment time. Set to true when the provisioned_timestamp minus submitted_timestamp exceeds sla_target_hours. Used for SLA compliance reporting.',
    `sla_target_hours` STRING COMMENT 'The target number of business hours within which the access request should be fulfilled from submission, as defined by the applicable SLA policy. Varies by priority level and access category.',
    `source_channel` STRING COMMENT 'The channel through which the access request was initiated: self_service_portal (employee self-service), manager_initiated (submitted by a manager on behalf of a direct report), hr_system (triggered by an HR event such as hire or transfer in the HCM platform), automated_onboarding (system-generated for new hires), helpdesk (submitted via IT helpdesk).',
    `submitted_timestamp` TIMESTAMP COMMENT 'The date and time when the access request was formally submitted by the requester. This is the principal business event timestamp marking the start of the request lifecycle.',
    `system_module` STRING COMMENT 'Specific module or sub-component within the target system for which access is requested (e.g., Payroll within Workday, Inventory Management within WMS, Financial Reporting within ERP). Provides granularity beyond system_name.',
    `system_name` STRING COMMENT 'Name of the application, system, or platform for which access is being requested (e.g., Workday HCM, POS System, Warehouse Management System, Databricks, SAP). Free-text to accommodate the full breadth of retail enterprise systems.',
    `ticket_reference` STRING COMMENT 'Reference number of the corresponding IT service management ticket (e.g., ServiceNow, Jira) created to fulfill the access request. Enables cross-system traceability between the access request record and the provisioning work order.',
    `updated_timestamp` TIMESTAMP COMMENT 'The date and time when the access request record was most recently modified. Updated on every status change, approval action, or field edit.',
    `workforce_type` STRING COMMENT 'Classification of the employees employment relationship at the time of the request: full_time, part_time, contractor, seasonal (common in retail), or vendor. Influences access policy rules, approval routing, and expiry date defaults.',
    CONSTRAINT pk_access_request PRIMARY KEY(`access_request_id`)
) COMMENT 'Master reference table for access_request. Referenced by access_request_id.';

-- ========= FOREIGN KEYS =========
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ADD CONSTRAINT `fk_workforce_associate_collective_bargaining_agreement_id` FOREIGN KEY (`collective_bargaining_agreement_id`) REFERENCES `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement`(`collective_bargaining_agreement_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ADD CONSTRAINT `fk_workforce_associate_manager_associate_id` FOREIGN KEY (`manager_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ADD CONSTRAINT `fk_workforce_shift_schedule_coverage_request_id` FOREIGN KEY (`coverage_request_id`) REFERENCES `vibe_retail_v1`.`workforce`.`coverage_request`(`coverage_request_id`);
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ADD CONSTRAINT `fk_workforce_time_entry_shift_schedule_id` FOREIGN KEY (`shift_schedule_id`) REFERENCES `vibe_retail_v1`.`workforce`.`shift_schedule`(`shift_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ADD CONSTRAINT `fk_workforce_time_entry_time_associate_id` FOREIGN KEY (`time_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ADD CONSTRAINT `fk_workforce_payroll_record_pay_period_id` FOREIGN KEY (`pay_period_id`) REFERENCES `vibe_retail_v1`.`workforce`.`pay_period`(`pay_period_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ADD CONSTRAINT `fk_workforce_payroll_record_payroll_run_id` FOREIGN KEY (`payroll_run_id`) REFERENCES `vibe_retail_v1`.`workforce`.`payroll_run`(`payroll_run_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ADD CONSTRAINT `fk_workforce_payroll_record_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ADD CONSTRAINT `fk_workforce_payroll_record_shift_schedule_id` FOREIGN KEY (`shift_schedule_id`) REFERENCES `vibe_retail_v1`.`workforce`.`shift_schedule`(`shift_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ADD CONSTRAINT `fk_workforce_leave_request_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ADD CONSTRAINT `fk_workforce_leave_request_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ADD CONSTRAINT `fk_workforce_leave_request_shift_schedule_id` FOREIGN KEY (`shift_schedule_id`) REFERENCES `vibe_retail_v1`.`workforce`.`shift_schedule`(`shift_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ADD CONSTRAINT `fk_workforce_leave_request_tertiary_leave_hr_reviewer_associate_id` FOREIGN KEY (`tertiary_leave_hr_reviewer_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ADD CONSTRAINT `fk_workforce_benefit_enrollment_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ADD CONSTRAINT `fk_workforce_benefit_enrollment_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ADD CONSTRAINT `fk_workforce_performance_review_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ADD CONSTRAINT `fk_workforce_performance_review_merit_cycle_id` FOREIGN KEY (`merit_cycle_id`) REFERENCES `vibe_retail_v1`.`workforce`.`merit_cycle`(`merit_cycle_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ADD CONSTRAINT `fk_workforce_performance_review_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ADD CONSTRAINT `fk_workforce_performance_review_tertiary_performance_hr_reviewer_associate_id` FOREIGN KEY (`tertiary_performance_hr_reviewer_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ADD CONSTRAINT `fk_workforce_training_enrollment_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ADD CONSTRAINT `fk_workforce_training_enrollment_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ADD CONSTRAINT `fk_workforce_requisition_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ADD CONSTRAINT `fk_workforce_requisition_labor_budget_id` FOREIGN KEY (`labor_budget_id`) REFERENCES `vibe_retail_v1`.`workforce`.`labor_budget`(`labor_budget_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ADD CONSTRAINT `fk_workforce_requisition_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ADD CONSTRAINT `fk_workforce_requisition_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_candidate_id` FOREIGN KEY (`candidate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`candidate`(`candidate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_requisition_id` FOREIGN KEY (`requisition_id`) REFERENCES `vibe_retail_v1`.`workforce`.`requisition`(`requisition_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ADD CONSTRAINT `fk_workforce_job_application_tertiary_job_modified_by_user_associate_id` FOREIGN KEY (`tertiary_job_modified_by_user_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ADD CONSTRAINT `fk_workforce_labor_budget_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ADD CONSTRAINT `fk_workforce_labor_budget_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ADD CONSTRAINT `fk_workforce_labor_budget_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ADD CONSTRAINT `fk_workforce_labor_budget_staffing_plan_id` FOREIGN KEY (`staffing_plan_id`) REFERENCES `vibe_retail_v1`.`workforce`.`staffing_plan`(`staffing_plan_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ADD CONSTRAINT `fk_workforce_labor_budget_tertiary_labor_modified_by_user_associate_id` FOREIGN KEY (`tertiary_labor_modified_by_user_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_merit_cycle_id` FOREIGN KEY (`merit_cycle_id`) REFERENCES `vibe_retail_v1`.`workforce`.`merit_cycle`(`merit_cycle_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_performance_review_id` FOREIGN KEY (`performance_review_id`) REFERENCES `vibe_retail_v1`.`workforce`.`performance_review`(`performance_review_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ADD CONSTRAINT `fk_workforce_compensation_change_tertiary_compensation_hr_approver_associate_id` FOREIGN KEY (`tertiary_compensation_hr_approver_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ADD CONSTRAINT `fk_workforce_org_unit_parent_org_unit_id` FOREIGN KEY (`parent_org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ADD CONSTRAINT `fk_workforce_org_unit_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ADD CONSTRAINT `fk_workforce_wf_certification_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ADD CONSTRAINT `fk_workforce_wf_certification_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ADD CONSTRAINT `fk_workforce_wf_certification_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ADD CONSTRAINT `fk_workforce_wf_certification_renewed_from_wf_certification_id` FOREIGN KEY (`renewed_from_wf_certification_id`) REFERENCES `vibe_retail_v1`.`workforce`.`wf_certification`(`wf_certification_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ADD CONSTRAINT `fk_workforce_wf_certification_tertiary_wf_modified_by_user_associate_id` FOREIGN KEY (`tertiary_wf_modified_by_user_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ADD CONSTRAINT `fk_workforce_staffing_plan_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ADD CONSTRAINT `fk_workforce_staffing_plan_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ADD CONSTRAINT `fk_workforce_staffing_plan_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ADD CONSTRAINT `fk_workforce_dashboard_access_access_request_id` FOREIGN KEY (`access_request_id`) REFERENCES `vibe_retail_v1`.`workforce`.`access_request`(`access_request_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ADD CONSTRAINT `fk_workforce_dashboard_access_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ADD CONSTRAINT `fk_workforce_dashboard_access_dashboard_delegated_to_associate_id` FOREIGN KEY (`dashboard_delegated_to_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ADD CONSTRAINT `fk_workforce_dashboard_access_dashboard_granted_by_associate_id` FOREIGN KEY (`dashboard_granted_by_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ADD CONSTRAINT `fk_workforce_dashboard_access_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ADD CONSTRAINT `fk_workforce_workforce_kpi_target_job_profile_id` FOREIGN KEY (`job_profile_id`) REFERENCES `vibe_retail_v1`.`workforce`.`job_profile`(`job_profile_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ADD CONSTRAINT `fk_workforce_workforce_kpi_target_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ADD CONSTRAINT `fk_workforce_workforce_kpi_target_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ADD CONSTRAINT `fk_workforce_org_unit_compliance_scope_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ADD CONSTRAINT `fk_workforce_candidate_referred_by_candidate_id` FOREIGN KEY (`referred_by_candidate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`candidate`(`candidate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ADD CONSTRAINT `fk_workforce_payroll_run_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ADD CONSTRAINT `fk_workforce_payroll_run_payroll_associate_id` FOREIGN KEY (`payroll_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ADD CONSTRAINT `fk_workforce_payroll_run_payroll_calendar_id` FOREIGN KEY (`payroll_calendar_id`) REFERENCES `vibe_retail_v1`.`workforce`.`payroll_calendar`(`payroll_calendar_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ADD CONSTRAINT `fk_workforce_payroll_run_payroll_reversal_of_run_id` FOREIGN KEY (`payroll_reversal_of_run_id`) REFERENCES `vibe_retail_v1`.`workforce`.`payroll_run`(`payroll_run_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ADD CONSTRAINT `fk_workforce_payroll_run_primary_reversal_payroll_run_id` FOREIGN KEY (`primary_reversal_payroll_run_id`) REFERENCES `vibe_retail_v1`.`workforce`.`payroll_run`(`payroll_run_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ADD CONSTRAINT `fk_workforce_shift_swap_request_counterpart_shift_swap_request_id` FOREIGN KEY (`counterpart_shift_swap_request_id`) REFERENCES `vibe_retail_v1`.`workforce`.`shift_swap_request`(`shift_swap_request_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ADD CONSTRAINT `fk_workforce_shift_swap_request_shift_schedule_id` FOREIGN KEY (`shift_schedule_id`) REFERENCES `vibe_retail_v1`.`workforce`.`shift_schedule`(`shift_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ADD CONSTRAINT `fk_workforce_shift_swap_request_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ADD CONSTRAINT `fk_workforce_shift_swap_request_shift_associate_id` FOREIGN KEY (`shift_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ADD CONSTRAINT `fk_workforce_shift_swap_request_target_employee_associate_id` FOREIGN KEY (`target_employee_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ADD CONSTRAINT `fk_workforce_coverage_request_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ADD CONSTRAINT `fk_workforce_coverage_request_coverage_associate_id` FOREIGN KEY (`coverage_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ADD CONSTRAINT `fk_workforce_bargaining_unit_parent_bargaining_unit_id` FOREIGN KEY (`parent_bargaining_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`bargaining_unit`(`bargaining_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ADD CONSTRAINT `fk_workforce_payroll_calendar_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `vibe_retail_v1`.`workforce`.`org_unit`(`org_unit_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ADD CONSTRAINT `fk_workforce_payroll_calendar_prior_payroll_calendar_id` FOREIGN KEY (`prior_payroll_calendar_id`) REFERENCES `vibe_retail_v1`.`workforce`.`payroll_calendar`(`payroll_calendar_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ADD CONSTRAINT `fk_workforce_access_request_approver_employee_associate_id` FOREIGN KEY (`approver_employee_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ADD CONSTRAINT `fk_workforce_access_request_associate_id` FOREIGN KEY (`associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ADD CONSTRAINT `fk_workforce_access_request_requester_employee_associate_id` FOREIGN KEY (`requester_employee_associate_id`) REFERENCES `vibe_retail_v1`.`workforce`.`associate`(`associate_id`);
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ADD CONSTRAINT `fk_workforce_access_request_delegated_from_access_request_id` FOREIGN KEY (`delegated_from_access_request_id`) REFERENCES `vibe_retail_v1`.`workforce`.`access_request`(`access_request_id`);
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Store Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `manager_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Manager Associate Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `badge_number` SET TAGS ('dbx_business_glossary_term' = 'Employee Badge Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `emergency_contact_name` SET TAGS ('dbx_business_glossary_term' = 'Emergency Contact Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `emergency_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `emergency_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `employment_status` SET TAGS ('dbx_business_glossary_term' = 'Employment Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `employment_type` SET TAGS ('dbx_business_glossary_term' = 'Employment Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `hire_date` SET TAGS ('dbx_business_glossary_term' = 'Hire Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `hr_business_partner_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `job_code` SET TAGS ('dbx_business_glossary_term' = 'Job Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `job_title` SET TAGS ('dbx_business_glossary_term' = 'Job Title');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `last_performance_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Performance Review Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `last_training_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Last Training Completion Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `legal_first_name` SET TAGS ('dbx_business_glossary_term' = 'Legal First Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `legal_first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `legal_last_name` SET TAGS ('dbx_business_glossary_term' = 'Legal Last Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `legal_last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `legal_middle_name` SET TAGS ('dbx_business_glossary_term' = 'Legal Middle Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `pay_grade` SET TAGS ('dbx_business_glossary_term' = 'Pay Grade');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `pay_grade` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `pay_type` SET TAGS ('dbx_business_glossary_term' = 'Pay Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `preferred_name` SET TAGS ('dbx_business_glossary_term' = 'Preferred Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `primary_work_location_type` SET TAGS ('dbx_business_glossary_term' = 'Primary Work Location Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `rehire_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Rehire Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `standard_hours_per_week` SET TAGS ('dbx_business_glossary_term' = 'Standard Hours Per Week');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Termination Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `termination_reason` SET TAGS ('dbx_business_glossary_term' = 'Termination Reason');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `union_local_code` SET TAGS ('dbx_business_glossary_term' = 'Union Local Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `union_membership_flag` SET TAGS ('dbx_business_glossary_term' = 'Union Membership Flag');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `work_authorization_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Work Authorization Expiration Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`associate` ALTER COLUMN `work_authorization_status` SET TAGS ('dbx_business_glossary_term' = 'Work Authorization Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `department` SET TAGS ('dbx_business_glossary_term' = 'Department');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `division` SET TAGS ('dbx_business_glossary_term' = 'Division');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `employment_type` SET TAGS ('dbx_business_glossary_term' = 'Employment Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `flsa_classification` SET TAGS ('dbx_business_glossary_term' = 'Fair Labor Standards Act (FLSA) Classification');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `flsa_classification` SET TAGS ('dbx_value_regex' = 'exempt|non-exempt');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_bonus_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Bonus Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_commission_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Commission Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_corporate` SET TAGS ('dbx_business_glossary_term' = 'Is Corporate');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_customer_facing` SET TAGS ('dbx_business_glossary_term' = 'Is Customer Facing');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_dc_facing` SET TAGS ('dbx_business_glossary_term' = 'Is Distribution Center (DC) Facing');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_leadership_role` SET TAGS ('dbx_business_glossary_term' = 'Is Leadership Role');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_overtime_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Overtime Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_store_facing` SET TAGS ('dbx_business_glossary_term' = 'Is Store Facing');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `is_union_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Union Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_code` SET TAGS ('dbx_business_glossary_term' = 'Job Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,12}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_description` SET TAGS ('dbx_business_glossary_term' = 'Job Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_family` SET TAGS ('dbx_business_glossary_term' = 'Job Family');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_level` SET TAGS ('dbx_business_glossary_term' = 'Job Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_profile_status` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `job_title` SET TAGS ('dbx_business_glossary_term' = 'Job Title');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `last_reviewed_date` SET TAGS ('dbx_business_glossary_term' = 'Last Reviewed Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `location_type` SET TAGS ('dbx_business_glossary_term' = 'Location Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `maximum_pay_range` SET TAGS ('dbx_business_glossary_term' = 'Maximum Pay Range');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `maximum_pay_range` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `minimum_pay_range` SET TAGS ('dbx_business_glossary_term' = 'Minimum Pay Range');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `minimum_pay_range` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `minimum_years_experience` SET TAGS ('dbx_business_glossary_term' = 'Minimum Years Experience');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `pay_frequency` SET TAGS ('dbx_business_glossary_term' = 'Pay Frequency');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `physical_requirements` SET TAGS ('dbx_business_glossary_term' = 'Physical Requirements');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `required_certifications` SET TAGS ('dbx_business_glossary_term' = 'Required Certifications');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `required_education_level` SET TAGS ('dbx_business_glossary_term' = 'Required Education Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `required_education_level` SET TAGS ('dbx_value_regex' = 'high-school|associate|bachelor|master|doctorate|none');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `span_of_control_max` SET TAGS ('dbx_business_glossary_term' = 'Span of Control Maximum');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `span_of_control_min` SET TAGS ('dbx_business_glossary_term' = 'Span of Control Minimum');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `standard_hours_per_week` SET TAGS ('dbx_business_glossary_term' = 'Standard Hours Per Week');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `succession_planning_tier` SET TAGS ('dbx_business_glossary_term' = 'Succession Planning Tier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `succession_planning_tier` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low|not-applicable');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_profile` ALTER COLUMN `travel_requirement_percentage` SET TAGS ('dbx_business_glossary_term' = 'Travel Requirement Percentage');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Shift Schedule ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `coverage_request_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Request ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Dc Facility Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_swap_request_id` SET TAGS ('dbx_business_glossary_term' = 'Shift Swap Request ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `break_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Break Duration Minutes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `cancelled_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancelled Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `confirmed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Confirmed Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `employee_availability_match` SET TAGS ('dbx_business_glossary_term' = 'Employee Availability Match');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `estimated_labor_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Labor Cost');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `estimated_labor_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `is_holiday_shift` SET TAGS ('dbx_business_glossary_term' = 'Is Holiday Shift');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `is_overtime_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Overtime Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `labor_cost_allocation_code` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Allocation Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `paid_break_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Paid Break Duration Minutes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `published_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Published Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `schedule_notes` SET TAGS ('dbx_business_glossary_term' = 'Schedule Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `schedule_source` SET TAGS ('dbx_business_glossary_term' = 'Schedule Source');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `schedule_status` SET TAGS ('dbx_business_glossary_term' = 'Schedule Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `schedule_week_start_date` SET TAGS ('dbx_business_glossary_term' = 'Schedule Week Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `scheduled_end_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled End Time');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `scheduled_hours` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `scheduled_start_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Start Time');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_date` SET TAGS ('dbx_business_glossary_term' = 'Shift Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_priority` SET TAGS ('dbx_business_glossary_term' = 'Shift Priority');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_type` SET TAGS ('dbx_business_glossary_term' = 'Shift Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_schedule` ALTER COLUMN `shift_type` SET TAGS ('dbx_value_regex' = 'opening|mid|closing|overnight|split|on_call');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `time_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Time Entry ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Dc Facility Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `pay_period_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Period ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Work Location ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `actual_hours_worked` SET TAGS ('dbx_business_glossary_term' = 'Actual Hours Worked');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `break_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Break Time Minutes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `clock_in_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Clock In Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `clock_out_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Clock Out Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `department_code` SET TAGS ('dbx_business_glossary_term' = 'Department Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `double_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Double Time Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `exception_flag` SET TAGS ('dbx_business_glossary_term' = 'Exception Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `exception_notes` SET TAGS ('dbx_business_glossary_term' = 'Exception Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `exception_type` SET TAGS ('dbx_business_glossary_term' = 'Exception Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `geolocation_latitude` SET TAGS ('dbx_business_glossary_term' = 'Geolocation Latitude');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `geolocation_latitude` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `geolocation_longitude` SET TAGS ('dbx_business_glossary_term' = 'Geolocation Longitude');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `geolocation_longitude` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `is_holiday_work` SET TAGS ('dbx_business_glossary_term' = 'Is Holiday Work');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `labor_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `labor_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `meal_break_minutes` SET TAGS ('dbx_business_glossary_term' = 'Meal Break Minutes');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `paid_break_minutes` SET TAGS ('dbx_business_glossary_term' = 'Paid Break Minutes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `pay_rate` SET TAGS ('dbx_business_glossary_term' = 'Pay Rate');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `payroll_processed_flag` SET TAGS ('dbx_business_glossary_term' = 'Payroll Processed Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `regular_hours` SET TAGS ('dbx_business_glossary_term' = 'Regular Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `shift_differential_eligible` SET TAGS ('dbx_business_glossary_term' = 'Shift Differential Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `shift_differential_rate` SET TAGS ('dbx_business_glossary_term' = 'Shift Differential Rate');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `shift_differential_type` SET TAGS ('dbx_business_glossary_term' = 'Shift Differential Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `source` SET TAGS ('dbx_business_glossary_term' = 'Time Entry Source');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `time_entry_type` SET TAGS ('dbx_business_glossary_term' = 'Time Entry Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`time_entry` ALTER COLUMN `work_date` SET TAGS ('dbx_business_glossary_term' = 'Work Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `payroll_record_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Record ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `pay_period_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `payroll_run_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Run ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `bank_account_last_four_digits` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Last Four Digits');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `bonus_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Bonus Pay Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `commission_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Commission Pay Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `department_code` SET TAGS ('dbx_business_glossary_term' = 'Department Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `federal_tax_withheld_amount` SET TAGS ('dbx_business_glossary_term' = 'Federal Tax Withheld Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `fica_medicare_withheld_amount` SET TAGS ('dbx_business_glossary_term' = 'FICA Medicare Withheld Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `fica_social_security_withheld_amount` SET TAGS ('dbx_business_glossary_term' = 'FICA Social Security Withheld Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `garnishment_amount` SET TAGS ('dbx_business_glossary_term' = 'Garnishment Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `gross_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Pay Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `health_insurance_deduction_amount` SET TAGS ('dbx_business_glossary_term' = 'Health Insurance Deduction Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `local_tax_withheld_amount` SET TAGS ('dbx_business_glossary_term' = 'Local Tax Withheld Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `location_code` SET TAGS ('dbx_business_glossary_term' = 'Location Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `net_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Pay Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `other_benefits_deduction_amount` SET TAGS ('dbx_business_glossary_term' = 'Other Benefits Deduction Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `pay_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `pay_frequency` SET TAGS ('dbx_business_glossary_term' = 'Pay Frequency');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `payment_method` SET TAGS ('dbx_value_regex' = 'direct_deposit|paper_check|paycard|cash');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `payroll_status` SET TAGS ('dbx_business_glossary_term' = 'Payroll Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `processed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Processed Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `regular_hours_worked` SET TAGS ('dbx_business_glossary_term' = 'Regular Hours Worked');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `regular_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Regular Pay Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `retirement_contribution_amount` SET TAGS ('dbx_business_glossary_term' = 'Retirement Contribution Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `shift_differential_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Shift Differential Pay Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `state_tax_withheld_amount` SET TAGS ('dbx_business_glossary_term' = 'State Tax Withheld Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_record` ALTER COLUMN `total_deductions_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Deductions Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` SET TAGS ('dbx_subdomain' = 'payroll_benefits');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_request_id` SET TAGS ('dbx_business_glossary_term' = 'Leave Request ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `shift_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Shift Schedule Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `tertiary_leave_hr_reviewer_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Human Resources (HR) Reviewer ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Work Location ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `actual_return_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Return Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approval_comments` SET TAGS ('dbx_business_glossary_term' = 'Approval Comments');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'Pending|Approved|Denied|Cancelled|Withdrawn');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approved_end_date` SET TAGS ('dbx_business_glossary_term' = 'Approved End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `approved_start_date` SET TAGS ('dbx_business_glossary_term' = 'Approved Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `fmla_designation_status` SET TAGS ('dbx_business_glossary_term' = 'Family and Medical Leave Act (FMLA) Designation Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `fmla_designation_status` SET TAGS ('dbx_value_regex' = 'Not Applicable|Pending|Designated|Not Designated');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `intermittent_leave_flag` SET TAGS ('dbx_business_glossary_term' = 'Intermittent Leave Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_balance_after` SET TAGS ('dbx_business_glossary_term' = 'Leave Balance After');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_balance_before` SET TAGS ('dbx_business_glossary_term' = 'Leave Balance Before');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_reason` SET TAGS ('dbx_business_glossary_term' = 'Leave Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_reason` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_request_number` SET TAGS ('dbx_business_glossary_term' = 'Leave Request Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_subtype` SET TAGS ('dbx_business_glossary_term' = 'Leave Subtype');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `leave_type` SET TAGS ('dbx_business_glossary_term' = 'Leave Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `medical_certification_due_date` SET TAGS ('dbx_business_glossary_term' = 'Medical Certification Due Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `medical_certification_received_flag` SET TAGS ('dbx_business_glossary_term' = 'Medical Certification Received Flag');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `medical_certification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Medical Certification Required Flag');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `paid_leave_flag` SET TAGS ('dbx_business_glossary_term' = 'Paid Leave Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `reduced_schedule_flag` SET TAGS ('dbx_business_glossary_term' = 'Reduced Schedule Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `request_submitted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Request Submitted Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `requested_end_date` SET TAGS ('dbx_business_glossary_term' = 'Requested End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `requested_start_date` SET TAGS ('dbx_business_glossary_term' = 'Requested Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `return_to_work_date` SET TAGS ('dbx_business_glossary_term' = 'Return to Work Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `total_days_approved` SET TAGS ('dbx_business_glossary_term' = 'Total Days Approved');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `total_days_requested` SET TAGS ('dbx_business_glossary_term' = 'Total Days Requested');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `total_hours_approved` SET TAGS ('dbx_business_glossary_term' = 'Total Hours Approved');
ALTER TABLE `vibe_retail_v1`.`workforce`.`leave_request` ALTER COLUMN `total_hours_requested` SET TAGS ('dbx_business_glossary_term' = 'Total Hours Requested');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `benefit_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Enrollment ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_1095c_offer_code` SET TAGS ('dbx_business_glossary_term' = 'Affordable Care Act (ACA) Form 1095-C Offer of Coverage Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_1095c_offer_code` SET TAGS ('dbx_value_regex' = '^1[A-J]$|^$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_1095c_safe_harbor_code` SET TAGS ('dbx_business_glossary_term' = 'Affordable Care Act (ACA) Form 1095-C Safe Harbor Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_1095c_safe_harbor_code` SET TAGS ('dbx_value_regex' = '^2[A-H]$|^$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Affordable Care Act (ACA) Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `aca_measurement_period_end` SET TAGS ('dbx_business_glossary_term' = 'Affordable Care Act (ACA) Measurement Period End Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `annual_election_amount` SET TAGS ('dbx_business_glossary_term' = 'Annual Election Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `beneficiary_name` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `beneficiary_relationship` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Relationship');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `benefit_type` SET TAGS ('dbx_business_glossary_term' = 'Benefit Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `carrier_policy_number` SET TAGS ('dbx_business_glossary_term' = 'Carrier Policy Number');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `coverage_tier` SET TAGS ('dbx_business_glossary_term' = 'Coverage Tier');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `employee_contribution_amount` SET TAGS ('dbx_business_glossary_term' = 'Employee Contribution Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `employee_contribution_frequency` SET TAGS ('dbx_business_glossary_term' = 'Employee Contribution Frequency');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `employer_contribution_amount` SET TAGS ('dbx_business_glossary_term' = 'Employer Contribution Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `employer_contribution_frequency` SET TAGS ('dbx_business_glossary_term' = 'Employer Contribution Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `employer_contribution_frequency` SET TAGS ('dbx_value_regex' = 'weekly|biweekly|semimonthly|monthly|annual|per_pay_period');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Confirmation Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_method` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_method` SET TAGS ('dbx_value_regex' = 'online_self_service|paper_form|phone|hr_representative|auto_enrollment|default');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_source` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Source');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_source` SET TAGS ('dbx_value_regex' = 'open_enrollment|new_hire|qualifying_life_event|administrative_correction|rehire|cobra');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_status` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_status` SET TAGS ('dbx_value_regex' = 'active|terminated|pending|waived|suspended|cancelled');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_termination_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Termination Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `enrollment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `plan_name` SET TAGS ('dbx_business_glossary_term' = 'Benefit Plan Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `plan_year` SET TAGS ('dbx_business_glossary_term' = 'Plan Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `plan_year` SET TAGS ('dbx_value_regex' = '^d{4}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `qualifying_life_event_date` SET TAGS ('dbx_business_glossary_term' = 'Qualifying Life Event (QLE) Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `qualifying_life_event_type` SET TAGS ('dbx_business_glossary_term' = 'Qualifying Life Event (QLE) Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `source_system_record_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `total_premium_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Premium Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`benefit_enrollment` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` SET TAGS ('dbx_subdomain' = 'associate_development');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `performance_review_id` SET TAGS ('dbx_business_glossary_term' = 'Performance Review ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `kpi_value_id` SET TAGS ('dbx_business_glossary_term' = 'Kpi Value Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `merit_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Merit Cycle Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `tertiary_performance_hr_reviewer_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Human Resources (HR) Reviewer ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `acknowledgment_date` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `action_type` SET TAGS ('dbx_business_glossary_term' = 'Action Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `action_type` SET TAGS ('dbx_value_regex' = 'performance_review|verbal_warning|written_warning|final_warning|suspension|termination');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `appeal_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `appeal_outcome` SET TAGS ('dbx_business_glossary_term' = 'Appeal Outcome');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `appeal_outcome` SET TAGS ('dbx_value_regex' = 'upheld|overturned|modified|pending');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `calibration_date` SET TAGS ('dbx_business_glossary_term' = 'Calibration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `calibration_status` SET TAGS ('dbx_business_glossary_term' = 'Calibration Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `calibration_status` SET TAGS ('dbx_value_regex' = 'pending|calibrated|approved|rejected');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `competency_rating` SET TAGS ('dbx_business_glossary_term' = 'Competency Rating');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `competency_rating` SET TAGS ('dbx_value_regex' = 'exceeds_expectations|meets_expectations|needs_improvement|unsatisfactory');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `employee_acknowledgment_flag` SET TAGS ('dbx_business_glossary_term' = 'Employee Acknowledgment Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `goal_achievement_rating` SET TAGS ('dbx_business_glossary_term' = 'Goal Achievement Rating');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `incident_description` SET TAGS ('dbx_business_glossary_term' = 'Incident Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `incident_description` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `manager_comments` SET TAGS ('dbx_business_glossary_term' = 'Manager Comments');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `manager_comments` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `merit_increase_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Merit Increase Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `merit_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Merit Increase Percentage');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `overall_rating_score` SET TAGS ('dbx_business_glossary_term' = 'Overall Rating Score');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `performance_review_status` SET TAGS ('dbx_value_regex' = 'draft|submitted|approved|completed|cancelled');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `pip_end_date` SET TAGS ('dbx_business_glossary_term' = 'Performance Improvement Plan (PIP) End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `pip_flag` SET TAGS ('dbx_business_glossary_term' = 'Performance Improvement Plan (PIP) Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `pip_start_date` SET TAGS ('dbx_business_glossary_term' = 'Performance Improvement Plan (PIP) Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `policy_violation_category` SET TAGS ('dbx_business_glossary_term' = 'Policy Violation Category');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `prior_actions_count` SET TAGS ('dbx_business_glossary_term' = 'Prior Actions Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `progressive_discipline_sequence` SET TAGS ('dbx_business_glossary_term' = 'Progressive Discipline Sequence');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `promotion_recommended_flag` SET TAGS ('dbx_business_glossary_term' = 'Promotion Recommended Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `review_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Review Period End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `review_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Review Period Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `review_type` SET TAGS ('dbx_business_glossary_term' = 'Review Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`performance_review` ALTER COLUMN `termination_recommended_flag` SET TAGS ('dbx_business_glossary_term' = 'Termination Recommended Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` SET TAGS ('dbx_subdomain' = 'associate_development');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `training_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Training Enrollment ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `certification_id` SET TAGS ('dbx_business_glossary_term' = 'Certification ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Completion Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `attempt_number` SET TAGS ('dbx_business_glossary_term' = 'Attempt Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `certification_earned_flag` SET TAGS ('dbx_business_glossary_term' = 'Certification Earned Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `completion_status` SET TAGS ('dbx_business_glossary_term' = 'Completion Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `compliance_training_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Training Flag');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `delivery_method` SET TAGS ('dbx_business_glossary_term' = 'Delivery Method');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `enrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `enrollment_source` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Source');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `enrollment_source` SET TAGS ('dbx_value_regex' = 'manager_assigned|self_enrolled|hr_assigned|system_triggered|onboarding_workflow');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `feedback_comments` SET TAGS ('dbx_business_glossary_term' = 'Training Feedback Comments');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `feedback_rating` SET TAGS ('dbx_business_glossary_term' = 'Training Feedback Rating');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `instructor_name` SET TAGS ('dbx_business_glossary_term' = 'Instructor Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `instructor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `mandatory_training_flag` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Training Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `overdue_flag` SET TAGS ('dbx_business_glossary_term' = 'Overdue Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `pass_fail_indicator` SET TAGS ('dbx_business_glossary_term' = 'Pass Fail Indicator');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `pass_fail_indicator` SET TAGS ('dbx_value_regex' = 'pass|fail|not_applicable');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `passing_score_threshold` SET TAGS ('dbx_business_glossary_term' = 'Passing Score Threshold');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `scheduled_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Completion Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `scheduled_start_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `score` SET TAGS ('dbx_business_glossary_term' = 'Training Score');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `training_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Training Duration Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `training_location` SET TAGS ('dbx_business_glossary_term' = 'Training Location');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `training_type` SET TAGS ('dbx_business_glossary_term' = 'Training Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `waiver_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approval Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `waiver_approved_by` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approved By');
ALTER TABLE `vibe_retail_v1`.`workforce`.`training_enrollment` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requisition_id` SET TAGS ('dbx_business_glossary_term' = 'Requisition ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Dc Facility Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Hiring Location ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `labor_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Labor Budget Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `approved_headcount` SET TAGS ('dbx_business_glossary_term' = 'Approved Headcount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `employment_type` SET TAGS ('dbx_business_glossary_term' = 'Employment Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `flsa_status` SET TAGS ('dbx_business_glossary_term' = 'Fair Labor Standards Act (FLSA) Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `flsa_status` SET TAGS ('dbx_value_regex' = 'exempt|non_exempt');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `hiring_location_type` SET TAGS ('dbx_business_glossary_term' = 'Hiring Location Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `hiring_location_type` SET TAGS ('dbx_value_regex' = 'store|distribution_center|corporate_office|regional_hub|fulfillment_center');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `is_remote_eligible` SET TAGS ('dbx_business_glossary_term' = 'Is Remote Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `job_description` SET TAGS ('dbx_business_glossary_term' = 'Job Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `job_family` SET TAGS ('dbx_business_glossary_term' = 'Job Family');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `job_level` SET TAGS ('dbx_business_glossary_term' = 'Job Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `job_title` SET TAGS ('dbx_business_glossary_term' = 'Job Title');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `minimum_education_level` SET TAGS ('dbx_business_glossary_term' = 'Minimum Education Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `minimum_education_level` SET TAGS ('dbx_value_regex' = 'none|high_school|associate|bachelor|master|doctorate');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `minimum_experience_years` SET TAGS ('dbx_business_glossary_term' = 'Minimum Experience Years');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `open_date` SET TAGS ('dbx_business_glossary_term' = 'Requisition Open Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `pay_frequency` SET TAGS ('dbx_business_glossary_term' = 'Pay Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `pay_frequency` SET TAGS ('dbx_value_regex' = 'hourly|weekly|biweekly|semimonthly|monthly|annual');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `positions_filled` SET TAGS ('dbx_business_glossary_term' = 'Positions Filled');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `posting_title` SET TAGS ('dbx_business_glossary_term' = 'Posting Title');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requires_background_check` SET TAGS ('dbx_business_glossary_term' = 'Requires Background Check');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requires_drug_screening` SET TAGS ('dbx_business_glossary_term' = 'Requires Drug Screening');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requisition_number` SET TAGS ('dbx_business_glossary_term' = 'Requisition Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requisition_number` SET TAGS ('dbx_value_regex' = '^REQ-[0-9]{6,10}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requisition_status` SET TAGS ('dbx_business_glossary_term' = 'Requisition Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `requisition_type` SET TAGS ('dbx_business_glossary_term' = 'Requisition Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `salary_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Salary Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `sourcing_channels` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Channels');
ALTER TABLE `vibe_retail_v1`.`workforce`.`requisition` ALTER COLUMN `target_start_date` SET TAGS ('dbx_business_glossary_term' = 'Target Start Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `job_application_id` SET TAGS ('dbx_business_glossary_term' = 'Job Application ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `candidate_id` SET TAGS ('dbx_business_glossary_term' = 'Candidate ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Recruiter ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `requisition_id` SET TAGS ('dbx_business_glossary_term' = 'Requisition ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `tertiary_job_modified_by_user_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `application_date` SET TAGS ('dbx_business_glossary_term' = 'Application Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `application_notes` SET TAGS ('dbx_business_glossary_term' = 'Application Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `application_number` SET TAGS ('dbx_business_glossary_term' = 'Application Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `application_number` SET TAGS ('dbx_value_regex' = '^APP-[0-9]{8}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `application_status` SET TAGS ('dbx_business_glossary_term' = 'Application Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `background_check_date` SET TAGS ('dbx_business_glossary_term' = 'Background Check Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `background_check_status` SET TAGS ('dbx_business_glossary_term' = 'Background Check Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `disability_status` SET TAGS ('dbx_business_glossary_term' = 'Disability Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `drug_screen_date` SET TAGS ('dbx_business_glossary_term' = 'Drug Screen Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `drug_screen_status` SET TAGS ('dbx_business_glossary_term' = 'Drug Screen Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `hire_date` SET TAGS ('dbx_business_glossary_term' = 'Hire Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `interview_count` SET TAGS ('dbx_business_glossary_term' = 'Interview Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `interview_stage` SET TAGS ('dbx_business_glossary_term' = 'Interview Stage');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `last_interview_date` SET TAGS ('dbx_business_glossary_term' = 'Last Interview Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_acceptance_date` SET TAGS ('dbx_business_glossary_term' = 'Offer Acceptance Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_accepted_flag` SET TAGS ('dbx_business_glossary_term' = 'Offer Accepted Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_amount` SET TAGS ('dbx_business_glossary_term' = 'Offer Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Offer Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_date` SET TAGS ('dbx_business_glossary_term' = 'Offer Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `offer_extended_flag` SET TAGS ('dbx_business_glossary_term' = 'Offer Extended Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `rejection_date` SET TAGS ('dbx_business_glossary_term' = 'Rejection Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `source_channel` SET TAGS ('dbx_business_glossary_term' = 'Source Channel');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `source_detail` SET TAGS ('dbx_business_glossary_term' = 'Source Detail');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`job_application` ALTER COLUMN `veteran_status` SET TAGS ('dbx_business_glossary_term' = 'Veteran Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `labor_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Labor Budget ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `staffing_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Staffing Plan Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `attrition_assumption_percent` SET TAGS ('dbx_business_glossary_term' = 'Attrition Assumption Percent');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budget_notes` SET TAGS ('dbx_business_glossary_term' = 'Budget Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budget_status` SET TAGS ('dbx_business_glossary_term' = 'Budget Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budget_version_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Version Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budgeted_labor_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Budgeted Labor Cost Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budgeted_labor_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budgeted_labor_cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Budgeted Labor Cost Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budgeted_labor_cost_percent_of_sales` SET TAGS ('dbx_business_glossary_term' = 'Budgeted Labor Cost Percent of Sales');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `budgeted_regular_hours` SET TAGS ('dbx_business_glossary_term' = 'Budgeted Regular Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `internal_transfer_assumption` SET TAGS ('dbx_business_glossary_term' = 'Internal Transfer Assumption');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `net_new_hire_target` SET TAGS ('dbx_business_glossary_term' = 'Net New Hire Target');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `planned_part_time_headcount` SET TAGS ('dbx_business_glossary_term' = 'Planned Part-Time Headcount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `planned_seasonal_headcount` SET TAGS ('dbx_business_glossary_term' = 'Planned Seasonal Headcount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `planning_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Planning Period End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `planning_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planning Period Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `planning_period_type` SET TAGS ('dbx_business_glossary_term' = 'Planning Period Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`labor_budget` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `compensation_change_id` SET TAGS ('dbx_business_glossary_term' = 'Compensation Change ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `merit_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Merit Cycle ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `budget_impact_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Impact Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `change_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Change Effective Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `hr_approval_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected|cancelled');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `is_lump_sum` SET TAGS ('dbx_business_glossary_term' = 'Is Lump Sum Payment');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `new_pay_grade` SET TAGS ('dbx_business_glossary_term' = 'New Pay Grade');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `new_pay_rate` SET TAGS ('dbx_business_glossary_term' = 'New Pay Rate');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `pay_frequency` SET TAGS ('dbx_business_glossary_term' = 'Pay Frequency');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `pay_rate_change_amount` SET TAGS ('dbx_business_glossary_term' = 'Pay Rate Change Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `pay_rate_change_percentage` SET TAGS ('dbx_business_glossary_term' = 'Pay Rate Change Percentage');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `performance_rating` SET TAGS ('dbx_business_glossary_term' = 'Performance Rating');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `previous_pay_grade` SET TAGS ('dbx_business_glossary_term' = 'Previous Pay Grade');
ALTER TABLE `vibe_retail_v1`.`workforce`.`compensation_change` ALTER COLUMN `previous_pay_rate` SET TAGS ('dbx_business_glossary_term' = 'Previous Pay Rate');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Manager Employee ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `business_unit` SET TAGS ('dbx_business_glossary_term' = 'Business Unit');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_code` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,20}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,12}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `department_category` SET TAGS ('dbx_business_glossary_term' = 'Department Category');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `district_name` SET TAGS ('dbx_business_glossary_term' = 'District Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `district_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `division_name` SET TAGS ('dbx_business_glossary_term' = 'Division Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `division_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `external_system_code` SET TAGS ('dbx_business_glossary_term' = 'External System Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `gl_company_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Company Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `gl_company_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `hierarchy_path` SET TAGS ('dbx_business_glossary_term' = 'Organizational Hierarchy Path');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `is_customer_facing` SET TAGS ('dbx_business_glossary_term' = 'Customer Facing Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `is_revenue_generating` SET TAGS ('dbx_business_glossary_term' = 'Revenue Generating Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_level` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Hierarchy Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_name` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_status` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_status` SET TAGS ('dbx_value_regex' = 'active|inactive|pending|closed|suspended');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `org_unit_type` SET TAGS ('dbx_business_glossary_term' = 'Organizational Unit Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `osha_establishment_number` SET TAGS ('dbx_business_glossary_term' = 'OSHA Establishment ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `profit_center_code` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `profit_center_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,12}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `region_name` SET TAGS ('dbx_business_glossary_term' = 'Geographic Region Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `region_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `safety_committee_required` SET TAGS ('dbx_business_glossary_term' = 'Safety Committee Required Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `sort_order` SET TAGS ('dbx_business_glossary_term' = 'Display Sort Order');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `store_format` SET TAGS ('dbx_business_glossary_term' = 'Store Format Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `store_format` SET TAGS ('dbx_value_regex' = 'hypermarket|superstore|discount_store|neighborhood_market|express|online_only');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `time_zone` SET TAGS ('dbx_business_glossary_term' = 'Time Zone');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit` ALTER COLUMN `union_local_code` SET TAGS ('dbx_business_glossary_term' = 'Union Local Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` SET TAGS ('dbx_subdomain' = 'associate_development');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `renewed_from_wf_certification_id` SET TAGS ('dbx_business_glossary_term' = 'Renewed From Wf Certification Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Work Location ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_business_glossary_term' = 'Certification Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `certification_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `certification_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Number');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `certification_type` SET TAGS ('dbx_business_glossary_term' = 'Certification Type');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `continuing_education_hours_completed` SET TAGS ('dbx_business_glossary_term' = 'Continuing Education Hours Completed');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `continuing_education_hours_required` SET TAGS ('dbx_business_glossary_term' = 'Continuing Education Hours Required');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Cost Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `is_compliance_required` SET TAGS ('dbx_business_glossary_term' = 'Is Compliance Required Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `is_mandatory` SET TAGS ('dbx_business_glossary_term' = 'Is Mandatory Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Issue Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `issuing_authority_type` SET TAGS ('dbx_business_glossary_term' = 'Issuing Authority Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `issuing_authority_type` SET TAGS ('dbx_value_regex' = 'government|professional_association|internal_training|third_party_vendor|educational_institution|industry_body');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `issuing_body` SET TAGS ('dbx_business_glossary_term' = 'Issuing Body');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `job_eligibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Job Eligibility Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `reimbursement_amount` SET TAGS ('dbx_business_glossary_term' = 'Reimbursement Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `reimbursement_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Reimbursement Eligible Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `renewal_due_date` SET TAGS ('dbx_business_glossary_term' = 'Renewal Due Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `source_system_record_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Verification Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `verification_status` SET TAGS ('dbx_business_glossary_term' = 'Verification Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`wf_certification` ALTER COLUMN `verification_status` SET TAGS ('dbx_value_regex' = 'verified|pending_verification|failed_verification|not_verified');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` SET TAGS ('dbx_association_edges' = 'workforce.org_unit,workforce.job_profile');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `staffing_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Staffing Plan Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approver Associate ID');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `actual_headcount` SET TAGS ('dbx_business_glossary_term' = 'Actual Headcount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `annual_labor_cost_budget` SET TAGS ('dbx_business_glossary_term' = 'Annual Labor Cost Budget');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `budgeted_fte_count` SET TAGS ('dbx_business_glossary_term' = 'Budgeted FTE Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `headcount_planned` SET TAGS ('dbx_business_glossary_term' = 'Planned Headcount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `planning_cycle` SET TAGS ('dbx_business_glossary_term' = 'Planning Cycle');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `staffing_plan_status` SET TAGS ('dbx_business_glossary_term' = 'Staffing Plan Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`staffing_plan` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` SET TAGS ('dbx_subdomain' = 'associate_development');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `access_count` SET TAGS ('dbx_business_glossary_term' = 'Access Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `access_granted_date` SET TAGS ('dbx_business_glossary_term' = 'Access Granted Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `access_level` SET TAGS ('dbx_business_glossary_term' = 'Access Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `access_revoked_date` SET TAGS ('dbx_business_glossary_term' = 'Access Revoked Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `access_status` SET TAGS ('dbx_business_glossary_term' = 'Access Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `custom_filter_settings` SET TAGS ('dbx_business_glossary_term' = 'Custom Filter Settings');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `delivery_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `favorite_flag` SET TAGS ('dbx_business_glossary_term' = 'Favorite Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`dashboard_access` ALTER COLUMN `last_accessed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Accessed Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_ssot_authoritative_owner' = 'analytics.analytics_kpi_target');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_ssot_role' = 'non_authoritative');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_ssot_owner' = 'analytics.analytics_kpi_target');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_ssot_reference' = 'analytics.analytics_kpi_target');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` SET TAGS ('dbx_ssot_authoritative_source' = 'analytics.analytics_kpi_target');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `job_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `kpi_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Kpi Target - Kpi Definition Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Kpi Target - Org Unit Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Target Setter Associate Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `accountability_level` SET TAGS ('dbx_business_glossary_term' = 'Accountability Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Target Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Target Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `is_stretch_target` SET TAGS ('dbx_business_glossary_term' = 'Stretch Target Indicator');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `measurement_frequency` SET TAGS ('dbx_business_glossary_term' = 'Measurement Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Target Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `target_set_date` SET TAGS ('dbx_business_glossary_term' = 'Target Set Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `target_value` SET TAGS ('dbx_business_glossary_term' = 'Target Value');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `threshold_green` SET TAGS ('dbx_business_glossary_term' = 'Green Threshold Value');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `threshold_red` SET TAGS ('dbx_business_glossary_term' = 'Red Threshold Value');
ALTER TABLE `vibe_retail_v1`.`workforce`.`workforce_kpi_target` ALTER COLUMN `threshold_yellow` SET TAGS ('dbx_business_glossary_term' = 'Yellow Threshold Value');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` SET TAGS ('dbx_subdomain' = 'associate_development');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `org_program_compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Compliance Scope - Compliance Program Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Compliance Scope - Org Unit Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Scope Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `exemption_reason` SET TAGS ('dbx_business_glossary_term' = 'Exemption Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Scope Expiration Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `last_audit_result` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Result');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `next_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Date');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `responsible_party` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `scope_status` SET TAGS ('dbx_business_glossary_term' = 'Scope Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `training_completion_rate` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Rate');
ALTER TABLE `vibe_retail_v1`.`workforce`.`org_unit_compliance_scope` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `bargaining_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Bargaining Unit Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `superseded_collective_bargaining_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Collective Bargaining Agreement Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `union_id` SET TAGS ('dbx_business_glossary_term' = 'Union Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `agreement_name` SET TAGS ('dbx_business_glossary_term' = 'Agreement Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `agreement_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `agreement_number` SET TAGS ('dbx_business_glossary_term' = 'Agreement Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `agreement_type` SET TAGS ('dbx_business_glossary_term' = 'Agreement Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `arbitration_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Arbitration Required Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `covered_employee_count` SET TAGS ('dbx_business_glossary_term' = 'Covered Employee Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `covered_job_classifications` SET TAGS ('dbx_business_glossary_term' = 'Covered Job Classifications');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `covered_locations` SET TAGS ('dbx_business_glossary_term' = 'Covered Locations');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `document_url` SET TAGS ('dbx_business_glossary_term' = 'Document Url');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `dues_checkoff_authorized_flag` SET TAGS ('dbx_business_glossary_term' = 'Dues Checkoff Authorized Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `geographic_scope` SET TAGS ('dbx_business_glossary_term' = 'Geographic Scope');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `grievance_procedure_steps` SET TAGS ('dbx_business_glossary_term' = 'Grievance Procedure Steps');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `healthcare_contribution_employer_percentage` SET TAGS ('dbx_business_glossary_term' = 'Healthcare Contribution Employer Percentage');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `lockout_clause` SET TAGS ('dbx_business_glossary_term' = 'Lockout Clause');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `management_rights_clause` SET TAGS ('dbx_business_glossary_term' = 'Management Rights Clause');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `minimum_wage_rate` SET TAGS ('dbx_business_glossary_term' = 'Minimum Wage Rate');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `negotiation_end_date` SET TAGS ('dbx_business_glossary_term' = 'Negotiation End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `negotiation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Negotiation Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `overtime_multiplier` SET TAGS ('dbx_business_glossary_term' = 'Overtime Multiplier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `paid_time_off_days_annual` SET TAGS ('dbx_business_glossary_term' = 'Paid Time Off Days Annual');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `pension_contribution_rate` SET TAGS ('dbx_business_glossary_term' = 'Pension Contribution Rate');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `probationary_period_days` SET TAGS ('dbx_business_glossary_term' = 'Probationary Period Days');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `ratification_date` SET TAGS ('dbx_business_glossary_term' = 'Ratification Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `reopener_clause` SET TAGS ('dbx_business_glossary_term' = 'Reopener Clause');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `seniority_system_type` SET TAGS ('dbx_business_glossary_term' = 'Seniority System Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `sick_leave_days_annual` SET TAGS ('dbx_business_glossary_term' = 'Sick Leave Days Annual');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `collective_bargaining_agreement_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `strike_clause` SET TAGS ('dbx_business_glossary_term' = 'Strike Clause');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `successor_clause_included_flag` SET TAGS ('dbx_business_glossary_term' = 'Successor Clause Included Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `union_representative_name` SET TAGS ('dbx_business_glossary_term' = 'Union Representative Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `union_representative_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `union_security_clause` SET TAGS ('dbx_business_glossary_term' = 'Union Security Clause');
ALTER TABLE `vibe_retail_v1`.`workforce`.`collective_bargaining_agreement` ALTER COLUMN `wage_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Wage Increase Percentage');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `prior_merit_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Merit Cycle Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `prior_merit_cycle_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `approval_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Deadline Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `approval_workflow_required` SET TAGS ('dbx_business_glossary_term' = 'Approval Workflow Required');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `approved_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approved Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `budget_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `budget_percentage` SET TAGS ('dbx_business_glossary_term' = 'Budget Percentage');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `budget_percentage` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `calibration_required` SET TAGS ('dbx_business_glossary_term' = 'Calibration Required');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `closed_by` SET TAGS ('dbx_business_glossary_term' = 'Closed By');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `closed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Closed Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `communication_date` SET TAGS ('dbx_business_glossary_term' = 'Communication Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `cycle_code` SET TAGS ('dbx_business_glossary_term' = 'Cycle Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `cycle_name` SET TAGS ('dbx_business_glossary_term' = 'Cycle Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `cycle_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `cycle_type` SET TAGS ('dbx_business_glossary_term' = 'Cycle Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `merit_cycle_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `eligible_employee_count` SET TAGS ('dbx_business_glossary_term' = 'Eligible Employee Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `guideline_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Guideline Increase Percentage');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `maximum_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Maximum Increase Percentage');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `minimum_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Minimum Increase Percentage');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `modified_by` SET TAGS ('dbx_business_glossary_term' = 'Modified By');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `performance_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Performance Period End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `performance_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Performance Period Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `planning_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planning Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `proration_method` SET TAGS ('dbx_business_glossary_term' = 'Proration Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `review_end_date` SET TAGS ('dbx_business_glossary_term' = 'Review End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `review_start_date` SET TAGS ('dbx_business_glossary_term' = 'Review Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `merit_cycle_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`merit_cycle` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `requisition_id` SET TAGS ('dbx_business_glossary_term' = 'Job Requisition Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `referred_by_candidate_id` SET TAGS ('dbx_business_glossary_term' = 'Referred By Candidate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `referred_by_candidate_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `alternate_phone_number` SET TAGS ('dbx_business_glossary_term' = 'Alternate Phone Number');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `alternate_phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `application_date` SET TAGS ('dbx_business_glossary_term' = 'Application Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `application_number` SET TAGS ('dbx_business_glossary_term' = 'Application Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `availability_date` SET TAGS ('dbx_business_glossary_term' = 'Availability Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `background_check_completed_date` SET TAGS ('dbx_business_glossary_term' = 'Background Check Completed Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `background_check_status` SET TAGS ('dbx_business_glossary_term' = 'Background Check Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `candidate_status` SET TAGS ('dbx_business_glossary_term' = 'Candidate Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `cover_letter_file_path` SET TAGS ('dbx_business_glossary_term' = 'Cover Letter File Path');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `cover_letter_file_path` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `desired_salary` SET TAGS ('dbx_business_glossary_term' = 'Desired Salary');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `email_address` SET TAGS ('dbx_business_glossary_term' = 'Email Address');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `email_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `first_name` SET TAGS ('dbx_business_glossary_term' = 'First Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `first_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `highest_education_level` SET TAGS ('dbx_business_glossary_term' = 'Highest Education Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `interview_completed_date` SET TAGS ('dbx_business_glossary_term' = 'Interview Completed Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `interview_scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Interview Scheduled Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `is_eligible_for_rehire` SET TAGS ('dbx_business_glossary_term' = 'Is Eligible For Rehire');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `last_name` SET TAGS ('dbx_business_glossary_term' = 'Last Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `last_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `location_preference` SET TAGS ('dbx_business_glossary_term' = 'Location Preference');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `middle_name` SET TAGS ('dbx_business_glossary_term' = 'Middle Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `offer_accepted_date` SET TAGS ('dbx_business_glossary_term' = 'Offer Accepted Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `offer_extended_date` SET TAGS ('dbx_business_glossary_term' = 'Offer Extended Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `phone_number` SET TAGS ('dbx_business_glossary_term' = 'Phone Number');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `phone_number` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `position_applied_for` SET TAGS ('dbx_business_glossary_term' = 'Position Applied For');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `referral_source` SET TAGS ('dbx_business_glossary_term' = 'Referral Source');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `rejection_date` SET TAGS ('dbx_business_glossary_term' = 'Rejection Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `resume_file_path` SET TAGS ('dbx_business_glossary_term' = 'Resume File Path');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `resume_file_path` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `salary_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Salary Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `screening_score` SET TAGS ('dbx_business_glossary_term' = 'Screening Score');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `source` SET TAGS ('dbx_business_glossary_term' = 'Candidate Source');
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State Province');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`candidate` ALTER COLUMN `years_of_experience` SET TAGS ('dbx_business_glossary_term' = 'Years Of Experience');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Calendar Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_reversal_of_run_id` SET TAGS ('dbx_business_glossary_term' = 'Reversal Of Run Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `primary_reversal_payroll_run_id` SET TAGS ('dbx_business_glossary_term' = 'Reversal Payroll Run Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `check_date` SET TAGS ('dbx_business_glossary_term' = 'Check Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `company_code` SET TAGS ('dbx_business_glossary_term' = 'Company Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `employee_count` SET TAGS ('dbx_business_glossary_term' = 'Employee Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `employer_tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Employer Tax Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `frequency` SET TAGS ('dbx_business_glossary_term' = 'Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `gl_posting_date` SET TAGS ('dbx_business_glossary_term' = 'Gl Posting Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `gl_posting_status` SET TAGS ('dbx_business_glossary_term' = 'Gl Posting Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `gross_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Pay Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `integration_batch_code` SET TAGS ('dbx_business_glossary_term' = 'Integration Batch Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `is_final_run` SET TAGS ('dbx_business_glossary_term' = 'Is Final Run');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `net_pay_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Pay Amount');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `pay_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `payment_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `processing_end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Processing End Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `processing_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Processing Start Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `run_number` SET TAGS ('dbx_business_glossary_term' = 'Run Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `run_type` SET TAGS ('dbx_business_glossary_term' = 'Run Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `payroll_run_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `tax_filing_quarter` SET TAGS ('dbx_business_glossary_term' = 'Tax Filing Quarter');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `tax_year` SET TAGS ('dbx_business_glossary_term' = 'Tax Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `total_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Cost Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_run` ALTER COLUMN `total_deductions_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Deductions Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `counterpart_shift_swap_request_id` SET TAGS ('dbx_business_glossary_term' = 'Counterpart Shift Swap Request Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `shift_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Proposed Shift Shift Schedule Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approving Manager Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `shift_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store Location Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `target_employee_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Target Employee Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `approval_notes` SET TAGS ('dbx_business_glossary_term' = 'Approval Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `auto_approved` SET TAGS ('dbx_business_glossary_term' = 'Auto Approved');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completion Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `compliance_check_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Check Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `compliance_violation_details` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Details');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `expiration_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Expiration Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `notification_sent` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Notification Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `request_number` SET TAGS ('dbx_business_glossary_term' = 'Request Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `request_reason` SET TAGS ('dbx_business_glossary_term' = 'Request Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `request_reason_category` SET TAGS ('dbx_business_glossary_term' = 'Request Reason Category');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `request_status` SET TAGS ('dbx_business_glossary_term' = 'Request Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `request_type` SET TAGS ('dbx_business_glossary_term' = 'Request Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `requires_manager_approval` SET TAGS ('dbx_business_glossary_term' = 'Requires Manager Approval');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `response_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Response Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `submitted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Submitted Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`shift_swap_request` ALTER COLUMN `swap_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Swap Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` SET TAGS ('dbx_subdomain' = 'scheduling_labor');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approver Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_covering_employee_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Covering Employee Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_requesting_employee_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Requesting Employee Associate Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `superseded_coverage_request_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Coverage Request Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `shift_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Shift Schedule Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `approval_notes` SET TAGS ('dbx_business_glossary_term' = 'Approval Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `approval_required` SET TAGS ('dbx_business_glossary_term' = 'Approval Required');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `cancellation_date` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_end_time` SET TAGS ('dbx_business_glossary_term' = 'Coverage End Time');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_start_time` SET TAGS ('dbx_business_glossary_term' = 'Coverage Start Time');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Duration Hours');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `fulfillment_date` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `fulfillment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `labor_cost_center` SET TAGS ('dbx_business_glossary_term' = 'Labor Cost Center');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `notification_sent` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Notification Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `premium_pay_eligible` SET TAGS ('dbx_business_glossary_term' = 'Premium Pay Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `premium_pay_rate` SET TAGS ('dbx_business_glossary_term' = 'Premium Pay Rate');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Priority');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Reason Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Reason Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `request_date` SET TAGS ('dbx_business_glossary_term' = 'Request Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `request_number` SET TAGS ('dbx_business_glossary_term' = 'Request Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `request_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Request Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `request_type` SET TAGS ('dbx_business_glossary_term' = 'Request Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `response_count` SET TAGS ('dbx_business_glossary_term' = 'Response Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`coverage_request` ALTER COLUMN `coverage_request_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `prior_pay_period_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Pay Period Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `prior_pay_period_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `approval_deadline` SET TAGS ('dbx_business_glossary_term' = 'Approval Deadline');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `calendar_month` SET TAGS ('dbx_business_glossary_term' = 'Calendar Month');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `calendar_year` SET TAGS ('dbx_business_glossary_term' = 'Calendar Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `closed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Closed Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `finalized_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Finalized Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Quarter');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `is_adjustment_period` SET TAGS ('dbx_business_glossary_term' = 'Is Adjustment Period');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `is_holiday_period` SET TAGS ('dbx_business_glossary_term' = 'Is Holiday Period');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `overtime_eligible` SET TAGS ('dbx_business_glossary_term' = 'Overtime Eligible');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `pay_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `payroll_cycle_code` SET TAGS ('dbx_business_glossary_term' = 'Payroll Cycle Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `payroll_processing_date` SET TAGS ('dbx_business_glossary_term' = 'Payroll Processing Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `period_name` SET TAGS ('dbx_business_glossary_term' = 'Period Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `period_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `period_number` SET TAGS ('dbx_business_glossary_term' = 'Period Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `period_type` SET TAGS ('dbx_business_glossary_term' = 'Period Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `pay_period_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `time_entry_method` SET TAGS ('dbx_business_glossary_term' = 'Time Entry Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `timesheet_submission_deadline` SET TAGS ('dbx_business_glossary_term' = 'Timesheet Submission Deadline');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `total_days_count` SET TAGS ('dbx_business_glossary_term' = 'Total Days Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`pay_period` ALTER COLUMN `working_days_count` SET TAGS ('dbx_business_glossary_term' = 'Working Days Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` SET TAGS ('dbx_subdomain' = 'people_management');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_id` SET TAGS ('dbx_business_glossary_term' = 'Union Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `parent_union_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Union Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `parent_union_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `abbreviation` SET TAGS ('dbx_business_glossary_term' = 'Union Abbreviation');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `affiliation` SET TAGS ('dbx_business_glossary_term' = 'Affiliation');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `bargaining_unit_description` SET TAGS ('dbx_business_glossary_term' = 'Bargaining Unit Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `certification_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_code` SET TAGS ('dbx_business_glossary_term' = 'Union Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_email` SET TAGS ('dbx_business_glossary_term' = 'Contact Email');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_person_name` SET TAGS ('dbx_business_glossary_term' = 'Contact Person Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_person_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_phone` SET TAGS ('dbx_business_glossary_term' = 'Contact Phone');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contract_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `contract_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `decertification_date` SET TAGS ('dbx_business_glossary_term' = 'Decertification Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `dues_amount` SET TAGS ('dbx_business_glossary_term' = 'Dues Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `dues_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `dues_calculation_method` SET TAGS ('dbx_business_glossary_term' = 'Dues Calculation Method');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `dues_frequency` SET TAGS ('dbx_business_glossary_term' = 'Dues Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `grievance_procedure_description` SET TAGS ('dbx_business_glossary_term' = 'Grievance Procedure Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `initiation_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Initiation Fee Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `initiation_fee_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `is_right_to_work_applicable` SET TAGS ('dbx_business_glossary_term' = 'Is Right To Work Applicable');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `local_number` SET TAGS ('dbx_business_glossary_term' = 'Local Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `member_count` SET TAGS ('dbx_business_glossary_term' = 'Member Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_name` SET TAGS ('dbx_business_glossary_term' = 'Union Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `national_union_name` SET TAGS ('dbx_business_glossary_term' = 'National Union Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `national_union_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Office Address Line1');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Office Address Line2');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_city` SET TAGS ('dbx_business_glossary_term' = 'Office City');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_country_code` SET TAGS ('dbx_business_glossary_term' = 'Office Country Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Office Postal Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `office_state_province` SET TAGS ('dbx_business_glossary_term' = 'Office State Province');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `recognition_date` SET TAGS ('dbx_business_glossary_term' = 'Recognition Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `union_type` SET TAGS ('dbx_business_glossary_term' = 'Union Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`union` ALTER COLUMN `website_url` SET TAGS ('dbx_business_glossary_term' = 'Website Url');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` SET TAGS ('dbx_subdomain' = 'people_management');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `parent_bargaining_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Bargaining Unit Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `certification_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `certification_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `contract_document_reference` SET TAGS ('dbx_business_glossary_term' = 'Contract Document Reference');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `contract_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Effective Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `contract_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Expiration Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `dues_amount` SET TAGS ('dbx_business_glossary_term' = 'Dues Amount');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `dues_frequency` SET TAGS ('dbx_business_glossary_term' = 'Dues Frequency');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `eligible_employee_count` SET TAGS ('dbx_business_glossary_term' = 'Eligible Employee Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `geographic_scope` SET TAGS ('dbx_business_glossary_term' = 'Geographic Scope');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `grievance_procedure_reference` SET TAGS ('dbx_business_glossary_term' = 'Grievance Procedure Reference');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `health_benefits_plan_code` SET TAGS ('dbx_business_glossary_term' = 'Health Benefits Plan Code');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `jurisdiction_state` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction State');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `last_negotiation_date` SET TAGS ('dbx_business_glossary_term' = 'Last Negotiation Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `last_strike_date` SET TAGS ('dbx_business_glossary_term' = 'Last Strike Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `member_count` SET TAGS ('dbx_business_glossary_term' = 'Member Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `next_negotiation_scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Next Negotiation Scheduled Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `nlrb_region` SET TAGS ('dbx_business_glossary_term' = 'Nlrb Region');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `overtime_rules_reference` SET TAGS ('dbx_business_glossary_term' = 'Overtime Rules Reference');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `pension_plan_code` SET TAGS ('dbx_business_glossary_term' = 'Pension Plan Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Email');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Name');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_phone` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Phone');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `primary_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `seniority_system_type` SET TAGS ('dbx_business_glossary_term' = 'Seniority System Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `shift_differential_applicable` SET TAGS ('dbx_business_glossary_term' = 'Shift Differential Applicable');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `bargaining_unit_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `strike_history_flag` SET TAGS ('dbx_business_glossary_term' = 'Strike History Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `union_affiliation` SET TAGS ('dbx_business_glossary_term' = 'Union Affiliation');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `union_local_number` SET TAGS ('dbx_business_glossary_term' = 'Union Local Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `unit_code` SET TAGS ('dbx_business_glossary_term' = 'Unit Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `unit_description` SET TAGS ('dbx_business_glossary_term' = 'Unit Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `unit_name` SET TAGS ('dbx_business_glossary_term' = 'Unit Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `unit_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `unit_type` SET TAGS ('dbx_business_glossary_term' = 'Unit Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`bargaining_unit` ALTER COLUMN `wage_scale_reference` SET TAGS ('dbx_business_glossary_term' = 'Wage Scale Reference');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Business Unit Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `prior_payroll_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Payroll Calendar Id');
...
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `adjustment_period_flag` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Period Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `approval_deadline` SET TAGS ('dbx_business_glossary_term' = 'Approval Deadline');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_code` SET TAGS ('dbx_business_glossary_term' = 'Calendar Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_month` SET TAGS ('dbx_business_glossary_term' = 'Calendar Month');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_name` SET TAGS ('dbx_business_glossary_term' = 'Calendar Name');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_type` SET TAGS ('dbx_business_glossary_term' = 'Calendar Type');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `calendar_year` SET TAGS ('dbx_business_glossary_term' = 'Calendar Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `check_date` SET TAGS ('dbx_business_glossary_term' = 'Check Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_business_glossary_term' = 'Created By User Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `payroll_calendar_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `employee_group_code` SET TAGS ('dbx_business_glossary_term' = 'Employee Group Code');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `fiscal_quarter` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Quarter');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `last_modified_by_user_code` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `last_modified_by_user_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `last_modified_by_user_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `off_cycle_flag` SET TAGS ('dbx_business_glossary_term' = 'Off Cycle Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `pay_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `pay_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period End Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `pay_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Start Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `payroll_processing_date` SET TAGS ('dbx_business_glossary_term' = 'Payroll Processing Date');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `period_number` SET TAGS ('dbx_business_glossary_term' = 'Period Number');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `payroll_calendar_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `timesheet_submission_deadline` SET TAGS ('dbx_business_glossary_term' = 'Timesheet Submission Deadline');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `total_days_count` SET TAGS ('dbx_business_glossary_term' = 'Total Days Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `workday_calendar_code` SET TAGS ('dbx_business_glossary_term' = 'Workday Calendar Id');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `working_days_count` SET TAGS ('dbx_business_glossary_term' = 'Working Days Count');
ALTER TABLE `vibe_retail_v1`.`workforce`.`payroll_calendar` ALTER COLUMN `year_end_flag` SET TAGS ('dbx_business_glossary_term' = 'Year End Flag');
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` SET TAGS ('dbx_subdomain' = 'associate_development');
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ALTER COLUMN `access_request_id` SET TAGS ('dbx_business_glossary_term' = 'Access Request Identifier');
ALTER TABLE `vibe_retail_v1`.`workforce`.`access_request` ALTER COLUMN `delegated_from_access_request_id` SET TAGS ('dbx_self_ref_fk' = 'true');
