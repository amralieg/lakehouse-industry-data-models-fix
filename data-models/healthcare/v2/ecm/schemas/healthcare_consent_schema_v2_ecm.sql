...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`form_template` (
    ...
    `exchange_standard_id` BIGINT COMMENT 'FK to exchange standard if form is exchanged electronically',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` (
    ...
    `cda_document_id` BIGINT COMMENT 'FK to CDA document',
    ...
    `created_timestamp` TIMESTAMP COMMENT 'Record creation timestamp',
    `disclosing_party_name` STRING COMMENT 'Name of disclosing party',
    `disclosing_party_npi` STRING COMMENT 'NPI of disclosing party',
    ...
    `form_version` STRING COMMENT 'Version of authorization form',
    `irb_approval_number` STRING COMMENT 'IRB approval number',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` (
    ...
    `consent_record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: The consent_record product description explicitly states it is Referenced by disclosure_consent_record_id, and disclosure_log records every PHI disclosure made under a patient consent or authorizati',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user who last modified this consent policy record.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`deficiency` (
    ...
    `primary_deficiency_created_by_user_employee_id` BIGINT COMMENT 'Reference to the user who created the consent deficiency record.',
    `quaternary_deficiency_last_updated_by_user_employee_id` BIGINT COMMENT 'Reference to the user who last updated the consent deficiency record.',
    `quinary_deficiency_remediated_by_user_employee_id` BIGINT COMMENT 'Reference to the user or staff member who performed the remediation action.',
    `consent_record_id` BIGINT COMMENT 'Reference to the related consent record if one exists but is deficient.',
    `tertiary_deficiency_escalated_to_user_employee_id` BIGINT COMMENT 'Reference to the user or authority to whom the consent deficiency was escalated.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` (
    ...
    `workflow_id` BIGINT COMMENT 'FK to behavioral_health.part2_consent_workflow; links this 42 CFR Part 2 consent record to the disclosure governance workflow that controls redisclosure of SUD/behavioral health treatment data.',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` (
    ...
    `consent_record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: expiration_alert tracks consent records approaching or past their expiration date that require proactive renewal. Each alert is generated for a specific consent record. consent_record is the generic S',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`consent_record` (
    ...
    `care_site_id` BIGINT COMMENT 'Reference to the facility or care site where the consent was obtained, supporting multi-facility health system governance.',
    `consent_policy_id` BIGINT COMMENT 'Reference to the governing consent policy that defines the rules, scope, and legal basis under which this consent record was captured.',
    ...
    `superseded_by_consent_record_id` BIGINT COMMENT 'Self-reference to the consent record that replaces this one when a patient re-consents or updates their authorization, preserving consent version lineage.',
    ...
    `supersedes_consent_record_id` BIGINT COMMENT 'Self-referencing FK on consent_record (supersedes_consent_record_id)',
...
CREATE OR REPLACE TABLE `vibe_healthcare_v1`.`consent`.`workflow` (
    `workflow_id` BIGINT COMMENT 'Primary key for workflow',
    `consent_policy_id` BIGINT COMMENT 'Reference to the consent policy that this workflow operationalizes and enforces, linking the process definition to its governing rule set.',
    `allows_electronic_signature` BOOLEAN COMMENT 'Indicates whether the workflow permits electronic signature capture for consent, governed by ESIGN Act and 21 CFR Part 11 requirements.',
    `allows_proxy_consent` BOOLEAN COMMENT 'Indicates whether the workflow permits consent by a legally authorized representative, guardian, or healthcare proxy on behalf of the patient.',
    `allows_revocation` BOOLEAN COMMENT 'Indicates whether patients may revoke consent captured through this workflow, a mandatory right under HIPAA authorization rules.',
    `applicable_facility_scope` STRING COMMENT 'Organizational scope at which the workflow applies, indicating whether it is enterprise-wide or restricted to a specific facility, department, or service line.',
    `approval_date` DATE COMMENT 'Date the workflow definition version was formally approved for use.',
    `approved_by_role` STRING COMMENT 'Role or committee that approved this workflow definition version for production use, supporting governance audit trails.',
    `auto_routing_enabled` BOOLEAN COMMENT 'Indicates whether the workflow automatically routes consent tasks to the appropriate role or queue without manual assignment.',
    `consent_context_type` STRING COMMENT 'Categorical classification of the consent context the workflow governs (patient treatment consent, research consent, data sharing authorization, HIPAA authorization, HIE opt-in/opt-out, telehealth consent). CLASSIFICATION_OR_TYPE category.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this workflow definition record was first captured in the consent system of record.',
    `data_retention_years` STRING COMMENT 'Number of years consent records produced by this workflow must be retained to satisfy HIPAA and applicable state retention requirements.',
    `default_language_code` STRING COMMENT 'Default ISO 639-1 language code in which consent materials are presented within this workflow when no patient preference is specified.',
    `workflow_description` STRING COMMENT 'Detailed narrative explaining the purpose and scope of the consent workflow, the steps it orchestrates, and the consent context it governs. Supports governance documentation and self-service understanding.',
    `effective_from_date` DATE COMMENT 'Date on which this workflow definition version becomes effective and available for consent capture.',
    `effective_until_date` DATE COMMENT 'Date on which this workflow definition version ceases to be effective; null for open-ended active workflows.',
    `escalation_threshold_hours` STRING COMMENT 'Number of hours a pending consent task may remain incomplete before the workflow triggers escalation to a supervisor or compliance officer.',
    `expiration_period_days` STRING COMMENT 'Default number of days after capture for which consent obtained via this workflow remains valid before requiring renewal.',
    `form_template_id` BIGINT COMMENT 'Reference to the default consent form template presented to the patient or subject within this workflow.',
    `is_current_version` BOOLEAN COMMENT 'Indicates whether this row represents the currently active version of the workflow definition among all versions sharing the same workflow_code.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this workflow definition record was last modified, supporting change tracking and governance.',
    `minimum_age_requirement` STRING COMMENT 'Minimum age in years at which a patient may self-consent within this workflow, controlling routing to proxy or guardian consent paths for minors.',
    `workflow_name` STRING COMMENT 'Human-readable name of the consent workflow (e.g., Inpatient Treatment Consent Process). IDENTITY_LABEL category supporting business user navigation and reporting.',
    `notification_channel` STRING COMMENT 'Primary channel used to notify patients or subjects of consent tasks and renewal reminders within this workflow.',
    `owner_role` STRING COMMENT 'The accountable role or governance body responsible for maintaining this workflow definition (e.g., Compliance Office, Research IRB, Privacy Officer).',
    `regulatory_basis` STRING COMMENT 'Primary regulatory framework that mandates or governs the consent captured by this workflow, driving compliance controls and retention rules.',
    `renewal_required` BOOLEAN COMMENT 'Indicates whether consent captured through this workflow must be periodically renewed rather than remaining open-ended.',
    `requires_interpreter` BOOLEAN COMMENT 'Indicates whether the workflow requires interpreter involvement and documentation when the patient has limited English proficiency, supporting language-access compliance.',
    `requires_legal_review` BOOLEAN COMMENT 'Indicates whether the workflow mandates a legal or compliance review step before consent is finalized.',
    `requires_witness` BOOLEAN COMMENT 'Indicates whether the workflow mandates a witness signature for valid consent capture, a key compliance control for high-risk procedures and research.',
    `step_count` STRING COMMENT 'Total number of discrete steps defined in the workflow, used for process configuration validation and completion tracking.',
    `superseded_by_consent_workflow_id` BIGINT COMMENT 'Self-reference to the consent workflow definition that supersedes this version when it is deprecated or replaced, supporting version lineage. Renamed per next_vibes P5-P20 disambiguation pass to remove table-name stutter.',
    `version_number` STRING COMMENT 'Semantic version identifier of the consent workflow definition, enabling controlled change management and reproducibility of which workflow version governed a given consent capture.',
    `workflow_category` STRING COMMENT 'Higher-level functional category segmenting the workflow population for governance and analytics. Secondary CLASSIFICATION_OR_TYPE category distinct from consent_context_type.',
    `workflow_code` STRING COMMENT 'Externally-known business code uniquely identifying the consent workflow definition (e.g., TREAT-CONSENT-V2, RESEARCH-IC-001). Used by source systems and integration interfaces to reference the workflow. BUSINESS_IDENTIFIER category.',
    `workflow_status` STRING COMMENT 'Current lifecycle state of the workflow definition indicating whether it is available for use, under development, or retired. LIFECYCLE_STATUS category.',
    CONSTRAINT pk_workflow PRIMARY KEY(`workflow_id`)
) COMMENT 'Master reference table for workflow. ';
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ADD CONSTRAINT `fk_consent_disclosure_log_record_id` FOREIGN KEY (`record_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`consent_record`(`consent_record_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ADD CONSTRAINT `fk_consent_deficiency_parent_deficiency_id` FOREIGN KEY (`parent_deficiency_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`deficiency`(`deficiency_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ADD CONSTRAINT `fk_consent_substance_use_consent_workflow_id` FOREIGN KEY (`workflow_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`workflow`(`workflow_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ADD CONSTRAINT `fk_consent_behavioral_health_consent_workflow_id` FOREIGN KEY (`workflow_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`workflow`(`workflow_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ADD CONSTRAINT `fk_consent_expiration_alert_record_id` FOREIGN KEY (`record_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`consent_record`(`consent_record_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ADD CONSTRAINT `fk_consent_consent_record_superseded_by_consent_record_id` FOREIGN KEY (`superseded_by_consent_record_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`consent_record`(`consent_record_id`);
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ADD CONSTRAINT `fk_consent_consent_record_supersedes_consent_record_id` FOREIGN KEY (`supersedes_consent_record_id`) REFERENCES `vibe_healthcare_v1`.`consent`.`consent_record`(`consent_record_id`);
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` SET TAGS ('pii_consent_management' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` SET TAGS ('pii_regulatory_compliance' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `form_template_id` SET TAGS ('pii_business_glossary_term' = 'Form Template Identifier');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `compliance_policy_id` SET TAGS ('pii_business_glossary_term' = 'Compliance Policy');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_business_glossary_term' = 'Consent Policy');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `exchange_standard_id` SET TAGS ('pii_business_glossary_term' = 'Exchange Standard');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `primary_superseded_by_form_template_id` SET TAGS ('pii_business_glossary_term' = 'Superseded By Form Template');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_category` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_category` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_subcategory` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `consent_subcategory` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `electronic_signature_enabled_flag` SET TAGS ('pii_business_glossary_term' = 'Electronic Signature Enabled');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `form_document_url` SET TAGS ('pii_business_glossary_term' = 'Form Document URL');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `form_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `irb_approval_date` SET TAGS ('pii_business_glossary_term' = 'IRB Approval Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `irb_approval_number` SET TAGS ('pii_business_glossary_term' = 'IRB Approval Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `irb_expiration_date` SET TAGS ('pii_business_glossary_term' = 'IRB Expiration Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `irb_expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `legal_representative_allowed_flag` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Allowed');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `minor_assent_required_flag` SET TAGS ('pii_business_glossary_term' = 'Minor Assent Required');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `revocation_allowed_flag` SET TAGS ('pii_business_glossary_term' = 'Revocation Allowed');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `scope_of_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `scope_of_consent` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`form_template` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` SET TAGS ('pii_hipaa_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` SET TAGS ('pii_regulatory_compliance' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `hipaa_authorization_id` SET TAGS ('pii_business_glossary_term' = 'HIPAA Authorization Identifier');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Care Site');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `compliance_policy_id` SET TAGS ('pii_business_glossary_term' = 'Compliance Policy');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_business_glossary_term' = 'Demographics');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Employee');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `research_study_id` SET TAGS ('pii_business_glossary_term' = 'Research Study');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `superseded_hipaa_authorization_id` SET TAGS ('pii_business_glossary_term' = 'Superseded HIPAA Authorization');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `cda_document_id` SET TAGS ('pii_business_glossary_term' = 'Transmitted CDA Document');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Visit');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `disclosing_party_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `disclosing_party_npi` SET TAGS ('pii_business_glossary_term' = 'Disclosing Party NPI');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `disclosing_party_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `disclosing_party_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `expiration_event` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `irb_approval_number` SET TAGS ('pii_business_glossary_term' = 'IRB Approval Number');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `phi_category` SET TAGS ('pii_business_glossary_term' = 'PHI Category');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `phi_description` SET TAGS ('pii_business_glossary_term' = 'PHI Description');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `recipient_address` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `recipient_address` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `recipient_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `signature_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Signature Obtained');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hipaa_authorization` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `demographics_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `demographics_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `consent_form_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `consent_form_version` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `consent_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `consent_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `patient_education_provided` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `patient_education_provided` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `patient_instructions` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `patient_instructions` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`hie_directive` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `treatment_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `cpt_code_id` SET TAGS ('pii_phi' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `superseded_treatment_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `capacity_determination` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_document_location` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_document_location` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_form_number` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_form_number` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `consent_version` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `legal_representative_phone` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `patient_questions_addressed_flag` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `patient_questions_notes` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `patient_questions_notes` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `revoked_by_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`treatment_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `research_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `research_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `superseded_research_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `superseded_research_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_discussion_duration_minutes` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_discussion_duration_minutes` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_discussion_duration_minutes` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_document_url` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_document_url` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_location` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_location` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `consenting_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `contact_for_future_studies_authorized` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `contact_for_future_studies_authorized` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `lar_contact_phone` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `lar_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_date` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_reason` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_reason` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_required` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `reconsent_required` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`research_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `telehealth_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `superseded_telehealth_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_document_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_document_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_obtained_by` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_obtained_by` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_obtained_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `consent_obtained_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `legal_guardian_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `legal_guardian_relationship` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `legal_guardian_relationship` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `minor_consent_applicable` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `minor_consent_applicable` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `patient_location_state` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `recording_consent_provided` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `recording_consent_provided` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `telehealth_modality` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `telehealth_platform` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`telehealth_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `minor_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `minor_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `superseded_minor_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `superseded_minor_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_document_reference` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_document_reference` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_form_version` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_language` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_scope_description` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `consent_scope_description` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `court_ordered_consent_restriction_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `court_ordered_consent_restriction_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `minor_age_at_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `minor_age_at_consent` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `minor_consenting_independently_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `parent_guardian_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `parental_consent_required_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `parental_consent_required_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `research_consent_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `research_consent_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `state_specific_minor_consent_category` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`minor_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_address_line1` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_address_line2` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_city` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_contact_email` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_contact_phone` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `delegate_postal_code` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `emergency_contact_flag` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `emergency_contact_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`delegation` ALTER COLUMN `emergency_contact_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Identifier (ID)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `patient_notification_sent_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `patient_notification_sent_flag` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `patient_notification_timestamp` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `patient_notification_timestamp` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`revocation` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` ALTER COLUMN `ip_address` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` ALTER COLUMN `patient_portal_session_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` ALTER COLUMN `patient_portal_session_code` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` ALTER COLUMN `retention_expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`npp_acknowledgment` ALTER COLUMN `signer_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `insurance_coverage_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record Id (Foreign Key)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `decision_rationale` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `operational_instructions` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`restriction_request` ALTER COLUMN `restricted_recipient_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Disclosure Consent Record Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `consent_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `consent_record_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `record_id` SET TAGS ('pii_business_glossary_term' = 'Consent ID');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `patient_notification_required` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `patient_notification_required` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `recipient_address` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`disclosure_log` ALTER COLUMN `recipient_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_assessment_id` SET TAGS ('pii_business_glossary_term' = 'Consent Capacity Assessment ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_assessment_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_assessment_id` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Assessing Clinician ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `compliance_policy_id` SET TAGS ('pii_business_glossary_term' = 'Compliance Policy Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `delegation_id` SET TAGS ('pii_business_glossary_term' = 'Surrogate Decision Maker ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Witness ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `employee_id` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `prior_capacity_assessment_id` SET TAGS ('pii_business_glossary_term' = 'Prior Capacity Assessment Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `prior_capacity_assessment_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `prior_capacity_assessment_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `prior_capacity_assessment_id` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `amendment_reason` SET TAGS ('pii_business_glossary_term' = 'Amendment Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `appreciation_ability` SET TAGS ('pii_business_glossary_term' = 'Appreciation Ability');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `appreciation_ability` SET TAGS ('pii_value_regex' = 'intact|impaired|severely_impaired|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessing_clinician_specialty` SET TAGS ('pii_business_glossary_term' = 'Assessing Clinician Specialty');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_date` SET TAGS ('pii_business_glossary_term' = 'Assessment Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_duration_minutes` SET TAGS ('pii_business_glossary_term' = 'Assessment Duration Minutes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_duration_minutes` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_location` SET TAGS ('pii_business_glossary_term' = 'Assessment Location');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_status` SET TAGS ('pii_business_glossary_term' = 'Assessment Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_status` SET TAGS ('pii_value_regex' = 'completed|in_progress|cancelled|amended|entered_in_error');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_timestamp` SET TAGS ('pii_business_glossary_term' = 'Assessment Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_tool_used` SET TAGS ('pii_business_glossary_term' = 'Assessment Tool Used');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `assessment_tool_version` SET TAGS ('pii_business_glossary_term' = 'Assessment Tool Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_determination` SET TAGS ('pii_business_glossary_term' = 'Capacity Determination');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_determination` SET TAGS ('pii_value_regex' = 'full_capacity|diminished_capacity|lacks_capacity|indeterminate|reassessment_required');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_determination` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_determination` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_score` SET TAGS ('pii_business_glossary_term' = 'Capacity Score');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_score` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `capacity_score` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `choice_expression_ability` SET TAGS ('pii_business_glossary_term' = 'Choice Expression Ability');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `choice_expression_ability` SET TAGS ('pii_value_regex' = 'intact|impaired|severely_impaired|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `clinical_basis_for_determination` SET TAGS ('pii_business_glossary_term' = 'Clinical Basis for Determination');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `clinical_basis_for_determination` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `contributing_conditions` SET TAGS ('pii_business_glossary_term' = 'Contributing Conditions');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `contributing_conditions` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `contributing_medications` SET TAGS ('pii_business_glossary_term' = 'Contributing Medications');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `contributing_medications` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `contributing_medications` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `documentation_reference_code` SET TAGS ('pii_business_glossary_term' = 'Documentation Reference ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `ethics_consultation_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Ethics Consultation Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `language_code` SET TAGS ('pii_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `language_code` SET TAGS ('pii_value_regex' = '^[a-z]{2,3}$');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `legal_authority_type` SET TAGS ('pii_business_glossary_term' = 'Legal Authority Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `legal_authority_type` SET TAGS ('pii_value_regex' = 'healthcare_proxy|power_of_attorney|legal_guardian|next_of_kin|court_appointed|none');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `legal_consultation_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Legal Consultation Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `reasoning_ability` SET TAGS ('pii_business_glossary_term' = 'Reasoning Ability');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `reasoning_ability` SET TAGS ('pii_value_regex' = 'intact|impaired|severely_impaired|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `reassessment_recommended_flag` SET TAGS ('pii_business_glossary_term' = 'Reassessment Recommended Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `reassessment_timeframe` SET TAGS ('pii_business_glossary_term' = 'Reassessment Timeframe');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `source_system_record_code` SET TAGS ('pii_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `specific_deficits_identified` SET TAGS ('pii_business_glossary_term' = 'Specific Deficits Identified');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `surrogate_decision_maker_engaged_flag` SET TAGS ('pii_business_glossary_term' = 'Surrogate Decision Maker Engaged Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `surrogate_relationship` SET TAGS ('pii_business_glossary_term' = 'Surrogate Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `understanding_ability` SET TAGS ('pii_business_glossary_term' = 'Understanding Ability');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `understanding_ability` SET TAGS ('pii_value_regex' = 'intact|impaired|severely_impaired|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`capacity_assessment` ALTER COLUMN `witness_present_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Present Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` SET TAGS ('pii_data_type' = 'reference_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` SET TAGS ('pii_subdomain' = 'compliance_governance');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` SET TAGS ('pii_'ecm_member' = 'true]');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `primary_superseded_by_consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `primary_superseded_by_consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Last Updated By User Identifier (ID)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `employee_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `employee_id` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `capacity_assessment_required_flag` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `capacity_assessment_triggers` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_category` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_category` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_subcategory` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `consent_subcategory` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `irb_expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `policy_name` SET TAGS ('pii_uc_classification' = 'pii');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_interval_months` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_interval_months` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_new_risk` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_new_risk` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_protocol_amendment` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_protocol_amendment` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_time_based` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `reconsent_trigger_time_based` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `required_consent_elements` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_policy` ALTER COLUMN `required_consent_elements` SET TAGS ('pii_uc_classification' = 'phi');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` SET TAGS ('pii_subdomain' = 'compliance_governance');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_id` SET TAGS ('pii_business_glossary_term' = 'Consent Deficiency Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Waiver Approved By User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `audit_finding_id` SET TAGS ('pii_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Responsible Provider Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_business_glossary_term' = 'Consent Policy Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `corrective_action_plan_id` SET TAGS ('pii_business_glossary_term' = 'Corrective Action Plan Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `form_template_id` SET TAGS ('pii_business_glossary_term' = 'Consent Form Template Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `parent_deficiency_id` SET TAGS ('pii_business_glossary_term' = 'Related Deficiency Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `parent_deficiency_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `primary_deficiency_created_by_user_employee_id` SET TAGS ('pii_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `primary_deficiency_created_by_user_employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `primary_deficiency_created_by_user_employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quaternary_deficiency_last_updated_by_user_employee_id` SET TAGS ('pii_business_glossary_term' = 'Last Updated By User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quaternary_deficiency_last_updated_by_user_employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quaternary_deficiency_last_updated_by_user_employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quinary_deficiency_remediated_by_user_employee_id` SET TAGS ('pii_business_glossary_term' = 'Remediated By User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quinary_deficiency_remediated_by_user_employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `quinary_deficiency_remediated_by_user_employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `tertiary_deficiency_escalated_to_user_employee_id` SET TAGS ('pii_business_glossary_term' = 'Escalated To User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `tertiary_deficiency_escalated_to_user_employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `tertiary_deficiency_escalated_to_user_employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `workflow_id` SET TAGS ('pii_business_glossary_term' = 'Workflow Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `audit_reference_number` SET TAGS ('pii_business_glossary_term' = 'Audit Reference Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_category` SET TAGS ('pii_business_glossary_term' = 'Deficiency Category');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_category` SET TAGS ('pii_value_regex' = 'clinical|research|administrative|hipaa_authorization|hie_consent|telehealth');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_number` SET TAGS ('pii_business_glossary_term' = 'Deficiency Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_status` SET TAGS ('pii_business_glossary_term' = 'Deficiency Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_status` SET TAGS ('pii_value_regex' = 'open|in_progress|remediated|waived|closed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_type` SET TAGS ('pii_business_glossary_term' = 'Deficiency Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `deficiency_description` SET TAGS ('pii_business_glossary_term' = 'Deficiency Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `discovered_by_role` SET TAGS ('pii_business_glossary_term' = 'Discovered By Role');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `discovery_date` SET TAGS ('pii_business_glossary_term' = 'Discovery Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `discovery_method` SET TAGS ('pii_business_glossary_term' = 'Discovery Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `discovery_timestamp` SET TAGS ('pii_business_glossary_term' = 'Discovery Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `escalation_date` SET TAGS ('pii_business_glossary_term' = 'Escalation Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `escalation_required_flag` SET TAGS ('pii_business_glossary_term' = 'Escalation Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `patient_safety_impact_flag` SET TAGS ('pii_business_glossary_term' = 'Patient Safety Impact Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `patient_safety_impact_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `patient_safety_impact_flag` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_code` SET TAGS ('pii_business_glossary_term' = 'Procedure Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_code` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_code` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_description` SET TAGS ('pii_business_glossary_term' = 'Procedure Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_description` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `procedure_description` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `regulatory_impact_flag` SET TAGS ('pii_business_glossary_term' = 'Regulatory Impact Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `remediation_action_taken` SET TAGS ('pii_business_glossary_term' = 'Remediation Action Taken');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `remediation_date` SET TAGS ('pii_business_glossary_term' = 'Remediation Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `remediation_required_flag` SET TAGS ('pii_business_glossary_term' = 'Remediation Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `remediation_timestamp` SET TAGS ('pii_business_glossary_term' = 'Remediation Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `resolution_date` SET TAGS ('pii_business_glossary_term' = 'Resolution Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `resolution_notes` SET TAGS ('pii_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `responsible_department_code` SET TAGS ('pii_business_glossary_term' = 'Responsible Department Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_business_glossary_term' = 'Scheduled Procedure Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `severity_level` SET TAGS ('pii_business_glossary_term' = 'Severity Level');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `severity_level` SET TAGS ('pii_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `source_system_deficiency_code` SET TAGS ('pii_business_glossary_term' = 'Source System Deficiency Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `waiver_date` SET TAGS ('pii_business_glossary_term' = 'Waiver Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`deficiency` ALTER COLUMN `waiver_reason` SET TAGS ('pii_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `substance_use_consent_id` SET TAGS ('pii_business_glossary_term' = 'Substance Use Consent ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `substance_use_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `substance_use_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `workflow_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `workflow_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `superseded_substance_use_consent_id` SET TAGS ('pii_business_glossary_term' = 'Superseded Substance Use Consent Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `superseded_substance_use_consent_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `superseded_substance_use_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `superseded_substance_use_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Performed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Result');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_value_regex' = 'has_capacity|lacks_capacity|partial_capacity');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `cares_act_compliant_flag` SET TAGS ('pii_business_glossary_term' = 'Coronavirus Aid, Relief, and Economic Security (CARES) Act Compliant Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `disclosure_purpose` SET TAGS ('pii_business_glossary_term' = 'Disclosure Purpose');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `disclosure_purpose_category` SET TAGS ('pii_business_glossary_term' = 'Disclosure Purpose Category');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `disclosure_purpose_category` SET TAGS ('pii_value_regex' = 'treatment|payment|healthcare_operations|research|legal|other');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `document_reference` SET TAGS ('pii_business_glossary_term' = 'Document Reference');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `expiration_event` SET TAGS ('pii_business_glossary_term' = 'Expiration Event');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `expiration_event` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `form_version` SET TAGS ('pii_business_glossary_term' = 'Form Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `information_disclosed_description` SET TAGS ('pii_business_glossary_term' = 'Information Disclosed Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_business_glossary_term' = 'Interpreter Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `language_code` SET TAGS ('pii_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_flag` SET TAGS ('pii_business_glossary_term' = 'Personal Representative Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_business_glossary_term' = 'Personal Representative Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `personal_representative_relationship` SET TAGS ('pii_business_glossary_term' = 'Personal Representative Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_address` SET TAGS ('pii_business_glossary_term' = 'Recipient Address');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_address` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_address` SET TAGS ('pii_address' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_address` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_address` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_name` SET TAGS ('pii_business_glossary_term' = 'Recipient Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_name` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_npi` SET TAGS ('pii_business_glossary_term' = 'Recipient National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `recipient_organization` SET TAGS ('pii_business_glossary_term' = 'Recipient Organization');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `redisclosure_prohibition_included_flag` SET TAGS ('pii_business_glossary_term' = 'Redisclosure Prohibition Included Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `redisclosure_prohibition_notice` SET TAGS ('pii_business_glossary_term' = 'Redisclosure Prohibition Notice');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `right_to_revoke_statement` SET TAGS ('pii_business_glossary_term' = 'Right to Revoke Statement');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `signature_date` SET TAGS ('pii_business_glossary_term' = 'Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_business_glossary_term' = 'Signature Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_value_regex' = 'wet_signature|electronic|digital|biometric');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `signature_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Signature Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_name` SET TAGS ('pii_business_glossary_term' = 'Substance Use Disorder (SUD) Program Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_name` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_npi` SET TAGS ('pii_business_glossary_term' = 'Substance Use Disorder (SUD) Program National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_npi` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `sud_program_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_business_glossary_term' = 'Witness Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`substance_use_consent` ALTER COLUMN `witness_signature_date` SET TAGS ('pii_business_glossary_term' = 'Witness Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `behavioral_health_consent_id` SET TAGS ('pii_business_glossary_term' = 'Behavioral Health Consent ID');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `behavioral_health_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `workflow_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `workflow_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `superseded_behavioral_health_consent_id` SET TAGS ('pii_business_glossary_term' = 'Superseded Behavioral Health Consent Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `superseded_behavioral_health_consent_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `superseded_behavioral_health_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `superseded_behavioral_health_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_address` SET TAGS ('pii_business_glossary_term' = 'Authorized Recipient Address');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_address` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_address` SET TAGS ('pii_address' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_address` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_address` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_name` SET TAGS ('pii_business_glossary_term' = 'Authorized Recipient Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_name` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_npi` SET TAGS ('pii_business_glossary_term' = 'Authorized Recipient National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_type` SET TAGS ('pii_business_glossary_term' = 'Authorized Recipient Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `authorized_recipient_type` SET TAGS ('pii_value_regex' = 'individual|organization|provider|payer|legal_entity|research_institution');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Performed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_determination` SET TAGS ('pii_business_glossary_term' = 'Capacity Determination');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_determination` SET TAGS ('pii_value_regex' = 'has_capacity|lacks_capacity|partial_capacity|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_determination` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `capacity_determination` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_number` SET TAGS ('pii_business_glossary_term' = 'Behavioral Health Consent Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_number` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_number` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_status` SET TAGS ('pii_business_glossary_term' = 'Consent Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_status` SET TAGS ('pii_value_regex' = 'active|revoked|expired|pending|superseded|voided');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_status` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_status` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_type` SET TAGS ('pii_business_glossary_term' = 'Consent Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_type` SET TAGS ('pii_value_regex' = 'disclosure|treatment|research|data_sharing|hie_participation');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `consent_type` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `disclosure_purpose` SET TAGS ('pii_business_glossary_term' = 'Disclosure Purpose');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `disclosure_purpose_category` SET TAGS ('pii_business_glossary_term' = 'Disclosure Purpose Category');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `document_reference_code` SET TAGS ('pii_business_glossary_term' = 'Document Reference ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `effective_date` SET TAGS ('pii_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `expiration_date` SET TAGS ('pii_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `expiration_event` SET TAGS ('pii_business_glossary_term' = 'Expiration Event');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `expiration_event` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `form_version` SET TAGS ('pii_business_glossary_term' = 'Form Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_business_glossary_term' = 'Interpreter Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `language_code` SET TAGS ('pii_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_authority_type` SET TAGS ('pii_business_glossary_term' = 'Legal Authority Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_authority_type` SET TAGS ('pii_value_regex' = 'parental|guardianship|power_of_attorney|conservatorship|court_order');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_flag` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `legal_representative_relationship` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mental_health_data_elements_covered` SET TAGS ('pii_business_glossary_term' = 'Mental Health Data Elements Covered');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mental_health_data_elements_covered` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mental_health_data_elements_covered` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mental_health_data_elements_covered` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mental_health_data_elements_covered` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `patient_imposed_restrictions` SET TAGS ('pii_business_glossary_term' = 'Patient-Imposed Restrictions');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `patient_imposed_restrictions` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `patient_imposed_restrictions` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `psychotherapy_notes_included_flag` SET TAGS ('pii_business_glossary_term' = 'Psychotherapy Notes Included Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `psychotherapy_notes_included_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `redisclosure_prohibition_statement` SET TAGS ('pii_business_glossary_term' = 'Redisclosure Prohibition Statement');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `revocation_date` SET TAGS ('pii_business_glossary_term' = 'Revocation Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `revocation_method` SET TAGS ('pii_business_glossary_term' = 'Revocation Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `revocation_method` SET TAGS ('pii_value_regex' = 'written|verbal|electronic|in_person');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `revocation_reason` SET TAGS ('pii_business_glossary_term' = 'Revocation Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `right_to_revoke_statement` SET TAGS ('pii_business_glossary_term' = 'Right to Revoke Statement');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `signature_date` SET TAGS ('pii_business_glossary_term' = 'Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_business_glossary_term' = 'Signature Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_value_regex' = 'wet_signature|electronic|digital|biometric|verbal_recorded');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `signature_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Signature Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `state_jurisdiction` SET TAGS ('pii_business_glossary_term' = 'State Jurisdiction');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `state_law_basis` SET TAGS ('pii_business_glossary_term' = 'State Law Basis');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `substance_abuse_records_included_flag` SET TAGS ('pii_business_glossary_term' = 'Substance Abuse Records Included Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `substance_abuse_records_included_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_business_glossary_term' = 'Witness Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`behavioral_health_consent` ALTER COLUMN `witness_signature_date` SET TAGS ('pii_business_glossary_term' = 'Witness Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` SET TAGS ('pii_data_type' = 'transactional_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` SET TAGS ('pii_subdomain' = 'compliance_governance');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `expiration_alert_id` SET TAGS ('pii_business_glossary_term' = 'Consent Expiration Alert Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Responsible Staff Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Expiring Consent Record Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_record_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `org_unit_id` SET TAGS ('pii_business_glossary_term' = 'Assigned Department Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `prior_expiration_alert_id` SET TAGS ('pii_business_glossary_term' = 'Prior Expiration Alert Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `prior_expiration_alert_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Scheduled Encounter Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `workflow_id` SET TAGS ('pii_business_glossary_term' = 'Workflow Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_generation_date` SET TAGS ('pii_business_glossary_term' = 'Alert Generation Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_generation_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_generation_timestamp` SET TAGS ('pii_business_glossary_term' = 'Alert Generation Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_generation_timestamp` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_number` SET TAGS ('pii_business_glossary_term' = 'Alert Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_number` SET TAGS ('pii_value_regex' = '^CEA-[0-9]{10}$');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_priority` SET TAGS ('pii_business_glossary_term' = 'Alert Priority');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_priority` SET TAGS ('pii_value_regex' = 'low|medium|high|urgent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_status` SET TAGS ('pii_business_glossary_term' = 'Alert Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_status` SET TAGS ('pii_value_regex' = 'open|acknowledged|in_progress|resolved|escalated|cancelled');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_type` SET TAGS ('pii_business_glossary_term' = 'Alert Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `alert_type` SET TAGS ('pii_value_regex' = 'approaching_expiration|expired|renewal_required|grace_period|critical');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `auto_renewal_eligible_flag` SET TAGS ('pii_business_glossary_term' = 'Auto-Renewal Eligible Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `care_impact_flag` SET TAGS ('pii_business_glossary_term' = 'Care Impact Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_expiration_date` SET TAGS ('pii_business_glossary_term' = 'Consent Expiration Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_expiration_date` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_expiration_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_expiration_date` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_scope` SET TAGS ('pii_business_glossary_term' = 'Consent Scope');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_scope` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_scope` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_type` SET TAGS ('pii_business_glossary_term' = 'Consent Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `consent_type` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `days_until_expiration` SET TAGS ('pii_business_glossary_term' = 'Days Until Expiration');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `days_until_expiration` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `escalation_date` SET TAGS ('pii_business_glossary_term' = 'Escalation Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `escalation_flag` SET TAGS ('pii_business_glossary_term' = 'Escalation Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `escalation_reason` SET TAGS ('pii_business_glossary_term' = 'Escalation Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `last_reminder_date` SET TAGS ('pii_business_glossary_term' = 'Last Reminder Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `next_action_due_date` SET TAGS ('pii_business_glossary_term' = 'Next Action Due Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `notification_channel` SET TAGS ('pii_business_glossary_term' = 'Notification Channel');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `notification_delivery_status` SET TAGS ('pii_business_glossary_term' = 'Notification Delivery Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `notification_delivery_status` SET TAGS ('pii_value_regex' = 'pending|sent|delivered|failed|bounced|read');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `notification_sent_flag` SET TAGS ('pii_business_glossary_term' = 'Notification Sent Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `notification_sent_timestamp` SET TAGS ('pii_business_glossary_term' = 'Notification Sent Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_attempted_flag` SET TAGS ('pii_business_glossary_term' = 'Patient Contact Attempted Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_attempted_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_attempted_flag` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_date` SET TAGS ('pii_business_glossary_term' = 'Patient Contact Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_contact_date` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_response` SET TAGS ('pii_business_glossary_term' = 'Patient Response');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_response` SET TAGS ('pii_value_regex' = 'agreed_to_renew|declined_renewal|requested_modification|no_response|unreachable');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_response` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `patient_response` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `regulatory_compliance_flag` SET TAGS ('pii_business_glossary_term' = 'Regulatory Compliance Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `reminder_count` SET TAGS ('pii_business_glossary_term' = 'Reminder Count');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `renewal_method` SET TAGS ('pii_business_glossary_term' = 'Renewal Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `renewal_required_flag` SET TAGS ('pii_business_glossary_term' = 'Renewal Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `resolution_date` SET TAGS ('pii_business_glossary_term' = 'Resolution Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `resolution_notes` SET TAGS ('pii_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `resolution_status` SET TAGS ('pii_business_glossary_term' = 'Resolution Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `resolution_timestamp` SET TAGS ('pii_business_glossary_term' = 'Resolution Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `responsible_staff_role` SET TAGS ('pii_business_glossary_term' = 'Responsible Staff Role');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_business_glossary_term' = 'Scheduled Procedure Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `scheduled_procedure_date` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `sla_breach_flag` SET TAGS ('pii_business_glossary_term' = 'Service Level Agreement (SLA) Breach Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`expiration_alert` ALTER COLUMN `sla_target_resolution_hours` SET TAGS ('pii_business_glossary_term' = 'Service Level Agreement (SLA) Target Resolution Hours');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `genetic_testing_consent_id` SET TAGS ('pii_business_glossary_term' = 'Genetic Testing Consent Identifier (ID)');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `genetic_testing_consent_id` SET TAGS ('pii_uc_classification' = 'sensitive');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Genetic Counselor Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `form_template_id` SET TAGS ('pii_business_glossary_term' = 'Consent Form Template Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_study_id` SET TAGS ('pii_business_glossary_term' = 'Research Study Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `superseded_genetic_testing_consent_id` SET TAGS ('pii_business_glossary_term' = 'Superseded Genetic Testing Consent Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `superseded_genetic_testing_consent_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `superseded_genetic_testing_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `superseded_genetic_testing_consent_id` SET TAGS ('pii_sensitive' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `superseded_genetic_testing_consent_id` SET TAGS ('pii_uc_classification' = 'sensitive');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_flag` SET TAGS ('pii_business_glossary_term' = 'Authorized Representative Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_name` SET TAGS ('pii_business_glossary_term' = 'Authorized Representative Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `authorized_representative_relationship` SET TAGS ('pii_business_glossary_term' = 'Authorized Representative Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `biobank_storage_consent` SET TAGS ('pii_business_glossary_term' = 'Biobank Storage Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `biobank_storage_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized|time_limited');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `biobank_storage_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `biobank_storage_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Performed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Result');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_value_regex' = 'has_capacity|lacks_capacity|partial_capacity');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `commercial_use_consent` SET TAGS ('pii_business_glossary_term' = 'Commercial Use Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `commercial_use_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `commercial_use_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `commercial_use_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_decision` SET TAGS ('pii_business_glossary_term' = 'Genetic Testing Consent Decision');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_decision` SET TAGS ('pii_value_regex' = 'granted|declined|partial');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_decision` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_decision` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_scope` SET TAGS ('pii_business_glossary_term' = 'Genetic Testing Consent Scope');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_scope` SET TAGS ('pii_value_regex' = 'specific_test_only|future_research_use|biobank_storage|data_sharing|all');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_scope` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `consent_scope` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `counseling_date` SET TAGS ('pii_business_glossary_term' = 'Genetic Counseling Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `counseling_provided_flag` SET TAGS ('pii_business_glossary_term' = 'Genetic Counseling Provided Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `data_retention_period_years` SET TAGS ('pii_business_glossary_term' = 'Data Retention Period in Years');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `document_reference_code` SET TAGS ('pii_business_glossary_term' = 'Document Reference Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `family_implications_disclosed_flag` SET TAGS ('pii_business_glossary_term' = 'Family Implications Disclosed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `family_notification_consent` SET TAGS ('pii_business_glossary_term' = 'Family Notification Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `family_notification_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized|conditional');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `family_notification_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `family_notification_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `form_version` SET TAGS ('pii_business_glossary_term' = 'Consent Form Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `gina_disclosure_flag` SET TAGS ('pii_business_glossary_term' = 'Genetic Information Nondiscrimination Act (GINA) Disclosure Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `incidental_findings_consent` SET TAGS ('pii_business_glossary_term' = 'Incidental Findings Return Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `incidental_findings_consent` SET TAGS ('pii_value_regex' = 'return_all|return_medically_actionable|do_not_return|patient_to_decide_later');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `incidental_findings_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `incidental_findings_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `insurance_discrimination_risk_disclosed_flag` SET TAGS ('pii_business_glossary_term' = 'Insurance Discrimination Risk Disclosed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `insurance_discrimination_risk_disclosed_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `insurance_discrimination_risk_disclosed_flag` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `irb_approval_number` SET TAGS ('pii_business_glossary_term' = 'Institutional Review Board (IRB) Approval Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `language_code` SET TAGS ('pii_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Consent Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider National Provider Identifier (NPI)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `obtaining_provider_npi` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `recontact_consent` SET TAGS ('pii_business_glossary_term' = 'Recontact Consent for New Findings');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `recontact_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized|conditional');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `recontact_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `recontact_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_registry_sharing_consent` SET TAGS ('pii_business_glossary_term' = 'Research Registry Data Sharing Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_registry_sharing_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized|authorized_with_restrictions');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_registry_sharing_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_registry_sharing_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_use_consent` SET TAGS ('pii_business_glossary_term' = 'Research Use Consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_use_consent` SET TAGS ('pii_value_regex' = 'authorized|not_authorized|authorized_with_restrictions');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_use_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `research_use_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `signature_date` SET TAGS ('pii_business_glossary_term' = 'Patient Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_business_glossary_term' = 'Signature Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_value_regex' = 'electronic|wet_signature|verbal|proxy');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `signature_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Patient Signature Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `state_jurisdiction` SET TAGS ('pii_business_glossary_term' = 'State Jurisdiction Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `test_name` SET TAGS ('pii_business_glossary_term' = 'Genetic Test Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `test_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `test_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `test_purpose` SET TAGS ('pii_business_glossary_term' = 'Genetic Test Purpose');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `test_type` SET TAGS ('pii_business_glossary_term' = 'Genetic Test Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_business_glossary_term' = 'Witness Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`genetic_testing_consent` ALTER COLUMN `witness_signature_date` SET TAGS ('pii_business_glossary_term' = 'Witness Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `photography_media_consent_id` SET TAGS ('pii_business_glossary_term' = 'Photography Media Consent ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `photography_media_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `photography_media_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `clinician_id` SET TAGS ('pii_business_glossary_term' = 'Obtaining Provider ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `form_template_id` SET TAGS ('pii_business_glossary_term' = 'Consent Form Template ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Record Id (Foreign Key)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `research_study_id` SET TAGS ('pii_business_glossary_term' = 'Research Study ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `superseded_photography_media_consent_id` SET TAGS ('pii_business_glossary_term' = 'Superseded Photography Media Consent Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `superseded_photography_media_consent_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `superseded_photography_media_consent_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `superseded_photography_media_consent_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Performed Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_performed_flag` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_business_glossary_term' = 'Capacity Assessment Result');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_value_regex' = 'has_capacity|lacks_capacity|partial_capacity|not_assessed');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `capacity_assessment_result` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_description` SET TAGS ('pii_business_glossary_term' = 'Compensation Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_description` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_description` SET TAGS ('pii_financial' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_disclosure_flag` SET TAGS ('pii_business_glossary_term' = 'Compensation Disclosure Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_disclosure_flag` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `compensation_disclosure_flag` SET TAGS ('pii_financial' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `distribution_channels` SET TAGS ('pii_business_glossary_term' = 'Distribution Channels');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `document_reference` SET TAGS ('pii_business_glossary_term' = 'Document Reference');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `form_version` SET TAGS ('pii_business_glossary_term' = 'Form Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `identifiable_flag` SET TAGS ('pii_business_glossary_term' = 'Identifiable Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `intended_use` SET TAGS ('pii_business_glossary_term' = 'Intended Use');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_business_glossary_term' = 'Interpreter Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `interpreter_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `irb_approval_number` SET TAGS ('pii_business_glossary_term' = 'Institutional Review Board (IRB) Approval Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `language_code` SET TAGS ('pii_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `language_code` SET TAGS ('pii_value_regex' = '^[a-z]{2}(-[A-Z]{2})?$');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_flag` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `legal_representative_relationship` SET TAGS ('pii_business_glossary_term' = 'Legal Representative Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `media_type` SET TAGS ('pii_business_glossary_term' = 'Media Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `media_type` SET TAGS ('pii_value_regex' = 'photography|video|audio|digital_image|mixed_media');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `right_to_withdraw_statement` SET TAGS ('pii_business_glossary_term' = 'Right to Withdraw Statement');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `scope_description` SET TAGS ('pii_business_glossary_term' = 'Scope Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `signature_date` SET TAGS ('pii_business_glossary_term' = 'Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_business_glossary_term' = 'Signature Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `signature_method` SET TAGS ('pii_value_regex' = 'electronic|wet_signature|verbal|biometric|digital_certificate');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `signature_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'Signature Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System ID');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_business_glossary_term' = 'Witness Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`photography_media_consent` ALTER COLUMN `witness_signature_date` SET TAGS ('pii_business_glossary_term' = 'Witness Signature Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` SET TAGS ('pii_data_type' = 'transactional_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` SET TAGS ('pii_subdomain' = 'privacy_rights');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_request_id` SET TAGS ('pii_business_glossary_term' = 'Consent Amendment Request Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Facility Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `org_unit_id` SET TAGS ('pii_business_glossary_term' = 'Department Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `original_amendment_request_id` SET TAGS ('pii_business_glossary_term' = 'Original Amendment Request Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `original_amendment_request_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Decision Made By User Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Encounter Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_applied_timestamp` SET TAGS ('pii_business_glossary_term' = 'Amendment Applied Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_scope` SET TAGS ('pii_business_glossary_term' = 'Amendment Scope');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_scope` SET TAGS ('pii_value_regex' = 'consent_record|phi_documentation|authorization_details|consent_restrictions|disclosure_log|all');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_type` SET TAGS ('pii_business_glossary_term' = 'Amendment Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `amendment_type` SET TAGS ('pii_value_regex' = 'correction|addition|clarification|deletion|update');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `audit_trail_reference` SET TAGS ('pii_business_glossary_term' = 'Audit Trail Reference');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `compliance_review_date` SET TAGS ('pii_business_glossary_term' = 'Compliance Review Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `decision_date` SET TAGS ('pii_business_glossary_term' = 'Decision Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `decision_rationale` SET TAGS ('pii_business_glossary_term' = 'Decision Rationale');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `decision_rationale` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `decision_timestamp` SET TAGS ('pii_business_glossary_term' = 'Decision Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `denial_reason_code` SET TAGS ('pii_business_glossary_term' = 'Denial Reason Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `denial_reason_description` SET TAGS ('pii_business_glossary_term' = 'Denial Reason Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `effective_date` SET TAGS ('pii_business_glossary_term' = 'Amendment Effective Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `extension_granted_flag` SET TAGS ('pii_business_glossary_term' = 'Extension Granted Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `extension_notification_date` SET TAGS ('pii_business_glossary_term' = 'Extension Notification Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `extension_reason` SET TAGS ('pii_business_glossary_term' = 'Extension Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `last_updated_timestamp` SET TAGS ('pii_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mrn` SET TAGS ('pii_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mrn` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mrn` SET TAGS ('pii_identifier' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mrn` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `mrn` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `notes` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `organization_decision` SET TAGS ('pii_business_glossary_term' = 'Organization Decision');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `organization_decision` SET TAGS ('pii_value_regex' = 'accepted|denied|partially_accepted');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `organization_rebuttal` SET TAGS ('pii_business_glossary_term' = 'Organization Rebuttal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_business_glossary_term' = 'Patient Notification Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_date` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_business_glossary_term' = 'Patient Notification Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_value_regex' = 'mail|email|portal|phone|in_person');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_notification_method` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_statement_of_disagreement` SET TAGS ('pii_business_glossary_term' = 'Patient Statement of Disagreement');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_statement_of_disagreement` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `patient_statement_of_disagreement` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `reason_for_amendment` SET TAGS ('pii_business_glossary_term' = 'Reason for Amendment');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_channel` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Channel');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_date` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_method` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_number` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_status` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `request_timestamp` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `requested_change_description` SET TAGS ('pii_business_glossary_term' = 'Requested Change Description');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System Identifier (ID)');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `supporting_documentation_reference` SET TAGS ('pii_business_glossary_term' = 'Supporting Documentation Reference');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `third_party_notification_date` SET TAGS ('pii_business_glossary_term' = 'Third Party Notification Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `third_party_notification_required_flag` SET TAGS ('pii_business_glossary_term' = 'Third Party Notification Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `third_party_recipients_notified` SET TAGS ('pii_business_glossary_term' = 'Third Party Recipients Notified');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `withdrawal_date` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Withdrawal Date');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`amendment_request` ALTER COLUMN `withdrawal_reason` SET TAGS ('pii_business_glossary_term' = 'Amendment Request Withdrawal Reason');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` SET TAGS ('pii_['model_tier' = 'ECM');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` SET TAGS ('pii_'ecm_member' = 'true]');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_session_id` SET TAGS ('pii_business_glossary_term' = 'Consent Session Identifier');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_session_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_session_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `care_site_id` SET TAGS ('pii_business_glossary_term' = 'Care Site Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `employee_id` SET TAGS ('pii_business_glossary_term' = 'Employee Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `employee_id` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `employee_id` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `mpi_record_id` SET TAGS ('pii_business_glossary_term' = 'Mpi Record Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `resumed_consent_session_id` SET TAGS ('pii_business_glossary_term' = 'Resumed Consent Session Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `resumed_consent_session_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `resumed_consent_session_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `resumed_consent_session_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `visit_id` SET TAGS ('pii_business_glossary_term' = 'Visit Id');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `all_consents_obtained_flag` SET TAGS ('pii_business_glossary_term' = 'All Consents Obtained Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `all_consents_obtained_flag` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `all_consents_obtained_flag` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `audit_trail_reference` SET TAGS ('pii_business_glossary_term' = 'Audit Trail Reference');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_channel` SET TAGS ('pii_business_glossary_term' = 'Consent Collection Channel');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_channel` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_channel` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_method` SET TAGS ('pii_business_glossary_term' = 'Consent Collection Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_method` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_collection_method` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_count` SET TAGS ('pii_business_glossary_term' = 'Consent Count');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_count` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_count` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_document_url` SET TAGS ('pii_business_glossary_term' = 'Consent Document Url');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_document_url` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_document_url` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_document_url` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_form_version` SET TAGS ('pii_business_glossary_term' = 'Consent Form Version');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_form_version` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_form_version` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_language` SET TAGS ('pii_business_glossary_term' = 'Consent Language');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_language` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consent_language` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_business_glossary_term' = 'Consenting Party Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_relationship` SET TAGS ('pii_business_glossary_term' = 'Consenting Party Relationship');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_relationship` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_relationship` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_type` SET TAGS ('pii_business_glossary_term' = 'Consenting Party Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `consenting_party_type` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `created_timestamp` SET TAGS ('pii_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `device_type` SET TAGS ('pii_business_glossary_term' = 'Device Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `electronic_signature_method` SET TAGS ('pii_business_glossary_term' = 'Electronic Signature Method');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_language` SET TAGS ('pii_business_glossary_term' = 'Interpreter Language');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_name` SET TAGS ('pii_business_glossary_term' = 'Interpreter Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_name` SET TAGS ('pii_name' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `interpreter_required_flag` SET TAGS ('pii_business_glossary_term' = 'Interpreter Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('pii_business_glossary_term' = 'Ip Address');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('pii_ip' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `ip_address` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `modified_timestamp` SET TAGS ('pii_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `regulatory_compliance_flag` SET TAGS ('pii_business_glossary_term' = 'Regulatory Compliance Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_duration_minutes` SET TAGS ('pii_business_glossary_term' = 'Session Duration Minutes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_duration_minutes` SET TAGS ('pii_vibe_rate_typing' = 'decimal');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_end_timestamp` SET TAGS ('pii_business_glossary_term' = 'Session End Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_notes` SET TAGS ('pii_business_glossary_term' = 'Session Notes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_notes` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_number` SET TAGS ('pii_business_glossary_term' = 'Session Number');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_start_timestamp` SET TAGS ('pii_business_glossary_term' = 'Session Start Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_status` SET TAGS ('pii_business_glossary_term' = 'Session Status');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_timeout_minutes` SET TAGS ('pii_business_glossary_term' = 'Session Timeout Minutes');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `session_type` SET TAGS ('pii_business_glossary_term' = 'Session Type');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `source_system_code` SET TAGS ('pii_business_glossary_term' = 'Source System Code');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `staff_role` SET TAGS ('pii_business_glossary_term' = 'Staff Role');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `user_agent` SET TAGS ('pii_business_glossary_term' = 'User Agent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `user_agent` SET TAGS ('pii_internal' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `user_agent` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `witness_name` SET TAGS ('pii_business_glossary_term' = 'Witness Name');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `witness_name` SET TAGS ('pii_restricted' = 'true');
...
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `witness_required_flag` SET TAGS ('pii_business_glossary_term' = 'Witness Required Flag');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_session` ALTER COLUMN `witness_signature_timestamp` SET TAGS ('pii_business_glossary_term' = 'Witness Signature Timestamp');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` SET TAGS ('pii_subdomain' = 'specialized_consent');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_record_id` SET TAGS ('pii_business_glossary_term' = 'Consent Record Identifier');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_record_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `superseded_by_consent_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `superseded_by_consent_record_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `supersedes_consent_record_id` SET TAGS ('pii_self_ref_fk' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `supersedes_consent_record_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `supersedes_consent_record_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_category` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_category` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_direction` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_direction` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_number` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_number` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_type` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `document_uri` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `expiration_event` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `scope_description` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `signer_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `signer_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `signer_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_record_status` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `consent_record_status` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `withdrawal_reason` SET TAGS ('pii_confidential' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `witness_name` SET TAGS ('pii_restricted' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `witness_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`consent_record` ALTER COLUMN `witness_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` SET TAGS ('pii_data_type' = 'master_data');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` SET TAGS ('pii_subdomain' = 'compliance_governance');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `workflow_id` SET TAGS ('pii_business_glossary_term' = 'Workflow Identifier');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `consent_policy_id` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `allows_proxy_consent` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `allows_proxy_consent` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `consent_context_type` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `consent_context_type` SET TAGS ('pii_uc_classification' = 'phi');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `workflow_name` SET TAGS ('pii_pii' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `workflow_name` SET TAGS ('pii_uc_classification' = 'pii');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `superseded_by_consent_workflow_id` SET TAGS ('pii_phi' = 'true');
ALTER TABLE `vibe_healthcare_v1`.`consent`.`workflow` ALTER COLUMN `superseded_by_consent_workflow_id` SET TAGS ('pii_uc_classification' = 'phi');
