-- Schema for Domain: network | Business: Health_Insurance | Version: v3_mvm
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`tier` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Facilities are assigned to network tiers (Tier 1 preferred hospital vs Tier 2 standard). Drives differential member cost-sharing, benefit design, steerage strategies, and value-based contracting arran',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`plan_association` (
    ...
    `capitation_arrangement_id` BIGINT COMMENT 'Foreign key linking to contract.capitation_arrangement. Business justification: When plans use capitated payment models, plan associations must reference the specific capitation arrangement governing PMPM rates and risk-sharing. Critical for financial reconciliation, member attri',
    ...
    `provider_contract_id` BIGINT COMMENT 'Foreign key linking to contract.provider_contract. Business justification: Plan associations bind health plans to provider networks and must reference the underlying provider contracts that legally enable network participation. Required for regulatory filings, network adequa',
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to network.tier. Business justification: plan_association.network_tier is a denormalized STRING referencing the tier structure associated with this plan-network pairing. The tier table is the authoritative reference for tier definitions with',
    `vbc_contract_id` BIGINT COMMENT 'Foreign key linking to contract.vbc_contract. Business justification: Value-based care contracts are specialized provider arrangements that plan associations must reference for performance tracking, quality measurement, and member attribution to ACOs or VBC programs. Es',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`adequacy_standard` (
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'add column compliance_regulatory_obligation_id (BIGINT) with FK to compliance.compliance_regulatory_obligation.compliance_regulatory_obligation_id - adequacy standards derive from regulatory requirements',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` (
    ...
    `audit_finding_id` BIGINT COMMENT 'Foreign key linking to compliance.audit_finding. Business justification: Network adequacy assessments that identify gaps (gap_identified_flag=true, gap_severity populated) generate formal audit findings requiring remediation tracking. Assessors document deficiencies as aud',
    ...
    `dispensing_pharmacy_id` BIGINT COMMENT 'Foreign key linking to pharmacy.dispensing_pharmacy. Business justification: CMS Part D and state regulations require pharmacy-specific network adequacy assessments evaluating dispensing pharmacy coverage within service areas. Adequacy assessments must reference specific dispe',
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Adequacy assessments evaluate facility availability (hospital beds, emergency services) within service areas. Required for state/federal network adequacy filings, measuring compliance with facility ac',
    ...
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.practice_location. Business justification: Adequacy assessments measure time/distance access from member populations to specific practice locations. Core regulatory activity for CMS/state network adequacy certification, identifying geographic ',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` (
    ...
    `group_id` BIGINT COMMENT 'Foreign key linking to employer.group. Business justification: Provider Directory Maintenance: assigns an employee who updates directory entries, required for data quality and audit trails.',
    `network_service_area_id` BIGINT COMMENT 'Foreign key linking to network.service_area. Business justification: A provider directory entry represents a provider at a specific practice location. That location falls within one or more service areas. Linking provider_directory to service_area enables geographic fi',
    `par_agreement_id` BIGINT COMMENT 'Foreign key linking to network.par_agreement. Business justification: A provider directory listing is the member-facing publication of a providers network participation, which is formally authorized by a PAR agreement. Linking provider_directory to par_agreement establ',
    `provider_assignment_id` BIGINT COMMENT 'Foreign key linking to network.provider_assignment. Business justification: The provider directory is the member-facing published view of a providers network participation. The provider_assignment is the internal operational record of that participation. A directory entry sh',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`par_agreement` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: PAR agreements can be facility-specific (hospital participation contracts) distinct from individual provider agreements. Required for facility-level rate negotiation, contract management, credentialin',
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to network.tier. Business justification: par_agreement.provider_tier_assignment is a denormalized STRING storing the tier designation for the participating provider. The tier table is the authoritative reference. A PAR agreement specifies on',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` (
    ...
    `plan_service_area_id` BIGINT COMMENT 'SSOT reference to plan.plan_service_area (single source of truth)',
    `provider_network_id` BIGINT COMMENT 'Identifier of the provider network to which this service area belongs.',
    `regulatory_obligation_id` BIGINT COMMENT 'Identifier of the regulatory filing (e.g., SERFF filing, CMS HPMS submission) that established or modified this service area.',
...
CREATE OR REPLACE TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` (
    ...
    `capitation_arrangement_id` BIGINT COMMENT 'Foreign key linking to contract.capitation_arrangement. Business justification: When providers are assigned under capitated payment models, assignments must reference the specific capitation arrangement for PMPM payment calculation and member attribution. Operational necessity fo',
    `facility_id` BIGINT COMMENT 'Foreign key linking to provider.facility. Business justification: Assignments link providers to facilities where they have admitting/surgical privileges. Required for referral management, care coordination, hospital-based service authorization, and ensuring continui',
    `fee_schedule_id` BIGINT COMMENT 'Foreign key linking to contract.fee_schedule. Business justification: Provider assignments operationalize network participation and must reference the applicable fee schedule for claims adjudication and payment calculation. Core operational link between network particip',
    `network_service_area_id` BIGINT COMMENT 'Foreign key linking to network.service_area. Business justification: provider_assignment.geographic_service_area is a denormalized STRING storing the geographic service area for the providers assignment. The service_area table in the same domain is the authoritative r',
    `par_agreement_id` BIGINT COMMENT 'Foreign key linking to network.par_agreement. Business justification: A provider assignment (the internal record of a providers participation in a network) is authorized by a PAR agreement. The PAR agreement is the contractual basis for the assignment. One PAR agreemen',
    `practice_location_id` BIGINT COMMENT 'Foreign key linking to provider.practice_location. Business justification: Provider assignments specify which practice location(s) serve network members. Essential for accurate provider directory data, member PCP selection, appointment scheduling, and ensuring directory refl',
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to network.tier. Business justification: provider_assignment.tier_assignment is a denormalized STRING storing the tier designation. The tier table in the same domain is the authoritative reference for tier definitions. A provider assignment ',
    `vbc_contract_id` BIGINT COMMENT 'Foreign key linking to contract.vbc_contract. Business justification: Provider assignments under value-based care models must reference the VBC contract for performance attribution, quality measurement, and shared savings/risk calculation. Core VBC operations requiremen',
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ADD CONSTRAINT `fk_network_plan_association_tier_id` FOREIGN KEY (`tier_id`) REFERENCES `vibe_health_insurance_v1`.`network`.`tier`(`tier_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ADD CONSTRAINT `fk_network_provider_directory_network_service_area_id` FOREIGN KEY (`network_service_area_id`) REFERENCES `vibe_health_insurance_v1`.`network`.`network_service_area`(`network_service_area_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ADD CONSTRAINT `fk_network_provider_assignment_network_service_area_id` FOREIGN KEY (`network_service_area_id`) REFERENCES `vibe_health_insurance_v1`.`network`.`network_service_area`(`network_service_area_id`);
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_network` SET TAGS ('dbx_subdomain' = 'network_configuration');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_network` ALTER COLUMN `star_rating` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`tier` SET TAGS ('dbx_subdomain' = 'network_configuration');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`tier` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` SET TAGS ('dbx_subdomain' = 'network_configuration');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `capitation_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Capitation Arrangement Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Identifier (ID)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `vbc_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Vbc Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`plan_association` ALTER COLUMN `star_rating_impact_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_standard` SET TAGS ('dbx_subdomain' = 'compliance_assessment');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_standard` ALTER COLUMN `compliance_reporting_frequency` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` SET TAGS ('dbx_subdomain' = 'compliance_assessment');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `dispensing_pharmacy_id` SET TAGS ('dbx_business_glossary_term' = 'Dispensing Pharmacy Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area ID');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Location Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `accepting_new_patients_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`adequacy_assessment` ALTER COLUMN `wait_time_days_routine` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` SET TAGS ('dbx_subdomain' = 'provider_participation');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Maintaining Employee Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `group_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `group_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `accepting_new_patients_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_directory` ALTER COLUMN `county` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`par_agreement` ALTER COLUMN `provider_directory_listing_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` SET TAGS ('dbx_subdomain' = 'network_configuration');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area ID');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `provider_network_id` SET TAGS ('dbx_business_glossary_term' = 'Network ID');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing ID');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `cms_service_area_code` SET TAGS ('dbx_business_glossary_term' = 'Centers for Medicare and Medicaid Services (CMS) Service Area ID');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `cms_service_area_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{1,20}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `county_fips_code` SET TAGS ('dbx_business_glossary_term' = 'County Federal Information Processing Standards (FIPS) Code');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `county_fips_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `county_fips_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `county_name` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `msa_code` SET TAGS ('dbx_business_glossary_term' = 'Metropolitan Statistical Area (MSA) Code');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `msa_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `msa_name` SET TAGS ('dbx_business_glossary_term' = 'Metropolitan Statistical Area (MSA) Name');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `network_adequacy_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|under_review|waiver_granted|not_applicable');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `network_service_area_status` SET TAGS ('dbx_business_glossary_term' = 'Service Area Status');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `network_service_area_status` SET TAGS ('dbx_value_regex' = 'active|inactive|pending_approval|suspended|terminated');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Service Area Notes');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `qhp_approved_flag` SET TAGS ('dbx_business_glossary_term' = 'Qualified Health Plan (QHP) Approved Flag');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `regulatory_approval_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{1,30}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `service_area_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,15}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `service_area_type` SET TAGS ('dbx_value_regex' = 'county_based|zip_based|msa_based|state_based|custom_boundary|multi_state');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `state_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `updated_by` SET TAGS ('dbx_business_glossary_term' = 'Updated By User');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `urban_rural_designation` SET TAGS ('dbx_value_regex' = 'urban|suburban|rural|frontier|mixed');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `zip_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}$');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`network_service_area` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By User');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `capitation_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Capitation Arrangement Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `fee_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Fee Schedule Id (Foreign Key)');
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `network_service_area_id` SET TAGS ('dbx_business_glossary_term' = 'Service Area Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `practice_location_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Location Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `vbc_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Vbc Contract Id (Foreign Key)');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `accepting_new_patients_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `continuity_of_care_end_date` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `directory_listing_flag` SET TAGS ('dbx_pii_national_id' = 'true');
...
ALTER TABLE `vibe_health_insurance_v1`.`network`.`provider_assignment` ALTER COLUMN `panel_capacity` SET TAGS ('dbx_pii_address' = 'true');
