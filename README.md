...
Forty production-ready industry data models, each shipped in two flavours (ECM + MVM) — auto-generated by the Vibe Data Modeling AI agent that turns a "user vibes" into a Unity-Catalog-ready model with schemas, foreign keys, sample data, metric views, ontology tags, and DBML diagrams.
...
Every model in this repository was generated by the **Vibe Modeling Agent**, vendored here under [`model-agent/`](./model-agent/). Point it at a plain-English description of a business and it produces a governed, Unity-Catalog-ready model; the 80 models above are its output.
...
- **Some Metric Views will faik.** During generation, the agent confuses some column names in Metric Views and you might get installation errors, fixing these errors should be very simple by using the correct column names.
