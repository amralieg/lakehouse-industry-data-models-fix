...
-- Generated on: 2026-06-22 18:57:23
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` (
    ...
    `service_class_id` BIGINT COMMENT 'Foreign key linking this duplicate entity to the single source of truth in service.service_class',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`condition_assessment` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the manager or authority who approved the assessment and recommended actions.',
    ...
    `condition_employee_id` BIGINT COMMENT 'Reference to the employee or contractor who performed the condition assessment.',
    `condition_registry_id` BIGINT COMMENT 'Reference to the infrastructure asset being assessed (pipe, pump, valve, treatment equipment, vehicle, etc.).',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`work_order` (
    ...
    `work_registry_id` BIGINT COMMENT 'Reference to the physical asset (pipe, pump, valve, treatment equipment, vehicle) that is the subject of this work order.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`job_plan` (
    ...
    `job_work_center_org_unit_id` BIGINT COMMENT 'Reference to the maintenance work center or crew responsible for executing this job plan.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`failure_record` (
    ...
    `failure_registry_id` BIGINT COMMENT 'Reference to the physical asset that experienced the failure (equipment, pipe, pump, valve, etc.).',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` (
    ...
    `primary_asset_registry_id` BIGINT COMMENT 'Reference to the parent physical asset (pump, motor, valve, pipe segment, vehicle, etc.) to which this operational meter or counter is attached. Links to the asset master record in the asset domain.',
    `metering_meter_id` BIGINT COMMENT 'Foreign key to the canonical metering_meter record in metering domain',
    ...
    `asset_metering_meter_id` BIGINT COMMENT 'The native meter identifier from IBM Maximo Asset Management CMMS (METERNAME or internal Maximo meter key). Used for system-of-record traceability and bidirectional synchronization between the Silver Layer and Maximo.',
    ...
    `calibration_interval_days` DECIMAL(18,2) COMMENT 'The required frequency of calibration expressed in calendar days. Used to compute next_calibration_date from last_calibration_date and to schedule calibration work orders in IBM Maximo.',
    `calibration_status` DECIMAL(18,2) COMMENT 'Current calibration status of the meter instrument. CALIBRATED: within valid calibration period. DUE: calibration due within the next calibration_interval_days. OVERDUE: calibration date has passed without recalibration. IN_PROGRESS: currently being calibrated. NOT_REQUIRED: meter type does not require formal calibration (e.g., simple cycle counters).',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`warranty` (
    ...
    `warranty_registry_id` BIGINT COMMENT 'Reference to the physical infrastructure asset (e.g., pump, valve, pipe, electrical panel, vehicle, WTP/WWTP equipment) covered by this warranty record.',
    ...
    `claimed_amount_to_date` DATE COMMENT 'Cumulative monetary value of all warranty claims submitted against this warranty record to date, expressed in USD. Used to track remaining claim capacity against the maximum claim value.',
    ...
    `duration_months` DECIMAL(18,2) COMMENT 'Total length of the warranty coverage period expressed in months. Derived from the vendor contract terms and used for planning and renewal tracking.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`depreciation_schedule` (
    ...
    `depreciation_registry_id` BIGINT COMMENT 'Reference to the capital asset for which this depreciation schedule is maintained. Links to the enterprise asset registry.',
    ...
    `cost_center_code` DECIMAL(18,2) COMMENT 'SAP CO cost center to which depreciation charges are posted, enabling OPEX/CAPEX allocation by operational unit (e.g., WTP operations, distribution, wastewater).',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`disposal` (
    ...
    `disposal_replacement_asset_registry_id` BIGINT COMMENT 'Reference to the new asset that replaced the disposed asset, if disposal reason was replacement. Links retired asset to its successor for lifecycle tracking.',
    ...
    `primary_disposal_registry_id` BIGINT COMMENT 'Reference to the asset being disposed from the asset registry. Links to the physical infrastructure asset (pipe, pump, valve, equipment, vehicle) that is being retired or decommissioned.',
    ...
    `cost_center` DECIMAL(18,2) COMMENT 'The cost center responsible for the asset at the time of disposal. Used for departmental cost allocation and budget tracking.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`inspection_event` (
    ...
    `inspection_registry_id` BIGINT COMMENT 'Reference to the asset or infrastructure component that was inspected.',
    ...
    `duration_minutes` DECIMAL(18,2) COMMENT 'Total time spent performing the inspection, measured in minutes.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`document` (
    ...
    `registry_id` BIGINT COMMENT 'Reference to the physical asset this document is associated with. Links to the asset registry.',
    `document_registry_id` BIGINT COMMENT 'Reference to the physical asset this document is associated with. Links to the asset registry for equipment, pipes, pumps, valves, and other infrastructure.',
    ...
    `primary_superseded_by_document_id` BIGINT COMMENT 'Reference to the newer document that replaces this document. Maintains document version lineage and ensures users access the current revision.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` (
    ...
    `vendor_id` BIGINT COMMENT 'Default vendor for this asset class procurement',
    `material_master_id` BIGINT COMMENT 'Reference to substitute material if primary is unavailable',
    `procurement_category_id` BIGINT COMMENT 'Foreign key linking to the procurement category used for sourcing',
    `procurement_contract_id` BIGINT COMMENT 'Active procurement contract for this asset class mapping.',
    `procurement_material_master_id` BIGINT COMMENT 'FK to material master',
    `document_id` BIGINT COMMENT 'FK to specification document',
    `employee_id` BIGINT COMMENT 'Employee identifier for the procurement professional who last updated this mapping',
    `abc_classification` STRING COMMENT 'ABC classification for inventory management',
    `alternate_vendor_count` STRING COMMENT 'Number of alternate approved vendors',
    `annual_spend_budget` DECIMAL(18,2) COMMENT 'Budgeted annual spend for this asset class procurement.',
    ...
    `bid_threshold_amount` DECIMAL(18,2) COMMENT 'Dollar threshold requiring competitive bidding',
    `budget_code` STRING COMMENT 'Budget code used for procurement of this asset class.',
    `buy_america_compliant` BOOLEAN COMMENT 'Whether item is Buy America compliant',
    `commodity_code` STRING COMMENT 'Standard commodity code for procurement',
    `contract_required_flag` BOOLEAN COMMENT 'Indicates if contract is required for procurement',
    `cooperative_contract_available` BOOLEAN COMMENT 'Whether cooperative purchasing contract is available',
    `cooperative_contract_name` STRING COMMENT 'Name of cooperative purchasing contract',
    `created_date` TIMESTAMP COMMENT 'Timestamp when this mapping was established in the system',
    `critical_spare_flag` BOOLEAN COMMENT 'Whether this is a critical spare part',
    `criticality_tier` STRING COMMENT 'Criticality tier for procurement prioritization (e.g., Critical, High, Medium, Low).',
    `economic_order_quantity` STRING COMMENT 'Economic order quantity for procurement',
    `effective_date` DATE COMMENT 'Date when this procurement mapping became effective.',
    `environmental_compliance_required` BOOLEAN COMMENT 'Whether environmental compliance is required',
    `expiration_date` DATE COMMENT 'Date when this procurement mapping expires and must be reviewed.',
    `gsa_schedule_available` BOOLEAN COMMENT 'Whether GSA schedule is available',
    `gsa_schedule_number` STRING COMMENT 'GSA schedule number if available',
    `hazmat_handling_required_flag` BOOLEAN COMMENT 'Indicates if hazardous material handling is required',
    `installation_required_flag` BOOLEAN COMMENT 'Indicates if professional installation is required',
    `last_price_update_date` DATE COMMENT 'Date of last price update',
    ...
    `last_unit_cost` DECIMAL(18,2) COMMENT 'Unit cost from the most recent procurement transaction.',
    ...
    `local_preference_applicable` BOOLEAN COMMENT 'Whether local vendor preference applies',
    ...
    `minimum_order_quantity` STRING COMMENT 'Minimum order quantity for this asset class',
    `minority_business_preference` BOOLEAN COMMENT 'Whether minority business preference applies',
    `naics_code` STRING COMMENT 'NAICS industry code for suppliers',
    `nigp_code` STRING COMMENT 'NIGP commodity code',
    `notes` STRING COMMENT 'Additional notes about the procurement mapping',
    `obsolescence_risk_flag` BOOLEAN COMMENT 'Indicates if item has obsolescence risk',
    `preferred_sourcing_strategy` DECIMAL(18,2) COMMENT 'Recommended procurement approach for this specific asset class when sourced through this procurement category. Drives purchasing workflow and approval routing.',
    `price_volatility_flag` BOOLEAN COMMENT 'Whether item has high price volatility',
    `reorder_point` STRING COMMENT 'Inventory level that triggers reorder',
    `reorder_point_quantity` STRING COMMENT 'Inventory level at which a reorder should be triggered.',
    `safety_stock_quantity` STRING COMMENT 'Safety stock quantity to maintain',
    `small_business_set_aside` BOOLEAN COMMENT 'Whether procurement is set aside for small business',
    `sole_source_allowed_flag` BOOLEAN COMMENT 'Indicates if sole source procurement is allowed',
    `sole_source_flag` BOOLEAN COMMENT 'Indicates whether this asset class has only a single approved vendor.',
    `specification_document_url` STRING COMMENT 'URL or path to the procurement specification document.',
    ...
    `typical_warranty_months` STRING COMMENT 'Typical warranty period in months',
    `unspsc_code` STRING COMMENT 'UNSPSC commodity code',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` (
    ...
    `asset_quality_sampling_point_id` BIGINT COMMENT 'Foreign key linking to the approved water quality sampling location where samples are collected',
    `quality_sampling_point_id` BIGINT COMMENT 'Foreign key to the canonical quality_sampling_point record in quality domain',
    ...
    `configuration_status` DECIMAL(18,2) COMMENT 'Current operational status of this asset-sampling point configuration. Active = in use; suspended = temporarily inactive; under_review = pending validation; decommissioned = permanently retired.',
...
CREATE OR REPLACE TABLE `vibe_water_utilities_v1`.`asset`.`inspection_checklist` (
    ...
    `estimated_duration_minutes` DECIMAL(18,2) COMMENT 'Estimated time required to complete the checklist, in minutes.',
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ADD CONSTRAINT `fk_asset_procurement_mapping_document_id` FOREIGN KEY (`document_id`) REFERENCES `vibe_water_utilities_v1`.`asset`.`document`(`document_id`);
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`registry` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_ssot_group' = 'asset.asset_class');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_ssot' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_data_governance' = 'system_of_record');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_ssot_pair' = 'asset.asset_class');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_service_service_class' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_ssot_source' = 'service.service_class');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_deprecated' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` SET TAGS ('dbx_ssot_reference' = 'service.service_class');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` ALTER COLUMN `service_class_id` SET TAGS ('dbx_business_glossary_term' = 'FK to SSOT owner service.service_class');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` ALTER COLUMN `service_class_id` SET TAGS ('dbx_ssot_fk' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_class` ALTER COLUMN `service_class_id` SET TAGS ('dbx_ssot_mvm_resolved' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`location` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`location` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Employee Id (Foreign Key)');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`location` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`location` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`condition_assessment` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`criticality_rating` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`work_order` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_reference' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_master' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_references' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_role' = 'alias');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_canonical' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_group' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_data_governance' = 'reference');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_pair' = 'asset.asset_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_metering_metering_meter' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_ref' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_master_ref' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_ssot_source' = 'metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` SET TAGS ('dbx_deprecated' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` ALTER COLUMN `metering_meter_id` SET TAGS ('dbx_business_glossary_term' = 'Reference to single source of truth in metering.metering_meter');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` ALTER COLUMN `metering_meter_id` SET TAGS ('dbx_ssot_reference' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` ALTER COLUMN `asset_metering_meter_id` SET TAGS ('dbx_business_glossary_term' = 'IBM Maximo Meter ID');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` ALTER COLUMN `asset_metering_meter_id` SET TAGS ('dbx_ssot_link' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_meter` ALTER COLUMN `asset_metering_meter_id` SET TAGS ('dbx_ssot_mvm_resolved' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`warranty` SET TAGS ('dbx_subdomain' = 'financial_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`depreciation_schedule` SET TAGS ('dbx_subdomain' = 'financial_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`acquisition` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`disposal` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`document` SET TAGS ('dbx_subdomain' = 'physical_infrastructure');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` SET TAGS ('dbx_subdomain' = 'financial_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` SET TAGS ('dbx_thin_product_expanded' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_role' = 'origin_material_master');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `annual_spend_budget` SET TAGS ('dbx_business_glossary_term' = 'Annual Spend Budget');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `budget_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Code');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `critical_spare_flag` SET TAGS ('dbx_business_glossary_term' = 'Critical Spare');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `last_unit_cost` SET TAGS ('dbx_business_glossary_term' = 'Last Unit Cost');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`procurement_mapping` ALTER COLUMN `minimum_order_quantity` SET TAGS ('dbx_business_glossary_term' = 'Minimum Order Qty');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_subdomain' = 'compliance_integration');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_reference' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_master' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_references' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_role' = 'alias');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_group' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot' = 'false');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_data_governance' = 'reference');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_pair' = 'asset.asset_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_quality_quality_sampling_point' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_ref' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_master_ref' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_ssot_source' = 'quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` SET TAGS ('dbx_deprecated' = 'true');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `asset_quality_sampling_point_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Sampling Point - Sampling Point Id');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `asset_quality_sampling_point_id` SET TAGS ('dbx_ssot_link' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `asset_quality_sampling_point_id` SET TAGS ('dbx_ssot_mvm_resolved' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `quality_sampling_point_id` SET TAGS ('dbx_business_glossary_term' = 'Reference to single source of truth in quality.quality_sampling_point');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `quality_sampling_point_id` SET TAGS ('dbx_ssot_reference' = 'true');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `quality_sampling_point_id` SET TAGS ('dbx_renamed_from' = 'quality_sampling_point_id');
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`asset_sampling_point` ALTER COLUMN `quality_sampling_point_id` SET TAGS ('dbx_business_role' = 'primary_sampling_point');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`grant_funding` SET TAGS ('dbx_subdomain' = 'financial_tracking');
...
ALTER TABLE `vibe_water_utilities_v1`.`asset`.`compliance_requirement` SET TAGS ('dbx_subdomain' = 'compliance_integration');
