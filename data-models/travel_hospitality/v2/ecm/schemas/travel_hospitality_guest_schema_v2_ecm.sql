...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`profile` (
    ...
    `nationality_country_code` BIGINT COMMENT 'The guests nationality expressed as an ISO 3166-1 alpha-3 three-letter country code (e.g., USA, GBR, DEU). Required for regulatory reporting, visa compliance, and international guest analytics.. Valid values are `^[A-Z]{3}$`',
    ...
    `passport_issuing_country_code` BIGINT COMMENT 'The ISO 3166-1 alpha-3 country code of the authority that issued the guests passport. Required for immigration compliance and international guest regulatory reporting.. Valid values are `^[A-Z]{3}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`contact_info` (
    ...
    `phone_country_code` BIGINT COMMENT 'International dialing country code prefix for the guest phone number (e.g., +1 for USA, +44 for UK). Stored separately to support international call routing and country-level analytics. Conforms to ITU-T E.164 country code standards.. Valid values are `^+[1-9]d{0,3}$`',
    `phone_number` BIGINT COMMENT 'Guest phone number in E.164 international format. Used for reservation confirmations, pre-arrival calls, service recovery, and emergency contact. Sourced from OPERA PMS and Salesforce CRM. Subject to GDPR and CCPA data protection requirements.. Valid values are `^+?[1-9]d{1,14}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`preference` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the staff member who captured or last updated this preference record. Supports accountability tracking and training analytics for preference capture quality.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_account` (
    ...
    `billing_address_country_code` BIGINT COMMENT 'ISO 3166-1 alpha-3 three-letter country code for the corporate clients billing address. Drives tax treatment, currency defaults, and regulatory compliance requirements (GDPR for EU entities).. Valid values are `^[A-Z]{3}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the internal hotel employee or sales coordinator assigned to manage this group block from the property side.',
    ...
    `group_rate_code` DECIMAL(18,2) COMMENT 'Rate plan code applied to individual reservations made under this group block. Links to the negotiated group rate in the revenue rate plan. Sourced from OPERA PMS and distributed via SynXis CRS. Denormalized natural-key reference; non-authoritative copy resolved per v2 rebuild (authoritative source is the linked parent via FK).. Valid values are `^[A-Z0-9_]{2,20}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`vip_designation` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the workforce employee record for the dedicated property host, butler, or guest relations manager assigned to personally manage this VIP guests experience. Drives pre-arrival communication and on-property service coordination.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`stay_history` (
    ...
    `loyalty_member_number` BIGINT COMMENT 'Loyalty program membership number associated with the guest for this stay. Used to post loyalty points, track qualifying nights, and calculate tier status. Sourced from Salesforce CRM loyalty management module. Subject to GDPR and CCPA privacy regulations.',
    ...
    `room_number` BIGINT COMMENT 'Physical room number assigned to the guest for the duration of the stay as recorded in Oracle OPERA PMS. Supports housekeeping scheduling, maintenance tracking, and room-level revenue attribution.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`note` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the specific staff member assigned to action or follow up on this note. Null if assigned to a department rather than an individual.',
    ...
    `extracted_topics` STRING COMMENT 'NLP-extracted topics from note text',
    ...
    `loyalty_member_number` BIGINT COMMENT 'The guests loyalty program membership number at the time the note was created. Captured denormalized to support loyalty-aware pre-arrival briefing and cross-property service continuity without requiring a join to the loyalty profile.',
    ...
    `priority_level` BIGINT COMMENT 'Graduated priority ranking of the note for staff triage and pre-arrival briefing. Critical notes require immediate escalation; high notes require same-day action; medium notes are addressed during the stay; low notes are informational.. Valid values are `critical|high|medium|low`',
    ...
    `sentiment_score` DECIMAL(18,2) COMMENT 'Sentiment score derived from note text',
    ...
    `topic_confidence_score` DECIMAL(18,2) COMMENT 'Confidence of topic extraction',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`communication_consent` (
    ...
    `guest_country_code` BIGINT COMMENT 'Three-letter ISO 3166-1 alpha-3 country code representing the guest primary country of residence at the time consent was captured. Used to determine applicable privacy regulations.. Valid values are `^[A-Z]{3}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`identity_document` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the staff member who performed the identity document verification. Links to workforce/staff master data. Null if verification was automated.',
    ...
    `issuing_country_code` BIGINT COMMENT 'Three-letter ISO 3166-1 alpha-3 country code of the country or jurisdiction that issued the identity document.. Valid values are `^[A-Z]{3}$`',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'The system user identifier or staff ID of the person who initiated or approved the merge operation. May be system account for automated merges.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`lifetime_value` (
    ...
    `loyalty_member_number` BIGINT COMMENT 'The guests loyalty program membership number. Links LTV to loyalty tier and rewards fulfillment. Personally identifiable.',
    ...
    `model_version` STRING COMMENT 'Predictive model version',
    ...
    `next_best_offer` STRING COMMENT 'Next best offer recommendation for the guest',
    ...
    `score_generated_timestamp` TIMESTAMP COMMENT 'Timestamp when predictive scores generated',
    ...
    `upsell_propensity_score` DECIMAL(18,2) COMMENT '',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`relationship` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Identifier of the staff member who created this relationship record. Null if created through an automated process or API import.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` (
    ...
    `procurement_employee_id` BIGINT COMMENT 'Reference to the data privacy officer or staff member assigned to process this request.',
    ...
    `guest_country_code` BIGINT COMMENT 'Three-letter ISO country code of the guests primary residence at time of request submission.',
    ...
    `request_reference_number` BIGINT COMMENT 'Externally-visible unique reference number provided to the guest for tracking their privacy request.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` (
    ...
    `negotiated_rate_code` DECIMAL(18,2) COMMENT 'Property-specific rate code assigned in the Central Reservation System for this corporate account at this property. Rate codes vary by property based on local competitive positioning.',
    `payment_terms` DECIMAL(18,2) COMMENT 'Property-specific payment terms for direct billing invoices. Terms may vary by property based on local AR policies and account credit standing.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`household` (
    ...
    `primary_merged_into_household_id` BIGINT COMMENT 'Reference to the target household ID if this household record has been merged into another household.',
...
CREATE OR REPLACE TABLE `vibe_travel_hospitality_v1`.`guest`.`predictive_score` (
    ...
    `churn_risk` DECIMAL(18,2) COMMENT '',
    `upsell_propensity` DECIMAL(18,2) COMMENT '',
    `next_best_offer` STRING COMMENT '',
    `score_date` DATE COMMENT '',
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ADD CONSTRAINT `fk_guest_household_primary_merged_into_household_id` FOREIGN KEY (`primary_merged_into_household_id`) REFERENCES `vibe_travel_hospitality_v1`.`guest`.`household`(`household_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`predictive_score` ADD CONSTRAINT `fk_guest_predictive_score_profile_id` FOREIGN KEY (`profile_id`) REFERENCES `vibe_travel_hospitality_v1`.`guest`.`profile`(`profile_id`);
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`preference` SET TAGS ('dbx_subdomain' = 'guest_preferences');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`preference` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Captured By Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`preference` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`preference` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_account` SET TAGS ('dbx_subdomain' = 'corporate_accounts');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_account` ALTER COLUMN `market_segment_code` SET TAGS ('dbx_normalization' = 'denormalized_natural_key_redundant');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` SET TAGS ('dbx_subdomain' = 'corporate_accounts');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` SET TAGS ('dbx_ssot_source' = 'reservation.reservation_group_block');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` SET TAGS ('dbx_ssot_family' = 'group_block');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` SET TAGS ('dbx_ssot_role' = 'reference');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` SET TAGS ('dbx_ssot_owner' = 'reservation.reservation_group_block');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Group Coordinator Employee ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_group_block` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`vip_designation` SET TAGS ('dbx_subdomain' = 'guest_preferences');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`vip_designation` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Property Host ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`vip_designation` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`vip_designation` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`stay_history` SET TAGS ('dbx_subdomain' = 'guest_preferences');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`stay_history` SET TAGS ('dbx_revpar_measure_corrected' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`stay_history` ALTER COLUMN `loyalty_member_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`stay_history` ALTER COLUMN `room_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` SET TAGS ('dbx_subdomain' = 'guest_preferences');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `loyalty_member_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `opera_trace_reference` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `opera_trace_reference` SET TAGS ('dbx_PII' = 'special_category');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`note` ALTER COLUMN `priority_level` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`identity_document` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Verified By Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`identity_document` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`identity_document` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`segment` SET TAGS ('dbx_subdomain' = 'segment_enrollment');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Operator ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `operator_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `operator_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`profile_merge_history` ALTER COLUMN `operator_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`lifetime_value` SET TAGS ('dbx_subdomain' = 'guest_preferences');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`lifetime_value` ALTER COLUMN `loyalty_member_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`relationship` SET TAGS ('dbx_subdomain' = 'identity_management');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`relationship` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Staff ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`relationship` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`relationship` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Handler ID');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `procurement_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `assigned_handler_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `guest_country_code` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`privacy_request` ALTER COLUMN `request_reference_number` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` SET TAGS ('dbx_subdomain' = 'corporate_accounts');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `negotiated_rate_code` SET TAGS ('dbx_typed' = 'numeric_correction');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_name` SET TAGS ('dbx_pii_class' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`corporate_property_contract` ALTER COLUMN `property_account_manager_name` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`group_function_space_booking` SET TAGS ('dbx_subdomain' = 'corporate_accounts');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`guest_enrollment` SET TAGS ('dbx_subdomain' = 'segment_enrollment');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`segment_membership` SET TAGS ('dbx_subdomain' = 'segment_enrollment');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` SET TAGS ('dbx_subdomain' = 'identity_management');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_merged_into_household_id` SET TAGS ('dbx_business_glossary_term' = 'Merged Into Household Id');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_1` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_class' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_1` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_2` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_class' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `address_line_2` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `household_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `household_name` SET TAGS ('dbx_pii_type' = 'person_name');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `household_name` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `postal_code` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_type' = 'address');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `postal_code` SET TAGS ('dbx_mask_non_prod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_email` SET TAGS ('dbx_pii_type' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_email` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_email` SET TAGS ('dbx_pii_class' = 'email');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_email` SET TAGS ('dbx_mask_nonprod' = 'true');
...
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_phone` SET TAGS ('dbx_pii_type' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_phone` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_phone` SET TAGS ('dbx_pii_class' = 'phone');
ALTER TABLE `vibe_travel_hospitality_v1`.`guest`.`household` ALTER COLUMN `primary_phone` SET TAGS ('dbx_mask_nonprod' = 'true');
