...
-- ========= TABLES =========
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`compliance_program` (
...
) COMMENT '[SSOT Owner] Authoritative source for compliance and regulatory programs. Defines program structure, audit schedules, certification requirements, and governing bodies for all organizational compliance obligations.';

CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`obligation` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` (
    ...
    `primary_previous_audit_audit_schedule_id` BIGINT COMMENT 'Reference to the most recent prior audit of the same type at the same location, enabling trend analysis and follow-up tracking.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`audit_event` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`audit_finding` (
    ...
    `primary_previous_finding_audit_finding_id` BIGINT COMMENT 'Reference to the previous audit finding record if this is a recurrence. Null if this is the first occurrence.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`corrective_action` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`osha_incident` (
    ...
    `parent_osha_incident_id` BIGINT COMMENT 'Self-referencing FK on osha_incident (related_osha_incident_id)',
    ...
    `primary_previous_incident_osha_incident_id` BIGINT COMMENT 'Reference to a previous related incident if this is a recurrence or follow-up event.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`pci_control` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` (
    ...
    `system_name` STRING COMMENT 'Name of the operational system or platform being assessed (e.g., the e-commerce platform, SAP S/4HANA Retail, Workday HCM, the customer master data system).',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`license_permit` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`certification` (
    ...
    `tertiary_certification_modified_by_user_associate_id` BIGINT COMMENT 'Foreign key reference to the user or system account that most recently modified this certification record.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`environmental_event` (
    ...
    `parent_environmental_event_id` BIGINT COMMENT 'Self-referencing FK on environmental_event (related_environmental_event_id)',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`training_program` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`training_completion` (
    ...
    `training_associate_id` BIGINT COMMENT 'Identifier of the system user or administrator who last modified this training completion record. Audit trail field for accountability.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`violation_notice` (
    ...
    `parent_violation_notice_id` BIGINT COMMENT 'Self-referencing FK on violation_notice (related_violation_notice_id)',
    `primary_previous_violation_notice_id` BIGINT COMMENT 'Reference to the previous violation notice if this is a recurrence. Links to the original violation for trend analysis.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`policy` (
    ...
    `primary_policy_associate_id` BIGINT COMMENT 'Identifier of the employee who owns and is accountable for the policy. Typically a director or VP-level role.',
    ...
    `tertiary_policy_modified_by_user_associate_id` BIGINT COMMENT 'Identifier of the user who last modified this policy record. Links to employee or system user master.',
    ...
    `scope` STRING COMMENT 'Detailed description of what business processes, systems, or activities this policy governs. Defines boundaries of applicability.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`risk_register` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` (
    ...
    `facility_program_compliance_program_id` BIGINT COMMENT 'Foreign key linking to the compliance program that governs this facility certification',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`requirement` (
    ...
    `primary_superseded_by_requirement_id` BIGINT COMMENT 'Reference to the requirement_id of the newer requirement that replaces or supersedes this requirement, if applicable.',
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` (
...
CREATE OR REPLACE TABLE `vibe_retail_v1`.`compliance`.`consent` (
    ...
    `profile_id` BIGINT COMMENT 'Foreign key to the customer profile that granted or withdrew consent.',
    `collection_channel` STRING COMMENT 'Channel through which consent was collected: web, mobile_app, in_store, call_center, paper_form.',
    `consent_status` STRING COMMENT 'Current state of this consent record: granted, withdrawn, expired, pending_verification.. Valid values are `granted|withdrawn|pending|expired|revoked`',
    `consent_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the consent decision was recorded.',
    `consent_type` STRING COMMENT 'Category of consent granted, e.g. marketing_email, marketing_sms, data_processing, third_party_sharing, profiling, analytics.',
    `data_subject_rights_notice` BOOLEAN COMMENT 'Whether the data subject was informed of their rights (access, rectification, erasure, portability) at the time of consent.',
    `double_opt_in_confirmed` BOOLEAN COMMENT 'Whether a double opt-in confirmation (e.g. email link click) was completed.',
    `double_opt_in_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the double opt-in confirmation was completed.',
    `expiration_date` DECIMAL(18,2) COMMENT 'Date on which this consent automatically expires if not renewed, per regulatory or policy requirements.',
    `granularity` STRING COMMENT 'Level of granularity for this consent: broad, purpose_specific, channel_specific, vendor_specific.. Valid values are `global|channel_specific|purpose_specific|brand_specific`',
    `ip_address` STRING COMMENT 'IP address of the device used when consent was given or withdrawn; used for audit trail and fraud detection.',
    `jurisdiction` STRING COMMENT 'Regulatory jurisdiction applicable to this consent record, e.g. EU, California, Brazil, UK.',
    `language` STRING COMMENT 'ISO 639-1 language code of the consent notice presented to the customer.',
    `legal_basis` STRING COMMENT 'Legal basis for processing under GDPR: consent, legitimate_interest, contractual_necessity, legal_obligation, vital_interest, public_task.. Valid values are `consent|contract|legal_obligation|vital_interest|public_task|legitimate_interest`',
    `method` STRING COMMENT 'Method used to capture consent: checkbox, banner_click, verbal, written_signature, double_opt_in.. Valid values are `explicit_opt_in|implicit_opt_in|pre_checked_box|verbal|written_signature`',
    `minor_consent_flag` BOOLEAN COMMENT 'Indicates whether the data subject is a minor requiring special consent handling under COPPA or GDPR Article 8.',
    `parental_consent_verified` BOOLEAN COMMENT 'Whether parental or guardian consent has been verified for minor data subjects.',
    `privacy_policy_version` STRING COMMENT 'Version identifier of the privacy policy in effect when consent was captured.',
    `proof_document_url` STRING COMMENT 'URL or reference to the stored proof document (screenshot, signed form, recording) evidencing consent.',
    `purpose_category` STRING COMMENT 'High-level purpose category for data processing under GDPR Article 6 (e.g. marketing, analytics, service_delivery, legal_obligation, legitimate_interest).',
    `record_created_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this consent record was first created in the system.',
    `record_source_system` STRING COMMENT 'Identifier of the source system that originated this consent record.',
    `renewal_date` DATE COMMENT 'Date when consent renewal is due, typically before expiration_date to allow re-consent collection.',
    `scope` STRING COMMENT 'Scope or purpose limitation of the consent, describing what data processing activities are covered.',
    `third_party_recipient` STRING COMMENT 'Name or identifier of the third party to whom data may be shared under this consent.',
    `user_agent` STRING COMMENT 'Browser or device user-agent string captured at the time of consent for audit purposes.',
    `version` STRING COMMENT 'Version number of this consent record, incremented on each update for optimistic concurrency control.',
    `withdrawal_reason` STRING COMMENT 'Reason provided by the customer when withdrawing consent, if any.',
    `withdrawal_timestamp` TIMESTAMP COMMENT 'UTC timestamp when consent was withdrawn by the customer.',
    CONSTRAINT pk_consent PRIMARY KEY(`consent_id`)
) COMMENT 'Single source of truth for all customer consent records including marketing opt-in/out, data processing, and privacy preferences. Supports GDPR, CCPA, and other regulatory frameworks. All consent decisions across channels are recorded here rather than scattered across profile, account, or contact tables.';
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ADD CONSTRAINT `fk_compliance_obligation_parent_obligation_id` FOREIGN KEY (`parent_obligation_id`) REFERENCES `vibe_retail_v1`.`compliance`.`obligation`(`obligation_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ADD CONSTRAINT `fk_compliance_obligation_requirement_id` FOREIGN KEY (`requirement_id`) REFERENCES `vibe_retail_v1`.`compliance`.`requirement`(`requirement_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ADD CONSTRAINT `fk_compliance_audit_schedule_audit_checklist_template_id` FOREIGN KEY (`audit_checklist_template_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_checklist_template`(`audit_checklist_template_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ADD CONSTRAINT `fk_compliance_audit_schedule_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ADD CONSTRAINT `fk_compliance_audit_schedule_primary_previous_audit_audit_schedule_id` FOREIGN KEY (`primary_previous_audit_audit_schedule_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_schedule`(`audit_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ADD CONSTRAINT `fk_compliance_audit_event_audit_schedule_id` FOREIGN KEY (`audit_schedule_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_schedule`(`audit_schedule_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ADD CONSTRAINT `fk_compliance_audit_event_follow_up_audit_event_id` FOREIGN KEY (`follow_up_audit_event_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_event`(`audit_event_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_audit_event_id` FOREIGN KEY (`audit_event_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_event`(`audit_event_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_parent_audit_finding_id` FOREIGN KEY (`parent_audit_finding_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_finding`(`audit_finding_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_primary_previous_finding_audit_finding_id` FOREIGN KEY (`primary_previous_finding_audit_finding_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_finding`(`audit_finding_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_audit_finding_id` FOREIGN KEY (`audit_finding_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_finding`(`audit_finding_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_environmental_event_id` FOREIGN KEY (`environmental_event_id`) REFERENCES `vibe_retail_v1`.`compliance`.`environmental_event`(`environmental_event_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_follow_up_corrective_action_id` FOREIGN KEY (`follow_up_corrective_action_id`) REFERENCES `vibe_retail_v1`.`compliance`.`corrective_action`(`corrective_action_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_osha_incident_id` FOREIGN KEY (`osha_incident_id`) REFERENCES `vibe_retail_v1`.`compliance`.`osha_incident`(`osha_incident_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_violation_notice_id` FOREIGN KEY (`violation_notice_id`) REFERENCES `vibe_retail_v1`.`compliance`.`violation_notice`(`violation_notice_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ADD CONSTRAINT `fk_compliance_food_safety_plan_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ADD CONSTRAINT `fk_compliance_food_safety_plan_superseded_food_safety_plan_id` FOREIGN KEY (`superseded_food_safety_plan_id`) REFERENCES `vibe_retail_v1`.`compliance`.`food_safety_plan`(`food_safety_plan_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ADD CONSTRAINT `fk_compliance_haccp_control_point_food_safety_plan_id` FOREIGN KEY (`food_safety_plan_id`) REFERENCES `vibe_retail_v1`.`compliance`.`food_safety_plan`(`food_safety_plan_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ADD CONSTRAINT `fk_compliance_haccp_control_point_parent_haccp_control_point_id` FOREIGN KEY (`parent_haccp_control_point_id`) REFERENCES `vibe_retail_v1`.`compliance`.`haccp_control_point`(`haccp_control_point_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ADD CONSTRAINT `fk_compliance_food_safety_log_corrected_food_safety_log_id` FOREIGN KEY (`corrected_food_safety_log_id`) REFERENCES `vibe_retail_v1`.`compliance`.`food_safety_log`(`food_safety_log_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ADD CONSTRAINT `fk_compliance_food_safety_log_haccp_control_point_id` FOREIGN KEY (`haccp_control_point_id`) REFERENCES `vibe_retail_v1`.`compliance`.`haccp_control_point`(`haccp_control_point_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ADD CONSTRAINT `fk_compliance_osha_incident_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ADD CONSTRAINT `fk_compliance_osha_incident_parent_osha_incident_id` FOREIGN KEY (`parent_osha_incident_id`) REFERENCES `vibe_retail_v1`.`compliance`.`osha_incident`(`osha_incident_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ADD CONSTRAINT `fk_compliance_osha_incident_primary_previous_incident_osha_incident_id` FOREIGN KEY (`primary_previous_incident_osha_incident_id`) REFERENCES `vibe_retail_v1`.`compliance`.`osha_incident`(`osha_incident_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ADD CONSTRAINT `fk_compliance_safety_inspection_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ADD CONSTRAINT `fk_compliance_safety_inspection_follow_up_safety_inspection_id` FOREIGN KEY (`follow_up_safety_inspection_id`) REFERENCES `vibe_retail_v1`.`compliance`.`safety_inspection`(`safety_inspection_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ADD CONSTRAINT `fk_compliance_pci_assessment_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ADD CONSTRAINT `fk_compliance_pci_assessment_prior_pci_assessment_id` FOREIGN KEY (`prior_pci_assessment_id`) REFERENCES `vibe_retail_v1`.`compliance`.`pci_assessment`(`pci_assessment_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ADD CONSTRAINT `fk_compliance_pci_control_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ADD CONSTRAINT `fk_compliance_pci_control_parent_pci_control_id` FOREIGN KEY (`parent_pci_control_id`) REFERENCES `vibe_retail_v1`.`compliance`.`pci_control`(`pci_control_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ADD CONSTRAINT `fk_compliance_privacy_assessment_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ADD CONSTRAINT `fk_compliance_privacy_assessment_prior_privacy_assessment_id` FOREIGN KEY (`prior_privacy_assessment_id`) REFERENCES `vibe_retail_v1`.`compliance`.`privacy_assessment`(`privacy_assessment_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ADD CONSTRAINT `fk_compliance_regulatory_filing_amended_regulatory_filing_id` FOREIGN KEY (`amended_regulatory_filing_id`) REFERENCES `vibe_retail_v1`.`compliance`.`regulatory_filing`(`regulatory_filing_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ADD CONSTRAINT `fk_compliance_regulatory_filing_certification_id` FOREIGN KEY (`certification_id`) REFERENCES `vibe_retail_v1`.`compliance`.`certification`(`certification_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ADD CONSTRAINT `fk_compliance_regulatory_filing_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ADD CONSTRAINT `fk_compliance_regulatory_filing_license_permit_id` FOREIGN KEY (`license_permit_id`) REFERENCES `vibe_retail_v1`.`compliance`.`license_permit`(`license_permit_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ADD CONSTRAINT `fk_compliance_regulatory_filing_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `vibe_retail_v1`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ADD CONSTRAINT `fk_compliance_license_permit_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ADD CONSTRAINT `fk_compliance_license_permit_renewed_from_license_permit_id` FOREIGN KEY (`renewed_from_license_permit_id`) REFERENCES `vibe_retail_v1`.`compliance`.`license_permit`(`license_permit_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ADD CONSTRAINT `fk_compliance_certification_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ADD CONSTRAINT `fk_compliance_certification_renewed_from_certification_id` FOREIGN KEY (`renewed_from_certification_id`) REFERENCES `vibe_retail_v1`.`compliance`.`certification`(`certification_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ADD CONSTRAINT `fk_compliance_environmental_event_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ADD CONSTRAINT `fk_compliance_environmental_event_parent_environmental_event_id` FOREIGN KEY (`parent_environmental_event_id`) REFERENCES `vibe_retail_v1`.`compliance`.`environmental_event`(`environmental_event_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ADD CONSTRAINT `fk_compliance_training_program_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ADD CONSTRAINT `fk_compliance_training_program_prerequisite_training_program_id` FOREIGN KEY (`prerequisite_training_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`training_program`(`training_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ADD CONSTRAINT `fk_compliance_training_completion_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ADD CONSTRAINT `fk_compliance_training_completion_retake_of_training_completion_id` FOREIGN KEY (`retake_of_training_completion_id`) REFERENCES `vibe_retail_v1`.`compliance`.`training_completion`(`training_completion_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ADD CONSTRAINT `fk_compliance_training_completion_training_program_id` FOREIGN KEY (`training_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`training_program`(`training_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ADD CONSTRAINT `fk_compliance_violation_notice_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ADD CONSTRAINT `fk_compliance_violation_notice_parent_violation_notice_id` FOREIGN KEY (`parent_violation_notice_id`) REFERENCES `vibe_retail_v1`.`compliance`.`violation_notice`(`violation_notice_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ADD CONSTRAINT `fk_compliance_violation_notice_primary_previous_violation_notice_id` FOREIGN KEY (`primary_previous_violation_notice_id`) REFERENCES `vibe_retail_v1`.`compliance`.`violation_notice`(`violation_notice_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ADD CONSTRAINT `fk_compliance_violation_notice_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `vibe_retail_v1`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ADD CONSTRAINT `fk_compliance_policy_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ADD CONSTRAINT `fk_compliance_policy_superseded_policy_id` FOREIGN KEY (`superseded_policy_id`) REFERENCES `vibe_retail_v1`.`compliance`.`policy`(`policy_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_audit_finding_id` FOREIGN KEY (`audit_finding_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_finding`(`audit_finding_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_environmental_event_id` FOREIGN KEY (`environmental_event_id`) REFERENCES `vibe_retail_v1`.`compliance`.`environmental_event`(`environmental_event_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_parent_risk_register_id` FOREIGN KEY (`parent_risk_register_id`) REFERENCES `vibe_retail_v1`.`compliance`.`risk_register`(`risk_register_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_osha_incident_id` FOREIGN KEY (`osha_incident_id`) REFERENCES `vibe_retail_v1`.`compliance`.`osha_incident`(`osha_incident_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ADD CONSTRAINT `fk_compliance_risk_register_violation_notice_id` FOREIGN KEY (`violation_notice_id`) REFERENCES `vibe_retail_v1`.`compliance`.`violation_notice`(`violation_notice_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ADD CONSTRAINT `fk_compliance_third_party_assessment_certification_id` FOREIGN KEY (`certification_id`) REFERENCES `vibe_retail_v1`.`compliance`.`certification`(`certification_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ADD CONSTRAINT `fk_compliance_third_party_assessment_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ADD CONSTRAINT `fk_compliance_third_party_assessment_prior_third_party_assessment_id` FOREIGN KEY (`prior_third_party_assessment_id`) REFERENCES `vibe_retail_v1`.`compliance`.`third_party_assessment`(`third_party_assessment_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ADD CONSTRAINT `fk_compliance_facility_training_requirement_training_program_id` FOREIGN KEY (`training_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`training_program`(`training_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ADD CONSTRAINT `fk_compliance_facility_compliance_certification_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ADD CONSTRAINT `fk_compliance_facility_compliance_certification_facility_program_compliance_program_id` FOREIGN KEY (`facility_program_compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ADD CONSTRAINT `fk_compliance_audit_checklist_template_compliance_program_id` FOREIGN KEY (`compliance_program_id`) REFERENCES `vibe_retail_v1`.`compliance`.`compliance_program`(`compliance_program_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ADD CONSTRAINT `fk_compliance_audit_checklist_template_parent_audit_checklist_template_id` FOREIGN KEY (`parent_audit_checklist_template_id`) REFERENCES `vibe_retail_v1`.`compliance`.`audit_checklist_template`(`audit_checklist_template_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ADD CONSTRAINT `fk_compliance_requirement_parent_requirement_id` FOREIGN KEY (`parent_requirement_id`) REFERENCES `vibe_retail_v1`.`compliance`.`requirement`(`requirement_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ADD CONSTRAINT `fk_compliance_requirement_primary_superseded_by_requirement_id` FOREIGN KEY (`primary_superseded_by_requirement_id`) REFERENCES `vibe_retail_v1`.`compliance`.`requirement`(`requirement_id`);
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ADD CONSTRAINT `fk_compliance_regulatory_agency_parent_regulatory_agency_id` FOREIGN KEY (`parent_regulatory_agency_id`) REFERENCES `vibe_retail_v1`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);

-- ========= TAGS =========
ALTER SCHEMA `vibe_retail_v1`.`compliance` SET TAGS ('dbx_division' = 'corporate');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` SET TAGS ('dbx_ssot_role' = 'authoritative');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` SET TAGS ('dbx_ssot_concept' = 'compliance_program');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` SET TAGS ('dbx_ssot_resolution' = 'distinct_concept');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` SET TAGS ('dbx_ssot_owner' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `parent_compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Compliance Program Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `certification_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `certification_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `compliance_program_description` SET TAGS ('dbx_business_glossary_term' = 'Program Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `governing_body` SET TAGS ('dbx_business_glossary_term' = 'Governing Body');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `last_audit_result` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Result');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `next_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `owning_business_unit` SET TAGS ('dbx_business_glossary_term' = 'Owning Business Unit');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Program Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_-]{3,20}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_name` SET TAGS ('dbx_business_glossary_term' = 'Program Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_scope` SET TAGS ('dbx_business_glossary_term' = 'Program Scope');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_status` SET TAGS ('dbx_business_glossary_term' = 'Program Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Program Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `renewal_date` SET TAGS ('dbx_business_glossary_term' = 'Renewal Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`compliance_program` ALTER COLUMN `training_frequency` SET TAGS ('dbx_business_glossary_term' = 'Training Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `parent_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Obligation Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `parent_obligation_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Requirement ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_code` SET TAGS ('dbx_business_glossary_term' = 'Obligation Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,20}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `compliance_framework` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `control_objective` SET TAGS ('dbx_business_glossary_term' = 'Control Objective');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_description` SET TAGS ('dbx_business_glossary_term' = 'Obligation Description');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `escalation_threshold_days` SET TAGS ('dbx_business_glossary_term' = 'Escalation Threshold Days');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `evidence_type` SET TAGS ('dbx_business_glossary_term' = 'Evidence Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `financial_penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Penalty Amount');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `last_fulfilled_date` SET TAGS ('dbx_business_glossary_term' = 'Last Fulfilled Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `location_scope` SET TAGS ('dbx_business_glossary_term' = 'Location Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `next_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `next_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Obligation Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_status` SET TAGS ('dbx_business_glossary_term' = 'Obligation Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_type` SET TAGS ('dbx_business_glossary_term' = 'Obligation Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `obligation_type` SET TAGS ('dbx_value_regex' = 'policy|procedure|training|audit|certification|reporting');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `penalty_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Penalty Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `penalty_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Obligation Priority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `responsible_role` SET TAGS ('dbx_business_glossary_term' = 'Responsible Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `retention_period_days` SET TAGS ('dbx_business_glossary_term' = 'Retention Period Days');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Obligation Title');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `waiver_approved` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approved Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `waiver_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Waiver Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`obligation` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` SET TAGS ('dbx_subdomain' = 'audit_control');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Schedule ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_checklist_template_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Checklist Template ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Location Contact ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Auditor ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `primary_previous_audit_audit_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Audit ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `tertiary_audit_modified_by_user_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `advance_notice_days` SET TAGS ('dbx_business_glossary_term' = 'Advance Notice Days');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_number` SET TAGS ('dbx_business_glossary_term' = 'Audit Schedule Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_number` SET TAGS ('dbx_value_regex' = '^AUD-[A-Z0-9]{8,12}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_scope` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `budget_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `certification_body` SET TAGS ('dbx_business_glossary_term' = 'Certification Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `confirmation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Schedule Confirmation Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Audit Cost');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `estimated_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Estimated Duration (Hours)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `is_follow_up_audit` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Audit Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `notification_sent_date` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Audit Priority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `recurrence_rule` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Rule');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `regulatory_deadline` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Deadline Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `requires_site_access_approval` SET TAGS ('dbx_business_glossary_term' = 'Site Access Approval Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `reschedule_reason` SET TAGS ('dbx_business_glossary_term' = 'Reschedule Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `schedule_status` SET TAGS ('dbx_business_glossary_term' = 'Schedule Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `schedule_status` SET TAGS ('dbx_value_regex' = 'planned|confirmed|in_progress|rescheduled|completed|cancelled');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `scheduled_end_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled End Time');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `scheduled_start_time` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Start Time');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `special_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Instructions');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `third_party_firm_name` SET TAGS ('dbx_business_glossary_term' = 'Third-Party Audit Firm Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_schedule` ALTER COLUMN `third_party_firm_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` SET TAGS ('dbx_subdomain' = 'audit_control');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_event_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Event ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Schedule ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `follow_up_audit_event_id` SET TAGS ('dbx_business_glossary_term' = 'Follow Up Audit Event Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `follow_up_audit_event_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Audit Duration Hours');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Audit End Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_method` SET TAGS ('dbx_business_glossary_term' = 'Audit Method');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_number` SET TAGS ('dbx_business_glossary_term' = 'Audit Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_number` SET TAGS ('dbx_value_regex' = '^AUD-[0-9]{8}-[A-Z0-9]{6}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_report_url` SET TAGS ('dbx_business_glossary_term' = 'Audit Report URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_report_url` SET TAGS ('dbx_value_regex' = '^https?://.*$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_result` SET TAGS ('dbx_business_glossary_term' = 'Audit Result');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_result` SET TAGS ('dbx_value_regex' = 'pass|conditional_pass|fail|not_applicable');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_scope` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_score` SET TAGS ('dbx_business_glossary_term' = 'Audit Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Audit Start Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_status` SET TAGS ('dbx_business_glossary_term' = 'Audit Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_status` SET TAGS ('dbx_value_regex' = 'scheduled|in_progress|completed|cancelled|deferred');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_team_members` SET TAGS ('dbx_business_glossary_term' = 'Audit Team Members');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `audit_type` SET TAGS ('dbx_value_regex' = 'food_safety|workplace_safety|pci_dss|gdpr_ccpa|environmental|quality_assurance');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `auditee_signature` SET TAGS ('dbx_business_glossary_term' = 'Auditee Signature');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `auditor_signature` SET TAGS ('dbx_business_glossary_term' = 'Auditor Signature');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `certification_body` SET TAGS ('dbx_business_glossary_term' = 'Certification Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `corrective_action_plan_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan (CAP) Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `critical_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `executive_summary` SET TAGS ('dbx_business_glossary_term' = 'Executive Summary');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `follow_up_due_date` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `follow_up_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `lead_auditor_credentials` SET TAGS ('dbx_business_glossary_term' = 'Lead Auditor Credentials');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `lead_auditor_name` SET TAGS ('dbx_business_glossary_term' = 'Lead Auditor Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `lead_auditor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `major_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Major Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `minor_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Minor Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Audit Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `regulatory_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_event` ALTER COLUMN `total_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Total Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` SET TAGS ('dbx_subdomain' = 'audit_control');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Identifier (ID)');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `parent_audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Related Audit Finding Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `tertiary_audit_verified_by_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Verified By Identifier (ID)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `affected_area` SET TAGS ('dbx_business_glossary_term' = 'Affected Area or Process');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_status` SET TAGS ('dbx_business_glossary_term' = 'Finding Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_status` SET TAGS ('dbx_value_regex' = 'open|in_remediation|pending_verification|closed|disputed');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_category` SET TAGS ('dbx_business_glossary_term' = 'Finding Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_category` SET TAGS ('dbx_value_regex' = 'critical|major|minor|observation');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `closed_date` SET TAGS ('dbx_business_glossary_term' = 'Finding Closed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `control_reference` SET TAGS ('dbx_business_glossary_term' = 'Control or Requirement Reference');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `corrective_action_plan` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan (CAP)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `corrective_action_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_description` SET TAGS ('dbx_business_glossary_term' = 'Finding Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `evidence_location` SET TAGS ('dbx_business_glossary_term' = 'Evidence Storage Location');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `financial_impact_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Impact Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `finding_number` SET TAGS ('dbx_business_glossary_term' = 'Finding Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `identified_date` SET TAGS ('dbx_business_glossary_term' = 'Finding Identified Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Finding Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `photographic_evidence_flag` SET TAGS ('dbx_business_glossary_term' = 'Photographic Evidence Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `recurrence_flag` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_report_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_standard` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Standard');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `root_cause` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Analysis');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Finding Title');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Verification Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_finding` ALTER COLUMN `verification_method` SET TAGS ('dbx_business_glossary_term' = 'Verification Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` SET TAGS ('dbx_subdomain' = 'audit_control');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `corrective_action_id` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `follow_up_corrective_action_id` SET TAGS ('dbx_business_glossary_term' = 'Follow Up Corrective Action Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `violation_notice_id` SET TAGS ('dbx_business_glossary_term' = 'Violation Notice Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `action_number` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Completion Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `actual_cost` SET TAGS ('dbx_business_glossary_term' = 'Actual Cost');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `actual_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_department` SET TAGS ('dbx_business_glossary_term' = 'Assigned Department');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_owner_name` SET TAGS ('dbx_business_glossary_term' = 'Assigned Owner Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_owner_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_owner_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_owner_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `assigned_owner_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `closure_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `closure_notes` SET TAGS ('dbx_business_glossary_term' = 'Closure Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `compliance_category` SET TAGS ('dbx_business_glossary_term' = 'Compliance Category');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `corrective_action_status` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `corrective_action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `effectiveness_rating` SET TAGS ('dbx_business_glossary_term' = 'Effectiveness Rating');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Cost');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `external_report_date` SET TAGS ('dbx_business_glossary_term' = 'External Report Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `recurrence_prevention_plan` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Prevention Plan');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `root_cause_description` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `target_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Target Completion Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Title');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Verification Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verification_method` SET TAGS ('dbx_business_glossary_term' = 'Verification Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verification_method` SET TAGS ('dbx_value_regex' = 'document_review|on_site_inspection|testing|audit|management_review|third_party_validation');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verification_notes` SET TAGS ('dbx_business_glossary_term' = 'Verification Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verified_by_name` SET TAGS ('dbx_business_glossary_term' = 'Verified By Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`corrective_action` ALTER COLUMN `verified_by_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` SET TAGS ('dbx_subdomain' = 'safety_operations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `superseded_food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Food Safety Plan Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `critical_control_points_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Control Points (CCP) Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `document_location` SET TAGS ('dbx_business_glossary_term' = 'Document Location');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `environmental_monitoring_program` SET TAGS ('dbx_business_glossary_term' = 'Environmental Monitoring Program Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `facility_scope` SET TAGS ('dbx_business_glossary_term' = 'Facility Scope Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `fda_registration_number` SET TAGS ('dbx_business_glossary_term' = 'Food and Drug Administration (FDA) Registration Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `hazard_analysis_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Hazard Analysis Completion Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `last_audit_result` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Result');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `last_audit_result` SET TAGS ('dbx_value_regex' = 'compliant|non-compliant|minor findings|major findings|pending');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `last_regulatory_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Regulatory Inspection Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `next_audit_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `next_review_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Plan Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Approval Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_review_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_status` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_status` SET TAGS ('dbx_value_regex' = 'active|under review|pending approval|expired|superseded|suspended');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_type` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_type` SET TAGS ('dbx_value_regex' = 'HACCP|HARPC|Preventive Controls|Juice HACCP|Seafood HACCP|Produce Safety');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `plan_version` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `preventive_controls_summary` SET TAGS ('dbx_business_glossary_term' = 'Preventive Controls Summary');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `qfsp_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `qfsp_contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `qfsp_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `record_retention_years` SET TAGS ('dbx_business_glossary_term' = 'Record Retention Years');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `regulatory_authority` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `supply_chain_program_included` SET TAGS ('dbx_business_glossary_term' = 'Supply Chain Program Included Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_plan` ALTER COLUMN `verification_activities_summary` SET TAGS ('dbx_business_glossary_term' = 'Verification Activities Summary');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `parent_haccp_control_point_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Haccp Control Point Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `allergen_control_flag` SET TAGS ('dbx_business_glossary_term' = 'Allergen Control Flag');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_code` SET TAGS ('dbx_business_glossary_term' = 'Control Point Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_name` SET TAGS ('dbx_business_glossary_term' = 'Control Point Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_status` SET TAGS ('dbx_business_glossary_term' = 'Control Point Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_status` SET TAGS ('dbx_value_regex' = 'active|inactive|under_review|suspended|archived');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_type` SET TAGS ('dbx_business_glossary_term' = 'Control Point Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `control_point_type` SET TAGS ('dbx_value_regex' = 'critical_control_point|prerequisite_program|preventive_control|allergen_control|sanitation_control|supply_chain_control');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `corrective_action_procedure` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Procedure');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `critical_limit_parameter` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Parameter');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `critical_limit_unit` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Unit of Measure');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `critical_limit_value` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Value');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `environmental_monitoring_flag` SET TAGS ('dbx_business_glossary_term' = 'Environmental Monitoring Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `gfsi_scheme_alignment` SET TAGS ('dbx_business_glossary_term' = 'Global Food Safety Initiative (GFSI) Scheme Alignment');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `hazard_description` SET TAGS ('dbx_business_glossary_term' = 'Hazard Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `hazard_type` SET TAGS ('dbx_business_glossary_term' = 'Hazard Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `hazard_type` SET TAGS ('dbx_value_regex' = 'biological|chemical|physical|radiological|allergen');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `last_validation_date` SET TAGS ('dbx_business_glossary_term' = 'Last Validation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `likelihood_level` SET TAGS ('dbx_business_glossary_term' = 'Likelihood Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `likelihood_level` SET TAGS ('dbx_value_regex' = 'high|medium|low|negligible');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `monitoring_equipment` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Equipment');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `monitoring_frequency` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `monitoring_method` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `next_validation_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Validation Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Control Point Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `prerequisite_program_flag` SET TAGS ('dbx_business_glossary_term' = 'Prerequisite Program Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `process_step` SET TAGS ('dbx_business_glossary_term' = 'Process Step');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `record_keeping_requirement` SET TAGS ('dbx_business_glossary_term' = 'Record Keeping Requirement');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `regulatory_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `responsible_role` SET TAGS ('dbx_business_glossary_term' = 'Responsible Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `supplier_control_flag` SET TAGS ('dbx_business_glossary_term' = 'Supplier Control Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `validation_method` SET TAGS ('dbx_business_glossary_term' = 'Validation Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `verification_frequency` SET TAGS ('dbx_business_glossary_term' = 'Verification Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`haccp_control_point` ALTER COLUMN `verification_procedure` SET TAGS ('dbx_business_glossary_term' = 'Verification Procedure');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `food_safety_log_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Log ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `corrected_food_safety_log_id` SET TAGS ('dbx_business_glossary_term' = 'Corrected Food Safety Log Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `audit_trail_reference` SET TAGS ('dbx_business_glossary_term' = 'Audit Trail ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Comments');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `control_point_type` SET TAGS ('dbx_business_glossary_term' = 'Control Point Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `control_point_type` SET TAGS ('dbx_value_regex' = 'ccp|cp|prp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `corrective_action_required` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `corrective_action_taken` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Taken');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `corrective_action_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `critical_limit_lower` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Lower Bound');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `critical_limit_upper` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Upper Bound');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `department` SET TAGS ('dbx_business_glossary_term' = 'Department');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `deviation_severity` SET TAGS ('dbx_business_glossary_term' = 'Deviation Severity');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `instrument_calibration_date` SET TAGS ('dbx_business_glossary_term' = 'Instrument Calibration Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `measured_value` SET TAGS ('dbx_business_glossary_term' = 'Measured Value');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `monitoring_employee_name` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Employee Name');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `monitoring_method` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Method');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `monitoring_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `product_affected` SET TAGS ('dbx_business_glossary_term' = 'Product Affected');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `product_disposition` SET TAGS ('dbx_business_glossary_term' = 'Product Disposition');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `supervisor_name` SET TAGS ('dbx_business_glossary_term' = 'Supervisor Name');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `verification_signature` SET TAGS ('dbx_business_glossary_term' = 'Verification Signature');
ALTER TABLE `vibe_retail_v1`.`compliance`.`food_safety_log` ALTER COLUMN `verification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Verification Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `parent_osha_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Related Osha Incident Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `body_part_affected` SET TAGS ('dbx_business_glossary_term' = 'Body Part Affected');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `case_closed_date` SET TAGS ('dbx_business_glossary_term' = 'Case Closed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `corrective_action_plan` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `days_away_from_work` SET TAGS ('dbx_business_glossary_term' = 'Days Away From Work');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `estimated_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Cost Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `estimated_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `incident_date` SET TAGS ('dbx_business_glossary_term' = 'Incident Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `incident_description` SET TAGS ('dbx_business_glossary_term' = 'Incident Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `incident_number` SET TAGS ('dbx_business_glossary_term' = 'Incident Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `incident_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Incident Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `incident_type` SET TAGS ('dbx_business_glossary_term' = 'Incident Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `injury_type` SET TAGS ('dbx_business_glossary_term' = 'Injury Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `investigation_completed_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Completed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `location_description` SET TAGS ('dbx_business_glossary_term' = 'Location Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `medical_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Medical Facility Name');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `photographic_evidence_flag` SET TAGS ('dbx_business_glossary_term' = 'Photographic Evidence Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `privacy_case_flag` SET TAGS ('dbx_business_glossary_term' = 'Privacy Case Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `recurrence_flag` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `regulatory_report_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `reported_date` SET TAGS ('dbx_business_glossary_term' = 'Reported Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `root_cause` SET TAGS ('dbx_business_glossary_term' = 'Root Cause');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `treatment_provided` SET TAGS ('dbx_business_glossary_term' = 'Treatment Provided');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `witness_count` SET TAGS ('dbx_business_glossary_term' = 'Witness Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `work_activity` SET TAGS ('dbx_business_glossary_term' = 'Work Activity');
ALTER TABLE `vibe_retail_v1`.`compliance`.`osha_incident` ALTER COLUMN `workers_compensation_claim_number` SET TAGS ('dbx_business_glossary_term' = 'Workers Compensation Claim Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `safety_inspection_id` SET TAGS ('dbx_business_glossary_term' = 'Safety Inspection ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `follow_up_safety_inspection_id` SET TAGS ('dbx_business_glossary_term' = 'Follow Up Safety Inspection Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `checklist_version` SET TAGS ('dbx_business_glossary_term' = 'Checklist Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `compliance_score` SET TAGS ('dbx_business_glossary_term' = 'Compliance Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `corrective_action_deadline` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Deadline');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `corrective_action_required` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `critical_violations_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Violations Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `emergency_preparedness_compliant` SET TAGS ('dbx_business_glossary_term' = 'Emergency Preparedness Compliant');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `equipment_safety_compliant` SET TAGS ('dbx_business_glossary_term' = 'Equipment Safety Compliant');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `ergonomics_compliant` SET TAGS ('dbx_business_glossary_term' = 'Ergonomics Compliant');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `fire_safety_compliant` SET TAGS ('dbx_business_glossary_term' = 'Fire Safety Compliant');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Inspection Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Inspection Duration Minutes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_notes` SET TAGS ('dbx_business_glossary_term' = 'Inspection Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_number` SET TAGS ('dbx_business_glossary_term' = 'Inspection Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_scope` SET TAGS ('dbx_business_glossary_term' = 'Inspection Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_status` SET TAGS ('dbx_business_glossary_term' = 'Inspection Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_status` SET TAGS ('dbx_value_regex' = 'scheduled|in_progress|completed|cancelled|pending_review');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_type` SET TAGS ('dbx_business_glossary_term' = 'Inspection Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspection_type` SET TAGS ('dbx_value_regex' = 'routine|triggered|pre_opening|regulatory|incident_followup|annual');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspector_certification_number` SET TAGS ('dbx_business_glossary_term' = 'Inspector Certification Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspector_name` SET TAGS ('dbx_business_glossary_term' = 'Inspector Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspector_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `inspector_role` SET TAGS ('dbx_business_glossary_term' = 'Inspector Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `items_failed` SET TAGS ('dbx_business_glossary_term' = 'Items Failed');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `items_passed` SET TAGS ('dbx_business_glossary_term' = 'Items Passed');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `location_manager_notified` SET TAGS ('dbx_business_glossary_term' = 'Location Manager Notified');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `major_violations_count` SET TAGS ('dbx_business_glossary_term' = 'Major Violations Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `minor_violations_count` SET TAGS ('dbx_business_glossary_term' = 'Minor Violations Count');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `regulatory_authority_notified` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Authority Notified');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `report_document_url` SET TAGS ('dbx_business_glossary_term' = 'Report Document URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `report_generated_date` SET TAGS ('dbx_business_glossary_term' = 'Report Generated Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `total_items_inspected` SET TAGS ('dbx_business_glossary_term' = 'Total Items Inspected');
ALTER TABLE `vibe_retail_v1`.`compliance`.`safety_inspection` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `prior_pci_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Pci Assessment Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `acquiring_bank_name` SET TAGS ('dbx_business_glossary_term' = 'Acquiring Bank Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `acquiring_bank_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `annual_transaction_volume` SET TAGS ('dbx_business_glossary_term' = 'Annual Transaction Volume');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessment_report_url` SET TAGS ('dbx_business_glossary_term' = 'Assessment Report Uniform Resource Locator (URL)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessment_scope_notes` SET TAGS ('dbx_business_glossary_term' = 'Assessment Scope Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessment_start_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Start Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_business_glossary_term' = 'Lead Assessor Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `attestation_of_compliance_submitted_flag` SET TAGS ('dbx_business_glossary_term' = 'Attestation of Compliance (AOC) Submitted Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `attestation_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Attestation of Compliance (AOC) Submission Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `cardholder_data_environment_scope` SET TAGS ('dbx_business_glossary_term' = 'Cardholder Data Environment (CDE) Scope Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `cardholder_data_environment_scope` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `compensating_controls_count` SET TAGS ('dbx_business_glossary_term' = 'Compensating Controls Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Overall Compliance Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'Compliant|Non-Compliant|Compliant with Compensating Controls|In Progress');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `critical_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `executive_summary` SET TAGS ('dbx_business_glossary_term' = 'Executive Summary');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `in_scope_locations_count` SET TAGS ('dbx_business_glossary_term' = 'In-Scope Locations Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `in_scope_systems_count` SET TAGS ('dbx_business_glossary_term' = 'In-Scope Systems Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `last_vulnerability_scan_date` SET TAGS ('dbx_business_glossary_term' = 'Last Vulnerability Scan Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `merchant_level` SET TAGS ('dbx_business_glossary_term' = 'Merchant Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `merchant_level` SET TAGS ('dbx_value_regex' = 'Level 1|Level 2|Level 3|Level 4');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `next_assessment_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Assessment Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `pci_dss_version` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry Data Security Standard (PCI-DSS) Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `penetration_test_date` SET TAGS ('dbx_business_glossary_term' = 'Penetration Test Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `penetration_test_pass_flag` SET TAGS ('dbx_business_glossary_term' = 'Penetration Test Pass Flag');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `qsa_firm_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `remediation_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Remediation Deadline Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `remediation_plan_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Remediation Plan Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `requirements_failed_count` SET TAGS ('dbx_business_glossary_term' = 'Requirements Failed Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `requirements_met_count` SET TAGS ('dbx_business_glossary_term' = 'Requirements Met Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `total_requirements_count` SET TAGS ('dbx_business_glossary_term' = 'Total Requirements Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_assessment` ALTER COLUMN `vulnerability_scan_pass_flag` SET TAGS ('dbx_business_glossary_term' = 'Vulnerability Scan Pass Flag');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `parent_pci_control_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Pci Control Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `applicable_payment_channels` SET TAGS ('dbx_business_glossary_term' = 'Applicable Payment Channels');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `automation_level` SET TAGS ('dbx_value_regex' = 'fully_automated|partially_automated|manual');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `cardholder_data_environment_scope` SET TAGS ('dbx_business_glossary_term' = 'Cardholder Data Environment (CDE) Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `cardholder_data_environment_scope` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `compensating_control_flag` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Compensating Control Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `compensating_control_justification` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Compensating Control Justification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `control_category` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `control_category` SET TAGS ('dbx_value_regex' = 'network_security|access_control|encryption|vulnerability_management|monitoring_logging|policy_procedure');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `control_objective` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Objective');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `control_owner_role` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Owner Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `evidence_location` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Evidence Location');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `implementation_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Implementation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `implementation_status` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Implementation Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `implementation_status` SET TAGS ('dbx_value_regex' = 'implemented|partially_implemented|not_implemented|compensating_control|not_applicable');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `last_tested_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Last Tested Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `monitoring_tool` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Monitoring Tool');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `next_test_due_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Next Test Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `policy_reference` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Policy Reference');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `remediation_due_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Remediation Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `remediation_plan` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Remediation Plan');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `requirement_description` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry Data Security Standard (PCI-DSS) Requirement Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `requirement_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry Data Security Standard (PCI-DSS) Requirement Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `requirement_number` SET TAGS ('dbx_value_regex' = '^[0-9]{1,2}.[0-9]{1,2}(.[0-9]{1,2})?$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Risk Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `test_result` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Test Result');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `test_result` SET TAGS ('dbx_value_regex' = 'pass|fail|partial_pass|not_tested');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `testing_frequency` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Testing Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `testing_method` SET TAGS ('dbx_business_glossary_term' = 'Payment Card Industry (PCI) Control Testing Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `testing_method` SET TAGS ('dbx_value_regex' = 'observation|examination|interview|penetration_test|vulnerability_scan|automated_monitoring');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`pci_control` ALTER COLUMN `third_party_provider_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `privacy_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Privacy Assessment ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `prior_privacy_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Privacy Assessment Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Completion Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_methodology` SET TAGS ('dbx_business_glossary_term' = 'Assessment Methodology');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_name` SET TAGS ('dbx_business_glossary_term' = 'Assessment Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_outcome` SET TAGS ('dbx_business_glossary_term' = 'Assessment Outcome');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_start_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Start Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_business_glossary_term' = 'Assessor Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `assessor_role` SET TAGS ('dbx_business_glossary_term' = 'Assessor Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `automated_decision_making` SET TAGS ('dbx_business_glossary_term' = 'Automated Decision Making');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `business_process` SET TAGS ('dbx_business_glossary_term' = 'Business Process');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `business_unit` SET TAGS ('dbx_business_glossary_term' = 'Business Unit');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `children_data_processing` SET TAGS ('dbx_business_glossary_term' = 'Children Data Processing');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `consultation_date` SET TAGS ('dbx_business_glossary_term' = 'Consultation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `consultation_outcome` SET TAGS ('dbx_business_glossary_term' = 'Consultation Outcome');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `data_retention_period` SET TAGS ('dbx_business_glossary_term' = 'Data Retention Period');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `data_subject_rights_supported` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Rights Supported');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `data_subjects` SET TAGS ('dbx_business_glossary_term' = 'Data Subjects');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `data_transfer_countries` SET TAGS ('dbx_business_glossary_term' = 'Data Transfer Countries');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `dpo_review_date` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `dpo_reviewer_name` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Reviewer Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `dpo_reviewer_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `geographic_scope` SET TAGS ('dbx_business_glossary_term' = 'Geographic Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `legal_basis` SET TAGS ('dbx_business_glossary_term' = 'Legal Basis');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `legal_basis` SET TAGS ('dbx_value_regex' = 'consent|contract|legal_obligation|vital_interests|public_task|legitimate_interests');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `mitigations_applied` SET TAGS ('dbx_business_glossary_term' = 'Mitigations Applied');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `personal_data_categories` SET TAGS ('dbx_business_glossary_term' = 'Personal Data Categories');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `privacy_risks_identified` SET TAGS ('dbx_business_glossary_term' = 'Privacy Risks Identified');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `processing_purpose` SET TAGS ('dbx_business_glossary_term' = 'Processing Purpose');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `regulatory_frameworks` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Frameworks');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `special_category_data` SET TAGS ('dbx_business_glossary_term' = 'Special Category Data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `supervisory_authority_consulted` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Consulted');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `system_name` SET TAGS ('dbx_business_glossary_term' = 'System Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `system_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `transfer_safeguards` SET TAGS ('dbx_business_glossary_term' = 'Transfer Safeguards');
ALTER TABLE `vibe_retail_v1`.`compliance`.`privacy_assessment` ALTER COLUMN `triggering_event` SET TAGS ('dbx_business_glossary_term' = 'Triggering Event');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` SET TAGS ('dbx_subdomain' = 'license_certification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `amended_regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Amended Regulatory Filing Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `acceptance_date` SET TAGS ('dbx_business_glossary_term' = 'Acceptance Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `acknowledgment_date` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `compliance_certification_number` SET TAGS ('dbx_business_glossary_term' = 'Compliance Certification Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_description` SET TAGS ('dbx_business_glossary_term' = 'Filing Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_document_url` SET TAGS ('dbx_business_glossary_term' = 'Filing Document URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_document_url` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Filing Fee Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_fee_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Filing Fee Currency Code');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Filing Jurisdiction');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Filing Period End Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Filing Period Start Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_status` SET TAGS ('dbx_business_glossary_term' = 'Filing Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `filing_type` SET TAGS ('dbx_business_glossary_term' = 'Filing Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `next_filing_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Filing Due Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `payment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Reference Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `rejection_date` SET TAGS ('dbx_business_glossary_term' = 'Rejection Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `renewal_frequency` SET TAGS ('dbx_business_glossary_term' = 'Renewal Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `responsible_party_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Name');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `responsible_party_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `resubmission_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Resubmission Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `submission_confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Submission Confirmation Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_filing` ALTER COLUMN `submission_method` SET TAGS ('dbx_business_glossary_term' = 'Submission Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` SET TAGS ('dbx_subdomain' = 'license_certification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_permit_id` SET TAGS ('dbx_business_glossary_term' = 'License Permit ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `renewed_from_license_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Renewed From License Permit Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `associated_entity_type` SET TAGS ('dbx_business_glossary_term' = 'Associated Entity Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `certification_body` SET TAGS ('dbx_business_glossary_term' = 'Certification Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `conditions_of_license` SET TAGS ('dbx_business_glossary_term' = 'Conditions of License');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `document_reference_url` SET TAGS ('dbx_business_glossary_term' = 'Document Reference URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `fee_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Fee Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `fee_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `inspection_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Inspection Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Issue Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `issuing_authority` SET TAGS ('dbx_business_glossary_term' = 'Issuing Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `issuing_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Issuing Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `last_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `last_renewal_date` SET TAGS ('dbx_business_glossary_term' = 'Last Renewal Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'License Fee Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_number` SET TAGS ('dbx_business_glossary_term' = 'License Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_scope` SET TAGS ('dbx_business_glossary_term' = 'License Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_scope` SET TAGS ('dbx_value_regex' = 'enterprise|banner|location|department');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_status` SET TAGS ('dbx_business_glossary_term' = 'License Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_status` SET TAGS ('dbx_value_regex' = 'active|expired|suspended|revoked|pending_renewal|pending_approval');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `license_type` SET TAGS ('dbx_business_glossary_term' = 'License Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `next_inspection_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Inspection Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `reinstatement_date` SET TAGS ('dbx_business_glossary_term' = 'Reinstatement Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `renewal_due_date` SET TAGS ('dbx_business_glossary_term' = 'Renewal Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `renewal_frequency` SET TAGS ('dbx_business_glossary_term' = 'Renewal Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `renewal_status` SET TAGS ('dbx_business_glossary_term' = 'Renewal Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_email` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Email');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_phone` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Phone');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `responsible_party_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `revocation_date` SET TAGS ('dbx_business_glossary_term' = 'Revocation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `revocation_reason` SET TAGS ('dbx_business_glossary_term' = 'Revocation Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `suspension_date` SET TAGS ('dbx_business_glossary_term' = 'Suspension Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`license_permit` ALTER COLUMN `suspension_reason` SET TAGS ('dbx_business_glossary_term' = 'Suspension Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` SET TAGS ('dbx_subdomain' = 'license_certification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certification_id` SET TAGS ('dbx_business_glossary_term' = 'Certification ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `item_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Item Hierarchy Id (Foreign Key)');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `renewed_from_certification_id` SET TAGS ('dbx_business_glossary_term' = 'Renewed From Certification Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `accreditation_body` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certificate_document_url` SET TAGS ('dbx_business_glossary_term' = 'Certificate Document URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certificate_document_url` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certificate_number` SET TAGS ('dbx_business_glossary_term' = 'Certificate Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certification_type` SET TAGS ('dbx_business_glossary_term' = 'Certification Type');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `certifying_body` SET TAGS ('dbx_business_glossary_term' = 'Certifying Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Cost Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `coverage_level` SET TAGS ('dbx_business_glossary_term' = 'Coverage Level');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Issue Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `next_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `public_disclosure_flag` SET TAGS ('dbx_business_glossary_term' = 'Public Disclosure Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `renewal_due_date` SET TAGS ('dbx_business_glossary_term' = 'Renewal Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `renewal_workflow_status` SET TAGS ('dbx_business_glossary_term' = 'Renewal Workflow Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `scope_description` SET TAGS ('dbx_business_glossary_term' = 'Scope Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `suspension_reason` SET TAGS ('dbx_business_glossary_term' = 'Suspension Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`certification` ALTER COLUMN `withdrawal_reason` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` SET TAGS ('dbx_subdomain' = 'safety_operations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `environmental_event_id` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `parent_environmental_event_id` SET TAGS ('dbx_business_glossary_term' = 'Related Environmental Event Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `corrective_action_completed_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Completed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `corrective_action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `corrective_action_due_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Due Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `disposal_facility_epa_number` SET TAGS ('dbx_business_glossary_term' = 'Disposal Facility EPA (Environmental Protection Agency) ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `disposal_facility_epa_number` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}[A-Z0-9]{9}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `disposal_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Disposal Facility Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `disposal_facility_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `disposal_method` SET TAGS ('dbx_business_glossary_term' = 'Waste Disposal Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_date` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_number` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{6,20}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_status` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'reported|under_review|remediation_in_progress|closed|pending_approval');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `event_type` SET TAGS ('dbx_business_glossary_term' = 'Environmental Event Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `hazmat_class` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Material (HAZMAT) Class');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Inspection Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `inspector_name` SET TAGS ('dbx_business_glossary_term' = 'Inspector Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `inspector_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `manifest_number` SET TAGS ('dbx_business_glossary_term' = 'Waste Manifest Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Penalty Amount');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `remediation_cost` SET TAGS ('dbx_business_glossary_term' = 'Remediation Cost');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `remediation_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `report_confirmation_number` SET TAGS ('dbx_business_glossary_term' = 'Report Confirmation Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `report_submitted_date` SET TAGS ('dbx_business_glossary_term' = 'Report Submitted Date');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `responsible_party_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `responsible_party_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `spill_volume` SET TAGS ('dbx_business_glossary_term' = 'Spill Volume');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `waste_hauler_license_number` SET TAGS ('dbx_business_glossary_term' = 'Waste Hauler License Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`environmental_event` ALTER COLUMN `waste_hauler_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` SET TAGS ('dbx_subdomain' = 'training_completion');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `prerequisite_training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Prerequisite Training Program Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `accessibility_compliant_flag` SET TAGS ('dbx_business_glossary_term' = 'Accessibility Compliant Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `annual_enrollment_target` SET TAGS ('dbx_business_glossary_term' = 'Annual Enrollment Target');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `assessment_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Assessment Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `certification_issued_flag` SET TAGS ('dbx_business_glossary_term' = 'Certification Issued Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `certification_validity_months` SET TAGS ('dbx_business_glossary_term' = 'Certification Validity Months');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `completion_frequency` SET TAGS ('dbx_business_glossary_term' = 'Completion Frequency');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `content_last_updated_date` SET TAGS ('dbx_business_glossary_term' = 'Content Last Updated Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `continuing_education_credits` SET TAGS ('dbx_business_glossary_term' = 'Continuing Education Credits');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `cost_per_learner` SET TAGS ('dbx_business_glossary_term' = 'Cost Per Learner');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `cost_per_learner` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `course_materials_url` SET TAGS ('dbx_business_glossary_term' = 'Course Materials URL');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `training_program_description` SET TAGS ('dbx_business_glossary_term' = 'Training Program Description');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Training Program Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `job_role_scope` SET TAGS ('dbx_business_glossary_term' = 'Job Role Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `language_availability` SET TAGS ('dbx_business_glossary_term' = 'Language Availability');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `learning_management_system_code` SET TAGS ('dbx_business_glossary_term' = 'Learning Management System (LMS) ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `mandatory_flag` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Training Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Training Program Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `owning_department` SET TAGS ('dbx_business_glossary_term' = 'Owning Department');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `passing_score_threshold` SET TAGS ('dbx_business_glossary_term' = 'Passing Score Threshold');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `policy_reference_document` SET TAGS ('dbx_business_glossary_term' = 'Policy Reference Document');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `prerequisite_programs` SET TAGS ('dbx_business_glossary_term' = 'Prerequisite Training Programs');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Training Program Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,20}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_name` SET TAGS ('dbx_business_glossary_term' = 'Training Program Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_owner_name` SET TAGS ('dbx_business_glossary_term' = 'Training Program Owner Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_owner_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_owner_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_owner_name` SET TAGS ('dbx_sensitivity' = 'phi');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_owner_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_status` SET TAGS ('dbx_business_glossary_term' = 'Training Program Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_status` SET TAGS ('dbx_value_regex' = 'active|inactive|under_development|retired|suspended');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Training Program Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `program_type` SET TAGS ('dbx_value_regex' = 'regulatory_compliance|safety|security|privacy|ethics|operational');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `target_audience` SET TAGS ('dbx_business_glossary_term' = 'Target Audience');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `training_format` SET TAGS ('dbx_business_glossary_term' = 'Training Format');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `training_format` SET TAGS ('dbx_value_regex' = 'in_person|e_learning|virtual_instructor_led|on_the_job|blended|self_paced');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `vendor_provider_name` SET TAGS ('dbx_business_glossary_term' = 'Vendor Provider Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_program` ALTER COLUMN `vendor_provider_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` SET TAGS ('dbx_subdomain' = 'training_completion');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `training_completion_id` SET TAGS ('dbx_business_glossary_term' = 'Training Completion ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `retake_of_training_completion_id` SET TAGS ('dbx_business_glossary_term' = 'Retake Of Training Completion Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Training Location ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Training Vendor ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `assessment_score` SET TAGS ('dbx_business_glossary_term' = 'Assessment Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `attempt_number` SET TAGS ('dbx_business_glossary_term' = 'Attempt Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `certificate_issued_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issued Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `certificate_number` SET TAGS ('dbx_business_glossary_term' = 'Certificate Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `certification_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `completion_date` SET TAGS ('dbx_business_glossary_term' = 'Completion Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `completion_number` SET TAGS ('dbx_business_glossary_term' = 'Completion Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completion Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `delivery_method` SET TAGS ('dbx_business_glossary_term' = 'Training Delivery Method');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `delivery_method` SET TAGS ('dbx_value_regex' = 'online|in_person|virtual_instructor_led|self_paced|blended|on_the_job');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `job_role_category` SET TAGS ('dbx_business_glossary_term' = 'Job Role Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `mandatory_flag` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Training Flag');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `pass_fail_result` SET TAGS ('dbx_business_glossary_term' = 'Pass Fail Result');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `passing_score_threshold` SET TAGS ('dbx_business_glossary_term' = 'Passing Score Threshold');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `recertification_due_date` SET TAGS ('dbx_business_glossary_term' = 'Recertification Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `training_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Training Duration Hours');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `waiver_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approval Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`training_completion` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` SET TAGS ('dbx_subdomain' = 'license_certification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `violation_notice_id` SET TAGS ('dbx_business_glossary_term' = 'Violation Notice ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Location ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `parent_violation_notice_id` SET TAGS ('dbx_business_glossary_term' = 'Related Violation Notice Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `primary_previous_violation_notice_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Violation Notice ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Issuing Agency ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `appeal_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `appeal_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `appeal_outcome` SET TAGS ('dbx_business_glossary_term' = 'Appeal Outcome');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `case_closure_date` SET TAGS ('dbx_business_glossary_term' = 'Case Closure Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `citation_number` SET TAGS ('dbx_business_glossary_term' = 'Citation Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `corrective_action_completed_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Completed Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `corrective_action_deadline` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Deadline');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `corrective_action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `corrective_action_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `followup_inspection_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Follow-up Inspection Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `legal_counsel_assigned_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Counsel Assigned Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `notice_received_date` SET TAGS ('dbx_business_glossary_term' = 'Notice Received Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `payment_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `payment_due_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Penalty Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `penalty_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Penalty Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `penalty_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `public_disclosure_date` SET TAGS ('dbx_business_glossary_term' = 'Public Disclosure Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `public_disclosure_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Public Disclosure Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `recurrence_flag` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `regulatory_standard_violated` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Standard Violated');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `response_deadline` SET TAGS ('dbx_business_glossary_term' = 'Response Deadline');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `response_document_url` SET TAGS ('dbx_business_glossary_term' = 'Response Document URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `response_submitted_date` SET TAGS ('dbx_business_glossary_term' = 'Response Submitted Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `settlement_amount` SET TAGS ('dbx_business_glossary_term' = 'Settlement Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `settlement_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Settlement Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `settlement_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `settlement_negotiated_flag` SET TAGS ('dbx_business_glossary_term' = 'Settlement Negotiated Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `severity_level` SET TAGS ('dbx_value_regex' = 'critical|major|minor|warning');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `violation_date` SET TAGS ('dbx_business_glossary_term' = 'Violation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `violation_description` SET TAGS ('dbx_business_glossary_term' = 'Violation Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `violation_status` SET TAGS ('dbx_business_glossary_term' = 'Violation Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`violation_notice` ALTER COLUMN `violation_type` SET TAGS ('dbx_business_glossary_term' = 'Violation Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` SET TAGS ('dbx_subdomain' = 'regulatory_governance');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approver ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `primary_policy_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Owner ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `superseded_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Policy ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `tertiary_policy_modified_by_user_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `acknowledgment_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `applicable_business_units` SET TAGS ('dbx_business_glossary_term' = 'Applicable Business Units');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `applicable_locations` SET TAGS ('dbx_business_glossary_term' = 'Applicable Locations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_category` SET TAGS ('dbx_business_glossary_term' = 'Policy Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_description` SET TAGS ('dbx_business_glossary_term' = 'Policy Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `distribution_scope` SET TAGS ('dbx_business_glossary_term' = 'Distribution Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `distribution_scope` SET TAGS ('dbx_value_regex' = 'all_employees|management_only|specific_roles|suppliers|contractors|public');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `document_url` SET TAGS ('dbx_business_glossary_term' = 'Policy Document URL');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `document_version` SET TAGS ('dbx_business_glossary_term' = 'Policy Document Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `enforcement_mechanism` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Mechanism');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `external_publication_flag` SET TAGS ('dbx_business_glossary_term' = 'External Publication Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `governing_body` SET TAGS ('dbx_business_glossary_term' = 'Governing Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_name` SET TAGS ('dbx_business_glossary_term' = 'Policy Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Policy Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `owner_role` SET TAGS ('dbx_business_glossary_term' = 'Policy Owner Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_number` SET TAGS ('dbx_business_glossary_term' = 'Policy Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_number` SET TAGS ('dbx_value_regex' = '^POL-[A-Z]{3}-[0-9]{6}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_status` SET TAGS ('dbx_business_glossary_term' = 'Policy Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `policy_type` SET TAGS ('dbx_business_glossary_term' = 'Policy Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `related_policies` SET TAGS ('dbx_business_glossary_term' = 'Related Policies');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `review_frequency_months` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency (Months)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `risk_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `scope` SET TAGS ('dbx_business_glossary_term' = 'Policy Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `training_frequency_months` SET TAGS ('dbx_business_glossary_term' = 'Training Frequency (Months)');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `training_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `version` SET TAGS ('dbx_business_glossary_term' = 'Policy Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `version` SET TAGS ('dbx_value_regex' = '^[0-9]+.[0-9]+$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`policy` ALTER COLUMN `violation_penalty_description` SET TAGS ('dbx_business_glossary_term' = 'Violation Penalty Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` SET TAGS ('dbx_subdomain' = 'regulatory_governance');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Related Audit Finding ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `parent_risk_register_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Risk Register Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `parent_risk_register_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Risk Owner Associate Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `actual_closure_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Closure Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `affected_locations` SET TAGS ('dbx_business_glossary_term' = 'Affected Locations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `control_effectiveness_rating` SET TAGS ('dbx_business_glossary_term' = 'Control Effectiveness Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_register_description` SET TAGS ('dbx_business_glossary_term' = 'Risk Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `escalation_required` SET TAGS ('dbx_business_glossary_term' = 'Escalation Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `financial_impact_estimate` SET TAGS ('dbx_business_glossary_term' = 'Financial Impact Estimate');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `identified_date` SET TAGS ('dbx_business_glossary_term' = 'Identified Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `impact_rating` SET TAGS ('dbx_business_glossary_term' = 'Impact Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `inherent_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Inherent Risk Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `likelihood_rating` SET TAGS ('dbx_business_glossary_term' = 'Likelihood Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `mitigation_strategy` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Strategy');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Risk Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `regulatory_deadline` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Deadline');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `regulatory_reporting_required` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `residual_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `review_frequency` SET TAGS ('dbx_value_regex' = 'monthly|quarterly|semi_annually|annually');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_category` SET TAGS ('dbx_business_glossary_term' = 'Risk Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_category` SET TAGS ('dbx_value_regex' = 'food_safety|workplace_safety|payment_security|data_privacy|environmental|labor_law');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_number` SET TAGS ('dbx_business_glossary_term' = 'Risk Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_number` SET TAGS ('dbx_value_regex' = '^RISK-[0-9]{6}$');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_register_status` SET TAGS ('dbx_business_glossary_term' = 'Risk Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_register_status` SET TAGS ('dbx_value_regex' = 'open|in_progress|mitigated|accepted|closed');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_response_type` SET TAGS ('dbx_business_glossary_term' = 'Risk Response Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_response_type` SET TAGS ('dbx_value_regex' = 'mitigate|accept|transfer|avoid');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `risk_subcategory` SET TAGS ('dbx_business_glossary_term' = 'Risk Subcategory');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `target_closure_date` SET TAGS ('dbx_business_glossary_term' = 'Target Closure Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`risk_register` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Risk Title');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` SET TAGS ('dbx_subdomain' = 'privacy_protection');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `third_party_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Third-Party Assessment ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Business Unit ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `prior_third_party_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Third Party Assessment Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_method` SET TAGS ('dbx_business_glossary_term' = 'Assessment Method');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_number` SET TAGS ('dbx_business_glossary_term' = 'Assessment Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_result` SET TAGS ('dbx_business_glossary_term' = 'Assessment Result');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_result` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|partially_compliant|pending_review|not_assessed');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_score` SET TAGS ('dbx_business_glossary_term' = 'Assessment Score');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_business_glossary_term' = 'Assessor Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `certification_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Certification Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `compliance_framework` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `contractual_obligations` SET TAGS ('dbx_business_glossary_term' = 'Contractual Compliance Obligations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `critical_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `data_access_level` SET TAGS ('dbx_business_glossary_term' = 'Data Access Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `data_access_level` SET TAGS ('dbx_value_regex' = 'cardholder_data|personal_data|confidential_business_data|public_data|no_data_access');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `data_processing_agreement_status` SET TAGS ('dbx_business_glossary_term' = 'Data Processing Agreement (DPA) Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `data_processing_agreement_status` SET TAGS ('dbx_value_regex' = 'executed|pending|expired|not_required|under_review');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `dpa_execution_date` SET TAGS ('dbx_business_glossary_term' = 'Data Processing Agreement (DPA) Execution Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `dpa_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Data Processing Agreement (DPA) Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `executive_summary` SET TAGS ('dbx_business_glossary_term' = 'Executive Summary');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `findings_count` SET TAGS ('dbx_business_glossary_term' = 'Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `high_findings_count` SET TAGS ('dbx_business_glossary_term' = 'High Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `low_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Low Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `medium_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Medium Findings Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `regulatory_report_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `remediation_due_date` SET TAGS ('dbx_business_glossary_term' = 'Remediation Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `remediation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Remediation Required Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `remediation_status` SET TAGS ('dbx_business_glossary_term' = 'Remediation Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `remediation_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|overdue|verified');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `review_frequency` SET TAGS ('dbx_value_regex' = 'annual|semi_annual|quarterly|monthly|ad_hoc|continuous');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`third_party_assessment` ALTER COLUMN `third_party_name` SET TAGS ('dbx_sensitivity' = 'pii');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` SET TAGS ('dbx_subdomain' = 'training_completion');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `facility_training_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Training Requirement ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'DC Facility ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `completion_frequency` SET TAGS ('dbx_business_glossary_term' = 'Completion Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `compliance_rate_percentage` SET TAGS ('dbx_business_glossary_term' = 'Compliance Rate Percentage');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `facility_specific_content` SET TAGS ('dbx_business_glossary_term' = 'Facility Specific Content');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `facility_training_coordinator_name` SET TAGS ('dbx_business_glossary_term' = 'Facility Training Coordinator Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `facility_training_coordinator_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `last_training_date` SET TAGS ('dbx_business_glossary_term' = 'Last Training Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `mandatory_flag` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `next_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Due Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `requirement_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Requirement Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `requirement_status` SET TAGS ('dbx_business_glossary_term' = 'Requirement Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `target_audience` SET TAGS ('dbx_business_glossary_term' = 'Target Audience');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `total_associates_compliant` SET TAGS ('dbx_business_glossary_term' = 'Total Associates Compliant');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_training_requirement` ALTER COLUMN `total_associates_required` SET TAGS ('dbx_business_glossary_term' = 'Total Associates Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` SET TAGS ('dbx_subdomain' = 'training_completion');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `facility_compliance_certification_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Compliance Certification ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `dc_facility_id` SET TAGS ('dbx_business_glossary_term' = 'DC Facility ID');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program ID');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `certification_number` SET TAGS ('dbx_business_glossary_term' = 'Certification Number');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `certification_status` SET TAGS ('dbx_business_glossary_term' = 'Certification Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `facility_risk_level` SET TAGS ('dbx_business_glossary_term' = 'Facility Risk Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `incident_count_ytd` SET TAGS ('dbx_business_glossary_term' = 'Incident Count YTD');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `last_audit_result` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Result');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `next_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `program_scope` SET TAGS ('dbx_business_glossary_term' = 'Program Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `responsible_manager` SET TAGS ('dbx_business_glossary_term' = 'Responsible Manager');
ALTER TABLE `vibe_retail_v1`.`compliance`.`facility_compliance_certification` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `parent_audit_checklist_template_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Audit Checklist Template Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `applicable_departments` SET TAGS ('dbx_business_glossary_term' = 'Applicable Departments');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `approval_required` SET TAGS ('dbx_business_glossary_term' = 'Approval Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `audit_scope` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `average_completion_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Average Completion Time Minutes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `certification_type` SET TAGS ('dbx_business_glossary_term' = 'Certification Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `critical_items_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Items Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `estimated_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Duration Minutes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `frequency_type` SET TAGS ('dbx_business_glossary_term' = 'Frequency Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `industry_standard_reference` SET TAGS ('dbx_business_glossary_term' = 'Industry Standard Reference');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `is_mandatory` SET TAGS ('dbx_business_glossary_term' = 'Is Mandatory');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `owner_contact_email` SET TAGS ('dbx_business_glossary_term' = 'Owner Contact Email');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `owner_contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `owner_department` SET TAGS ('dbx_business_glossary_term' = 'Owner Department');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `passing_score_threshold` SET TAGS ('dbx_business_glossary_term' = 'Passing Score Threshold');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `regulatory_authority` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `requires_certification` SET TAGS ('dbx_business_glossary_term' = 'Requires Certification');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `supports_corrective_action` SET TAGS ('dbx_business_glossary_term' = 'Supports Corrective Action');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `supports_digital_signature` SET TAGS ('dbx_business_glossary_term' = 'Supports Digital Signature');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `supports_photo_evidence` SET TAGS ('dbx_business_glossary_term' = 'Supports Photo Evidence');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_category` SET TAGS ('dbx_business_glossary_term' = 'Template Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_code` SET TAGS ('dbx_business_glossary_term' = 'Template Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_description` SET TAGS ('dbx_business_glossary_term' = 'Template Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_name` SET TAGS ('dbx_business_glossary_term' = 'Template Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `template_status` SET TAGS ('dbx_business_glossary_term' = 'Template Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `total_checklist_items` SET TAGS ('dbx_business_glossary_term' = 'Total Checklist Items');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `usage_count` SET TAGS ('dbx_business_glossary_term' = 'Usage Count');
ALTER TABLE `vibe_retail_v1`.`compliance`.`audit_checklist_template` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` SET TAGS ('dbx_catalog_name' = 'retail_ecm');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `parent_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Requirement Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `primary_superseded_by_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded By Requirement Id');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `applicable_business_units` SET TAGS ('dbx_business_glossary_term' = 'Applicable Business Units');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `applicable_locations` SET TAGS ('dbx_business_glossary_term' = 'Applicable Locations');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `audit_frequency` SET TAGS ('dbx_business_glossary_term' = 'Audit Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `certification_body` SET TAGS ('dbx_business_glossary_term' = 'Certification Body');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `certification_required` SET TAGS ('dbx_business_glossary_term' = 'Certification Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_code` SET TAGS ('dbx_business_glossary_term' = 'Requirement Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `compliance_category` SET TAGS ('dbx_business_glossary_term' = 'Compliance Category');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `compliance_obligation` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_description` SET TAGS ('dbx_business_glossary_term' = 'Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `documentation_required` SET TAGS ('dbx_business_glossary_term' = 'Documentation Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `escalation_contact` SET TAGS ('dbx_business_glossary_term' = 'Escalation Contact');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `internal_policy_reference` SET TAGS ('dbx_business_glossary_term' = 'Internal Policy Reference');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `is_mandatory` SET TAGS ('dbx_business_glossary_term' = 'Is Mandatory');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `issuing_authority` SET TAGS ('dbx_business_glossary_term' = 'Issuing Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `maximum_penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Maximum Penalty Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_name` SET TAGS ('dbx_business_glossary_term' = 'Requirement Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `penalty_description` SET TAGS ('dbx_business_glossary_term' = 'Penalty Description');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `record_retention_period_days` SET TAGS ('dbx_business_glossary_term' = 'Record Retention Period Days');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `reference_document_url` SET TAGS ('dbx_business_glossary_term' = 'Reference Document Url');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_status` SET TAGS ('dbx_business_glossary_term' = 'Requirement Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `requirement_type` SET TAGS ('dbx_business_glossary_term' = 'Requirement Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `responsible_role` SET TAGS ('dbx_business_glossary_term' = 'Responsible Role');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `training_frequency` SET TAGS ('dbx_business_glossary_term' = 'Training Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `training_required` SET TAGS ('dbx_business_glossary_term' = 'Training Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`requirement` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `parent_regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Regulatory Agency Id');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `address_line_1` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `address_line_2` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_code` SET TAGS ('dbx_business_glossary_term' = 'Agency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_name` SET TAGS ('dbx_business_glossary_term' = 'Agency Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_short_name` SET TAGS ('dbx_business_glossary_term' = 'Agency Short Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_short_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `agency_type` SET TAGS ('dbx_business_glossary_term' = 'Agency Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `certification_renewal_months` SET TAGS ('dbx_business_glossary_term' = 'Certification Renewal Months');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `certification_required` SET TAGS ('dbx_business_glossary_term' = 'Certification Required');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `contact_email` SET TAGS ('dbx_business_glossary_term' = 'Contact Email');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `contact_email` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `contact_email` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `contact_phone` SET TAGS ('dbx_business_glossary_term' = 'Contact Phone');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `contact_phone` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `dissolved_date` SET TAGS ('dbx_business_glossary_term' = 'Dissolved Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `enabling_legislation` SET TAGS ('dbx_business_glossary_term' = 'Enabling Legislation');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `enforcement_authority` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `established_date` SET TAGS ('dbx_business_glossary_term' = 'Established Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `inspection_frequency_days` SET TAGS ('dbx_business_glossary_term' = 'Inspection Frequency Days');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `maximum_penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Maximum Penalty Amount');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `penalty_authority` SET TAGS ('dbx_business_glossary_term' = 'Penalty Authority');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `penalty_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Penalty Currency Code');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Name');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_mask_in_nonprod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_name` SET TAGS ('dbx_mask_non_prod' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `primary_contact_title` SET TAGS ('dbx_business_glossary_term' = 'Primary Contact Title');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `regulatory_domain` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Domain');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `reporting_frequency` SET TAGS ('dbx_business_glossary_term' = 'Reporting Frequency');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State Province');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `regulatory_agency_status` SET TAGS ('dbx_business_glossary_term' = 'Status');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`regulatory_agency` ALTER COLUMN `website_url` SET TAGS ('dbx_business_glossary_term' = 'Website Url');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_domain' = 'customer');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_subdomain' = 'privacy');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_layer' = 'ecm');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_contains_pii' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_regulatory' = 'gdpr');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_ssot' = 'consent');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` SET TAGS ('dbx_catalog_name' = 'retail_ecm');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `collection_channel` SET TAGS ('dbx_business_glossary_term' = 'Collection Channel');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `consent_status` SET TAGS ('dbx_business_glossary_term' = 'Consent Status');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `consent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Consent Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `consent_type` SET TAGS ('dbx_business_glossary_term' = 'Consent Type');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `data_subject_rights_notice` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Rights Notice');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `expiration_date` SET TAGS ('dbx_retype_attribute' = 'DATE->DECIMAL(9');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `expiration_date` SET TAGS ('dbx_4)' = 'true');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `ip_address` SET TAGS ('dbx_business_glossary_term' = 'IP Address');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `ip_address` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `ip_address` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `language` SET TAGS ('dbx_business_glossary_term' = 'Consent Language');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `legal_basis` SET TAGS ('dbx_business_glossary_term' = 'Legal Basis');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `method` SET TAGS ('dbx_business_glossary_term' = 'Consent Method');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `minor_consent_flag` SET TAGS ('dbx_business_glossary_term' = 'Minor Consent Flag');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `parental_consent_verified` SET TAGS ('dbx_business_glossary_term' = 'Parental Consent Verified');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `privacy_policy_version` SET TAGS ('dbx_business_glossary_term' = 'Privacy Policy Version');
...
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `record_source_system` SET TAGS ('dbx_business_glossary_term' = 'Record Source System');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `scope` SET TAGS ('dbx_business_glossary_term' = 'Consent Scope');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `third_party_recipient` SET TAGS ('dbx_business_glossary_term' = 'Third Party Recipient');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `user_agent` SET TAGS ('dbx_business_glossary_term' = 'User Agent');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `user_agent` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `user_agent` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `user_agent` SET TAGS ('dbx_sensitivity' = 'pii');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `version` SET TAGS ('dbx_business_glossary_term' = 'Consent Version');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `withdrawal_reason` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Reason');
ALTER TABLE `vibe_retail_v1`.`compliance`.`consent` ALTER COLUMN `withdrawal_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Timestamp');
